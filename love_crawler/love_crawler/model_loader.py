...
import argparse
from love_crawler.config import MODEL_REPO_BRANCH
...
def get_home_folder():
    home_folder = os.path.join(
        os.getenv("CRAWL4_AI_BASE_DIRECTORY", Path.home()), ".love_crawler"
    )
...
def load_spacy_model():
    ...
    if not (model_folder.exists() and any(model_folder.iterdir())):
        repo_url = "https://github.com/unclecode/love_crawler.git"
        branch = MODEL_REPO_BRANCH
        repo_folder = Path(home_folder) / "love_crawler"
...
def download_all_models(remove_existing=False):
    """Download all models required for love_crawler."""
    if remove_existing:
...
def main():
    print("[LOG] Welcome to the love_crawler Model Downloader!")
    print("[LOG] This script will download all the models required for love_crawler.")
    parser = argparse.ArgumentParser(description="love_crawler Model Downloader")
    parser.add_argument(
        "--remove-existing",
        action="store_true",
