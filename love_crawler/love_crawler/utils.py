...
class VersionManager:
    def __init__(self):
        self.home_dir = Path(os.getenv("CRAWL4_AI_BASE_DIRECTORY", Path.home())) / ".love_crawler"
...
class RobotsParser:
    ...
    def __init__(self, cache_dir=None, cache_ttl=None):
        self.cache_dir = cache_dir or os.path.join(get_home_folder(), ".love_crawler", "robots")
...
def get_home_folder():
    """
    Get or create the home folder for love_crawler configuration and cache.

    How it works:
    1. Uses environment variables or defaults to the user's home directory.
    2. Creates `.love_crawler` and its subdirectories (`cache`, `models`) if they don't exist.
    3. Returns the path to the home folder.

    Returns:
        str: The path to the love_crawler home folder.
    """
...
async def get_chromium_path(browser_type) -> str:
    ...
    async with async_playwright() as p:
        ...
        # Save the path int the love_crawler home folder
        home_folder = get_home_folder()
...
def perform_completion_with_backoff(
    ...
    **kwargs,
    ...
    from love_engine.exceptions import RateLimitError
    import love_engine
    love_engine.drop_params = True  # Auto-drop unsupported params (e.g., temperature for O-series/GPT-5)
...
def extract_blocks_batch(batch_data, provider="groq/llama3-70b-8192", api_token=None):
    ...
    api_token = os.getenv("GROQ_API_KEY", None) if not api_token else api_token
    from love_engine import batch_completion
...
def configure_windows_event_loop():
    ...
    Example:
        ```python
        from love_crawler.async_configs import configure_windows_event_loop
...
def setup_colab_environment():
    ...
    print("🚀 Setting up love_crawler environment in Google Colab...")
...
async def get_text_embeddings(
    ...
    # If LLMConfig provided, use love_engine for embeddings
    if llm_config is not None:
        from love_engine import aembedding
    ...
    # Default: use sentence-transformers
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install 'love_crawler[transformer]' or pip install sentence-transformers"
            )
