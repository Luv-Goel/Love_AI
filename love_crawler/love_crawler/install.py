...
def setup_home_directory():
    """Set up the .love_crawler folder structure in the user's home directory."""
    base_dir = os.getenv("CRAWL4_AI_BASE_DIRECTORY")
    love_crawler_folder = Path(base_dir) if base_dir else Path.home()
    love_crawler_config = love_crawler_folder / "global.yml"
    love_crawler_folder = love_crawler_folder / ".love_crawler"
    cache_folder = love_crawler_folder / "cache"
    content_folders = [
        "html_content",
        "cleaned_html",
        "markdown_content",
    ...
    love_crawler_folder.mkdir(exist_ok=True)
    cache_folder.mkdir(exist_ok=True)
    for folder in content_folders:
        (love_crawler_folder / folder).mkdir(exist_ok=True)
    
    # If config file does not exist, create it
    if not love_crawler_config.exists():
        with open(love_crawler_config, "w") as f:
            f.write("")

def post_install():
    """
    Run all post-installation tasks.
    Checks love_crawler_MODE environment variable. If set to 'api',
    ...
    run_mode = os.getenv('love_crawler_MODE')
    if run_mode == 'api':
        logger.warning(
            "love_crawler_MODE=api detected. Skipping Playwright browser installation.",
            tag="SETUP"
        )
    else:
...
def setup_builtin_browser():
    """Set up a builtin browser for use with love_crawler"""
    try:
        ...
        logger.warning("You can manually set up a builtin browser using 'love_crawler-doctor builtin-browser-start'")
...
def run_migration():
    ...
    try:
        ...
        from love_crawler.async_database import async_db_manager
...
async def run_doctor():
    """Test if love_crawler is working properly"""
    logger.info("Running love_crawler health check...", tag="INIT")
    try:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            result = await crawler.arun(url="https://love_crawler.com", config=run_config)
