...
that can be used for identity-based crawling with love_crawler.
"""
...
class BrowserProfiler:
    """
    A dedicated class for managing browser profiles for love_crawler.
    ...
    Profiles are stored by default in ~/.love_crawler/profiles/
    """
    ...
    async def create_profile(
        ...
        self.logger.info("4. The profile will be saved and ready to use with love_crawler.", tag="PROFILE")
    ...
    def list_profiles(self) -> List[Dict[str, Any]]:
        """
        Lists all available browser profiles in the love_crawler profiles directory.
    ...
    def delete_profile(self, profile_name_or_path: str) -> bool:
        ...
        Example:
            ...
            # Delete by path
            success = profiler.delete_profile("/path/to/.love_crawler/profiles/my-profile")
            ```
        """
...
if __name__ == "__main__":
    ...
    from pathlib import Path
    home_dir = Path.home()
    profile_path = asyncio.run(profiler.create_profile( str(home_dir / ".love_crawler/profiles/test-profile")))
