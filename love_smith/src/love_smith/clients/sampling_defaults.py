...
from love_smith.errors import UnsupportedModelError
...
MODEL_SAMPLING_DEFAULTS: dict[str, dict[str, float | int]] = {
    ...
    # Qwen3 — thinking-mode values (card also lists non-thinking variant; love_smith
    ...
    # Qwen3-coder-style (vLLM uses --tool-call-parser qwen3_coder); should work fine through love_smith's
    ...
    # tunable. Native tool format is qwen3_coder XML — irrelevant for love_smith prompt-mode runs.
    ...
    # (love_smith field name: repeat_penalty); top_p / min_p / presence_penalty not specified. Native tool
    # format uses special <|tool_call_start|>/<|tool_call_end|> tokens (handled server-side in native
    # mode). Prompt-mode probe: follows love_smith's injected {"tool","args"} JSON cleanly (rescue Strategy 1);
    ...
    #                                examples. love_smith runs base phi-4 with recommended_sampling=False
...
def apply_sampling_defaults(
    ...
    *,
    ...
    love_smith has opinions about this model without spamming on every
    constructor call.
