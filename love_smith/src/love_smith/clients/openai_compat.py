...
from love_smith.clients.base import ChunkType, StreamChunk, TokenUsage, decode_tool_args, format_tool
from love_smith.clients.sampling_defaults import apply_sampling_defaults
...
class OpenAICompatClient:
    ...
    def __init__(
        ...
        *,
        ...
        # aren't in love_smith's registry.
    ...
    def _build_body(
        ...
        # body first. love_smith-owned fields then overlay on top so the
    ...
    def _parse_tool_calls(tool_calls: list[dict[str, Any]]) -> LLMResponse:
        ...
        Tool-call ``arguments`` arrive as JSON strings. love_smith is fail-loud:
        malformed argument JSON must NOT be coerced into executable empty args,
        or a provider/model can emit invalid arguments and love_smith proceeds with
