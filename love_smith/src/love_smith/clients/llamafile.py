...
from love_smith.errors import BackendError, ContextDiscoveryError
from love_smith.prompts.templates import build_tool_prompt, extract_tool_call
...
class LlamafileClient:
    ...
    async def send(
        ...
        accepts and ignores it (it keeps love_smith's prompt-injection format).
        """
        if self.mode == "native":
    ...
    async def _send_native(
        ...
        original schema instead of love_smith's re-emitted ``format_tool(spec)``.
        """
