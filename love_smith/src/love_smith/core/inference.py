...
from love_smith.clients.base import (
    ChunkType,
    LLMClient,
    RawOpenAIMessages,
    RawOpenAITools,
...
from love_smith.core.workflow import LLMResponse, TextResponse, ToolCall, ToolSpec
from love_smith.errors import StreamError, ToolCallError
from love_smith.guardrails import ErrorTracker, ResponseValidator
from love_smith.guardrails.nudge import TOOL_ERROR_KINDS
...
def fold_and_serialize(
    ...
    """Reasoning-fold and serialize love_smith Messages to API dicts.
...
def prepare_backend_messages(
    ...
    """Prepare backend-facing messages from raw OpenAI or love_smith history.

    This is the single backend replay-policy choke point. Raw OpenAI messages
    preserve client-authored shape while filtering only reasoning fields; love_smith
...
async def run_inference(
    ...
    Args:
        messages: The current conversation history (love_smith Messages). This
    ...
    # Path-1 verbatim opt-in: drop on any love_smith mutation (compaction,
    ...
    for _attempt in range(attempt_limit):
        ...
        # instead of love_smith's parsed/re-emitted form. Any love_smith mutation
        ...
        # Forward raw tools only on the clean first attempt — on retries love_smith
