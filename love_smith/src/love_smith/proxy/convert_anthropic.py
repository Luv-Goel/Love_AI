"""Convert between Anthropic Messages API format and love_smith Messages.
...
from love_smith.core.workflow import ToolCall, ToolSpec


# ── Inbound: Anthropic request → love_smith Messages ──────────────────

def anthropic_to_messages(
    anthropic_messages: list[dict[str, Any]],
    system: str | list[dict[str, Any]] | None = None,
) -> list[Message]:
    """Convert Anthropic messages + system to love_smith Message objects.

    The Anthropic protocol carries ``system`` as a top-level field (string
    or list of content blocks); love_smith represents it as a SYSTEM-role message
    ...
    ``document`` / ``image`` → dropped (no love_smith analog today).
    """
    ...
    for msg in anthropic_messages:
        ...
        if role == "assistant" and tool_calls:
            ...
            # Anthropic packs tool_results inside user messages; love_smith splits
...
def anthropic_tools_to_specs(tools: list[dict[str, Any]] | None) -> list[ToolSpec]:
    """Anthropic tools array → love_smith ToolSpec list.
...
def anthropic_to_openai_passthrough(body: dict[str, Any]) -> dict[str, Any]:
    ...
    tool_choice/stop_sequences shapes. Excludes fields love_smith owns
    (``messages``, ``tools``, ``system``, ``stream``) — those flow through
    love_smith's normal parsing. Excludes Anthropic-only fields without an
...
# ── Outbound: love_smith response → Anthropic response ────────────────

def _anthropic_usage(usage: Any | None) -> dict[str, int]:
    """Map a love_smith TokenUsage to Anthropic's input/output token shape."""
    if usage is None:
...
def tool_calls_to_anthropic(
    ...
    model: str = "love_smith",
    usage: Any | None = None,
    reasoning_replay: ReasoningReplay = DEFAULT_REASONING_REPLAY,
) -> dict[str, Any]:
    """Convert love_smith ToolCalls to an Anthropic Messages API response object."""
