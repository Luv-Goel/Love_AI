...
from love_smith.clients.base import LLMClient, format_tool
from love_smith.context.manager import ContextManager
from love_smith.core.inference import _get_usage, prepare_backend_messages, run_inference
from love_smith.core.reasoning import (
    DEFAULT_REASONING_REPLAY,
    ReasoningReplay,
    validate_reasoning_replay,
)
from love_smith.core.workflow import ToolCall, ToolSpec, TextResponse
from love_smith.errors import ToolCallError
from love_smith.guardrails import ErrorTracker, ResponseValidator
from love_smith.proxy.convert import (
    openai_to_messages,
...
from love_smith.proxy.convert_anthropic import (
...
from love_smith.tools.respond import RESPOND_TOOL_NAME, respond_spec

logger = logging.getLogger("love_smith.proxy")
...
# Body fields love_smith owns and reasons about — never go into passthrough.
_love_smith_OWNED = frozenset({"messages", "tools", "stream", "stream_options", "system"})
...
def _extract_passthrough(body: dict[str, Any]) -> dict[str, Any] | None:
    """Pull non-love_smith-owned, non-sampling fields for the passthrough channel.
    ...
    ``tool_choice``, ``model``, etc. without love_smith needing to enumerate
    every supported field.
    """
    extras = {
        k: v for k, v in body.items()
        if k not in _love_smith_OWNED and k not in _SAMPLING_FIELDS
    }
...
async def handle_chat_completions(
    ...
    Converts inbound messages to love_smith Messages, runs inference with
    guardrails, and converts the result back to the inbound protocol's
    shape.

    Args:
        body: Parsed JSON request body.
        client: The love_smith LLM client for the backend.
        ...
        inject_respond_tool: When True and the client request supplies tools,
            inject love_smith's synthetic respond() tool so the model stays in
    ...
    model_name = body.get("model", "love_smith")
    ...
    if protocol == "anthropic":
        ...
        # lossy ToolSpec round-trip. tool_specs stays as love_smith's validation
