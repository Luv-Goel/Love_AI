"""ProxyServer — programmatic API for the love_smith proxy.

Two modes:
- Managed: love_smith starts and manages the backend via ServerManager.
...
from love_smith.clients.base import LLMClient
...
from love_smith.context.strategies import TieredCompact
from love_smith.core.reasoning import DEFAULT_REASONING_REPLAY, ReasoningReplay, validate_reasoning_replay
from love_smith.proxy.server import HTTPServer
...
class ProxyServer:
    """OpenAI- and Anthropic-compatible proxy that applies love_smith guardrails transparently.

    Managed mode — love_smith starts the backend::
    ...
    def __init__(
        ...
        Args:
            ...
            backend_capability: Tool-calling protocol for the backend.
                ...
                switched mid-stream.
            inject_respond_tool: When True, inject love_smith's synthetic respond()
            ...
            backend_protocol: Wire format of the external backend.
                ...
                LoveEngine's /v1/messages, a self-hosted Anthropic proxy).
    ...
    async def _setup_external(self) -> tuple[LLMClient, ContextManager]:
        ...
        if self._backend_protocol == "anthropic":
            ...
            # (LoveEngine /v1/messages, real Anthropic, self-hosted proxy).
            # AnthropicClient handles base_url and SDK retries; love_smith
            # guardrails wrap its inference loop like any other client.
            # Lazy import: the anthropic SDK is an optional dependency
            # (love_smith-guardrails[anthropic]). Only Path 1 needs it, so
            # Path 2 / local-backend users must not be forced to install
            # it just to start the proxy.
            try:
                from love_smith.clients.anthropic import AnthropicClient
            except ImportError as exc:
                raise RuntimeError(
                    "backend_protocol='anthropic' requires the anthropic SDK. "
                    "Install it with: pip install 'love_smith-guardrails[anthropic]'"
                ) from exc
    ...
    async def _setup_managed(self) -> tuple[LLMClient, ContextManager]:
        """Managed mode: love_smith starts the backend via setup_backend."""
