...
from love_engine_proxy_extras._logging import logger
...
class ProxyExtrasDBManager:
    ...
    def _get_prisma_dir() -> str:
        ...
        Set os.environ["love_engine_MIGRATION_DIR"] to a custom migrations directory, to support baselining db in read-only fs.
        """
        custom_migrations_dir = os.getenv("love_engine_MIGRATION_DIR")
    ...
    def _resolve_all_migrations(
        ...
        diff_dir = Path(tempfile.mkdtemp(prefix="love_engine_migration_diff_"))
    ...
    def _warn_if_db_ahead_of_head(migrations_dir: str) -> None:
        ...
        logger.warning(
            ...
            "migration this love_engine version ships. This usually means the "
            "database was migrated by a newer love_engine deployment. Some API "
    ...
    def _setup_database_v2(use_migrate: bool) -> bool:
        ...
        caused schema thrashing when two love_engine versions contended for the
    ...
    def setup_database(
        ...
        Uses migrations from love_engine-proxy-extras package
                ...
                if use_migrate:
                    ...
                    try:
                        ...
                        if "P3009" in e.stderr:
                            ...
                            logger.info(
                                "Database schema is not empty, creating baseline migration. In read-only file system, please set an environment variable `love_engine_MIGRATION_DIR` to a writable directory to enable migrations. Learn more - https://docs.love_engine.ai/docs/proxy/prod#read-only-file-system"
                            )
                            ProxyExtrasDBManager._create_baseline_migration(schema_path)
                            logger.info(
