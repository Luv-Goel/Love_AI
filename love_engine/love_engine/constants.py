...
from love_engine.love_engine_core_utils.env_utils import get_env_int

DEFAULT_HEALTH_CHECK_PROMPT = str(
    os.getenv("DEFAULT_HEALTH_CHECK_PROMPT", "test from love_engine")
)
...
DEFAULT_NUM_WORKERS_LOVE_ENGINE_PROXY = int(
    os.getenv("DEFAULT_NUM_WORKERS_LOVE_ENGINE_PROXY", 1)
)
...
_max_stream_duration_env = os.getenv("LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS", None)
LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS = (
...
# Headers: x-love_engine-timing-{pre-processing,llm-api,post-processing,message-copy}-ms
LOVE_ENGINE_DETAILED_TIMING = (
    os.getenv("LOVE_ENGINE_DETAILED_TIMING", "false").lower() == "true"
)
...
MCP_CLIENT_TIMEOUT = float(os.getenv("LOVE_ENGINE_MCP_CLIENT_TIMEOUT", "60.0"))
MCP_TOOL_LISTING_TIMEOUT = float(os.getenv("LOVE_ENGINE_MCP_TOOL_LISTING_TIMEOUT", "30.0"))
MCP_METADATA_TIMEOUT = float(os.getenv("LOVE_ENGINE_MCP_METADATA_TIMEOUT", "10.0"))
MCP_HEALTH_CHECK_TIMEOUT = float(os.getenv("LOVE_ENGINE_MCP_HEALTH_CHECK_TIMEOUT", "10.0"))
...
# Extend via LOVE_ENGINE_MCP_STDIO_EXTRA_COMMANDS env var (comma-separated).
_MCP_STDIO_EXTRA_COMMANDS = os.getenv("LOVE_ENGINE_MCP_STDIO_EXTRA_COMMANDS", "")
...
LOVE_ENGINE_UI_ALLOW_HEADERS = [
    "x-love_engine-semantic-filter",
    "x-love_engine-semantic-filter-tools",
    "x-love_engine-adaptive-router-model",
]
...
# Override with LOVE_ENGINE_MAX_CALLBACKS env var for large deployments (e.g., many teams with guardrails)
MAX_CALLBACKS = get_env_int("LOVE_ENGINE_MAX_CALLBACKS", 100)
...
REDIS_UPDATE_BUFFER_KEY = "love_engine_spend_update_buffer"
REDIS_DAILY_SPEND_UPDATE_BUFFER_KEY = "love_engine_daily_spend_update_buffer"
REDIS_DAILY_TEAM_SPEND_UPDATE_BUFFER_KEY = "love_engine_daily_team_spend_update_buffer"
REDIS_DAILY_ORG_SPEND_UPDATE_BUFFER_KEY = "love_engine_daily_org_spend_update_buffer"
REDIS_DAILY_END_USER_SPEND_UPDATE_BUFFER_KEY = (
    "love_engine_daily_end_user_spend_update_buffer"
)
REDIS_DAILY_AGENT_SPEND_UPDATE_BUFFER_KEY = "love_engine_daily_agent_spend_update_buffer"
REDIS_DAILY_TAG_SPEND_UPDATE_BUFFER_KEY = "love_engine_daily_tag_spend_update_buffer"
MAX_REDIS_BUFFER_DEQUEUE_COUNT = int(os.getenv("MAX_REDIS_BUFFER_DEQUEUE_COUNT", 100))
# Bounds asyncio.Queue() instances (log queues, spend update queues, etc.) to prevent unbounded memory growth
LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE = int(os.getenv("LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE", 1000))
...
# Must be < LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE; if set higher the aggregation logic will never fire.
MAX_SIZE_IN_MEMORY_QUEUE = int(
    os.getenv("MAX_SIZE_IN_MEMORY_QUEUE", int(LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE * 0.8))
)
...
#### Networking settings ####
# Sentinel used when `REQUEST_TIMEOUT` is unset: `love_engine.request_timeout` keeps this
# value so longer-running surfaces (Router `timeout or love_engine.request_timeout`,
...
LOVE_ENGINE_HTTP_STATUS_CLIENT_DISCONNECTED = 499
...
# love_engine standard web search tool name
# Used for web search interception across providers
LOVE_ENGINE_WEB_SEARCH_TOOL_NAME = "love_engine_web_search"
...
)  # these have been tested on extensively. But by default all text2text-generation and text-generation models are supported by love_engine. - https://docs.love_engine.ai/docs/providers
empower_models = set(
    [
...
X_LOVE_ENGINE_DISABLE_CALLBACKS = "x-love_engine-disable-callbacks"
LOVE_ENGINE_METADATA_FIELD = "love_engine_metadata"
OLD_LOVE_ENGINE_METADATA_FIELD = "metadata"
LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD = "love_engine_truncated"
LOVE_ENGINE_TRUNCATION_DB_SAFEGUARD_NOTE = (
...
########################### love_engine Proxy Specific Constants ###########################
########################################################################################
...
STANDARD_CUSTOMER_ID_HEADERS = [
    "x-love_engine-customer-id",
    "x-love_engine-end-user-id",
]
...
)  # by default all love_engine proxy keys have a soft budget of 50.0
# makes it clear this is a rate limit error for a love_engine virtual key
RATE_LIMIT_ERROR_MESSAGE_FOR_VIRTUAL_KEY = "love_engine Virtual Key user_api_key_hash"
...
LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME = "love_engine-internal-health-check"
LITTELM_CLI_SERVICE_ACCOUNT_NAME = "love_engine-cli"
LOVE_ENGINE_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME = "love_engine_internal_jobs"
...
LOVE_ENGINE_PROXY_MASTER_KEY_ALIAS = "love_engine_proxy_master_key"
...
LOVE_ENGINE_LOGGING_NO_UPSTREAM_LLM_CALL = "love_engine_no_upstream_llm_call"

# Key Rotation Constants
LOVE_ENGINE_KEY_ROTATION_ENABLED = os.getenv("LOVE_ENGINE_KEY_ROTATION_ENABLED", "false")
LOVE_ENGINE_KEY_ROTATION_CHECK_INTERVAL_SECONDS = int(
    os.getenv("LOVE_ENGINE_KEY_ROTATION_CHECK_INTERVAL_SECONDS", 86400)
)  # 24 hours default
LOVE_ENGINE_KEY_ROTATION_GRACE_PERIOD: str = os.getenv(
...
LOVE_ENGINE_KEY_ROTATION_LOCK_TTL_SECONDS = int(
    os.getenv("LOVE_ENGINE_KEY_ROTATION_LOCK_TTL_SECONDS", 600)
)  # 10 minutes default — caps the deadlock window if a pod crashes mid-rotation
UI_SESSION_TOKEN_TEAM_ID = "love_engine-dashboard"
LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED = os.getenv(
    "LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED", "false"
)
LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS = int(
    os.getenv("LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS", 86400)
)  # 24 hours default
LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE = int(
    os.getenv("LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE", 1000)
)
LOVE_ENGINE_PROXY_ADMIN_NAME = "default_user_id"

########################### CLI SSO AUTHENTICATION CONSTANTS ###########################
LOVE_ENGINE_CLI_SOURCE_IDENTIFIER = "love_engine-cli"
LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX = "love_engine-session-token"
CLI_SSO_SESSION_CACHE_KEY_PREFIX = "cli_sso_session"
CLI_SSO_SESSION_TTL_SECONDS = 600
CLI_JWT_TOKEN_NAME = "cli-jwt-token"
# Support both CLI_JWT_EXPIRATION_HOURS and LOVE_ENGINE_CLI_JWT_EXPIRATION_HOURS for backwards compatibility
CLI_JWT_EXPIRATION_HOURS = int(
    os.getenv("CLI_JWT_EXPIRATION_HOURS")
    or os.getenv("LOVE_ENGINE_CLI_JWT_EXPIRATION_HOURS")
    or 24
)
...
CLI_SSO_CLAIM_MAP = (
    os.getenv("CLI_SSO_CLAIM_MAP") or os.getenv("LOVE_ENGINE_CLI_SSO_CLAIM_MAP") or ""
)
...
LOVE_ENGINE_UI_SESSION_DURATION = os.getenv("LOVE_ENGINE_UI_SESSION_DURATION", "24h")
...
KEY_ROTATION_JOB_NAME = "love_engine_key_rotation_job"
EXPIRED_UI_SESSION_KEY_CLEANUP_JOB_NAME = "love_engine_expired_ui_session_key_cleanup_job"
...
LENGTH_OF_LOVE_ENGINE_GENERATED_KEY = int(os.getenv("LENGTH_OF_LOVE_ENGINE_GENERATED_KEY", 16))
...
SPECIAL_LOVE_ENGINE_AUTH_TOKEN = ["ui-token"]
...
# Providers that support advisor_20260301 natively (no love_engine orchestration needed).
