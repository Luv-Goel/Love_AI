...
from love_engine.caching.redis_cache import RedisPipelineIncrementOperation
from love_engine.integrations.custom_logger import CustomLogger, Span
from love_engine.love_engine_core_utils.duration_parser import duration_in_seconds
from love_engine.router_strategy.tag_based_routing import _get_tags_from_request_kwargs
from love_engine.love_engine_core_utils.core_helpers import (
    get_metadata_variable_name_from_kwargs,
)
from love_engine.router_utils.cooldown_callbacks import (
    _get_prometheus_logger_from_callbacks,
)
from love_engine.types.llms.openai import AllMessageValues
from love_engine.types.router import DeploymentTypedDict, LOVE_ENGINE_Params, RouterErrors
from love_engine.types.utils import BudgetConfig
from love_engine.types.utils import BudgetConfig as GenericBudgetInfo
from love_engine.types.utils import GenericBudgetConfigType, StandardLoggingPayload

DEFAULT_REDIS_SYNC_INTERVAL = 1


class _LoveEngineParamsDictView:
    """
    Lightweight attribute view over `love_engine_params` dict.

    This avoids pydantic construction in request hot-path while preserving
    attribute-style access used by `love_engine.get_llm_provider(...)`.
    """
...
class RouterBudgetLimiting(CustomLogger):
    def __init__(
        ...
        # Add self to love_engine callbacks if it's a list
        if isinstance(love_engine.callbacks, list):
            love_engine.logging_callback_manager.add_love_engine_callback(self)  # type: ignore
    ...
    def _filter_out_deployments_above_budget(
        ...
        for idx, deployment in enumerate(healthy_deployments):
            ...
            if self.deployment_budget_config and is_within_budget:
                ...
                _love_engine_params = deployment.get("love_engine_params") or {}
                ...
                if model_id in deployment_configs:
                    ...
                    if config.max_budget and current_spend >= config.max_budget:
                        debug_msg = f"Exceeded budget for deployment model_name: {_model_name}, love_engine_params.model: {_love_engine_model_name}, model_id: {model_id}: {current_spend} >= {config.budget_duration}"
    ...
    async def _increment_spend_in_current_window(
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of LoveEngine)
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        custom_llm_provider: str = kwargs.get("love_engine_params", {}).get(
    ...
    def _get_llm_provider_for_deployment(self, deployment: Dict) -> Optional[str]:
        try:
            deployment_love_engine_params = deployment.get("love_engine_params") or {}

            if isinstance(deployment_love_engine_params, LOVE_ENGINE_Params):
                model = deployment_love_engine_params.model or ""
                provider_resolution_params: Any = deployment_love_engine_params
            elif isinstance(deployment_love_engine_params, dict):
                model = deployment_love_engine_params.get("model") or ""
                provider_resolution_params = _LoveEngineParamsDictView(
                    deployment_love_engine_params
                )
            else:
                model = ""
                provider_resolution_params = _LoveEngineParamsDictView({})
            ...
            _, custom_llm_provider, _, _ = love_engine.get_llm_provider(
                model=str(model),
                love_engine_params=provider_resolution_params,
            )
        except Exception:
            verbose_router_logger.error(
    ...
    def should_init_router_budget_limiter(
        ...
        if love_engine.tag_budget_config is not None:
            return True
        ...
        for _model in model_list:
            _love_engine_params = _model.get("love_engine_params", {})
            if (
                _love_engine_params.get("max_budget")
                or _love_engine_params.get("budget_duration") is not None
            ):
                return True
        return False
    ...
    def _init_deployment_budgets(
        ...
        for _model in model_list:
            ...
            _max_budget = _love_engine_params.get("max_budget")
            _budget_duration = _love_engine_params.get("budget_duration")
    ...
    def _init_tag_budgets(self):
        if love_engine.tag_budget_config is None:
            return
        ...
        for _tag, _tag_budget_config in love_engine.tag_budget_config.items():
