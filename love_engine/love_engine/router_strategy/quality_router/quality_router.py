...
`model_info.love_engine_routing_preferences`.

Optional keyword override: deployments may also declare `keywords` in
`love_engine_routing_preferences`. If any declared keyword appears in the user
...
class QualityRouter(CustomLogger):
    ...
    def _get_routing_preferences(self, deployment: Any) -> Optional[Dict[str, Any]]:
        """
        Extract love_engine_routing_preferences from a deployment, handling both
        ...
        if isinstance(deployment, dict):
            ...
            if isinstance(model_info, dict):
                return model_info.get("love_engine_routing_preferences")
            # Pydantic ModelInfo nested in a dict.
            return getattr(model_info, "love_engine_routing_preferences", None)
    ...
    def _build_tier_index(self) -> Dict[int, List[str]]:
        ...
        missing `love_engine_routing_preferences`.
        """
        model_list = getattr(self.love_engine_router_instance, "model_list", None) or []
        ...
        for deployment in model_list:
            ...
            if raw_prefs is None:
                raise ValueError(
                    ...
                    f"but has no model_info.love_engine_routing_preferences"
                )
            ...
            try:
                ...
                raise ValueError(
                    ...
                    f"love_engine_routing_preferences: {e}"
                ) from e
    ...
    def _stash_decision(
        ...
        lift it into response headers (`x-love_engine-quality-router-*`). The same
