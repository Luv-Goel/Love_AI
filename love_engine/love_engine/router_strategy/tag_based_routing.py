...
def _match_deployment(
    ...
    deployment_tags: Optional[List[str]] = love_engine_params.get("tags")
    deployment_tag_regex: Optional[List[str]] = love_engine_params.get("tag_regex")
...
async def get_deployments_for_tag(
    llm_router_instance: LoveEngineRouter,
    model: str,  # used to raise the correct error
    healthy_deployments: Union[List[Any], Dict[Any, Any]],
    request_kwargs: Optional[Dict[Any, Any]] = None,
    metadata_variable_name: Literal["metadata", "love_engine_metadata"] = "metadata",
):
    """
    ...
    if metadata_variable_name in request_kwargs:
        ...
        has_regex_deployments = any(
            d.get("love_engine_params", {}).get("tag_regex") for d in healthy_deployments
        )
    ...
    for deployment in healthy_deployments:
        if "default" in deployment.get("love_engine_params", {}).get("tags", []):
...
def _get_tags_from_request_kwargs(
    ...
    elif "love_engine_params" in request_kwargs:
        love_engine_params = request_kwargs["love_engine_params"] or {}
        _metadata = love_engine_params.get(metadata_variable_name, {}) or {}
