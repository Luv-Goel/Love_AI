...
def simple_shuffle(
    ...
    Args:
        llm_router_instance: LoveEngineRouter instance
    ...
    for weight_by in ["weight", "rpm", "tpm"]:
        weight = healthy_deployments[0].get("love_engine_params").get(weight_by, None)
        if weight is not None:
            weights = [
                m["love_engine_params"].get(weight_by, 0) for m in healthy_deployments
            ]
