...
class AdaptiveRouterUpdateQueue:
    ...
    async def add_session_state(
        ...
        upsert this into LOVE_ENGINE_AdaptiveRouterSession.
        """
    ...
    async def flush_state_to_db(self, prisma_client: Any) -> int:
        """
        Drain state aggregator and apply to LOVE_ENGINE_AdaptiveRouterState.
    ...
    async def flush_session_to_db(self, prisma_client: Any) -> int:
        """
        Drain session aggregator and upsert into LOVE_ENGINE_AdaptiveRouterSession.
        ...
        for key in sorted(batch.keys()):
            ...
            try:
                ...
                # `LOVE_ENGINE_AdaptiveRouterSession` -> `love_engine_adaptiveroutersession`
                # (single 's', not 'love_engine_adaptiverouterssession').
