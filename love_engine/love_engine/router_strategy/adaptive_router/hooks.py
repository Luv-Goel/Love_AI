...
from love_engine.router_strategy.adaptive_router.classifier import classify_prompt
from love_engine.router_strategy.adaptive_router.config import (
    ADAPTIVE_ROUTER_CHOSEN_MODEL_KEY,
    ADAPTIVE_ROUTER_RESPONSE_HEADER,
    SIGNAL_GATE_MIN_MESSAGES,
)
from love_engine.router_strategy.adaptive_router.signals import Turn
...
def _resolve_session_key(kwargs: Dict[str, Any]) -> Optional[str]:
    ...
    Order:
      1. Honor a client-supplied session id (`love_engine_session_id` on either
         `love_engine_params` or `love_engine_params.metadata`, or `session_id` on
    ...
    sid = love_engine_params.get("love_engine_session_id")
    if sid:
        return str(sid)
    metadata = love_engine_params.get("metadata") or {}
    if isinstance(metadata, dict):
        sid = metadata.get("session_id") or metadata.get("love_engine_session_id")
        if sid:
            return str(sid)
...
class AdaptiveRouterPostCallHook(CustomLogger):
    """One hook instance per AdaptiveRouter. Registered into love_engine.callbacks."""
    ...
    async def async_post_call_response_headers_hook(
        ...
        """
        Surface the chosen logical model as the `x-love_engine-adaptive-router-model`
