...
from love_engine.router_strategy.adaptive_router.bandit import (
    BanditCell,
    apply_delta,
    initial_cell,
    pick_best,
...
from love_engine.router_strategy.adaptive_router.signals import (
    SessionState,
    SignalDelta,
    Turn,
    apply_turn,
)
from love_engine.router_strategy.adaptive_router.update_queue import (
...
from love_engine.repositories.table_repositories import AdaptiveRouterStateRepository
...
class AdaptiveRouter:
    ...
    async def async_pre_routing_hook(
        ...
        # it as the `x-love_engine-adaptive-router-model` response header. We use
        # `metadata` (not a top-level kwarg) so the value doesn't leak into
        # `love_engine.acompletion(**input_kwargs)`.
    ...
    def _extract_min_quality_tier(
        ...
        Precedence: headers (`x-love_engine-min-quality-tier`) over metadata
