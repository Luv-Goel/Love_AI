...
from love_engine._logging import verbose_logger, verbose_router_logger
...
from love_engine.types.utils import LoveEnginePydanticObjectBase, StandardLoggingPayload
from love_engine.utils import get_utc_datetime, print_verbose
...
class RoutingArgs(LoveEnginePydanticObjectBase):
...
class LowestTPMLoggingHandler_v2(BaseRoutingStrategy, CustomLogger):
    ...
    def pre_call_check(self, deployment: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            deployment_name = deployment.get("love_engine_params", {}).get("model")
            ...
            if deployment_rpm is None:
                deployment_rpm = deployment.get("love_engine_params", {}).get("rpm")
            if deployment_rpm is None:
            ...
            if local_result is not None and local_result >= deployment_rpm:
                raise love_engine.RateLimitError(
                    ...
                    llm_provider="",
                    model=deployment.get("love_engine_params", {}).get("model"),
                    response=httpx.Response(
                        ...
                        request=httpx.Request(method="tpm_rpm_limits", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                    ),
                )
            else:
            ...
            if isinstance(e, love_engine.RateLimitError):
                raise e
    ...
    async def async_pre_call_check(
        ...
        Why? solves concurrency issue - https://github.com/BerriAI/love_engine/issues/2994
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            model = standard_logging_object["hidden_params"].get("love_engine_model_name")
            ...
            verbose_logger.exception(
                "love_engine.proxy.hooks.lowest_tpm_rpm_v2.py::log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if standard_logging_object is None:
                raise ValueError("standard_logging_object not passed in.")
            model_group = standard_logging_object.get("model_group")
            model = standard_logging_object["hidden_params"]["love_engine_model_name"]
            id = standard_logging_object.get("model_id")
            ...
            verbose_logger.exception(
                "love_engine.proxy.hooks.lowest_tpm_rpm_v2.py::async_log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def _return_potential_deployments(
        ...
        for item, item_tpm in all_deployments.items():
            ...
            if _deployment_tpm is None:
                _deployment_tpm = _deployment.get("love_engine_params", {}).get("tpm")
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = _deployment.get("love_engine_params", {}).get("rpm")
            if _deployment_rpm is None:
    ...
    async def async_get_available_deployments(
        ...
        for m in healthy_deployments:
            if isinstance(m, dict):
                ...
                deployment_name = m.get("love_engine_params", {}).get("model")
        ...
        try:
            ...
            for index, _deployment in enumerate(healthy_deployments):
                if isinstance(_deployment, dict):
                    ...
                    if _deployment_tpm is None:
                        _deployment_tpm = _deployment.get("love_engine_params", {}).get(
                            "tpm", None
                        )
                    if _deployment_tpm is None:
                    ...
                    if _deployment_rpm is None:
                        _deployment_rpm = _deployment.get("love_engine_params", {}).get(
                            "rpm", None
                        )
                    if _deployment_rpm is None:
