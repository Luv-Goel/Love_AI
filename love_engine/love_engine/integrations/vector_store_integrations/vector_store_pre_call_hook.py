...
import love_engine
import love_engine.vector_stores
...
class VectorStorePreCallHook(CustomLogger):
    ...
    2. Calls love_engine.vector_stores.search() to get relevant context
    ...
    async def async_get_chat_completion_prompt(
        ...
        try:
            ...
            vector_stores_to_run: List[LOVE_ENGINE_ManagedVectorStore] = (
                await love_engine.vector_store_registry.pop_vector_stores_to_run_with_db_fallback(
            ...
            for vector_store_to_run in vector_stores_to_run:
                ...
                love_engine_params_for_vector_store = (
                    vector_store_to_run.get("love_engine_params", {}) or {}
                )
                # Call love_engine.vector_stores.search() with the required parameters
                search_response = await love_engine.vector_stores.asearch(
                    **{
            ...
            if love_engine_logging_obj and all_search_results:
                love_engine_logging_obj.model_call_details["search_results"] = (
                    all_search_results
                )
    ...
    async def async_post_call_success_deployment_hook(
        ...
        try:
            ...
            if not love_engine_logging_obj:
                verbose_logger.debug("No love_engine_logging_obj in request_data")
                return None

            verbose_logger.debug(
                f"model_call_details keys: {list(love_engine_logging_obj.model_call_details.keys())}"
            )
            ...
            search_results: Optional[List[VectorStoreSearchResponse]] = (
                love_engine_logging_obj.model_call_details.get("search_results")
            )
