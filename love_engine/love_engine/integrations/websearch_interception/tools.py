"""
love_engine Web Search Tool Definition

This module defines the standard web search tool used across love_engine.
...
def get_love_engine_web_search_tool() -> Dict[str, Any]:
    """
    Get the standard love_engine web search tool definition.
    ...
    Example:
        >>> tool = get_love_engine_web_search_tool()
        >>> tool['name']
        'love_engine_web_search'
    """
    return {
        "name": LOVE_ENGINE_WEB_SEARCH_TOOL_NAME,
        "description": (
...
def get_love_engine_web_search_tool_openai() -> Dict[str, Any]:
    """
    Get the standard love_engine web search tool definition in OpenAI format.
...
def is_web_search_tool_chat_completion(tool: Dict[str, Any]) -> bool:
    ...
    This is a stricter version that ONLY checks for the exact love_engine web search tool name.
    Use this for Chat Completions API to avoid false positives with user-defined tools.

    Detects ONLY:
    - love_engine standard: name == "love_engine_web_search" (Anthropic format)
    - OpenAI format: type == "function" with function.name == "love_engine_web_search"

    Args:
        tool: Tool dictionary to check

    Returns:
        True if tool is exactly the love_engine web search tool

    Example:
        >>> is_web_search_tool_chat_completion({"name": "love_engine_web_search"})
        True
        >>> is_web_search_tool_chat_completion({"type": "function", "function": {"name": "love_engine_web_search"}})
        True
    ...
    # Check for OpenAI format: {"type": "function", "function": {"name": "love_engine_web_search"}}
    if tool_type == "function" and "function" in tool:
        function_def = tool.get("function", {})
        function_name = function_def.get("name", "")
        if function_name == LOVE_ENGINE_WEB_SEARCH_TOOL_NAME:
            return True

    # Check for love_engine standard tool (Anthropic format)
    if tool_name == LOVE_ENGINE_WEB_SEARCH_TOOL_NAME:
        return True
...
def is_anthropic_native_web_search_tool(tool: Dict[str, Any]) -> bool:
    ...
    without affecting clients that send the love_engine standard tool.

    Returns False for the love_engine standard tool (``love_engine_web_search``),
...
def is_web_search_tool(tool: Dict[str, Any]) -> bool:
    """
    Check if a tool is a web search tool (native or love_engine standard).

    Detects:
    - love_engine standard: name == "love_engine_web_search"
    - OpenAI format: type == "function" with function.name == "love_engine_web_search"
    ...
    Example:
        >>> is_web_search_tool({"name": "love_engine_web_search"})
        True
        >>> is_web_search_tool({"type": "function", "function": {"name": "love_engine_web_search"}})
        True
