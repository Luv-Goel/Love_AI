...
Transforms between Anthropic/OpenAI tool_use format and love_engine search format.
"""
...
class WebSearchTransformation:
    ...
    - Anthropic tool_use format → love_engine search requests
    - OpenAI tool_calls format → love_engine search requests
    - love_engine SearchResponse → Anthropic/OpenAI tool_result format
    """
    ...
    def _detect_from_non_streaming_response(
        ...
        for block in content:
            ...
            # ``love_engine_web_search``; the bare ``web_search`` entry handles
            # callers that bypass our pre-request hooks (e.g. direct
            # love_engine.acompletion). "WebSearch" is intentionally omitted —
            # see is_web_search_tool for the Cowork rationale.
            if block_type == "tool_use" and block_name in (
                LOVE_ENGINE_WEB_SEARCH_TOOL_NAME,
                "web_search",
            ):
    ...
    def transform_response(
        ...
        """
        Transform love_engine search results to Anthropic/OpenAI tool_result format.
    ...
    def build_web_search_tool_result_block(
        ...
        Args:
            ...
            search_response: Structured ``SearchResponse`` from
                ``love_engine.asearch()``. If None or empty, the block is still
    ...
    def format_search_response(result: SearchResponse) -> str:
        ...
        Args:
            result: SearchResponse from love_engine.asearch()
