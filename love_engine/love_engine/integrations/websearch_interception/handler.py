...
server-side using love_engine router's search tools.
"""
...
from love_engine.constants import LOVE_ENGINE_WEB_SEARCH_TOOL_NAME
...
from love_engine.integrations.websearch_interception.tools import (
    ...
    get_love_engine_web_search_tool_openai,
    is_anthropic_native_web_search_tool,
    is_web_search_tool,
    is_web_search_tool_chat_completion,
)
from love_engine.integrations.websearch_interception.transformation import (
    WebSearchTransformation,
)
from love_engine.llms.base_llm.search.transformation import SearchResponse
from love_engine.types.integrations.websearch_interception import (
...
class WebSearchInterceptionLogger(CustomLogger):
    ...
    2. Executes love_engine.asearch() for each query using router's search tools
    ...
    async def async_pre_call_deployment_hook(
        ...
        # Try top-level kwargs first, then nested love_engine_params, then derive from model name
        ...
        # Check if any tool is a web search tool (native or already love_engine standard)
        ...
        verbose_logger.debug(
            "WebSearchInterception: Converting native web_search tools to love_engine standard"
        )
        ...
        # Convert native/custom web_search tools to love_engine standard
        converted_tools = []
        for tool in tools:
            if is_web_search_tool(tool):
                # Convert to love_engine standard web search tool
                converted_tool = get_love_engine_web_search_tool_openai()
                converted_tools.append(converted_tool)
                verbose_logger.debug(
                    f"WebSearchInterception: Converted {tool.get('name', 'unknown')} "
                    f"(type={tool.get('type', 'none')}) to {LOVE_ENGINE_WEB_SEARCH_TOOL_NAME}"
                )
            else:
    ...
    def from_config_yaml(
        ...
        Args:
            config: Configuration dictionary from love_engine_settings.websearch_interception_params
        ...
        Example:
            ...
            Usage:
                config = love_engine_settings.get("websearch_interception_params", {})
    ...
    async def async_pre_request_hook(
        ...
        """
        Pre-request hook to convert native web search tools to love_engine standard.
        ...
        2. Convert them to love_engine standard format (love_engine_web_search)
        ...
        custom_llm_provider = kwargs.get("love_engine_params", {}).get(
            "custom_llm_provider", ""
        )
        ...
        # Convert native web search tools to love_engine standard
        converted_tools = []
        for tool in tools:
            if is_web_search_tool(tool):
                standard_tool = get_love_engine_web_search_tool()
    ...
    async def async_should_run_agentic_loop(
        ...
        # Check if tools include any web search tool (love_engine standard or native)
    ...
    async def async_should_run_chat_completion_agentic_loop(
        ...
        if not has_websearch_tool:
            verbose_logger.debug(
                "WebSearchInterception: No love_engine_web_search tool in request"
            )
            return False, {}
    ...
    def _prepare_followup_kwargs(kwargs: Dict) -> Dict:
        ...
        ``love_engine_logging_obj`` MUST be excluded so the follow-up call creates
        ...
        """
        _internal_keys = {"love_engine_logging_obj"}
        return {
            k: v
    ...
    async def _build_anthropic_request_patch(
        ...
        """
        Execute love_engine.search() and build follow-up request patch.
        ...
        _call_id = getattr(logging_obj, "love_engine_call_id", None) or kwargs.get(
            "love_engine_call_id", "unknown"
        )
        ...
        try:
            ...
            try:
                ...
                verbose_logger.debug(
                    ...
                    "falling back to direct love_engine.asearch() with perplexity"
                )
                llm_router = None
            ...
            if llm_router is not None and hasattr(llm_router, "search_tools"):
                ...
                if not search_provider and llm_router.search_tools:
                    ...
                    search_provider = first_tool.get("love_engine_params", {}).get(
                        "search_provider"
                    )
                    verbose_logger.debug(
            ...
            result = await love_engine.asearch(query=query, search_provider=search_provider)
    ...
    async def _build_chat_completion_request_patch(
        ...
        """Execute love_engine.search() and build chat-completion rerun patch."""
    ...
    def initialize_from_proxy_config(
        ...
        Args:
            love_engine_settings: Dictionary containing love_engine_settings from proxy_config.yaml
        ...
        """
        # Get websearch_interception_params from love_engine_settings or callback_specific_params
        websearch_params: WebSearchInterceptionConfig = {}
        if "websearch_interception_params" in love_engine_settings:
            websearch_params = love_engine_settings["websearch_interception_params"]
