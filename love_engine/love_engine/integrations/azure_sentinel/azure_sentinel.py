...
love_engine logs to the Log Analytics workspace using the Azure Monitor Logs Ingestion API.

Reference API: https://learn.microsoft.com/en-us/azure/azure-monitor/logs/logs-ingestion-api-overview

`async_log_success_event` - used by love_engine proxy to send logs to Azure Sentinel
`async_log_failure_event` - used by love_engine proxy to send failure logs to Azure Sentinel
...
from love_engine.types.utils import StandardAuditLogPayload, StandardLoggingPayload


class AzureSentinelLogger(CustomBatchLogger):
    """
    Logger that sends love_engine logs to Azure Sentinel via Azure Monitor Logs Ingestion API
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            stream_name (str, optional): Stream name from DCR (e.g., "Custom-love_engine").
                If not provided, will use AZURE_SENTINEL_STREAM_NAME env var or default to "Custom-love_engine".
        ...
        resolved_stream_name = (
            stream_name or os.getenv("AZURE_SENTINEL_STREAM_NAME") or "Custom-love_engine"
        )
    ...
    async def async_log_audit_log_event(
        ...
        """
        Async log love_engine audit log events to Azure Sentinel.
