...
class LLMResponse(BaseModel):
    ...
    cost: Optional[float] = Field(
        default=None,
        description="Total cost of the LLM call in USD as computed by love_engine.",
    )
...
class GalileoObserve(CustomLogger):
    ...
    def _format_created_at(dt: Union[datetime, Any]) -> str:
        ...
        if dt.tzinfo is None:
            # love_engine often passes naive datetimes in local time; convert to UTC
    ...
    def _record_to_v2_span(
        ...
        *,
        ...
        span: Dict[str, Any] = {
            ...
            "trace_id": trace_id,
            "parent_id": trace_id,
            "name": record.get("node_type", "love_engine"),
    ...
    @staticmethod
    def _get_chat_content_for_galileo(response_obj: love_engine.ModelResponse) -> Any:
    ...
    def _get_galileo_input_output_content(
        ...
        if response_obj is not None and isinstance(response_obj, love_engine.ModelResponse):
        ...
        if response_obj is not None and isinstance(response_obj, love_engine.ImageResponse):
    ...
    async def _async_log_success_event_impl(
        ...
        _call_type: str = str(
            slo.get("call_type") or kwargs.get("call_type") or "love_engine"
        )
