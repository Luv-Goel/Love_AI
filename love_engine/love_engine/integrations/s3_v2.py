...
from love_engine.constants import DEFAULT_S3_BATCH_SIZE, DEFAULT_S3_FLUSH_INTERVAL_SECONDS
...
from love_engine.types.integrations.s3_v2 import s3BatchLoggingElement
...
class S3Logger(CustomBatchLogger, BaseAWSLLM):
        ...
        try:
            ...
            if s3_callback_params_override is not None:
                ...
                verbose_logger.debug(
                    ...
                    f"{_masker.mask_dict(dict(love_engine.s3_callback_params or {}))}"
                )
    ...
    def _init_s3_params(
        ...
        audit-log instance), otherwise falls back to `love_engine.s3_callback_params`.
        Resolves `os.environ/X` markers into a local dict; never mutates the source.
        """
        if params_source is None:
            params_source = love_engine.s3_callback_params or {}
        params: dict = {
            key: (
                love_engine.get_secret(value)
    ...
    def create_s3_batch_logging_element(
        ...
        s3_file_name = (
            love_engine.utils.get_logging_id(start_time, standard_logging_payload) or ""
        )
        verbose_logger.debug(
