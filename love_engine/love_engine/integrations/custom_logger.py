...
from love_engine.constants import DEFAULT_MAX_RECURSE_DEPTH_SENSITIVE_DATA_MASKER
from love_engine.types.integrations.argilla import ArgillaItem
from love_engine.types.llms.openai import AllMessageValues, ChatCompletionRequest
from love_engine.types.prompts.init_prompts import PromptSpec
from love_engine.types.integrations.custom_logger import AgenticLoopPlan
...
if TYPE_CHECKING:
    ...
    from love_engine.types.router import PreRoutingHookResponse
...
class CustomLogger:  # https://docs.love_engine.ai/docs/observability/custom_callback#callback-class
    ...
    def get_callback_env_vars(callback_name: Optional[str] = None) -> List[str]:
        ...
        try:
            from love_engine.proxy._types import AllCallbacks
        except Exception:
            return []
        ...
        params = getattr(callback_info, "love_engine_callback_params", None)
        if not params:
            return []
    ...
    #### ADAPTERS #### Allow calling 100+ LLMs in custom format - https://github.com/BerriAI/love_engine/pulls

    def translate_completion_input_params(
        self, kwargs
    ) -> Optional[ChatCompletionRequest]:
        """
        Translates the input params, from the provider's native format to the love_engine.completion() format.
        """
        pass
    ...
    ]:  # raise exception if invalid, return a str for the user to receive - if rejected, or return a modified dictionary for passing into love_engine
        pass
    ...
    async def async_post_call_response_headers_hook(
        ...
        Args:
            ...
            - love_engine_call_info: Optional[Dict[str, Any]] - Normalized routing metadata:
    ...
    #### SINGLE-USE #### - https://docs.love_engine.ai/docs/observability/custom_callback#using-your-custom-callback-function
    ...
    #########################################################
    # AGENTIC LOOP HOOKS (for love_engine.messages + future completion support)
    #########################################################
    ...
    async def async_should_run_agentic_loop(
        ...
        - WebSearch: Intercept WebSearch tool calls for Bedrock/Claude, execute
          love_engine.search(), return final answer with search results
        ...
        Flow:
        1. User calls love_engine.messages.acreate(tools=[...])
    ...
    async def async_run_agentic_loop(
        ...
        2. Execute the tools (love_engine.search, code execution, DB queries, etc.)
        3. Build assistant message with tool_use blocks
        4. Build user message with tool_result blocks containing results
        5. Make follow-up love_engine.messages.acreate() call with results
        ...
        Args:
            ...
            logging_obj: love_engine logging object
            stream: Whether response is streaming
            kwargs: Additional request parameters
        ...
        Example:
            ...
            search_results = await asyncio.gather(
                *[love_engine.asearch(tc["input"]["query"]) for tc in tool_calls]
            )
            ...
            from love_engine.anthropic_interface import messages
    ...
    def _truncate_text(self, text: str, max_length: int) -> str:
        ...
        return (
            text[:max_length]
            + "...truncated by love_engine, this logger does not support large content"
    ...
    def _select_metadata_field(
        ...
        1. If `love_engine_metadata` is in the request kwargs, use it
        2. Otherwise, use `metadata`
        """
        from love_engine.constants import LOVE_ENGINE_METADATA_FIELD, OLD_LOVE_ENGINE_METADATA_FIELD

        if request_kwargs is None:
            return None
        if LOVE_ENGINE_METADATA_FIELD in request_kwargs:
            return LOVE_ENGINE_METADATA_FIELD
        return OLD_LOVE_ENGINE_METADATA_FIELD
    ...
    def redact_standard_logging_payload_from_model_call_details(
        ...
        if turn_off_message_logging:
            redacted_str = "redacted-by-love_engine"
