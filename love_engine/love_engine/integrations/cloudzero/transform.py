...
"""Transform love_engine data to CloudZero AnyCost CBF format."""
...
class CBFTransformer:
    """Transform love_engine usage data to CloudZero Billing Format (CBF)."""
    ...
    def transform(self, data: pl.DataFrame) -> pl.DataFrame:
        """Transform love_engine data to CBF format, dropping records with zero successful_requests or invalid CZRNs."""
        if data.is_empty():
            return pl.DataFrame()
    ...
    def _create_cbf_record(self, row: dict[str, Any]) -> CBFRecord:
        """Create a single CBF record from love_engine daily spend row."""
        ...
        resource_id = self.czrn_generator.create_from_love_engine_data(row)
