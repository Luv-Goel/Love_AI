...
from love_engine import _logging


def normalize_provider_name(provider: Optional[str]) -> Optional[str]:
    """
    Normalize love_engine provider names to standardized string names.

    Args:
        provider: love_engine internal provider name
...
def extract_opik_metadata(
    love_engine_metadata: Dict[str, Any],
    ...
    Merge Opik metadata from three sources in increasing priority order:
      ...
      2. love_engine_metadata (request)– overrides auth-key defaults
      3. requester_metadata – highest priority (e.g. proxy header overrides)

    Args:
        love_engine_metadata: Metadata from love_engine_params.mak
    ...
    request_opik = love_engine_metadata.get("opik") or {}
    ...
    _logging.verbose_logger.debug(
        f"love_engine_opik_metadata - {json.dumps(opik_meta, default=str)}"
    )
...
def extract_and_build_metadata(
    ...
    love_engine_kwargs: Dict[str, Any],
) -> Dict[str, Any]:
    """
    ...
    - Cost information from love_engine_kwargs (calculated after completion)
    ...
    Args:
        ...
        love_engine_kwargs: Original love_engine kwargs (includes response_cost)
    ...
    # response_cost is calculated by love_engine after completion and added to kwargs
    # See: love_engine/love_engine_core_utils/llm_response_utils/response_metadata.py
    if "response_cost" in love_engine_kwargs:
        metadata["cost"] = {
            "total_tokens": love_engine_kwargs["response_cost"],
            "currency": "USD",
        }

    # Add debug info if cost calculation failed
    if "response_cost_failure_debug_info" in love_engine_kwargs:
        metadata["response_cost_failure_debug_info"] = love_engine_kwargs[
