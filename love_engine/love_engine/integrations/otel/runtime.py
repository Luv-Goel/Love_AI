...
``love_engine.integrations.otel.logger`` (which imports the SDK at module scope) at
...
def phase_span(name: str) -> "Iterator[Any]":
    ...
    """
    try:
        from love_engine.integrations.otel.logger import phase_span as _phase_span
    except Exception:
        yield None
        return
...
def seed_request_identity(user_api_key_dict: Any, model: Any = None) -> None:
    ...
    try:
        from love_engine.integrations.otel.logger import (
