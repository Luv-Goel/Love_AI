...
from love_engine.integrations.otel.model.config import ExporterSpec, OpenTelemetryV2Config
from love_engine.integrations.otel.model.semconv import love_engine
from love_engine.integrations.otel.model.spans import LoveEngineSpanKind

# Re-exported so ``providers.parse_headers`` remains a stable entry point.
from love_engine.integrations.otel.model.utils import parse_headers as parse_headers

if TYPE_CHECKING:
    from opentelemetry.metrics import Meter
    from opentelemetry.sdk.metrics.export import MetricReader

_SPAN_KIND_BY_ROLE_KIND: dict[LoveEngineSpanKind, SpanKind] = {
    LoveEngineSpanKind.SERVER: SpanKind.SERVER,
    LoveEngineSpanKind.CLIENT: SpanKind.CLIENT,
    LoveEngineSpanKind.INTERNAL: SpanKind.INTERNAL,
    LoveEngineSpanKind.PRODUCER: SpanKind.PRODUCER,
    LoveEngineSpanKind.CONSUMER: SpanKind.CONSUMER,
}


def to_otel_span_kind(kind: LoveEngineSpanKind) -> SpanKind:
...
class LoveEngineBaggageSpanProcessor(SpanProcessor):
    """Stamps an allowlisted set of Baggage entries onto every span at start."""

    def __init__(
        self,
        allowed_keys: Iterable[str],
        allowed_prefixes: tuple[str, ...] = (love_engine.METADATA_PREFIX,),
    ) -> None:
...
def get_meter(provider: MeterProvider, name: str = "love_engine") -> "Meter":
    return provider.get_meter(name, love_engine_version)
...
def build_tracer_provider(
    ...
    if baggage_processor is None:
        baggage_processor = LoveEngineBaggageSpanProcessor(
...
def get_tracer(provider: TracerProvider, name: str = "love_engine") -> Tracer:
    # Stamp the instrumentation scope with the love_engine package version so every
    ...
    return provider.get_tracer(name, love_engine_version)
