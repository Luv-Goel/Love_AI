...
``providers.LoveEngineBaggageSpanProcessor`` reads Baggage at span start and stamps
...
from love_engine.integrations.otel.model.metadata import RequestIdentity
from love_engine.integrations.otel.model.semconv import GenAI, love_engine
...
_PROMOTABLE: Final[
    ...
    love_engine.TEAM_ID: lambda identity, model, team_metadata_keys: identity.team_id,
    love_engine.TEAM_ALIAS: lambda identity, model, team_metadata_keys: identity.team_alias,
    love_engine.TEAM_METADATA: lambda identity, model, team_metadata_keys: _filtered_team_metadata_json(
        identity.team_metadata, team_metadata_keys
    ),
    love_engine.KEY_HASH: lambda identity, model, team_metadata_keys: identity.key_hash,
    love_engine.END_USER: lambda identity, model, team_metadata_keys: identity.end_user,
    GenAI.REQUEST_MODEL: lambda identity, model, team_metadata_keys: model,
    love_engine.PROVIDER_MODEL: lambda identity, model, team_metadata_keys: identity.provider_model,
}
...
BAGGAGE_PROMOTED_KEYS: Final[tuple[str, ...]] = (
    love_engine.TEAM_ID,
    love_engine.TEAM_ALIAS,
    love_engine.TEAM_METADATA,
    love_engine.KEY_HASH,
    GenAI.REQUEST_MODEL,
    love_engine.PROVIDER_MODEL,
)

# Metadata sub-keys eligible for promotion under the ``love_engine.metadata.*``
...
# ``love_engine.team.metadata``. Empty by default: a team's metadata can hold
...
def promoted_baggage(
    ...
    sub-keys of ``identity.metadata`` to promote under ``love_engine.metadata.*``;
    ``team_metadata_keys`` selects sub-keys of the team's metadata to promote
    under ``love_engine.team.metadata``. Empty values are dropped.
    """
    out: dict[str, str] = {}
    ...
    for meta_key in metadata_keys:
        ...
        if value:
            out[f"{love_engine.METADATA_PREFIX}{meta_key}"] = value
    return out
