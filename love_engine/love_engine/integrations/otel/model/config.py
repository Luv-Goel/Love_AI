...
OTEL_V2_ENV = "LOVE_ENGINE_OTEL_V2"
...
class OpenTelemetryV2Config(BaseSettings):
    ...
    service_name: str = Field(
        default="love_engine", validation_alias=AliasChoices("OTEL_SERVICE_NAME")
    )
    ...
    enable_metrics: bool = Field(
        default=False,
        validation_alias=AliasChoices("LOVE_ENGINE_OTEL_INTEGRATION_ENABLE_METRICS"),
    )
    enable_events: bool = Field(
        default=False,
        validation_alias=AliasChoices("LOVE_ENGINE_OTEL_INTEGRATION_ENABLE_EVENTS"),
    )
    ...
    legacy_compat: bool = Field(
        default=True, validation_alias=AliasChoices("LOVE_ENGINE_OTEL_LEGACY_COMPAT")
    )
    ...
    baggage_promoted_keys: Annotated[List[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices(
            "baggage_promoted_keys", "LOVE_ENGINE_OTEL_BAGGAGE_PROMOTED_KEYS"
        ),
        description=(
            "Identity attribute keys written into Baggage and stamped on every "
            "child span (e.g. ``love_engine.team.id``). Configure via the "
            "``LOVE_ENGINE_OTEL_BAGGAGE_PROMOTED_KEYS`` env var (comma-separated) or "
    ...
    baggage_metadata_keys: Annotated[List[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices(
            "baggage_metadata_keys", "LOVE_ENGINE_OTEL_BAGGAGE_METADATA_KEYS"
        ),
        description=(
            "Metadata sub-keys promoted under the ``love_engine.metadata.*`` "
            "namespace. Configure via the ``LOVE_ENGINE_OTEL_BAGGAGE_METADATA_KEYS`` "
    ...
    baggage_team_metadata_keys: Annotated[List[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices(
            "baggage_team_metadata_keys", "LOVE_ENGINE_OTEL_BAGGAGE_TEAM_METADATA_KEYS"
        ),
        description=(
            "Sub-keys of the team's free-form metadata promoted under "
            "``love_engine.team.metadata``. Empty by default so none of a team's "
            "metadata leaves the process until explicitly allowlisted. Configure "
            "via the ``LOVE_ENGINE_OTEL_BAGGAGE_TEAM_METADATA_KEYS`` env var "
            "(comma-separated) or "
    ...
    def _split_csv(cls, value: Any) -> Any:
        ...
        an operator write ``LOVE_ENGINE_OTEL_BAGGAGE_PROMOTED_KEYS=love_engine.team.id,love_engine.api_key.hash``.
