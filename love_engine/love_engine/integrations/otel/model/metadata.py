...
Every relevant field love_engine exposes about a request — the user-facing model,
...
* ``model`` — already reconstructed (see ``reconstruct_model_name``) to the name
  love_engine dispatched to the provider (the deployment, provider-prefixed).
* ``hidden_params.love_engine_model_name`` — a secondary source for the dispatched
...
SDK path, which has no group), and ``love_engine.provider.model`` is the *dispatched*
...
from love_engine.constants import LOVE_ENGINE_LOGGING_NO_UPSTREAM_LLM_CALL
from love_engine.integrations.otel.model.semconv import resolve_operation
from love_engine.integrations.otel.model.utils import as_str
...
class RequestIdentity:
    ...
    # The model love_engine dispatched to the provider. Only known once the call
    ...
    def from_payload(cls, payload: "StandardLoggingPayload") -> "RequestIdentity":
        ...
        return cls(
            call_id=as_str(payload.get("love_engine_call_id")) or as_str(payload.get("id")),
...
class RequestContext:
    ...
    (on :attr:`identity`) is the model love_engine dispatched to the provider; the two
...
class LLMCallEvent:
    ...
    love_engine hands every callback an untyped ``kwargs`` god object. The fields the
    ...
    # The ``love_engine_call_id`` correlating ``pre_call`` with the close callback.
    ...
    def from_dict(cls, kwargs: Mapping[str, Any]) -> "LLMCallEvent":
        ...
        return cls(
            ...
            is_no_upstream_call=bool(kwargs.get(LOVE_ENGINE_LOGGING_NO_UPSTREAM_LLM_CALL)),
...
def _call_id(
    ...
    if payload is not None:
        ...
        if call_id:
            return call_id
    return as_str(kwargs.get("love_engine_call_id"))
...
def resolve_provider_model(payload: "StandardLoggingPayload") -> str | None:
    """The model love_engine dispatched to the provider, from the payload.

    Prefers the explicit ``hidden_params.love_engine_model_name`` (set on call paths
    ...
    return (
        ...
        or as_str(hidden.get("love_engine_model_name"))
