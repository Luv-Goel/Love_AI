...
from love_engine.integrations.otel.model.utils import (
    as_bool,
    as_float,
    as_int,
    as_str,
...
class LLMCost:
    ...
    ``cost_breakdown`` (``love_engine.types.utils.CostBreakdown``).
...
class ServiceSpanData:
    ...
    # these are namespaced: the canonical vocabulary uses ``love_engine.metadata.*``
