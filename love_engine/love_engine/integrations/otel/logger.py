...
from love_engine.integrations.otel.model.baggage import promoted_baggage
from love_engine.integrations.otel.model.config import OpenTelemetryV2Config
from love_engine.integrations.otel.plumbing.context import (
    is_recordable_span,
    request_root_span,
...
from love_engine.integrations.otel.emitter import SpanEmitter
from love_engine.integrations.otel.mappers import resolve_mappers
...
from love_engine.integrations.otel.plumbing.metrics import (
    GenAIMetricRecorder,
    create_genai_metrics,
)
from love_engine.integrations.otel.plumbing.providers import (
    build_tracer_provider,
    get_meter,
    get_tracer,
    resolve_meter_provider,
)
from love_engine.integrations.otel.plumbing.routing import TenantTracerCache
from love_engine.integrations.otel.model.spans import SpanRole, span_role_for_service
from love_engine.integrations.otel.model.utils import to_ns
...
class OpenTelemetryV2(CustomLogger):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.tracer: Tracer = get_tracer(self._tracer_provider, LOVE_ENGINE_TRACER_NAME)
        ...
        self._tenant_tracers = TenantTracerCache(
            self.config, callback_name, LOVE_ENGINE_TRACER_NAME
        )
    ...
    def _init_metrics(self, meter_provider: Any | None) -> "GenAIMetricRecorder | None":
        ...
        meter = get_meter(provider, LOVE_ENGINE_TRACER_NAME)
    ...
    def _init_otel_logger_on_love_engine_proxy(self) -> None:
        try:
            from love_engine.proxy import proxy_server
        except Exception:
            return
        try:
            self._register_in_callback_list(love_engine.service_callback)
            self._register_in_callback_list(love_engine.input_callback)
            self._register_in_callback_list(love_engine._async_success_callback)
            self._register_in_callback_list(love_engine._async_failure_callback)
        except Exception:
            pass
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        ``LoveEngineLoggingObj`` (a typed object) and closed in the async callback.
    ...
    def seed_request_identity(self, user_api_key_dict: Any, model: Any = None) -> None:
        ...
        ``LoveEngineBaggageSpanProcessor``. Called once at the auth boundary (as soon
        ...
        try:
            ...
            if bag:
                ...
                if is_recordable_span(server_span):
                    ...
                    # ``create_love_engine_proxy_request_started_span`` (e.g. the SDK
