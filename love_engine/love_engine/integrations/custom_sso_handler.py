...
class CustomSSOLoginHandler(CustomLogger):
    ...
    Useful when you have an OAuth proxy in front of love_engine
    ...
    async def handle_custom_ui_sso_sign_in(
        ...
        from love_engine.proxy.auth.trusted_proxy_utils import (
        ...
        return OpenID(
            id=request_headers_dict.get("x-love_engine-user-id"),
            email=request_headers_dict.get("x-love_engine-user-email"),
            first_name="Test",
            last_name="Test",
            display_name="Test",
