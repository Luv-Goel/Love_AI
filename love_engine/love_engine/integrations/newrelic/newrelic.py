...
Usage - Python SDK:
    import love_engine
    love_engine.callbacks = ["newrelic"]

    # Or with explicit configuration:
    from love_engine.integrations.newrelic import NewRelicLogger
    love_engine.callbacks = [NewRelicLogger(turn_off_message_logging=True)]
...
from love_engine.love_engine_core_utils.redact_messages import should_redact_message_logging
from love_engine.types.integrations.newrelic import NewRelicInitParams
from love_engine.types.integrations.base_health_check import IntegrationHealthCheckStatus
from love_engine.types.utils import ModelResponse, Message, StandardLoggingPayload
...
class NewRelicLogger(CustomLogger):
    """
    New Relic logger for love_engine to send AI monitoring events.
    ...
    def __init__(self, **kwargs):
        #########################################################
        # Handle newrelic_params set as love_engine.newrelic_params
        #########################################################
        ...
        if not self.license_key or not self.app_name:
            ...
            verbose_logger.error(
                "New Relic Python agent not installed. Review the New Relic integration documentation at https://docs.love_engine.ai/docs/observability/newrelic."
            )
            self.enabled = False
        else:
            try:
    ...
    def _get_newrelic_params(self) -> Dict:
        """
        Get the newrelic_params from love_engine.newrelic_params

        These are params specific to initializing the NewRelicLogger e.g. turn_off_message_logging
        """
        dict_newrelic_params: Dict = {}
        if love_engine.newrelic_params is not None:
            if isinstance(love_engine.newrelic_params, NewRelicInitParams):
                dict_newrelic_params = love_engine.newrelic_params.model_dump()
            elif isinstance(love_engine.newrelic_params, Dict):
                # only allow params that are of NewRelicInitParams
                dict_newrelic_params = NewRelicInitParams(
                    **love_engine.newrelic_params
                ).model_dump()
    ...
    def _parse_bool_env(self, var_name: str, default: bool = False) -> bool:
        ...
        ``love_engine/__init__.py`` and the standard library's
    ...
    def _get_love_engine_version(self) -> str:
        """
        Get love_engine version for supportability metrics.
        ...
        try:
            ...
            return version("love_engine")
        except Exception as e:
            verbose_logger.warning(f"Unable to determine love_engine version: {e}")
            return "unknown"

    def _emit_supportability_metric(self):
        """
        Emit New Relic supportability metric for love_engine usage.

        Per spec, this metric should be emitted at least once every 27 hours
        to indicate the library is in use. Format:
        Supportability/Python/ML/love_engine/{version}

        This method updates _last_metric_emission_time and should
        be called within a lock when checking periodic emission.
        """
        try:
            love_engine_version = self._get_love_engine_version()
            metric_name = f"Supportability/Python/ML/love_engine/{love_engine_version}"
    ...
    def _get_trace_context(
        ...
        This integration runs in love_engine's async logging worker, outside the
        ...
        Resolution order:
        1. W3C traceparent header (love_engine_params.metadata.headers.traceparent) -
           what the agent would link to if we were in-transaction.
        2. StandardLoggingPayload.trace_id - love_engine's internal trace for
    ...
    def _extract_completion_id(self, kwargs: Dict, response_obj: ModelResponse) -> str:
        ...
        if not completion_id:
            completion_id = kwargs.get("love_engine_call_id")
    ...
    def _get_vendor(
        ...
        if standard_logging_object:
            ...
            if vendor:
                return vendor
        love_engine_params = kwargs.get("love_engine_params", {}) or {}
        return love_engine_params.get("custom_llm_provider") or "love_engine"
    ...
    def _get_duration(
        ...
        Resolution order:
        1. StandardLoggingPayload.response_time (already computed by love_engine)
    ...
    def _extract_all_messages(
        ...
        # love_engine's wider redaction decision (turn_off_message_logging, dynamic
        ...
        try:
            ...
            for message in messages:
                ...
                event_data = {
                    ...
                    "ingest_source": "love_engine",
                    "token_count": 0,  # Per-message token counts are not available from love_engine
                }
    ...
    def _record_error_metric(self):
        ...
        try:
            ...
            app = _newrelic_agent.application()
            if app and app.enabled:
                app.record_custom_metric("LLM/love_engine/Error", 1)
        except Exception as e:
    ...
    async def async_health_check(self) -> IntegrationHealthCheckStatus:
        ...
        `LoveEngineConnectionTest` custom event so the user can confirm the
        end-to-end pipeline in the New Relic UI via NRQL:
        `SELECT * FROM LoveEngineConnectionTest SINCE 1 hour ago`.

        The `LoveEngineConnectionTest` event type is intentionally outside the
        ...
        try:
            ...
            app.record_custom_event(
                "LoveEngineConnectionTest",
                {
                    "is_test_event": True,
                    "app_name": self.app_name,
                    "source": "love_engine-proxy",
                    "timestamp": time.time(),
                },
            )
