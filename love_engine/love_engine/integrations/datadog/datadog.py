...
`async_log_success_event` - used by love_engine proxy to send logs to datadog
`log_success_event` - sync version of logging to DataDog, only used on love_engine Python SDK, if user opts in to using sync functions
...
from love_engine.integrations.datadog.datadog_mock_client import (
    should_use_datadog_mock,
    create_mock_datadog_client,
)
from love_engine.integrations.datadog.datadog_handler import (
...
from love_engine.types.integrations.datadog import (
    DD_ERRORS,
    DD_MAX_BATCH_SIZE,
    DataDogStatus,
    DatadogInitParams,
...
from love_engine.types.services import ServiceLoggerPayload, ServiceTypes
...
class DataDogLogger(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            dd_agent_host: Hostname or IP of DataDog agent. Falls back to LOVE_ENGINE_DD_AGENT_HOST env var.
            dd_agent_port: Port of DataDog agent (default: 10518). Falls back to LOVE_ENGINE_DD_AGENT_PORT env var.
        ...
        `LOVE_ENGINE_DD_AGENT_HOST` - hostname or IP of DataDog agent, example = `"localhost"`
        `LOVE_ENGINE_DD_AGENT_PORT` - port of DataDog agent (default: 10518 for logs)

        Note: We use LOVE_ENGINE_DD_AGENT_HOST instead of DD_AGENT_HOST to avoid conflicts
        ...
        try:
            ...
            #########################################################
            # Handle datadog_params set as love_engine.datadog_params
            #########################################################
            ...
            resolved_agent_host = dd_agent_host or os.getenv("LOVE_ENGINE_DD_AGENT_HOST")
    ...
    def _get_datadog_params(self) -> Dict:
        """
        Get the datadog_params from love_engine.datadog_params

        These are params specific to initializing the DataDogLogger e.g. turn_off_message_logging
        """
        dict_datadog_params: Dict = {}
        if love_engine.datadog_params is not None:
            if isinstance(love_engine.datadog_params, DatadogInitParams):
                dict_datadog_params = love_engine.datadog_params.model_dump()
            elif isinstance(love_engine.datadog_params, Dict):
                # only allow params that are of DatadogInitParams
                dict_datadog_params = DatadogInitParams(
                    **love_engine.datadog_params
                ).model_dump()
    ...
    def _configure_dd_agent(
        ...
        Args:
            ...
            dd_agent_port: Port of DataDog agent. Falls back to LOVE_ENGINE_DD_AGENT_PORT env var (default: 10518).
        ...
        resolved_port = dd_agent_port or os.getenv(
            "LOVE_ENGINE_DD_AGENT_PORT", "10518"
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        """
        try:
            if love_engine.datadog_use_v1 is True:
    ...
    def _create_v0_logging_payload(
        ...
        (Not Recommended) If you want this to get logged set `love_engine.datadog_use_v1 = True`
        """
        ...
        call_type = kwargs.get("call_type", "love_engine.completion")
