...
from love_engine.types.integrations.datadog_llm_obs import *
...
class DataDogLLMObsLogger(CustomBatchLogger):
    def __init__(self, **kwargs):
        try:
            ...
            # Use LOVE_ENGINE_DD_AGENT_HOST to avoid conflicts with ddtrace's DD_AGENT_HOST
            # Check for agent mode FIRST - agent mode doesn't require DD_API_KEY or DD_SITE
            dd_agent_host = os.getenv("LOVE_ENGINE_DD_AGENT_HOST")
            ...
            #########################################################
            # Handle datadog_llm_observability_params set as love_engine.datadog_llm_observability_params
            #########################################################
    ...
    def _configure_dd_agent(self, dd_agent_host: str):
        ...
        agent_port = os.getenv("LOVE_ENGINE_DD_LLM_OBS_PORT", "8126")
    ...
    def _get_datadog_llm_obs_params(self) -> Dict:
        """
        Get the datadog_llm_observability_params from love_engine.datadog_llm_observability_params
        ...
        if love_engine.datadog_llm_observability_params is not None:
            if isinstance(
                love_engine.datadog_llm_observability_params, DatadogLLMObsInitParams
            ):
                dict_datadog_llm_obs_params = (
                    love_engine.datadog_llm_observability_params.model_dump()
                )
            elif isinstance(love_engine.datadog_llm_observability_params, Dict):
                # only allow params that are of DatadogLLMObsInitParams
                dict_datadog_llm_obs_params = DatadogLLMObsInitParams(
                    **love_engine.datadog_llm_observability_params
                ).model_dump()
    ...
    def create_llm_obs_payload(
        ...
        metadata = kwargs.get("love_engine_params", {}).get("metadata", {})
        ...
        payload: LLMObsPayload = LLMObsPayload(
            ...
            name=metadata.get("name", "love_engine_llm_call"),
            meta=meta,
    ...
    def _get_datadog_span_kind(
        ...
        """
        Map love_engine call_type to appropriate DataDog LLM Observability span kind.
    ...
    def _get_dd_llm_obs_payload_metadata(
        ...
        """
        Fields to track in DD LLM Observability metadata from love_engine standard logging payload
        """
    ...
    def _get_latency_metrics(
        ...
        # love_engine overhead time
        hidden_params = standard_logging_payload.get("hidden_params", {})
        love_engine_overhead_ms = hidden_params.get("love_engine_overhead_time_ms")
        if love_engine_overhead_ms is not None:
            latency_metrics["love_engine_overhead_time_ms"] = love_engine_overhead_ms
