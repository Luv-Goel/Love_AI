...
from love_engine.love_engine_core_utils.core_helpers import redact_nested_match_and_regex_keys
...
# honors markers carrying this token, so a caller cannot love_smith the metadata
...
def get_session_id_from_request_data(request_data: Dict[str, Any]) -> Optional[str]:
    """Extract session_id from request data (love_engine_session_id or metadata)."""
    session_id = request_data.get("love_engine_session_id")
    if session_id:
...
class CustomGuardrail(CustomLogger):
    ...
    def _get_admin_metadata(data: dict) -> dict:
        ...
        ``love_engine_metadata`` depending on endpoint. Check both so a caller
    ...
    def _is_valid_response_type(self, result: Any) -> bool:
        ...
        For non-love_engine responses (like passthrough httpx.Response), returns True
    ...
    def get_guardrail_from_metadata(
        ...
        # "love_engine_metadata". We check the one that actually contains the "guardrails"
        # key so that a non-empty love_engine_metadata without guardrails does not shadow
        ...
        for meta_key in ("metadata", "love_engine_metadata"):
    ...
    async def async_pre_call_deployment_hook(
        ...
        love_engine_guardrails = kwargs.get("guardrails")
        if love_engine_guardrails is None or not isinstance(love_engine_guardrails, list):
            return kwargs
    ...
    async def async_post_call_success_deployment_hook(
        ...
        # should run guardrail
        love_engine_guardrails = request_data.get("guardrails")
        if love_engine_guardrails is None or not isinstance(love_engine_guardrails, list):
            return response
        ...
        if self.default_on is True and disable_global_guardrail is not True:
            if self._event_hook_is_event_type(event_type):
                if isinstance(self.event_hook, Mode):
                    try:
                        from love_engine_enterprise.integrations.custom_guardrail import (
                            EnterpriseCustomGuardrailHelper,
                        )
                    except ImportError:
                        raise ImportError(
                            "Setting tag-based guardrails is only available in love_engine-enterprise. You must be a premium user to use this feature."
                        )
    ...
    def get_guardrail_dynamic_request_body_params(self, request_data: dict) -> dict:
        ...
        Args:
            request_data: The original `request_data` passed to love_engine Proxy
        """
    ...
    def add_standard_logging_guardrail_information_to_request_data(
        ...
        from love_engine.types.utils import GuardrailMode
        ...
        elif "love_engine_metadata" in request_data:
            _append_guardrail_info(request_data["love_engine_metadata"])
        else:
        ...
        try:
            from love_engine.integrations.otel.logger import emit_guardrail_span
    ...
    def update_in_memory_love_engine_params(self, love_engine_params: LoveEngineParams) -> None:
        """
        Update the guardrails love_engine params in memory
        """
        for key, value in vars(love_engine_params).items():
    ...
    def get_guardrails_messages_for_call_type(
        ...
        #########################################################
        # /responses
        # User/System messages are stored in the "input" key, use love_engine transformation to get the messages
        #########################################################
        if (
...
def log_guardrail_information(func):
    ...
    def _count_recorded_guardrail_entries(request_data: dict) -> int:
        total = 0
        for container_key in ("metadata", "love_engine_metadata"):
