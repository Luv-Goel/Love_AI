...
from love_engine.integrations._types.open_inference import (
    OpenInferenceSpanKindValues,
    SpanAttributes,
)
from love_engine.integrations.custom_logger import CustomLogger
from love_engine.integrations.opentelemetry_utils.gen_ai_semconv import (
...
from love_engine.types.services import ServiceLoggerPayload
...
if TYPE_CHECKING:
    ...
    from love_engine.proxy.proxy_server import UserAPIKeyAuth as _UserAPIKeyAuth
...
LOVE_ENGINE_TRACER_NAME = os.getenv("OTEL_TRACER_NAME", "love_engine")
LOVE_ENGINE_METER_NAME = os.getenv("LOVE_ENGINE_METER_NAME", "love_engine")
LOVE_ENGINE_LOGGER_NAME = os.getenv("LOVE_ENGINE_LOGGER_NAME", "love_engine")
LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME = "Received Proxy Server Request"
...
PREPROCESSING_DURATION_MS_ATTRIBUTE = "love_engine.preprocessing.duration_ms"
TEAM_METADATA_ATTRIBUTE = "love_engine.team.metadata"
MODEL_GROUP_ATTRIBUTE = "love_engine.model_group"
PROVIDER_MODEL_ATTRIBUTE = "love_engine.provider.model"
# Remove the hardcoded LOVE_ENGINE_RESOURCE dictionary - we'll create it properly later
RAW_REQUEST_SPAN_NAME = "raw_gen_ai_request"
LOVE_ENGINE_REQUEST_SPAN_NAME = "love_engine_request"
...
class OpenTelemetryConfig:
    ...
    # under ``love_engine.team.metadata``. Empty by default so none of a team's
    ...
    def __post_init__(self) -> None:
        ...
        if not self.service_name:
            self.service_name = os.getenv("OTEL_SERVICE_NAME", "love_engine")
        ...
        self.baggage_team_metadata_keys = _normalize_team_metadata_keys(
            ...
            os.getenv("LOVE_ENGINE_OTEL_BAGGAGE_TEAM_METADATA_KEYS")
        )
    ...
    def from_env(cls):
        ...
        enable_metrics: bool = (
            os.getenv("LOVE_ENGINE_OTEL_INTEGRATION_ENABLE_METRICS", "false").lower()
            == "true"
        )
        enable_events: bool = (
            os.getenv("LOVE_ENGINE_OTEL_INTEGRATION_ENABLE_EVENTS", "false").lower()
            == "true"
        )
        service_name = os.getenv("OTEL_SERVICE_NAME", "love_engine")
...
class OpenTelemetry(OTELGenAISemconvMixin, CustomLogger):
    def __init__(
        ...
        **kwargs,
        ...
        self._init_otel_logger_on_love_engine_proxy()

    @staticmethod
    def _get_love_engine_resource(config: OpenTelemetryConfig):
    ...
    def _init_otel_logger_on_love_engine_proxy(self):
        """
        Initializes OpenTelemetry for love_engine proxy server

        - Adds Otel as a service callback
        - Sets `proxy_server.open_telemetry_logger` to self
        """
        try:
            from love_engine.proxy import proxy_server
        except ImportError:
        ...
        if "otel" not in love_engine.service_callback and all(
            not isinstance(cb, OpenTelemetry) for cb in love_engine.service_callback
        ):
            love_engine.service_callback.append(self)
    ...
    def _resolve_capture_mode(self) -> str:
        ...
        Precedence:
          1. ``love_engine.turn_off_message_logging=True`` forces ``NO_CONTENT``
    ...
    def _init_tracing(self, tracer_provider):
        ...
        def create_tracer_provider():
            provider = TracerProvider(resource=self._get_love_engine_resource(self.config))
        ...
        self.tracer = tracer_provider.get_tracer(LOVE_ENGINE_TRACER_NAME)
    ...
    def _init_metrics(self, meter_provider):
        ...
        def create_meter_provider():
            ...
            return MeterProvider(
                ...
                resource=self._get_love_engine_resource(self.config),
            )
        ...
        self._response_duration_histogram = meter.create_histogram(
            ...
            description="Total LLM API generation time (excludes love_engine overhead)",
            unit="s",
        )
                ...
                for key, value in event_metadata.items():
                    ...
                    if isinstance(value, dict):
                        try:
                            ...
                            value = "love_engine logging error - could_not_json_serialize"
                    self.safe_set_attribute(
                        span=service_logging_span,
                        key=key,
                        value=value,
    ...
    async def async_post_call_failure_hook(
        ...
        if parent_otel_span is not None:
            ...
            # records on the love_engine_request child span). Inline import:
            # love_engine_logging <-> integrations is circular.
    ...
    def _emit_guardrail_spans_from_request_data(
        ...
        # kwargs["love_engine_params"]["metadata"]["_otel_internal"]. Pass the
    ...
    async def async_post_call_success_hook(
        ...
        love_engine_logging_obj = data.get("love_engine_logging_obj")

        if love_engine_logging_obj is not None and isinstance(
            love_engine_logging_obj, LoveEngineLogging
        ):
            kwargs = love_engine_logging_obj.model_call_details
    ...
    def _get_tracer_with_dynamic_headers(self, dynamic_headers: dict):
        ...
        if cache_key in self._tracer_provider_cache:
            return self._tracer_provider_cache[cache_key].get_tracer(
                LOVE_ENGINE_TRACER_NAME
            )

        # Create a temporary tracer provider with dynamic headers
        temp_provider = TracerProvider(resource=self._get_love_engine_resource(self.config))
        ...
        return temp_provider.get_tracer(LOVE_ENGINE_TRACER_NAME)
    ...
    def _emit_once(self, kwargs: dict, *scope: object) -> bool:
        ...
        1. **Handler-level dual-fire**: streaming code paths trigger both
           ...
           ``love_engine_request`` spans. Scope: ``("success",)`` / ``("failure",)``.
        ...
        in ``kwargs["love_engine_params"]["metadata"]["_otel_internal"]`` so it
        ...
        if not isinstance(love_engine_params, dict):
            love_engine_params = {}
            kwargs["love_engine_params"] = love_engine_params

        _metadata = love_engine_params.get("metadata")
        if not isinstance(_metadata, dict):
            _metadata = {}
            love_engine_params["metadata"] = _metadata
    ...
    def _end_proxy_span_from_kwargs(self, kwargs: dict, end_time) -> None:
        ...
        Only spans named ``LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME`` are closed —
        externally provided spans must not be closed by love_engine.
        """
        love_engine_params = kwargs.get("love_engine_params", {}) or {}
        _metadata = love_engine_params.get("metadata", {}) or {}
        proxy_span = _metadata.get("love_engine_parent_otel_span", None)

        # Fallback: check love_engine_metadata (used by /v1/messages and other
        # LOVE_ENGINE_METADATA_ROUTES).
        if proxy_span is None:
            _love_engine_metadata = love_engine_params.get("love_engine_metadata", {}) or {}
            proxy_span = _love_engine_metadata.get("love_engine_parent_otel_span", None)

        if (
            proxy_span is not None
            and getattr(proxy_span, "name", None) == LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME
    ...
    def _handle_success(self, kwargs, response_obj, start_time, end_time):
        """Create the love_engine_request span then close the proxy span."""
        verbose_logger.debug(
        ...
        # OR if USE_OTEL_LOVE_ENGINE_REQUEST_SPAN is explicitly enabled
        should_create_primary_span = parent_span is None or get_secret_bool(
            "USE_OTEL_LOVE_ENGINE_REQUEST_SPAN"
        )

        if should_create_primary_span:
            # Create a new love_engine_request span
            span = self._start_primary_span(
                kwargs, response_obj, start_time, end_time, ctx
            )
            # Raw-request sub-span (if enabled) - child of love_engine_request span
            ...
            # The child love_engine_request span already carries all attributes;
        ...
        # External spans (from Langfuse, user code, HTTP headers, global context) must not be closed by love_engine
        # However, proxy-created spans should be closed here.
        if (
            parent_span is not None
            and hasattr(parent_span, "name")
            and parent_span.name == LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME
        ...
        # after all child spans (love_engine_request, guardrail, raw_request)
    ...
    def _set_team_attributes_on_span(
        ...
        love_engine_request trace carries them, not just the root span.
    ...
    def _set_team_attributes_on_proxy_span_from_kwargs(self, kwargs: dict) -> None:
        ...
        Guarded to the love_engine-created proxy span (by name + recording) so
        externally provided parent spans are never mutated.
        """
        love_engine_params = kwargs.get("love_engine_params") or {}
        metadata = love_engine_params.get("metadata") or {}
        proxy_span = metadata.get("love_engine_parent_otel_span")
        if (
            proxy_span is not None
            and getattr(proxy_span, "name", None) == LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME
            and hasattr(proxy_span, "is_recording")
    ...
    def _set_inference_identity_attributes(
        ...
        # ``metadata`` and ``love_engine_metadata`` are alternate names for the same
        # full metadata dict (the name varies by endpoint), so first-truthy wins.
        raw_metadata = (
            love_engine_params.get("metadata")
            or love_engine_params.get("love_engine_metadata")
            or {}
        )
    ...
    def _ensure_metric_attribute_filter(self) -> None:
        ...
        if attributes is None and self.callback_name in (None, "otel"):
            otel_settings = (love_engine.callback_settings or {}).get("otel") or {}
            raw = (
    ...
    def _record_metrics(self, kwargs, response_obj, start_time, end_time):
        ...
        params = kwargs.get("love_engine_params") or {}
    ...
    def _record_time_to_first_token_metric(self, kwargs: dict, common_attrs: dict):
        ...
        # This excludes love_engine overhead and measures pure LLM API latency
    ...
    def _record_response_duration_metric(
        ...
        This excludes love_engine overhead and measures only the LLM provider's response time.
        Works for both streaming and non-streaming requests.

        Mirrors Prometheus's love_engine_llm_api_latency_metric.
    ...
    def _emit_semantic_logs(self, kwargs, response_obj, span: Span):
        ...
        otel_logger = self._logger_provider.get_logger(LOVE_ENGINE_LOGGER_NAME)

        parent_ctx = span.get_span_context()
        provider = (kwargs.get("love_engine_params") or {}).get(
    ...
    def _resolve_guardrail_context(
        ...
        never orphaned (Issue #5).  Priority:
          1. The love_engine_request span that was just created
    ...
    def _handle_failure(self, kwargs, response_obj, start_time, end_time):
        ...
        span = None
        if should_create_primary_span:
            # Span 1: Request sent to love_engine SDK
            otel_tracer: Tracer = self.get_tracer_to_use_for_request(kwargs)
            span_kwargs: Dict[str, Any] = {
                "name": self._get_span_name(kwargs),
                "start_time": self._to_ns(start_time),
            ...
            # When parent span exists and USE_OTEL_LOVE_ENGINE_REQUEST_SPAN=false,
        ...
        if (
            ...
            and parent_otel_span.name == LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME
        ):
    ...
    def set_tools_attributes(self, span: Span, tools):
        ...
        from love_engine.proxy._types import SpanAttributes
    ...
    def set_attributes(self, span: Span, kwargs, response_obj: Optional[Any]):
        try:
            if self.callback_name == "langtrace":
                from love_engine.integrations.langtrace import LangtraceAttributes
                ...
                from love_engine.integrations.langfuse.langfuse_otel import (
                    LangfuseOtelLogger,
                )
            ...
            provider_name = love_engine_params.get("custom_llm_provider", "Unknown")
            ...
            # the love_engine call ID so every call type can be correlated
            # across love_engine UI, Phoenix traces, and provider logs (Issue #8).
            response_id = (
            ...
            love_engine_call_id = standard_logging_payload.get("love_engine_call_id")
            if love_engine_call_id:
                self.safe_set_attribute(
                    span=span,
                    key="love_engine.call_id",
                    value=love_engine_call_id,
                )
    ...
    def _transform_messages_to_otel_semantic_conventions(
        ...
        """
        Transforms love_engine/OpenAI style messages into OTEL GenAI 1.38 compliant format.
    ...
    def _to_dict(obj) -> Optional[dict]:
        ...
        2. love_engine's ``BaseLoveEngineOpenAIResponseObject`` — exposes a
        ...
        if isinstance(obj, dict):
            return obj
        if hasattr(obj, "get"):
            # BaseLoveEngineOpenAIResponseObject duck-type
    ...
    def _transform_responses_api_output_to_otel(self, output: List) -> List[dict]:
        ...
        Items may be plain dicts, love_engine wrapper objects (with ``.get()``),
    ...
    def set_raw_request_attributes(self, span: Span, kwargs, response_obj):
        try:
            ...
            # The parent love_engine_request span already carries the standard
            # gen_ai.* / metadata.* attributes — duplicating them here doubles
            # storage and adds noise (Issue #3).
            love_engine_params = kwargs.get("love_engine_params", {}) or {}
            custom_llm_provider = love_engine_params.get("custom_llm_provider", "Unknown")
            ...
            if _raw_response and isinstance(_raw_response, str):
                ...
                try:
                    ...
                    verbose_logger.debug(
                        "love_engine.integrations.opentelemetry.py::set_raw_request_attributes() - raw_response not json string - {}".format(
                            _raw_response
                        )
                    )
    ...
    def _get_span_name(self, kwargs):
        ...
        return LOVE_ENGINE_REQUEST_SPAN_NAME
    ...
    def _get_span_context(self, kwargs, default_span: Optional[Span] = None):
        ...
        _metadata = love_engine_params.get("metadata", {}) or {}
        parent_otel_span = _metadata.get("love_engine_parent_otel_span", None)

        # Fallback: check love_engine_metadata (used by /v1/messages and other
        # LOVE_ENGINE_METADATA_ROUTES that store proxy-internal metadata
        # separately from the provider's native "metadata" field).
        if parent_otel_span is None:
            _love_engine_metadata = love_engine_params.get("love_engine_metadata", {}) or {}
            parent_otel_span = _love_engine_metadata.get("love_engine_parent_otel_span", None)
        ...
        if self.OTEL_EXPORTER == "console":
            ...
            try:
                ...
                raise ImportError(
                    ...
                    "`opentelemetry-exporter-otlp` and `grpcio` (or `love_engine[grpc]`)."
                ) from exc
    ...
    def create_love_engine_proxy_request_started_span(
        self,
        start_time: datetime,
        headers: dict,
    ) -> Optional[Span]:
        ...
        return self.tracer.start_span(
            name=LOVE_ENGINE_PROXY_REQUEST_SPAN_NAME,
            start_time=self._to_ns(start_time),
    ...
    def set_preprocessing_duration_attribute(
        ...
        """
        Set ``love_engine.preprocessing.duration_ms`` (proxy-receive -> first
        provider handoff) on the proxy SERVER span. ``love_engine_received_at``
        ...
        _lp = container.get("love_engine_params")
        for _md in (
            (_lp or {}).get("metadata") if isinstance(_lp, dict) else None,
            container.get("metadata"),
            container.get("love_engine_metadata"),
        ):
            if isinstance(_md, dict):
                received_at = received_at or _md.get("love_engine_received_at")
