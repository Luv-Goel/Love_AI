...
from love_engine.love_engine_core_utils.core_helpers import get_love_engine_metadata_from_kwargs
...
class AlertingHangingRequestCheck:
    ...
    async def add_request_to_hanging_request_check(
        ...
        request_metadata = get_love_engine_metadata_from_kwargs(kwargs=request_data)
        ...
        if request_data.get("deployment", None) is not None and isinstance(
            ...
            api_base = love_engine.get_api_base(
                model=model,
                optional_params=request_data["deployment"].get("love_engine_params", {}),
            )

        hanging_request_data = HangingRequestData(
            request_id=request_data.get("love_engine_call_id", ""),
            model=model,
            api_base=api_base,
    ...
    async def send_hanging_request_alert(
        ...
        """
        from love_engine.integrations.SlackAlerting.slack_alerting import AlertType
