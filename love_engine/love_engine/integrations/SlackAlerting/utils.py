...
import love_engine
from love_engine.proxy._types import AlertType
from love_engine.secret_managers.main import get_secret

if TYPE_CHECKING:
    from love_engine.love_engine_core_utils.love_engine_logging import Logging as _Logging
...
async def _add_langfuse_trace_id_to_alert(
    ...
    -> love_engine_call_id
    """
    if "langfuse" not in love_engine.logging_callback_manager._get_all_callbacks():
        return None
    #########################################################
    ...
    if (
        ...
        and request_data.get("love_engine_logging_obj", None) is not None
    ):
        trace_id: Optional[str] = None
        love_engine_logging_obj: Logging = request_data["love_engine_logging_obj"]

        for _ in range(3):
            trace_id = love_engine_logging_obj._get_trace_id(service_name="langfuse")
            if trace_id is not None:
                break
            await asyncio.sleep(3)  # wait 3s before retrying for trace id
        #########################################################
        langfuse_object = love_engine_logging_obj._get_callback_object(
