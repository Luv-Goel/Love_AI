...
from love_engine.constants import HOURS_IN_A_DAY
from love_engine.integrations.custom_batch_logger import CustomBatchLogger
from love_engine.integrations.SlackAlerting.budget_alert_types import get_budget_alert_type
from love_engine.integrations.SlackAlerting.hanging_request_check import (
    AlertingHangingRequestCheck,
)
from love_engine.love_engine_core_utils.duration_parser import duration_in_seconds
from love_engine.love_engine_core_utils.exception_mapping_utils import (
...
from love_engine.types.integrations.slack_alerting import *
...
class SlackAlerting(CustomBatchLogger):
    ...
    def _response_taking_too_long_callback_helper(
        ...
        try:
            ...
            api_base = love_engine.get_api_base(model=model, optional_params=love_engine_params)
    ...
    async def response_taking_too_long_callback(
        ...
        if love_engine.turn_off_message_logging or love_engine.redact_messages_in_exceptions:
            messages = "Message not logged. love_engine.redact_messages_in_exceptions=True"
        request_info = f"\nRequest Model: `{model}`\nAPI Base: `{api_base}`\nMessages: `{messages}`"
        ...
        if time_difference_float > self.alerting_threshold:
            ...
            if (
                kwargs is not None
                and "love_engine_params" in kwargs
                and "metadata" in kwargs["love_engine_params"]
            ):
                _metadata: dict = kwargs["love_engine_params"]["metadata"]
    ...
    async def send_daily_reports(self, router) -> bool:
        ...
        # format alert -> return the love_engine model name + api base
        ...
        for i in range(len(top_5_failed)):
            ...
            if isinstance(_deployment, dict):
                deployment_name = _deployment["love_engine_params"].get("model", "")
            else:
                return False

            api_base = love_engine.get_api_base(
                model=deployment_name,
                optional_params=(
                    _deployment["love_engine_params"] if _deployment is not None else {}
                ),
            )
            if api_base is None:
                api_base = ""
    ...
    async def budget_alerts(
        ...
        """
        ## PREVENTITIVE ALERTING ## - https://github.com/BerriAI/love_engine/issues/2727
    ...
    def _get_user_info_str(self, user_info: CallInfo) -> str:
        ...
        for k, v in _all_fields_as_dict.items():
            if isinstance(v, LOVE_ENGINE_EntityType):
                v = v.value
            msg += f"*{k}:* `{v}`\n"
    ...
    async def customer_spend_alert(
        ...
        if (
            ...
            event = WebhookEvent(
                ...
                event="spend_tracked",
                event_group=LOVE_ENGINE_EntityType.END_USER,
...
"""
    ...
    async def region_outage_alerts(
        ...
        ### GET PROVIDER ###
        provider = deployment.love_engine_params.custom_llm_provider
        if provider is None:
            model, provider, _, _ = love_engine.get_llm_provider(model=model)

        ### GET REGION ###
        region_name = deployment.love_engine_params.region_name
        if region_name is None:
            region_name = love_engine.utils._get_model_region(
                custom_llm_provider=provider, love_engine_params=deployment.love_engine_params
            )
        ...
        try:
            ...
            if provider is None:
                try:
                    ...
                    provider = ""
            api_base = love_engine.get_api_base(
                model=model, optional_params=deployment.love_engine_params
            )
    ...
    async def model_added_alert(
        self, model_name: str, love_engine_model_name: str, passed_model_info: Any
    ):
        base_model_from_user = getattr(passed_model_info, "base_model", None)
        model_info = {}
        base_model = ""
        if base_model_from_user is not None:
            model_info = love_engine.model_cost.get(base_model_from_user, {})
            base_model = f"Base Model: `{base_model_from_user}`\n"
        else:
            model_info = love_engine.model_cost.get(love_engine_model_name, {})
        model_info_str = ""
...
"""
    ...
    async def send_key_created_or_user_invited_email(
        ...
        try:
            from love_engine.proxy.utils import send_email
            ...
            if email_logo_url is None:
                email_logo_url = LOVE_ENGINE_LOGO_URL
            if email_support_contact is None:
                email_support_contact = LOVE_ENGINE_SUPPORT_CONTACT
            ...
            email_html_content = "Alert from love_engine Server"
            ...
            email_event = {
                "to": recipient_email,
                "subject": f"love_engine: {event_name}",
    ...
    async def send_email_alert_using_smtp(
        ...
        if webhook_event.event == "budget_crossed":
            ...
            <img src="{email_logo_url}" alt="love_engine Logo" width="150" height="50" />
        ...
        if webhook_event.event_group == LOVE_ENGINE_EntityType.TEAM:
            from love_engine.integrations.email_alerting import send_team_budget_alert
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            if "daily_reports" in self.alert_types:
                ...
                if isinstance(response_obj, love_engine.ModelResponse) and (
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _love_engine_params = kwargs.get("love_engine_params", {})
        _model_info = _love_engine_params.get("model_info", {}) or {}
    ...
    async def _run_scheduler_helper(self, llm_router) -> bool:
        ...
        if report_sent is None:
            ...
            if current_time - report_sent >= interval_seconds:
                ...
                # Also, don't love_smitht to update the report_sent time after sending the report!
                await self.internal_usage_cache.async_set_cache(
                    key=SlackAlertingCacheKeys.report_sent_key.value,
                    value=current_time,
                )
    ...
    async def _request_is_completed(self, request_data: Optional[dict]) -> bool:
        ...
        if (
            request_data.get("love_engine_status", "") != "success"
            and request_data.get("love_engine_status", "") != "fail"
        ):
            ## CHECK IF CACHE IS UPDATED
            love_engine_call_id = request_data.get("love_engine_call_id", "")
            status: Optional[str] = await self.internal_usage_cache.async_get_cache(
                key="request_status:{}".format(love_engine_call_id), local_only=True
            )
