"""Rubrik love_engine Plugin for tool blocking and batch logging."""
...
_WEBHOOK_PATH_LOGGING_BATCH = "/v1/love_engine/batch"
...
class RubrikLogger(CustomGuardrail, CustomBatchLogger):
    ...
    def _extract_request_data(call_details: dict[str, Any]) -> dict[str, Any]:
        ...
        if not call_details:
            return {}
        love_engine_params = call_details.get("love_engine_params", {}) or {}
        return {
    ...
    async def _prepare_log_payload(
        ...
        # For Anthropic /v1/messages requests, love_engine creates a separate
        ...
        # Normalize to love_engine_call_id so that the logging and tool-blocking
        # endpoints see the same request identifier.
        love_engine_params = kwargs.get("love_engine_params", {}) or {}
        proxy_request = love_engine_params.get("proxy_server_request", {}) or {}
        url_path = urllib.parse.urlparse(proxy_request.get("url", "")).path
        if url_path.endswith(_ENDPOINT_ANTHROPIC_MESSAGES):
            _love_engine_call_id = kwargs.get("love_engine_call_id")
            if _love_engine_call_id:
                standard_logging_payload["id"] = _love_engine_call_id  # type: ignore[literal-required]
