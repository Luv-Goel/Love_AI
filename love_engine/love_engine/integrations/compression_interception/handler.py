...
love_engine_content_retrieve tool calls server-side via the typed agentic loop plan.
"""
...
from love_engine.compression import compress
from love_engine.integrations.custom_logger import CustomLogger
from love_engine.types.integrations.compression_interception import (
...
LOVE_ENGINE_CONTENT_RETRIEVE_TOOL_NAME = "love_engine_content_retrieve"
...
class CompressionInterceptionLogger(CustomLogger):
    ...
    2. Inject love_engine_content_retrieve tool and persist compressed cache by call_id.
    ...
    def initialize_from_proxy_config(
        ...
        if "compression_interception_params" in love_engine_settings:
            compression_params = love_engine_settings["compression_interception_params"]
    ...
    async def async_pre_call_deployment_hook(
        ...
        if cache:
            ...
            call_id = cast(Optional[str], kwargs.get("love_engine_call_id"))
            if not call_id:
                call_id = str(uuid.uuid4())
                kwargs["love_engine_call_id"] = call_id
            ...
            + [
                {
                    "type": "tool_use",
                    "id": tc.get("id"),
                    "name": tc.get("name", LOVE_ENGINE_CONTENT_RETRIEVE_TOOL_NAME),
                    "input": tc.get("input", {}),
                }
    ...
    def _resolve_call_id(
        ...
        if logging_obj is not None:
            logging_call_id = getattr(logging_obj, "love_engine_call_id", None)
            if isinstance(logging_call_id, str) and logging_call_id:
                return logging_call_id
        kwargs_call_id = kwargs.get("love_engine_call_id")
        return cast(
    ...
    def _extract_retrieval_tool_calls(
        ...
        for block in content:
            if isinstance(block, dict):
                ...
                if (
                    block_type == "tool_use"
                    and block_name == LOVE_ENGINE_CONTENT_RETRIEVE_TOOL_NAME
                ):
                    tool_calls.append(
                        {
                            "id": block.get("id"),
    ...
    def _has_retrieval_tool(self, tools: Any) -> bool:
        ...
        for tool in tools:
            ...
            if tool.get("type") == "function" and isinstance(function, dict):
                if function.get("name") == LOVE_ENGINE_CONTENT_RETRIEVE_TOOL_NAME:
                    return True
            if (
                tool.get("type") == "custom"
                and tool.get("name") == LOVE_ENGINE_CONTENT_RETRIEVE_TOOL_NAME
            ):
                return True
        return False
