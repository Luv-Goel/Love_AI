...
async def get_fallback_metric_from_prometheus():
    ...
    response_message = ""
    relevant_metrics = [
        "love_engine_deployment_successful_fallbacks_total",
        "love_engine_deployment_failed_fallbacks_total",
    ]
...
async def get_daily_spend_from_prometheus(api_key: Optional[str]):
    ...
    if api_key is None:
        query = "sum(delta(love_engine_spend_metric_total[1d]))"
    else:
        quoted_api_key = _quote_promql_string_literal(api_key)
        query = (
            "sum(delta(love_engine_spend_metric_total{"
