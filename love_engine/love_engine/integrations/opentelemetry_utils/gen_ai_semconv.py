...
default love_engine output:

Request span:

- name is ``{operation} {model}`` (e.g. ``chat gpt-4``) instead of
  ``love_engine_request``; span kind is ``CLIENT``.
...
# love_engine optional_params key -> OTEL gen_ai semconv span attribute.
...
class OTELGenAISemconvMixin:
    ...
    - ``_gen_ai_operation_name`` -- love_engine ``call_type`` -> spec
    ...
    def _gen_ai_operation_name(kwargs: dict) -> str:
        """Map a love_engine ``call_type`` to spec ``gen_ai.operation.name``.
