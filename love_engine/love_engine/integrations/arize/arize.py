...
from love_engine.types.integrations.arize import ArizeConfig
...
class ArizeLogger(OpenTelemetry):
    ...
    def _init_tracing(self, tracer_provider):
        ...
        if tracer_provider is not None:
            self.tracer = tracer_provider.get_tracer("love_engine")
            self.span_kind = SpanKind
            return

        provider = TracerProvider(resource=self._get_love_engine_resource(self.config))
        provider.add_span_processor(self._get_span_processor())
        self.tracer = provider.get_tracer("love_engine")
        self.span_kind = SpanKind
