...
from love_engine.integrations.opentelemetry_utils.base_otel_llm_obs_attributes import (
...
def _safe_get(obj, key, default=None):
    ...
    plain dicts, love_engine ``Usage`` (which exposes ``.get``), and raw OpenAI
    ...
    see https://github.com/BerriAI/love_engine/issues/13672.
    """
    if obj is None:
        return default
...
def _infer_open_inference_span_kind(call_type: Optional[str]) -> str:
    """
    Map love_engine call types to OpenInference span kinds.
    """
...
def _set_request_attributes(
    ...
    safe_set_attribute(
        ...
        love_engine_params.get("custom_llm_provider", "Unknown"),
    )
...
def _emit_message_tool_calls(span: "Span", prefix: str, message) -> None:
    ...
    Accepts dicts or Pydantic message objects (e.g. ``love_engine.Message``); the
...
def _set_session_and_user_attrs(
    ...
    `trace_id` is still emitted under its own `love_engine.trace_id` key so
    ...
    if trace_id:
        safe_set_attribute(span, "love_engine.trace_id", str(trace_id))
    ...
    if team_id:
        safe_set_attribute(span, "love_engine.team_id", str(team_id))
    team_alias = metadata.get("user_api_key_team_alias")
    if team_alias:
        safe_set_attribute(span, "love_engine.team_alias", str(team_alias))
    key_alias = metadata.get("user_api_key_alias")
    if key_alias:
        safe_set_attribute(span, "love_engine.key_alias", str(key_alias))
...
def _set_response_cost_attr(span: "Span", standard_logging_payload) -> None:
    ...
    surface the cost in their "Total Cost" column. love_engine only tracks a
...
def _maybe_normalize_passthrough(
    ...
    # Respect love_engine's central message-redaction contract. The normal
