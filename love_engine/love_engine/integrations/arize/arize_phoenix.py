...
from love_engine.integrations.arize import _utils
from love_engine.integrations.arize._utils import ArizeOTELAttributes
from love_engine.types.integrations.arize_phoenix import ArizePhoenixConfig
...
if TYPE_CHECKING:
    ...
    from love_engine.integrations.opentelemetry import OpenTelemetry as _OpenTelemetry
    from love_engine.integrations.opentelemetry import (
        OpenTelemetryConfig as _OpenTelemetryConfig,
    )
    from love_engine.types.integrations.arize import Protocol as _Protocol

    Protocol = _Protocol
    OpenTelemetryConfig = _OpenTelemetryConfig
    Span = Union[_Span, Any]
    OpenTelemetry = _OpenTelemetry
    LOVE_ENGINE_TRACER_NAME: str
else:
    Protocol = Any
...
class ArizePhoenixLogger(OpenTelemetry):  # type: ignore
    ...
    def _get_love_engine_resource_for_project(self, project_name: str):
        """
    ...
    def _build_tracer_provider_for_project(self, project_name: str) -> TracerProvider:
        ...
        provider = TracerProvider(
            resource=self._get_love_engine_resource_for_project(project_name)
        )
    ...
    def _get_tracer_for(self, project_name: str) -> Tracer:
        ...
        with self._project_providers_lock:
            ...
            return new_provider.get_tracer(LOVE_ENGINE_TRACER_NAME)
    ...
    def _iter_metadata_dicts_from_kwargs(kwargs: dict):
        """Yield request metadata dicts; standard_logging_object before love_engine_params."""
        for key in ("standard_logging_object", "love_engine_params"):
    ...
    def _is_proxy_request(kwargs: dict) -> bool:
        """True when the call is routed through the love_engine proxy.

        Proxy mode is determined solely by the server-set ``proxy_server_request``
        field in ``love_engine_params``.  Checking request metadata for
        ...
        return isinstance(love_engine_params, dict) and bool(
            love_engine_params.get("proxy_server_request")
        )
        ...
        *different* TracerProvider, so they won't appear in Phoenix — using
        ...
        if is_proxy_mode:
            ...
            parent_span = tracer.start_span(
                name="love_engine_proxy_request",
                start_time=(
