"""
Arize Phoenix prompt manager that integrates with love_engine's prompt management system.
...
class ArizePhoenixPromptManager(CustomPromptManagement):
    ...
    love_engine completion() function by implementing the PromptManagementBase interface.
    ...
    def pre_call_hook(
        ...
        **kwargs,
        ...
        """
        if not prompt_id:
            return messages, love_engine_params
        ...
        try:
            ...
            # Update love_engine_params with prompt metadata
            ...
            if prompt_metadata.get("model") and not self.ignore_prompt_manager_model:
                love_engine_params["model"] = prompt_metadata["model"]
            ...
            if not self.ignore_prompt_manager_optional_params:
                for param in [
                    ...
                    if param in prompt_metadata:
                        love_engine_params[param] = prompt_metadata[param]

            return final_messages, love_engine_params

        except Exception as e:
            # Log error but don't fail the call
            import love_engine

            love_engine._logging.verbose_proxy_logger.error(
