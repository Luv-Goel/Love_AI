...
from love_engine.integrations.langfuse.langfuse_otel_attributes import (
    LangfuseLLMObsOTELAttributes,
)
from love_engine.integrations.opentelemetry import OpenTelemetry, OpenTelemetryConfig
from love_engine.types.integrations.langfuse_otel import (
...
class LangfuseOtelLogger(OpenTelemetry):
    ...
    def _extract_langfuse_metadata(kwargs: dict) -> dict:
        """
        Extracts Langfuse metadata from the standard love_engine kwargs structure.

        1. Reads kwargs["love_engine_params"]["metadata"] if present and is a dict.
        ...
        try:
            from love_engine.integrations.langfuse.langfuse import (
                LangFuseLogger as _LFLogger,
            )

            metadata = _LFLogger.add_metadata_from_header(love_engine_params, metadata)  # type: ignore
        except Exception:
    ...
    def _set_metadata_attributes(span: Span, metadata: dict):
        ...
        from love_engine.integrations.arize._utils import safe_set_attribute
        ...
        for key, enum_attr in mapping.items():
            if key in metadata and metadata[key] is not None:
                ...
                if key == "trace_id" and isinstance(value, str):
                    # trace_id must be 32 hex char no dashes for langfuse : love_engine sends uuid with dashes (might be breaking at some point)
                    value = value.replace("-", "")
