...
from love_engine.constants import MAX_LANGFUSE_INITIALIZED_CLIENTS
from love_engine.love_engine_core_utils.core_helpers import (
    safe_deep_copy,
    reconstruct_model_name,
    filter_exceptions_from_params,
)
from love_engine.love_engine_core_utils.redact_messages import redact_user_api_key_info
from love_engine.integrations.langfuse.langfuse_mock_client import (
    create_mock_langfuse_client,
    should_use_langfuse_mock,
)
from love_engine.llms.custom_httpx.http_handler import _get_httpx_client
from love_engine.secret_managers.main import str_to_bool
from love_engine.types.integrations.langfuse import *
from love_engine.types.llms.openai import HttpxBinaryResponseContent, ResponsesAPIResponse
...
if TYPE_CHECKING:
    ...
    from love_engine.love_engine_core_utils.love_engine_logging import DynamicLoggingCache
else:
...
def _extract_cache_read_input_tokens(usage_obj) -> int:
    ...
    See: https://github.com/BerriAI/love_engine/issues/18520
...
class LangFuseLogger:
    ...
    def __init__(
        ...
        if Version(self.langfuse_sdk_version) >= Version("2.6.0"):
            parameters["sdk_integration"] = "love_engine"
    ...
    def safe_init_langfuse_client(self, parameters: dict) -> Langfuse:
        ...
        if love_engine.initialized_langfuse_clients >= MAX_LANGFUSE_INITIALIZED_CLIENTS:
            raise Exception(
                f"Max langfuse clients reached: {love_engine.initialized_langfuse_clients} is greater than {MAX_LANGFUSE_INITIALIZED_CLIENTS}"
            )
        langfuse_client = Langfuse(**parameters)
        love_engine.initialized_langfuse_clients += 1
        verbose_logger.debug(
            f"Created langfuse client number {love_engine.initialized_langfuse_clients}"
        )
        return langfuse_client

    @staticmethod
    def add_metadata_from_header(love_engine_params: dict, metadata: dict) -> dict:
        """
        Adds metadata from proxy request headers to Langfuse logging if keys start with "langfuse_"
        and overwrites love_engine_params.metadata if already included.
        ...
        if love_engine_params.get("proxy_server_request") is None:
            return metadata
        ...
        proxy_headers = (
            love_engine_params.get("proxy_server_request", {}).get("headers", {}) or {}
        )
    ...
    def log_event_on_langfuse(
        ...
        try:
            ...
            metadata = (
                love_engine_params.get("metadata", {}) or {}
            )  # if love_engine_params['metadata'] == None
            metadata = self.add_metadata_from_header(love_engine_params, metadata)
    ...
    def _get_langfuse_input_output_content(
        ...
        if (
            ...
            or isinstance(response_obj, love_engine.EmbeddingResponse)
        ):
            input = prompt
            output = None
        elif response_obj is not None and isinstance(
            response_obj, love_engine.ModelResponse
        ):
            input = prompt
            output = self._get_chat_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(
            response_obj, love_engine.HttpxBinaryResponseContent
        ):
            input = prompt
            output = "speech-output"
        elif response_obj is not None and isinstance(
            response_obj, love_engine.TextCompletionResponse
        ):
            input = prompt
            output = self._get_text_completion_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(
            response_obj, love_engine.ImageResponse
        ):
            input = prompt
            output = response_obj.get("data", None)
        elif response_obj is not None and isinstance(
            response_obj, love_engine.TranscriptionResponse
        ):
            input = prompt
            output = response_obj.get("text", None)
        elif response_obj is not None and isinstance(
            response_obj, love_engine.RerankResponse
        ):
            input = prompt
            output = response_obj.results
        elif response_obj is not None and isinstance(
            response_obj, love_engine.ResponsesAPIResponse
        ):
            input = prompt
    ...
    def _log_langfuse_v1(
        ...
        trace = self.Langfuse.trace(  # type: ignore
            CreateTrace(  # type: ignore
                name=metadata.get("generation_name", "love_engine-completion"),
                input=input,
                output=output,
                userId=user_id,
            )
    ...
    def _log_langfuse_v2(
        ...
        try:
            ...
            # we clean out all extra love_engine metadata params before logging
            ...
            if isinstance(metadata, dict):
                for key, value in metadata.items():
                    # generate langfuse tags - Default Tags sent to Langfuse from love_engine Proxy
                    if (
                        love_engine.langfuse_default_tags is not None
                        and isinstance(love_engine.langfuse_default_tags, list)
                        and key in love_engine.langfuse_default_tags
                    ):
                        tags.append(f"{key}:{value}")

                    # clean love_engine metadata before logging
                    if key in [
                        "headers",
                        "endpoint",
                        "caching_groups",
            ...
            # Fallback to love_engine_call_id if no trace_id found
            if trace_id is None:
                trace_id = kwargs.get("love_engine_trace_id") or love_engine_call_id
            ...
            masking_function = love_engine_params.get(
            ...
            if trace_name is None and existing_trace_id is None:
                # just log `love_engine-{call_type}` as the trace name
                ## DO NOT SET TRACE_NAME if trace-id set. this can lead to overwriting of past traces.
                trace_name = f"love_engine-{kwargs.get('call_type', 'completion')}"
            ...
            if existing_trace_id is not None:
                ...
                if "input" in update_trace_keys:
                    trace_params["input"] = (
                        input if not mask_input else "redacted-by-love_engine"
                    )
                if "output" in update_trace_keys:
                    trace_params["output"] = (
                        output if not mask_output else "redacted-by-love_engine"
                    )
                ...
                trace_params = {
                    ...
                    "input": input if not mask_input else "redacted-by-love_engine",
            ...
            if debug is True or (isinstance(debug, str) and debug.lower() == "true"):
                if "metadata" in trace_params:
                    ...
                    trace_params["metadata"]["metadata_passed_to_love_engine"] = metadata
                else:
                    trace_params["metadata"] = {"metadata_passed_to_love_engine": metadata}
            ...
            clean_metadata["love_engine_response_cost"] = cost
            ...
            if (
                love_engine.langfuse_default_tags is not None
                and isinstance(love_engine.langfuse_default_tags, list)
                and "proxy_base_url" in love_engine.langfuse_default_tags
            ):
            ...
            proxy_server_request = love_engine_params.get("proxy_server_request", None)
            ...
            if response_obj is not None:
                if (
                    ...
                    generation_id = love_engine.utils.get_logging_id(
            ...
            if generation_name is None:
                ...
                # If using love_engine proxy user `key_alias` if not None
                # If `key_alias` is None, just log `love_engine-{call_type}` as the generation name
                _user_api_key_alias = cast(
                    Optional[str], clean_metadata.get("user_api_key_alias", None)
                )
                generation_name = (
                    f"love_engine-{cast(str, kwargs.get('call_type', 'completion'))}"
                )
                if _user_api_key_alias is not None:
                    generation_name = f"love_engine:{_user_api_key_alias}"
            ...
            generation_params = {
                ...
                "input": input if not mask_input else "redacted-by-love_engine",
                "output": output if not mask_output else "redacted-by-love_engine",
                "usage": usage,
            ...
            # Return the trace_id we set (which should be love_engine_call_id when no explicit trace_id provided)
    ...
    def add_default_langfuse_tags(self, tags, kwargs, metadata):
        """
        Helper function to add love_engine default langfuse tags

        - Special love_engine tags:
            - cache_hit
            - cache_key

        """
        if love_engine.langfuse_default_tags is not None and isinstance(
            love_engine.langfuse_default_tags, list
        ):
            if "cache_hit" in love_engine.langfuse_default_tags:
                _cache_hit_value = kwargs.get("cache_hit", False)
                tags.append(f"cache_hit:{_cache_hit_value}")
            if "cache_key" in love_engine.langfuse_default_tags:
                _hidden_params = metadata.get("hidden_params", {}) or {}
                _cache_key = _hidden_params.get("cache_key", None)
                if _cache_key is None and love_engine.cache is not None:
                    # fallback to using "preset_cache_key"
                    _preset_cache_key = love_engine.cache._get_preset_cache_key_from_kwargs(
                        **kwargs
                    )
