...
from love_engine.integrations.deepeval.api import Api, Endpoints, HttpMethods
from love_engine.integrations.deepeval.types import (
    BaseApiSpan,
    SpanApiType,
    TraceApi,
    TraceSpanApiStatus,
)
from love_engine.integrations.deepeval.utils import (
...
# This file includes the custom callbacks for love_engine Proxy
# Once defined, these can be passed in proxy_config.yaml
class DeepEvalLogger(CustomLogger):
    """Logs love_engine traces to DeepEval's platform."""

    def __init__(self, *args, **kwargs):
        api_key = os.getenv("CONFIDENT_API_KEY")
        self.love_engine_environment = os.getenv("LITELM_ENVIRONMENT", "development")
        validate_environment(self.love_engine_environment)
        if not api_key:
            raise ValueError(
    ...
    def _prepare_trace_api(
        ...
        trace_api = self._create_trace_api(
            ...
            start_time=_start_time,
            end_time=_end_time,
            love_engine_environment=self.love_engine_environment,
        )
    ...
    def _create_base_api_span(
        ...
        return BaseApiSpan(
            ...
            name=(
                "love_engine_success_callback" if is_success else "love_engine_failure_callback"
            ),
            status=(
    ...
    def _create_trace_api(
        ...
        return TraceApi(
            ...
            startTime=str(start_time),
            endTime=str(end_time),
            environment=love_engine_environment,
        )
