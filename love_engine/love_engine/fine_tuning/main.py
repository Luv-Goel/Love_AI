...
from love_engine.llms.azure.fine_tuning.handler import AzureOpenAIFineTuningAPI
from love_engine.llms.openai.fine_tuning.handler import OpenAIFineTuningAPI
from love_engine.llms.vertex_ai.fine_tuning.handler import VertexFineTuningAPI
from love_engine.secret_managers.main import get_secret_str
from love_engine.types.llms.openai import FineTuningJobCreate, Hyperparameters
from love_engine.types.router import *
from love_engine.types.utils import LoveEngineFineTuningJob
from love_engine.utils import client, supports_httpx_timeout
...
def create_fine_tuning_job(
    ...
    extra_body: Optional[Dict[str, str]] = None,
    **kwargs,
) -> Union[LoveEngineFineTuningJob, Coroutine[Any, Any, LoveEngineFineTuningJob]]:
    """
    ...
    try:
        ...
        if custom_llm_provider == "openai":
            ...
            response = openai_fine_tuning_apis_instance.create_fine_tuning_job(
                ...
                client=kwargs.get(
                    "client", None
                ),  # note, when we add this to `GenericLoveEngineParams` it impacts a lot of other tests + linting
            )
        # Azure OpenAI
            ...
            raise love_engine.exceptions.BadRequestError(
                message="love_engine doesn't support {} for 'create_batch'. Only 'openai' is supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            api_base = optional_params.api_base or love_engine.api_base or get_secret("AZURE_API_BASE")  # type: ignore
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            raise love_engine.exceptions.BadRequestError(
                message="love_engine doesn't support {} for 'retrieve_fine_tuning_job'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="retrieve_fine_tuning_job", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
