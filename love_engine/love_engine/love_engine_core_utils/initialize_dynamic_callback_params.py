...
def _raise_env_reference_error(param: str, *, source: str) -> None:
    raise ValueError(
        ...
        "config.yaml under 'love_engine_settings' / 'general_settings', or "
        "(b) pass the resolved secret value directly in the request.\n"
        "See https://docs.love_engine.ai/docs/proxy/logging for server-side "
...
def initialize_standard_callback_dynamic_params(
    ...
    if kwargs:
        ...
        # 2. Fallback: check "metadata" or "love_engine_params" -> "metadata"
        ...
        if isinstance(love_engine_params, dict):
            metadata.update(love_engine_params.get("metadata") or {})
