...
import love_engine
from love_engine._logging import _ENABLE_SECRET_REDACTION, _redact_string, verbose_logger
from love_engine.love_engine_core_utils.secret_redaction import redact_string
    ...
    try:
        ...
        if not _response_headers:
            _response_headers = getattr(
                original_exception, "love_engine_response_headers", None
            )
    except Exception:
        return None
...
def extract_and_raise_love_engine_exception(
    ...
    """
    Covers scenario where love_engine sdk calling proxy.

    Enables raising the special errors raised by love_engine, eg. ContextWindowExceededError.

    Relevant Issue: https://github.com/BerriAI/love_engine/issues/7259
    """
    pattern = r"love_engine\.\w+Error"
    ...
    if match:
        ...
        exception_name = exception_name.strip().replace("love_engine.", "")
        raised_exception_obj = getattr(love_engine, exception_name, None)
...
def exception_type(  # type: ignore
    ...
    if any(
        ...
        for exc_type in love_engine.LOVE_ENGINE_EXCEPTION_TYPES
    ):
    ...
    if love_engine.suppress_debug_info is False:
        ...
        print(  # noqa: T201
            "LoveEngine.Info: If you need to debug this error, use `love_engine._turn_on_debug()'."
        )
        print()  # noqa: T201

    love_engine_response_headers = _get_response_headers(
    ...
    try:
        ...
        if model:
            ...
            extra_information = ""
            try:
                _api_base = love_engine.get_api_base(
                    model=model, optional_params=extra_kwargs
                )
                messages = love_engine.get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if (
                    messages
                    and len(messages) > 0
                    and love_engine.redact_messages_in_exceptions is False
                ):
                    extra_information += f"\nMessages: `{messages}`"
                ...
                # on love_engine proxy add key name + team to exceptions
            ...
            if (
                ...
                raise Timeout(
                    ...
                    model=model,
                    llm_provider=custom_llm_provider,
                    love_engine_debug_info=extra_information,
                )

            if (
                custom_llm_provider == "love_engine_proxy"
            ):  # handle special case where calling love_engine proxy + exception str contains error message
                extract_and_raise_love_engine_exception(
            ...
            if (
                ...
                if message is not None and isinstance(
                    message, str
                ):  # done to prevent user-confusion. Relevant issue - https://github.com/BerriAI/love_engine/issues/1414
                ...
                if ExceptionCheckers.is_error_str_rate_limit(error_str):
                    ...
                    helpful_message = (
                        ...
                        "   Learn more: https://docs.love_engine.ai/docs/response_api#encrypted-content-affinity-multi-region-load-balancing"
                    )
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 401:
                        ...
                        exception_mapping_worked = True
                        raise love_engine.InternalServerError(
                            message=f"AnthropicException - {error_str}. Handle with `love_engine.InternalServerError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
                        ...
                        raise love_engine.ServiceUnavailableError(
                            message=f"AnthropicException - {error_str}. Handle with `love_engine.ServiceUnavailableError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
            ...
            elif custom_llm_provider in love_engine._openai_like_providers:
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Enable 'love_engine.modify_params=True' (for PROXY do: `love_engine_settings::modify_params: True`) to insert a dummy assistant message and fix this error.",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Pass in default user message via `completion(..,user_continue_message=)` or enable `love_engine.modify_params=True`.\nFor Proxy: do via `love_engine_settings::modify_params: True` or user_continue_message under `love_engine_params`",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                ...
                if "Unable to locate credentials" in error_str:
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"love_engine.BadRequestError: SagemakerException - {error_str}",
                        model=model,
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"love_engine.BadRequestError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        response=httpx.Response(
                            status_code=400,
                ...
                if "400 Request payload size exceeds" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise love_engine.InternalServerError(
                        message=f"love_engine.InternalServerError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        response=httpx.Response(
                            status_code=500,
                            content=str(original_exception),
                            request=httpx.Request(method="completion", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                        ),
                        love_engine_debug_info=extra_information,
                    )
                    ...
                    exception_mapping_worked = True
                    raise RateLimitError(
                        message=f"love_engine.RateLimitError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        love_engine_debug_info=extra_information,
                        response=httpx.Response(
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 429:
                        exception_mapping_worked = True
                        raise RateLimitError(
                            message=f"love_engine.RateLimitError: {custom_llm_provider.capitalize()}Exception - {error_str}",
                            model=model,
                            llm_provider=custom_llm_provider,
                            love_engine_debug_info=extra_information,
                            response=httpx.Response(
                ...
                if "Internal server error" in error_str:
                    ...
                    exception_mapping_worked = True
                    from love_engine.llms.azure.exception_mapping import (
        ...
        if (
            ...
            """
            For unmapped exceptions - raise the exception with traceback - https://github.com/BerriAI/love_engine/issues/4201
            """
            exception_mapping_worked = True
        ...
        if exception_mapping_worked:
            setattr(e, "love_engine_response_headers", love_engine_response_headers)
            raise e
        else:
            for error_type in love_engine.LOVE_ENGINE_EXCEPTION_TYPES:
            ...
            setattr(raised_exc, "love_engine_response_headers", love_engine_response_headers)
            raise raised_exc
    ...
    try:
        ...
        if logger_fn and callable(logger_fn):
            try:
                ...
                verbose_logger.debug(
                    f"LoveEngine.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
                )
    except Exception:
        verbose_logger.debug(
            f"LoveEngine.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
...
def _add_key_name_and_team_to_alert(request_info: str, metadata: dict) -> str:
    """
    Internal helper function for love_engine proxy
