...
Admins can opt out via two ``love_engine`` globals (wired from proxy config):

- ``love_engine.user_url_validation`` (bool, default True): master switch.
  When False, ``safe_get``/``async_safe_get`` perform a plain fetch with
  no DNS check, no block list, and no rewrite.
- ``love_engine.user_url_allowed_hosts`` (List[str], default []): per-host
...
def _is_host_allowlisted(hostname: str, effective_port: int) -> bool:
    ...
    """
    configured: List[str] = getattr(love_engine, "user_url_allowed_hosts", []) or []
    if not configured:
        return False
...
def validate_url(url: str) -> Tuple[str, str]:
    ...
    ssl_verify = getattr(love_engine, "ssl_verify", True)
...
def safe_get(client: Any, url: str, **kwargs: Any) -> Any:
    ...
    When ``love_engine.user_url_validation`` is False, validation is bypassed
    ...
    """
    if not getattr(love_engine, "user_url_validation", True):
