...
from love_engine.constants import MAX_BASE64_LENGTH_FOR_LOGGING
...
if TYPE_CHECKING:
    ...
    from love_engine import ModelResponse as _ModelResponse
    from love_engine.love_engine_core_utils.love_engine_logging import (
        Logging as LoveEngineLoggingObject,
    )

    LoveEngineModelResponse = _ModelResponse
    Span = Union[_Span, Any]
else:
    LoveEngineModelResponse = Any
...
def _get_parent_otel_span_from_logging_obj(
    ...
    Args:
        logging_obj: The LoveEngine logging object containing model call details
...
def convert_love_engine_response_object_to_str(
    response_obj: Union[Any, LoveEngineModelResponse],
) -> Optional[str]:
    """
    Get the string of the response object from LoveEngine

    """
    if isinstance(response_obj, love_engine.ModelResponse):
        response_str = ""
...
def _set_duration_in_model_call_details(
    logging_obj: Any,  # we're not guaranteed this will be `LoveEngineLoggingObject`
    start_time: datetime,
    end_time: datetime,
):
                ...
                try:
                    ...
                    asyncio.create_task(
                        service_logger.async_service_success_hook(
                            service=ServiceTypes.LOVE_ENGINE,
                            duration=duration,
                            call_type=call_type,
