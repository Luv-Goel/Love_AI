...
def _ensure_extra_body_is_safe(extra_body: Optional[Dict]) -> Optional[Dict]:
    ...
    Relevant Issue: https://github.com/BerriAI/love_engine/issues/4140
    """
...
def pick_cheapest_chat_models_from_llm_provider(custom_llm_provider: str, n=1):
    ...
    """
    if custom_llm_provider not in love_engine.models_by_provider:
        return []

    known_models = love_engine.models_by_provider.get(custom_llm_provider, [])
    model_costs = []
...
def get_proxy_server_request_headers(love_engine_params: Optional[dict]) -> dict:
    """
    Get the `proxy_server_request` headers from the love_engine_params.\

    Use this if you want to access the request headers made to LoveEngine proxy server.
    """
    if love_engine_params is None:
        return {}

    proxy_request_headers = (love_engine_params.get("proxy_server_request") or {}).get(
        "headers"
    ) or {}
