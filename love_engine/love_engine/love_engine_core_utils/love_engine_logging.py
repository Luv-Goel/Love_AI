...
from love_engine._logging import _is_debugging_on, _redact_string, verbose_logger
from love_engine.exceptions import (
    validate_rate_limit_category,
    validate_rate_limit_type,
)
from love_engine._uuid import uuid
from love_engine.batches.batch_utils import _handle_completed_batch
from love_engine.caching.caching import DualCache, InMemoryCache
...
from love_engine.cost_calculator import (
...
from love_engine.integrations.arize.arize import ArizeLogger
...
from love_engine.integrations.deepeval.deepeval import DeepEvalLogger
from love_engine.integrations.mlflow import MlflowLogger
from love_engine.integrations.sqs import SQSLogger
from love_engine.love_engine_core_utils.core_helpers import reconstruct_model_name
from love_engine.love_engine_core_utils.get_love_engine_params import get_love_engine_params
from love_engine.love_engine_core_utils.llm_cost_calc.tool_call_cost_tracking import (
    StandardBuiltInToolCostTracking,
)
from love_engine.love_engine_core_utils.logging_utils import truncate_base64_in_messages
from love_engine.love_engine_core_utils.model_param_helper import ModelParamHelper
from love_engine.love_engine_core_utils.redact_messages import (
    redact_message_input_output_from_custom_logger,
    redact_message_input_output_from_logging,
)
from love_engine.llms.base_llm.ocr.transformation import OCRResponse
from love_engine.llms.base_llm.search.transformation import SearchResponse
from love_engine.responses.utils import ResponseAPILoggingUtils
from love_engine.types.agents import LoveEngineSendMessageResponse
from love_engine.types.containers.main import ContainerObject
...
from love_engine.types.mcp import MCPPostCallResponseObject
...
from love_engine.types.utils import (
    ...
    ImageResponse,
    LoveEngineBatch,
    LoveEngineLoggingBaseClass,
    LoveEngineRealtimeStreamLoggingObject,
    ModelResponse,
    ModelResponseStream,
    RawRequestTypedDict,
...
from love_engine.utils import _get_base_model_from_metadata, executor, print_verbose
...
from ..integrations.love_engine_agent import LoveEngineAgentModelResolver
...
try:
    from love_engine_enterprise.enterprise_callbacks.callback_controls import (
    ...
    from love_engine_enterprise.love_engine_core_utils.love_engine_logging import (
        StandardLoggingPayloadSetup as EnterpriseStandardLoggingPayloadSetup,
    )

    from love_engine.integrations.generic_api.generic_api_callback import GenericAPILogger
    ...
    verbose_logger.debug(
        f"[Non-Blocking] Unable to import GenericAPILogger - LoveEngine Enterprise Feature - {str(e)}"
    )
...
_CUSTOM_PRICING_KEYS: frozenset = frozenset(
    CustomPricingLoveEngineParams.model_fields.keys()
)
...
class ServiceTraceIDCache:
    ...
    def get_cache(self, love_engine_call_id: str, service_name: str) -> Optional[str]:
        key_name = "{}:{}".format(service_name, love_engine_call_id)
        response = self.cache.get_cache(key=key_name)
        return response

    def set_cache(self, love_engine_call_id: str, service_name: str, trace_id: str) -> None:
        key_name = "{}:{}".format(service_name, love_engine_call_id)
...
class Logging(LoveEngineLoggingBaseClass):
    ...
    stream_options = None
    love_engine_request_debug: bool = False
    ...
    def __init__(
        ...
        call_type,
        start_time,
        love_engine_call_id: str,
        function_id: str,
        love_engine_trace_id: Optional[str] = None,
        ...
        self.love_engine_call_id = love_engine_call_id
        self.love_engine_trace_id: str = (
            love_engine_trace_id if love_engine_trace_id else str(uuid.uuid4())
        )
        ...
        # INITIAL LOVE_ENGINE_PARAMS
        love_engine_params = {}
        if kwargs is not None:
            love_engine_params = get_love_engine_params(**kwargs)
            love_engine_params = scrub_sensitive_keys_in_metadata(love_engine_params)

        self.love_engine_params = love_engine_params
        ...
        self.model_call_details: Dict[str, Any] = {
            "love_engine_trace_id": self.love_engine_trace_id,
    ...
    def process_dynamic_callbacks(self):
        ...
        If a callback is in love_engine._known_custom_logger_compatible_callbacks, it needs to be intialized and added to the respective dynamic_* callback list.
        """
    ...
    def _process_dynamic_callback_list(
        ...
        - If a callback is in love_engine._known_custom_logger_compatible_callbacks,
        ...
        for callback in callback_list:
            if (
                ...
                and callback in love_engine._known_custom_logger_compatible_callbacks
            ):
    ...
    def get_router_model_id(self) -> Optional[str]:
        """Extract the router deployment model_id from love_engine_params.

        Checks both love_engine_metadata and metadata for model_info.id.
        Used by cost calculators to look up custom pricing registered
        under the deployment's model_info.id in love_engine.model_cost.
        """
        if not hasattr(self, "love_engine_params"):
            return None
        for key in ("love_engine_metadata", "metadata"):
            meta = self.love_engine_params.get(key, {}) or {}
    ...
    def update_environment_variables(
        ...
        **additional_params,
        ...
        if model is not None:
            self.model = model
        self.user = user
        self.love_engine_params = {
            **self.love_engine_params,
            **scrub_sensitive_keys_in_metadata(love_engine_params),
        }
        self.love_engine_request_debug = love_engine_params.get("love_engine_request_debug", False)
        self.logger_fn = love_engine_params.get("logger_fn", None)
        if _is_debugging_on() or self.love_engine_request_debug:
        ...
        self.model_call_details.update(
            {
                ...
                "love_engine_params": self.love_engine_params,
                "start_time": self.start_time,
                "stream": self.stream,
                "user": user,
                "call_type": str(self.call_type),
                "love_engine_call_id": self.love_engine_call_id,
        ...
        if any(
            love_engine_params.get(key) is not None
            for key in _CUSTOM_PRICING_KEYS & love_engine_params.keys()
        ):
    ...
    def update_from_kwargs(
        ...
        **additional_params,
        ...
        automatically extracts metadata/love_engine_metadata from kwargs,
        so callers don't need to manually plumb them into love_engine_params.
        """
        base_love_engine_params: Dict[str, Any] = {}

        if "metadata" in kwargs:
            base_love_engine_params["metadata"] = kwargs["metadata"]
        if "love_engine_metadata" in kwargs and isinstance(
            kwargs["love_engine_metadata"], dict
        ):
            base_love_engine_params["love_engine_metadata"] = kwargs["love_engine_metadata"]
            if "metadata" not in base_love_engine_params:
                base_love_engine_params["metadata"] = kwargs["love_engine_metadata"].copy()

        if love_engine_params:
            # Merge metadata carefully — don't overwrite the merged metadata
            # from kwargs/love_engine_metadata with the caller's love_engine_params metadata.
            # e.g. anthropic_messages passes Anthropic's native metadata ({user_id: ...})
            # in love_engine_params, which would overwrite proxy key-auth fields.
            lp_metadata = love_engine_params.pop("metadata", None)
            base_love_engine_params.update(love_engine_params)
            if lp_metadata and isinstance(lp_metadata, dict):
                base_love_engine_params.setdefault("metadata", {})
                for k, v in lp_metadata.items():
                    if k not in base_love_engine_params["metadata"]:
                        base_love_engine_params["metadata"][k] = v

        self.update_environment_variables(
            love_engine_params=base_love_engine_params,
    ...
    def should_run_prompt_management_hooks(
        ...
        # Check if model uses love_engine_agent prefix (model replacement without prompt_id)
        model = non_default_params.get("model", "")
        if isinstance(model, str) and model.startswith("love_engine_agent/"):
            return True
    ...
    def _should_run_prompt_management_hooks_without_prompt_id(
        ...
        #############################################################################
        if love_engine.vector_store_registry is not None:
            if love_engine.vector_store_registry.get_vector_store_to_run(
    ...
    def get_custom_logger_for_prompt_management(
        ...
        for callback_name in love_engine._known_custom_logger_compatible_callbacks:
        ...
        if love_engine.vector_store_registry is not None:
            ...
            if (
                ...
                and vector_store_custom_logger not in love_engine.callbacks
            ):
    ...
    def _pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        self.model_call_details["love_engine_params"]["api_base"] = (
            self._get_masked_api_base(additional_args.get("api_base", ""))
        )

    def pre_call(self, input, api_key, model=None, additional_args={}):
        # Log the exact input to the LLM API
        love_engine.error_logs["PRE_CALL"] = locals()
        try:
            self._pre_call(
                input=input,
                api_key=api_key,
            ...
            if (
                ...
                _love_engine_params = self.model_call_details.get("love_engine_params", {})
                _metadata = _love_engine_params.get("metadata", {}) or {}
                try:
                    # [Non-blocking Extra Debug Information in metadata]
                    if turn_off_message_logging is True:
                        _metadata["raw_request"] = "redacted by love_engine. \
                            'love_engine.turn_off_message_logging=True'"
                    else:
            ...
            if getattr(self, "logger_fn", None) and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "LoveEngine.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(
                            str(e)
                        )
                    )
            ...
            # love_engine_params["metadata"] (caller request metadata, typed
            ...
            callbacks = love_engine.input_callback + (self.dynamic_input_callbacks or [])
                ...
                try:
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.input_log_event(
                            ...
                            love_engine_call_id=self.love_engine_params["love_engine_call_id"],
                    ...
                    verbose_logger.exception(
                        "love_engine.Logging.pre_call(): Exception occured - {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
                        f"LoveEngine.Logging: is sentry capture exception initialized {capture_exception}"
                    )
    ...
    def _get_masked_headers(
        ...
        Masks the headers of the request sent from LoveEngine
        """
    ...
    def post_call(
        ...
        love_engine.error_logs["POST_CALL"] = locals()
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "LoveEngine.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
    ...
    async def async_post_mcp_tool_call_hook(
        ...
        """
        from love_engine.types.llms.base import HiddenParams
        from love_engine.types.mcp import MCPPostCallResponseObject

        callbacks = self.get_combined_callback_list(
            dynamic_success_callbacks=self.dynamic_success_callbacks,
            global_callbacks=love_engine.success_callback,
        )
    ...
    def _response_cost_calculator(
        ...
        love_engine_model_name: Optional[str] = None,
        ...
        # Fallback: extract router_model_id from love_engine_params when not available
        ...
        custom_pricing = use_custom_pricing_for_model(
            love_engine_params=(
                self.love_engine_params if hasattr(self, "love_engine_params") else None
            )
        )
        ...
        try:
            response_cost_calculator_kwargs = {
                ...
                "model": love_engine_model_name or self.model,
                "cache_hit": cache_hit,
                ...
                "data_residency": (
                    self.love_engine_params.get("data_residency")
                    if hasattr(self, "love_engine_params") and self.love_engine_params
                    else None
                ),
            }
        ...
        try:
            response_cost = love_engine.response_cost_calculator(
                **response_cost_calculator_kwargs
            )
    ...
    @staticmethod
    def _is_sync_love_engine_request(love_engine_params: dict) -> bool:
        """True for sync SDK entrypoints (``completion``), false for async (``acompletion``, etc.)."""
        return (
            love_engine_params.get(CallTypes.acompletion.value, False) is not True
            and love_engine_params.get(CallTypes.aresponses.value, False) is not True
            and love_engine_params.get(CallTypes.aembedding.value, False) is not True
            and love_engine_params.get(CallTypes.aimage_generation.value, False) is not True
            and love_engine_params.get(CallTypes.atranscription.value, False) is not True
        )
    ...
    async def dispatch_success_handlers(
        ...
        **kwargs,
        ...
        love_engine_params = self.model_call_details.get("love_engine_params", {}) or {}
        sync_sdk = self._is_sync_love_engine_request(love_engine_params)
    ...
    def should_run_callback(
        self, callback: love_engine.CALLBACK_TYPES, love_engine_params: dict, event_hook: str
    ) -> bool:
        if love_engine.global_disable_no_log_param:
            return True

        if love_engine_params.get("no-log", False) is True:
        ...
        if (
            ...
            verbose_logger.debug(
                f"Callback {callback} disabled via x-love_engine-disable-callbacks header for {event_hook} event"
            )
            return False
    ...
    def _merge_hidden_params_from_response_into_metadata(
        ...
        """
        Copy response._hidden_params into love_engine_params.metadata['hidden_params'].
        ...
        if not hidden_params:
            return
        if self.model_call_details.get("love_engine_params") is None:
            return
        ...
        love_engine_params = self.model_call_details["love_engine_params"]
        metadata = love_engine_params.get("metadata") or {}
        love_engine_params["metadata"] = metadata
    ...
    def _process_hidden_params_and_response_cost(
        ...
        if hidden_params:
            if self.model_call_details.get("love_engine_params") is not None:
                self.model_call_details["love_engine_params"].setdefault("metadata", {})
                if self.model_call_details["love_engine_params"]["metadata"] is None:
                    self.model_call_details["love_engine_params"]["metadata"] = {}
                self.model_call_details["love_engine_params"]["metadata"]["hidden_params"] = getattr(logging_result, "_hidden_params", {})  # type: ignore
    ...
    def _transform_usage_objects(self, result):
        if isinstance(result, ResponsesAPIResponse):
            ...
            from love_engine.love_engine_core_utils.llm_cost_calc.usage_object_transformation import (
    ...
    def _success_handler_helper_fn(
        ...
        try:
            ...
            if (
                ...
                love_engine._current_cost += love_engine.completion_cost(
                    model=self.model,
                    prompt="",
            ...
            raise Exception(f"[Non-Blocking] LoveEngine.Success_Call Error: {str(e)}")
    ...
    def _is_recognized_call_type_for_logging(
        ...
        if (
            ...
            or isinstance(logging_result, LoveEngineBatch)
            or isinstance(logging_result, ResponsesAPIResponse)
            or isinstance(logging_result, OpenAIFileObject)
            or isinstance(logging_result, LoveEngineRealtimeStreamLoggingObject)
            ...
            or isinstance(logging_result, LoveEngineSendMessageResponse)  # A2A
    ...
    def success_handler(
        ...
        verbose_logger.debug(
            f"Logging Details LoveEngine-Success Call: Cache_hit={cache_hit}"
        )
        ...
        love_engine_params = self.model_call_details.get("love_engine_params", {})
        is_sync_request = self._is_sync_love_engine_request(love_engine_params)
        try:
            ...
            if complete_streaming_response is not None:
                verbose_logger.debug(
                    "Logging Details LoveEngine-Success Call streaming complete"
                )
            ...
            for callback in callbacks:
                if isinstance(callback, CustomGuardrail):
                    from love_engine.types.guardrails import GuardrailEventHooks
                ...
                try:
                    ...
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.log_event(
                            ...
                            love_engine_call_id=(
                                current_call_id
                                if (
                                    current_call_id := love_engine_params.get(
                                        "love_engine_call_id"
                                    )
                                )
                                is not None
                    ...
                    if callback == "lunary" and lunaryLogger is not None:
                        ...
                        lunaryLogger.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            run_id=self.love_engine_call_id,
                            print_verbose=print_verbose,
                        )
                    ...
                    print_verbose(
                        f"LoveEngine.LoggingError: [Non-Blocking] Exception occurred while success logging with integrations {traceback.format_exc()}"
                    )
                    print_verbose(
                        f"LoveEngine.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "LoveEngine.LoggingError: [Non-Blocking] Exception occurred while success logging {}".format(
                    str(e)
                ),
            )
    ...
    async def async_success_handler(
        ...
        """
        print_verbose(
            "Logging Details LoveEngine-Async Success Call, cache_hit={}".format(cache_hit)
        )
        ...
        if self.call_type == CallTypes.aretrieve_batch.value and isinstance(
            result, LoveEngineBatch
        ):
            love_engine_params = self.love_engine_params or {}
            love_engine_metadata = love_engine_params.get("love_engine_metadata") or {}
            if (
                love_engine_metadata.get("batch_ignore_default_logging", False) is True
            ):  # polling job will query these frequently, don't spam db logs
                return

            from love_engine.proxy.openai_files_endpoints.common_utils import (
        ...
        if complete_streaming_response is not None:
            ...
            except love_engine.NotFoundError:
                verbose_logger.warning(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=love_engine._async_success_callback,
        )
        ...
        for callback in callbacks:
            if isinstance(callback, CustomGuardrail):
                ...
                result = redact_message_input_output_from_custom_logger(
                    result=result, love_engine_logging_obj=self, custom_logger=callback
                )
                self.model_call_details, result = await callback.async_logging_hook(
                    kwargs=self.model_call_details,
                    result=result,
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.error(
                    f"LoveEngine.LoggingError: [Non-Blocking] Exception occurred while success logging {traceback.format_exc()}"
                )
                self._handle_callback_failure(callback=callback)
                pass
    ...
    def _handle_callback_failure(self, callback: Any):
        ...
        try:
            ...
            all_callbacks = love_engine.logging_callback_manager._get_all_callbacks()
    ...
    def _failure_handler_helper_fn(
        ...
        if hasattr(exception, "headers") and isinstance(exception.headers, dict):
            self.model_call_details.setdefault("love_engine_params", {})
            metadata = (
                self.model_call_details["love_engine_params"].get("metadata", {}) or {}
            )
    ...
    async def special_failure_handlers(self, exception: Exception):
        ...
        """
        from love_engine.types.router import RouterErrors

        love_engine_params: dict = self.model_call_details.get("love_engine_params") or {}
        metadata = love_engine_params.get("metadata") or {}
        ...
        for callback in love_engine._async_failure_callback:
            if isinstance(callback, CustomLogger):  # custom logger class
    ...
    def failure_handler(
        ...
        verbose_logger.debug(
            f"Logging Details LoveEngine-Failure Call: {love_engine.failure_callback}"
        )
        if not self.should_run_logging(
        ...
        try:
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=love_engine.failure_callback,
            )
                ...
                try:
                    ...
                    if callback == "sentry":
                        ...
                        supabaseClient.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            love_engine_call_id=self.model_call_details["love_engine_call_id"],
                            print_verbose=print_verbose,
                        )
                    if callable(callback):  # custom logger functions
                        global customLogger
                    ...
                    print_verbose(
                        f"LoveEngine.LoggingError: [Non-Blocking] Exception occurred while failure logging with integrations {str(e)}"
                    )
                    print_verbose(
                        f"LoveEngine.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "LoveEngine.LoggingError: [Non-Blocking] Exception occurred while failure logging {}".format(
                    str(e)
                )
            )
    ...
    async def async_failure_handler(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=love_engine._async_failure_callback,
        )
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(
                    "LoveEngine.LoggingError: [Non-Blocking] Exception occurred while failure \
                        logging {}\nCallback={}".format(
                        str(e), callback
                    )
                )
    ...
    def _get_trace_id(self, service_name: Literal["langfuse"]) -> Optional[str]:
        ...
        if service_name == "langfuse":
            trace_id = in_memory_trace_id_cache.get_cache(
                love_engine_call_id=self.love_engine_call_id, service_name=service_name
            )
    ...
    def _should_run_sync_callbacks_for_async_calls(self) -> bool:
        ...
        _filtered_success_callbacks = self._remove_internal_love_engine_callbacks(
    ...
    def _remove_internal_love_engine_callbacks(self, callbacks: List) -> List:
        """
        Creates a filtered list of callbacks, excluding internal LoveEngine callbacks.
        ...
        """
        filtered = [
            cb for cb in callbacks if not self._is_internal_love_engine_proxy_callback(cb)
        ]
    ...
    def _is_internal_love_engine_proxy_callback(self, cb) -> bool:
    ...
    def _remove_internal_custom_logger_callbacks(self, callbacks: List) -> List:
        ...
        for _c in callbacks:
            if isinstance(_c, CustomLogger):
                ...
                isinstance(_c, str)
                and _c in love_engine._known_custom_logger_compatible_callbacks
            ):
                continue
    ...
    def _handle_anthropic_messages_response_logging(self, result: Any) -> ModelResponse:
        ...
        if httpx_response and isinstance(httpx_response, httpx.Response):
            result = love_engine.AnthropicConfig().transform_response(
            ...
            from love_engine.types.llms.anthropic import AnthropicResponse

            pydantic_result = AnthropicResponse.model_validate(result)
            import httpx

            result = love_engine.AnthropicConfig().transform_parsed_response(
    ...
    def _translate_responses_api_response_to_model_response(
        ...
        """
        from love_engine.completion_extras.love_engine_responses_transformation.transformation import (
            LoveEngineResponsesTransformationHandler,
        )

        try:
            return LoveEngineResponsesTransformationHandler().transform_response(
                model=self.model,
                raw_response=result,
                model_response=love_engine.ModelResponse(),
                logging_obj=self,
    ...
    def _handle_non_streaming_google_genai_generate_content_response_logging(
        ...
        result = love_engine.VertexGeminiConfig()._transform_google_generate_content_to_openai_model_response(
    ...
    def _handle_a2a_response_logging(self, result: Any) -> Any:
        ...
        Args:
            result: The LoveEngineSendMessageResponse from the A2A call
    ...
    try:
        for callback in callback_list:
            if callback == "sentry":
                ...
                from love_engine.integrations.weights_biases import WeightsBiasesLogger
...
def _init_custom_logger_compatible_class(
    ...
    llm_router: Optional[
        Any
    ],  # expect love_engine.Router, but typing errors due to circular import
    ...
    try:
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
        ...
        elif logging_integration == "love_engine_agent":
            for callback in _in_memory_loggers:
                if isinstance(callback, LoveEngineAgentModelResolver):
                    return callback  # type: ignore

            _love_engine_agent_resolver = LoveEngineAgentModelResolver()
            _in_memory_loggers.append(_love_engine_agent_resolver)
            return _love_engine_agent_resolver  # type: ignore
            ...
            if _dd_api_key or _dd_site or _dd_agent_host:
                ...
                from love_engine.integrations.datadog.datadog_team_handler import (
                    DataDogHandler,
                )
            ...
            if _v2 is not None:
                return _v2  # type: ignore
            from love_engine.integrations.opentelemetry import (
                OpenTelemetry,
                OpenTelemetryConfig,
            )
            ...
            # Gate the new typed V2 adapter behind LOVE_ENGINE_OTEL_V2. When off,
            # the legacy 3,227-line god-class is used unchanged. The two are
            # never registered simultaneously — the dedup loop below treats
            # any module under ``love_engine.integrations.otel`` or
            # ``love_engine.integrations.opentelemetry`` as "the OTel callback".
            ...
            from love_engine.integrations.mavvrik_focus.mavvrik_focus_logger import (
                MavvrikFocusLogger,
            )
            ...
            from love_engine.proxy.hooks.dynamic_rate_limiter import (
            ...
            if llm_router is not None and isinstance(llm_router, love_engine.Router):
            ...
            from love_engine.proxy.hooks.dynamic_rate_limiter_v3 import (
            ...
            if _v2 is not None:
                return _v2  # type: ignore
            from love_engine.integrations.langfuse.langfuse_otel import LangfuseOtelLogger
            ...
            if _v2 is not None:
                return _v2  # type: ignore
            from love_engine.integrations.opentelemetry import OpenTelemetryConfig
            from love_engine.integrations.weave.weave_otel import (
                WeaveOtelLogger,
            ...
            from love_engine.integrations.bitbucket.bitbucket_prompt_manager import (
            ...
            bitbucket_config = getattr(love_engine, "global_bitbucket_config", None)
            if bitbucket_config is None:
                raise ValueError(
                    "BitBucket configuration not found. Please set love_engine.global_bitbucket_config first."
                )
            ...
            from love_engine.integrations.gitlab.gitlab_prompt_manager import (
                GitLabPromptManager,
            )
            ...
            # Get global BitBucket config
            gitlab_config = getattr(love_engine, "global_gitlab_config", None)
            if gitlab_config is None:
                raise ValueError(
                    "Gitlab configuration not found. Please set love_engine.global_gitlab_config first."
                )
...
def _maybe_construct_otel_v2(
    ...
    """If ``LOVE_ENGINE_OTEL_V2`` is on, build (or reuse) a single ``OpenTelemetryV2``
    ...
    if not is_otel_v2_enabled():
        return None
    from love_engine.integrations.otel.logger import OpenTelemetryV2
    from love_engine.integrations.otel.presets import PRESET_BY_CALLBACK
...
def _maybe_auto_initialize_arize_phoenix(_in_memory_loggers: list) -> None:
    ...
    try:
        ...
        # Register as a love_engine callback so it receives success/failure events
        love_engine.logging_callback_manager.add_love_engine_callback(phoenix_logger)
...
def _get_custom_logger_settings_from_proxy_server(callback_name: str) -> Dict:
    ...
    """
    if love_engine.callback_settings:
        return dict(love_engine.callback_settings.get(callback_name, {}))
    return {}


def use_custom_pricing_for_model(love_engine_params: Optional[dict]) -> bool:
    """
    Check if the model uses custom pricing

    Returns True if any of `SPECIAL_MODEL_INFO_PARAMS` are present in `love_engine_params` or `model_info`
    """
    if love_engine_params is None:
        return False

    # Check love_engine_params using set intersection (only check keys that exist in both)
    matching_keys = _CUSTOM_PRICING_KEYS & love_engine_params.keys()
    for key in matching_keys:
        if love_engine_params.get(key) is not None:
            return True

    # Check model_info from metadata or love_engine_metadata (generic_api_call routes
    # like /responses and /messages store model_info under love_engine_metadata)
    for metadata_key in ("metadata", "love_engine_metadata"):
        metadata: dict = love_engine_params.get(metadata_key, {}) or {}
...
class StandardLoggingPayloadSetup:
    ...
    @staticmethod
    def merge_love_engine_metadata(love_engine_params: dict) -> dict:
        """
        Merge both love_engine_metadata and metadata from love_engine_params.

        love_engine_metadata contains model-related fields, metadata contains user API key fields.
        We need both for complete standard logging payload.

        Args:
            love_engine_params: Dictionary containing metadata and love_engine_metadata
        ...
        if love_engine_params.get("metadata") and isinstance(
            love_engine_params.get("metadata"), dict
        ):
            for key, value in love_engine_params["metadata"].items():
                # Skip non-serializable objects like UserAPIKeyAuth
                if key in {"user_api_key_auth", "user_api_key_budget_reservation"}:
                    continue
                merged_metadata[key] = value

        # Then merge love_engine_metadata (model-related fields) - this will NOT overwrite existing keys
        if love_engine_params.get("love_engine_metadata") and isinstance(
            love_engine_params.get("love_engine_metadata"), dict
        ):
            for key, value in love_engine_params["love_engine_metadata"].items():
                if (
    ...
    def get_standard_logging_metadata(
        ...
        if love_engine_params is not None:
            prompt_id = cast(Optional[str], love_engine_params.get("prompt_id", None))
            prompt_variables = cast(
                Optional[dict], love_engine_params.get("prompt_variables", None)
            )
        ...
        if isinstance(metadata, dict):
            ...
            _potential_requester_metadata = metadata.get(
                "metadata", None
            )  # check if user passed metadata in the sdk request - e.g. metadata for langsmith logging - https://docs.love_engine.ai/docs/observability/langsmith_integration#set-langsmith-fields
            if (
    ...
    def get_model_cost_information(
        ...
        if model_cost_name is None:
            ...
            try:
                _model_cost_information = love_engine.get_model_info(
    ...
    def get_hidden_params(
        ...
        clean_hidden_params = StandardLoggingHiddenParams(
            ...
            love_engine_overhead_time_ms=None,
            batch_models=None,
    ...
    def _generate_cold_storage_object_key(
        ...
        from love_engine.integrations.s3 import get_s3_object_key

        # Only generate object key if cold storage is configured
        cold_storage_custom_logger = love_engine.cold_storage_custom_logger
        if cold_storage_custom_logger is None:
            return None

        try:
            # Generate file name in same format as love_engine.utils.get_logging_id
            ...
            try:
                custom_logger = love_engine.logging_callback_manager.get_active_custom_logger_for_callback_name(
    ...
    def get_error_information(
        ...
        from love_engine.constants import MAXIMUM_TRACEBACK_LINES_TO_LOG

        # ProxyException uses .code, LoveEngine exceptions use .status_code,
        ...
        # love_engine.exceptions.* class) over str(exc); ProxyException does not
    ...
    def _get_standard_logging_payload_trace_id(
        ...
        """
        Returns the `love_engine_trace_id` for this request

        This helps link sessions when multiple requests are made in a single session
        """
        dynamic_love_engine_session_id = love_engine_params.get("love_engine_session_id")
        dynamic_love_engine_trace_id = love_engine_params.get("love_engine_trace_id")

        # Note: we recommend using `love_engine_session_id` for session tracking
        # `love_engine_trace_id` is an internal love_engine param
        if dynamic_love_engine_session_id:
            return str(dynamic_love_engine_session_id)
        elif dynamic_love_engine_trace_id:
            return str(dynamic_love_engine_trace_id)
        ...
        return logging_obj.love_engine_trace_id
    ...
    def _get_user_agent_tags(proxy_server_request: dict) -> Optional[List[str]]:
        ...
        """
        if love_engine.disable_add_user_agent_to_request_tags is True:
            return None
    ...
    def _get_extra_header_tags(proxy_server_request: dict) -> Optional[List[str]]:
        ...
        extra_headers: List[str] = (
            getattr(love_engine, "extra_spend_tag_headers", None) or []
        )
    ...
    def _get_request_tags(
        love_engine_params: dict, proxy_server_request: dict
    ) -> List[str]:
        # check for 'tags' in both 'metadata' and 'love_engine_metadata'
        metadata = love_engine_params.get("metadata") or {}
        love_engine_metadata = love_engine_params.get("love_engine_metadata") or {}
        if metadata.get("tags", []):
            request_tags = metadata.get("tags", []).copy()
        elif love_engine_metadata.get("tags", []):
            request_tags = love_engine_metadata.get("tags", []).copy()
        else:
            request_tags = []
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        proxy_server_request = love_engine_params.get("proxy_server_request") or {}

        # Merge both love_engine_metadata and metadata to get complete metadata
        metadata: dict = StandardLoggingPayloadSetup.merge_love_engine_metadata(
            love_engine_params
        )
        ...
        id = response_obj.get("id", kwargs.get("love_engine_call_id"))

        _model_id = metadata.get("model_info", {}).get("id", "")
        _model_group = metadata.get("model_group", "")

        request_tags = StandardLoggingPayloadSetup._get_request_tags(
            love_engine_params=love_engine_params, proxy_server_request=proxy_server_request
        )
        ...
        custom_pricing = use_custom_pricing_for_model(love_engine_params=love_engine_params)
        raw_response_cost = kwargs.get("response_cost")
        response_cost: float = raw_response_cost or 0.0

        # clean up love_engine hidden params
        ...
        payload: StandardLoggingPayload = StandardLoggingPayload(
            id=str(id),
            love_engine_call_id=kwargs.get("love_engine_call_id")
            or love_engine_params.get("love_engine_call_id"),
            ...
            api_base=StandardLoggingPayloadSetup.strip_trailing_slash(
                love_engine_params.get("api_base", "")
            )
            or "",
...
def emit_standard_logging_payload(payload: StandardLoggingPayload):
    if os.getenv("LOVE_ENGINE_PRINT_STANDARD_LOGGING_PAYLOAD"):
...
def scrub_sensitive_keys_in_metadata(love_engine_params: Optional[dict]):
    if love_engine_params is None:
        love_engine_params = {}
    ...
    if "langfuse_masking_function" in metadata:
        ...
        if callable(masking_fn):
            love_engine_params["_langfuse_masking_function"] = masking_fn
        love_engine_params["metadata"] = metadata
        ...
        for k, v in metadata["user_api_key_metadata"].items():
            if k == "logging":  # prevent logging user logging keys
                cleaned_user_api_key_metadata[k] = (
                    "scrubbed_by_love_engine_for_sensitive_keys"
                )
            else:
