"""
Pulls the latest LoveEngine blog posts from the docs RSS feed.

Falls back to the bundled local backup on any failure.
RSS URL is configured via love_engine.blog_posts_url (or LOVE_ENGINE_BLOG_POSTS_URL env var).

Disable remote fetching entirely:
    export LOVE_ENGINE_LOCAL_BLOG_POSTS=True
"""
...
class GetBlogPosts:
    """
    Fetches, validates, and caches LoveEngine blog posts.
    ...
    def load_local_blog_posts() -> List[Dict[str, str]]:
        ...
        content = json.loads(
            files("love_engine").joinpath("blog_posts.json").read_text(encoding="utf-8")
        )
    ...
    def validate_blog_posts(posts: List[Dict[str, str]]) -> bool:
        ...
        if not isinstance(posts, list) or len(posts) == 0:
            verbose_logger.warning(
                "LoveEngine: Parsed RSS feed has no valid posts. "
    ...
    def get_blog_posts(cls, url: str) -> List[Dict[str, str]]:
        ...
        """
        if os.getenv("LOVE_ENGINE_LOCAL_BLOG_POSTS", "").lower() == "true":
        ...
        try:
            ...
            verbose_logger.warning(
                "LoveEngine: Failed to fetch blog posts from %s: %s. "
                "Falling back to local backup.",
                url,
                str(e),
            )
