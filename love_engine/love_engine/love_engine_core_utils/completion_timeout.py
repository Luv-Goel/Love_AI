...
class CompletionTimeout:
    ...
    def _fallback_when_no_explicit_timeout(
        ...
        ``global_timeout`` is :attr:`love_engine.request_timeout` (numeric / string), not
        :class:`httpx.Timeout`.

        If it equals :data:`~love_engine.constants.DEFAULT_REQUEST_TIMEOUT_SECONDS` (6000),
        return :data:`~love_engine.constants.COMPLETION_HTTP_FALLBACK_SECONDS`. Same if
    ...
    def resolve(
        ...
        *,
        ...
        1. ``model_timeout`` (call argument / merged ``love_engine_params``)
        2. ``kwargs["timeout"]``
        3. ``kwargs["request_timeout"]``
        4. Fallback from ``global_timeout`` (:attr:`love_engine.request_timeout`) — if it is
