...
import love_engine
from love_engine.constants import REPLICATE_MODEL_NAME_WITH_ID_LENGTH
from love_engine.llms.openai_like.json_loader import JSONProviderRegistry
from love_engine.secret_managers.main import get_secret, get_secret_str

from ..types.router import GenericLoveEngineParams, LOVE_ENGINE_Params
...
def _is_non_openai_azure_model(model: str) -> bool:
    try:
        ...
        if (
            model_name in love_engine.cohere_chat_models
            or f"mistral/{model_name}" in love_engine.mistral_chat_models
        ):
            return True
    except Exception:
        return False
...
def handle_cohere_chat_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if custom_llm_provider == "cohere" and model in love_engine.cohere_chat_models:
    ...
    if model and "/" in model:
        ...
        if (
            ...
            and _model in love_engine.cohere_chat_models
        ):
...
def handle_anthropic_text_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if (
            ...
            and love_engine.AnthropicTextConfig._is_anthropic_text_model(model)
        ):
    ...
    if model and "/" in model:
        ...
        if (
            ...
            and love_engine.AnthropicTextConfig._is_anthropic_text_model(_model)
        ):
...
def get_llm_provider(
    ...
    For router -> Can also give the whole love_engine param dict -> this function will extract the relevant details
    ...
    try:
        ...
        if love_engine.LoveEngineProxyChatConfig._should_use_love_engine_proxy_by_default(
            love_engine_params=cast(Optional[LOVE_ENGINE_Params], love_engine_params)
        ):
            return love_engine.LoveEngineProxyChatConfig.love_engine_proxy_get_custom_llm_provider_info(
                model=model, api_base=api_base, api_key=api_key
            )

        ## IF LOVE_ENGINE PARAMS GIVEN ##
        ...
        # the model may still carry LoveEngine's "openrouter/" routing prefix.
        ...
        if (
            model.split("/", 1)[0] in love_engine.provider_list
            and model.split("/", 1)[0] not in love_engine.model_list_set
            and len(model.split("/"))
            > 1  # handle edge case where user passes in `love_engine --model mistral` https://github.com/BerriAI/love_engine/issues/1351
        ):
            return _get_openai_compatible_provider_info(
                model=model,
                api_base=api_base,
        ...
        elif model.split("/", 1)[0] in love_engine.provider_list:
        ...
        if api_base:
            for endpoint in love_engine.openai_compatible_endpoints:
                if _endpoint_matches_api_base(endpoint, api_base):
                    ...
                    elif endpoint == love_engine.NscaleConfig.API_BASE_URL:
                        custom_llm_provider = "nscale"
                        dynamic_api_key = love_engine.NscaleConfig.get_api_key()
        ...
        if (
            ...
            or model in love_engine.openai_video_generation_models
        ):
            custom_llm_provider = "openai"
        elif model in love_engine.open_ai_text_completion_models:
            custom_llm_provider = "text-completion-openai"
        ## anthropic
        elif model in love_engine.anthropic_models:
            if love_engine.AnthropicTextConfig._is_anthropic_text_model(model):
        ...
        ## anthropic - pattern-based matching for future Claude models
            ...
            custom_llm_provider = "anthropic"
        ## cohere
        elif model in love_engine.cohere_models or model in love_engine.cohere_embedding_models:
            custom_llm_provider = "cohere"
        ## cohere chat models
        elif model in love_engine.cohere_chat_models:
            custom_llm_provider = "cohere_chat"
        ## replicate
        elif model in love_engine.replicate_models or (
        ...
        ## maritalk
        elif model in love_engine.maritalk_models:
        ...
        ## vertex - text + chat + language (gemini) models
            ...
            or model in love_engine.vertex_embedding_models
            or model in love_engine.vertex_vision_models
            or model in love_engine.vertex_ai_image_models
            or model in love_engine.vertex_ai_video_models
        ):
            custom_llm_provider = "vertex_ai"
        ## ai21
        elif model in love_engine.ai21_chat_models or model in love_engine.ai21_models:
            custom_llm_provider = "ai21_chat"
            api_base = (
                api_base
        ...
        ## petals
        elif model in love_engine.petals_models:
            custom_llm_provider = "petals"
        ## bedrock
        elif (
            model in love_engine.bedrock_models
            or model in love_engine.bedrock_embedding_models
            or model in love_engine.bedrock_converse_models
        ):
            custom_llm_provider = "bedrock"
        elif model in love_engine.watsonx_models:
            custom_llm_provider = "watsonx"
        # openai embeddings
        elif model in love_engine.open_ai_embedding_models:
            custom_llm_provider = "openai"
        elif model in love_engine.empower_models:
            custom_llm_provider = "empower"
        elif model in love_engine.gradient_ai_models:
        ...
        if not custom_llm_provider:
            if love_engine.suppress_debug_info is False:
                print()  # noqa: T201
                print(  # noqa: T201
                    "\033[1;31mProvider List: https://docs.love_engine.ai/docs/providers\033[0m"
                )
                print()  # noqa: T201
            error_str = f"LLM Provider NOT provided. Pass in the LLM provider you are trying to call. You passed model={model}\n Pass model as E.g. For 'Huggingface' inference endpoints pass in `completion(model='huggingface/starcoder',..)` Learn more: https://docs.love_engine.ai/docs/providers"
            # maps to openai.NotFoundError, this is raised when openai does not recognize the llm
            raise love_engine.exceptions.BadRequestError(  # type: ignore
                message=error_str,
                model=model,
                response=None,
                llm_provider="",
        ...
        if isinstance(e, love_engine.exceptions.BadRequestError):
            raise e
        else:
            error_str = (
...
def _get_openai_compatible_provider_info(
    ...
    if custom_llm_provider == "perplexity":
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.PerplexityChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.DeepInfraConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.GroqChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.BedrockMantleChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key, love_engine_params=love_engine_params
        )
        ...
        if api_base is None:
            api_base = love_engine.BasetenConfig.get_api_base_for_model(model)
        else:
            api_base = (
                api_base
        ...
        custom_llm_provider == "ai21" and model in love_engine.ai21_chat_models
    ):
        api_base = (
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.HostedVLLMChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.LlamafileChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.DataRobotConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.LMStudioChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.FireworksAIConfig()._get_openai_compatible_provider_info(
        ...
        (
            ...
            dynamic_api_key,
            custom_llm_provider,
        ) = love_engine.AzureAIStudioConfig()._get_openai_compatible_provider_info(
    ...
    elif custom_llm_provider == "love_engine_proxy":
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.LoveEngineProxyChatConfig()._get_openai_compatible_provider_info(
            api_base=api_base, api_key=api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.MistralConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            ...
            api_base,
            dynamic_api_key,
        ) = love_engine.JinaAIEmbeddingConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.XAIChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.ZAIChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            ...
            dynamic_api_key,
            custom_llm_provider,
        ) = love_engine.GithubCopilotConfig()._get_openai_compatible_provider_info(
            model, api_base, api_key, custom_llm_provider
        )
        ...
        (
            ...
            dynamic_api_key,
            custom_llm_provider,
        ) = love_engine.ChatGPTConfig()._get_openai_compatible_provider_info(
            model, api_base, api_key, custom_llm_provider
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.SnowflakeConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.GradientAIConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.FeatherlessAIConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.NscaleConfig()._get_openai_compatible_provider_info(
            api_base=api_base, api_key=api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.HerokuChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.DashScopeChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.ModelScopeChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.MoonshotChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
    # publicai is now handled by JSON config (see love_engine/llms/openai_like/providers.json)
    elif custom_llm_provider == "docker_model_runner":
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.DockerModelRunnerChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.V0ChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.MorphChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.LambdaAIChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.InceptionChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.HyperbolicChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.VercelAIGatewayConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.AIMLChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.LemonadeChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = love_engine.ClarifaiConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            ...
            dynamic_api_key,
            _,
        ) = love_engine.RAGFlowConfig()._get_openai_compatible_provider_info(
