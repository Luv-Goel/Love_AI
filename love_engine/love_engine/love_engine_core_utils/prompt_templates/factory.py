...
import love_engine
import love_engine.types
import love_engine.types.llms
from love_engine import verbose_logger
from love_engine._uuid import uuid
from love_engine.love_engine_core_utils.url_utils import async_safe_get, safe_get
from love_engine.llms.custom_httpx.http_handler import HTTPHandler, get_async_httpx_client
from love_engine.types.files import get_file_extension_from_mime_type
from love_engine.types.llms.anthropic import *
from love_engine.types.llms.bedrock import CachePointBlock
from love_engine.types.llms.bedrock import MessageBlock as BedrockMessageBlock
from love_engine.types.llms.custom_http import httpxSpecialProvider
from love_engine.types.llms.ollama import OllamaVisionModelObject
...
from love_engine.types.llms.vertex_ai import FunctionCall as VertexFunctionCall
from love_engine.types.llms.vertex_ai import FunctionResponse as VertexFunctionResponse
from love_engine.types.llms.vertex_ai import PartType as VertexPartType
from love_engine.types.utils import GenericImageParsingChunk
...
}  # similar to autogen. Only used if `love_engine.modify_params=True`.
...
async def _afetch_and_extract_template(
    ...
    """
    from love_engine.love_engine_core_utils.prompt_templates.huggingface_template_handler import (
    ...
    if chat_template is None:
        ...
        if model in love_engine.known_tokenizer_config:
            tokenizer_config = love_engine.known_tokenizer_config[model]
        else:
            tokenizer_config = await get_config_fn(hf_model_name=model)
            love_engine.known_tokenizer_config.update({model: tokenizer_config})
...
def convert_to_anthropic_image_obj(
    ...
    except love_engine.ImageFetchError:
        raise
    except Exception as e:
        raise Exception(
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1. Anthropic supports roles like "user" and "assistant", (here love_engine translates system-> assistant)
    2. The first message always needs to be of role "user"
    3. Each message must alternate between "user" and "assistant" (this is not addressed as now by love_engine)
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if love_engine.modify_params:
            new_messages.insert(
                0, {"role": "user", "content": [{"type": "text", "text": "."}]}
            )
        else:
            raise Exception(
                "Invalid first message. Should always start with 'role'='user' for Anthropic. System prompt is sent separately for Anthropic. set 'love_engine.modify_params = True' or 'love_engine_settings:modify_params = True' on proxy, to insert a placeholder user message - '.' as the first message, "
            )
...
def _get_thought_signature_from_tool(
    ...
    from love_engine.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        VertexGeminiConfig,
    )
...
def convert_to_gemini_tool_call_result(
    ...
    """
    from love_engine.types.llms.vertex_ai import BlobType
...
def convert_to_anthropic_tool_invoke(
    ...
    Fixes: https://github.com/BerriAI/love_engine/issues/17737
    """
...
def sanitize_messages_for_tool_calling(
    ...
    """
    if not love_engine.modify_params:
        return messages
...
def anthropic_messages_pt(
    ...
    # for Anthropic-shaped requests, independent of `love_engine.modify_params`,
    ...
    if len(messages) == 0:
        if not love_engine.modify_params:
            raise love_engine.BadRequestError(
                message=f"Anthropic requires at least one non-system message. Either provide one, or set `love_engine.modify_params = True` // `love_engine_settings::modify_params: True` to add the dummy user message - {DEFAULT_USER_CONTINUE_MESSAGE_TYPED}.",
                model=model,
    ...
    while msg_i < len(messages):
        ...
        while msg_i < len(messages) and messages[msg_i]["role"] == "assistant":
            ...
            # Fixes: https://github.com/BerriAI/love_engine/issues/23047
...
from love_engine.types.llms.cohere import (
    CallObject,
    ChatHistory,
    ChatHistoryChatBot,
    ChatHistorySystem,
...
def amazon_titan_pt(
    messages: list,
):  # format - https://github.com/BerriAI/love_engine/issues/1896
    """
    ...
    try:
        ...
        for message in messages:
            if isinstance(message["content"], str):
                ...
                # see https://docs.love_engine.ai/docs/providers/openai#openai-vision-models
...
from love_engine.types.llms.bedrock import (
    BedrockConverseReasoningContentBlock,
    BedrockConverseReasoningTextBlock,
)
from love_engine.types.llms.bedrock import ContentBlock as BedrockContentBlock
from love_engine.types.llms.bedrock import DocumentBlock as BedrockDocumentBlock
from love_engine.types.llms.bedrock import ImageBlock as BedrockImageBlock
from love_engine.types.llms.bedrock import SourceBlock as BedrockSourceBlock
from love_engine.types.llms.bedrock import BedrockToolSpec
from love_engine.types.llms.bedrock import ToolBlock as BedrockToolBlock
from love_engine.types.llms.bedrock import SearchResultBlock
from love_engine.types.llms.bedrock import ToolResultBlock as BedrockToolResultBlock
from love_engine.types.llms.bedrock import (
    ToolResultContentBlock as BedrockToolResultContentBlock,
)
from love_engine.types.llms.bedrock import ToolUseBlock as BedrockToolUseBlock
from love_engine.types.llms.bedrock import VideoBlock as BedrockVideoBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        supported_image_formats = (
            love_engine.AmazonConverseConfig().get_supported_image_types()
        )
        supported_doc_formats = (
            love_engine.AmazonConverseConfig().get_supported_document_types()
        )
        supported_video_formats = (
            love_engine.AmazonConverseConfig().get_supported_video_types()
        )
    ...
    def _get_document_format(mime_type: str, supported_doc_formats: List[str]) -> str:
        ...
        Relevant Issue: https://github.com/BerriAI/love_engine/issues/12260
        ...
        #################
        # love_engine runs on docker containers and `mimetypes` depends on the installed mimetypes of the OS
            ...
            if "function" in tool:
                ...
                if not arguments or not arguments.strip():
                    ...
                    try:
                        ...
                        # Fixes: https://github.com/BerriAI/love_engine/issues/20543
...
def _insert_assistant_continue_message(
    ...
    Conversation blocks and tool result blocks cannot be provided in the same turn. Issue: https://github.com/BerriAI/love_engine/issues/6053
    """
    ...
    elif love_engine.modify_params:
...
def get_user_message_block_or_continue_message(
    ...
    Relevant Issue: https://github.com/BerriAI/love_engine/issues/7169
    """
    content_block = message.get("content", None)

    # Handle None case
    if content_block is None or (
        user_continue_message is None and love_engine.modify_params is False
    ):
...
def get_assistant_message_block_or_continue_message(
    ...
    # Handle Base case
    if content_block is None or (
        assistant_continue_message is None and love_engine.modify_params is False
    ):
        return skip_empty_text_blocks(message=message)
...
class BedrockConverseMessagesProcessor:
                    ...
                    for element in message_block["content"]:
                        if isinstance(element, dict):
                            if element["type"] == "text":
                                ...
                                _parts.append(_part)
                            _cache_point_block = (
                                love_engine.AmazonConverseConfig()._get_cache_point_block(
                                    message_block=cast(
            ...
            if user_content:
                if len(contents) > 0 and contents[-1]["role"] == "user":
                    if (
                        ...
                        or love_engine.modify_params is True
                    ):
    ...
    def add_thinking_blocks_to_assistant_content(
        ...
        Relevant Issue: https://github.com/BerriAI/love_engine/issues/9063
        """
...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    if input_tool_name != valid_string:
        ...
        love_engine.bedrock_tool_name_mappings.set_cache(
...
def add_cache_point_tool_block(
    ...
    from love_engine.llms.bedrock.common_utils import is_claude_4_5_on_bedrock
...
def _bedrock_tools_pt(
    ...
    """
    from love_engine.llms.bedrock.common_utils import (
        get_bedrock_base_model,
        normalize_json_schema_custom_types_to_object,
    )
    from love_engine.love_engine_core_utils.prompt_templates.common_utils import unpack_defs
    ...
    for tool_idx, tool in enumerate(tools):
        ...
        if not (raw_name and str(raw_name).strip()):
            raw_name = f"love_engine_unnamed_tool_{tool_idx}"

        # related issue: https://github.com/BerriAI/love_engine/issues/5007
...
def response_schema_prompt(model: str, response_schema: dict) -> str:
    ...
    if f"{model}/response_schema_prompt" in love_engine.custom_prompt_dict:
        custom_prompt_details = love_engine.custom_prompt_dict[
            f"{model}/response_schema_prompt"
        ]  # allow user to define custom response schema prompt by model
    elif "response_schema_prompt" in love_engine.custom_prompt_dict:
        custom_prompt_details = love_engine.custom_prompt_dict["response_schema_prompt"]
...
def prompt_factory(
    ...
    if custom_llm_provider == "ollama":
        ...
        if (
            ...
            or love_engine.supports_vision(model=model)
            or love_engine.supports_vision(model=custom_llm_provider + "/" + model)
        ):
            return _gemini_vision_convert_messages(messages=messages)
        else:
            return gemini_text_image_pt(messages=messages)
    elif custom_llm_provider == "mistral":
        return love_engine.MistralConfig()._transform_messages(
        ...
        from love_engine.llms.watsonx.chat.transformation import IBMWatsonXChatConfig
