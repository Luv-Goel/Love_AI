...
VERTEX_AI_MANAGED_GCS_PREFIX = "love_engine-vertex-files/"
BEDROCK_MANAGED_S3_BATCH_PREFIX = "love_engine-bedrock-files-"
BEDROCK_MANAGED_S3_UPLOAD_PREFIX = "love_engine-bedrock-files/"
BEDROCK_MANAGED_S3_OUTPUT_PREFIX = "love_engine-batch-outputs/"
...
def should_allow_legacy_cloud_file_ids(
    love_engine_params: Optional[Mapping[str, Any]] = None,
) -> bool:
    value = None
    if isinstance(love_engine_params, Mapping):
        trusted_model_credentials = love_engine_params.get(
            "_love_engine_internal_model_credentials"
        )
...
def validate_managed_cloud_file_id(
    ...
    raise ValueError("file_id must reference a LoveEngine-managed storage object")
