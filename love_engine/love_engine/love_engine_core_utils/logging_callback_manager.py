...
from love_engine.constants import MAX_CALLBACKS
from love_engine.integrations.additional_logging_utils import AdditionalLoggingUtils
...
from love_engine.types.utils import CallbacksByType
...
class LoggingCallbackManager:
    """
    A centralized class that allows easy add / remove callbacks for love_engine.
    ...
    def add_love_engine_input_callback(self, callback: Union[CustomLogger, str, Callable]):
        """
        Add a input callback to love_engine.input_callback.
        Auto-routes async callbacks to love_engine._async_input_callback.
        """
        if not isinstance(callback, str) and self._is_async_callable(callback):
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine._async_input_callback
            )
        else:
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine.input_callback
            )

    def add_love_engine_service_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a service callback to love_engine.service_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=love_engine.service_callback
        )

    def add_love_engine_callback(self, callback: Union[CustomLogger, str, Callable]):
        """
        Add a callback to love_engine.callbacks

        Ensures no duplicates are added.
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=love_engine.callbacks  # type: ignore
        )

    def add_love_engine_success_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a success callback to `love_engine.success_callback`.
        Auto-routes async callbacks to love_engine._async_success_callback.
        Special-cases 'dynamodb' and 'openmeter' as async callbacks.
        """
        if isinstance(callback, str) and callback in ("dynamodb", "openmeter"):
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine._async_success_callback
            )
            ...
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine.success_callback
            )

    def add_love_engine_failure_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a failure callback to `love_engine.failure_callback`.
        Auto-routes async callbacks to love_engine._async_failure_callback.
        """
        if not isinstance(callback, str) and self._is_async_callable(callback):
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine._async_failure_callback
            )
        else:
            self._safe_add_callback_to_list(
                callback=callback, parent_list=love_engine.failure_callback
            )

    def add_love_engine_async_success_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a success callback to love_engine._async_success_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=love_engine._async_success_callback
        )

    def add_love_engine_async_failure_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a failure callback to love_engine._async_failure_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=love_engine._async_failure_callback
        )
    ...
    def remove_callbacks_by_type(self, callback_list, callback_type):
        ...
        Args:
            callback_list: The list to remove callbacks from (e.g., love_engine.callbacks)
    ...
    def _add_custom_callback_generic_api_str(
        ...
        """
        callback_config = love_engine.callback_settings.get(callback)
    ...
    def _get_all_callbacks(self) -> List[Union[CustomLogger, Callable, str]]:
        """
        Get all callbacks from love_engine.callbacks, love_engine.success_callback, love_engine.failure_callback, love_engine._async_success_callback, love_engine._async_failure_callback
        """
        return (
            love_engine.callbacks
            + love_engine.success_callback
    ...
    def get_callbacks_by_type(self) -> CallbacksByType:
        ...
        success_callbacks = set(
            love_engine.success_callback + love_engine._async_success_callback
        )
        failure_callbacks = set(
            love_engine.failure_callback + love_engine._async_failure_callback
        )
        general_callbacks = set(love_engine.callbacks)
    ...
    def _get_callback_string(self, callback: Union[CustomLogger, Callable, str]) -> str:
        from love_engine.love_engine_core_utils.custom_logger_registry import (
