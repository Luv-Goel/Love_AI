"""
Pulls the cost + context window + provider route for known models from https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json

This can be disabled by setting the LOVE_ENGINE_LOCAL_MODEL_COST_MAP environment variable to True.

```
export LOVE_ENGINE_LOCAL_MODEL_COST_MAP=True
```
"""
...
class GetModelCostMap:
    ...
    def _check_is_valid_dict(fetched_map: dict) -> bool:
        ...
        if not isinstance(fetched_map, dict):
            verbose_logger.warning(
                "LoveEngine: Fetched model cost map is not a dict (type=%s). "
        ...
        if len(fetched_map) == 0:
            verbose_logger.warning(
                "LoveEngine: Fetched model cost map is empty. "
                "Falling back to local backup.",
            )
            return False
    ...
    def _check_model_count_not_reduced(
        ...
        if fetched_count < min_model_count:
            verbose_logger.warning(
                "LoveEngine: Fetched model cost map has only %d models (minimum=%d). "
        ...
        if (
            ...
            verbose_logger.warning(
                "LoveEngine: Fetched model cost map shrank significantly "
...
def _expand_model_aliases(model_cost: dict) -> dict:
    ...
    for model_name, model_info in model_cost.items():
        ...
        if not isinstance(aliases, list):
            verbose_logger.warning(
                "LoveEngine model alias field for '%s' is not a list (got %s) — skipping.",
                model_name,
        ...
        for alias in aliases:
            if alias in model_cost:
                verbose_logger.warning(
                    "LoveEngine model alias conflict: alias '%s' (from '%s') "
...
def get_model_cost_map(url: str) -> dict:
    ...
    1. If ``LOVE_ENGINE_LOCAL_MODEL_COST_MAP`` is set, uses the local backup only.
    ...
    """
    # Note: can't use get_secret_bool here — this runs during love_engine.__init__
    # before love_engine._key_management_settings is set.
    if os.getenv("LOVE_ENGINE_LOCAL_MODEL_COST_MAP", "").lower() == "true":
    ...
    try:
        ...
        verbose_logger.warning(
            "LoveEngine: Failed to fetch remote model cost map from %s: %s. "
            "Falling back to local backup.",
            url,
            str(e),
        )
    ...
    if not GetModelCostMap.validate_model_cost_map(
        ...
        verbose_logger.warning(
            "LoveEngine: Fetched model cost map failed integrity check. "
