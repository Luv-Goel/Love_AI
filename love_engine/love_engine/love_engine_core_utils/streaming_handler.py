...
from love_engine.love_engine_core_utils.model_response_utils import (
    is_model_response_stream_empty,
)
from love_engine.love_engine_core_utils.redact_messages import LoveEngineLoggingObject
from love_engine.love_engine_core_utils.thread_pool_executor import executor
from love_engine.types.llms.openai import OpenAIChatCompletionChunk
from love_engine.types.router import GenericLoveEngineParams
from love_engine.types.utils import (
    Delta,
)
from love_engine.types.utils import GenericStreamingChunk as GChunk
from love_engine.types.utils import (
    LlmProviders,
    ModelResponse,
    ModelResponseStream,
...
class CustomStreamWrapper:
    def __init__(
        ...
        self.logging_obj: LoveEngineLoggingObject = logging_obj
        self.completion_stream = completion_stream
        self.sent_first_chunk = False
        self.sent_last_chunk = False
        self._stream_created_time: float = time.time()

        love_engine_params: GenericLoveEngineParams = GenericLoveEngineParams(
            **self.logging_obj.model_call_details.get("love_engine_params", {})
        )
        self.merge_reasoning_content_in_choices: bool = (
            love_engine_params.merge_reasoning_content_in_choices or False
        )
        ...
        _model_info: Dict = love_engine_params.model_info or {}
        ...
        self._hidden_params = {
            ...
            "api_base": _api_base,
        }  # returned as x-love_engine-model-id response header in proxy
        ...
        # Snapshot assumes self._hidden_params is populated from love_engine_params
    ...
    def _check_max_streaming_duration(self) -> None:
        ...
        from love_engine.constants import LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS
    ...
    def raise_on_model_repetition(self) -> None:
        """
        Fixes - https://github.com/BerriAI/love_engine/issues/5158
        ...
        ):  # ignore empty content - https://github.com/BerriAI/love_engine/issues/5158#issuecomment-2287156946
            self._repeated_messages_count = 1
            return
        ...
        if self._repeated_messages_count >= love_engine.REPEATED_STREAMING_CHUNK_LIMIT:
    ...
    def handle_baseten_chunk(self, chunk):
        try:
            ...
            verbose_logger.exception(
                "love_engine.CustomStreamWrapper.handle_baseten_chunk(): Exception occured - {}".format(
                    str(e)
                )
            )
            return ""
    ...
    def model_response_creator(
        ...
        ):  # maintain same 'created' across all chunks - https://github.com/BerriAI/love_engine/issues/11437
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore
        ...
        try:
            ...
            if (
                ...
                and self.custom_llm_provider in love_engine._custom_providers
            ):
                _has_content = bool(
                    chunk.choices
            ...
            if (
                ...
                if anthropic_response_obj["usage"] is not None:
                    setattr(
                        model_response,
                        "usage",
                        love_engine.Usage(**anthropic_response_obj["usage"]),
                    )
                        ...
                        try:
                            ...
                            if "Part has no text." in str(e):
                                ...
                                try:
                                    ...
                                    raise e
                                _delta_obj = love_engine.utils.Delta(
                                    content=None,
                                    tool_calls=[
                                        {
                ...
                response_obj = cast(
                    Dict[str, Any],
                    love_engine.CodestralTextCompletionConfig()._chunk_parser(chunk),
                )
                completion_obj["content"] = response_obj["text"]
                print_verbose(f"completion obj content: {completion_obj['content']}")
                if response_obj["is_finished"]:
                ...
                if response_obj["usage"] is not None:
                    if isinstance(response_obj["usage"], dict):
                        ...
                        setattr(
                            model_response,
                            "usage",
                            love_engine.Usage(**response_obj["usage"].model_dump()),
                        )
                ...
                if original_chunk.choices and len(original_chunk.choices) > 0:
                    ...
                    if delta is not None and (
                        ...
                        try:
                            ...
                            verbose_logger.exception(
                                "love_engine.CustomStreamWrapper.chunk_creator(): Exception occured - {}".format(
                                    str(e)
                                )
                            )
                            model_response.choices[0].delta = Delta()
    ...
    def set_logging_event_loop(self, loop):
        """
        import love_engine, asyncio

        loop = asyncio.get_event_loop() # 👈 gets the current event loop

        response = love_engine.completion(.., stream=True)
    ...
    def run_success_logging_and_cache_storage(self, processed_chunk, cache_hit: bool):
        ...
        """
        if love_engine.disable_streaming_logging is True:
            """
            [NOT RECOMMENDED]
            Set this via `love_engine.disable_streaming_logging = True`.
        ...
        love_engine_params = self.logging_obj.model_call_details.get("love_engine_params", {})
        if self.logging_obj._is_sync_love_engine_request(love_engine_params):
        ...
        try:
            ...
            while True:
                ...
                if chunk is not None and chunk != b"":
                    ...
                    ## LOGGING
                    if not love_engine.disable_streaming_logging:
                        executor.submit(
    ...
    def fetch_sync_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = self.make_call(client=love_engine.module_level_client)
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = await self.make_call(
                client=love_engine.module_level_aclient
            )
    ...
    async def __anext__(self) -> "ModelResponseStream":
        ...
        try:
            ...
            traceback_exception = traceback.format_exc()
            ## ADD DEBUG INFORMATION - E.G. LOVE_ENGINE REQUEST TIMEOUT
            traceback_exception += "\nLoveEngine Default Request Timeout - {}".format(
                love_engine.request_timeout
            )
...
def convert_generic_chunk_to_model_response_stream(
    ...
    from love_engine.types.utils import Delta
