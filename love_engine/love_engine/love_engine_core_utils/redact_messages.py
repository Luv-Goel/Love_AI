...
from love_engine.llms.vertex_ai.common_utils import (
    redact_vertex_ai_metadata_from_love_engine_params,
...
if TYPE_CHECKING:
    from love_engine.love_engine_core_utils.love_engine_logging import (
        Logging as _LoveEngineLoggingObject,
    )

    LoveEngineLoggingObject = _LoveEngineLoggingObject
else:
    LoveEngineLoggingObject = Any


def redact_message_input_output_from_custom_logger(
    love_engine_logging_obj: LoveEngineLoggingObject, result, custom_logger: CustomLogger
):
    if (
        hasattr(custom_logger, "message_logging")
        and custom_logger.message_logging is not True
    ):
        return perform_redaction(love_engine_logging_obj.model_call_details, result)
    return result


def _redact_choice_content(choice):
    """Helper to redact content in a choice (message or delta)."""
    if isinstance(choice, love_engine.Choices):
        choice.message.content = "redacted-by-love_engine"
        if hasattr(choice.message, "reasoning_content"):
            choice.message.reasoning_content = "redacted-by-love_engine"
        if hasattr(choice.message, "thinking_blocks"):
            choice.message.thinking_blocks = None
    elif isinstance(choice, love_engine.utils.StreamingChoices):
        choice.delta.content = "redacted-by-love_engine"
        if hasattr(choice.delta, "reasoning_content"):
            choice.delta.reasoning_content = "redacted-by-love_engine"
...
def _redact_responses_api_output(output_items):
    ...
    for output_item in output_items:
        if hasattr(output_item, "text"):
            output_item.text = "redacted-by-love_engine"

        if hasattr(output_item, "content") and isinstance(output_item.content, list):
            for content_part in output_item.content:
                if hasattr(content_part, "text"):
                    content_part.text = "redacted-by-love_engine"
            ...
            if hasattr(output_item, "summary") and isinstance(
                ...
                for summary_item in output_item.summary:
                    if hasattr(summary_item, "text"):
                        summary_item.text = "redacted-by-love_engine"
...
def perform_redaction(model_call_details: dict, result):
    ...
    model_call_details["messages"] = [
        {"role": "user", "content": "redacted-by-love_engine"}
    ]
    model_call_details["prompt"] = ""
    model_call_details["input"] = ""
    _redact_standard_logging_object(model_call_details)
    redact_vertex_ai_metadata_from_love_engine_params(model_call_details)
    ...
    if result is not None:
        ...
        if (
            ...
            return {"text": "redacted-by-love_engine"}

        _result = copy.deepcopy(result)
        if isinstance(_result, love_engine.ModelResponse):
            ...
            if _result.get("choices") is not None:
                _redact_model_response_dict_choices(
                    _result["choices"], "redacted-by-love_engine"
                )
            redact_vertex_ai_metadata_from_logged_object(_result)
        elif isinstance(_result, dict) and "output" in _result:
            if isinstance(_result.get("output"), list):
                _redact_responses_api_output_dict(
                    _result["output"], "redacted-by-love_engine"
                )
        elif isinstance(_result, love_engine.ResponsesAPIResponse):
        ...
        elif isinstance(_result, love_engine.EmbeddingResponse):
...
def should_redact_message_logging(model_call_details: dict) -> bool:
    ...
    2. Headers (love_engine-disable-message-redaction / love_engine-enable-message-redaction)
    3. Global setting (love_engine.turn_off_message_logging)
    """
    love_engine_params = model_call_details.get("love_engine_params", {})

    metadata_field = get_metadata_variable_name_from_kwargs(love_engine_params)
    metadata = love_engine_params.get(metadata_field, {})
    if not isinstance(metadata, dict):
        # Fall back: love_engine_metadata was None, try metadata
    ...
    if request_headers and bool(
        request_headers.get("love_engine-disable-message-redaction", False)
    ):
        # User explicitly disabled redaction via header
        return False

    possible_enable_headers = [
        "love_engine-enable-message-redaction",  # old header. maintain backwards compatibility
        "x-love_engine-enable-message-redaction",  # new header
    ]
    ...
    return love_engine.turn_off_message_logging is True
...
def redact_message_input_output_from_logging(
    ...
    only redacts when love_engine.turn_off_message_logging == True
    """
...
def redact_user_api_key_info(metadata: dict) -> dict:
    ...
    SDK
    ```python
    love_engine.redact_user_api_key_info = True
    ```
    ...
    ```
    """
    if love_engine.redact_user_api_key_info is not True:
        return metadata
