...
from love_engine.love_engine_core_utils.default_encoding import encoding as default_encoding
from love_engine.llms.custom_httpx.http_handler import _get_httpx_client
from love_engine.love_engine_core_utils.url_utils import safe_get
...
from love_engine.types.utils import Message, SelectTokenizerResponse
...
def get_modified_max_tokens(
    ...
    try:
        ...
        ## MODEL INFO
        _model_info = love_engine.get_model_info(model=model)

        max_output_tokens = love_engine.get_max_tokens(
            model=base_model
        ...
        input_tokens = love_engine.token_counter(model=base_model, messages=messages)
        ...
        verbose_logger.debug(
            f"love_engine.love_engine_core_utils.token_counter.py::get_modified_max_tokens() - user_max_tokens: {user_max_tokens}"
        )

        return user_max_tokens
    except Exception as e:
        verbose_logger.debug(
            "love_engine.love_engine_core_utils.token_counter.py::get_modified_max_tokens() - Error while checking max token limit: {}\nmodel={}, base_model={}".format(
...
class _MessageCountParams:
    ...
    def __init__(
        ...
        from love_engine.utils import print_verbose
        ...
        elif actual_model in love_engine.open_ai_chat_completion_models:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        elif actual_model in love_engine.azure_llms:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        else:
            print_verbose(
...
def _get_count_function(
    ...
    from love_engine.utils import _select_tokenizer, print_verbose
...
def _fix_model_name(model: str) -> str:
    ...
    if model in love_engine.azure_llms:
        # azure llms use gpt-35-turbo instead of gpt-3.5-turbo 🙃
        return model.replace("-35", "-3.5")
    elif model in love_engine.open_ai_chat_completion_models:
...
def _count_anthropic_content(
    ...
    Uses TypedDict definitions from love_engine.types.llms.anthropic to determine
