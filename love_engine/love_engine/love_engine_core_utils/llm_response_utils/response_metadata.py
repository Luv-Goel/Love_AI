...
from love_engine.constants import LOVE_ENGINE_DETAILED_TIMING
...
from love_engine.love_engine_core_utils.logging_utils import LoveEngineLoggingObject
...
class ResponseMetadata:
    """
    Handles setting and managing `_hidden_params`, `response_time_ms`, and `love_engine_overhead_time_ms` for LoveEngine responses
    """
    ...
    def set_hidden_params(
        self, logging_obj: LoveEngineLoggingObject, model: Optional[str], kwargs: dict
    ) -> None:
        ...
        new_params = {
            "love_engine_call_id": getattr(logging_obj, "love_engine_call_id", None),
            "api_base": get_api_base(model=model or "", optional_params=kwargs),
            "model_id": model_id,
            "response_cost": logging_obj._response_cost_calculator(
                result=self.result, love_engine_model_name=model, router_model_id=model_id
            ),
            "additional_headers": process_response_headers(
                self._get_value_from_hidden_params("additional_headers") or {},
                preserve_love_engine_internal_headers=True,
            ),
    ...
    def set_timing_metrics(
        ...
        #########################################################
        # 2. Add LoveEngine overhead duration
        #########################################################
        ...
        if llm_api_duration_ms is not None:
            ...
            self._update_hidden_params(
                {
                    "love_engine_overhead_time_ms": overhead_ms,
                }
            )
        ...
        # In this case overhead from love_engine is the difference between the cache read duration and the total response time
        #########################################################
        if (
        ...
        #########################################################
        if LOVE_ENGINE_DETAILED_TIMING and llm_api_duration_ms is not None:
            detailed: dict = {
...
def update_response_metadata(
    ...
    Updates response metadata, adds the following:
        ...
        - response._hidden_params["love_engine_overhead_time_ms"]
