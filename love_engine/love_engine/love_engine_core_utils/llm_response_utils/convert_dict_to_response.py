...
from love_engine.types.llms.databricks import DatabricksTool
...
from love_engine.types.utils import Logprobs as TextCompletionLogprobs
from love_engine.types.utils import (
    Message,
    ModelResponse,
    ModelResponseStream,
...
def _safe_convert_created_field(created_value) -> int:
    ...
    (Unix timestamp with fractional seconds), but LoveEngine expects an integer.
...
def convert_tool_call_to_json_mode(
    ...
    if _should_convert_tool_call_to_json_mode(
        ...
        if json_mode_content_str is not None:
            message = love_engine.Message(content=json_mode_content_str)
            finish_reason = "stop"
...
async def convert_to_streaming_response_async(response_object: Optional[dict] = None):
    ...
    if not response_object.get("choices"):
        from love_engine.exceptions import APIError

        raise APIError(
            status_code=500,
            message=(
                "LoveEngine: provider returned a response with no 'choices'. "
...
class LoveEngineResponseObjectHandler:
    @staticmethod
    ...
    def convert_chat_to_text_completion(
        ...
        """
        transformed_logprobs = LoveEngineResponseObjectHandler._convert_provider_response_logprobs_to_text_completion_logprobs(
            response=response,
...
def convert_to_model_response_object(
    ...
    try:
        if response_type == "completion" and (
            ...
            if "usage" in response_object and response_object["usage"] is not None:
                usage_object = love_engine.Usage(**response_object["usage"])
            ...
            return LoveEngineResponseObjectHandler.convert_to_image_response(
