...
from ...love_engine_core_utils.get_llm_provider_logic import get_llm_provider
from ...types.router import LOVE_ENGINE_Params


def get_api_base(
    model: str, optional_params: Union[dict, LOVE_ENGINE_Params]
) -> Optional[str]:
    """
    Returns the api base used for calling the model.

    Parameters:
    - model: str - the model passed to love_engine.completion()
    - optional_params - the 'love_engine_params' in router.completion *OR* additional params passed to love_engine.completion - eg. api_base, api_key, etc. See `LOVE_ENGINE_Params` - https://github.com/BerriAI/love_engine/blob/f09e6ba98d65e035a79f73bc069145002ceafd36/love_engine/router.py#L67
    ...
    Example:
    ```
    from love_engine import get_api_base
    ...
    try:
        if isinstance(optional_params, LOVE_ENGINE_Params):
            _optional_params = optional_params
        elif "model" in optional_params:
            _optional_params = LOVE_ENGINE_Params(**optional_params)
        else:  # prevent needing to copy and pop the dict
            _optional_params = LOVE_ENGINE_Params(
    ...
    if love_engine.model_alias_map and model in love_engine.model_alias_map:
        model = love_engine.model_alias_map[model]
    try:
        (
            model,
            custom_llm_provider,
