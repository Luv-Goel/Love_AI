...
from love_engine.types.llms.openai import AllMessageValues, OpenAIChatCompletionFinishReason

if TYPE_CHECKING:
    from opentelemetry.trace import Span as _Span

    from love_engine.types.utils import ModelResponseStream
...
def add_missing_spend_metadata_to_love_engine_metadata(
    love_engine_metadata: dict, metadata: dict
) -> dict:
    """
    Helper to get love_engine metadata for spend tracking

    PATCH for issue where both `love_engine_metadata` and `metadata` are present in the kwargs
    ...
    for key, value in metadata.items():
        if potential_spend_tracking_metadata_substring in key:
            love_engine_metadata[key] = value
...
def get_love_engine_metadata_from_kwargs(kwargs: dict):
    """
    Helper to get love_engine metadata from all love_engine request kwargs

    Return `love_engine_metadata` if it exists, otherwise return `metadata`
    """
    love_engine_params = kwargs.get("love_engine_params", {})
    if love_engine_params:
        metadata = love_engine_params.get("metadata", {})
        love_engine_metadata = love_engine_params.get("love_engine_metadata", {})
        if love_engine_metadata and metadata:
            love_engine_metadata = add_missing_spend_metadata_to_love_engine_metadata(
...
def _get_parent_otel_span_from_kwargs(
    ...
    try:
        ...
        if "love_engine_parent_otel_span" in _metadata:
            return _metadata["love_engine_parent_otel_span"]
        elif (
            love_engine_params is not None
            and love_engine_params.get("metadata") is not None
            and "love_engine_parent_otel_span" in love_engine_params.get("metadata", {})
        ):
            return love_engine_params["metadata"]["love_engine_parent_otel_span"]
        elif "love_engine_parent_otel_span" in kwargs:
            return kwargs["love_engine_parent_otel_span"]
        return None
...
def process_response_headers(
    ...
    preserve_love_engine_internal_headers: bool = False,
) -> dict:
    """
    `preserve_love_engine_internal_headers` must only be True when the input is a
    LoveEngine-owned dict (e.g. `_hidden_params["additional_headers"]` that has
    ...
    remain False, otherwise a malicious provider returning `x-love_engine-*` could
    spoof LoveEngine-internal markers (e.g. `x-love_engine-attempted-fallbacks`).
    ...
    always a raw provider response and can never be LoveEngine-owned.
    """
    from love_engine.types.utils import OPENAI_RESPONSE_HEADERS

    # Raw httpx.Headers objects come directly from provider HTTP responses and
    # must never be treated as LoveEngine-owned, regardless of caller intent.
    _preserve = preserve_love_engine_internal_headers and isinstance(response_headers, dict)
    ...
    for k, v in response_headers.items():
        ...
        elif _preserve and k.startswith("x-love_engine-"):
            # LoveEngine's own internal headers (e.g. x-love_engine-attempted-fallbacks,
            # x-love_engine-model-group) are not LLM provider headers and must not be
...
def safe_deep_copy(data):
    ...
    The LoveEngine request may contain objects that cannot be pickled/deep-copied
    ...
    if love_engine.safe_memory_mode is True:
        return data

    love_engine_parent_otel_span: Optional[Any] = None
    # Step 1: Remove the love_engine_parent_otel_span
    love_engine_parent_otel_span = None
    if isinstance(data, dict):
        # remove love_engine_parent_otel_span since this is not picklable
        if "metadata" in data and "love_engine_parent_otel_span" in data["metadata"]:
            love_engine_parent_otel_span = data["metadata"].pop("love_engine_parent_otel_span")
            data["metadata"]["love_engine_parent_otel_span"] = "placeholder"
        if (
            "love_engine_metadata" in data
            and "love_engine_parent_otel_span" in data["love_engine_metadata"]
        ):
            love_engine_parent_otel_span = data["love_engine_metadata"].pop(
                "love_engine_parent_otel_span"
            )
            data["love_engine_metadata"]["love_engine_parent_otel_span"] = "placeholder"
    ...
    # Step 3: re-add the love_engine_parent_otel_span after doing a deep copy
    if isinstance(data, dict) and love_engine_parent_otel_span is not None:
        if "metadata" in data and "love_engine_parent_otel_span" in data["metadata"]:
            data["metadata"]["love_engine_parent_otel_span"] = love_engine_parent_otel_span
        if (
            "love_engine_metadata" in data
            and "love_engine_parent_otel_span" in data["love_engine_metadata"]
        ):
            data["love_engine_metadata"][
                "love_engine_parent_otel_span"
            ] = love_engine_parent_otel_span
    return new_data
...
def filter_exceptions_from_params(data: Any, max_depth: int = 20) -> Any:
    ...
    if obj_type_name in ["Logging", "LoveEngineLoggingObj", "Router"]:
        return None
...
def filter_internal_params(
    ...
    """
    Filter out LoveEngine internal parameters that shouldn't be sent to provider APIs.

    This removes internal/MCP-related parameters that are used by LoveEngine internally
