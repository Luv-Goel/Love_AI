...
def _get_web_search_requests(server_tool_use: Any) -> Optional[int]:
    ...
    See https://github.com/BerriAI/love_engine/issues/26153 — ``stream_chunk_builder``
...
def _generic_cost_per_character(
    ...
    try:
        ...
        verbose_logger.exception(
            "love_engine.love_engine_core_utils.llm_cost_calc.utils.py::cost_per_character(): Exception occured - {}\nDefaulting to None".format(
                str(e)
            )
        )
...
def _get_cost_per_unit(
    ...
    if isinstance(cost_per_unit, str):
        try:
            ...
            verbose_logger.exception(
                f"love_engine.love_engine_core_utils.llm_cost_calc.utils.py::calculate_cost_per_component(): Exception occured - {cost_per_unit}\nDefaulting to 0.0"
            )
            ...
            if suffix in cost_key:
                ...
                if isinstance(fallback_cost, str):
                    try:
                        ...
                        verbose_logger.exception(
                            f"love_engine.love_engine_core_utils.llm_cost_calc.utils.py::_get_cost_per_unit(): Exception occured - {fallback_cost}\nDefaulting to 0.0"
                        )
...
def generic_cost_per_token(
    ...
    Input:
        - model: str, the model name without provider prefix
        - usage: LoveEngine Usage block, containing anthropic caching information
    ...
    ## Ref: https://github.com/BerriAI/love_engine/issues/19680, #14874, #14875
...
def calculate_image_response_web_search_cost(
    ...
    from love_engine.llms import get_cost_for_web_search_request
...
class CostCalculatorUtils:
    ...
    def route_image_generation_cost_calculator(
        ...
        from love_engine.llms.azure_ai.image_generation.cost_calculator import (
            cost_calculator as azure_ai_image_cost_calculator,
        )
        from love_engine.llms.bedrock.image_generation.cost_calculator import (
            cost_calculator as bedrock_image_cost_calculator,
        )
        from love_engine.llms.gemini.image_generation.cost_calculator import (
            cost_calculator as gemini_image_cost_calculator,
        )
        from love_engine.llms.recraft.cost_calculator import (
            cost_calculator as recraft_image_cost_calculator,
        )
        from love_engine.llms.vertex_ai.image_generation.cost_calculator import (
        ...
        if custom_llm_provider == love_engine.LlmProviders.VERTEX_AI.value:
            if isinstance(completion_response, ImageResponse):
                return vertex_ai_image_cost_calculator(
                    model=model,
                    image_response=completion_response,
                )
        elif custom_llm_provider == love_engine.LlmProviders.BEDROCK.value:
            if isinstance(completion_response, ImageResponse):
        ...
        elif custom_llm_provider == love_engine.LlmProviders.RECRAFT.value:
            from love_engine.llms.recraft.cost_calculator import (
                cost_calculator as recraft_image_cost_calculator,
            )
        ...
        elif custom_llm_provider == love_engine.LlmProviders.AIML.value:
            from love_engine.llms.aiml.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.COMETAPI.value:
            from love_engine.llms.cometapi.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.GEMINI.value:
            if call_type in (
                CallTypes.image_edit.value,
                CallTypes.aimage_edit.value,
            ):
                from love_engine.llms.gemini.image_edit.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.AZURE_AI.value:
            return azure_ai_image_cost_calculator(
                model=model,
                image_response=completion_response,
            )
        elif custom_llm_provider == love_engine.LlmProviders.FAL_AI.value:
            from love_engine.llms.fal_ai.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.RUNWAYML.value:
            from love_engine.llms.runwayml.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.OPENAI.value:
            # gpt-image models use token-based pricing.
            model_lower = model.lower()
            if "gpt-image" in model_lower:
                from love_engine.llms.openai.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == love_engine.LlmProviders.AZURE.value:
            # gpt-image models use token-based pricing.
            model_lower = model.lower()
            if "gpt-image" in model_lower:
                from love_engine.llms.openai.image_generation.cost_calculator import (
