...
import love_engine
from love_engine.constants import OPENAI_FILE_SEARCH_COST_PER_1K_CALLS
from love_engine.love_engine_core_utils.llm_cost_calc.utils import _get_web_search_requests
...
class StandardBuiltInToolCostTracking:
    ...
    def _handle_web_search_cost(
        ...
        if custom_llm_provider is None and model_info is not None:
            custom_llm_provider = model_info["love_engine_provider"]
    ...
    def _safe_get_model_info(
        ...
        try:
            return love_engine.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
        except Exception:
            return None
    ...
    def get_cost_for_file_search(
        ...
        if provider == "azure":
            from love_engine.constants import AZURE_FILE_SEARCH_COST_PER_GB_PER_DAY
    ...
    def get_cost_for_vector_store(
        ...
        if provider == "azure":
            from love_engine.constants import AZURE_VECTOR_STORE_COST_PER_GB_PER_DAY
    ...
    def _get_code_interpreter_cost_from_model_map(
        ...
        try:
            ...
            if model_key and model_key in love_engine.model_cost:
                return love_engine.model_cost[model_key].get(
