...
from love_engine.files.streaming import FileContentStreamingResponse
from love_engine.files.types import FileContentProvider, FileContentStreamingResult
...
from love_engine.llms.azure.common_utils import get_azure_credentials
from love_engine.llms.azure.files.handler import AzureOpenAIFilesAPI
from love_engine.llms.bedrock.files.handler import BedrockFilesHandler
...
from love_engine.llms.openai.common_utils import get_openai_credentials
from love_engine.llms.openai.openai import FileDeleted, FileObject, OpenAIFilesAPI
from love_engine.llms.vertex_ai.files.handler import VertexAIFilesHandler
...
def _add_trusted_model_credentials_to_love_engine_params(
    love_engine_params_dict: Dict[str, Any], kwargs: Dict[str, Any]
) -> None:
    trusted_model_credentials = kwargs.get("_love_engine_internal_model_credentials")
    if isinstance(trusted_model_credentials, type(MappingProxyType({}))):
        love_engine_params_dict["_love_engine_internal_model_credentials"] = (
...
async def acreate_file(
    ...
    **kwargs,
    ...
    love_engine Equivalent of POST: POST https://api.openai.com/v1/files
    """
    try:
...
def create_file(
    ...
    **kwargs,
    ...
    try:
        ...
        love_engine_params_dict = dict(**kwargs)
        logging_obj = cast(
            Optional[LoveEngineLoggingObj], kwargs.get("love_engine_logging_obj")
        )
        ...
        if provider_config is not None:
            ...
            raise love_engine.exceptions.BadRequestError(
                message="love_engine doesn't support {} for 'create_file'. Only ['openai', 'azure', 'vertex_ai', 'manus', 'anthropic'] are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                ...
                response=httpx.Response(
                    ...
                    request=httpx.Request(method="create_file", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
...
async def afile_retrieve(
    ...
    **kwargs,
    ...
    love_engine Equivalent of GET https://api.openai.com/v1/files
    """
    try:
        loop = asyncio.get_event_loop()
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            if provider_config is not None:
                ...
                _add_trusted_model_credentials_to_love_engine_params(
                    love_engine_params_dict=love_engine_params_dict,
                    kwargs=kwargs,
                )
                love_engine_params_dict["api_key"] = optional_params.api_key
                love_engine_params_dict["api_base"] = optional_params.api_base

                logging_obj = kwargs.get("love_engine_logging_obj")
                if logging_obj is None:
                    from love_engine.love_engine_core_utils.love_engine_logging import (
                        Logging as LoveEngineLoggingObj,
                    )
                    ...
                    logging_obj = LoveEngineLoggingObj(
                        ...
                        love_engine_call_id=kwargs.get(
                            "love_engine_call_id", str(uuid_module.uuid4())
                        ),
                ...
                raise love_engine.exceptions.BadRequestError(
                    message="love_engine doesn't support {} for 'file_retrieve'. Only 'openai', 'azure', 'manus', and 'anthropic' are supported.".format(
                        custom_llm_provider
                    ),
                    model="n/a",
                    llm_provider=custom_llm_provider,
                    response=httpx.Response(
                        status_code=400,
                        content="Unsupported provider",
                        request=httpx.Request(method="create_thread", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                    ),
                )
...
async def afile_delete(
    ...
    **kwargs,
    ...
    love_engine Equivalent of DELETE https://api.openai.com/v1/files
    """
    try:
        loop = asyncio.get_event_loop()
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            if provider_config is not None:
                ...
                raise love_engine.exceptions.BadRequestError(
                    message="love_engine doesn't support {} for 'file_delete'. Only 'openai', 'azure', 'gemini', 'manus', and 'anthropic' are supported.".format(
                        custom_llm_provider
                    ),
                    model="n/a",
                    llm_provider=custom_llm_provider,
    ...
    **kwargs,
    ...
    try:
        ...
        if provider_config is not None:
            ...
            raise love_engine.exceptions.BadRequestError(
                message="love_engine doesn't support {} for 'file_list'. Only 'openai', 'azure', 'manus', and 'anthropic' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="file_list", url="https://github.com/BerriAI/love_engine"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            response = bedrock_files_instance.file_content(
                ...
                file_content_request=_file_content_request,
                api_base=optional_params.api_base,
                optional_params=love_engine_params_dict,
                timeout=timeout,
                max_retries=optional_params.max_retries,
            )
        else:
            raise love_engine.exceptions.BadRequestError(
                message="love_engine doesn't support {} for 'file_content'. Supported providers are 'openai', 'azure', 'vertex_ai', 'bedrock', 'manus', 'anthropic'.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
...
def file_content_streaming(
    *,
    ...
    chunk_size: int,
    optional_params: GenericLoveEngineParams,
    timeout: Union[float, httpx.Timeout],
    logging_obj: Optional[LoveEngineLoggingObj],
    _is_async: bool,
    client: Optional[Any],
    ...
    if logging_obj is not None:
        ...
        love_engine_params = logging_obj.model_call_details.get("love_engine_params", {}) or {}
        if optional_params.api_base is not None:
            love_engine_params["api_base"] = optional_params.api_base
        logging_obj.model_call_details["love_engine_params"] = love_engine_params
    ...
    if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
        ...
        raise love_engine.exceptions.BadRequestError(
            message="love_engine doesn't support {} for streaming 'file_content'. Supported providers are {}.".format(
                custom_llm_provider,
