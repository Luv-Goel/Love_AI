...
import anyio
from love_engine.files.types import FileContentProvider

if TYPE_CHECKING:
    from love_engine.love_engine_core_utils.love_engine_logging import (
        Logging as LoveEngineLoggingObj,
    )
    from love_engine.types.utils import StandardLoggingHiddenParams, StandardLoggingPayload


class FileContentStreamingResponse:
    """
    Iterator wrapper for file content streaming that carries love_engine metadata
    ...
    def _sync_hidden_params(self) -> None:
        ...
        if self.logging_obj is not None:
            love_engine_params = (
                self.logging_obj.model_call_details.get("love_engine_params", {}) or {}
            )

        if "api_base" not in self._hidden_params and love_engine_params.get("api_base"):
            self._hidden_params["api_base"] = love_engine_params["api_base"]
        ...
        self._hidden_params["love_engine_model_name"] = self.model
