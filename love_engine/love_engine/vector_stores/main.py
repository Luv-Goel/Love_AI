"""
LoveEngine SDK Functions for Creating and Searching Vector Stores
"""
...
from love_engine.vector_stores.utils import VectorStoreRequestUtils
    ...
    **kwargs,
    ...
    try:
        ...
        if love_engine_params.mock_response and isinstance(
            love_engine_params.mock_response, dict
        ):
            return mock_vector_store_create_response(
                mock_response=VectorStoreCreateResponse(**love_engine_params.mock_response)
            )
...
def search(
    ...
    **kwargs,
    ...
    try:
        ...
        if love_engine.vector_store_registry is not None and vector_store_id is not None:
            try:
                registry_credentials = (
                    love_engine.vector_store_registry.get_credentials_for_vector_store(
                        vector_store_id
                    )
                )
        ...
        # get llm provider logic
        love_engine_params = GenericLoveEngineParams(vector_store_id=vector_store_id, **kwargs)

        ## MOCK RESPONSE LOGIC
        if love_engine_params.mock_response and isinstance(
            love_engine_params.mock_response, (str, builtins.list)
        ):
            mock_results = None
            if isinstance(love_engine_params.mock_response, builtins.list):
                mock_results = love_engine_params.mock_response  # type: ignore[assignment]
