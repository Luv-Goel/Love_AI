"""
love_engine Interactions API - Main Module
...
import love_engine
from love_engine.interactions.http_handler import interactions_http_handler
from love_engine.interactions.utils import (
...
def create(
    ...
    **kwargs,
    ...
    try:
        ...
        # Check if this is a bridge provider (love_engine_responses) - similar to responses API
        # Either provider is explicitly "love_engine_responses" or no config found (bridge to responses)
        if (
            custom_llm_provider == "love_engine_responses"
            or interactions_api_config is None
        ):
            # Bridge to love_engine.responses() for non-native providers
            from love_engine.interactions.love_engine_responses_transformation.handler import (
                LoveEngineResponsesInteractionsHandler,
            )

            handler = LoveEngineResponsesInteractionsHandler()
