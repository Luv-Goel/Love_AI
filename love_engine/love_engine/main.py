# love_engine main module: public completion, embedding, streaming, and moderation entrypoints.
#
# +-----------------------------------------------+
# |                                               |
...
from love_engine._logging import _redact_string
...
# client must be imported from love_engine as it's a decorator used at function definition time
from love_engine import client
...
# Logging is imported lazily when needed to avoid loading love_engine_logging at import time
if TYPE_CHECKING:
    from love_engine.love_engine_core_utils.love_engine_logging import Logging
    from love_engine.types.utils import TokenCountResponse

from love_engine.constants import (
    DEFAULT_MOCK_RESPONSE_COMPLETION_TOKEN_COUNT,
    DEFAULT_MOCK_RESPONSE_PROMPT_TOKEN_COUNT,
)
from love_engine.exceptions import LoveEngineUnknownProvider
...
from love_engine.love_engine_core_utils.completion_timeout import CompletionTimeout
from love_engine.love_engine_core_utils.get_love_engine_params import OPTIONAL_KWARGS_KEYS
from love_engine.love_engine_core_utils.dd_tracing import tracer
from love_engine.love_engine_core_utils.get_provider_specific_headers import (
    ProviderSpecificHeaderUtils,
)
from love_engine.love_engine_core_utils.health_check_utils import (
...
from love_engine.love_engine_core_utils.mock_functions import (
    mock_embedding,
...
from love_engine.llms.base_llm import BaseConfig, BaseImageGenerationConfig
...
from love_engine.llms.openai.chat.gpt_5_transformation import OpenAIGPT5Config
from love_engine.llms.openai_like.json_loader import JSONProviderRegistry
from love_engine.llms.vertex_ai.common_utils import (
    VertexAIModelRoute,
    get_vertex_ai_model_route,
)
from love_engine.realtime_api.main import _realtime_health_check
from love_engine.secret_managers.main import get_secret_bool, get_secret_str
...
from .love_engine_core_utils.core_helpers import safe_deep_copy
from .love_engine_core_utils.fallback_utils import (
    async_completion_with_fallbacks,
    completion_with_fallbacks,
)
from .love_engine_core_utils.prompt_templates.common_utils import (
    add_system_prompt_to_messages,
    get_completion_messages,
    update_messages_with_model_file_ids,
)
from .love_engine_core_utils.prompt_templates.factory import (
    custom_prompt,
...
from .love_engine_core_utils.streaming_chunk_builder_utils import ChunkProcessor
...
class LoveEngine:
    def __init__(
        ...
        *,
        ...
        max_retries: Optional[int] = love_engine.num_retries,
...
async def acompletion(
    ...
    # Optional love_engine function params
    thinking: Optional[AnthropicThinkingParam] = None,
    web_search_options: Optional[OpenAIWebSearchOptions] = None,
    include_server_side_tool_invocations: Optional[bool] = None,
    # Session management
    shared_session: Optional["ClientSession"] = None,
    # Per-request JSON schema validation (overrides love_engine.enable_json_schema_validation)
    enable_json_schema_validation: Optional[bool] = None,
    **kwargs,
) -> Union[ModelResponse, CustomStreamWrapper]:
    """
    Asynchronously executes a love_engine.completion() call for any of love_engine supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)

    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://docs.love_engine.ai/docs/providers/
    ...
    #########################################################
    #########################################################
    love_engine_logging_obj = kwargs.get("love_engine_logging_obj", None)
    if isinstance(love_engine_logging_obj, LoveEngineLoggingObj) and (
        love_engine_logging_obj.should_run_prompt_management_hooks(
        ...
        (
            ...
            messages,
            _,
        ) = await love_engine_logging_obj.async_get_chat_completion_prompt(
            model=model,
            messages=messages,
            non_default_params=kwargs,
            prompt_id=kwargs.get("prompt_id", None),
        ...
        # relevant issue: https://github.com/BerriAI/love_engine/issues/11404
        #########################################################
    ...
    fallbacks = fallbacks or love_engine.model_fallbacks
    if fallbacks is not None:
        ...
        if response is None:
            raise Exception(
                "No response from fallbacks. Got none. Turn on `love_engine.set_verbose=True` to see more details."
            )
        return response
    ...
    try:
        ...
        if (
            ...
            response = love_engine.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
                response_object=response,
                model_response_object=love_engine.ModelResponse(),
            )
...
def _handle_mock_potential_exceptions(
    ...
    if isinstance(mock_response, Exception):
        if isinstance(mock_response, openai.APIError):
            raise mock_response
        raise love_engine.MockException(
    ...
    elif isinstance(mock_response, str) and mock_response == "love_engine.RateLimitError":
        ...
        and mock_response == "love_engine.ContextWindowExceededError"
    ):
        ...
        isinstance(mock_response, str)
        and mock_response == "love_engine.InternalServerError"
    ):
...
def mock_completion(
    ...
    **kwargs,
    ...
    Returns:
        love_engine.ModelResponse: A ModelResponse simulating a completion response with the specified model, messages, and mock response.
    ...
    try:
        ...
        if isinstance(mock_response, str) and mock_response.startswith(
            ...
            mock_response = love_engine.MockException(
        ...
        if n is None:
            ...
            _all_choices = []
            for i in range(n):
                _choice = love_engine.utils.Choices(
                    index=i,
                    message=love_engine.utils.Message(
        ...
        try:
            _, custom_llm_provider, _, _ = love_engine.utils.get_llm_provider(model=model)
...
def responses_api_bridge_check(
    ...
    if love_engine.route_all_chat_openai_to_responses and custom_llm_provider == "openai":
...
def _build_custom_pricing_entry(
    ...
    Collects all CustomPricingLoveEngineParams fields present in kwargs and
    ...
    """
    entry: dict = {"love_engine_provider": custom_llm_provider}

    for field_name in CustomPricingLoveEngineParams.model_fields:
...
def completion(  # type: ignore
    ...
    **kwargs,
    ...
    """
    Perform a completion() using any of love_engine supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)
    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://docs.love_engine.ai/docs/providers/
        messages (List): A list of message objects representing the conversation context (default is an empty list).
    ...
    if not skip_mcp_handler and tools:
        from love_engine.responses.mcp.chat_completions_handler import acompletion_with_mcp
        from love_engine.responses.mcp.love_engine_proxy_mcp_handler import (
            LOVE_ENGINE_Proxy_MCP_Handler,
        )
        from love_engine.types.llms.openai import ToolParam
        ...
        if LOVE_ENGINE_Proxy_MCP_Handler._should_use_love_engine_mcp_gateway(
            tools=tools_for_mcp
        ):
    ...
    if love_engine.proxy_auth is not None:
        try:
            proxy_headers = love_engine.proxy_auth.get_auth_headers()
    ...
    love_engine_system_prompt = kwargs.get("love_engine_system_prompt", None)
    ### COPY MESSAGES ### - related issue https://github.com/BerriAI/love_engine/discussions/4489
    ...
    love_engine_params = {}  # used to prevent unbound var errors
    ## PROMPT MANAGEMENT HOOKS ##
    ...
    if isinstance(love_engine_logging_obj, LoveEngineLoggingObj) and (
        ...
        (
            ...
            messages,
            optional_params,
        ) = love_engine_logging_obj.get_chat_completion_prompt(
            model=model,
            messages=messages,
    ...
    ### love_engine SYSTEM PROMPT ###
    if love_engine_system_prompt:
        messages = add_system_prompt_to_messages(
            messages=messages,
            system_prompt=love_engine_system_prompt,
    ...
    try:
        ...
        logging: LoveEngineLoggingObj = cast(LoveEngineLoggingObj, love_engine_logging_obj)
        fallbacks = fallbacks or love_engine.model_fallbacks
        if fallbacks is not None:
            return completion_with_fallbacks(**args)
        if model_list is not None:
            deployments = [
                m["love_engine_params"] for m in model_list if m["model_name"] == model
            ]
            return love_engine.batch_completion_models(deployments=deployments, **args)
        ...
        setattr(model_response, "usage", love_engine.Usage())
        if (
        ...
        model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
            ...
            api_key=api_key,
            love_engine_params=(
                GenericLoveEngineParams(**_supplemental_provider_params)
        ...
        timeout = CompletionTimeout.resolve(
            ...
            kwargs,
            custom_llm_provider,
            global_timeout=getattr(love_engine, "request_timeout", None),
        ...
        if love_engine.add_function_to_prompt and optional_params.get(
        ...
        # For logging - save the values of the love_engine-specific params passed in
        love_engine_params = get_love_engine_params(
            ...
            custom_llm_provider=custom_llm_provider,
            api_base=api_base,
            love_engine_call_id=kwargs.get("love_engine_call_id", None),
            model_alias_map=love_engine.model_alias_map,
            ...
            love_engine_session_id=kwargs.get("love_engine_session_id"),
            ...
            use_love_engine_proxy=kwargs.get("use_love_engine_proxy", False),
            api_version=api_version,
            ...
            max_retries=max_retries,
            timeout=timeout,
            love_engine_request_debug=kwargs.get("love_engine_request_debug", False),
            tpm=kwargs.get("tpm"),
            rpm=kwargs.get("rpm"),
            use_xai_oauth=kwargs.get("use_xai_oauth", False),
            aws_bedrock_project_id=kwargs.get("aws_bedrock_project_id"),
        )
        cast(LoveEngineLoggingObj, logging).update_environment_variables(
            model=model,
            user=user,
        ...
        ## RESPONSES API BRIDGE LOGIC ## - check if model has 'mode: responses' in love_engine.model_cost map
        ...
        if responses_api_model_info.get("mode") == "responses":
            from love_engine.completion_extras import responses_api_bridge
            ...
            custom_llm_provider == "azure"
            and love_engine.AzureOpenAIGPT5Config.is_model_gpt_5_model(
        ...
        if custom_llm_provider == "azure":
            ...
            api_base = api_base or love_engine.api_base or get_secret("AZURE_API_BASE")
            ...
            azure_ad_token_provider = love_engine_params.get(
                "azure_ad_token_provider", None
            )

            headers = headers or love_engine.headers
            ...
            if love_engine.AzureOpenAIO1Config().is_o_series_model(
                ...
                ## LOAD CONFIG - if set
                config = love_engine.AzureOpenAIO1Config.get_config()
                ...
                ## LOAD CONFIG - if set
                config = love_engine.AzureOpenAIConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in optional_params
                    ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_base = api_base or love_engine.api_base or get_secret_str("AZURE_API_BASE")
            ...
            if azure_ai_route == "agents":
                from love_engine.llms.azure_ai.agents import AzureAIAgentsConfig
            ...
            # Check if this is a Claude model - route to Azure Anthropic handler
                ...
                response = azure_anthropic_chat_completions.completion(
                    ...
                    api_base=api_base,
                    acompletion=acompletion,
                    custom_prompt_dict=love_engine.custom_prompt_dict,
                    model_response=model_response,
                    print_verbose=print_verbose,
                    optional_params=optional_params,
                    love_engine_params=love_engine_params,
            ...
            in love_engine.openai_text_completion_compatible_providers
            ...
            ## LOAD CONFIG - if set
            config = love_engine.OpenAITextCompletionConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_text_config(top_k=3) <- allows for dynamic variables to be passed in
                    optional_params[k] = v
            if love_engine.organization:
                openai.organization = love_engine.organization
            ...
            if (
                ...
                _response = love_engine.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
            ...
            api_key = (
                api_key
                or love_engine.api_key  # for deepinfra/perplexity/anyscale/friendliai we check in get_llm_provider and pass in the api key from there
                or love_engine.groq_key
            ...
            ## LOAD CONFIG - if set
            config = love_engine.GroqChatConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
            ...
            api_base = (
                api_base or love_engine.api_base or get_secret("BEDROCK_MANTLE_API_BASE")
            )
            api_key = api_key or love_engine.api_key or get_secret("BEDROCK_MANTLE_API_KEY")
            headers = headers or love_engine.headers
            config = love_engine.BedrockMantleChatConfig.get_config()
            for k, v in config.items():
            ...
            (
                ...
                api_key,
                headers,
            ) = love_engine.A2AConfig.resolve_agent_config_from_registry(
                model=model,
                api_base=api_base,
                api_key=api_key,
                headers=headers,
            ...
            api_base = api_base or love_engine.api_base or get_secret_str("A2A_API_BASE")
            ...
            api_key = (
                api_key
                or love_engine.api_key
                or love_engine.gigachat_key
                or get_secret("GIGACHAT_API_KEY")
                or get_secret("GIGACHAT_CREDENTIALS")
            )

            headers = headers or love_engine.headers or {}
            ...
            headers = headers or love_engine.headers
            ## LOAD CONFIG - if set
            config = love_engine.GenAIHubOrchestrationConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_key = api_key or get_secret_str("MINIMAX_API_KEY") or love_engine.api_key
            ...
            api_base = (
                api_base or love_engine.api_base or get_secret_str("HOSTED_VLLM_API_BASE")
            )
            ...
            if custom_llm_provider == "github_copilot":
                from love_engine.llms.github_copilot.authenticator import Authenticator
                from love_engine.llms.github_copilot.common_utils import (
            ...
            if (
                love_engine.enable_preview_features and metadata is not None
            ...
            ## LOAD CONFIG - if set
            config = love_engine.OpenAIConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_key = api_key or love_engine.api_key or get_secret("MISTRAL_API_KEY")
            api_base = (
                api_base
                or love_engine.api_base
            ...
            or model in love_engine.replicate_models
        ):
            # Setting the relevant API KEY for replicate, replicate defaults to using os.environ.get("REPLICATE_API_TOKEN")
            replicate_key = (
                api_key
                or love_engine.replicate_key
                or love_engine.api_key
            ...
            custom_prompt_dict = custom_prompt_dict or love_engine.custom_prompt_dict
            ...
            disable_url_suffix = get_secret_bool("LOVE_ENGINE_ANTHROPIC_DISABLE_URL_SUFFIX")
            if (
                ...
                verbose_logger.debug(
                    "LOVE_ENGINE_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/complete suffix"
                )
            ...
            if (
                ...
                verbose_logger.debug(
                    "LOVE_ENGINE_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/messages suffix"
                )
            ...
            aleph_alpha_key = (
                api_key
                or love_engine.aleph_alpha_key
            ...
            model_response = aleph_alpha.completion(
                ...
                logger_fn=logger_fn,
                encoding=_get_encoding(),
                default_max_tokens_to_sample=love_engine.max_tokens,
            ...
            huggingface_key = (
                api_key
                or love_engine.huggingface_key
                or os.environ.get("HF_TOKEN")
                or os.environ.get("HUGGINGFACE_API_KEY")
                or love_engine.api_key
            )
            hf_headers = headers or love_engine.headers
            response = base_llm_http_handler.completion(
                model=model,
                messages=messages,
                headers=hf_headers,
            ...
            api_key = (
                api_key or get_secret_str("COMPACTIFAI_API_KEY") or love_engine.api_key
            )
            ...
            api_key = (
                api_key
                or love_engine.api_key  # for databricks we check in get_llm_provider and pass in the api key from there
                or love_engine.databricks_key
            ...
            api_key = (
                api_key
                or love_engine.api_key
                or love_engine.openrouter_key
                or get_secret_str("OPENROUTER_API_KEY")
                or get_secret_str("OR_API_KEY")
            )

            openrouter_site_url = get_secret("OR_SITE_URL") or "https://love_engine.ai"
            openrouter_app_name = get_secret("OR_APP_NAME") or "love_engine"
            ...
            _headers = headers or love_engine.headers
            if _headers:
            ...
            ## Load Config
            config = love_engine.OpenrouterConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            api_key = (
                api_key or love_engine.api_key or get_secret("VERCEL_AI_GATEWAY_API_KEY")
            )

            vercel_site_url = get_secret("VERCEL_SITE_URL") or "https://love_engine.ai"
            vercel_app_name = get_secret("VERCEL_APP_NAME") or "love_engine"
            ...
            ## Load Config
            config = love_engine.VercelAIGatewayConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            or (model in love_engine.together_ai_models)
        ):
            """
            ...
            api_base = api_base or love_engine.api_base or get_secret("GEMINI_API_BASE")
            ...
            response = vertex_chat_completion.completion(  # type: ignore
                ...
                love_engine_params=love_engine_params,  # type: ignore
                logger_fn=logger_fn,
                encoding=_get_encoding(),
            ...
            api_base = api_base or love_engine.api_base or get_secret("VERTEXAI_API_BASE")

            new_params = safe_deep_copy(optional_params or {})
            model_route = get_vertex_ai_model_route(
                model=model, love_engine_params=love_engine_params
            )
            ...
            if model_route == VertexAIModelRoute.PARTNER_MODELS:
                ...
                from love_engine.llms.vertex_ai.agent_engine.transformation import (
                    VertexAgentEngineConfig,
                )

                vertex_agent_engine_config = VertexAgentEngineConfig()

                # Update love_engine_params with vertex credentials
                love_engine_params["vertex_project"] = vertex_ai_project
                love_engine_params["vertex_location"] = vertex_ai_location
                love_engine_params["vertex_credentials"] = vertex_credentials
            ...
            tenant_id = (
                ...
                or love_engine.predibase_tenant_id
            ...
            api_key = api_key or love_engine.api_key or get_secret("CODESTRAL_API_KEY")

            text_completion_model_response = love_engine.TextCompletionResponse(
                stream=stream
            )
            ...
            if "aws_bedrock_client" in optional_params:
                verbose_logger.warning(
                    "'aws_bedrock_client' is a deprecated param. Please move to another auth method - https://docs.love_engine.ai/docs/providers/bedrock#boto3---authentication."
                )
            ...
            api_base = love_engine.api_base or api_base
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
                messages=messages,
            ...
            api_key = (
                api_key
                or love_engine.cloudflare_api_key
                or love_engine.api_key
        ...
        elif custom_llm_provider == "petals" or model in love_engine.petals_models:
            api_base = api_base or love_engine.api_base
            ...
            if stream is True:  ## [BETA]
                ...
                return response
            response = model_response
        elif custom_llm_provider == "snowflake" or model in love_engine.snowflake_models:
            try:
                client = (
        ...
        elif custom_llm_provider == "ovhcloud" or model in love_engine.ovhcloud_models:
            api_key = (
                api_key
                or love_engine.ovhcloud_key
            ...
            url = love_engine.api_base or api_base or ""
            if url is None or url == "":
                raise ValueError(
                    "api_base not set. Set api_base or love_engine.api_base for custom endpoints"
                )

            """
            assume input to custom LLM api bases follow this format:
            resp = love_engine.module_level_client.post(
                api_base,
                json={
            ...
            custom_llm_provider in love_engine._custom_providers
        ):  # Assume custom LLM provider
            # Get the Custom Handler
            custom_handler: Optional[CustomLLM] = None
            for item in love_engine.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]

            if custom_handler is None:
                raise LoveEngineUnknownProvider(
                    model=model, custom_llm_provider=custom_llm_provider
                )
            ...
            (
                ...
                api_base=api_base or love_engine.api_base,
                api_key=api_key or love_engine.api_key,
            )
...
def completion_with_retries(*args, **kwargs):
    """
    Executes a love_engine.completion() with 3 retries
    """
    try:
        import tenacity
    except Exception as e:
...
def responses_with_retries(*args, **kwargs):
    """
    Executes a love_engine.responses() with retries
    """
    try:
        import tenacity
    except Exception as e:
    ...
    from love_engine.responses.main import responses
...
async def aresponses_with_retries(*args, **kwargs):
    """
    Executes a love_engine.aresponses() with retries
    """
    try:
        import tenacity
    except Exception as e:
    ...
    from love_engine.responses.main import aresponses
...
def embedding(
    ...
    **kwargs,
    ...
    - love_engine_call_id: The call ID for love_engine logging.
    - love_engine_logging_obj: The love_engine logging object.
    ...
    default_params = openai_params + love_engine_params
    non_default_params = {
    ...
    logging: LoveEngineLoggingObj = love_engine_logging_obj  # type: ignore
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            or custom_llm_provider == "together_ai"
            or custom_llm_provider == "nvidia_nim"
            or custom_llm_provider == "love_engine_proxy"
            or (
                model in love_engine.open_ai_embedding_models
            ...
            if encoding_format is not None:
                ...
                env_fmt = get_secret_str("LOVE_ENGINE_DEFAULT_EMBEDDING_ENCODING_FORMAT")
            ...
            api_base = api_base or love_engine.api_base or get_secret("DATABRICKS_API_BASE")  # type: ignore
            ...
            # set API KEY
            if api_key is None:
                api_key = love_engine.api_key or get_secret_str("HOSTED_VLLM_API_KEY")
            ...
            api_base = (
                api_base or love_engine.api_base or get_secret_str("OPENAI_LIKE_API_BASE")
            )
            ...
            if api_key is None:
                api_key = (
                    api_key
                    or love_engine.api_key
                    or love_engine.openai_like_key
            ...
            gemini_api_key = api_key or get_api_key_from_env() or love_engine.api_key

            api_base = api_base or love_engine.api_base or get_secret_str("GEMINI_API_BASE")
            ...
            api_key = api_key or love_engine.api_key or get_secret_str("MISTRAL_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = (
                api_key or love_engine.api_key or get_secret_str("FIREWORKS_AI_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
            ...
            api_key = api_key or love_engine.api_key or get_secret_str("NEBIUS_API_KEY")
            api_base = (
                api_base
                or love_engine.api_base
            ...
            api_key = api_key or love_engine.api_key or get_secret_str("WANDB_API_KEY")
            api_base = (
                api_base
                or love_engine.api_base
            ...
            api_key = api_key or love_engine.api_key or get_secret_str("SAMBANOVA_API_KEY")
            api_base = (
                api_base
                or love_engine.api_base
            ...
            dashscope_key = (
                api_key or love_engine.api_key or get_secret_str("DASHSCOPE_API_KEY")
            )
            ...
            api_key = api_key or love_engine.api_key or get_secret_str("OVHCLOUD_API_KEY")
            api_base = (
                api_base
                or love_engine.api_base
                or get_secret_str("OVHCLOUD_API_BASE")
            ...
            response = base_llm_http_handler.embedding(
                ...
                client=client,
                aembedding=aembedding,
                love_engine_params={"ssl_verify": kwargs.get("ssl_verify", None)},
            )
        ...
        ## LOGGING
        love_engine_logging_obj.post_call(
            input=input,
            api_key=api_key,
            original_response=str(e),
        )
    ...
    *args, **kwargs
    ...
    try:
        ...
        if (
            ...
            text_completion_response = love_engine.utils.LoveEngineResponseObjectHandler.convert_chat_to_text_completion(
...
def text_completion(
    ...
    **kwargs,
    ...
    # default values for all optional params are none, love_engine only passes them to the llm when they are set to non None values
    ...
    if isinstance(prompt, list) and len(prompt) > 0 and isinstance(prompt[0], str):
        ...
        raise Exception(
            f"Unmapped prompt format. Your prompt is neither a list of strings nor a string. prompt={prompt}. File an issue - https://github.com/BerriAI/love_engine/issues"
        )
    ...
    if _model is not None and (
        ...
        if _model not in love_engine.open_ai_chat_completion_models:
    ...
    text_completion_response = (
        love_engine.utils.LoveEngineResponseObjectHandler.convert_chat_to_text_completion(
            response=response,
            text_completion_response=text_completion_response,
        )
    )
...
async def aadapter_completion(
    *, adapter_id: str, **kwargs
    ...
    try:
        ...
        for item in love_engine.adapters:
            if item["id"] == adapter_id:
                translation_obj = item["adapter"]

        if translation_obj is None:
            raise ValueError(
                "No matching adapter given. Received 'adapter_id'={}, love_engine.adapters={}".format(
                    adapter_id, love_engine.adapters
                )
            )
...
async def aadapter_generate_content(
    **kwargs,
    ...
    from love_engine.google_genai.adapters.handler import GenerateContentToCompletionHandler
...
def moderation(
    ...
    return love_engine.utils.LoveEngineResponseObjectHandler.convert_to_moderation_response(
...
async def amoderation(
    ...
    **kwargs,
    ...
    optional_params = GenericLoveEngineParams(**kwargs)
    love_engine_logging_obj: Optional[LoveEngineLoggingObj] = kwargs.get(
    ...
    except love_engine.BadRequestError:
    ...
    # update love_engine_logging_obj with environment variables
    custom_llm_provider = custom_llm_provider or love_engine.LlmProviders.OPENAI.value
    if love_engine_logging_obj is not None:
        love_engine_logging_obj.update_environment_variables(
            model=model,
...
def transcription(
    ...
    **kwargs,
    ...
    if love_engine_logging_obj:
        love_engine_logging_obj.model_call_details["client"] = str(client)

    if max_retries is None:
        max_retries = openai.DEFAULT_MAX_RETRIES

    model_response = love_engine.utils.TranscriptionResponse()
    ...
    if custom_llm_provider == "azure" and provider_config is None:
        ...
        api_version = (
            api_version or love_engine.api_version or get_secret_str("AZURE_API_VERSION")
        )
        ...
        custom_llm_provider in love_engine.openai_compatible_providers
    ):
        api_base = (
            api_base
            or love_engine.api_base
        ...
        api_key = api_key or love_engine.api_key or love_engine.openai_key or get_secret("OPENAI_API_KEY")  # type: ignore
        ...
        from love_engine.llms.soniox.audio_transcription.handler import (
...
def speech(
    ...
    **kwargs,
    ...
    if timeout is None:
        timeout = love_engine.request_timeout

    if max_retries is None:
        max_retries = love_engine.num_retries or openai.DEFAULT_MAX_RETRIES
    love_engine_params_dict = get_love_engine_params(**kwargs)
    ...
    logging_obj: LoveEngineLoggingObj = cast(
        LoveEngineLoggingObj, kwargs.get("love_engine_logging_obj")
    )
    ...
    if (
        ...
        api_key = (
            api_key
            or love_engine.api_key  # for deepinfra/perplexity/anyscale we check in get_llm_provider and pass in the api key from there
            or love_engine.openai_key
            or get_secret("OPENAI_API_KEY")
        )  # type: ignore
        ...
        if model.startswith("speech/"):
            ...
            response = azure_config.dispatch_text_to_speech(  # type: ignore
                ...
                voice=voice,
                optional_params=optional_params,
                love_engine_params_dict=love_engine_params_dict,
            ...
            api_base = api_base or love_engine.api_base or get_secret("AZURE_API_BASE")  # type: ignore

            api_version = api_version or love_engine.api_version or get_secret("AZURE_API_VERSION")  # type: ignore
        ...
        if isinstance(query_params, dict):
            love_engine_params_dict[
                ElevenLabsTextToSpeechConfig.ELEVENLABS_QUERY_PARAMS_KEY
            ] = query_params

        love_engine_params_dict[ElevenLabsTextToSpeechConfig.ELEVENLABS_VOICE_ID_KEY] = (
            voice_id
        )

        if api_base is not None:
            love_engine_params_dict["api_base"] = api_base
        if api_key is not None:
            love_engine_params_dict["api_key"] = api_key
        ...
        generic_optional_params = GenericLoveEngineParams(**kwargs)
        ...
        # Store Vertex AI specific params in love_engine_params_dict
        love_engine_params_dict.update(
            {
    ...
    if response is None:
        raise Exception(
            "Unable to map the custom llm provider={} to a known provider={}.".format(
                custom_llm_provider, love_engine.provider_list
            )
        )
    return response
...
async def ahealth_check(
    ...
    """
    from love_engine.love_engine_core_utils.cached_imports import get_love_engine_logging_class
    from love_engine.love_engine_core_utils.health_check_helpers import HealthCheckHelpers

    # Use cached import helper to lazy-load Logging class (only loads when function is called)
    Logging = get_love_engine_logging_class()
    ...
    love_engine_logging_obj = Logging(
        ...
        love_engine_call_id=str(uuid.uuid4()),
    ...
    try:
        ...
        if model in love_engine.model_cost and mode is None:
            mode = love_engine.model_cost[model].get("mode")
        ...
        if mode in mode_handlers:
            ...
            raise Exception(
                f"Mode {mode} not supported. See modes here: https://docs.love_engine.ai/docs/proxy/health"
            )
    except Exception as e:
        ...
        if mode is None:
            return {
                "error": f"error:{str(e)}. Missing `mode`. Set the `mode` for the model - https://docs.love_engine.ai/docs/proxy/health#embedding-models  \nstacktrace: {stack_trace}",
                "exception": e,
            }
...
####### HELPER FUNCTIONS ################
## Set verbose to true -> ```love_engine.set_verbose = True```
def print_verbose(print_statement):
    try:
        verbose_logger.debug(print_statement)
        if love_engine.set_verbose:
...
def config_completion(**kwargs):
    if love_engine.config_path is not None:
        config_args = read_config_args(love_engine.config_path)
        # overwrite any args passed in with config args
        return completion(**kwargs, **config_args)
    else:
        raise ValueError(
            "No config path set, please set a config path using `love_engine.config_path = 'path/to/config.json'`"
        )
...
def stream_chunk_builder(
    ...
    try:
        ...
        if first_chunk_with_choices is not None and isinstance(
            first_chunk_with_choices["choices"][0], love_engine.utils.TextChoices
        ):  # route to the text completion logic
        ...
        if is_simple_text_stream:
            ...
            if love_engine.include_cost_in_streaming_usage and logging_obj is not None:
                setattr(
                    usage,
                    "cost",
        ...
        # See: https://github.com/BerriAI/love_engine/issues/19478
        image_chunks = [
            chunk
            for chunk in chunks
            if len(chunk["choices"]) > 0
        ...
        # See: https://github.com/BerriAI/love_engine/issues/17737
        ...
        verbose_logger.exception(
            "love_engine.main.py::stream_chunk_builder() - Exception occurred - {}".format(
                str(e)
            )
        )
        raise love_engine.APIError(
...
async def acount_tokens(
    ...
    from love_engine.types.utils import LlmProviders, TokenCountResponse
    ...
    local_count = love_engine.token_counter(
        model=model,
...
def __getattr__(name: str) -> Any:
    ...
    if name == "encoding":
        ...
        from love_engine._lazy_imports import _get_default_encoding
