...
import love_engine
import love_engine._logging
from love_engine import verbose_logger
...
from love_engine.llms.anthropic.cost_calculation import (
    cost_per_token as anthropic_cost_per_token,
)
from love_engine.llms.azure.cost_calculation import (
...
from love_engine.llms.bedrock.cost_calculation import (
    cost_per_token as bedrock_cost_per_token,
)
from love_engine.llms.databricks.cost_calculator import (
    cost_per_token as databricks_cost_per_token,
)
from love_engine.llms.deepseek.cost_calculator import (
    cost_per_token as deepseek_cost_per_token,
)
from love_engine.llms.fireworks_ai.cost_calculator import (
    cost_per_token as fireworks_ai_cost_per_token,
)
from love_engine.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from love_engine.llms.lemonade.cost_calculator import (
    cost_per_token as lemonade_cost_per_token,
)
from love_engine.llms.openai.cost_calculation import (
    _video_output_cost_per_second,
    cost_per_second as openai_cost_per_second,
    cost_per_token as openai_cost_per_token,
)
from love_engine.llms.perplexity.cost_calculator import (
    cost_per_token as perplexity_cost_per_token,
)
from love_engine.llms.together_ai.cost_calculator import get_model_params_and_category
from love_engine.llms.vertex_ai.cost_calculator import (
    cost_per_character as google_cost_per_character,
)
from love_engine.llms.vertex_ai.cost_calculator import (
    cost_per_token as google_cost_per_token,
)
from love_engine.llms.vertex_ai.cost_calculator import cost_router as google_cost_router
from love_engine.llms.xai.cost_calculator import cost_per_token as xai_cost_per_token
...
from love_engine.types.rerank import RerankBilledUnits, RerankResponse
...
if TYPE_CHECKING:
    from love_engine.love_engine_core_utils.love_engine_logging import (
        Logging as LoveEngineLoggingObject,
    )
else:
    LoveEngineLoggingObject = Any
...
def cost_per_token(
    ...
    model_cost_ref = love_engine.model_cost
    ...
    if caller_supplied_provider:
        ...
        _, custom_llm_provider, _, _ = love_engine.get_llm_provider(model=model)
    ...
    if len(model_parts) > 1:
        ...
        model_without_prefix = model
    """
    Code block that formats model to lookup in love_engine.model_cost
    ...
    if call_type == "speech" or call_type == "aspeech":
        speech_model_info = love_engine.get_model_info(
        ...
        from love_engine.search import search_provider_cost_per_query
        ...
        from love_engine.llms.dashscope.cost_calculator import (
...
def get_replicate_completion_pricing(completion_response: dict, total_time=0.0):
    ...
    # for all love_engine currently supported LLMs, almost all requests go to a100_80gb
...
def _get_provider_for_cost_calc(
    ...
    try:
        ...
        verbose_logger.debug(
            f"love_engine.cost_calculator.py::_get_provider_for_cost_calc() - Error inferring custom_llm_provider - {str(e)}"
        )
        return None
...
def _select_model_name_for_cost_calc(
    ...
    if custom_pricing is True:
        if router_model_id is not None and router_model_id in love_engine.model_cost:
            entry = love_engine.model_cost[router_model_id]
            if (
...
def _map_traffic_type_to_service_tier(traffic_type: Optional[str]) -> Optional[str]:
    """
    Map a Gemini usageMetadata.trafficType value to a love_engine service_tier string.
...
def _is_known_usage_objects(usage_obj):
    ...
    return (
        isinstance(usage_obj, love_engine.Usage)
...
def _infer_call_type(
    ...
    elif isinstance(completion_response, LoveEngineSendMessageResponse):
        return "send_message"
...
def _apply_cost_discount(
    ...
    if custom_llm_provider and custom_llm_provider in love_engine.cost_discount_config:
        discount_percent = love_engine.cost_discount_config[custom_llm_provider]
...
def _apply_cost_margin(
    ...
    margin_config = None
    if custom_llm_provider and custom_llm_provider in love_engine.cost_margin_config:
        margin_config = love_engine.cost_margin_config[custom_llm_provider]
        if verbose_logger.isEnabledFor(logging.DEBUG):
            verbose_logger.debug(
                f"Found provider-specific margin config for {custom_llm_provider}: {margin_config}"
            )
    elif "global" in love_engine.cost_margin_config:
        margin_config = love_engine.cost_margin_config["global"]
        if verbose_logger.isEnabledFor(logging.DEBUG):
        ...
        if verbose_logger.isEnabledFor(logging.DEBUG):
            verbose_logger.debug(
                ...
                f"Available configs: {list(love_engine.cost_margin_config.keys())}"
            )
...
def _store_cost_breakdown_in_logging_obj(
    love_engine_logging_obj: Optional[LoveEngineLoggingObject],
    ...
    Args:
        love_engine_logging_obj: The logging object to store breakdown in
    ...
    try:
        ...
        love_engine_logging_obj.set_cost_breakdown(
...
def completion_cost(
    ...
    router_model_id: Optional[str] = None,
    love_engine_logging_obj: Optional[LoveEngineLoggingObject] = None,
    ### SERVICE TIER ###
    service_tier: Optional[str] = None,  # for OpenAI service tier pricing
    ### DATA RESIDENCY ###
    data_residency: Optional[
    ...
    """
    Calculate the cost of a given completion call fot GPT-3.5-turbo, llama2, any love_engine supported llm.

    Parameters:
        completion_response (love_engine.ModelResponses): [Required] The response received from a love_engine completion request.
    ...
    Exceptions:
        Raises exception if model not in the love_engine model cost map. Register model, via custom pricing or PR - https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json
        ...
        for idx, model in enumerate(potential_model_names):
            try:
                ...
                if completion_response is not None and (
                    ...
                    if isinstance(usage_obj, BaseModel) and not _is_known_usage_objects(
                        ...
                        setattr(
                            completion_response,
                            "usage",
                            love_engine.Usage(**_usage_for_dump.model_dump()),
                        )
                    if usage_obj is None:
                        _usage = {}
                    elif isinstance(usage_obj, BaseModel):
                ...
                if call_type in _A2A_CALL_TYPES:
                    from love_engine.a2a_protocol.cost_calculator import A2ACostCalculator
                ...
                if custom_llm_provider is None:
                    try:
                        ...
                        verbose_logger.debug(
                            "love_engine.cost_calculator.py::completion_cost() - Error inferring custom_llm_provider - {}".format(
                                str(e)
                            )
                        )
                if CostCalculatorUtils._call_type_has_image_response(
                    ...
                    if custom_pricing and love_engine_logging_obj is not None:
                        _love_engine_params = getattr(
                            love_engine_logging_obj, "love_engine_params", None
                        )
                        if _love_engine_params is not None:
                    ...
                    prompt_characters = love_engine.utils._count_characters(text=prompt)
                    ...
                    completion_response, LoveEngineRealtimeStreamLoggingObject
                ):
                    if (
                    ...
                    from love_engine.proxy._experimental.mcp_server.cost_calculator import (
                        MCPCostCalculator,
                    )
                ...
                # see https://replicate.com/pricing
                    ...
                    model in love_engine.replicate_models or "replicate" in model
                ) and model not in love_engine.model_cost:
                ...
                if (
                    ...
                    if len(messages) > 0:
                        prompt_string = love_engine.utils.get_formatted_prompt(
                            data={"messages": messages}, call_type="completion"
                        )

                        prompt_characters = love_engine.utils._count_characters(
                            text=prompt_string
                        )
                    if completion_response is not None and isinstance(
                        completion_response, ModelResponse
                    ):
                        completion_string = love_engine.utils.get_response_string(
                            response_obj=completion_response
                        )
                        completion_characters = love_engine.utils._count_characters(
                ...
                if love_engine_logging_obj is not None:
                    request_model_for_cost = love_engine_logging_obj.model
                ...
                original_cost = _final_cost
                if love_engine.cost_discount_config:
                    (
                        _final_cost,
                        discount_percent,
                        discount_amount,
                ...
                # Apply margin from module-level config if configured
                if love_engine.cost_margin_config:
                    (
                        _final_cost,
                        margin_percent,
                        margin_fixed_amount,
                ...
                if love_engine_logging_obj is not None:
                    ...
                    if cost_per_token_usage_object is not None:
                        ...
                        if (_cr or _cc) and model:
                            try:
                                _mi = love_engine.get_model_info(
                                    model=model, custom_llm_provider=custom_llm_provider
                                )
                ...
                verbose_logger.debug(
                    "love_engine.cost_calculator.py::completion_cost() - Error calculating cost for model={} - {}".format(
                        model, str(e)
                    )
                )
...
def get_response_cost_from_hidden_params(
    ...
    if (
        additional_headers
        and "llm_provider-x-love_engine-response-cost" in additional_headers
    ):
        response_cost = additional_headers["llm_provider-x-love_engine-response-cost"]
        if response_cost is None:
            return None
        return float(additional_headers["llm_provider-x-love_engine-response-cost"])
    return None
...
def ocr_cost(
    ...
    try:
        model_info: Optional[ModelInfo] = love_engine.get_model_info(
            model=model, custom_llm_provider=custom_llm_provider
        )
    except Exception:
        model_info = None
...
def vector_store_search_cost(
    ...
    if model is not None and "/" in model:
        api_type, custom_llm_provider, _, _ = love_engine.get_llm_provider(
            model=model,
        )
...
def rerank_cost(
    ...
    """
    _, custom_llm_provider, _, _ = love_engine.get_llm_provider(
        model=model, custom_llm_provider=custom_llm_provider
    )
...
def default_image_cost_calculator(
    ...
    for _model in models_to_check:
        if _model is not None and _model in love_engine.model_cost:
            cost_info = love_engine.model_cost[_model]
            break
...
def default_video_cost_calculator(
    ...
    Args:
        ...
        model_info (Optional[ModelInfo]): Deployment-level model info containing
            ...
            the global love_engine.model_cost lookup.
    ...
    if model_info is not None:
        ...
        if cost_info is None and custom_llm_provider:
            ...
            if prefixed_model in love_engine.model_cost:
                cost_info = love_engine.model_cost[prefixed_model]
...
def batch_cost_calculator(
    ...
    Args:
        model_info: Optional deployment-level model info containing custom
            ...
            skips the global love_engine.get_model_info() lookup so that
    ...
    if model_info is None:
        try:
            model_info = love_engine.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
        except Exception:
            model_info = None
        ...
        try:
            global_info = love_engine.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
            if global_info:
...
class RealtimeAPITokenUsageProcessor(BaseTokenUsageProcessor):
    ...
    def create_logging_realtime_object(
        ...
        return LoveEngineRealtimeStreamLoggingObject(
            usage=usage,
            results=results,
        )
...
def handle_realtime_stream_cost_calculation(
    ...
    potential_model_names.append(love_engine_model_name)
...
def handle_realtime_transcription_cost_calculation(
    ...
    model_name = (
        _get_transcription_model_name_from_results(results) or love_engine_model_name
    )
    try:
        model_info = love_engine.get_model_info(
