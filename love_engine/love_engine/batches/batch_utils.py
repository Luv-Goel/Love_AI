...
from love_engine.types.llms.openai import Batch
from love_engine.types.utils import CallTypes, ModelInfo, Usage
...
async def _handle_completed_batch(
    ...
    Args:
        ...
        love_engine_params: Optional love_engine parameters containing credentials (api_key, api_base, etc.)
    """
    # Get batch results
    file_content_dictionary = await _get_batch_output_file_content_as_dictionary(
        batch, custom_llm_provider, love_engine_params=love_engine_params
    )
...
def _batch_cost_calculator(
    ...
    if (
        ...
        and model_name
        and getattr(love_engine, "disable_vertex_batch_output_transformation", False)
    ):
...
def calculate_vertex_ai_batch_cost_and_usage(
    ...
    Used only when ``love_engine.disable_vertex_batch_output_transformation = True``.
    ...
    """
    from love_engine.cost_calculator import batch_cost_calculator
...
async def _get_batch_output_file_content_as_dictionary(
    ...
    """
    from love_engine.files.main import afile_content
    ...
    # Build kwargs for afile_content with credentials from love_engine_params
    file_content_kwargs = {
        "file_id": file_id,
        "custom_llm_provider": custom_llm_provider,
    }
    ...
    credentials = _extract_file_access_credentials(love_engine_params)
...
def _extract_file_access_credentials(love_engine_params: Optional[dict]) -> dict:
    """
    Extract credentials from love_engine_params for file access operations.
    ...
    Args:
        love_engine_params: Dictionary containing love_engine parameters with credentials
    ...
    if love_engine_params:
        ...
        for key in credential_keys:
            if key in love_engine_params:
                credentials[key] = love_engine_params[key]
        ...
        for _item in file_content_dictionary:
            if _batch_response_was_successful(_item):
                ...
                if model_info is not None:
                    ...
                    total_cost += love_engine.completion_cost(
