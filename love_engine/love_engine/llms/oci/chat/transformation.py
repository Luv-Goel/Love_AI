...
implement the love_engine BaseConfig interface.  Heavy-lifting lives in:

  - :mod:`love_engine.llms.oci.chat.cohere`  — Cohere message/tool/response logic
  - :mod:`love_engine.llms.oci.chat.generic` — Generic message/tool/response logic
  - :mod:`love_engine.llms.oci.common_utils` — auth, signing, schema utilities
"""
...
import love_engine
from love_engine.constants import DEFAULT_OCI_CHAT_MAX_TOKENS
...
from love_engine.llms.oci.chat.cohere import (
...
from love_engine.llms.oci.chat.generic import (
...
from love_engine.llms.oci.common_utils import (
    OCI_API_VERSION,
    OCIError,
...
from love_engine.types.llms.oci import (
    CohereChatRequest,
...
from love_engine.utils import supports_reasoning
...
class OCIChatConfig(BaseConfig):
    """love_engine BaseConfig implementation for OCI Generative AI chat."""
    ...
    def __init__(self) -> None:
        ...
        # accepts ``topK`` and love_engine commonly forwards ``top_k`` as a
    ...
    def map_openai_params(
        ...
        for key, value in {**non_default_params, **optional_params}.items():
            ...
            if alias is False:
                # max_retries is a love_engine-level control param (love_engine applies
                # retries itself); it is never a generation param OCI accepts, so
                # drop it silently. The love_engine proxy injects it on every request,
                ...
                if key == "n" and (value is None or value == 1):
                    continue
                if drop_params or love_engine.drop_params:
                    continue
                raise OCIError(
                    status_code=400,
            ...
            # OCI-mapped ``responseFormat`` so downstream love_engine framework code
    ...
    def get_complete_url(
        ...
        base = get_oci_base_url(optional_params, api_base or love_engine.api_base)
    ...
    def _get_optional_params(
        ...
        # sane default when one is absent, mirroring love_engine's Anthropic config.
