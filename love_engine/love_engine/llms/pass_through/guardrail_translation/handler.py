...
It uses the field targeting configuration from love_engine_logging_obj
...
from love_engine.proxy._types import PassThroughGuardrailSettings
...
class PassThroughEndpointHandler(BaseTranslation):
    ...
    Uses passthrough_guardrails_config from love_engine_logging_obj
    ...
    def _get_guardrail_settings(
        ...
        passthrough_config = getattr(
            love_engine_logging_obj, "passthrough_guardrails_config", None
        )
    ...
    def _extract_text_for_guardrail(
        ...
        """
        from love_engine.proxy.pass_through_endpoints.jsonpath_extractor import (
            JsonPathExtractor,
        )
        ...
        payload_to_check = {
            ...
            if not k.startswith("_") and k not in ("metadata", "love_engine_logging_obj")
        }
        verbose_proxy_logger.debug(
    ...
    async def process_input_messages(
        ...
        settings = self._get_guardrail_settings(love_engine_logging_obj, guardrail_name)
