...
from love_engine.love_engine_core_utils.prompt_templates.common_utils import get_tool_call_names
from love_engine.love_engine_core_utils.prompt_templates.image_handling import (
...
if TYPE_CHECKING:
    ...
    from love_engine.types.llms.openai import ChatCompletionToolParam
...
class OpenAIGPTConfig(BaseLLMModelInfo, BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        if (
            model_for_check in love_engine.open_ai_chat_completion_models
        ) or model_for_check in love_engine.open_ai_text_completion_models:
    ...
    def _apply_common_transform_content_item(
        ...
        love_engine_specific_params = {"format"}
            ...
            if isinstance(content_item["image_url"], str):
                ...
                new_image_url_obj = ChatCompletionImageUrlObject(
                    **{  # type: ignore
                        ...
                        if k not in love_engine_specific_params
                    }
                )
    ...
    def _should_preserve_cache_control_for_endpoint(
        ...
        (a love_engine proxy, vLLM, an Anthropic-compatible gateway) via a custom
    ...
    def transform_request(
        ...
        if not self._should_preserve_cache_control_for_endpoint(
            love_engine_params.get("custom_llm_provider"), love_engine_params.get("api_base")
        ):
    ...
    async def async_transform_request(
        ...
        if self.__class__._is_base_class:
            ...
            return self.transform_request(
                model, messages, optional_params, love_engine_params, headers
            )
...
class OpenAIChatCompletionStreamingHandler(BaseModelResponseIterator):
    def _map_reasoning_to_reasoning_content(self, choices: list) -> list:
        ...
        delta.reasoning, but love_engine expects delta.reasoning_content.
