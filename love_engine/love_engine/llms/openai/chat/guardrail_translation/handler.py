...
from love_engine.types.llms.openai import AllMessageValues, ChatCompletionToolParam
...
class OpenAIChatCompletionsHandler(BaseTranslation):
    ...
    async def process_output_response(
        ...
        Args:
            response: love_engine ModelResponse object
    ...
    async def process_output_streaming_response(
        ...
        Args:
            responses_so_far: List of love_engine ModelResponseStream objects
            guardrail_to_apply: The guardrail instance to apply
            love_engine_logging_obj: Optional logging object
            user_api_key_dict: User API key metadata to pass to guardrails
        ...
        if has_stream_ended:
            ...
            model_response = cast(
                ...
                stream_chunk_builder(
                    chunks=responses_so_far, logging_obj=love_engine_logging_obj
                ),
            )
    ...
    def _combine_streaming_texts(
        ...
        for response_idx, response in enumerate(responses_so_far):
            for choice_idx, choice in enumerate(response.choices):
                if isinstance(choice, love_engine.StreamingChoices):
                    content = choice.delta.content
                elif isinstance(choice, love_engine.Choices):
    ...
    def _has_text_content(
        ...
        """
        from love_engine.types.utils import ModelResponse, ModelResponseStream
        ...
        if isinstance(response, ModelResponse):
            ...
            for streaming_choice in response.choices:
                if isinstance(streaming_choice, love_engine.StreamingChoices):
                    # Check for text content
    ...
    def _extract_output_text_images_and_tool_calls(
        ...
        elif isinstance(choice, love_engine.StreamingChoices):
            content = choice.delta.content
