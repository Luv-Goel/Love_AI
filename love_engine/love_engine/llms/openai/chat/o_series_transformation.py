...
Translations handled by love_engine:
- modalities: image => drop param (if user opts in to dropping param)
- role: system ==> translate to role 'user'
- streaming => faked by love_engine
...
class OpenAIOSeriesConfig(OpenAIGPTConfig):
                ...
                if temperature_value == 1:
                    ...
                    if love_engine.drop_params is True or drop_params is True:
                        ...
                        raise love_engine.utils.UnsupportedParamsError(
                            message="O-series models don't support temperature={}. Only temperature=1 is supported. To drop unsupported openai params from the call, set `love_engine.drop_params = True`".format(
                                temperature_value
                            ),
                            status_code=400,
                        )
    ...
    def is_model_o_series_model(self, model: str) -> bool:
        ...
        return (
            ...
            and model[0] == "o"
            and model[1].isdigit()
            and model in love_engine.open_ai_chat_completion_models
        )
