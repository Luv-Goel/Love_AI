...
class BaseOpenAILLM:
    ...
    def get_cached_openai_client(
        ...
        _cached_client = love_engine.in_memory_llm_clients_cache.get_cache(_cache_key)
    ...
    def get_openai_client_cache_key(
        ...
        LOVE_ENGINE_CLIENT_SPECIFIC_PARAMS = (
            "timeout",
            "max_retries",
            "organization",
            "api_base",
    ...
    def _get_async_http_client(
        ...
        if love_engine.aclient_session is not None:
            return love_engine.aclient_session

        if getattr(love_engine, "network_mock", False):
            from love_engine.llms.custom_httpx.mock_transport import MockOpenAITransport
    ...
    def _get_sync_http_client() -> Optional[httpx.Client]:
        if love_engine.client_session is not None:
            return love_engine.client_session
...
def get_openai_credentials(
    ...
    """Resolve OpenAI credentials from params, love_engine globals, and env vars."""
    resolved_api_base = (
        api_base
    ...
    resolved_api_key = (
        api_key or love_engine.api_key or love_engine.openai_key or os.getenv("OPENAI_API_KEY")
    )
