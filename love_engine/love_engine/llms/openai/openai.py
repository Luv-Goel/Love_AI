...
from love_engine.constants import DEFAULT_MAX_RETRIES
from love_engine.files.types import FileContentStreamingResult
...
from love_engine.llms.bedrock.chat.invoke_handler import MockResponseIterator
...
class OpenAIConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        elif love_engine.openAIGPTAudioConfig.is_model_gpt_audio_model(model=model):
            return love_engine.openAIGPTAudioConfig.get_supported_openai_params(model=model)
        else:
            return love_engine.openAIGPTConfig.get_supported_openai_params(model=model)
    ...
    def map_openai_params(
        ...
        if openaiOSeriesConfig.is_model_o_series_model(model=model):
            ...
            return love_engine.openAIGPTAudioConfig.map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=drop_params,
        ...
        return love_engine.openAIGPTConfig.map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params,
...
class OpenAIChatCompletion(BaseLLM, BaseOpenAILLM):
    ...
    async def make_openai_chat_completion_request(
        ...
        """
        Helper to:
        - call chat.completions.create.with_raw_response when love_engine.return_response_headers is True
    ...
    async def _call_agentic_completion_hooks_openai(
        ...
        verbose_logger.debug("love_engine.AgenticHooks: callbacks_count=%s", len(callbacks))
        tools = optional_params.get("tools", [])
        # Avoid logging full tools payloads; they may contain sensitive parameters
        verbose_logger.debug(
            "love_engine.AgenticHooks: tools_count=%s",
            len(tools) if isinstance(tools, list) else 1 if tools else 0,
        )
        # Get custom_llm_provider from love_engine_params
        custom_llm_provider = love_engine_params.get("custom_llm_provider", "openai")
        ...
        for callback in callbacks:
            try:
                if isinstance(callback, CustomLogger):
                    ...
                    if should_run:
                        ...
                        kwargs_with_provider = (
                            love_engine_params.copy() if love_engine_params else {}
                        )
                ...
                verbose_logger.exception(
                    f"love_engine.AgenticHookError: Exception in agentic completion hooks for OpenAI: {str(e)}"
                )
        ...
        try:
            ...
            for _ in range(
                ...
                try:
                    ...
                    ## check if body contains unprocessable params - related issue https://github.com/BerriAI/love_engine/issues/4800
                    ...
                    if (
                        ...
                        love_engine.remove_index_from_tool_calls(messages=messages)
                    else:
                        raise e
    ...
    def get_stream_options(
        ...
        if stream_options is not None:
            ...
            # by default love_engine will include usage for openai endpoints
    ...
    async def make_openai_embedding_request(
        ...
        """
        Helper to:
        - call embeddings.create.with_raw_response when love_engine.return_response_headers is True
    ...
    def embedding(  # type: ignore
        ...
        try:
            ...
            max_retries = max_retries or love_engine.DEFAULT_MAX_RETRIES
            if not isinstance(max_retries, int):
...
class OpenAIBatchesAPI(BaseLLM):
    ...
    async def acreate_batch(
        ...
        return LoveEngineBatch(**response.model_dump())
...
class OpenAIAssistantsAPI(BaseLLM):
    ...
    def create_thread(
        ...
        Here's an example:
        ```
        from love_engine.llms.openai.openai import OpenAIAssistantsAPI, MessageData
