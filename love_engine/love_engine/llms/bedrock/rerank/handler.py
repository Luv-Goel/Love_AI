...
from love_engine.types.llms.bedrock import BedrockPreparedRequest
from love_engine.types.rerank import RerankRequest
from love_engine.types.utils import RerankResponse
...
class BedrockRerankHandler(BaseAWSLLM):
    async def arerank(
        ...
        if client is None:
            client = get_async_httpx_client(llm_provider=love_engine.LlmProviders.BEDROCK)
        try:
