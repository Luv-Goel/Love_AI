"""
TODO: DELETE FILE. Bedrock LLM is no longer used. Goto `love_engine/llms/bedrock/chat/invoke_transformations/base_invoke_transformation.py`
"""
...
from love_engine.constants import RESPONSE_FORMAT_TOOL_NAME
...
from love_engine.love_engine_core_utils.logging_utils import track_llm_api_timing
...
from love_engine.llms.anthropic.chat.handler import (
...
from love_engine.utils import CustomStreamWrapper, get_secret
...
from love_engine.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
from love_engine.llms.bedrock.chat.invoke_transformations.amazon_openai_transformation import (
...
async def make_call(
    ...
    bedrock_invoke_provider: Optional[love_engine.BEDROCK_INVOKE_PROVIDERS_LITERAL] = None,
    ...
    try:
        if client is None:
            client = get_async_httpx_client(
                ...
                params=(
                    {"ssl_verify": logging_obj.love_engine_params.get("ssl_verify")}
                    if logging_obj
                    and logging_obj.love_engine_params
                    and logging_obj.love_engine_params.get("ssl_verify")
                    else None
                ),
        ...
        if fake_stream:
            model_response: (
                ModelResponse
            ) = love_engine.AmazonConverseConfig()._transform_response(
                model=model,
                response=response,
...
class BedrockLLM(BaseAWSLLM):
    ...
    def process_response(
            ...
            if provider == "cohere":
                ...
                if self.is_claude_messages_api_model(model):
                    ...
                    if (
                        ...
                        # streaming_model_response.choices = [love_engine.utils.StreamingChoices()]
                        streaming_choice = love_engine.utils.StreamingChoices()
                        ...
                        if isinstance(model_response.choices[0], love_engine.Choices):
                            if getattr(
                            ...
                            return love_engine.CustomStreamWrapper(
        ...
        if stream and provider == "ai21":
            ...
            streaming_choice = love_engine.utils.StreamingChoices()
            streaming_choice.index = model_response.choices[0].index
            delta_obj = love_engine.utils.Delta(
        ...
        if (
            ...
            prompt_tokens = int(
                bedrock_input_tokens or love_engine.token_counter(messages=messages)
            )

            completion_tokens = int(
                bedrock_output_tokens
                or love_engine.token_counter(
    ...
    def completion(
        ...
        if aws_region_name is None:
            ...
            if love_engine_aws_region_name is not None and isinstance(
                love_engine_aws_region_name, str
            ):
                aws_region_name = love_engine_aws_region_name
        ...
        if provider == "cohere":
            if model.startswith("cohere.command-r"):
                ## LOAD CONFIG
                config = love_engine.AmazonCohereChatConfig().get_config()
                ...
                ## LOAD CONFIG
                config = love_engine.AmazonCohereConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            if self.is_claude_messages_api_model(model):
                ...
                ## LOAD CONFIG
                config = love_engine.AmazonAnthropicClaudeConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
                ...
                ## LOAD CONFIG
                config = love_engine.AmazonAnthropicConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = love_engine.AmazonAI21Config.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = love_engine.AmazonMistralConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
            ...
            ## LOAD CONFIG
            config = love_engine.AmazonTitanConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > amazon_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = love_engine.AmazonLlamaConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
    ...
    async def _async_anthropic_messages_completion(
        ...
        transformed_request = (
            await love_engine.AmazonAnthropicClaudeConfig().async_transform_request(
                model=model,
                messages=messages,
                optional_params=optional_params,
        ...
        if client is None:
            ...
            if timeout is not None:
                if isinstance(timeout, float) or isinstance(timeout, int):
                    timeout = httpx.Timeout(timeout)
                _params["timeout"] = timeout
            client = get_async_httpx_client(params=_params, llm_provider=love_engine.LlmProviders.BEDROCK)  # type: ignore
        else:
    ...
    def _get_provider_from_model_path(
        model_path: str,
    ) -> Optional[love_engine.BEDROCK_INVOKE_PROVIDERS_LITERAL]:
        """
        ...
        if len(parts) >= 1:
            provider = parts[0]
            if provider in get_args(love_engine.BEDROCK_INVOKE_PROVIDERS_LITERAL):
                return cast(love_engine.BEDROCK_INVOKE_PROVIDERS_LITERAL, provider)
        return None
    ...
    def _handle_converse_start_event(
        ...
        if start_obj is not None:
            if "toolUse" in start_obj and start_obj["toolUse"] is not None:
                ## check tool name was formatted by love_engine
...
class AmazonDeepSeekR1StreamDecoder(AWSEventStreamDecoder):
    def __init__(
        ...
        super().__init__(model=model)
        from love_engine.llms.bedrock.chat.invoke_transformations.amazon_deepseek_transformation import (
...
class MockResponseIterator:  # for returning ai21 streaming responses
    ...
    def _handle_json_mode_chunk(
        ...
        if self.json_mode is True and tool_calls is not None:
            message = love_engine.AnthropicConfig()._convert_tool_response_to_message(
