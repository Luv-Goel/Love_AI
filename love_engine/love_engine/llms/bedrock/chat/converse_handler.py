...
from love_engine.love_engine_core_utils.love_engine_logging import Logging as LoveEngineLoggingObject
...
class BedrockConverseLLM(BaseAWSLLM):
    ...
    async def async_streaming(
        ...
        request_data = await love_engine.AmazonConverseConfig()._async_transform_request(
            model=model,
            messages=messages,
        ...
        prepped = self.get_request_headers(
            credentials=credentials,
            aws_region_name=love_engine_params.get("aws_region_name") or "us-west-2",
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            ...
            client = get_async_httpx_client(
                params=_params, llm_provider=love_engine.LlmProviders.BEDROCK
            )
        else:
        ...
        return love_engine.AmazonConverseConfig()._transform_response(
            model=model,
            response=response,
    ...
    def completion(
        ...
        fake_stream = love_engine.AmazonConverseConfig().should_fake_stream(
            fake_stream=fake_stream,
            model=model,
            stream=stream,
            custom_llm_provider="bedrock",
        ...
        love_engine_params["aws_region_name"] = (
        ...
        _data = love_engine.AmazonConverseConfig()._transform_request(
            model=model,
            messages=messages,
            optional_params=optional_params,
            love_engine_params=love_engine_params,
