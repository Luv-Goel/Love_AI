...
from love_engine.llms.anthropic.chat.transformation import (
...
class AmazonConverseConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> List[str]:
        from love_engine.utils import supports_function_calling
        ...
        if love_engine.utils.supports_tool_choice(
            model=model, custom_llm_provider=self.custom_llm_provider
        ) or love_engine.utils.supports_tool_choice(
        ...
        if tool_choice == "none":
            if love_engine.drop_params is True or drop_params is True:
                ...
                raise love_engine.utils.UnsupportedParamsError(
                    message="Bedrock doesn't support tool_choice={}. To drop it from the call, set `love_engine.drop_params = True.".format(
                        tool_choice
                    ),
                    status_code=400,
                )
            ...
            raise love_engine.utils.UnsupportedParamsError(
                message="Bedrock doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `love_engine.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def _supports_native_structured_outputs(
        ...
        which looks up the flag in ``love_engine.model_cost`` via
        ``_get_model_info_helper``.
        Ref: https://docs.aws.amazon.com/bedrock/latest/userguide/structured-output.html
        """
        from love_engine.utils import supports_native_structured_output
    ...
    def _translate_response_format_param(
        ...
        if (
            ...
            if (
                love_engine.utils.supports_tool_choice(
                    model=model, custom_llm_provider=self.custom_llm_provider
                )
    ...
    def update_optional_params_with_thinking_tokens(
        ...
        """
        from love_engine.constants import DEFAULT_MAX_TOKENS
    ...
    def _prepare_request_params(
        ...
        # These are love_engine internal parameters, not API parameters
        ...
        if anthropic_output_config is not None and isinstance(
            ...
            if base_model.startswith("anthropic"):
                if (
                    love_engine.drop_params is True
    ...
    def _filter_context_management_for_bedrock_converse(
        ...
        from love_engine.types.llms.anthropic import ANTHROPIC_BETA_HEADER_VALUES
    ...
    def _transform_request_helper(
        ...
        if (
            ...
            if love_engine.modify_params:
                ...
                raise love_engine.UnsupportedParamsError(
                    message="Bedrock doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `love_engine.modify_params = True` // `love_engine_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
                    llm_provider="bedrock",
                )
        ...
        # Related issues: https://github.com/BerriAI/love_engine/issues/14194
        if (
    ...
    async def _async_transform_request(
        ...
        bedrock_messages = (
            await BedrockConverseMessagesProcessor._bedrock_converse_messages_pt_async(
                ...
                model=model,
                llm_provider="bedrock_converse",
                user_continue_message=love_engine_params.pop("user_continue_message", None),
            )
        )
