...
if TYPE_CHECKING:
    from love_engine.types.llms.bedrock import BedrockCreateBatchRequest
...
from love_engine.llms.base_llm.base_utils import BaseLLMModelInfo, BaseTokenCounter
...
def get_cached_model_info():
    ...
    if _get_model_info is None:
        from love_engine import get_model_info
...
def _get_local_model_cost_map() -> Dict:
    from love_engine.love_engine_core_utils.get_model_cost_map import GetModelCostMap
...
def remove_custom_field_from_tools(request_body: dict) -> None:
    ...
    Ref: https://github.com/BerriAI/love_engine/issues/22847
    """
...
def ensure_bedrock_anthropic_messages_tool_names(request_body: dict) -> None:
    ...
    In-place: set ``name`` to ``love_engine_unnamed_tool_{index}`` when missing or blank.
    """
    tools = request_body.get("tools")
    if not tools or not isinstance(tools, list):
        return
    for i, tool in enumerate(tools):
        ...
        if name is None or (isinstance(name, str) and not name.strip()):
            tool["name"] = f"love_engine_unnamed_tool_{i}"
...
def _get_bedrock_client_ssl_verify() -> Union[bool, str]:
    ...
    """
    from love_engine.llms.custom_httpx.http_handler import get_ssl_verify
...
def init_bedrock_client(
    ...
    love_engine_aws_region_name = get_secret("AWS_REGION_NAME", None)
    ...
    elif love_engine_aws_region_name:
        region_name = love_engine_aws_region_name
    ...
    if (
        ...
        # aws_access_key_id is not None, assume user is trying to auth using love_engine.completion
...
def get_bedrock_tool_name(response_tool_name: str) -> str:
    """
    If love_engine formatted the input tool name, we need to convert it back to the original name.
    ...
    if response_tool_name in love_engine.bedrock_tool_name_mappings.cache_dict:
        response_tool_name = love_engine.bedrock_tool_name_mappings.cache_dict[
            response_tool_name
        ]
...
def strip_bedrock_routing_prefix(model: str) -> str:
    """Strip love_engine routing prefixes from model name."""
...
from love_engine.llms.bedrock.count_tokens.bedrock_token_counter import BedrockTokenCounter
...
class BedrockModelInfo(BaseLLMModelInfo):
    ...
    @staticmethod
    def get_non_love_engine_routing_model_name(model: str) -> str:
    ...
    def get_bedrock_route(
        ...
        alt_model = BedrockModelInfo.get_non_love_engine_routing_model_name(model=model)
        if (
            base_model in love_engine.bedrock_converse_models
            or alt_model in love_engine.bedrock_converse_models
        ):
            return "converse"
        return "invoke"
    ...
    def get_bedrock_provider_config_for_messages_api(
        ...
        All other routes should return None since they will go through love_engine.completion
        """
        ...
        if BedrockModelInfo._explicit_claude_platform_route(model):
            return love_engine.BedrockClaudePlatformMessagesConfig()

        #########################################################
        # Converse routes should go through love_engine.completion()
        ...
        if BedrockModelInfo._explicit_mantle_route(model):
            from love_engine.llms.bedrock.messages.mantle_transformation import (
        ...
        #########################################################
        # This goes through love_engine.AmazonAnthropicClaude3MessagesConfig()
        # Since bedrock Invoke supports Native Anthropic Messages API
        #########################################################
        if "claude" in model:
            return love_engine.AmazonAnthropicClaudeMessagesConfig()

        #########################################################
        # These routes will go through love_engine.completion()
        #########################################################
        return None
...
def get_bedrock_chat_config(model: str):
    ...
    bedrock_invoke_provider = love_engine.BedrockLLM.get_bedrock_invoke_provider(
        model=model
    )
    base_model = BedrockModelInfo.get_base_model(model)
    ...
    if bedrock_route == "claude_platform":
        return love_engine.BedrockClaudePlatformConfig()
    elif bedrock_route == "converse" or bedrock_route == "converse_like":
        return love_engine.AmazonConverseConfig()
    elif bedrock_route == "openai":
        return love_engine.AmazonBedrockOpenAIConfig()
    elif bedrock_route == "agent":
        from love_engine.llms.bedrock.chat.invoke_agent.transformation import (
            AmazonInvokeAgentConfig,
        )

        return AmazonInvokeAgentConfig()
    elif bedrock_route == "agentcore":
        from love_engine.llms.bedrock.chat.agentcore.transformation import (
            AmazonAgentCoreConfig,
        )

        return AmazonAgentCoreConfig()
    elif bedrock_route == "mantle":
        from love_engine.llms.bedrock.chat.mantle.transformation import (
            AmazonMantleConfig,
        )
    ...
    if bedrock_invoke_provider == "amazon":
        return love_engine.AmazonTitanConfig()
    elif bedrock_invoke_provider == "anthropic":
        if (
            base_model
            in love_engine.AmazonAnthropicConfig.get_legacy_anthropic_model_names()
        ):
            return love_engine.AmazonAnthropicConfig()
        else:
            return love_engine.AmazonAnthropicClaudeConfig()
    elif bedrock_invoke_provider == "meta" or bedrock_invoke_provider == "llama":
        return love_engine.AmazonLlamaConfig()
    elif bedrock_invoke_provider == "ai21":
        return love_engine.AmazonAI21Config()
    elif bedrock_invoke_provider == "cohere":
        return love_engine.AmazonCohereConfig()
    elif bedrock_invoke_provider == "mistral":
        return love_engine.AmazonMistralConfig()
    elif bedrock_invoke_provider == "moonshot":
        return love_engine.AmazonMoonshotConfig()
    elif bedrock_invoke_provider == "deepseek_r1":
        return love_engine.AmazonDeepSeekR1Config()
    elif bedrock_invoke_provider == "nova":
        return love_engine.AmazonInvokeNovaConfig()
    elif bedrock_invoke_provider == "qwen3":
        return love_engine.AmazonQwen3Config()
    elif bedrock_invoke_provider == "qwen2":
        return love_engine.AmazonQwen2Config()
    elif bedrock_invoke_provider == "twelvelabs":
        return love_engine.AmazonTwelveLabsPegasusConfig()
    else:
        return love_engine.AmazonInvokeConfig()
...
def _load_bedrock_response_stream_shape():
    ...
    try:
        ...
        verbose_logger.warning(
            "love_engine: could not load bedrock-runtime response stream shape "
...
class CommonBatchFilesUtils:
    ...
    def get_bedrock_model_id_from_love_engine_model(self, model: str) -> str:
        """
        Extract the actual Bedrock model ID from love_engine model name.

        Args:
            model: love_engine model name (e.g., "bedrock/anthropic.claude-3-sonnet-20240229-v1:0")
    ...
    def extract_model_from_s3_file_path(
        ...
        Format: s3://bucket/love_engine-bedrock-files-{model}-{uuid}.jsonl
        """
        # Check if model is provided in optional_params first
        if "model" in optional_params and optional_params["model"]:
            return self.get_bedrock_model_id_from_love_engine_model(
                optional_params["model"]
            )

        # Extract model from S3 URI path
        # Expected format: s3://bucket/love_engine-bedrock-files-{model}-{uuid}.jsonl
        try:
            bucket, object_key = self.parse_s3_uri(s3_uri)

            # Extract model from object key if it follows our naming pattern
            if object_key.startswith("love_engine-bedrock-files-"):
                # Remove prefix and suffix to get model part
                model_part = object_key[22:]  # Remove "love_engine-bedrock-files-"
    ...
    def generate_unique_job_name(self, model: str, prefix: str = "love_engine") -> str:
        """
        ...
        # Example: love_engine-batch-claude-266c398e
    ...
    def get_s3_bucket_and_key_from_config(
        ...
        # Get bucket name
        bucket_name = (
            love_engine_params.get("s3_bucket_name")
