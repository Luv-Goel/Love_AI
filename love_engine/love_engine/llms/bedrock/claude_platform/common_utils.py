...
class BedrockClaudePlatformMixin(BaseAWSLLM):
    @staticmethod
    def _get_workspace_id(optional_params: dict, love_engine_params: dict) -> Optional[str]:
        workspace_id = (
            optional_params.get("workspace_id")
            or love_engine_params.get("workspace_id")
            or optional_params.get("aws_workspace_id")
            or love_engine_params.get("aws_workspace_id")
            or optional_params.get("anthropic-workspace-id")
            or love_engine_params.get("anthropic-workspace-id")
        )
        if workspace_id is None:
            workspace_id = optional_params.get(
                "anthropic_workspace_id"
            ) or love_engine_params.get("anthropic_workspace_id")
