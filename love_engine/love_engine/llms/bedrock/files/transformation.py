...
from love_engine.types.utils import ExtractedFileData, LlmProviders
from love_engine.utils import get_llm_provider
...
class BedrockFilesConfig(BaseAWSLLM, BaseFilesConfig):
    ...
    def _get_s3_object_name_from_batch_jsonl(
        ...
        named as: love_engine-bedrock-files/{model}/{uuid}
        """
    ...
    def get_complete_file_url(
        ...
        """
        bucket_name = love_engine_params.get("s3_bucket_name") or os.getenv(
            "AWS_S3_BUCKET_NAME"
        )
        if not bucket_name:
            raise ValueError(
                "S3 bucket_name is required. Set 's3_bucket_name' in love_engine_params or AWS_S3_BUCKET_NAME env var"
            )
        bucket_name, object_prefix = split_configured_cloud_bucket_name(bucket_name)

        s3_region_name = love_engine_params.get("s3_region_name") or optional_params.get(
            "s3_region_name"
        )
    ...
    def _lookup_provider_specific_field(model_id: str, field: str) -> Optional[str]:
        ...
        `provider_specific_entry` dict via `love_engine.get_model_info`.
    ...
    def _map_openai_embedding_to_bedrock_params(
        ...
        """
        from love_engine.llms.bedrock.embed.amazon_titan_v2_transformation import (
            AmazonTitanV2Config,
        )
        ...
        if not self._is_titan_v2_embed_model(_model):
            ...
            raise NotImplementedError(
                ...
                "embedding models in https://github.com/BerriAI/love_engine/issues."
            )
    ...
    def _map_openai_to_bedrock_params(
        ...
        if provider in self.CONVERSE_INVOKE_PROVIDERS:
            from love_engine.llms.bedrock.chat.converse_transformation import (
        ...
        for idx, _openai_jsonl_content in enumerate(openai_jsonl_content):
            ...
            try:
                ...
                verbose_logger.exception(
                    f"love_engine.llms.bedrock.files.transformation.py::_transform_openai_jsonl_content_to_bedrock_jsonl_content() - Error inferring custom_llm_provider - {str(e)}"
                )
    ...
    def transform_create_file_request(
        ...
        love_engine_params["upload_url"] = api_base
    ...
    def _sign_s3_request(
        ...
        Reuses the exact pattern from love_engine/integrations/s3_v2.py
        """
        try:
            import hashlib
    ...
    def _convert_https_url_to_s3_uri(self, https_url: str) -> tuple[str, str]:
        ...
        Example:
            Input: "https://s3.us-west-2.amazonaws.com/love_engine-proxy/file.jsonl"
            Output: ("s3://love_engine-proxy/file.jsonl", "file.jsonl")
        """
        import re
    ...
    def transform_create_file_response(
        ...
        upload_url = love_engine_params.get("upload_url")
        file_id: str = ""
        filename: str = ""
        if upload_url:
            # Convert HTTPS S3 URL to s3:// URI format
