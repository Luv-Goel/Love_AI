...
import love_engine
from love_engine.anthropic_beta_headers_manager import filter_and_transform_beta_headers
from love_engine.constants import BEDROCK_MIN_THINKING_BUDGET_TOKENS
from love_engine.love_engine_core_utils.love_engine_logging import verbose_logger
...
from love_engine.llms.bedrock.chat.invoke_handler import AWSEventStreamDecoder
from love_engine.llms.bedrock.chat.invoke_transformations.base_invoke_transformation import (
    AmazonInvokeConfig,
)
...
from love_engine.types.llms.bedrock import BedrockInvokeAnthropicMessagesRequest
from love_engine.types.llms.openai import AllMessageValues
from love_engine.types.router import GenericLoveEngineParams
from love_engine.types.utils import GenericStreamingChunk
...
from love_engine.utils import _supports_factory
...
class AmazonAnthropicClaudeMessagesConfig(
    ...
    def _filter_context_management_for_bedrock_invoke(
        ...
        which Claude Code sends on every request) are love_engine-internal and would
        ...
        Ref: https://github.com/BerriAI/love_engine/issues/27532
        """
        cm = anthropic_messages_request.get("context_management")
        if not isinstance(cm, dict):
            return
    ...
    def transform_anthropic_messages_request(
        ...
        # Ref: https://github.com/BerriAI/love_engine/issues/22797
        if not (
            _supports_factory(
                model=model,
    ...
    async def bedrock_sse_wrapper(
        ...
        """
        Bedrock invoke does not return SSE formatted data. This function is a wrapper to ensure love_engine chunks are SSE formatted.
        ...
        """
        from love_engine.llms.anthropic.experimental_pass_through.messages.streaming_iterator import (
