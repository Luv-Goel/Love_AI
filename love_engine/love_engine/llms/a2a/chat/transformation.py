"""
A2A Protocol Transformation for love_engine
"""
...
class A2AConfig(BaseConfig):
    ...
    def resolve_agent_config_from_registry(
        ...
        Args:
            ...
            optional_params: Dict to merge additional love_engine_params into
        ...
        try:
            ...
            if agent:
                ...
                # Get api_key, headers, and other params from love_engine_params
                if agent.love_engine_params:
                    if api_key is None:
                        api_key = agent.love_engine_params.get("api_key")

                    if headers is None:
                        agent_headers = agent.love_engine_params.get("headers")
                        if agent_headers:
                            headers = agent_headers

                    # Merge other love_engine_params (timeout, max_retries, etc.)
                    for key, value in agent.love_engine_params.items():
                        if (
