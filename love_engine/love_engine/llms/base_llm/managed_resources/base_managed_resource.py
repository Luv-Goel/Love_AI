...
from love_engine.llms.base_llm.managed_resources.isolation import (
    build_list_page,
    build_owner_filter,
    can_access_resource,
)
...
if TYPE_CHECKING:
    ...
    from love_engine.proxy.utils import PrismaClient as _PrismaClient
...
class BaseManagedResource(ABC, Generic[ResourceObjectType]):
    ...
    def table_name(self) -> str:
        ...
        Example: 'love_engine_managedfiletable', 'love_engine_managedvectorstoretable'
        """
        pass
    ...
    def get_unified_resource_id_format(
        ...
        Example for files:
            "love_engine_proxy:application/json;unified_id,{uuid};target_model_names,{models};..."
    ...
    async def create_resource_for_model(
        ...
        love_engine_parent_otel_span: Span,
    ) -> ResourceObjectType:
        """
        Create a resource for a specific model.

        Args:
            llm_router: love_engine router instance
            model: Model name to create resource for
            request_data: Request data for resource creation
            love_engine_parent_otel_span: OpenTelemetry span for tracing
    ...
    async def store_unified_resource_id(
        ...
        love_engine_parent_otel_span: Optional[Span],
        ...
        """
        verbose_logger.info(
            f"Storing love_engine Managed {self.resource_type} with id={unified_resource_id} in cache"
        )
        ...
        verbose_logger.debug(
            f"love_engine Managed {self.resource_type} with id={unified_resource_id} stored in db: {result}"
        )

    async def get_unified_resource_id(
        self,
        unified_resource_id: str,
        love_engine_parent_otel_span: Optional[Span] = None,
    ...
    async def delete_unified_resource_id(
        ...
        if initial_value is None:
            raise Exception(
                f"love_engine Managed {self.resource_type} with id={unified_resource_id} not found"
            )
    ...
    async def can_user_access_unified_resource_id(
        ...
        resource = await self.get_unified_resource_id(
            unified_resource_id, love_engine_parent_otel_span
        )
    ...
    async def get_model_resource_id_mapping(
        ...
        for resource_id in resource_ids:
            ...
            unified_resource_object = await self.get_unified_resource_id(
                resource_id, love_engine_parent_otel_span
            )
    ...
    def get_unified_id_prefix(self) -> str:
        ...
        Returns:
            Prefix string (e.g., "love_engine_proxy:")
        """
