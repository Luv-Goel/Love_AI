...
import love_engine
from love_engine.constants import AZURE_OPERATION_POLLING_TIMEOUT, DEFAULT_MAX_RETRIES
...
from love_engine.love_engine_core_utils.url_utils import SSRFError, assert_same_origin
                ...
                if isinstance(value, list):
                    for item in value:
                        if "file_id" in item:
                            ...
                            if love_engine.drop_params is True:
                                pass
                            else:
                                raise love_engine.utils.UnsupportedParamsError(
                                    message="Azure doesn't support {}. To drop it from the call, set `love_engine.drop_params = True.".format(
                                        value
                                    ),
                                    status_code=400,
                                )
...
class AzureChatCompletion(BaseAzureLLM, BaseLLM):
    ...
    def completion(
        ...
        try:
            ...
            if api_base is not None and "gateway.ai.cloudflare.com" in api_base:
                ...
                model=love_engine_params.get("base_model") or model
            ):
                data = love_engine.AzureOpenAIGPT5Config().transform_request(
                    model=model,
                    messages=messages,
                ...
                data = love_engine.AzureOpenAIConfig().transform_request(
                    model=model,
                    messages=messages,
                    optional_params=optional_params,
                    love_engine_params=love_engine_params,
    ...
    def embedding(
        ...
        try:
            ...
            if max_retries is None:
                max_retries = love_engine.DEFAULT_MAX_RETRIES
            ## LOGGING
            logging_obj.pre_call(
                input=input,
                api_key=api_key,
    ...
    def make_sync_azure_httpx_request(
        ...
        if client is None:
            ...
            sync_handler = HTTPHandler(**_params, client=love_engine.client_session)  # type: ignore
        else:
    ...
    def image_generation(
        ...
        try:
            ...
            if (
                ...
                and love_engine_params is not None
                and love_engine_params.get("base_model", None) is not None
            ):
                model_response._hidden_params["model"] = love_engine_params.get(
                    "base_model", None
                )
            ...
            base_model = (
                love_engine_params.get("base_model", None) if love_engine_params else None
            )
    ...
    def get_headers(
        ...
        client_session = love_engine.client_session or httpx.Client()
        if api_base is not None and "gateway.ai.cloudflare.com" in api_base:
