...
from love_engine.llms.openai.common_utils import BaseOpenAILLM
from love_engine.secret_managers.get_azure_ad_token_provider import (
...
from love_engine.utils import _add_path_to_api_base
...
def get_azure_ad_token_from_oidc(
    ...
    client = love_engine.module_level_client
...
def get_azure_ad_token(
    ...
    Args:
        love_engine_params: Dictionary containing authentication parameters
    ...
    azure_ad_token_provider = love_engine_params.get("azure_ad_token_provider")
    azure_ad_token = love_engine_params.get("azure_ad_token") or get_secret_str(
        "AZURE_AD_TOKEN"
    )
    tenant_id = love_engine_params.get("tenant_id") or os.getenv("AZURE_TENANT_ID")
    client_id = love_engine_params.get("client_id") or os.getenv("AZURE_CLIENT_ID")
    client_secret = love_engine_params.get("client_secret") or os.getenv(
        "AZURE_CLIENT_SECRET"
    )
    azure_username = love_engine_params.get("azure_username") or os.getenv("AZURE_USERNAME")
    azure_password = love_engine_params.get("azure_password") or os.getenv("AZURE_PASSWORD")
    scope = love_engine_params.get("azure_scope") or os.getenv(
    ...
    # Try to get token provider from service principal or DefaultAzureCredential
        ...
        azure_ad_token_provider is None
        and love_engine.enable_azure_ad_token_refresh is True
    ):
        verbose_logger.debug(
        ...
        try:
            ...
            verbose_logger.error(
                f"Error calling Azure AD token provider: {str(e)}. Follow docs - https://docs.love_engine.ai/docs/providers/azure/#azure-ad-token-refresh---defaultazurecredential"
            )
            raise e

        #########################################################
        # If love_engine.enable_azure_ad_token_refresh is True and no other token provider is available,
...
class BaseAzureLLM(BaseOpenAILLM):
    ...
    def _try_get_default_azure_credential_provider(
        ...
        """
        from love_engine.types.secret_managers.get_azure_ad_token_provider import (
            AzureCredentialType,
        )
    ...
    def initialize_azure_sdk_client(
        ...
        azure_ad_token = love_engine_params.get("azure_ad_token")

        # love_engine_params sometimes contains the key, but the value is None
        # We should respect environment variables in this case
        tenant_id = self._resolve_env_var(
            love_engine_params, "tenant_id", "AZURE_TENANT_ID"
        )
        client_id = self._resolve_env_var(
            love_engine_params, "client_id", "AZURE_CLIENT_ID"
        )
        client_secret = self._resolve_env_var(
            love_engine_params, "client_secret", "AZURE_CLIENT_SECRET"
        )
        azure_username = self._resolve_env_var(
            love_engine_params, "azure_username", "AZURE_USERNAME"
        )
        azure_password = self._resolve_env_var(
            love_engine_params, "azure_password", "AZURE_PASSWORD"
        )
        scope = self._resolve_env_var(love_engine_params, "azure_scope", "AZURE_SCOPE")
        if scope is None:
            scope = "https://cognitiveservices.azure.com/.default"

        max_retries = love_engine_params.get("max_retries")
        timeout = love_engine_params.get("timeout")
        if (
            not api_key
        ...
        if api_version is None:
            api_version = os.getenv(
                "AZURE_API_VERSION", love_engine.AZURE_DEFAULT_API_VERSION
            )
    ...
    def _init_azure_client_for_cloudflare_ai_gateway(
        ...
        tenant_id = love_engine_params.get("tenant_id", os.getenv("AZURE_TENANT_ID"))
        client_id = love_engine_params.get("client_id", os.getenv("AZURE_CLIENT_ID"))
        scope = love_engine_params.get(
            "azure_scope",
        ...
        if client is None:
            ...
            azure_client_params: Dict[str, Any] = {
                "api_version": api_version,
                "base_url": f"{api_base}",
                "http_client": love_engine.client_session,
    ...
    def _base_validate_azure_environment(
        headers: dict, love_engine_params: Optional[GenericLoveEngineParams]
    ) -> dict:
        ...
        azure_ad_token = get_azure_ad_token(love_engine_params)
        if azure_ad_token:
    ...
    def _get_base_azure_url(
        api_base: Optional[str],
        love_engine_params: Optional[Union[GenericLoveEngineParams, Dict[str, Any]]],
        ...
        Args:
            ...
            love_engine_params: The love_engine parameters.
        ...
        love_engine_params = love_engine_params or {}
        api_version = (
            cast(Optional[str], love_engine_params.get("api_version"))
    ...
    def _resolve_env_var(
        self, love_engine_params: Dict[str, Any], param_key: str, env_var_key: str
    ) -> Optional[str]:
        ...
        love_engine_params may contain the key with a None value, in which case we want
        to fallback to the environment variable.
        """
        param_value = love_engine_params.get(param_key)
...
def get_azure_credentials(
    ...
    """Resolve Azure credentials from params, love_engine globals, and env vars."""
    resolved_api_base = api_base or love_engine.api_base or get_secret_str("AZURE_API_BASE")
