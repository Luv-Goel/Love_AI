...
`love_engine.main.transcription()` routes Soniox requests directly to this
...
from love_engine.llms.soniox.common_utils import (
...
class SonioxAudioTranscriptionHandler:
    ...
    def audio_transcriptions(
        ...
        ``love_engine.transcription`` but is **not yet implemented** for the Soniox
        ...
        ``love_engine.Router`` / ``num_retries`` mechanism for retry behaviour.
        """
    ...
    def _prepare(
        ...
        # response_format is handled by LoveEngine post-processing, not Soniox.
    ...
    def _redact_body_for_logging(body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        let anyone with read access to those sinks love_smith webhook requests, so
    ...
    async def _async_audio_transcriptions(
        ...
        http_client = (
            ...
            if isinstance(client, AsyncHTTPHandler)
                ...
                get_async_httpx_client(
                    llm_provider=love_engine.LlmProviders.SONIOX,
                    params={"ssl_verify": love_engine_params.get("ssl_verify", None)},
                )
            )
        )
