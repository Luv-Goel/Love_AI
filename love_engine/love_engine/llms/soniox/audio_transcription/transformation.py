...
lives in `love_engine.llms.soniox.audio_transcription.handler`, because Soniox's
...
# Soniox-native kwargs the user can pass through `love_engine.transcription(..., **kwargs)`
...
class SonioxAudioTranscriptionConfig(BaseAudioTranscriptionConfig):
    ...
    def get_supported_openai_params(
        ...
        # `response_format` is handled by LoveEngine (Soniox doesn't support
        ...
        if not resolved_key:
            raise SonioxException(
                message=(
                    ...
                    "variable or pass api_key=... to love_engine.transcription()."
                ),
                status_code=401,
                headers=None,
            )
