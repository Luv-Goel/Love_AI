...
# Keys inside love_engine_params that should be forwarded to the Gemini
# create-agent body verbatim.
_GEMINI_AGENT_BODY_KEYS = ("base_agent", "instructions", "base_environment")

# love_engine-internal keys that must never be forwarded to Gemini.
_LOVE_ENGINE_INTERNAL_KEYS = frozenset(
    {
        "custom_llm_provider",
        "api_key",
        "api_base",
...
class GeminiAgentsConfig(BaseAgentsAPIConfig):
    ...
    def validate_environment(
        ...
        explicit_api_key = love_engine_params.get("api_key")
        ...
        if love_engine_params.get("api_base") and not explicit_api_key:
            raise ValueError(
    ...
    def transform_create_request(
        ...
        for key in _GEMINI_AGENT_BODY_KEYS:
            value = love_engine_params.get(key)
    ...
    def transform_list_request(
        ...
        params: Dict[str, Any] = {}
        if love_engine_params.get("page_size"):
            params["pageSize"] = love_engine_params["page_size"]
        if love_engine_params.get("page_token"):
            params["pageToken"] = love_engine_params["page_token"]
        return url, params
