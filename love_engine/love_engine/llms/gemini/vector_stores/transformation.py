...
Implements the transformation between love_engine's unified vector store API
...
class GeminiVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def validate_environment(
        ...
        if love_engine_params:
            api_key = love_engine_params.get("api_key") or get_api_key_from_env()
            if api_key:
    ...
    def transform_search_vector_store_request(
        ...
        # Get model from love_engine_params or use default
        # Note: File Search requires gemini-2.5-flash or later
        model = love_engine_params.get("model") or "gemini-2.5-flash"
        if model and model.startswith("gemini/"):
            model = model.replace("gemini/", "")

        api_key = love_engine_params.get("api_key") or GeminiModelInfo.get_api_key()
        if not api_key:
        ...
        love_engine_logging_obj.model_call_details["vector_store_id"] = vector_store_id
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            query = love_engine_logging_obj.model_call_details.get("query", "")
