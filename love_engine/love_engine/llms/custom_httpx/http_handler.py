...
from love_engine.types.llms.custom_http import *

if TYPE_CHECKING:
    from love_engine import LlmProviders
    from love_engine.love_engine_core_utils.love_engine_logging import (
        Logging as LoveEngineLoggingObject,
    )
    from love_engine.llms.custom_httpx.aiohttp_transport import LoveEngineAiohttpTransport
else:
    LlmProviders = Any
    LoveEngineLoggingObject = Any
    LoveEngineAiohttpTransport = Any
...
def get_default_headers() -> dict:
    ...
    - Default: `User-Agent: love_engine/{version}`
    - Override: set `LOVE_ENGINE_USER_AGENT` to fully override the header value.
    """
    user_agent = os.environ.get("LOVE_ENGINE_USER_AGENT")
    if user_agent is not None:
        return {"User-Agent": user_agent}

    return {"User-Agent": f"love_engine/{version}"}
...
_STREAMING_ERROR_BODY_READ_EXECUTOR = concurrent.futures.ThreadPoolExecutor(
    max_workers=50,
    thread_name_prefix="love_engine-streaming-error-body-read",
)
...
def _create_ssl_context(
    ...
    # Set SSL_ECDH_CURVE env var or love_engine.ssl_ecdh_curve to 'X25519' to disable PQC
...
def get_ssl_verify(
    ...
    if ssl_verify is None:
        ssl_verify = os.getenv("SSL_VERIFY", love_engine.ssl_verify)
...
def get_ssl_configuration(
    ...
    Args:
        ssl_verify: SSL verification setting. Can be:
            - None: Use default from environment/love_engine settings
    ...
    ssl_security_level = os.getenv("SSL_SECURITY_LEVEL", love_engine.ssl_security_level)
    ssl_ecdh_curve = os.getenv("SSL_ECDH_CURVE", love_engine.ssl_ecdh_curve)
...
class AsyncHTTPHandler:
    ...
    def create_client(
        ...
        # /path/to/client.pem
        cert = os.getenv("SSL_CERTIFICATE", love_engine.ssl_certificate)
        ...
        # Get default headers (User-Agent, overridable via LOVE_ENGINE_USER_AGENT)
    ...
    async def post(
        ...
        timeout: Optional[Union[float, httpx.Timeout]] = None,
        stream: bool = False,
        logging_obj: Optional[LoveEngineLoggingObject] = None,
        ...
        try:
            ...
            raise love_engine.Timeout(
                ...
                llm_provider="love_engine-httpx-handler",
                headers=headers,
            )
    ...
    def _create_async_transport(
        ...
        shared_session: Optional["ClientSession"] = None,
    ) -> Optional[Union[LoveEngineAiohttpTransport, AsyncHTTPTransport]]:
        """
        - Creates a transport for httpx.AsyncClient
            - if love_engine.force_ipv4 is True, it will return AsyncHTTPTransport with local_address="0.0.0.0"
            - [Default] It will return AiohttpTransport
            - Users can opt out of using AiohttpTransport by setting love_engine.use_aiohttp_transport to False
    ...
    def _should_use_aiohttp_transport() -> bool:
        """
        AiohttpTransport is the default transport for love_engine.

        Httpx can be used by the following
            - love_engine.disable_aiohttp_transport = True
        ...
        ########################################################
        if (
            love_engine.disable_aiohttp_transport is True
    ...
    def _get_ssl_connector_kwargs(
        ...
        connector_kwargs: Dict[str, Any] = {
            "local_addr": ("0.0.0.0", 0) if love_engine.force_ipv4 else None,
        }
    ...
    def _create_aiohttp_transport(
        ...
        ########################################################
        trust_env: bool = love_engine.aiohttp_trust_env
        ...
        if shared_session is not None and not shared_session.closed:
            ...
            return LoveEngineAiohttpTransport(
...
class HTTPHandler:
    ...
    def _create_sync_transport(self) -> Optional[HTTPTransport]:
        """
        Create an HTTP transport with IPv4 only if love_engine.force_ipv4 is True.
        ...
        if love_engine.force_ipv4:
            ...
            return getattr(love_engine, "sync_transport", None)
...
def get_async_httpx_client(
    ...
    # love_engine globals being fully populated during import time.
    cache = getattr(love_engine, "in_memory_llm_clients_cache", None)
    if cache is None:
        from love_engine.caching.llm_caching_handler import LLMClientCache

        cache = LLMClientCache()
        setattr(love_engine, "in_memory_llm_clients_cache", cache)
