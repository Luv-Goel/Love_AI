...
class LoveEngineAiohttpTransport(AiohttpTransport):
    """
    love_engine wrapper around AiohttpTransport to handle %-encodings in URLs
    ...
    async def _get_proxy_settings(self, request: httpx.Request):
        proxy = None
        if not (
            love_engine.disable_aiohttp_trust_env
