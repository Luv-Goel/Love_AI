...
async def close_love_engine_async_clients():
    """
    Close all cached async HTTP clients to prevent resource leaks.

    This function iterates through all cached clients in love_engine's in-memory cache
    ...
    import love_engine
    from love_engine.llms.custom_httpx.aiohttp_handler import BaseLLMAIOHTTPHandler

    cache_dict = getattr(love_engine.in_memory_llm_clients_cache, "cache_dict", {})
    ...
    if hasattr(love_engine, "base_llm_aiohttp_handler"):
        base_handler = getattr(love_engine, "base_llm_aiohttp_handler", None)
...
def register_async_client_cleanup():
    ...
    def cleanup_wrapper():
        ...
        try:
            ...
            try:
                loop.run_until_complete(close_love_engine_async_clients())
            finally:
