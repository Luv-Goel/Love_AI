...
AWS_BEARER_TOKEN_BEDROCK, or love_engine_params.api_key) when present; otherwise
...
from love_engine.llms.bedrock_mantle.common_utils import (
    MANTLE_HOST_RE,
...
class BedrockMantleResponsesAPIConfig(BedrockMantleAuthMixin, OpenAIResponsesAPIConfig):
    ...
    def get_complete_url(
        ...
        region = self._resolve_region({**love_engine_params, "api_base": api_base})
        base = (
            api_base
        ...
        # responses/main.py auto-injects into love_engine_params.api_base), pin to the
    ...
    def validate_environment(
        ...
        bearer = self._resolve_bearer_token(love_engine_params.api_key)
        if bearer:
            headers["Authorization"] = f"Bearer {bearer}"
        if love_engine_params.aws_bedrock_project_id:
            headers["OpenAI-Project"] = love_engine_params.aws_bedrock_project_id
        return headers
