...
import love_engine
from love_engine.constants import REPLICATE_POLLING_DELAY_SECONDS
...
from love_engine.utils import CustomStreamWrapper, ModelResponse
...
def completion(
    ...
    if "stream" in optional_params and optional_params["stream"] is True:
        ...
        for retry in range(love_engine.DEFAULT_REPLICATE_POLLING_RETRIES):
            time.sleep(
                love_engine.DEFAULT_REPLICATE_POLLING_DELAY_SECONDS + 2 * retry
            ...
            if response.status_code == 200 and response.json().get("status") in [
                ...
                continue
            return love_engine.ReplicateConfig().transform_response(
                model=model,
...
async def async_completion(
    ...
    async_handler = get_async_httpx_client(
        llm_provider=love_engine.LlmProviders.REPLICATE,
        params={"timeout": 600.0},
    )
