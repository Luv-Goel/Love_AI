...
from love_engine.love_engine_core_utils.core_helpers import map_finish_reason
from love_engine.love_engine_core_utils.prompt_templates.common_utils import unpack_legacy_defs
from love_engine.llms.base_llm.base_utils import type_to_response_format_param
from love_engine.llms.base_llm.chat.transformation import BaseConfig, BaseLLMException
...
from love_engine.types.utils import Message as LoveEngineMessage
from love_engine.types.utils import (
...
if TYPE_CHECKING:
    ...
    LoggingClass = LoveEngineLoggingObj
else:
    LoggingClass = Any
...
# Single, internal-only key on ``love_engine_params`` used to thread the per-
# request reverse map (sanitized -> original) from request build to response
# parsing. ``love_engine_params`` is never serialized to a provider; ``optional_
...
class AnthropicConfig(AnthropicModelInfo, BaseConfig):
    ...
    def _raise_invalid_reasoning_effort(
        ...
        Raises:
            love_engine.exceptions.BadRequestError: Always.
        """
        raise love_engine.exceptions.BadRequestError(
            message=(
    ...
    def filter_anthropic_output_schema(schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Related issues:
        - https://github.com/BerriAI/love_engine/issues/19444
        """
    ...
    def get_json_schema_from_pydantic_object(
        ...
        )  # Relevant issue: https://github.com/BerriAI/love_engine/issues/7755
    ...
    def _map_tool_helper(
        ...
        if tool["type"] == "function" or tool["type"] == "custom":
            ...
            if _input_schema.get("type") != "object":
                love_engine.verbose_logger.debug(
            ...
            # their backing def blocks (https://github.com/BerriAI/love_engine/issues/26692).
            ...
            from love_engine.types.llms.anthropic import AnthropicToolSearchToolRegex
            ...
            from love_engine.types.llms.anthropic import AnthropicToolSearchToolBM25
            ...
            from love_engine.types.llms.anthropic import AnthropicAdvisorTool
    ...
    def _map_openai_mcp_server_tool(
        ...
        from love_engine.types.llms.anthropic import AnthropicMcpServerToolConfiguration
    ...
    def _map_stop_sequences(
        ...
        if isinstance(stop, str):
            if (
                stop.isspace() and love_engine.drop_params is True
            ...
            for v in stop:
                if (
                    v.isspace() and love_engine.drop_params is True
                ):  # anthropic doesn't allow whitespace characters as stop-sequences
                    continue
                new_v.append(v)
            if len(new_v) > 0:
    ...
    def map_openai_params(
        ...
        #   2. the maps land on ``love_engine_params`` (internal), never on
    ...
    def transform_request(
        ...
        """
        ## VALIDATE REQUEST
        """Anthropic requires ``tools`` when messages include tool blocks; love_engine injects a dummy tool if omitted (no ``modify_params`` needed)."""
        ...
        if (
            ...
            if love_engine.modify_params:
                optional_params.pop("thinking", None)
                love_engine.verbose_logger.warning(
        ...
        #     ``love_engine_params`` (internal-only), never on ``optional_params``
        ...
        if _name_reverse_map and isinstance(love_engine_params, dict):
            love_engine_params[ANTHROPIC_TOOL_NAME_REVERSE_MAP_KEY] = _name_reverse_map
        ...
        ## Load Config
        config = love_engine.AnthropicConfig.get_config(model=model)
        ...
        _love_engine_metadata = love_engine_params.get("metadata", None)
        if (
            _love_engine_metadata
            and isinstance(_love_engine_metadata, dict)
            and "user_id" in _love_engine_metadata
            and _love_engine_metadata["user_id"] is not None
            and _valid_user_id(_love_engine_metadata["user_id"])
        ):
            optional_params["metadata"] = {"user_id": _love_engine_metadata["user_id"]}
        ...
        # Remove internal love_engine parameters that should not be sent to Anthropic API
        ...
        if top_k is not None:
            AnthropicConfig._apply_sampling_param(
                ...
                param="top_k",
                value=top_k,
                drop_params=love_engine_params.get("drop_params") is True,
                output_key="top_k",
            )
    ...
    def _apply_output_config(
        ...
        if not output_config or not isinstance(output_config, dict):
            return
        if love_engine.drop_params is True and not self._model_supports_effort_param(model):
            love_engine.verbose_logger.warning(
                DROP_UNSUPPORTED_OUTPUT_CONFIG_WARNING,
                model,
            )
    ...
    def transform_parsed_response(
        ...
        if (
            ...
            and not love_engine.disable_add_prefix_to_prompt
        ):
    ...
    def transform_response(
        ...
        if isinstance(love_engine_params, dict):
            _candidate = love_engine_params.get(ANTHROPIC_TOOL_NAME_REVERSE_MAP_KEY)
        ...
        try:
            if json_mode_content_str is not None:
                ...
                if (
                    ...
                    _message = love_engine.Message(content=json.dumps(values))
                    return _message
                else:
                    # a lot of the times the `values` key is not present in the tool response
                    # relevant issue: https://github.com/BerriAI/love_engine/issues/6741
                    _message = love_engine.Message(content=json.dumps(args))
                    return _message
        except json.JSONDecodeError:
            # json decode error does occur, return the original tool response str
            return love_engine.Message(content=json_mode_content_str)
        return None
