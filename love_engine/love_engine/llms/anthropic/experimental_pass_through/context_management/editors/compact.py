...
async def _check_summary_model_access(
    ...
    ``love_engine.proxy.auth.common_checks`` runs for the client-requested model:
...
async def _check_summary_model_budget(
    ...
    if user_api_key_auth is None:
        return True
    try:
        from love_engine.proxy.proxy_server import model_max_budget_limiter
    except Exception:
        return True
    ...
    if isinstance(model_max_budget, dict) and model_max_budget and token is not None:
        ...
        except love_engine.BudgetExceededError:
            return False
        except Exception as e:
            verbose_logger.warning(
...
async def _check_summary_model_rate_limit(
    ...
    post-call success hook (via the propagated ``love_engine_metadata``).
...
def _propagate_metadata(
    parent_love_engine_metadata: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    ...
    """
    if not parent_love_engine_metadata:
        return {}
    propagated: Dict[str, Any] = {}
    for key in _PROPAGATED_METADATA_KEYS:
        if key in parent_love_engine_metadata:
            propagated[key] = parent_love_engine_metadata[key]
    return propagated
...
def _count_effective_tokens(
    ...
    from love_engine.llms.anthropic.experimental_pass_through.adapters.transformation import (
        LoveEngineAnthropicMessagesAdapter,
    )

    messages_without_compaction = _strip_compaction_blocks(effective_messages)
    adapter = LoveEngineAnthropicMessagesAdapter()
    try:
    ...
    total = love_engine.token_counter(
        model=model,
    ...
    if compaction_block is not None:
        ...
        if content:
            total += love_engine.token_counter(model=model, text=content)
    system_text = _system_to_text(system)
    if system_text:
        total += love_engine.token_counter(model=model, text=system_text)
    return total
...
def _build_summary_messages(
    ...
    try:
        openai_messages = (
            LoveEngineAnthropicMessagesAdapter().translate_anthropic_messages_to_openai(
...
async def _call_summary_model(
    *,
    ...
    proxy's ``model_list``; falls back to ``love_engine.acompletion`` if no router
    ...
    # must travel as ``love_engine_metadata`` — that is the parameter the proxy's
    ...
    # than from ``love_engine_metadata``, so without it the summary tokens would not
    ...
    return await love_engine.acompletion(**call_kwargs)
...
async def apply_compact_20260112(
    *,
    ...
    propagated_metadata = _propagate_metadata(love_engine_metadata)
