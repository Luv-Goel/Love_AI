...
from love_engine.llms.anthropic.experimental_pass_through.adapters.transformation import (
    AnthropicAdapter,
)
from love_engine.llms.anthropic.experimental_pass_through.context_management import (
    AnthropicContextManagementError,
    PolyfillResult,
    apply_context_management,
)
from love_engine.llms.anthropic.experimental_pass_through.utils import (
...
from love_engine.utils import get_model_info
...
def _extract_proxy_love_engine_metadata(kwargs: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Return ``kwargs["love_engine_metadata"]`` when it's a dict; ``None`` otherwise.

    The proxy attaches its auth/spend-attribution fields (``user_api_key``,
    ``user_api_key_team_id``, ``love_engine_call_id``, the full ``UserAPIKeyAuth``
    object under ``user_api_key_auth``, ...) to ``data["love_engine_metadata"]``
    for ``/v1/messages`` (see
    ``LoveEngineProxyRequestSetup.add_user_api_key_auth_to_request_metadata`` and
    ``LOVE_ENGINE_METADATA_ROUTES``). The Anthropic-shape ``metadata`` arg only
    ...
    if not isinstance(love_engine_metadata, dict):
        return None
...
async def _prepare_context_managed_request(
    *,
    ...
    from love_engine.llms.anthropic.experimental_pass_through.context_management.editors.compact import (
...
def _polyfill_will_run(
    *,
    ...
    from love_engine.llms.anthropic.experimental_pass_through.context_management.constants import (
        COMPACT_EDIT_TYPE,
    )
...
def _normalize_spec_edits(
    *,
    ...
    effective_drop_params = (
        drop_params if drop_params is not None else love_engine.drop_params
    )
    if effective_drop_params:
        return None

    from love_engine.llms.anthropic.experimental_pass_through.context_management.dispatcher import (
        _normalize_spec,
    )
...
class LoveEngineMessagesToCompletionTransformationHandler:
    @staticmethod
    def _route_openai_thinking_to_responses_api_if_needed(
        ...
        *,
        ...
        """
        When users call `love_engine.anthropic.messages.*` with a non-Anthropic model and
        `thinking={"type": "enabled", ...}`, love_engine converts this into OpenAI
        `reasoning_effort`.
        ...
        if custom_llm_provider is None:
            try:
                _, inferred_provider, _, _ = love_engine.utils.get_llm_provider(
    ...
    def _prepare_completion_kwargs(
        *,
        ...
        """Prepare kwargs for love_engine.completion/acompletion.
        ...
        for key, value in extra_kwargs.items():
            if (
                key == "love_engine_logging_obj"
                and value is not None
                and isinstance(value, LoveEngineLoggingObject)
            ):
        ...
        LoveEngineMessagesToCompletionTransformationHandler._normalize_reasoning_effort(
            completion_kwargs
        )

        LoveEngineMessagesToCompletionTransformationHandler._route_openai_thinking_to_responses_api_if_needed(
            completion_kwargs,
            thinking=thinking,
        )
    ...
    async def async_anthropic_messages_handler(
        ...
        **kwargs,
        ...
        love_engine_router = kwargs.pop("love_engine_router", None)
        if love_engine_router is None:
            try:
                from love_engine.proxy.proxy_server import llm_router as _proxy_router

                love_engine_router = _proxy_router
            except Exception:
                pass

        proxy_love_engine_metadata = _extract_proxy_love_engine_metadata(kwargs)
        user_api_key_auth = (
            proxy_love_engine_metadata.get("user_api_key_auth")
            if proxy_love_engine_metadata is not None
            else None
        )
        ...
        polyfill_result = await _prepare_context_managed_request(
            ...
            love_engine_metadata=proxy_love_engine_metadata,
            drop_params=drop_params,
            llm_router=love_engine_router,
            user_api_key_auth=user_api_key_auth,
        )
        ...
        (
            completion_kwargs,
            tool_name_mapping,
        ) = LoveEngineMessagesToCompletionTransformationHandler._prepare_completion_kwargs(
            max_tokens=max_tokens,
        ...
        completion_response = await love_engine.acompletion(**completion_kwargs)
    ...
    def anthropic_messages_handler(
        ...
        **kwargs,
        ...
        if _is_async is True:
            return LoveEngineMessagesToCompletionTransformationHandler.async_anthropic_messages_handler(
                max_tokens=max_tokens,
                messages=messages,
                model=model,
                metadata=metadata,
        ...
        # ``love_engine.messages.create()`` callers don't silently lose edits like
        ...
        # ``love_engine.acompletion`` (which creates a fresh client per call) when
        ...
        # majority of sync ``love_engine.messages.create()`` requests carry no
        ...
        completion_response = love_engine.completion(**completion_kwargs)
