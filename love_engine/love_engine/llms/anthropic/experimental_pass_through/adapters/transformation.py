...
from love_engine.types.utils import Choices, ModelResponse, StreamingChoices, Usage

from .streaming_iterator import AnthropicStreamWrapper

if TYPE_CHECKING:
    from love_engine.types.llms.anthropic import ContentBlockContentBlockDict
...
class AnthropicAdapter:
    ...
    def translate_completion_input_params_with_tool_mapping(
        ...
        (
            translated_body,
            tool_name_mapping,
        ) = LoveEngineAnthropicMessagesAdapter().translate_anthropic_to_openai(
    ...
    def translate_completion_output_params(
        ...
        """
        return LoveEngineAnthropicMessagesAdapter().translate_openai_response_to_anthropic(
            response=response,
    ...
    def translate_completion_output_params_streaming(
        ...
        Args:
            ...
            is_async: When ``True`` (default, for back-compat with existing
                ...
                (e.g. ``love_engine.anthropic.messages.create(stream=True)`` via
...
class LoveEngineAnthropicMessagesAdapter:
    def __init__(self):
        pass
    ...
    def translate_thinking_for_model(
        ...
        """
        if LoveEngineAnthropicMessagesAdapter.is_anthropic_claude_model(model):
            return {"thinking": thinking}
        else:
            reasoning_effort = LoveEngineAnthropicMessagesAdapter.translate_anthropic_thinking_to_reasoning_effort(
                thinking
            )
            if reasoning_effort:
                summary = (
    ...
    def translate_anthropic_tools_to_openai(
        ...
        for idx, tool in enumerate(tools):
            ...
            if raw_name is None or (
                ...
                original_name = f"love_engine_unnamed_tool_{idx}"
            else:
    ...
    def _add_additional_properties_false(schema: dict) -> None:
        ...
        if schema.get("type") == "object" and "properties" in schema:
            ...
            for prop in schema["properties"].values():
                LoveEngineAnthropicMessagesAdapter._add_additional_properties_false(prop)

        # Handle array items
        if "items" in schema:
            LoveEngineAnthropicMessagesAdapter._add_additional_properties_false(
                schema["items"]
            )
    ...
    def _translate_metadata_to_openai(
        ...
        if "love_engine_metadata" in anthropic_message_request:
            # metadata will be passed to love_engine.acompletion(), it's a love_engine_param
            new_kwargs["metadata"] = anthropic_message_request.pop("love_engine_metadata")
    ...
    def translate_anthropic_to_openai(
        ...
        ## CONVERT METADATA (user_id + love_engine metadata)
    ...
    def _translate_streaming_openai_chunk_to_anthropic_content_block(
        ...
        from love_engine.types.llms.anthropic import TextBlock
    ...
    def translate_streaming_openai_response_to_anthropic(
        ...
        if response.choices[0].finish_reason is not None:
            ...
            if getattr(response, "usage", None) is not None:
                love_engine_usage_chunk: Optional[Usage] = response.usage  # type: ignore
            elif (
                hasattr(response, "_hidden_params")
                and "usage" in response._hidden_params
            ):
                love_engine_usage_chunk = response._hidden_params["usage"]
            else:
                love_engine_usage_chunk = None
            if love_engine_usage_chunk is not None:
                uncached_input_tokens = love_engine_usage_chunk.prompt_tokens or 0
                cached_tokens = 0
                if (
                    hasattr(love_engine_usage_chunk, "prompt_tokens_details")
                    and love_engine_usage_chunk.prompt_tokens_details
                ):
                    cached_tokens = (
                        getattr(
                            love_engine_usage_chunk.prompt_tokens_details,
                            "cached_tokens",
                            0,
                        )
                        or 0
                ...
                usage_delta = UsageDelta(
                    input_tokens=uncached_input_tokens,
                    output_tokens=love_engine_usage_chunk.completion_tokens or 0,
                )
                if (
                    hasattr(love_engine_usage_chunk, "_cache_creation_input_tokens")
                    and love_engine_usage_chunk._cache_creation_input_tokens > 0
                ):
                    usage_delta["cache_creation_input_tokens"] = (
                        love_engine_usage_chunk._cache_creation_input_tokens
                    )
                if cached_tokens > 0:
