...
from love_engine.llms.anthropic.common_utils import (
...
from love_engine.types.llms.anthropic_messages.anthropic_request import AnthropicMetadata
...
from ..adapters.handler import LoveEngineMessagesToCompletionTransformationHandler
from ..responses_adapters.handler import LoveEngineMessagesToResponsesAPIHandler
...
def _should_route_to_responses_api(custom_llm_provider: Optional[str]) -> bool:
    ...
    Set ``love_engine.use_chat_completions_url_for_anthropic_messages = True`` to
    opt out and route OpenAI/Azure requests through chat/completions instead.
    """
    if love_engine.use_chat_completions_url_for_anthropic_messages:
        return False
...
async def _execute_pre_request_hooks(
    ...
    **kwargs,
    ...
    if not love_engine.callbacks:
        return request_kwargs

    from love_engine.integrations.custom_logger import CustomLogger as _CustomLogger
...
async def anthropic_messages(
    ...
    **kwargs,
    ...
    # The love_engine_params dict may have been overwritten by **kwargs in
    # _execute_pre_request_hooks, so fall back to get_llm_provider() if needed.
    if not custom_llm_provider:
        custom_llm_provider = request_kwargs.get("love_engine_params", {}).get(
            "custom_llm_provider"
        )
        if not custom_llm_provider:
            try:
                ...
                pass
    # Remove love_engine_params from kwargs (only needed for hooks)
    request_kwargs.pop("love_engine_params", None)
    ...
    func = partial(
        ...
        _love_engine_messages_presanitized=True,
        **kwargs,
    )
...
def validate_anthropic_api_metadata(metadata: Optional[Dict] = None) -> Optional[Dict]:
    ...
    If there are any love_engine specific metadata fields, use `love_engine_metadata` key to pass them.
    """
    if metadata is None:
        return None
...
def anthropic_messages_handler(
    ...
    **kwargs,
    ...
    # (love_engine.messages.create -> anthropic_messages_handler) gets the same
    ...
    # ``_love_engine_messages_presanitized`` to skip this redundant second
    # full-messages scan. Pop it so it never leaks into provider params.
    if not kwargs.pop("_love_engine_messages_presanitized", False):
        messages = strip_empty_text_blocks_from_anthropic_messages(messages)
    ...
    if love_engine_logging_obj is not None:
        love_engine_logging_obj.model_call_details["agentic_loop_params"] = {
    ...
    if love_engine_params.mock_response and isinstance(love_engine_params.mock_response, str):
    ...
    if anthropic_messages_provider_config is None:
        ...
        if _should_route_to_responses_api(custom_llm_provider):
            return LoveEngineMessagesToResponsesAPIHandler.anthropic_messages_handler(
                **_shared_kwargs
            )
        ...
        return (
            LoveEngineMessagesToCompletionTransformationHandler.anthropic_messages_handler(
                **_shared_kwargs,
            )
        )
