...
import love_engine
from love_engine.types.llms.anthropic import AnthropicMessagesRequest
...
from .transformation import LoveEngineAnthropicToResponsesAPIAdapter

_ADAPTER = LoveEngineAnthropicToResponsesAPIAdapter()
...
def _build_responses_kwargs(
    *,
    ...
    """
    Build the kwargs dict to pass directly to love_engine.responses() / love_engine.aresponses().
    """
    ...
    # Forward love_engine-specific kwargs (api_key, api_base, logging obj, etc.)
    excluded = {"anthropic_messages"}
    for key, value in (extra_kwargs or {}).items():
        if key == "love_engine_logging_obj" and value is not None:
            ...
            if isinstance(value, LoveEngineLoggingObject):
...
class LoveEngineMessagesToResponsesAPIHandler:
    """
    Handles Anthropic /v1/messages requests for OpenAI / Azure models by
    calling love_engine.responses() / love_engine.aresponses() directly and translating
    ...
    async def async_anthropic_messages_handler(
        ...
        **kwargs,
        ...
        result = await love_engine.aresponses(**responses_kwargs)
    ...
    def anthropic_messages_handler(
        ...
        **kwargs,
        ...
        if _is_async:
            return (
                LoveEngineMessagesToResponsesAPIHandler.async_anthropic_messages_handler(
                    max_tokens=max_tokens,
                    messages=messages,
                    model=model,
                    context_management=context_management,
        ...
        result = love_engine.responses(**responses_kwargs)
