...
from love_engine.utils import _cached_get_model_info_helper
...
def github_copilot_supports_responses_api(model: str) -> bool:
    ...
    key = info.get("key")
    raw_info = love_engine.model_cost.get(key) if isinstance(key, str) else None
    endpoints = (
...
class GithubCopilotResponsesAPIConfig(OpenAIResponsesAPIConfig):
    ...
    def validate_environment(
        ...
        try:
            ...
            input_param = self._get_input_from_params(love_engine_params)
    ...
    def _get_input_from_params(
        self, love_engine_params: Optional[GenericLoveEngineParams]
    ) -> Optional[Union[str, ResponseInputParam]]:
        """
        Extract input parameter from love_engine_params.
        ...
        # Try to get input from love_engine_params
        # This might be in different locations depending on how love_engine structures it
        if hasattr(love_engine_params, "input"):
            return love_engine_params.input
