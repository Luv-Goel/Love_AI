...
from love_engine.types.llms.ollama import (
...
class OllamaChatConfig(BaseConfig):
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            ...
            # Fixes: https://github.com/BerriAI/love_engine/issues/18922
            if param == "tools":
    ...
    def transform_request(
        ...
        love_engine_params["function_name"] = function_name
        ...
        for m in messages:
            if isinstance(
                m, BaseModel
            ):  # avoid message serialization issues - https://github.com/BerriAI/love_engine/issues/5319
    ...
    def transform_response(
        ...
        if (
            ...
            and love_engine_params.get("function_name") is not None
        ):
            function_call = json.loads(response_json_message["content"])
            message = love_engine.Message(
                content=None,
                tool_calls=[
                    {
                        ...
                        "function": {
                            "name": function_call.get(
                                "name", love_engine_params.get("function_name")
                            ),
            ...
            _message = love_engine.Message(**response_json_message)
        ...
        prompt_tokens = response_json.get("prompt_eval_count", love_engine.token_counter(messages=messages))  # type: ignore
        completion_tokens = response_json.get(
            "eval_count",
            love_engine.token_counter(text=response_json["message"]["content"]),
        )
        setattr(
            model_response,
            "usage",
