...
class RAGFlowVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def get_auth_credentials(
        ...
        if api_key is None:
            raise ValueError(
                "api_key is required (set RAGFLOW_API_KEY env var or pass in love_engine_params)"
            )
        return {
            "headers": {
    ...
    def validate_environment(
        ...
        api_key = love_engine_params.api_key or get_secret_str("RAGFLOW_API_KEY")

        if api_key is None:
            raise ValueError(
                "RAGFLOW_API_KEY is required (set env var or pass in love_engine_params)"
            )
    ...
    def get_complete_url(
        ...
        - api_base in love_engine_params
        - Default: http://localhost:9380
        """
        api_base = (
            api_base
            or love_engine_params.get("api_base")
    ...
    def transform_create_vector_store_request(
        ...
        Maps love_engine params to RAGFlow dataset creation parameters.
