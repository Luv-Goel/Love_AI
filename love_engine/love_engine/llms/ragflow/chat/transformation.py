...
class RAGFlowConfig(OpenAIConfig):
    ...
    def get_complete_url(
        ...
        Args:
            ...
            love_engine_params: love_engine parameters (may contain api_base)
            stream: Whether streaming is enabled

        Returns:
            Complete URL for the API call
        """
        # Get api_base from multiple sources: input param, love_engine_params, environment, or global love_engine setting
        if (
            love_engine_params
            and hasattr(love_engine_params, "api_base")
            and love_engine_params.api_base
        ):
            api_base = api_base or love_engine_params.api_base
        ...
        if api_base is None:
            raise ValueError(
                "api_base is required for RAGFlow provider. Set it via api_base parameter, RAGFLOW_API_BASE environment variable, or love_engine.api_base"
            )
    ...
    def _get_openai_compatible_provider_info(
        ...
        # The model name will be stored in love_engine_params for use in requests
        _, _, actual_model = self._parse_ragflow_model(model)

        # Get api_base from multiple sources: input param, environment, or global love_engine setting
        dynamic_api_base = (
            api_base
            or love_engine.api_base
            or get_secret("RAGFLOW_API_BASE")
        ...
        # Get api_key from multiple sources: input param, environment, or global love_engine setting
        dynamic_api_key = (
            api_key or love_engine.api_key or get_secret_str("RAGFLOW_API_KEY")
        )
    ...
    def validate_environment(
        ...
        Args:
            ...
            optional_params: Optional parameters
            love_engine_params: love_engine parameters (may contain api_key)
            api_key: API key (from input params)
        ...
        """
        # Use api_key from love_engine_params if available, otherwise fall back to other sources
        if (
            love_engine_params
            and hasattr(love_engine_params, "api_key")
            and love_engine_params.api_key
        ):
            api_key = api_key or love_engine_params.api_key

        # Get api_key from multiple sources: input param, love_engine_params, environment, or global love_engine setting
        api_key = api_key or love_engine.api_key or get_secret_str("RAGFLOW_API_KEY")
        ...
        try:
            _, _, actual_model = self._parse_ragflow_model(model)
            # Store the actual model name in love_engine_params for use in transform_request
            love_engine_params["_ragflow_actual_model"] = actual_model
        except ValueError:
    ...
    def transform_request(
        ...
        Args:
            ...
            messages: Chat messages
            optional_params: Optional parameters
            love_engine_params: love_engine parameters (may contain _ragflow_actual_model)
            headers: Request headers

        Returns:
            Transformed request dictionary
        """
        # Get the actual model name from love_engine_params if available
        actual_model = love_engine_params.get("_ragflow_actual_model")
        ...
        return super().transform_request(
            actual_model, messages, optional_params, love_engine_params, headers
        )
