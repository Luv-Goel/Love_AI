...
class SagemakerLLM(BaseAWSLLM):
    ...
    def completion(
        ...
        ## Load Config
        config = love_engine.SagemakerConfig.get_config()
        ...
        try:
            ...
            if "Inference Component Name header is required" in error_message:
                error_message += "\n pass in via `love_engine.completion(..., model_id={InferenceComponentName})`"
        ...
        try:
            if client is None:
                client = get_async_httpx_client(
                    llm_provider=love_engine.LlmProviders.SAGEMAKER
    ...
    async def async_streaming(
        ...
        if model_id is not None:  # Fixes https://github.com/BerriAI/love_engine/issues/8889
            prepared_request.headers.update(
                {"X-Amzn-SageMaker-Inference-Component": model_id}
            )
