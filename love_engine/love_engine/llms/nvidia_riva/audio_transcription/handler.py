...
This module bridges love_engine's transcription dispatch to NVIDIA Riva's gRPC
...
``riva-client`` is imported lazily so ``love_engine`` core remains usable
...
from love_engine.llms.nvidia_riva.audio_transcription.audio_utils import (
...
from love_engine.llms.nvidia_riva.common_utils import (
    NvidiaRivaException,
    grpc_error_to_love_engine_exception,
)
from love_engine.types.utils import FileTypes, TranscriptionResponse
...
_RIVA_INSTALL_HINT = (
    ...
    "Install with `pip install 'love_engine[stt-nvidia-riva]'`."
)
...
class NvidiaRivaAudioTranscription:
    ...
    def _run_sync(
        ...
        if not api_base:
            raise NvidiaRivaException(
                ...
                message=(
                    ...
                    "`localhost:50051`). Set it in love_engine_params or via "
        ...
        try:
            ...
            raise grpc_error_to_love_engine_exception(e) from e
