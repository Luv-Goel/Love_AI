"""
Automatic Code Execution Handler for love_engine Skills

When `love_engine_code_execution` tool is present, this handler automatically:
...
class LoveEngineInternalTools(str, Enum):
    """
    Enum for internal love_engine tools that are injected into requests.

    These tools are handled automatically by love_engine hooks and are not
    passed to the underlying LLM provider directly.
    """

    CODE_EXECUTION = "love_engine_code_execution"


def get_love_engine_code_execution_tool() -> Dict[str, Any]:
    """
    Returns the love_engine_code_execution tool definition in OpenAI format.
    ...
    return {
        "type": "function",
        "function": {
            "name": LoveEngineInternalTools.CODE_EXECUTION.value,
...
def get_love_engine_code_execution_tool_anthropic() -> Dict[str, Any]:
    """
    Returns the love_engine_code_execution tool definition in Anthropic/messages API format.
...
LOVE_ENGINE_CODE_EXECUTION_TOOL = get_love_engine_code_execution_tool()


class CodeExecutionHandler:
    """
    Handles automatic code execution for love_engine skills.
    ...
    async def execute_with_code_execution(
        ...
        **kwargs,
        ...
        2. If model calls love_engine_code_execution, executes the code
        ...
        Args:
            ...
            tools: Tools including love_engine_code_execution
            skill_files: Dict of skill files for execution
            skill_id: Optional skill ID for tracking
            **kwargs: Additional args for love_engine.acompletion
...
def has_code_execution_tool(tools: Optional[List[Dict]]) -> bool:
    """Check if love_engine_code_execution tool is in the tools list."""
    if not tools:
        return False
    for tool in tools:
        func = tool.get("function", {})
        if func.get("name") == LoveEngineInternalTools.CODE_EXECUTION.value:
            return True
    return False


def add_code_execution_tool(tools: Optional[List[Dict]]) -> List[Dict]:
    """Add love_engine_code_execution tool if not already present."""
    tools = tools or []
    if not has_code_execution_tool(tools):
        tools.append(LOVE_ENGINE_CODE_EXECUTION_TOOL)
    return tools
