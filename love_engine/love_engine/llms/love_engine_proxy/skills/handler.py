"""
Handler for love_engine database-backed skills operations.
...
from love_engine.proxy._types import LOVE_ENGINE_SkillsTable, NewSkillRequest, UserAPIKeyAuth
from love_engine.proxy.common_utils.resource_ownership import (
    get_primary_resource_owner_scope,
    get_resource_owner_scopes,
    is_proxy_admin,
    user_can_access_resource_owner,
)
from love_engine.repositories.table_repositories import SkillsRepository
...
_NEGATIVE_SKILL_SENTINEL = "__love_engine_skill_not_found__"
_SKILL_CACHE = InMemoryCache(max_size_in_memory=10000, default_ttl=60)


def _prisma_skill_to_love_engine(prisma_skill) -> LOVE_ENGINE_SkillsTable:
    """Convert a Prisma skill record to LOVE_ENGINE_SkillsTable.
    ...
    return LOVE_ENGINE_SkillsTable(**data)


class LoveEngineSkillsHandler:
    """CRUD for skills stored in ``love_engine_skillstable``."""
    ...
    async def _get_prisma_client():
        ...
        if prisma_client is None:
            raise ValueError(
                ...
                "Database connection required for love_engine skills."
            )
    ...
    async def create_skill(
        ...
        prisma_client = await LoveEngineSkillsHandler._get_prisma_client()

        skill_id = f"{LOVE_ENGINE_SKILL_ID_PREFIX}{uuid.uuid4()}"
        ...
        verbose_logger.debug(
            f"LoveEngineSkillsHandler: Creating skill {skill_id} with title={data.display_title}"
        )

        new_skill = await SkillsRepository(prisma_client).table.create(data=skill_data)
        return _prisma_skill_to_love_engine(new_skill)
    ...
    async def list_skills(
        ...
        offset: int = 0,
        user_api_key_dict: Optional[UserAPIKeyAuth] = None,
    ) -> List[LOVE_ENGINE_SkillsTable]:
        prisma_client = await LoveEngineSkillsHandler._get_prisma_client()

        verbose_logger.debug(
            f"LoveEngineSkillsHandler: Listing skills with limit={limit}, offset={offset}"
        )
        ...
        return [_prisma_skill_to_love_engine(s) for s in skills]
    ...
    async def get_skill(
        ...
        verbose_logger.debug(f"LoveEngineSkillsHandler: Getting skill {skill_id}")

        skill = await LoveEngineSkillsHandler._load_skill(skill_id)
        ...
        return _prisma_skill_to_love_engine(skill)
    ...
    async def delete_skill(
        ...
        prisma_client = await LoveEngineSkillsHandler._get_prisma_client()
        verbose_logger.debug(f"LoveEngineSkillsHandler: Deleting skill {skill_id}")
    ...
    async def fetch_skill_from_db(
        ...
        try:
            return await LoveEngineSkillsHandler.get_skill(
            ...
            verbose_logger.warning(
                f"LoveEngineSkillsHandler: Error fetching skill {skill_id}: {e}"
            )
            return None
