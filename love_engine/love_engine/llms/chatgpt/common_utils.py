...
def _get_love_engine_version() -> str:
    try:
...
def get_chatgpt_user_agent(originator: str) -> str:
    ...
    version = _get_love_engine_version()
...
def _normalize_love_engine_params(love_engine_params: Optional[Any]) -> dict:
    ...
    if hasattr(love_engine_params, "model_dump"):
        try:
            return love_engine_params.model_dump()
        except Exception:
            return {}
    if hasattr(love_engine_params, "dict"):
        try:
            return love_engine_params.dict()
        except Exception:
            return {}
    return {}


def get_chatgpt_session_id(love_engine_params: Optional[Any]) -> Optional[str]:
    params = _normalize_love_engine_params(love_engine_params)
    for key in ("love_engine_session_id", "session_id"):
    ...
    if isinstance(metadata, dict):
        ...
        if value:
            return str(value)
    for key in ("love_engine_trace_id", "love_engine_call_id"):
        value = params.get(key)
        if value:
            return str(value)
    return None
...
def ensure_chatgpt_session_id(love_engine_params: Optional[Any]) -> str:
    return get_chatgpt_session_id(love_engine_params) or str(uuid4())
