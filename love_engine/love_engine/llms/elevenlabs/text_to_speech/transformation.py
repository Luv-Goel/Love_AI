...
class ElevenLabsTextToSpeechConfig(BaseTextToSpeechConfig):
    ...
    def _resolve_voice_id(
        ...
        if mapped_voice is None:
            raise ValueError(
                "ElevenLabs voice_id is required. Pass `voice` when calling `love_engine.speech()`."
            )
    ...
    def _add_elevenlabs_specific_params(
        ...
        reserved_kwarg_keys = set(all_love_engine_params) | {
    ...
    def get_complete_url(
        ...
        voice_id = love_engine_params.get(self.ELEVENLABS_VOICE_ID_KEY)
        ...
        query_params = love_engine_params.get(self.ELEVENLABS_QUERY_PARAMS_KEY, {})
        if query_params:
