...
class AzureFoundryModelInfo(BaseLLMModelInfo):
    ...
    def get_api_base(api_base: Optional[str] = None) -> Optional[str]:
        return api_base or love_engine.api_base or get_secret_str("AZURE_AI_API_BASE")
    ...
    def get_azure_ai_config_for_model(model: str):
        ...
        if azure_ai_route == "model_router":
            from love_engine.llms.azure_ai.azure_model_router.transformation import (
                AzureModelRouterConfig,
            )

            return AzureModelRouterConfig()
        elif "claude" in model.lower():
            from love_engine.llms.azure_ai.anthropic.transformation import (
