...
class AzureAnthropicConfig(AnthropicConfig):
    ...
    def validate_environment(
        ...
        love_engine_params: Union[dict, GenericLoveEngineParams],
        ...
        # Convert dict to GenericLoveEngineParams if needed
        if isinstance(love_engine_params, dict):
            # Ensure api_key is included if provided
            if api_key and "api_key" not in love_engine_params:
                love_engine_params = {**love_engine_params, "api_key": api_key}
            love_engine_params_obj = GenericLoveEngineParams(**love_engine_params)
        else:
            love_engine_params_obj = love_engine_params or GenericLoveEngineParams()
            # Set api_key if provided and not already set
            if api_key and not love_engine_params_obj.api_key:
                love_engine_params_obj.api_key = api_key
