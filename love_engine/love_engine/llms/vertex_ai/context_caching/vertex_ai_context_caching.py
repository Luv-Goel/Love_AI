...
import love_engine
from love_engine.caching.caching import Cache, LoveEngineCacheType
from love_engine.constants import MINIMUM_PROMPT_CACHE_TOKEN_COUNT
...
from love_engine.llms.openai.openai import AllMessageValues
...
local_cache_obj = Cache(
    type=LoveEngineCacheType.LOCAL
    ...
    async def async_check_and_create_cache(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(
                params={"timeout": timeout}, llm_provider=love_engine.LlmProviders.VERTEX_AI
            )
        else:
            client = client
