...
from love_engine.types.llms.anthropic import AnthropicThinkingParam
from love_engine.types.llms.gemini import BidiGenerateContentServerMessage
...
from love_engine.types.llms.vertex_ai import (
...
if TYPE_CHECKING:
    ...
    from love_engine.types.utils import ModelResponseStream, StreamingChoices
...
class VertexGeminiConfig(VertexAIBaseConfig, BaseConfig):
    ...
    def get_json_schema_from_pydantic_object(
        ...
        See: https://github.com/BerriAI/love_engine/issues/21014
        """
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            ...
            raise love_engine.utils.UnsupportedParamsError(
                message="VertexAI doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `love_engine.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def _transform_computer_use_config(self, computer_use_config: dict) -> dict:
        ...
        Args:
            computer_use_config: The computer use configuration from LoveEngine
    ...
    def _extract_google_maps_retrieval_config(
        ...
        Args:
            google_maps_config: The googleMaps tool configuration from LoveEngine
    ...
    def _resolve_search_tool_conflict(
        ...
        Ref: https://github.com/BerriAI/love_engine/issues/23337
    ...
    def _map_function(self, value: List[dict], optional_params: dict) -> List[Tools]:
        ...
        for tool in value:
            ...
            # Handle tools with 'type' field (OpenAI spec compliance) Ignore this field -> https://github.com/BerriAI/love_engine/issues/14644#issuecomment-3342061838
            elif "type" in tool:
            ...
            if tool_name and (
                ...
                verbose_logger.warning(
                    "Invalid tool={}. Use `love_engine.set_verbose` or `love_engine --detailed_debug` to see raw request."
                )
        ...
        if model and VertexGeminiConfig._is_gemini_3_or_newer(model):
            if thinking_enabled:
                if thinking_budget is None or thinking_budget == 0:
                    ...
                    if love_engine.enable_gemini_default_thinking_level_low is True:
                        is_gemini3flash = (
        ...
        if num_function_declarations > 1:
            raise love_engine.utils.UnsupportedParamsError(
                message=(
                    ...
                    "`parallel_tool_calls=True`. If you want to drop this param, set `love_engine.drop_params = True` or pass in `(.., drop_params=True)` in the requst - https://docs.love_engine.ai/docs/completion/drop_params"
                ),
                status_code=400,
            )
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            if param == "temperature":
                ...
                optional_params["include_server_side_tool_invocations"] = True
        if love_engine.vertex_ai_safety_settings is not None:
            optional_params["safety_settings"] = love_engine.vertex_ai_safety_settings
    ...
    def get_finish_reason_mapping() -> Dict[str, OpenAIChatCompletionFinishReason]:
        ...
        """
        from love_engine.love_engine_core_utils.core_helpers import _FINISH_REASON_MAP
    ...
    def is_candidate_token_count_inclusive(usage_metadata: UsageMetadata) -> bool:
        ...
        Addresses - https://github.com/BerriAI/love_engine/pull/10141#discussion_r2052272035
        """
    ...
    def _calculate_usage(
        ...
        ## See: https://github.com/BerriAI/love_engine/issues/18750
        if cached_text_tokens is not None and prompt_text_tokens is not None:
            ...
            # See: https://github.com/BerriAI/love_engine/issues/16341
    ...
    def _check_prompt_level_content_filter(
        ...
        """
        from love_engine.types.utils import Delta, ModelResponseStream, StreamingChoices
    ...
    def _create_streaming_choice(
        ...
        """
        from love_engine.types.utils import Delta, StreamingChoices
    ...
    def transform_response(
        ...
        try:
            ...
            raise VertexAIError(
                message="Error converting to valid response block={}. File an issue if love_engine error - https://github.com/BerriAI/love_engine/issues".format(
                    str(e)
                ),
                status_code=422,
...
async def make_call(
    ...
    if client is None:
        client = get_async_httpx_client(
            llm_provider=love_engine.LlmProviders.VERTEX_AI,
        )
...
class VertexLLM(VertexBase):
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(
                params=_async_client_params, llm_provider=love_engine.LlmProviders.VERTEX_AI
            )
        else:
...
class ModelResponseIterator:
    ...
    def handle_valid_json_chunk(self, chunk: str) -> Optional["ModelResponseStream"]:
        ...
        try:
            ...
            # See: https://github.com/BerriAI/love_engine/issues/16562
    ...
    def handle_accumulated_json_chunk(
        ...
        chunk = love_engine.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or ""
