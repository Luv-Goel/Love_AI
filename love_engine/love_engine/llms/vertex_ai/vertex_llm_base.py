...
from love_engine.types.llms.vertex_ai import VERTEX_CREDENTIALS_TYPES, VertexPartnerProvider
...
GOOGLE_IMPORT_ERROR_MESSAGE = (
    "Google Cloud SDK not found. Install it with: pip install 'love_engine[google]' "
...
class VertexBase:
        ...
        if credentials is not None:
            if isinstance(credentials, str):
                _is_path = os.path.exists(
                    credentials
                )  # credentials is from server config (love_engine_params), not user input
            ...
            if "type" in json_obj and json_obj["type"] == "external_account":
                ...
                if isinstance(environment_id, str) and "aws" in environment_id:
                    ...
                    from love_engine.llms.vertex_ai.vertex_ai_aws_wif import (
                        VertexAIAwsWifAuth,
                    )
    ...
    def _check_custom_proxy(
        ...
        """
        for cloudflare ai gateway - https://github.com/BerriAI/love_engine/issues/4317
    ...
    @staticmethod
    def get_vertex_ai_project(love_engine_params: dict) -> Optional[str]:
        return (
            love_engine_params.pop("vertex_project", None)
            or love_engine_params.pop("vertex_ai_project", None)
            or love_engine.vertex_project
            or get_secret_str("VERTEXAI_PROJECT")
        )

    @staticmethod
    def get_vertex_ai_credentials(love_engine_params: dict) -> Optional[str]:
        return (
            love_engine_params.pop("vertex_credentials", None)
            or love_engine_params.pop("vertex_ai_credentials", None)
            or get_secret_str("VERTEXAI_CREDENTIALS")
        )

    @staticmethod
    def get_vertex_ai_location(love_engine_params: dict) -> Optional[str]:
        return (
            love_engine_params.pop("vertex_location", None)
            or love_engine_params.pop("vertex_ai_location", None)
    ...
    @staticmethod
    def safe_get_vertex_ai_project(love_engine_params: dict) -> Optional[str]:
        """
        Safely get Vertex AI project without mutating the love_engine_params dict.

        Unlike get_vertex_ai_project(), this does NOT pop values from the dict,
        making it safe to call multiple times with the same love_engine_params.

        Args:
            love_engine_params: Dictionary containing Vertex AI parameters

        Returns:
            Vertex AI project ID or None
        """
        return (
            love_engine_params.get("vertex_project")
            or love_engine_params.get("vertex_ai_project")
            or love_engine.vertex_project
            or get_secret_str("VERTEXAI_PROJECT")
        )

    @staticmethod
    def safe_get_vertex_ai_credentials(love_engine_params: dict) -> Optional[str]:
        """
        Safely get Vertex AI credentials without mutating the love_engine_params dict.
        ...
        """
        return (
            love_engine_params.get("vertex_credentials")
            or love_engine_params.get("vertex_ai_credentials")
            or get_secret_str("VERTEXAI_CREDENTIALS")
        )

    @staticmethod
    def safe_get_vertex_ai_location(love_engine_params: dict) -> Optional[str]:
        """
        Safely get Vertex AI location without mutating the love_engine_params dict.
        ...
        """
        return (
            love_engine_params.get("vertex_location")
            or love_engine_params.get("vertex_ai_location")
            or love_engine.vertex_location
            or get_secret_str("VERTEXAI_LOCATION")
            or get_secret_str("VERTEX_LOCATION")
        )
