...
Handles the transformation between LoveEngine's OpenAI-compatible format and
...
from love_engine.llms.vertex_ai.agent_engine.sse_iterator import (
    VertexAgentEngineResponseIterator,
)
from love_engine.llms.vertex_ai.common_utils import get_vertex_base_url
...
from love_engine.types.utils import Choices, Message, ModelResponse, Usage
...
class VertexAgentEngineConfig(BaseConfig, VertexBase):
    ...
    def get_complete_url(
        ...
        # Get project and location from love_engine_params or environment
        vertex_project = self.safe_get_vertex_ai_project(love_engine_params)
        vertex_location = (
            self.safe_get_vertex_ai_location(love_engine_params) or "us-central1"
        )
        ...
        if not resource_path.startswith("projects/"):
            if not vertex_project:
                raise ValueError(
                    ...
                    "Set via love_engine_params['vertex_project'] or VERTEXAI_PROJECT env var."
                )
    ...
    def _get_auth_headers(
        ...
        vertex_credentials = self.safe_get_vertex_ai_credentials(love_engine_params)
        vertex_project = self.safe_get_vertex_ai_project(love_engine_params)
    ...
    def _get_user_id(self, optional_params: dict) -> str:
        ...
        if user_id:
            return user_id
        # Generate a user ID
        return f"love_engine-user-{str(uuid.uuid4())[:8]}"
    ...
    def validate_environment(
        ...
        auth_headers = self._get_auth_headers(optional_params, love_engine_params)
    ...
    def _calculate_usage(
        ...
        """Calculate token usage using LoveEngine's token counter."""
        try:
    ...
    def transform_response(
        ...
        """
        Transform Vertex Agent Engine response to LoveEngine ModelResponse format.
