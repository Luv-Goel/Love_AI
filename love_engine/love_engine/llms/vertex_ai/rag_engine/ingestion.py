...
Uses:
- love_engine.files.acreate_file for uploading files to GCS
- Vertex AI RAG Engine REST API for importing files into corpus (via httpx)

Key differences from OpenAI:
- Files must be uploaded to GCS first (via love_engine.files.acreate_file)
...
from love_engine.llms.vertex_ai.rag_engine.transformation import VertexAIRAGTransformation
from love_engine.rag.ingestion.base_ingestion import BaseRAGIngestion

if TYPE_CHECKING:
    from love_engine import Router
    from love_engine.types.rag import RAGIngestOptions
...
class VertexAIRAGIngestion(BaseRAGIngestion):
    ...
    Uses love_engine.files.acreate_file for GCS upload, then imports into RAG corpus.
    ...
    async def _upload_file_to_gcs(
        ...
        """
        Upload file to GCS using love_engine.files.acreate_file.
        ...
        # Set GCS_BUCKET_NAME env var for love_engine.files.create_file
        ...
        try:
            # Create file tuple for love_engine.files.acreate_file
            file_tuple = (filename, file_content, content_type)

            verbose_logger.debug(
                f"Uploading file to GCS via love_engine.files.acreate_file: {filename} "
                f"(bucket: {self.gcs_bucket})"
            )

            # Upload to GCS using LoveEngine's file upload
    ...
    def _build_transformation_config(self) -> Any:
        ...
        from love_engine.types.rag import RAGChunkingStrategy
    ...
    async def store(
        ...
        Vertex AI workflow:
        1. Upload file to GCS via love_engine.files.acreate_file
