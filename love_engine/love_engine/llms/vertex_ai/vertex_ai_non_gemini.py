...
from love_engine.llms.bedrock.common_utils import ModelResponseIterator
from love_engine.llms.custom_httpx.http_handler import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
from love_engine.types.llms.vertex_ai import *
from love_engine.utils import CustomStreamWrapper, ModelResponse, Usage
...
def _get_client_from_cache(client_cache_key: str):
    return love_engine.in_memory_llm_clients_cache.get_cache(client_cache_key)


def _set_client_in_cache(client_cache_key: str, vertex_llm_model: Any):
    love_engine.in_memory_llm_clients_cache.set_cache(
...
def completion(
    ...
    try:
        ...
        raise VertexAIError(
            status_code=400,
            message="vertexai import failed please run `pip install google-cloud-aiplatform`. This is required for the 'vertex_ai/' route on LoveEngine",
        )
    ...
    try:
        ...
        ## Load Config
        config = love_engine.VertexAIConfig.get_config()
        ...
        if (
        ...
        elif model in love_engine.vertex_text_models:
        ...
        elif model in love_engine.vertex_code_text_models:
            ...
            fake_stream = True
        elif model in love_engine.vertex_code_chat_models:  # vertex_code_llm_models
        ...
        ## RESPONSE OBJECT
        if isinstance(completion_response, love_engine.Message):
        ...
        ## CALCULATING USAGE
        if model in love_engine.vertex_language_models and response_obj is not None:
            ...
            # init prompt tokens
            # this block attempts to get usage from response_obj if it exists, if not it uses the love_engine token counter
        ...
        if isinstance(e, VertexAIError):
            raise e
        raise love_engine.APIConnectionError(
