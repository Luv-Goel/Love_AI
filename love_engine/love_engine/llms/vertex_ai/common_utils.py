...
from love_engine.utils import supports_response_schema, supports_system_messages
...
def redact_vertex_ai_metadata_from_love_engine_params(model_call_details: dict) -> None:
    """
    success_handler() merges response._hidden_params into
    love_engine_params.metadata['hidden_params'] before redaction runs, so the Vertex
    ...
    for metadata_key in ("metadata", "love_engine_metadata"):
        metadata = love_engine_params.get(metadata_key)
...
def vertex_request_labels_from_love_engine_params(
    love_engine_params: Optional[dict],
) -> Optional[Dict[str, str]]:
    """
    Build Vertex/GCP billing labels from LoveEngine user metadata on ``love_engine_params``:
    ``metadata`` (``completion(..., metadata=...)``) or ``love_engine_metadata``,
    ...
    if not love_engine_params:
        return None
    for key in ("metadata", "love_engine_metadata"):
        if key not in love_engine_params:
            continue
        metadata = love_engine_params[key]
...
def pop_vertex_request_labels(
    ...
    ``love_engine_params["metadata"]`` / ``love_engine_params["love_engine_metadata"]``
    ...
    if not labels:
        labels = vertex_request_labels_from_love_engine_params(love_engine_params)
...
def get_vertex_ai_model_route(
    model: str, love_engine_params: Optional[dict] = None
) -> VertexAIModelRoute:
    """
    ...
    Args:
        ...
        love_engine_params: Optional love_engine parameters dict that may contain base_model for routing
    ...
    # Check base_model in love_engine_params for gemini override
    if love_engine_params and love_engine_params.get("base_model") is not None:
        if "gemini" in love_engine_params["base_model"]:
    ...
    if model.isdigit() and love_engine_params and love_engine_params.get("api_base"):
        return VertexAIModelRoute.GEMINI
...
def get_supports_system_message(
    ...
    try:
        ...
        if love_engine.VertexGeminiConfig._is_model_gemini_spec_model(model):
            supports_system_message = True
    except Exception as e:
        verbose_logger.warning(
            "Unable to identify if system message supported. Defaulting to 'False'. Received error message - {}\nAdd it here - https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json".format(
                str(e)
            )
        )
...
def _get_vertex_url(
    ...
    model = love_engine.VertexGeminiConfig.get_model_for_vertex_ai_url(model=model)
...
def _get_gemini_url(
    ...
    if mode == "chat":
        ...
        raise ValueError(
            "LoveEngine's `gemini/` route does not support image generation yet. Let us know if you need this feature by opening an issue at https://github.com/BerriAI/love_engine/issues"
        )
    else:
...
def _check_text_in_content(parts: List[PartType]) -> bool:
    ...
    check that user_content has 'text' parameter.
        ...
        - Relevant Issue: https://github.com/BerriAI/love_engine/issues/5515
    """
...
def _filter_anyof_fields(schema_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    When anyof is present, only keep the anyof field and its contents - otherwise VertexAI will throw an error - https://github.com/BerriAI/love_engine/issues/11164
...
def construct_target_url(
    ...
    Handle cachedContent scenario - https://github.com/BerriAI/love_engine/issues/5460
...
class VertexAITokenCounter(BaseTokenCounter):
    ...
    async def count_tokens(
        ...
        count_tokens_params_request = copy.deepcopy(
            deployment.get("love_engine_params", {})
        )
        ...
        if VertexAIPartnerModels.is_vertex_partner_model(model_to_use):
            ...
            # Extract vertex-specific params from love_engine_params
            ...
            result = await partner_models_handler.count_tokens(
                ...
                love_engine_params=count_tokens_params_request,
                vertex_project=vertex_project,
                vertex_location=vertex_location,
            ...
            from love_engine.llms.vertex_ai.count_tokens.handler import VertexAITokenCounter
