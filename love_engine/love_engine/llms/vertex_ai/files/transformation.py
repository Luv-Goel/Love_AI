...
from love_engine.files.utils import FilesAPIUtils
from love_engine.love_engine_core_utils.cloud_storage_security import (
...
from love_engine.love_engine_core_utils.prompt_templates.common_utils import extract_file_data
from love_engine.llms.base_llm.chat.transformation import BaseLLMException
from love_engine.llms.base_llm.files.transformation import (
    BaseFilesConfig,
...
from love_engine.llms.vertex_ai.gemini.transformation import _transform_request_body
...
from love_engine.types.llms.vertex_ai import GcsBucketResponse
from love_engine.types.utils import ExtractedFileData, LlmProviders, ModelResponse
...
def _set_love_engine_batch_custom_id_labels(labels: Dict[str, str], custom_id: Any) -> None:
    """
    Store OpenAI batch custom_id for Vertex batch correlation.

    ``love_engine_custom_id`` is GCP-label-safe (may alter casing and characters).
    ``love_engine_custom_id_raw`` encodes the original string for
    round-trip correlation in batch output transforms.
    """
    custom_id_str = str(custom_id)
    labels["love_engine_custom_id"] = _sanitize_gcp_label_value(custom_id_str)
    raw_label_chunks = _encode_gcp_label_value_chunks(custom_id_str)
    labels["love_engine_custom_id_raw"] = raw_label_chunks[0]
    for index, raw_label_chunk in enumerate(raw_label_chunks[1:], start=1):
        labels[f"love_engine_custom_id_raw_{index}"] = raw_label_chunk


def _get_love_engine_batch_custom_id_from_labels(labels: Dict[str, Any]) -> str:
    """Prefer encoded custom_id when present (see _set_love_engine_batch_custom_id_labels)."""
    raw = labels.get("love_engine_custom_id_raw")
    if raw:
        raw_chunks = [str(raw)]
        chunk_prefix = "love_engine_custom_id_raw_"
        indexed_chunks = []
        ...
        if decoded is not None:
            return decoded
        return str(raw)
    return str(labels.get("love_engine_custom_id", "unknown"))
...
def _openai_batch_jsonl_entries_to_vertex_wrapped_requests(
    ...
    for _openai_jsonl_content in openai_jsonl_content:
        ...
        if custom_id is not None:
            ...
            _set_love_engine_batch_custom_id_labels(
...
class VertexAIFilesConfig(VertexBase, BaseFilesConfig):
    ...
    def validate_environment(
        ...
        if not api_key:
            api_key, _ = self.get_access_token(
                credentials=love_engine_params.get("vertex_credentials"),
                project_id=love_engine_params.get("vertex_project"),
            )
            if not api_key:
    ...
    def _get_gcs_object_name_from_batch_jsonl(
        ...
        named as: love_engine-vertex-{model}-{uuid}
        """
    ...
    def _get_configured_bucket_name(self, love_engine_params: Dict) -> str:
        bucket_name = love_engine_params.get("bucket_name") or os.getenv("GCS_BUCKET_NAME")
        if not bucket_name:
    ...
    def get_complete_file_url(
        ...
        """
        bucket_name = self._get_configured_bucket_name(love_engine_params)
    ...
    def _parse_gcs_uri(
        self, file_id: str, love_engine_params: Optional[Dict] = None
    ) -> Tuple[str, str]:
        """
        Validate a managed GCS file_id and return (bucket, url-encoded-object-path).
        """
        configured_bucket_name = self._get_configured_bucket_name(love_engine_params or {})
    ...
    def transform_retrieve_file_request(
        ...
        bucket, encoded_object = self._parse_gcs_uri(file_id, love_engine_params)
    ...
    def transform_file_content_response(
        ...
        try:
            ...
            if getattr(love_engine, "disable_vertex_batch_output_transformation", False):
    ...
    def _try_transform_vertex_batch_output_to_openai(
        self, content: bytes, logging_obj: Optional[LoveEngineLoggingObj] = None
    ) -> bytes:
        """
        ...
        {
          "request": {"contents": [...], "labels": {"love_engine_custom_id": "request-1", "love_engine_custom_id_raw": "..."}},
          "status": "",
    ...
    def _transform_single_vertex_batch_output_to_openai(
        ...
        custom_id = _get_love_engine_batch_custom_id_from_labels(labels)
