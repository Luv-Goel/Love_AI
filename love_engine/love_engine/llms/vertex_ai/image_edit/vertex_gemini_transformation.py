...
from love_engine.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import VertexLLM
...
from love_engine.types.utils import FileTypes, ImageObject, ImageResponse, OpenAIImage
...
class VertexAIGeminiImageEditConfig(BaseImageEditConfig, VertexLLM):
    ...
    def _resolve_vertex_project(self) -> Optional[str]:
        return (
            ...
            or getattr(love_engine, "vertex_project", None)
    ...
    def _resolve_vertex_location(self) -> Optional[str]:
        return (
            ...
            or getattr(love_engine, "vertex_location", None)
    ...
    def _resolve_vertex_credentials(self) -> Optional[str]:
        return (
            ...
            or getattr(love_engine, "vertex_credentials", None)
    ...
    def validate_environment(
        ...
        _api_base = love_engine_params.get("api_base") or api_base
        if _api_base is not None:
            return headers

        # First check love_engine_params (where vertex_ai_project/vertex_ai_credentials are passed)
        # then fall back to environment variables and other sources
        vertex_project = (
            self.safe_get_vertex_ai_project(love_engine_params)
            or self._resolve_vertex_project()
        )
        vertex_credentials = (
            self.safe_get_vertex_ai_credentials(love_engine_params)
    ...
    def get_complete_url(
        ...
        # First check love_engine_params (where vertex_ai_project/vertex_ai_location are passed)
        # then fall back to environment variables and other sources
        vertex_project = (
            self.safe_get_vertex_ai_project(love_engine_params)
            or self._resolve_vertex_project()
        ...
        vertex_location = (
            self.safe_get_vertex_ai_location(love_engine_params)
