...
class MilvusVectorStoreConfig(BaseVectorStoreConfig):
    ...
    Supports vector search with embeddings generated via love_engine.embeddings.
    """
    ...
    def validate_environment(
        ...
        if love_engine_params is not None:
            api_key = love_engine_params.api_key or get_secret_str("MILVUS_API_KEY")

        if not api_key:
            raise ValueError(
                "MILVUS_API_KEY is not set. Either set it in the love_engine_params or set the MILVUS_API_KEY environment variable."
            )
    ...
    def get_complete_url(
        ...
        if not api_base:
            raise ValueError(
                "Milvus API base URL is required. Set MILVUS_API_BASE environment variable or pass api_base in love_engine_params."
            )
    ...
    def transform_search_vector_store_request(
        ...
        Generates embeddings using love_engine.embeddings and constructs Azure AI Search request
        """
        # Convert query to string if it's a list
        if isinstance(query, list):
            query = " ".join(query)

        # Get embedding model from love_engine_params (required)
        embedding_model = love_engine_params.get("love_engine_embedding_model")
        if not embedding_model:
            raise ValueError(
                "embedding_model is required in love_engine_params for Milvus. You can call any love_engine embedding model."
                "Example: love_engine_params['embedding_model'] = 'azure/text-embedding-3-large'"
            )

        embedding_config = love_engine_params.get("love_engine_embedding_config", {})
        if not embedding_config:
            raise ValueError(
                "embedding_config is required in love_engine_params for Milvus. You can call any love_engine embedding model."
                "Example: love_engine_params['embedding_config'] = {'api_base': 'https://krris-mh44uf7y-eastus2.cognitiveservices.azure.com/', 'api_key': 'os.environ/AZURE_API_KEY', 'api_version': '2025-09-01'}"
            )

        # Get top_k (number of results to return)
        # Generate embedding for the query using love_engine.embeddings
        try:
            embedding_response = love_engine.embedding(
        ...
        db_name = love_engine_params.get("milvus_db_name")
        if db_name:
            request_body["dbName"] = db_name

        partition_names = love_engine_params.get("milvus_partition_names")
        if partition_names:
        ...
        #########################################################
        love_engine_logging_obj.model_call_details["input"] = query
        love_engine_logging_obj.model_call_details["embedding_model"] = embedding_model
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            # Try to get text_field from optional_params first, then love_engine_params
            optional_params = love_engine_logging_obj.model_call_details.get(
                "optional_params", {}
            )
            text_field = optional_params.get("milvus_text_field", "")

            # Fallback to love_engine_params if not in optional_params

            if not text_field:
                text_field = love_engine_logging_obj.model_call_details.get(
                    "love_engine_params", {}
            ...
            return VectorStoreSearchResponse(
                ...
                search_query=love_engine_logging_obj.model_call_details.get("input", ""),
                data=search_results,
            )
