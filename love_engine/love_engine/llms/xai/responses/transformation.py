...
from love_engine.constants import XAI_API_BASE
...
from love_engine.types.llms.openai import ResponsesAPIOptionalRequestParams
from love_engine.types.llms.xai import XAIWebSearchTool, XAIXSearchTool
...
class XAIResponsesAPIConfig(OpenAIResponsesAPIConfig):
    ...
    def validate_environment(
        self, headers: dict, model: str, love_engine_params: Optional[GenericLoveEngineParams]
    ) -> dict:
        """
        ...
        api_key = XAIModelInfo.get_api_key(
            love_engine_params.api_key, legacy_generic_before_env=True
        )

        if not api_key:
            from love_engine.llms.xai.oauth import (
                XAIOAuthAuthenticator,
                XAIOAuthError,
                should_use_xai_oauth,
            )

            if should_use_xai_oauth(love_engine_params.model_dump()):
                try:
        ...
        if not api_key:
            raise ValueError(
                "XAI API key is required. Set api_key, love_engine.xai_key, "
                "love_engine.api_key, XAI_API_KEY, or use_xai_oauth=True."
            )
    ...
    def get_complete_url(
        ...
        """
        from love_engine.llms.xai.oauth import XAIOAuthAuthenticator, should_use_xai_oauth

        api_key = XAIModelInfo.get_api_key(
            love_engine_params.get("api_key"), legacy_generic_before_env=True
        )
        if should_use_xai_oauth(love_engine_params) and not api_key:
