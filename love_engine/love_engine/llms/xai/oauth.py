...
from love_engine.llms.custom_httpx.http_handler import HTTPHandler, _get_httpx_client
    ...
    def get_access_token(self) -> str:
        ...
        if not auth_data:
            raise XAIOAuthLoginRequiredError(
                "xAI OAuth login required. Run `love_engine xai-oauth login`."
            )
        ...
        if not refresh_token:
            raise XAIOAuthLoginRequiredError(
                "xAI OAuth refresh token missing. Run `love_engine xai-oauth login`."
            )
...
def should_use_xai_oauth(love_engine_params: Optional[Dict[str, Any]]) -> bool:
    return bool((love_engine_params or {}).get("use_xai_oauth"))
