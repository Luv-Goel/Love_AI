...
if TYPE_CHECKING:
    ...
    from love_engine.types.utils import ModelInfo, Usage
    ...
    try:
        ...
        for root, dirs, files in os.walk(llms_dir):
            ...
            if (
                ...
                # Build the module path relative to love_engine
                rel_path = os.path.relpath(root, os.path.dirname(llms_dir))
                module_path = "love_engine." + rel_path.replace(os.sep, ".")
        ...
        try:
            from love_engine.proxy._experimental.mcp_server.guardrail_translation import (
