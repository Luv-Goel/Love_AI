...
import love_engine
from love_engine.constants import DEFAULT_NUM_WORKERS_LOVE_ENGINE_PROXY
...
class LoveEngineDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    @staticmethod
    def _echo_love_engine_version():
        pkg_version = importlib.metadata.version("love_engine")  # type: ignore
        click.echo(f"\nLoveEngine: Current Version = {pkg_version}\n")

    @staticmethod
    def _run_health_check(host, port):
        print("\nLoveEngine: Health Testing models in config")
    ...
    def _run_test_chat_completion(
        ...
        click.echo(
            f"\nLoveEngine: Making a test ChatCompletions request to your proxy. Model={request_model}"
        )
        import openai
        ...
        click.echo(f"\nLoveEngine: response from proxy {response}")

        print(
            f"\n LoveEngine: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}"
        )
        ...
        for chunk in stream_response:
            click.echo(f"LoveEngine: streaming response from proxy {chunk}")
    ...
    def _get_default_unvicorn_init_args(
        ...
        import love_engine
        from love_engine._logging import _get_uvicorn_json_log_config

        uvicorn_args = {
            "app": "love_engine.proxy.proxy_server:app",
            "host": host,
            "port": port,
        }
        ...
        if timeout_worker_healthcheck is not None:
            if (
                ...
                print(
                    f"\033[1;33mLoveEngine Proxy: --timeout_worker_healthcheck "
    ...
    def _apply_uvicorn_max_requests_jitter(
        ...
        if max_requests_before_restart is None:
            print(
                "\033[1;33mLoveEngine Proxy: --max_requests_before_restart_jitter "
        ...
        if (
            ...
            print(
                f"\033[1;33mLoveEngine Proxy: --max_requests_before_restart_jitter "
    ...
    def _patch_statreload_extra_paths(paths: Iterable[Optional[str]]) -> bool:
        ...
        patched_paths = getattr(StatReload, "_love_engine_patched_config_paths", None)
        if patched_paths is None:
            ...
            def _iter_with_extra(self):  # type: ignore[no-untyped-def]
                ...
                for path in StatReload._love_engine_patched_config_paths:
                    if path.exists():
                        yield path

            StatReload.iter_py_files = _iter_with_extra  # type: ignore[assignment]
            StatReload._love_engine_patched_config_paths = patched_paths  # type: ignore[attr-defined]
    ...
    def _configure_dev_reload(uvicorn_args: dict, config_path: Optional[str]) -> None:
        ...
        os.environ["LOVE_ENGINE_DEV_ENV_HOT_RELOAD"] = "True"
        env_path = os.path.join(os.getcwd(), ".env")
        ProxyInitializationHelpers._patch_statreload_extra_paths(
            [config_path, env_path]
        )
        verbose_proxy_logger.warning(
            "LoveEngine --reload: worker processes re-read .env with override, so .env "
    ...
    def _init_hypercorn_server(
        ...
        """
        Initialize love_engine with `hypercorn`
        """
        import asyncio
        ...
        print(
            f"\033[1;32mLoveEngine Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n"
        )
        config = Config()
        ...
        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(
                f"\033[1;32mLoveEngine Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"
            )
    ...
    def _init_granian_server(
        ...
        Uses a string import path so workers load ``love_engine.proxy.proxy_server:app``
        ...
        print(
            f"\033[1;32mLoveEngine Proxy: Starting server on {host}:{port} using Granian\033[0m\n"
        )
        if max_requests_before_restart is not None:
            print(
                "\033[1;33mLoveEngine: --max_requests_before_restart is not supported by Granian "
                "(Granian uses workers_lifetime in seconds, not a per-request limit).\033[0m\n"
            )
        if ciphers is not None:
            print(
                "\033[1;33mLoveEngine: --ciphers is not applied when using --run_granian.\033[0m\n"
            )

        kwargs: dict[str, Any] = {
            "target": "love_engine.proxy.proxy_server:app",
            "address": host,
            "port": port,
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()
                print(
                    '\033[1;34mLoveEngine: Test your local proxy with: "love_engine --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(
                    f"\033[1;34mLoveEngine: Curl Command Test for your local proxy\n {curl_command} \033[0m\n"
                )
                print(
                    "\033[1;34mDocs: https://docs.love_engine.ai/docs/simple_proxy\033[0m\n"
                )
                print(
            ...
            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with LoveEngine Proxy config
        ...
        print(
            f"\033[1;32mLoveEngine Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n"
        )
        gunicorn_options = {
            "bind": f"{host}:{port}",
        ...
        if os.environ.get("PROMETHEUS_MULTIPROC_DIR"):
            from love_engine.proxy.prometheus_cleanup import mark_worker_exit
    ...
    def _maybe_setup_prometheus_multiproc_dir(
        num_workers: int,
        love_engine_settings: Optional[dict],
    ) -> None:
        """
        ...
        if num_workers <= 1 or love_engine_settings is None:
            return

        # Check if prometheus is in any callback list
        # Each setting can be a list or a single string; normalize to list
        callbacks = love_engine_settings.get("callbacks") or []
        success_callbacks = love_engine_settings.get("success_callback") or []
        failure_callbacks = love_engine_settings.get("failure_callback") or []
        ...
        from love_engine.proxy.prometheus_cleanup import wipe_directory
        ...
        if not multiproc_dir:
            multiproc_dir = os.path.join(
                tempfile.gettempdir(), "love_engine_prometheus_multiproc"
            )
        ...
        print(f"LoveEngine: {action} PROMETHEUS_MULTIPROC_DIR={multiproc_dir}")
...
@click.option(
    "--num_workers",
    default=DEFAULT_NUM_WORKERS_LOVE_ENGINE_PROXY,
    help=(
        "Number of worker processes for uvicorn / gunicorn, or Granian worker processes "
        "(--workers). Default is 1 (from DEFAULT_NUM_WORKERS_LOVE_ENGINE_PROXY). "
...
@click.option(
    "--api_version",
    default=love_engine.AZURE_DEFAULT_API_VERSION,
    help="For azure - pass in the api version.",
)
@click.option(
    "--model", "-m", default=None, help="The model name to pass to love_engine expects"
)
@click.option(
    "--alias",
    default=None,
    help='The alias for the model - use this to give a love_engine model name (e.g. "huggingface/codellama/CodeLlama-7b-Instruct-hf") a more user-friendly name ("codellama")',
)
@click.option(
    "--add_key", default=None, help="The model name to pass to love_engine expects"
)
...
@click.option(
    ...
    "-c",
    default=None,
    help="Path to the proxy configuration file (e.g. config.yaml). Usage `love_engine --config config.yaml`",
)
@click.option(
    "--max_budget",
    default=None,
...
@click.option(
    ...
    help=(
        ...
        "LoveEngine versions contend for the same DB. Default is the v1 resolver."
    ),
)
@click.option(
    "--reload",
...
def run_server(
    ...
    if cli_args:
        if cli_args == ("xai-oauth", "login"):
            from love_engine.llms.xai.oauth import XAIOAuthAuthenticator
    ...
    if setup:
        from love_engine.setup_wizard import run_setup_wizard
    ...
    if local:
        ...
        try:
            ...
            raise ModuleNotFoundError(
                f"Missing dependency {e}. Run `pip install 'love_engine[proxy]'`"
            )
        except ImportError as e:
            if "love_engine[proxy]" in str(e):
                # user is missing a proxy dependency, ask them to pip install love_engine[proxy]
                raise e
            else:
                # this is just a local/relative import error, user git cloned love_engine
                from proxy_server import (
                    KeyManagementSettings,
                    ProxyConfig,
                    app,
    ...
    if version is True:
        ProxyInitializationHelpers._echo_love_engine_version()
        return
    ...
    if test is True:
        ...
        if run_granian:
            try:
                ...
                raise ImportError(
                    ...
                    "Run `pip install granian` or `pip install 'love_engine[proxy]'` "
            ...
            try:
                ...
                raise ImportError(
                    "uvicorn, gunicorn needs to be imported. Run - `pip install 'love_engine[proxy]'`"
                )
        ...
        if iam_token_db_auth or get_secret_bool("IAM_TOKEN_DB_AUTH"):
            from love_engine.proxy.auth.rds_iam_token import generate_iam_auth_token
        ...
        from love_engine.secret_managers.aws_secret_manager import decrypt_env_var
        ...
        if config is not None:
            ...
            try:
                ...
                raise ImportError(
                    "yaml needs to be imported. Run - `pip install 'love_engine[proxy]'`"
                )

            proxy_config = ProxyConfig()
            _config = asyncio.run(proxy_config.get_config(config_file_path=config))

            ### LOVE_ENGINE SETTINGS ###
            love_engine_settings = _config.get("love_engine_settings", None)
            if (
                love_engine_settings is not None
                and "json_logs" in love_engine_settings
                and love_engine_settings["json_logs"] is True
            ):
                import love_engine
            ...
            if database_url is None and os.getenv("DATABASE_URL") is None:
                ...
                from love_engine.proxy.utils import construct_database_url_from_env_vars
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                LoveEngineDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            ...
            if db_connection_timeout is None:
                db_connection_timeout = (
                    LoveEngineDatabaseConnectionPool.database_connection_pool_timeout.value
                )
        ...
        if (
            ...
            if is_prisma_runnable:
                from love_engine.proxy.db.check_migration import check_prisma_schema_diff
                ...
                if (
                    ...
                    if not use_v2_migration_resolver:
                        print(
                            "\033[1;33mLoveEngine Proxy: Using default (v1) migration resolver. "
                    ...
                    try:
                        ...
                        print(
                            "\033[1;31mLoveEngine Proxy: Database migration cannot proceed. "
                            f"{e}\033[0m",
                            file=sys.stderr,
                            flush=True,
                        )
                    ...
                    if not setup_ok:
                        if enforce_prisma_migration_check:
                            print(
                                "\033[1;31mLoveEngine Proxy: Database setup failed after multiple retries. "
                                "The proxy cannot start safely. Please check your database connection and migration status.\033[0m"
                            )
                            sys.exit(1)
                        else:
                            print(
                                "\033[1;33mLoveEngine Proxy: Database migration failed but continuing startup. "
        ...
        ProxyInitializationHelpers._maybe_setup_prometheus_multiproc_dir(
            ...
            love_engine_settings=love_engine_settings if config else None,  # type: ignore[possibly-unbound]
        )

        # Skip server startup if requested (after all setup is done)
        if skip_server_startup:
            print("LoveEngine: Setup complete. Skipping server startup as requested.")
            return
