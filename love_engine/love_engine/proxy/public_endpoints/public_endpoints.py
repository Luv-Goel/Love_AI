...
from love_engine.love_engine_core_utils.get_blog_posts import (
    BlogPost,
    BlogPostsResponse,
    GetBlogPosts,
    get_blog_posts,
...
from love_engine.repositories.table_repositories import ClaudeCodePluginRepository
from love_engine.types.agents import AgentCard
from love_engine.types.mcp import MCPPublicServer
from love_engine.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from love_engine.types.proxy.public_endpoints.public_endpoints import (
    AgentCreateInfo,
    ProviderCreateInfo,
    PublicModelHubInfo,
...
async def public_model_hub():
    ...
    if love_engine.public_model_groups is not None:
        model_groups = _get_model_group_info(
            ...
            all_models_str=love_engine.public_model_groups,
            model_group=None,
        )
...
async def public_skill_hub():
    ...
    from love_engine.proxy.anthropic_endpoints.claude_code_endpoints.claude_code_marketplace import (
        _get_prisma_client,
    )
    from love_engine.types.proxy.claude_code_endpoints import (
        ListPluginsResponse,
        PluginListItem,
    )
...
async def public_model_hub_info():
    import love_engine
    from love_engine.proxy.proxy_server import _title, version
    ...
    return PublicModelHubInfo(
        ...
        useful_links=love_engine.public_model_groups_links,
    )
...
async def get_supported_providers() -> List[str]:
    """
    Return a sorted list of all providers supported by LoveEngine.
    """
...
async def get_love_engine_model_cost_map():
    """
    Public endpoint to get the LoveEngine model cost map.
    Returns pricing information for all supported models.
    """
    import love_engine

    try:
        _model_cost_map = love_engine.model_cost
...
@router.get(
    "/public/love_engine_blog_posts",
    tags=["public"],
    response_model=BlogPostsResponse,
)
async def get_love_engine_blog_posts():
    """
    Public endpoint to get the latest LoveEngine blog posts.

    Fetches from GitHub with a 1-hour in-process cache.
    Falls back to the bundled local backup on any failure.
    """
    try:
        posts_data = get_blog_posts(url=love_engine.blog_posts_url)
    except Exception as e:
        verbose_logger.warning(
            "LoveEngine: get_love_engine_blog_posts endpoint fallback triggered: %s", str(e)
        )
...
async def get_supported_endpoints() -> SupportedEndpointsResponse:
    """
    Return the list of LoveEngine proxy endpoints and which providers support each one.
...
async def get_agent_fields() -> List[AgentCreateInfo]:
    ...
    for agent in agent_create_fields:
        ...
        if inherit_from and inherit_from in provider_map:
            ...
            # Copy provider fields and mark them for inclusion in love_engine_params
            inherited_fields = []
            for field in provider.get("credential_fields", []):
                field_copy = field.copy()
                field_copy["include_in_love_engine_params"] = True
