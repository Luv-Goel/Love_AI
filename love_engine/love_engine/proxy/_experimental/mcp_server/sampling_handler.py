...
routing them through LoveEngine's internal completion infrastructure.
...
LoveEngine acts as the LLM provider using its existing 100+ provider support,
...
def _resolve_model_from_preferences(
    ...
    Returns:
        A model string suitable for love_engine.acompletion().
    """
    import love_engine

    # Build list of available model names from proxy Router or love_engine.model_list
    ...
    try:
        ...
        pass
    if not available_model_names and love_engine.model_list:
        for entry in love_engine.model_list:
    ...
    # Last resort - use LoveEngine default or raise error
    default_sampling_model = getattr(love_engine, "default_mcp_sampling_model", None)
    if default_sampling_model:
        verbose_logger.debug(
            "MCP sampling model resolution: using love_engine.default_mcp_sampling_model='%s'",
            default_sampling_model,
        )
        return default_sampling_model
    raise ValueError(
        "No model could be resolved for MCP sampling. Please configure 'default_mcp_sampling_model' in your LoveEngine configuration."
    )
...
def _select_model_by_priority(
    ...
    * **intelligencePriority** — higher means "prefer smarter models".
    ...
    for name in model_names:
        try:
            info = _love_engine.get_model_info(name)
        except Exception:
            continue
...
def _convert_openai_response_to_mcp_result(
    ...
    """
    Convert a love_engine completion response to MCP CreateMessageResult.
    Args:
        response: The love_engine ModelResponse.
...
async def _check_model_access(
    ...
    if not _has_real_credential and not _is_admin:
        verbose_logger.warning(
            ...
            "auth context has no real LoveEngine credential (possible "
        ...
        return ErrorData(
            code=-1,
            message=(
                "Model access denied: sampling requires a valid LoveEngine "
    ...
    try:
        ...
        try:
            from love_engine.proxy.proxy_server import llm_router as _llm_router
        except ImportError:
            _llm_router = None

        await can_key_call_model(
            model=model,
            llm_model_list=getattr(love_engine, "model_list", None),
...
async def _run_budget_checks(
    ...
    """
    try:
        from love_engine.proxy.auth.auth_checks import common_checks
        from love_engine.proxy.proxy_server import (
            general_settings,
    ...
    global_proxy_spend = getattr(love_engine, "_global_proxy_spend", None)

    # Build request body and merge x-love_engine-tags from MCP headers BEFORE
...
def _build_sampling_request(
    ...
    scope format so that ``add_love_engine_data_to_request`` can apply
    ...
    - **headers**: All original HTTP headers are forwarded (except
      ...
      ``x-love_engine-api-key`` are visible to pre-call utils.
    ...
    _server_port = 4000  # LoveEngine default
    try:
        import love_engine.proxy.proxy_server as proxy_server
...
async def _build_completion_kwargs(
    ...
    completion_kwargs = await add_love_engine_data_to_request(
...
async def _run_guardrails_and_call_llm(
    ...
    try:
        from love_engine.proxy.proxy_server import proxy_logging_obj as _plo
    ...
    try:
        ...
        return await love_engine.acompletion(**completion_kwargs)
    except ImportError:
        return await love_engine.acompletion(**completion_kwargs)
...
async def handle_sampling_create_message(
    ...
    """
    Handle an MCP sampling/createMessage request by routing through LoveEngine.
    ...
    try:
        ...
        verbose_logger.debug(
            "MCP sampling: calling love_engine.acompletion with model=%s, num_messages=%d, has_tools=%s",
            model,
            len(openai_messages),
            bool(openai_tools),
        )
        ...
        from love_engine.proxy._types import ProxyException
