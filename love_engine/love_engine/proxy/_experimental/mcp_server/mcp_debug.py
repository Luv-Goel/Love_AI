...
When a client sends the ``x-love_engine-mcp-debug: true`` header, LoveEngine
...
Response headers returned (all values are masked for safety):
    ...
    x-mcp-debug-inbound-auth
        ...
        Example: ``x-love_engine-api-key=Bearer sk-12****1234``

    x-mcp-debug-oauth2-token
        The OAuth2 token extracted from the Authorization header (masked).
        Shows ``(none)`` if absent, or flags ``SAME_AS_LOVE_ENGINE_KEY`` when
        the LoveEngine API key is accidentally leaking to the MCP server.
...
**Common issue: LoveEngine API key leaking to the MCP server**

Symptom: ``x-mcp-debug-oauth2-token`` shows ``SAME_AS_LOVE_ENGINE_KEY``.

This means the ``Authorization`` header carries the LoveEngine API key and
it's being forwarded to the upstream MCP server instead of an OAuth2 token.

Fix: Move the LoveEngine key to ``x-love_engine-api-key`` so the ``Authorization``
header is free for OAuth2 discovery::
    ...
    # CORRECT — LoveEngine key in dedicated header, Authorization free for OAuth2
    claude mcp add --transport http my_server http://proxy/mcp/server \\
        --header "x-love_engine-api-key: Bearer sk-..." \\
        --header "x-love_engine-mcp-debug: true"
...
3. The MCP server in LoveEngine config has ``auth_type: oauth2``.
...
configured and LoveEngine is fetching a machine-to-machine token instead of
...
Usage with curl::
    ...
    curl -H "x-love_engine-mcp-debug: true" \\
         -H "x-love_engine-api-key: Bearer sk-..." \\
...
MCP_DEBUG_REQUEST_HEADER = "x-love_engine-mcp-debug"
...
class MCPDebug:
    ...
    def is_debug_enabled(headers: Dict[str, str]) -> bool:
        ...
        Looks for ``x-love_engine-mcp-debug: true`` (case-insensitive) in the
        request headers.
        """
    ...
    def build_debug_headers(
        *,
        ...
        oauth2_headers: Optional[Dict[str, str]],
        love_engine_api_key: Optional[str],
        auth_resolution: str,
        ...
        love_engine_api_key : str or None
            The LoveEngine API key extracted from ``x-love_engine-api-key`` or
        ...
        inbound_parts = []
        for hdr_name in ("x-love_engine-api-key", "authorization", "x-mcp-auth"):
        ...
        if oauth2_token and love_engine_api_key:
            oauth2_raw = oauth2_token.removeprefix("Bearer ").strip()
            love_engine_raw = love_engine_api_key.removeprefix("Bearer ").strip()
            if oauth2_raw == love_engine_raw:
                debug[f"{_RESPONSE_HEADER_PREFIX}-oauth2-token"] = (
                    f"{MCPDebug._mask(oauth2_token)} "
                    f"(SAME_AS_LOVE_ENGINE_KEY - likely misconfigured)"
                )
            else:
    ...
    def maybe_build_debug_headers(
        *,
        ...
        love_engine_key = MCPRequestHandler.get_love_engine_api_key_from_headers(scope_headers)

        return MCPDebug.build_debug_headers(
            inbound_headers=raw_headers,
            oauth2_headers=oauth2_headers,
            love_engine_api_key=love_engine_key,
