...
from love_engine.proxy._experimental.mcp_server.oauth_utils import (
...
async def _extract_user_id_from_request(request: Request) -> Optional[str]:
    """Best-effort extraction of LoveEngine user_id from the request's Authorization header.
    ...
    try:
        from love_engine.proxy._types import hash_token  # noqa: PLC0415
        from love_engine.proxy.proxy_server import user_api_key_cache  # noqa: PLC0415
...
async def exchange_token_with_server(
    ...
    # the calling client has provided a valid LoveEngine identity.
    ...
    if mcp_server.needs_user_oauth_token:
        ...
        if user_id:
            ...
            verbose_logger.debug(
                "exchange_token_with_server: no LoveEngine user_id found in request; "
...
async def token_endpoint(
    ...
    2. Store the user's token in the db - and generate a LoveEngine virtual key
    3. Return the token
...
"""
    ...
    - LoveEngine legacy pattern: /{server_name}/mcp (backward compatibility)
...
async def _build_oauth_protected_resource_response(
    ...
    if mcp_server_name:
        if use_standard_pattern:
            ...
            # LoveEngine legacy pattern: /{server_name}/mcp
...
# LoveEngine legacy pattern: /.well-known/oauth-protected-resource/{server_name}/mcp
...
async def oauth_protected_resource_mcp(
    ...
    """
    OAuth protected resource discovery endpoint using LoveEngine legacy URL pattern.
...
# LoveEngine legacy pattern and root endpoint
@router.get(
...
async def openid_configuration(request: Request):
    ...
    # the signing keys and verify love_engine-issued tokens.
    try:
...
async def jwks_json(request: Request):
    ...
    MCP servers and gateways use this endpoint to verify love_engine-issued JWTs.
