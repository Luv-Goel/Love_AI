...
from love_engine.proxy._experimental.mcp_server.discoverable_endpoints import (
    get_request_base_url,
)
from love_engine.proxy._experimental.mcp_server.mcp_context import (
    _mcp_active_toolset_id,
    _mcp_gateway_initialize_instructions,
    _mcp_gateway_server_name,
)
from love_engine.proxy._experimental.mcp_server.mcp_debug import MCPDebug
from love_engine.proxy._experimental.mcp_server.utils import (
    LOVE_ENGINE_MCP_SERVER_DESCRIPTION,
    LOVE_ENGINE_MCP_SERVER_NAME,
    LOVE_ENGINE_MCP_SERVER_VERSION,
...
from love_engine.proxy.love_engine_pre_call_utils import (
    LoveEngineProxyRequestSetup,
    get_chain_id_from_headers,
)
from love_engine.types.mcp import MCPAuth, MCPSpecVersion
from love_engine.types.mcp_server.mcp_server_manager import MCPInfo, MCPServer
from love_engine.types.utils import CallTypes, StandardLoggingMCPToolCall
from love_engine.utils import Rules, client, function_setup
...
# "mcp" requires python 3.10 or higher, but several love_engine users use python 3.8
# We're making this conditional import to avoid breaking users who use python 3.8.
# TODO: Make this a util function for love_engine client usage
...
if MCP_AVAILABLE:
    ...
    from love_engine.proxy._experimental.mcp_server.auth.love_engine_auth_handler import (
    ...
    from love_engine.proxy._experimental.mcp_server.sse_transport import SseServerTransport
    ...
    # required dependency for `love_engine` pip package
    ######################################################
    ...
    server: Server = Server(
        name=LOVE_ENGINE_MCP_SERVER_NAME,
        version=LOVE_ENGINE_MCP_SERVER_VERSION,
    )
    ...
    async def mcp_server_tool_call(
        ...
        try:
            ...
            try:
                ...
                if chain_id:
                    body_data["love_engine_trace_id"] = chain_id
                    body_data["love_engine_session_id"] = chain_id
    ...
    async def _get_user_oauth_extra_headers_from_db(
        ...
        try:
            ...
            from love_engine.proxy._experimental.mcp_server.oauth2_token_cache import (  # noqa: PLC0415
            ...
            if prefetched_creds is not None:
                ...
                from love_engine.proxy.utils import (  # noqa: PLC0415
    ...
    async def _prefetch_oauth_creds_for_user(
        ...
        try:
            ...
            from love_engine.proxy.utils import get_prisma_client_or_throw  # noqa: PLC0415
    ...
    async def _get_tools_from_mcp_servers(
        ...
        if log_list_tools_to_spendlogs:
            ...
            effective_love_engine_trace_id = love_engine_trace_id or get_chain_id_from_headers(
                raw_headers
            )
            ...
            list_tools_request_data = {
                ...
                "love_engine_call_id": list_tools_call_id,
                "love_engine_trace_id": effective_love_engine_trace_id,
                "metadata": {
            ...
            if user_api_key_auth is not None:
                LoveEngineProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
            ...
            try:
                ...
                if love_engine_logging_obj:
                    love_engine_logging_obj.call_type = CallTypes.list_mcp_tools.value
                    love_engine_logging_obj.model = "MCP: list_tools"
        ...
        try:
            ...
            if love_engine_logging_obj:
                ...
                metadata_dict = love_engine_logging_obj.model_call_details.get("metadata")
    ...
    async def _get_byok_credential(
        ...
        from love_engine.proxy._experimental.mcp_server.db import get_user_credential
    ...
    async def execute_mcp_tool(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: Additional arguments (e.g., love_engine_logging_obj)
        ...
        if requested_server is not None and not name_is_prefixed:
            ...
            # LoveEngine prefix). The first segment is not a registered server
        ...
        if love_engine_logging_obj:
            ...
            love_engine_logging_obj.model = f"MCP: {name}"
            love_engine_logging_obj.model_call_details["model"] = f"MCP: {name}"
    ...
    async def call_mcp_tool(
        ...
        **kwargs: Any,
        ...
        if love_engine_logging_obj:
            love_engine_logging_obj.post_call(original_response=response)
            end_time = datetime.now()
    ...
    async def _handle_managed_mcp_tool(
        ...
        oauth2_headers: Optional[Dict[str, str]] = None,
        raw_headers: Optional[Dict[str, str]] = None,
        love_engine_logging_obj: Optional[Any] = None,
        host_progress_callback: Optional[Callable] = None,
    ) -> CallToolResult:
    ...
    async def _handle_stale_mcp_session(
        ...
        Fixes https://github.com/BerriAI/love_engine/issues/20992
        """
    ...
    def _get_forwarded_auth_from_scope(scope: Scope) -> Optional[str]:
        ...
        Only returns the ``Authorization`` header when ``x-love_engine-api-key`` is
        ...
        ``x-love_engine-api-key`` is absent the ``Authorization`` header may itself
        be the LoveEngine proxy API key (backward-compat path in
        ``MCPRequestHandler.process_mcp_request``), and forwarding it upstream
        would leak the proxy key to a third-party MCP server.
        """
        authorization = None
        has_love_engine_key_header = False
        for key, value in scope.get("headers", []):
            key_lower = key.lower()
            if key_lower == b"authorization":
                authorization = value.decode("latin-1")
            elif key_lower == b"x-love_engine-api-key":
                has_love_engine_key_header = True
        if not has_love_engine_key_header:
            return None
    ...
    async def _probe_upstream_auth(
        ...
        probe_payload = {
            "jsonrpc": "2.0",
            "id": "love_engine-mcp-auth-probe",
            "method": "initialize",
            "params": {
                "protocolVersion": MCPSpecVersion.jun_2025.value,
                "capabilities": {},
                "clientInfo": {
                    "name": "love_engine-mcp-auth-probe",
                    "version": "1.0.0",
                },
            },
        }
    ...
    async def _check_passthrough_upstream_auth(
        ...
        for srv, (probe_status, _) in zip(passthrough_servers, probe_results):
            if probe_status == 401:
                ...
                # the upstream IdP metadata proxied by LoveEngine.
                www_authenticate = _get_passthrough_www_authenticate(
                    scope=scope,
    ...
    async def handle_streamable_http_mcp(
        ...
        try:
            ...
            # Strip any client-supplied x-mcp-toolset-id to prevent love_smithry.
            scope["headers"] = [
                (k, v)
            ...
            # Inject masked debug headers when client sends x-love_engine-mcp-debug: true
    ...
    app = FastAPI(
        title=LOVE_ENGINE_MCP_SERVER_NAME,
        description=LOVE_ENGINE_MCP_SERVER_DESCRIPTION,
        version=LOVE_ENGINE_MCP_SERVER_VERSION,
        lifespan=lifespan,
    )
