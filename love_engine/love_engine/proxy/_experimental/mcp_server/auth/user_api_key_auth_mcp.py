...
def _is_love_engine_auth_admission_error(exc: Exception) -> bool:
...
class MCPRequestHandler:
    ...
    1. Authentication via LoveEngine API keys
    ...
    LOVE_ENGINE_API_KEY_HEADER_NAME_PRIMARY = SpecialHeaders.custom_love_engine_api_key.value
    LOVE_ENGINE_API_KEY_HEADER_NAME_SECONDARY = SpecialHeaders.openai_authorization.value

    # This is the header to use if you want LoveEngine to use this header for authenticating to the MCP server
    LOVE_ENGINE_MCP_AUTH_HEADER_NAME = SpecialHeaders.mcp_auth.value

    LOVE_ENGINE_MCP_SERVERS_HEADER_NAME = SpecialHeaders.mcp_servers.value

    LOVE_ENGINE_MCP_ACCESS_GROUPS_HEADER_NAME = SpecialHeaders.mcp_access_groups.value
    ...
    async def process_mcp_request(
        ...
        # Check if there is an explicit LoveEngine API key (primary header)
        has_explicit_love_engine_key = (
            headers.get(MCPRequestHandler.LOVE_ENGINE_API_KEY_HEADER_NAME_PRIMARY)
            is not None
        )

        love_engine_api_key = (
            MCPRequestHandler.get_love_engine_api_key_from_headers(headers) or ""
        )
        ...
        mcp_servers_header = headers.get(
            MCPRequestHandler.LOVE_ENGINE_MCP_SERVERS_HEADER_NAME
        )
        ...
        if request_route.startswith("/.well-known/"):
        ...
        elif has_explicit_love_engine_key:
            # An explicit x-love_engine-api-key is always a LoveEngine credential, even
            ...
            validated_user_api_key_auth = await user_api_key_auth(
                api_key=love_engine_api_key, request=request
            )
            ...
            # Authorization bearer is an upstream token, never a LoveEngine key. Skip
            # LoveEngine validation entirely — covering both the no-credential
            ...
            # LoveEngine credential, so a failed validation is a genuine 401/403 and
            ...
            try:
                ...
                if (
                    ...
                    and _is_love_engine_auth_admission_error(exc)
                    and _is_mcp_passthrough_cold_start(
                        mcp_servers_from_path, client_ip=client_ip
                    )
                ):
    ...
    def _extract_target_server_names_from_path(path: str) -> List[str]:
        ...
        as targeting the delegate server (bypassing LoveEngine auth) while
    ...
    def _target_servers_delegate_auth_to_upstream(
        ...
        Used by :meth:`process_mcp_request` to skip LoveEngine API-key/SSO auth
        ...
        one non-delegated server) fall back to normal LoveEngine auth.
        """
        ...
        # targets a non-delegate server, skipping LoveEngine auth for it.
        ...
        for name in target_names:
            ...
            if getattr(server, "delegate_auth_to_upstream", False) is not True:
                return False
            # Never delegate for M2M (client_credentials) servers: LoveEngine
            ...
            # tools authenticated as LoveEngine's service account.
    ...
    def _get_mcp_auth_header_from_headers(headers: Headers) -> Optional[str]:
        """
        Get the header passed to LoveEngine to pass to downstream MCP servers

        By default love_engine will check for the header `x-mcp-auth` by setting one of the following:
            1. `LOVE_ENGINE_MCP_CLIENT_SIDE_AUTH_HEADER_NAME` as an environment variable
            2. `mcp_client_side_auth_header_name` in the general settings on the config.yaml file

        Support this auth: https://docs.love_engine.ai/docs/mcp#using-your-mcp-with-client-side-credentials

        If you want to use a different header name, you can set the `LOVE_ENGINE_MCP_CLIENT_SIDE_AUTH_HEADER_NAME` in the secret manager or `mcp_client_side_auth_header_name` in the general settings.
    ...
    def _get_mcp_server_auth_headers_from_headers(
        ...
        for header_name, header_value in headers.items():
            if header_name.lower().startswith(prefix):
                ...
                if (
                    header_name.lower()
                    == MCPRequestHandler.LOVE_ENGINE_MCP_ACCESS_GROUPS_HEADER_NAME.lower()
                    or header_name.lower()
                    == MCPRequestHandler.LOVE_ENGINE_MCP_SERVERS_HEADER_NAME.lower()
                ):
                    continue
    ...
    def _get_mcp_client_side_auth_header_name() -> str:
        ...
        MCP_CLIENT_SIDE_AUTH_HEADER_NAME: str = (
            MCPRequestHandler.LOVE_ENGINE_MCP_AUTH_HEADER_NAME
        )
        if get_secret_str("LOVE_ENGINE_MCP_CLIENT_SIDE_AUTH_HEADER_NAME") is not None:
            MCP_CLIENT_SIDE_AUTH_HEADER_NAME = (
                get_secret_str("LOVE_ENGINE_MCP_CLIENT_SIDE_AUTH_HEADER_NAME")
    ...
    @staticmethod
    def get_love_engine_api_key_from_headers(headers: Headers) -> Optional[str]:
        """
        Get the LoveEngine API key from the headers using case-insensitive lookup

        1. Check if `x-love_engine-api-key` is in the headers
        ...
        api_key = headers.get(MCPRequestHandler.LOVE_ENGINE_API_KEY_HEADER_NAME_PRIMARY)
        if api_key:
            return api_key

        auth_header = headers.get(
            MCPRequestHandler.LOVE_ENGINE_API_KEY_HEADER_NAME_SECONDARY
        )
        if auth_header:
            return auth_header
    ...
    def _get_key_object_permission(
        ...
        get_key_object() in love_engine/proxy/auth/auth_checks.py
        """
    ...
    async def _get_team_object_permission(
        ...
        get_team_object() in love_engine/proxy/auth/auth_checks.py
        """
        ...
        team_obj: Optional[LOVE_ENGINE_TeamTable] = await get_team_object(
    ...
    async def _get_key_access_group_mcp_server_extras(
        ...
        """
        Resolve the key's unified `access_group_ids` (LOVE_ENGINE_AccessGroupTable) to
    ...
    async def _get_org_object_permission(
        ...
        """
        from love_engine.proxy.auth.auth_checks import get_object_permission, get_org_object
        from love_engine.proxy.proxy_server import (
            prisma_client,
            proxy_logging_obj,
            user_api_key_cache,
    ...
    async def _get_allowed_mcp_servers_for_end_user(
        ...
        """
        from love_engine.proxy.auth.auth_checks import get_end_user_object
        from love_engine.proxy.proxy_server import (
            prisma_client,
            proxy_logging_obj,
            user_api_key_cache,
    ...
    async def _get_agent_object_permission(
        ...
        """
        from love_engine.proxy.auth.auth_checks import get_object_permission
        from love_engine.proxy.proxy_server import (
            prisma_client,
            proxy_logging_obj,
            user_api_key_cache,
    ...
    def get_mcp_access_groups_from_headers(headers: Headers) -> Optional[List[str]]:
        ...
        mcp_access_groups_header = headers.get(
            MCPRequestHandler.LOVE_ENGINE_MCP_ACCESS_GROUPS_HEADER_NAME
        )
