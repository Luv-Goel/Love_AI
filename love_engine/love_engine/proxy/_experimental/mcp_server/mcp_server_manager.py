...
from love_engine.exceptions import BlockedPiiEntityError, GuardrailRaisedException
from love_engine.experimental_mcp_client.client import MCPClient, MCPSigV4Auth
from love_engine.love_engine_core_utils.url_utils import SSRFError, async_safe_get
...
from love_engine.proxy._experimental.mcp_server.exceptions import MCPUpstreamAuthError
from love_engine.proxy._experimental.mcp_server.elicitation_handler import (
    MCP_ELICITATION_AVAILABLE,
)
from love_engine.proxy._experimental.mcp_server.sampling_handler import (
    MCP_SAMPLING_AVAILABLE,
)
from love_engine.proxy._experimental.mcp_server.oauth2_token_cache import resolve_mcp_auth
from love_engine.proxy._experimental.mcp_server.utils import (
...
from love_engine.repositories.table_repositories import MCPServerRepository
from love_engine.types.llms.custom_http import httpxSpecialProvider
from love_engine.types.mcp import MCPAuth, MCPStdioConfig
from love_engine.types.mcp_server.mcp_server_manager import (
    MCPInfo,
    MCPOAuthMetadata,
    MCPServer,
)
...
_separator_probe_tool_name = f"love_engine{MCP_TOOL_PREFIX_SEPARATOR}probe"
...
def _should_strip_caller_authorization(
    ...
    - **OAuth pass-through servers**: strip when the ``Authorization``
      header is actually the LoveEngine API key — either because admission
      validated it (``user_api_key_auth.api_key`` is set) and the caller
      did NOT also supply ``x-love_engine-api-key`` to disambiguate, or
    ...
    has_explicit_love_engine_admission_header = (
        normalized_raw_headers.get("x-love_engine-api-key") is not None
    )
    admission_consumed_authorization_as_love_engine_key = (
        user_api_key_auth is not None
        and bool(getattr(user_api_key_auth, "api_key", None))
        and not has_explicit_love_engine_admission_header
    )
    return admission_consumed_authorization_as_love_engine_key or (
        user_api_key_auth is None and not has_explicit_love_engine_admission_header
    )
...
def _warn_internal_delegate_pkce_if_applicable(
    ...
    verbose_logger.warning(
        ...
        "/authorize flow and complete PKCE without a LoveEngine API key session; ensure the "
...
def _create_sampling_callback(user_api_key_auth: Optional[Any] = None):
    ...
    upstream MCP servers by routing them through love_engine.acompletion().
    """
    ...
    async def _sampling_callback(context, params):
        ...
        return await handle_sampling_create_message(
            context=context,
            params=params,
            default_model=getattr(love_engine, "default_mcp_sampling_model", None),
...
def _create_elicitation_callback():
    ...
    async def _elicitation_callback(context, params):
        ...
        from love_engine.proxy._experimental.mcp_server.server import get_active_mcp_session
...
class MCPServerManager:
    ...
    async def load_servers_from_config(
        ...
        Args:
            ...
            mcp_aliases: Optional dictionary mapping aliases to server names from love_engine_settings
        """
    ...
    async def _register_openapi_tools(
        ...
        """
        from love_engine.proxy._experimental.mcp_server.openapi_to_mcp_generator import (
            build_input_schema,
        ...
        try:
            ...
            if server.authentication_token:
                from love_engine.types.mcp import MCPAuth
    ...
    def remove_server(self, mcp_server: LOVE_ENGINE_MCPServerTable):
        """
    ...
    def _resolve_env_vars_list(
        self,
        mcp_server: LOVE_ENGINE_MCPServerTable,
        *,
        ...
        if env_vars_are_encrypted:
            from love_engine.proxy._experimental.mcp_server.db import (  # noqa: PLC0415
    ...
    async def add_server(self, mcp_server: LOVE_ENGINE_MCPServerTable):
    ...
    async def update_server(self, mcp_server: LOVE_ENGINE_MCPServerTable):
    ...
    async def get_allowed_mcp_servers(
        ...
        try:
            ...
            from love_engine.proxy._experimental.mcp_server.mcp_context import (  # noqa: PLC0415
            ...
            # LoveEngine granting access here does not bypass any security gate.
            is_anonymous = not (
                user_api_key_auth
                and (
            ...
            if is_anonymous:
                delegate_server_ids = [
                    ...
                    # unauthenticated caller would get LoveEngine to proxy tool
    ...
    async def resolve_toolset_tool_permissions(
        ...
        from love_engine.proxy._experimental.mcp_server.toolset_db import list_mcp_toolsets
    ...
    async def get_toolset_by_name_cached(
        ...
        from love_engine.constants import DEFAULT_MANAGEMENT_OBJECT_IN_MEMORY_CACHE_TTL
        from love_engine.proxy.proxy_server import user_api_key_cache
        from love_engine.types.mcp_server.mcp_toolset import MCPToolset
        ...
        from love_engine.proxy._experimental.mcp_server.toolset_db import (
    ...
    async def _resolve_static_headers_with_env_vars(
        ...
        *,
        ...
        Per-user values come from the ``LOVE_ENGINE_MCPUserEnvVars`` row for the
        calling user.
    ...
    async def _load_user_env_vars(
        ...
        *,
        ...
        from love_engine.proxy.proxy_server import prisma_client  # noqa: PLC0415
            ...
            if server.command:
                ...
                if (
                    ...
                    raise HTTPException(
                        ...
                        f"Add it to LOVE_ENGINE_MCP_STDIO_EXTRA_COMMANDS to allow this command.",
                    )
    ...
    async def _get_tools_from_server(
        ...
        try:
            ...
            if user_api_key_auth is not None and not server.spec_path:
                from love_engine.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer import (
                    get_mcp_jwt_signer,
    ...
    def _assign_unique_short_prefix(
        ...
        ``LOVE_ENGINE_USE_SHORT_MCP_TOOL_PREFIX`` is disabled, when the server
    ...
    def _create_during_hook_task(
        ...
        from love_engine.types.mcp import MCPDuringCallRequestObject
    ...
    async def _call_regular_mcp_tool(
        ...
        # the exact case of server alias/name (e.g., '1love_engineagcgateway' vs '1LoveEngineAGCGateway')
    ...
    async def _resolve_oauth2_headers_for_tool_call(
        ...
        try:
            from love_engine.proxy._experimental.mcp_server.server import (  # noqa: PLC0415
    ...
    async def reload_servers_from_database(self):
        ...
        from love_engine.proxy.management_endpoints.mcp_management_endpoints import (
        ...
        from love_engine.proxy._experimental.mcp_server.db import LOVE_ENGINE_MCPServerTable
        ...
        db_mcp_servers = [LOVE_ENGINE_MCPServerTable(**r.model_dump()) for r in raw_rows]
    ...
    def _is_server_accessible_from_ip(
        ...
        if server.available_on_public_internet:
            return True
        # Check backwards compat: love_engine.public_mcp_servers
        public_ids = set(love_engine.public_mcp_servers or [])
    ...
    def get_public_mcp_servers(self) -> List[MCPServer]:
        ...
        Default (love_engine.public_mcp_hub_strict_whitelist=True): mirrors
        /public/model_hub and /public/agent_hub — gates strictly on the
        love_engine.public_mcp_servers whitelist. Returns an empty list when no
        ...
        Legacy (love_engine.public_mcp_hub_strict_whitelist=False): preserves the
        ...
        """
        if love_engine.public_mcp_hub_strict_whitelist:
            if love_engine.public_mcp_servers is None:
                return []
            public_ids = set(love_engine.public_mcp_servers)
            return [
                server
                for server in self.get_registry().values()
                if server.server_id in public_ids
    ...
    def expand_permission_list(self, identifiers: List[str]) -> List[str]:
        ...
        for identifier in identifiers:
            ...
            if matches:
                ...
                # identifier with newlines cannot love_smith log lines.
                verbose_logger.debug(
    ...
    async def health_check_server(
        ...
        if not server:
            verbose_logger.warning(f"MCP Server {server_id} not found")
            return LOVE_ENGINE_MCPServerTable(
                server_id=server_id,
                server_name=None,
    ...
    ) -> List[LOVE_ENGINE_MCPServerTable]:
        """
    ...
    async def get_all_allowed_mcp_servers(
        ...
        list_mcp_servers: List[LOVE_ENGINE_MCPServerTable] = []
    ...
    def _build_mcp_server_table(self, server: MCPServer) -> LOVE_ENGINE_MCPServerTable:
        return LOVE_ENGINE_MCPServerTable(
            server_id=server.server_id,
            server_name=server.server_name,
            alias=server.alias,
    ...
    async def get_all_mcp_servers_unfiltered(self) -> List[LOVE_ENGINE_MCPServerTable]:
        ...
        servers: List[LOVE_ENGINE_MCPServerTable] = []
        for server in registry.values():
