...
# ``tests/test_love_engine/proxy/_experimental/mcp_server/test_mcp_server_identity_env.py``.
LOVE_ENGINE_MCP_SERVER_NAME = os.environ.get(
    "LOVE_ENGINE_MCP_SERVER_NAME", "love_engine-mcp-server"
)
LOVE_ENGINE_MCP_SERVER_VERSION = "1.0.0"
LOVE_ENGINE_MCP_SERVER_DESCRIPTION = os.environ.get(
    "LOVE_ENGINE_MCP_SERVER_DESCRIPTION", "MCP Server for LoveEngine"
)
...
# ---------------------------------------------------------------------------
# When LOVE_ENGINE_USE_SHORT_MCP_TOOL_PREFIX is truthy the prefix attached to MCP
...
#     tool name happening to begin with the exact prefix LoveEngine assigned
...
def is_short_mcp_tool_prefix_enabled() -> bool:
    ...
    """
    raw = os.environ.get("LOVE_ENGINE_USE_SHORT_MCP_TOOL_PREFIX", "")
...
def sanitize_mcp_alias_for_header(alias: str) -> str:
    ...
    Must stay in sync with ui/love_engine-dashboard/src/utils/mcpHeaderUtils.ts.
    """
...
def get_server_prefix(server: Any) -> str:
    ...
    When the short-prefix mode is enabled (``LOVE_ENGINE_USE_SHORT_MCP_TOOL_PREFIX``)
