...
from love_engine.constants import MCP_PER_USER_TOKEN_EXPIRY_BUFFER_SECONDS
...
from love_engine.types.mcp import MCPCredentials
...
def decrypt_global_env_var_values(env_vars: Optional[Iterable[Any]]) -> None:
    ...
    Accepts ``MCPEnvVar`` models (``LOVE_ENGINE_MCPServerTable``) or plain dicts
    ...
    for entry in env_vars:
        ...
        if decrypted is None:
            ...
            verbose_proxy_logger.warning(
                "MCP global env var %s failed to decrypt (LOVE_ENGINE_SALT_KEY "
...
async def get_all_mcp_servers(
    ...
    try:
        ...
        tables = [
            LOVE_ENGINE_MCPServerTable(**mcp_server.model_dump())
        ...
        verbose_proxy_logger.debug(
            "love_engine.proxy._experimental.mcp_server.db.py::get_all_mcp_servers - {}".format(
                str(e)
            )
        )
        return []
...
async def get_mcp_server(
...
) -> Optional[LOVE_ENGINE_MCPServerTable]:
    """
    Returns the matching mcp server from the db iff exists
    """
    mcp_server: Optional[LOVE_ENGINE_MCPServerTable] = await MCPServerRepository(
        prisma_client
    ).table.find_unique(
        where={
            "server_id": server_id,
    ...
    if mcp_server is None:
        return None
    table = LOVE_ENGINE_MCPServerTable(**mcp_server.model_dump())
    decrypt_global_env_var_values(table.env_vars)
    return table
...
async def get_mcp_servers(
    ...
    """
    _mcp_servers: List[LOVE_ENGINE_MCPServerTable] = await MCPServerRepository(
        prisma_client
    ).table.find_many(
        where={
    ...
    final_mcp_servers: List[LOVE_ENGINE_MCPServerTable] = []
    for _mcp_server in _mcp_servers:
        table = LOVE_ENGINE_MCPServerTable(**_mcp_server.model_dump())
        decrypt_global_env_var_values(table.env_vars)
...
async def get_mcp_servers_by_verificationtoken(
    ...
    """
    verification_token_record: LOVE_ENGINE_TeamTable = await VerificationTokenRepository(
        prisma_client
    ).table.find_unique(
        where={
            "token": token,
...
async def get_mcp_servers_by_team(
    ...
    """
    team_record: LOVE_ENGINE_TeamTable = await TeamRepository(
        prisma_client
    ).table.find_unique(
        where={
            "team_id": team_id,
...
) -> List[LOVE_ENGINE_ObjectPermissionTable]:
    """
...
async def delete_mcp_server(
    ...
    if deleted_server is not None:
        for model, label in (
            (prisma_client.db.love_engine_mcpusercredentials, "credential"),
            (prisma_client.db.love_engine_mcpuserenvvars, "env var"),
        ):
            try:
...
def _decode_user_credential(stored: str) -> Optional[str]:
    """Read back a value persisted in ``LOVE_ENGINE_MCPUserCredentials.credential_b64``.
...
async def rotate_mcp_user_credentials_master_key(
    ...
    """Re-encrypt every ``LOVE_ENGINE_MCPUserCredentials`` row with ``new_master_key``.
...
async def rotate_mcp_user_env_vars_master_key(
    ...
    """Re-encrypt every ``LOVE_ENGINE_MCPUserEnvVars`` row with ``new_master_key``.
    ...
    """
    rows = await prisma_client.db.love_engine_mcpuserenvvars.find_many()
    rotated = 0
    skipped = 0
    for row in rows:
        ...
        await prisma_client.db.love_engine_mcpuserenvvars.update(
            where={
                "user_id_server_id": {
                    "user_id": row.user_id,
                    "server_id": row.server_id,
...
async def approve_mcp_server(
    ...
    table = LOVE_ENGINE_MCPServerTable(**updated.model_dump())
    decrypt_global_env_var_values(table.env_vars)
    return table
...
async def get_mcp_submissions(
    ...
    items = [LOVE_ENGINE_MCPServerTable(**r.model_dump()) for r in rows]
    for item in items:
...
def _decode_user_env_vars(stored: str) -> Dict[str, str]:
    ...
    if decrypted is None:
        if stored:
            verbose_proxy_logger.warning(
                "MCP per-user env vars failed to decrypt (LOVE_ENGINE_SALT_KEY "
...
async def get_user_env_vars(
    ...
    row = await prisma_client.db.love_engine_mcpuserenvvars.find_unique(
        where={"user_id_server_id": {"user_id": user_id, "server_id": server_id}}
    )
    if row is None:
        return {}
...
async def merge_user_env_vars(
    ...
    async with prisma_client.db.tx() as tx:
        ...
        row = await tx.love_engine_mcpuserenvvars.find_unique(
            where={"user_id_server_id": {"user_id": user_id, "server_id": server_id}}
        )
        ...
        await tx.love_engine_mcpuserenvvars.upsert(
            where={"user_id_server_id": {"user_id": user_id, "server_id": server_id}},
            data={
                "create": {
                    "user_id": user_id,
...
async def delete_user_env_vars(
    ...
    """
    await prisma_client.db.love_engine_mcpuserenvvars.delete_many(
