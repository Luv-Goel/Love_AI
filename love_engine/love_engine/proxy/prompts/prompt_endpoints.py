...
from love_engine.proxy.auth.auth_utils import is_request_body_safe
from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth
from love_engine.proxy.common_utils.path_utils import safe_filename
from love_engine.repositories.table_repositories import PromptRepository
from love_engine.types.prompts.init_prompts import (
    ListPromptsResponse,
    PromptInfo,
    PromptInfoResponse,
    PromptLoveEngineParams,
    PromptSpec,
    PromptTemplateBase,
)
from love_engine.types.proxy.prompt_endpoints import TestPromptRequest
...
def create_versioned_prompt_spec(db_prompt) -> PromptSpec:
    ...
    from love_engine.types.prompts.init_prompts import PromptLoveEngineParams
    ...
    # Parse love_engine_params
    love_engine_params_data = prompt_dict.get("love_engine_params")
    if isinstance(love_engine_params_data, str):
        love_engine_params_data = json.loads(love_engine_params_data)
    love_engine_params = PromptLoveEngineParams(**love_engine_params_data)
...
class PatchPromptRequest(BaseModel):
    love_engine_params: Optional[PromptLoveEngineParams] = None
    prompt_info: Optional[PromptInfo] = None
...
async def list_prompts(
    ...
    👉 [Prompt docs](https://docs.love_engine.ai/docs/proxy/prompt_management)
        ...
        if prompts is not None:
            ...
            for original_prompt in all_prompts:
                ...
                prompt_copy = PromptSpec(
                    ...
                    love_engine_params=original_prompt.love_engine_params,
    ...
    if prisma_client is not None:
        ...
        for db_prompt in db_prompts:
            ...
            versioned_prompts.append(
                PromptSpec(
                    ...
                    love_engine_params=spec.love_engine_params,
        ...
        for prompt in prompt_versions:
            ...
            versioned_prompts.append(
                PromptSpec(
                    prompt_id=base_prompt_id,
                    love_engine_params=prompt.love_engine_params,
...
def _get_prompt_template(
    ...
    try:
        dotprompt_content = prompt_spec.love_engine_params.dotprompt_content
        if dotprompt_content:
            ...
            if parsed:
                return PromptTemplateBase(
                    love_engine_prompt_id=base_prompt_id,
            ...
            if prompt_callback is not None:
                ...
                if integration_name == "dotprompt":
                    from love_engine.integrations.dotprompt.dotprompt_manager import (
                        DotpromptManager,
                    )
                    ...
                    if isinstance(prompt_callback, DotpromptManager):
                        ...
                        if template is not None and len(template) == 1:
                            ...
                            return PromptTemplateBase(
                                love_engine_prompt_id=template_id,
...
async def get_prompt_info(
    ...
    prompt_spec_response = PromptSpec(
        ...
        love_engine_params=prompt_spec.love_engine_params,
    ...
    try:
        ...
        prompt_db_entry = await PromptRepository(prisma_client).table.create(
            data={
                ...
                "love_engine_params": request.love_engine_params.model_dump_json(),
                "prompt_info": (
...
async def patch_prompt(
    ...
    - love_engine_params: LoveEngine parameters for the prompt
    ...
    try:
        ...
        if existing_prompt:
            current_love_engine_params = existing_prompt.love_engine_params
            current_prompt_info = existing_prompt.prompt_info
        else:
            current_spec = create_versioned_prompt_spec(db_prompt=target_row)
            current_love_engine_params = current_spec.love_engine_params
            current_prompt_info = current_spec.prompt_info

        # Update fields if provided
        updated_love_engine_params = (
            request.love_engine_params
            if request.love_engine_params is not None
            else current_love_engine_params
        )
        ...
        # Ensure we have valid love_engine_params
        if updated_love_engine_params is None:
            raise HTTPException(status_code=400, detail="love_engine_params cannot be None")

        # Build update data dict
        update_data: Dict[str, Any] = {
            "love_engine_params": updated_love_engine_params.model_dump_json(),
...
async def test_prompt(
    ...
    from love_engine.integrations.dotprompt.dotprompt_manager import DotpromptManager
    from love_engine.integrations.dotprompt.prompt_manager import (
        PromptManager,
        PromptTemplate,
    )
...
async def convert_prompt_file_to_json(
    ...
    from love_engine.integrations.dotprompt.prompt_manager import PromptManager
