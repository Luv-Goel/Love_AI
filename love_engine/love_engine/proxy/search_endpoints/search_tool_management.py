...
from love_engine.proxy.search_endpoints.search_tool_registry import SearchToolRegistry
from love_engine.types.search import (
    ListSearchToolsResponse,
    SearchTool,
    SearchToolInfoResponse,
)
from love_engine.types.utils import SearchProviders
...
async def list_search_tools(
    ...
    try:
        ...
        for config_search_tool in config_search_tools:
            ...
            if tool_name:
                love_engine_params_dict = dict(config_search_tool.get("love_engine_params", {}))
                ...
                search_tool_configs.append(
                    SearchToolInfoResponse(
                        search_tool_id=None,
                        search_tool_name=tool_name,
                        love_engine_params=masked_love_engine_params_dict,
                        search_tool_info=(
        ...
        for db_search_tool in search_tools_from_db:
            love_engine_params_dict = dict(db_search_tool.get("love_engine_params", {}))
            masked_love_engine_params_dict = _get_masked_values(
                love_engine_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
...
async def get_search_tool_info(search_tool_id: str):
    ...
    try:
        ...
        # Mask sensitive data
        love_engine_params_dict = dict(result.get("love_engine_params", {}))
        masked_love_engine_params_dict = _get_masked_values(
            love_engine_params_dict,
            unmasked_length=4,
            number_of_asterisks=4,
...
async def test_search_tool_connection(request: TestSearchToolConnectionRequest):
    ...
    """
    try:
        from love_engine.search import asearch

        # Extract params from request
        love_engine_params = request.love_engine_params
        search_provider = love_engine_params.get("search_provider")
        api_key = love_engine_params.get("api_key")
        api_base = love_engine_params.get("api_base")

        if not search_provider:
            raise HTTPException(
                status_code=400, detail="search_provider is required in love_engine_params"
            )
