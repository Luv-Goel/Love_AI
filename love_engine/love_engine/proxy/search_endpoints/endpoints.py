...
async def search(
    ...
    ```bash
    curl -X POST "http://localhost:4000/v1/search/love_engine-search" \
    ...
    # add_love_engine_data_to_request() also injects these values, but this keeps
        ...
        if llm_router is not None and hasattr(llm_router, "search_tools"):
            for tool in llm_router.search_tools:
                tool_info = {
                    ...
                    "search_provider": tool.get("love_engine_params", {}).get(
                        "search_provider"
                    ),
                }
