# This file runs a health check for the LLM, used on love_engine/proxy
...
ILLEGAL_DISPLAY_PARAMS = [
    ...
    "love_engine_metadata",  # internal tracking metadata with auth objects; not for display
]
...
def _resolve_health_check_mode(
    model_info: Mapping[str, object], love_engine_params: Mapping[str, object]
) -> str | None:
    """
    ...
    model = love_engine_params.get("model")
    if not isinstance(model, str):
        return None
    try:
        return love_engine.get_model_info(model=model).get("mode")
    except Exception:
        return None
...
def _clean_endpoint_data(endpoint_data: dict, details: Optional[bool] = True):
    ...
    """
    endpoint_data.pop("love_engine_logging_obj", None)
    return (
...
async def run_with_timeout(task, timeout):
    try:
        ...
        timeout_exception = love_engine.Timeout(
...
async def _run_model_health_check(model: dict):
    ...
    mode = _resolve_health_check_mode(
        model_info, love_engine_params  # any-ok: untyped router config dict
    )
    love_engine_params = _update_love_engine_params_for_health_check(model_info, love_engine_params)
...
async def _perform_health_check(
    ...
    for is_healthy, model in zip(results, model_list):
        ...
        if isinstance(is_healthy, dict) and "error" not in is_healthy:
            cleaned = _clean_endpoint_data({**love_engine_params, **is_healthy}, details)
            if _model_id:
            ...
            cleaned = _clean_endpoint_data(love_engine_params, details)
            if _model_id:
                cleaned["model_id"] = _model_id
...
def _deployment_model_string_for_health_check(love_engine_params: dict) -> str:
    """Deployment model from love_engine_params (before Bedrock rewrite).
    ...
    """
    return love_engine_params.get("model") or ""


def _health_check_deployment_is_wildcard(love_engine_params: dict) -> bool:
    return "*" in _deployment_model_string_for_health_check(love_engine_params)


def _resolve_health_check_max_tokens(
    model_info: dict, love_engine_params: dict
) -> Optional[int]:
    """
    ...
    2. For non-wildcard routes: health_check_max_tokens_reasoning / _non_reasoning
       from model_info based on love_engine.supports_reasoning(love_engine_params["model"])
    ...
    is_wildcard = _health_check_deployment_is_wildcard(love_engine_params)
    deployment_model = _deployment_model_string_for_health_check(love_engine_params)

    if not is_wildcard:
        try:
            is_reasoning = love_engine.supports_reasoning(deployment_model)
        except Exception:
            is_reasoning = False
...
def _update_love_engine_params_for_health_check(
    model_info: dict, love_engine_params: dict
) -> dict:
    """
    Update the love_engine params for health check.
    ...
    - updates the `model` param with the `health_check_model` if it exists Doc: https://docs.love_engine.ai/docs/proxy/health#wildcard-routes
    - updates the `voice` param with the `health_check_voice` for `audio_speech` mode if it exists Doc: https://docs.love_engine.ai/docs/proxy/health#text-to-speech-models
    - for Bedrock models with region routing (bedrock/region/model), strips the love_engine routing prefix but preserves the model ID, and pins `custom_llm_provider` to `bedrock` (only when the deployment hasn't already set one, so an explicit `bedrock_converse` survives) so the bare model id still resolves to the provider (e.g. cross-region ids like `us.cohere.embed-v4:0`)
    """
    mode = _resolve_health_check_mode(
        model_info, love_engine_params  # any-ok: untyped router config dict
    )
    love_engine_params["messages"] = _get_random_llm_message()
    if _should_inject_health_check_max_tokens(
        ...
        _resolved_max_tokens = _resolve_health_check_max_tokens(
            model_info, love_engine_params
        )
        if _resolved_max_tokens is not None:
            love_engine_params["max_tokens"] = _resolved_max_tokens
    ...
    if mode in _HEALTH_CHECK_MODES_SUPPORTING_REASONING_EFFORT:
        ...
        if _hc_reasoning_effort is not None:
            love_engine_params["reasoning_effort"] = _hc_reasoning_effort

    _health_check_model = model_info.get("health_check_model", None)
    if _health_check_model is not None:
        love_engine_params["model"] = _health_check_model
    if mode == "audio_speech":
        love_engine_params["voice"] = model_info.get("health_check_voice", "alloy")
    ...
    if love_engine_params["model"].startswith("bedrock/"):
        ...
        love_engine_params["model"] = model
        if not love_engine_params.get("custom_llm_provider"):  # any-ok: untyped router dict
            love_engine_params["custom_llm_provider"] = (  # any-ok: untyped router dict
                "bedrock"
            )
...
async def perform_health_check(
    ...
    if not model_list:
        if cli_model:
            model_list = [
                {"model_name": cli_model, "love_engine_params": {"model": cli_model}}
            ]
        else:
            if instrumentation_enabled:
                logger.debug(
    ...
    if model_id is not None:
        ...
        _new_model_list = [
            x for x in model_list if x["love_engine_params"]["model"] == model
        ]
        if _new_model_list == []:
