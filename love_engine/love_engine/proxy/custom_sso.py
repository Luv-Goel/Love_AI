...
Use this if you want to run custom code after love_engine has retrieved information from your IDP (Identity Provider).

Flow:
- User lands on Admin UI
- LoveEngine redirects user to your SSO provider
- Your SSO provider redirects user back to LoveEngine
- LoveEngine has retrieved user information from your IDP
...
from love_engine.proxy._types import LoveEngineUserRoles, SSOUserDefinedValues
...
async def custom_sso_handler(userIDPInfo: OpenID) -> SSOUserDefinedValues:
    try:
        ...
        # check if user exists in love_engine proxy DB
        ...
        return SSOUserDefinedValues(
            ...
            user_role=LoveEngineUserRoles.INTERNAL_USER.value,
            max_budget=10,
            budget_duration="1d",
        )
    except Exception:
