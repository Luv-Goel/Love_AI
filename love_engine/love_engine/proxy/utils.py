...
from love_engine.constants import (
    ...
    LOVE_ENGINE_LOGGING_NO_UPSTREAM_LLM_CALL,
    MAX_TEAM_LIST_LIMIT,
)
...
from love_engine.proxy.spend_tracking.spend_log_error_logger import spend_log_error
from love_engine.types.guardrails import GuardrailEventHooks
from love_engine.types.proxy.model_listing import ModelInfoResponse
...
import love_engine.love_engine_core_utils.love_engine_logging
...
from love_engine._logging import _redact_string, verbose_proxy_logger
from love_engine._service_logger import ServiceLogging, ServiceTypes
from love_engine.caching.caching import DualCache, RedisCache
from love_engine.caching.dual_cache import LimitedSizeOrderedDict
from love_engine.exceptions import RejectedRequestError, SensitiveDataRouteException
from love_engine.integrations.custom_guardrail import (
    CustomGuardrail,
    ModifyResponseException,
)
from love_engine.proxy.hooks.sensitive_data_routing import (
...
from love_engine.integrations.SlackAlerting.utils import _add_langfuse_trace_id_to_alert
...
from love_engine.love_engine_core_utils.safe_json_loads import safe_json_loads
...
from love_engine.proxy.auth.route_checks import RouteChecks
from love_engine.proxy.common_utils.user_api_key_cache import UserApiKeyCache
from love_engine.proxy.db.create_views import (
    create_missing_views,
    should_create_missing_views,
)
from love_engine.proxy.db.db_spend_update_writer import DBSpendUpdateWriter
from love_engine.proxy.db.exception_handler import (
    PrismaDBExceptionHandler,
    call_with_db_reconnect_retry,
)
from love_engine.proxy.db.log_db_metrics import log_db_metrics
from love_engine.proxy.db.prisma_client import (
    PrismaWrapper,
    parse_iam_endpoint_from_url,
)
from love_engine.proxy.db.routing_prisma_wrapper import RoutingPrismaWrapper
from love_engine.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
    UnifiedLLMGuardrails,
)
from love_engine.proxy.hooks import PROXY_HOOKS, get_proxy_hook
from love_engine.proxy.hooks.cache_control_check import _PROXY_CacheControlCheck
from love_engine.proxy.hooks.max_budget_limiter import _PROXY_MaxBudgetLimiter
from love_engine.proxy.hooks.parallel_request_limiter import (
    _PROXY_MaxParallelRequestsHandler,
)
from love_engine.proxy.hooks.parallel_request_limiter_v3 import (
    _PROXY_MaxParallelRequestsHandler_v3,
)
from love_engine.proxy.love_engine_pre_call_utils import LoveEngineProxyRequestSetup
from love_engine.proxy.policy_engine.pipeline_executor import PipelineExecutor
...
from love_engine.types.integrations.slack_alerting import DEFAULT_ALERT_TYPES
from love_engine.types.mcp import (
    MCPDuringCallResponseObject,
    MCPPreCallRequestObject,
    MCPPreCallResponseObject,
)
from love_engine.types.proxy.policy_engine.pipeline_types import PipelineExecutionResult
from love_engine.types.utils import LLMResponseTypes, LoggedLoveEngineParams
...
def print_verbose(print_statement):
    """
    Prints the given `print_statement` to the console if `love_engine.set_verbose` is True.
    ...
    if love_engine.set_verbose:
        print(f"LoveEngine Proxy: {_redact_string(str(print_statement))}")  # noqa: T201
...
class InternalUsageCache:
    ...
    async def async_get_cache(
        self,
        key,
        love_engine_parent_otel_span: Union[Span, None],
        ...
        **kwargs,
        ...
        return await self.dual_cache.async_get_cache(
            ...
            parent_otel_span=love_engine_parent_otel_span,
            **kwargs,
        )
    ...
    async def async_set_cache(
        ...
        **kwargs,
        ...
        return await self.dual_cache.async_set_cache(
            ...
            value=value,
            local_only=local_only,
            love_engine_parent_otel_span=love_engine_parent_otel_span,
            **kwargs,
        )
...
def _accepts_love_engine_call_info(cb: CustomLogger) -> bool:
    key = id(type(cb))
    if key not in _CALLBACK_ACCEPTS_CALL_INFO:
        sig = inspect.signature(cb.async_post_call_response_headers_hook)
        _CALLBACK_ACCEPTS_CALL_INFO[key] = "love_engine_call_info" in sig.parameters
...
class _CallbackCapabilities:
    """Cached per-hook capability flags derived from ``love_engine.callbacks``.
    ...
    # Ordered the same as ``love_engine.callbacks``; used to build the streaming
    ...
    # every string entry in ``love_engine.callbacks``.
...
class ProxyLogging:
    ...
    def __init__(
        ...
        ## INITIALIZE  LOVE_ENGINE CALLBACKS ##
    ...
    def startup_event(
        ...
        self._init_love_engine_callbacks(
            llm_router=llm_router
        )  # INITIALIZE LOVE_ENGINE CALLBACKS ON SERVER STARTUP <- do this to catch any logging errors on startup, not when calls are being made
        ...
        if updated_slack_alerting is True:
            ...
            if self.alerting is not None and "slack" in self.alerting:
                ...
                if (
                    ...
                    love_engine.logging_callback_manager.add_love_engine_callback(self.slack_alerting_instance)  # type: ignore
    ...
    def _add_proxy_hooks(self, llm_router: Optional[Router] = None):
        """
        Add proxy hooks to love_engine.callbacks
        """
        ...
        for hook in PROXY_HOOKS:
            ...
            love_engine.logging_callback_manager.add_love_engine_callback(proxy_hook_obj)
    ...
    def _init_love_engine_callbacks(self, llm_router: Optional[Router] = None):
        self._add_proxy_hooks(llm_router)
        love_engine.logging_callback_manager.add_love_engine_callback(self.service_logging_obj)  # type: ignore
        ...
        # love_engine.callbacks which caused double-counting of metrics.
        string_callbacks_to_replace: Dict[int, CustomLogger] = {}

        for idx, callback in enumerate(love_engine.callbacks):
            if isinstance(callback, str):
                initialized_callback = love_engine.love_engine_core_utils.love_engine_logging._init_custom_logger_compatible_class(
        ...
        # Replace string entries in love_engine.callbacks with initialized instances
        for idx, initialized_callback in string_callbacks_to_replace.items():
            love_engine.callbacks[idx] = initialized_callback

        # Fan ``love_engine.callbacks`` (the "all events" registry) out into the
        ...
        # notably pass-through endpoints — read ``love_engine._async_success_callback``
    ...
    async def update_request_status(
        self, love_engine_call_id: str, status: Literal["success", "fail"]
    ):
        ...
        await self.internal_usage_cache.async_set_cache(
            key="request_status:{}".format(love_engine_call_id),
            value=status,
            local_only=True,
            ttl=alerting_threshold,
            love_engine_parent_otel_span=None,
        )
    ...
    def _convert_mcp_to_llm_format(self, request_obj, kwargs: dict) -> dict:
        ...
        """
        from love_engine.types.llms.openai import ChatCompletionUserMessage
    ...
    def _convert_llm_result_to_mcp_response(
        ...
        """
        from love_engine.types.mcp import MCPPreCallResponseObject
    ...
    def _create_mcp_request_object_from_kwargs(
        ...
        from love_engine.types.mcp import MCPPreCallRequestObject
    ...
    async def _process_guardrail_callback(
        ...
        try:
            ...
            for prom_callback in love_engine.callbacks:
    ...
    async def _process_prompt_template(
        ...
        from love_engine.proxy.prompts.prompt_endpoints import (
        ...
        from love_engine.utils import get_non_default_completion_params
        ...
        love_engine_prompt_id: Optional[str] = None
        if prompt_spec is not None:
            love_engine_prompt_id = prompt_spec.love_engine_params.prompt_id
        ...
        if custom_logger and prompt_spec is not None:
            (
                ...
                prompt_id=love_engine_prompt_id,
    ...
    def _process_guardrail_metadata(self, data: dict) -> None:
        ...
        metadata_love_engine = data.get("love_engine_metadata") or {}
        ...
        elif isinstance(metadata_love_engine, dict) and "guardrails" in metadata_love_engine:
            guardrails_in_metadata = metadata_love_engine.get("guardrails", [])
        ...
        if guardrails_in_metadata and isinstance(guardrails_in_metadata, list):
            ...
            if (
                ...
                isinstance(metadata_love_engine, dict)
                and "applied_guardrails" in metadata_love_engine
            ):
                applied_guardrails = metadata_love_engine.get("applied_guardrails", [])
    ...
    async def _maybe_execute_pipelines(
        ...
        """
        metadata = data.get("metadata", data.get("love_engine_metadata", {})) or {}
    ...
    async def pre_call_hook(
        ...
        love_engine_logging_obj = cast(
            Optional["LoveEngineLoggingObj"], data.get("love_engine_logging_obj", None)
        )
        ...
        if (
            love_engine_logging_obj is not None
        ...
        try:
            ...
            # ``love_engine.callbacks`` overrides ``async_pre_call_hook`` and no
    ...
    # love_engine.callbacks (length + each item's id) so we recompute when the
    ...
    def _callback_capabilities() -> "_CallbackCapabilities":
        """
        Inspect ``love_engine.callbacks`` once and answer the per-hook capability
        questions used to short-circuit no-op work on the chat-completions hot
        path. Per-request callers iterated ``love_engine.callbacks`` and called
        ...
        """
        callbacks = love_engine.callbacks
        ...
        for callback in callbacks:
            if isinstance(callback, str):
                resolved: Any = (
                    love_engine.love_engine_core_utils.love_engine_logging.get_custom_logger_compatible_class(
                        cast(_custom_logger_compatible_callbacks_literal, callback)
                    )
                )
            else:
    ...
    async def during_call_hook(
        ...
        # ``love_engine.callbacks`` plus an ``asyncio.gather([])`` round trip on
    ...
    async def alerting_handler(
        ...
        """
        Alerting based on thresholds: - https://github.com/BerriAI/love_engine/issues/1298
        ...
        for client in self.alerting:
            if client == "slack":
                ...
                if love_engine.utils.sentry_sdk_instance is not None:
                    love_engine.utils.sentry_sdk_instance.capture_message(formatted_message)
                else:
    ...
    async def failure_handler(
        ...
        if love_engine.utils.capture_exception:
            love_engine.utils.capture_exception(error=original_exception)
    ...
    async def post_call_failure_hook(
        ...
        await self.update_request_status(
            love_engine_call_id=request_data.get("love_engine_call_id", ""), status="fail"
        )
        if AlertType.llm_exceptions in self.alert_types and not isinstance(
            ...
            Related issue - https://github.com/BerriAI/love_engine/issues/3395
            """
            love_engine_debug_info = getattr(original_exception, "love_engine_debug_info", None)
            exception_str = str(original_exception)
            if love_engine_debug_info is not None:
                exception_str += love_engine_debug_info
        ...
        _logging_obj = request_data.get("love_engine_logging_obj")
        ...
        request_data.pop("love_engine_logging_obj", None)
        ...
        for callback in love_engine.callbacks:
            try:
                ...
                if isinstance(callback, str):
                    _callback = love_engine.love_engine_core_utils.love_engine_logging.get_custom_logger_compatible_class(
                        cast(_custom_logger_compatible_callbacks_literal, callback)
                    )
                else:
    ...
    async def _handle_logging_proxy_only_error(
        ...
        """
        Handle logging for proxy only errors by calling `love_engine_logging_obj.async_failure_handler`

        Is triggered when self._is_proxy_only_error() returns True
        """
        love_engine_logging_obj: Optional[Logging] = request_data.get(
            "love_engine_logging_obj", None
        )
        if love_engine_logging_obj is None:
            from love_engine._uuid import uuid

            request_data["love_engine_call_id"] = str(uuid.uuid4())
            user_api_key_logged_metadata = (
                LoveEngineProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )

            love_engine_logging_obj, data = love_engine.utils.function_setup(
                original_function=route or "IGNORE_THIS",
                rules_obj=love_engine.utils.Rules(),
        ...
        if love_engine_logging_obj is not None:
            ...
            love_engine_param_keys = LoggedLoveEngineParams.__annotations__.keys()
            for k, v in request_data.items():
                if k in love_engine_param_keys:
                    _love_engine_params[k] = v
                elif k not in ("model", "user", "love_engine_logging_obj"):
            ...
            if "messages" in request_data and isinstance(
                ...
                love_engine_logging_obj.model_call_details["messages"] = input
                if love_engine_logging_obj.call_type != CallTypes.pass_through.value:
                ...
                love_engine_logging_obj.model_call_details["prompt"] = input
                if love_engine_logging_obj.call_type != CallTypes.pass_through.value:
                ...
                love_engine_logging_obj.model_call_details["input"] = input
                if love_engine_logging_obj.call_type != CallTypes.pass_through.value:
                    normalized_call_type = CallTypes.aembedding.value
            if normalized_call_type is not None:
                love_engine_logging_obj.call_type = normalized_call_type
                love_engine_logging_obj.model_call_details["call_type"] = (
            ...
            love_engine_logging_obj.model_call_details[
                LOVE_ENGINE_LOGGING_NO_UPSTREAM_LLM_CALL
            ] = True
            love_engine_logging_obj.pre_call(
                input=input,
                api_key="",
            )

            # log the custom exception
            await love_engine_logging_obj.async_failure_handler(
                exception=original_exception,
                traceback_exception=traceback.format_exc(),
            )

            threading.Thread(
                target=love_engine_logging_obj.failure_handler,
                args=(
                    original_exception,
    ...
    async def post_call_response_headers_hook(
        ...
        try:
            # Build love_engine_call_info — normalized routing metadata for callbacks
            love_engine_call_info = self._build_love_engine_call_info(
            ...
            for callback in love_engine.callbacks:
                ...
                if _callback is not None and isinstance(_callback, CustomLogger):
                    if _accepts_love_engine_call_info(_callback):
                        result = await _callback.async_post_call_response_headers_hook(
                            data=data,
                            user_api_key_dict=user_api_key_dict,
                            response=response,
                            request_headers=request_headers,
                            love_engine_call_info=love_engine_call_info,
                        )
                    else:
                        # Backwards compat: callback doesn't accept love_engine_call_info
                        result = await _callback.async_post_call_response_headers_hook(
                            data=data,
                            user_api_key_dict=user_api_key_dict,
                            response=response,
    ...
    @staticmethod
    def _build_love_engine_call_info(data: dict, response: Any) -> Dict[str, Any]:
        """
        Build a normalized dict of routing metadata from response._hidden_params
        and data, abstracting away the metadata vs love_engine_metadata split.
        """
        hidden_params = getattr(response, "_hidden_params", {}) or {}

        # model_info: check both metadata keys (chat uses "metadata", responses uses "love_engine_metadata")
        model_info = (
            (data.get("metadata") or {}).get("model_info")
            or (data.get("love_engine_metadata") or {}).get("model_info")
            or {}
        )
    ...
    async def async_post_call_streaming_hook(
        ...
        if isinstance(response, (ModelResponse, ModelResponseStream)):
            response_str = love_engine.get_response_string(response_obj=response)
        elif isinstance(response, dict) and self.is_a2a_streaming_response(response):
            from love_engine.llms.a2a.common_utils import extract_text_from_a2a_response
    ...
    def _fire_deferred_stream_logging(request_data: dict) -> None:
        ...
        """
        logging_obj = request_data.get("love_engine_logging_obj")
    ...
    def _release_max_parallel_requests_on_disconnect(
        ...
        Scheduled fire-and-love_smitht (no await) because awaiting is not
...
async def _lookup_deprecated_key(
    ...
    try:
        deprecated_row = await db.love_engine_deprecatedverificationtoken.find_first(
            where={
                "token": hashed_token,
                "revoke_at": {"gt": now},
            }
...
# DualCache for LOVE_ENGINE_Config param_name reads.
# Redis layer is attached in proxy_server._init_cache.
LOVE_ENGINE_CONFIG_CACHE_TTL_SECONDS: int = int(
    os.environ.get("LOVE_ENGINE_CONFIG_PARAM_CACHE_TTL_SECONDS", "60")
)
_CONFIG_CACHE_MISS: str = "__love_engine_config_param_miss__"

love_engine_config_cache: DualCache = DualCache(
    default_in_memory_ttl=LOVE_ENGINE_CONFIG_CACHE_TTL_SECONDS,
    default_redis_ttl=LOVE_ENGINE_CONFIG_CACHE_TTL_SECONDS,
)


class _ConfigRow:
    """Mimics the Prisma love_engine_config row shape for cached entries."""
...
def _config_cache_key(param_name: str) -> str:
    return f"love_engine_config:param:{param_name}"
...
async def get_config_param(prisma_client: Any, param_name: str) -> Optional[Any]:
    """Cached read of a LOVE_ENGINE_Config row; returns row, _ConfigRow shim, or None."""
    cache_key = _config_cache_key(param_name)
    cached = await love_engine_config_cache.async_get_cache(cache_key)
    if cached is not None:
    ...
    await love_engine_config_cache.async_set_cache(
        cache_key, cache_value, ttl=LOVE_ENGINE_CONFIG_CACHE_TTL_SECONDS
    )
    return row


async def invalidate_config_param(param_name: str) -> None:
    """Evict from both cache layers; call after every LOVE_ENGINE_Config write."""
    await love_engine_config_cache.async_delete_cache(_config_cache_key(param_name))


async def prefetch_config_params(prisma_client: Any, param_names: List[str]) -> None:
    """Batch-load LOVE_ENGINE_Config rows into the cache with one find_many."""
    if not param_names:
        return
    try:
    ...
    for name in param_names:
        ...
        await love_engine_config_cache.async_set_cache(
            _config_cache_key(name), cache_value, ttl=LOVE_ENGINE_CONFIG_CACHE_TTL_SECONDS
        )
...
class PrismaClient:
        ...
        if read_replica_url:
            try:
                ...
                if iam_flag and reader_iam_endpoint is not None:
                    from love_engine.proxy.auth.rds_iam_token import (
    ...
    async def check_view_exists(self):
        """
        Checks if the LOVE_ENGINE_VerificationTokenView and MonthlyGlobalSpend exists in the user's db.

        LOVE_ENGINE_VerificationTokenView: This view is used for getting the token + team data in user_api_key_auth
        ...
        try:
            ...
            required_view = "LOVE_ENGINE_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    await self.db.execute_raw("""
                            CREATE VIEW "LOVE_ENGINE_VerificationTokenView" AS
                            SELECT
                            v.*,
                            ...
                            FROM "LOVE_ENGINE_VerificationToken" v
                            LEFT JOIN "LOVE_ENGINE_TeamTable" t ON v.team_id = t.team_id;
                        """)

                    verbose_proxy_logger.info(
                        "LOVE_ENGINE_VerificationTokenView Created in DB!"
                    )
                else:
                    ...
                    if should_create_views:
                        ...
                        verbose_proxy_logger.warning(
                            "\n\n\033[93mNot all views exist in db, needed for UI 'Usage' tab. Missing={}.\nRun 'create_views.py' from https://github.com/BerriAI/love_engine/tree/main/db_scripts to create missing views.\033[0m\n".format(
                                missing_views
                            )
                        )
    ...
    async def get_generic_data(
        ...
        try:
            ...
            error_msg = f"LoveEngine Prisma Client Exception get_generic_data: {str(e)}"
    ...
    async def get_data(
        ...
        try:
            ...
            if (token is not None and table_name is None) or (
                ...
                if query_type == "find_unique":
                    ...
                    if expires is not None:
                        ...
                        FROM
                            "LOVE_ENGINE_UserTable" u
                        LEFT JOIN "LOVE_ENGINE_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if query_type == "find_unique":
                    response = await TeamRepository(self).table.find_unique(
                        ...
                        include={"love_engine_model_table": True},  # type: ignore
                    )
                elif query_type == "find_all" and reset_at is not None:
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = """
                        SELECT 
                            ...
                            b.max_budget AS love_engine_budget_table_max_budget,
                            b.tpm_limit AS love_engine_budget_table_tpm_limit,
                            b.rpm_limit AS love_engine_budget_table_rpm_limit,
                            b.model_max_budget as love_engine_budget_table_model_max_budget,
                            b.soft_budget as love_engine_budget_table_soft_budget,
                        ...
                        FROM "LOVE_ENGINE_VerificationToken" AS v
                        LEFT JOIN "LOVE_ENGINE_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "LOVE_ENGINE_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "LOVE_ENGINE_BudgetTable" AS b_tm ON tm.budget_id = b_tm.budget_id
                        LEFT JOIN "LOVE_ENGINE_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "LOVE_ENGINE_BudgetTable" AS b ON v.budget_id = b.budget_id
                        LEFT JOIN "LOVE_ENGINE_ProjectTable" AS p ON v.project_id = p.project_id
                        LEFT JOIN "LOVE_ENGINE_OrganizationTable" AS o ON v.organization_id = o.organization_id
                        LEFT JOIN "LOVE_ENGINE_BudgetTable" AS b2 ON o.budget_id = b2.budget_id
                        WHERE v.token = $1
                    """
                    ...
                    if (
                        ...
                        if active_token_id:
                            ...
                            # LOVE_ENGINE_VerificationTokenView; the dict
                    ...
                    if response is not None:
                        ...
                        response = LOVE_ENGINE_VerificationTokenView(
            ...
            prisma_query_info = f"LoveEngine Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
    ...
    async def insert_data(
        ...
        try:
            ...
            if table_name == "key":
                ...
                print_verbose(
                    "PrismaClient: Before upsert into love_engine_verificationtoken"
                )
                ...
                try:
                    ...
                    if (
                        "Foreign key constraint failed on the field: `LOVE_ENGINE_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in LOVE_ENGINE_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
            ...
            error_msg = f"LoveEngine Prisma Client Exception in insert_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
        ...
        try:
            ...
            if token is not None:
                ...
                for idx, t in enumerate(data_list):
                    ...
                    batcher.love_engine_verificationtoken.update(
                ...
                for idx, user in enumerate(data_list):
                    ...
                    batcher.love_engine_usertable.upsert(
                ...
                for enduser in data_list:
                    ...
                    batcher.love_engine_endusertable.upsert(
                ...
                for budget in data_list:
                    ...
                    batcher.love_engine_budgettable.upsert(
                ...
                for idx, team in enumerate(data_list):
                    ...
                    batcher.love_engine_teamtable.upsert(
            ...
            error_msg = f"LoveEngine Prisma Client Exception - update_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            ...
            error_msg = f"LoveEngine Prisma Client Exception - delete_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg = f"LoveEngine Prisma Client Exception connect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg = f"LoveEngine Prisma Client Exception disconnect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def _get_spend_logs_row_count(self) -> int:
        """
        Get the row count from LOVE_ENGINE_SpendLogs table using PostgreSQL system statistics.
        """
        ...
        async def _fetch_row_count() -> int:
            ...
            FROM pg_class
            WHERE oid = '"LOVE_ENGINE_SpendLogs"'::regclass;
            """
        ...
        try:
            ...
            verbose_proxy_logger.error(
                f"Error getting LOVE_ENGINE_SpendLogs row count: {e}"
            )
            return 0
    ...
    async def _set_spend_logs_row_count_in_proxy_state(self) -> None:
        """
        Set the `LOVE_ENGINE_SpendLogs`row count in proxy state.

        This is used later to determine if we should run expensive UI Usage queries.
        """
        from love_engine.proxy.proxy_server import proxy_state
...
class ProxyUpdateSpend:
                ...
                async with prisma_client.db.tx(
                    ...
                    async with transaction.batch_() as batcher:
                        ...
                        for end_user_id, response_cost in sorted(
                            ...
                            if love_engine.max_end_user_budget is not None:
                                pass
                            batcher.love_engine_endusertable.upsert(
    ...
    def disable_spend_updates() -> bool:
        ...
        """
        from love_engine.proxy.proxy_server import general_settings
...
async def update_spend_logs_job(
    ...
    try:
        from love_engine.proxy.guardrails.usage_tracking import (
    ...
    try:
        from love_engine.proxy.db.spend_log_tool_index import process_spend_logs_tool_usage
...
def _raise_failed_update_spend_exception(
    ...
    error_msg = (
        f"[Non-Blocking]LoveEngine Prisma Client Exception - update spend logs: {str(e)}"
    )
    error_traceback = error_msg + "\n" + traceback.format_exc()
    end_time = time.time()
    _duration = end_time - start_time
...
def _check_and_merge_model_level_guardrails(
    ...
    model_level_guardrails = deployment.love_engine_params.get("guardrails")
...
def _premium_user_check(feature: Optional[str] = None):
    ...
    """
    from love_engine.proxy.proxy_server import premium_user

    if feature:
        detail_msg = f"This feature is only available for LoveEngine Enterprise users: {feature}. {CommonProxyErrors.not_premium_user.value}"
    else:
        detail_msg = f"This feature is only available for LoveEngine Enterprise users. {CommonProxyErrors.not_premium_user.value}"
...
def is_known_vector_store_index(index_name: str) -> bool:
    ...
    if love_engine.vector_store_index_registry is None:
        return False
    return index_name in love_engine.vector_store_index_registry.get_vector_store_indexes()
...
async def get_available_models_for_user(
    ...
    Args:
        ...
        llm_router: LoveEngine router instance
    ...
    from love_engine.proxy.management_endpoints.team_endpoints import validate_membership
...
def create_model_info_response(
    ...
    """
    from love_engine.proxy.auth.model_checks import get_all_fallbacks
