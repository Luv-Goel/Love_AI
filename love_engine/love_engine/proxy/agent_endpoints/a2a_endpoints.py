"""
A2A Protocol endpoints for LoveEngine Proxy.

Allows clients to invoke agents through LoveEngine using the A2A protocol.
The A2A SDK can point to LoveEngine's URL and invoke agents registered with LoveEngine.
"""
...
from love_engine.love_engine_core_utils.url_utils import SSRFError, validate_url
from love_engine.proxy._types import UserAPIKeyAuth
from love_engine.proxy.agent_endpoints.databricks_oauth import (
    DATABRICKS_OAUTH_PARAM,
    resolve_databricks_app_auth_header,
)
from love_engine.proxy.agent_endpoints.utils import merge_agent_headers
...
def _caller_identity_headers(user_api_key_dict: UserAPIKeyAuth) -> Dict[str, str]:
    ...
    if user_api_key_dict.user_id:
        headers["X-LoveEngine-User-Id"] = user_api_key_dict.user_id
    if user_api_key_dict.team_id:
        headers["X-LoveEngine-Team-Id"] = user_api_key_dict.team_id
    return headers
...
def _forwarding_headers(
    ...
    sanitized = (
        {
            ...
            if not k.lower().startswith("x-love_engine-")
        }
    ...
    trace_id = request_data.get("love_engine_trace_id")
    if trace_id:
        identity["X-LoveEngine-Trace-Id"] = str(trace_id)
...
def _enforce_inbound_trace_id(agent: Any, request: Request) -> None:
    """Raise 400 if agent requires x-love_engine-trace-id on inbound calls and it is missing."""
    agent_love_engine_params = agent.love_engine_params or {}
    if not agent_love_engine_params.get("require_trace_id_on_calls_to_agent"):
        return
    ...
    if not trace_id:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Agent '{agent.agent_id}' requires x-love_engine-trace-id header "
...
async def _a2a_sse_event_source(
    ...
    """
    from love_engine.llms.custom_httpx.http_handler import get_async_httpx_client
    from love_engine.types.agents import _normalize_a2a_jsonrpc_response
    from love_engine.types.llms.custom_http import httpxSpecialProvider
...
async def _handle_stream_message(
    ...
    *,
    ...
    from love_engine.a2a_protocol.main import A2A_SDK_AVAILABLE
...
async def get_agent_card(
    ...
    The URL in the agent card is rewritten to point to the LoveEngine proxy,
    so all subsequent A2A calls go through LoveEngine for logging and cost tracking.
    """
    ...
    try:
        ...
        # Copy and rewrite URL to point to LoveEngine proxy
        agent_card = {
...
async def invoke_agent_a2a(
    ...
    try:
        ...
        if isinstance(params, dict):
            # extract any love_engine params from the params - eg. 'guardrails'
            ...
            # collide with love_engine's spend-tracking ``metadata`` kwarg and
            # silently drop the caller's A2A request-level metadata.
            params_to_remove = []
            for key, value in params.items():
                if key in all_love_engine_params and key not in {"id", "metadata"}:
        ...
        # Get love_engine_params (may include custom_llm_provider for completion bridge)
        love_engine_params = agent.love_engine_params or {}
        ...
        # Set up data dict for love_engine processing
        ...
        # Add love_engine data (user_api_key, user_id, team_id, etc.)
        from love_engine.proxy.common_request_processing import (
            ProxyBaseLLMRequestProcessing,
        )
        ...
        if love_engine_params.get(DATABRICKS_OAUTH_PARAM):
            databricks_auth = await resolve_databricks_app_auth_header(love_engine_params)
            if databricks_auth:
                agent_extra_headers = {
        ...
        _agent_guardrails = love_engine_params.get("guardrails")
        ...
        if method == "message/send":
            ...
            response = LoveEngineSendMessageResponse.from_dict(
