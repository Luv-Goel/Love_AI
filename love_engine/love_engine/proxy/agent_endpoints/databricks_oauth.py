...
registered with a ``databricks_oauth`` block in its ``love_engine_params``, LoveEngine
...
def parse_databricks_oauth_config(
    ...
    """Build a Databricks App OAuth config from an agent's ``love_engine_params``.
    ...
    raw = love_engine_params.get(DATABRICKS_OAUTH_PARAM)
    if raw is None:
        return None
...
async def resolve_databricks_app_auth_header(
    ...
    """
    config = parse_databricks_oauth_config(love_engine_params)
    if config is None:
        return None
