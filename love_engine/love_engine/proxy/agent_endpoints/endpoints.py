"""
Agent endpoints for registering + discovering agents via LoveEngine.
...
from love_engine.proxy._types import CommonProxyErrors, LoveEngineUserRoles, UserAPIKeyAuth
from love_engine.proxy.a2a.agent_card import merge_agent_card
...
def _build_merged_agent_card(
    ...
    *,
    ...
    """Apply the LoveEngine-fronting merge to ``upstream_card`` for ``agent_id``."""
...
def _redact_sensitive_agent_fields(
    ...
    for agent in agents:
        ...
        if copy.love_engine_params:
            copy.love_engine_params = _get_masked_values(
                copy.love_engine_params,
                unmasked_length=4,
...
AGENT_HEALTH_CHECK_TIMEOUT_SECONDS = float(
    os.environ.get("LOVE_ENGINE_AGENT_HEALTH_CHECK_TIMEOUT", "5.0")
)
AGENT_HEALTH_CHECK_GATHER_TIMEOUT_SECONDS = float(
    os.environ.get("LOVE_ENGINE_AGENT_HEALTH_CHECK_GATHER_TIMEOUT", "30.0")
)
...
async def get_agents(
    ...
    from love_engine.proxy.agent_endpoints.auth.agent_permission_handler import (
        AgentRequestHandler,
    )
    ...
    try:
        ...
        if (
            ...
            or user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
        ):
        ...
        for agent in returned_agents:
            if agent.love_engine_params is None:
                agent.love_engine_params = {}
            agent.love_engine_params["is_public"] = (
                love_engine.public_agent_groups is not None
                and (agent.agent_id in love_engine.public_agent_groups)
            )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.agent_endpoints.get_agents(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise HTTPException(
...
from love_engine.repositories.table_repositories import AgentsRepository
...
async def create_agent(
    ...
    try:
        ...
        # Apply the LoveEngine-fronting merge only when the admin actually
...
async def update_agent(
    ...
    try:
        ...
        # Re-apply the LoveEngine-fronting merge — an update is a re-registration,
...
async def patch_agent(
    ...
    try:
        ...
        # patch updating just love_engine_params/rate limits (``agent_card_params``
        ...
        # merge so LoveEngine applies its security schemes and supported
...
async def make_agent_public(
    ...
    try:
        ...
        if love_engine.public_agent_groups is None:
            love_engine.public_agent_groups = []
        # handle duplicates
        if agent.agent_id in love_engine.public_agent_groups:
            raise HTTPException(
                status_code=400,
                detail=f"Agent with name {agent.agent_name} already in public agent groups",
            )
        love_engine.public_agent_groups.append(agent.agent_id)
        ...
        config["love_engine_settings"]["public_agent_groups"] = love_engine.public_agent_groups

        # Save the updated config
        await proxy_config.save_config(new_config=config)

        verbose_proxy_logger.debug(
            f"Updated public agent groups to: {love_engine.public_agent_groups} by user: {user_api_key_dict.user_id}"
        )

        return {
            "message": "Successfully updated public agent groups",
            "public_agent_groups": love_engine.public_agent_groups,
...
async def make_agents_public(
    ...
    try:
        ...
        love_engine.public_agent_groups = request.agent_ids
