...
def _is_admin(user_api_key_dict: UserAPIKeyAuth) -> bool:
    return user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
    ...
    for attempt in range(_MAX_SEQUENCE_RETRIES):
        try:
            ...
            async with prisma_client.db.tx() as tx:
                event = await tx.love_engine_workflowevent.create(data=event_data)
                if new_status:
                    await tx.love_engine_workflowrun.update(
                        where={"run_id": run_id},
