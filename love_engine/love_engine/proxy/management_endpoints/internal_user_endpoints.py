...
These are members of a Team on LoveEngine
...
from love_engine.proxy.auth.auth_checks import get_team_object, get_user_object
from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth
from love_engine.proxy.hooks.user_management_event_hooks import UserManagementEventHooks
from love_engine.proxy.management_endpoints.common_daily_activity import (
    get_daily_activity,
...
from love_engine.proxy.utils import handle_exception_on_proxy, hash_password
...
from love_engine.types.proxy.management_endpoints.internal_user_endpoints import (
...
if TYPE_CHECKING:
    from love_engine.proxy.proxy_server import PrismaClient
...
def _update_internal_new_user_params(data_json: dict, data: NewUserRequest) -> dict:
    ...
    if love_engine.default_internal_user_params and (
        data.user_role != LoveEngineUserRoles.PROXY_ADMIN.value
        and data.user_role != LoveEngineUserRoles.PROXY_ADMIN
    ):
        for key, value in love_engine.default_internal_user_params.items():
    ...
    if (
        ...
        and data.user_role == LoveEngineUserRoles.INTERNAL_USER.value
    ):
        if (
            love_engine.max_internal_user_budget is not None
            and data_json.get("max_budget") is None
        ):
            data_json["max_budget"] = love_engine.max_internal_user_budget

        if (
            love_engine.internal_user_budget_duration is not None
            and data_json.get("budget_duration") is None
        ):
            data_json["budget_duration"] = love_engine.internal_user_budget_duration
    ...
    try:
        ...
        if e.status_code == 400 and (
            ...
            verbose_proxy_logger.debug(
                "love_engine.proxy.management_endpoints.internal_user_endpoints.new_user(): User already exists in team - {}".format(
                    str(e)
                )
            )
        else:
            verbose_proxy_logger.debug(
                "love_engine.proxy.management_endpoints.internal_user_endpoints.new_user(): Exception occured - {}".format(
                    str(e)
                )
            )
...
def check_if_default_team_set() -> Optional[Union[List[str], List[NewUserRequestTeam]]]:
    if love_engine.default_internal_user_params is None:
        return None
    teams = love_engine.default_internal_user_params.get("teams")
...
async def new_user(
    ...
    Internal Users can access LoveEngine Admin UI to make keys, request access to models.
    ...
    - user_role: Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/BerriAI/love_engine/love_engine/proxy/_types.py#L20`
    ...
    - aliases: Optional[dict] - Model aliases for the user - [Docs](https://love_engine.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    - model_max_budget: Optional[dict] - Model-specific max budget for user. [Docs](https://docs.love_engine.ai/docs/proxy/users#add-model-specific-budgets-to-keys)
    - model_rpm_limit: Optional[float] - Model-specific rpm limit for user. [Docs](https://docs.love_engine.ai/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    - model_tpm_limit: Optional[float] - Model-specific tpm limit for user. [Docs](https://docs.love_engine.ai/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    - object_permission: Optional[LOVE_ENGINE_ObjectPermissionBase] - internal user-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"]}. IF null or {} then no object permission.
    ...
    """
    try:
        from love_engine.proxy.proxy_server import _license_check, prisma_client
        ...
        if (
            data.user_role
            in [LoveEngineUserRoles.PROXY_ADMIN, LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY]
...
async def ui_get_available_role(
    ...
    _data_to_return = {}
    for role in LoveEngineUserRoles:
...
def get_team_from_list(
    team_list: Optional[Union[List[LOVE_ENGINE_TeamTable], List[TeamListResponseObject]]],
    team_id: str,
) -> Optional[Union[LOVE_ENGINE_TeamTable, LOVE_ENGINE_TeamMembership]]:
...
async def _get_user_info_teams(
    ...
    from love_engine.proxy.management_endpoints.team_endpoints import list_team
...
def _build_user_info_response(
    user_id: Optional[str],
    user_info: Optional[Any],
    keys: Optional[List[LOVE_ENGINE_VerificationToken]],
    team_list: list[Any],
...
async def user_info(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.user_info(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def _check_user_info_v2_access(
    ...
    if user_api_key_dict.user_id is not None:
        ...
        if caller_user is not None and caller_user.teams:
            ...
            for team in teams:
                team_obj = LOVE_ENGINE_TeamTable(**team.model_dump())
...
async def user_info_v2(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.user_info_v2(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def _get_user_info_for_proxy_admin(user_api_key_dict: UserAPIKeyAuth):
    ...
    - get all teams in LOVE_ENGINE_TeamTable
    - get all keys in LOVE_ENGINE_VerificationToken table
    ...
    sql_query = """
        SELECT 
            (SELECT json_agg(t.*) FROM "LOVE_ENGINE_TeamTable" t) as teams,
            (SELECT json_agg(k.*) FROM "LOVE_ENGINE_VerificationToken" k WHERE k.team_id != 'love_engine-dashboard' OR k.team_id IS NULL) as keys
    """
    if prisma_client is None:
        raise Exception(
            "Database not connected. Connect a database to your proxy - https://docs.love_engine.ai/docs/simple_proxy#managing-auth---virtual-keys"
    ...
    # cast all keys to LOVE_ENGINE_VerificationToken
    keys_in_db = []
    for key in _keys_in_db:
        if key.get("models") is None:
            key["models"] = []
        keys_in_db.append(LOVE_ENGINE_VerificationToken(**key))

    # cast all teams to LOVE_ENGINE_TeamTable
    _teams_in_db: List = results[0]["teams"] or []
    _teams_in_db = [LOVE_ENGINE_TeamTable(**team) for team in _teams_in_db]
...
def _process_keys_for_user_info(
    keys: Optional[List[LOVE_ENGINE_VerificationToken]],
    all_teams: Optional[Union[List[LOVE_ENGINE_TeamTable], List[TeamListResponseObject]]],
):
    from love_engine.constants import UI_SESSION_TOKEN_TEAM_ID
    from love_engine.proxy.proxy_server import general_settings, love_engine_master_key_hash
    ...
    if keys is None:
        ...
        for key in keys:
            if (
                key.token == love_engine_master_key_hash
            ...
            # Filter out UI session tokens (team_id="love_engine-dashboard")
            ...
            if (
                ...
                and _key["team_id"] != "love_engine-dashboard"
            ):
...
def _update_internal_user_params(
    ...
    for k, v in data_json.items():
        if k == "max_budget":
            ...
            and k not in LOVE_ENGINE_ManagementEndpoint_MetadataFields
        ):  # models default to [], spend defaults to 0, we should not reset these values
            non_default_values[k] = v

    is_internal_user = False
    if data.user_role == LoveEngineUserRoles.INTERNAL_USER:
    ...
    if "max_budget" not in non_default_values:
        if (
            is_internal_user and love_engine.max_internal_user_budget is not None
        ):  # applies internal user limits, if user role updated
            non_default_values["max_budget"] = love_engine.max_internal_user_budget

    if (
        "budget_duration" not in non_default_values
    ):  # applies internal user limits, if user role updated
        if is_internal_user and love_engine.internal_user_budget_duration is not None:
...
async def _schedule_user_update_audit_log(
    ...
    try:
        ...
        if updated_user_row:
            user_row_typed = LOVE_ENGINE_UserTable(
            ...
            asyncio.create_task(
                UserManagementEventHooks.create_internal_user_audit_log(
                    ...
                    action="updated",
                    love_engine_changed_by=love_engine_changed_by or user_api_key_dict.user_id,
                    user_api_key_dict=user_api_key_dict,
...
def _check_user_update_authz(
    ...
    if existing_user_row is not None:
        typed_row = LOVE_ENGINE_UserTable(**existing_user_row.model_dump(exclude_none=True))
    ...
    elif user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN.value:
...
async def _update_single_user_helper(
    ...
    if existing_user_row is not None:
        existing_user_row = LOVE_ENGINE_UserTable(
...
async def user_update(
    ...
    --data '{
        "user_id": "test-love_engine-user-4",
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.user_update(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def get_users(
    ...
    # Prepare response
    user_list: List[LOVE_ENGINE_UserTableWithKeyCount] = []
    if users is not None:
        for user in users:
            user_list.append(
                LOVE_ENGINE_UserTableWithKeyCount(
    ...
    for user_id in data.user_ids:
        ...
        for team in fetch_all_teams:
            is_member_in_team, new_team_members = _cleanup_members_with_roles(
                existing_team_row=LOVE_ENGINE_TeamTable(**team.model_dump()),
...
async def add_internal_user_to_organization(
    ...
    Adds the user to LOVE_ENGINE_OrganizationMembership table
...
async def _resolve_team_org_filter(
    ...
    from love_engine.proxy.management_endpoints.common_utils import _is_user_team_admin
...
@router.get(
    ...
    include_in_schema=False,
    responses={
        200: {"model": List[LOVE_ENGINE_UserTableFiltered]},
    },
)
async def ui_view_users(
    ...
    (stored in the ``love_engine_uisettings`` table):
    ...
    * **Flag ON:**
    ...
    try:
        ...
        return [LOVE_ENGINE_UserTableFiltered(**user.model_dump()) for user in users]
...
async def get_user_daily_activity(
    ...
    try:
        ...
        return await get_daily_activity(
            prisma_client=prisma_client,
            table_name="love_engine_dailyuserspend",
