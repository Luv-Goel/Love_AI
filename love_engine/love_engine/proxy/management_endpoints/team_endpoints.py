...
from love_engine.proxy.common_utils.callback_utils import encrypt_callback_vars
...
from love_engine.proxy.management_helpers.utils import (
    add_new_member,
    management_endpoint_wrapper,
)
from love_engine.proxy.utils import PrismaClient, handle_exception_on_proxy
...
from love_engine.types.proxy.management_endpoints.common_daily_activity import (
    SpendAnalyticsPaginatedResponse,
)
from love_engine.types.proxy.management_endpoints.team_endpoints import (
...
async def _refresh_cached_team(
    ...
    Every endpoint that mutates `love_engine_teamtable` must call this so the
    cached `LOVE_ENGINE_TeamTableCachedObj` used by `common_checks` stays in
    ...
    `love_engine_teamtable`. It is converted to `LOVE_ENGINE_TeamTableCachedObj`
    via `model_dump()` to match the cache shape `_cache_team_object`
    expects.
    """
    await _cache_team_object(
        team_id=team_row.team_id,
        team_table=LOVE_ENGINE_TeamTableCachedObj(**team_row.model_dump()),
...
async def _verify_team_access(
    team_obj: LOVE_ENGINE_TeamTable,
...
class TeamMemberBudgetHandler:
    ...
    async def create_team_member_budget_table(
        data: Union[NewTeamRequest, LOVE_ENGINE_TeamTable],
        ...
        from love_engine.proxy._types import BudgetNewRequest
    ...
    async def upsert_team_member_budget_table(
        team_table: LOVE_ENGINE_TeamTable,
        user_api_key_dict: UserAPIKeyAuth,
        updated_kv: dict,
        team_member_budget: Optional[float] = None,
        team_member_rpm_limit: Optional[int] = None,
...
def _get_default_team_param(field: str) -> Any:
    """
    Returns a default value for the given field from love_engine.default_team_params config.
    ...
    """
    default_params = love_engine.default_team_params
...
def _is_available_team(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> bool:
    if love_engine.default_internal_user_params is None:
        return False
    if "available_teams" in love_engine.default_internal_user_params:
        return team_id in love_engine.default_internal_user_params["available_teams"]
    return False


async def get_all_team_memberships(
    prisma_client: PrismaClient, team_ids: List[str], user_id: Optional[str] = None
) -> List[LOVE_ENGINE_TeamMembership]:
    ...
    returned_tm: List[LOVE_ENGINE_TeamMembership] = []
    for tm in team_memberships:
        returned_tm.append(LOVE_ENGINE_TeamMembership(**tm.model_dump()))
...
async def _check_org_team_limits(
    ...
    if (
        ...
        and org_table.love_engine_budget_table is not None
        and org_table.love_engine_budget_table.max_budget is not None
        and data.max_budget > org_table.love_engine_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team max_budget ({data.max_budget}) exceeds organization's max_budget ({org_table.love_engine_budget_table.max_budget}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.tpm_limit is not None
        and org_table.love_engine_budget_table is not None
        and org_table.love_engine_budget_table.tpm_limit is not None
        and data.tpm_limit > org_table.love_engine_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team tpm_limit ({data.tpm_limit}) exceeds organization's tpm_limit ({org_table.love_engine_budget_table.tpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.rpm_limit is not None
        and org_table.love_engine_budget_table is not None
        and org_table.love_engine_budget_table.rpm_limit is not None
        and data.rpm_limit > org_table.love_engine_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team rpm_limit ({data.rpm_limit}) exceeds organization's rpm_limit ({org_table.love_engine_budget_table.rpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    # Convert teams to LOVE_ENGINE_TeamTable objects
    team_objs: List[LOVE_ENGINE_TeamTable] = []
    for team in teams:
        team_objs.append(LOVE_ENGINE_TeamTable(**team.model_dump()))
...
@router.post(
    ...
    response_model=LOVE_ENGINE_TeamTable,
)
@management_endpoint_wrapper
async def new_team(
    data: NewTeamRequest,
    ...
    👉 [Detailed Doc on setting team budgets](https://docs.love_engine.ai/docs/proxy/team_budgets)
    ...
    - budget_duration: Optional[str] - The duration of the budget for the team. Doc [here](https://docs.love_engine.ai/docs/proxy/team_budgets)
    ...
    - model_aliases: Optional[dict] - Model aliases for the team. [Docs](https://docs.love_engine.ai/docs/proxy/team_based_routing#create-team-with-model-alias)
    - guardrails: Optional[List[str]] - Guardrails for the team. [Docs](https://docs.love_engine.ai/docs/proxy/guardrails)
    - policies: Optional[List[str]] - Policies for the team. [Docs](https://docs.love_engine.ai/docs/proxy/guardrails/guardrail_policies)
    - disable_global_guardrails: Optional[bool] - Whether to disable global guardrails for the key.
    - object_permission: Optional[LOVE_ENGINE_ObjectPermissionBase] - team-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"], "agents": ["agent_1", "agent_2"], "agent_access_groups": ["dev_group"]}. IF null or {} then no object permission.
    - team_member_budget: Optional[float] - The maximum budget allocated to an individual team member.
    - team_member_budget_duration: Optional[str] - The duration of the budget for the team member. Doc [here](https://docs.love_engine.ai/docs/proxy/team_budgets)
    ...
    - secret_manager_settings: Optional[dict] - Secret manager settings for the team. [Docs](https://docs.love_engine.ai/docs/secret_managers/overview)
    ...
    try:
        ...
        # Apply defaults from love_engine.default_team_params for any fields
        ...
        if data.max_budget is None:
            if (
                isinstance(love_engine.default_team_settings, list)
                and len(love_engine.default_team_settings) > 0
                and isinstance(love_engine.default_team_settings[0], dict)
            ):
                default_budget = love_engine.default_team_settings[0].get("max_budget")
                if default_budget is not None:
                    data.max_budget = default_budget

        if (
            user_api_key_dict.user_role is None
            or user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN
        ...
        if data.model_aliases is not None and isinstance(data.model_aliases, dict):
            love_engine_modeltable = LOVE_ENGINE_ModelTable(
            ...
            model_dict = await ModelTableRepository(prisma_client).table.create(
                {**love_engine_modeltable.json(exclude_none=True)}  # type: ignore
            )  # type: ignore
        ...
        ## ADD TO TEAM TABLE
        complete_team_data = LOVE_ENGINE_TeamTable(
            **data_json,
            model_id=_model_id,
        )
        ...
        team_row: LOVE_ENGINE_TeamTable = await TeamRepository(prisma_client).table.create(
        ...
        # Enterprise Feature - Audit Logging. Enable with love_engine.store_audit_logs = True
            ...
            asyncio.create_task(
                create_audit_log_for_update(
                    request_data=LOVE_ENGINE_AuditLogs(
                        ...
                        table_name=LoveEngineTableNames.TEAM_TABLE_NAME,
...
async def _create_team_update_audit_log(
    existing_team_row: LOVE_ENGINE_TeamTable,
    updated_kv: dict,
    team_id: str,
    ...
    Args:
        ...
        love_engine_changed_by: Optional header indicating who made the change
        user_api_key_dict: User API key authentication details
        love_engine_proxy_admin_name: Name of the proxy admin
    """
    from love_engine.proxy.management_helpers.audit_logs import (
        create_audit_log_for_update,
        get_audit_log_changed_by,
...
async def _update_model_table(
    ...
    if data.model_aliases is not None and isinstance(data.model_aliases, dict):
        ...
        if model_id is None:
            model_dict = await ModelTableRepository(prisma_client).table.create(
                data={**love_engine_modeltable.json(exclude_none=True)}  # type: ignore
            )
        else:
            model_dict = await ModelTableRepository(prisma_client).table.upsert(
                where={"id": model_id},
                data={
                    "update": {**love_engine_modeltable.json(exclude_none=True)},  # type: ignore
                    "create": {**love_engine_modeltable.json(exclude_none=True)},  # type: ignore
                },
            )  # type: ignore
...
async def _auto_add_team_members_to_organization(
    ...
    organization: LOVE_ENGINE_OrganizationTableWithMembers,
    prisma_client: Any,
) -> None:
    """
        ...
        try:
            await add_member_to_organization(
                member=OrgMember(
                    ...
                    role=LoveEngineUserRoles.INTERNAL_USER,
                ),
...
async def fetch_and_validate_organization(
    ...
    organization_row = await OrganizationRepository(prisma_client).table.find_unique(
        ...
        include={"love_engine_budget_table": True, "members": True, "teams": True},
    )
    ...
    is_proxy_admin = (
        ...
        and user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN
    )
    organization = LOVE_ENGINE_OrganizationTableWithMembers(**organization_row.model_dump())
    validate_team_org_change(
        team=LOVE_ENGINE_TeamTable(**existing_team_row.model_dump()),
...
def validate_team_org_change(
    ...
    if (
        team.max_budget
        and organization.love_engine_budget_table
        and organization.love_engine_budget_table.max_budget
        and team.max_budget > organization.love_engine_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has max_budget {team.max_budget} that is greater than the organization's max_budget {organization.love_engine_budget_table.max_budget}."
            },
        )
    ...
    if (
        team.tpm_limit
        and organization.love_engine_budget_table
        and organization.love_engine_budget_table.tpm_limit
        and team.tpm_limit > organization.love_engine_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has tpm_limit {team.tpm_limit} that is greater than the organization's tpm_limit {organization.love_engine_budget_table.tpm_limit}."
            },
        )
    if (
        team.rpm_limit
        and organization.love_engine_budget_table
        and organization.love_engine_budget_table.rpm_limit
        and team.rpm_limit > organization.love_engine_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has rpm_limit {team.rpm_limit} that is greater than the organization's rpm_limit {organization.love_engine_budget_table.rpm_limit}."
            },
        )
    return True
...
async def update_team(
    ...
    try:
        ...
        await _verify_team_access(
            team_obj=LOVE_ENGINE_TeamTable(**existing_team_row.model_dump()),
            user_api_key_dict=user_api_key_dict,
        )
                ...
                caller_memberships = (
                    await OrganizationMembershipRepository(
                        ...
                        where={
                            ...
                            "user_role": LoveEngineUserRoles.ORG_ADMIN.value,
                        }
                    )
        ...
        team_row: Optional[LOVE_ENGINE_TeamTable] = await TeamRepository(
            prisma_client
        ).table.update(
            where={"team_id": data.team_id},
            data=updated_kv,
...
async def handle_update_object_permission(
    data_json: dict, existing_team_row: LOVE_ENGINE_TeamTable
) -> dict:
    """
    Handle the update of object permission for a team.

    - IF there's no object_permission_id, then create a new entry in LOVE_ENGINE_ObjectPermissionTable
    - IF there's an object_permission_id, then update the entry in LOVE_ENGINE_ObjectPermissionTable
    """
...
async def _process_team_members(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    love_engine_proxy_admin_name: str,
) -> Tuple[List[LOVE_ENGINE_UserTable], List[LOVE_ENGINE_TeamMembership]]:
...
async def _add_team_members_to_team(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    love_engine_proxy_admin_name: str,
) -> Tuple[LOVE_ENGINE_TeamTable, List[LOVE_ENGINE_UserTable], List[LOVE_ENGINE_TeamMembership]]:
...
async def team_member_add(
    ...
    complete_team_data = LOVE_ENGINE_TeamTable(**existing_team_row.model_dump())
...
async def team_member_delete(
    ...
    existing_team_row = LOVE_ENGINE_TeamTable(**_existing_team_row.model_dump())
...
async def team_member_update(
    ...
    if data.role == "admin" and not premium_user:
        ...
        raise HTTPException(
            status_code=400,
            detail="Assigning team admins is a premium feature. You must be a LoveEngine Enterprise user to use this feature. If you have a license please set `LOVE_ENGINE_LICENSE` in your env. Get a 7 day trial key here: https://www.love_engine.ai/#trial. Pricing: https://www.love_engine.ai/#pricing",
        )
    if data.user_id is None and data.user_email is None:
        raise HTTPException(
            status_code=400,
...
async def delete_team(
    ...
    team_rows: List[LOVE_ENGINE_TeamTable] = []
    for team_id in data.team_ids:
        ...
        team_row_pydantic = LOVE_ENGINE_TeamTable(**team_row_base.model_dump())
    ...
    if love_engine.store_audit_logs is True:
        ...
        for team_id in data.team_ids:
            team_row: Optional[LOVE_ENGINE_TeamTable] = await prisma_client.get_data(  # type: ignore
    ...
    keys_to_delete: List[LOVE_ENGINE_VerificationToken] = await VerificationTokenRepository(
        prisma_client
...
def _transform_teams_to_deleted_records(
    ...
    for team in teams:
        ...
        deleted_record = LOVE_ENGINE_DeletedTeamTable(
            **team_payload,
        ...
        for rel_key in (
            "love_engine_model_table",
            "object_permission",
            "id",
            "budget_limits",  # not in LOVE_ENGINE_DeletedTeamTable schema
            "default_team_member_models",  # not in LOVE_ENGINE_DeletedTeamTable schema
        ):
...
async def validate_membership(
    user_api_key_dict: UserAPIKeyAuth, team_table: LOVE_ENGINE_TeamTable
):
    if (
        user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
...
async def team_info(
    ...
    ```
    """
    from love_engine.proxy._types import TeamInfoResponseObjectTeamTable
    from love_engine.proxy.proxy_server import prisma_client
    ...
    try:
        if prisma_client is None:
            raise HTTPException(
                ...
                detail={
                    "error": "Database not connected. Connect a database to your proxy - https://docs.love_engine.ai/docs/simple_proxy#managing-auth---virtual-keys"
                },
            )
        if team_id is None:
            raise HTTPException(
        ...
        await validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=LOVE_ENGINE_TeamTable(**team_info.model_dump()),
        )
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.management_endpoints.team_endpoints.py::team_info - Exception occurred - {}\n{}".format(
...
async def team_member_me(
    ...
    return TeamMemberInfoResponse(
        ...
        love_engine_budget_table=membership.love_engine_budget_table,
    )
...
async def block_team(
    ...
    # Verify caller has access to manage this team
    await _verify_team_access(
        team_obj=LOVE_ENGINE_TeamTable(**existing_team.model_dump()),
        user_api_key_dict=user_api_key_dict,
    )
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=List[LOVE_ENGINE_TeamTable],
):
    from love_engine.proxy.proxy_server import prisma_client
    ...
    available_teams = cast(
        Optional[List[str]],
        (
            love_engine.default_internal_user_params.get("available_teams")
    ...
    user_info_correct_type = LOVE_ENGINE_UserTable(**user_info.model_dump())
    ...
    available_teams_correct_type = [
        LOVE_ENGINE_TeamTable(**team.model_dump()) for team in available_teams_db
    ]
...
async def _get_org_admin_org_ids(
    ...
    org_ids = [
        ...
        if m.user_role == LoveEngineUserRoles.ORG_ADMIN.value
...
def _convert_teams_to_response_models(
...
) -> List[Union[TeamListItem, LOVE_ENGINE_TeamTable, LOVE_ENGINE_DeletedTeamTable]]:
    """Convert raw Prisma team rows to response models."""
    team_list: List[
        Union[TeamListItem, LOVE_ENGINE_TeamTable, LOVE_ENGINE_DeletedTeamTable]
    ] = []
    ...
    for team in teams:
        ...
        if use_deleted_table:
            team_list.append(LOVE_ENGINE_DeletedTeamTable(**team_dict))
        else:
...
async def _get_keys_count_by_team(
    ...
    Runs a single GROUP BY against LOVE_ENGINE_VerificationToken. The IN clause is
...
async def _authorize_and_filter_teams(
    ...
    if allowed_org_ids is not None:
        ...
        org_teams = await TeamRepository(prisma_client).table.find_many(
            ...
            include={"love_engine_model_table": True},
        )
        if not user_id:
...
async def list_team(
    ...
    for team in filtered_response:
        _team_memberships: List[LOVE_ENGINE_TeamMembership] = []
...
async def get_paginated_teams(
    ...
    page_size: int = 10,
    page: int = 1,
) -> Tuple[List[LOVE_ENGINE_TeamTable], int]:
    """
    ...
    Returns:
        Tuple[List[LOVE_ENGINE_TeamTable], int] - (list of teams, total count)
    """
    try:
...
@router.get(
    ...
    responses={
        200: {"model": List[LOVE_ENGINE_TeamTable]},
    },
)
async def ui_view_teams(
    ...
    Returns:
        List[LOVE_ENGINE_SpendLogs]: Paginated list of matching user records
    """
    from love_engine.proxy.proxy_server import prisma_client
...
def add_new_models_to_team(
    team_obj: LOVE_ENGINE_TeamTable, new_models: List[str]
) -> List[str]:
    """
...
async def team_model_add(
    ...
    team_obj = LOVE_ENGINE_TeamTable(**team_row.model_dump())
...
async def _compute_and_batch_updates(
    ...
    if updates:
        ...
        for team_id, merged_perms in updates:
            batcher.love_engine_teamtable.update(
                where={"team_id": team_id},
...
async def get_team_daily_activity(
    ...
    if not _user_has_admin_view(user_api_key_dict) and team_ids_list and team_aliases:
        ...
        for team_alias in team_aliases:
            team_obj = LOVE_ENGINE_TeamTable(**team_alias.model_dump())
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="love_engine_dailyteamspend",
