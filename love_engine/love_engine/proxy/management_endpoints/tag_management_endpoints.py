...
from love_engine.proxy._types import UserAPIKeyAuth, user_api_key_has_admin_view
...
from love_engine.proxy.management_helpers.utils import handle_budget_for_entity
...
from love_engine.types.tag_management import (
    TagConfig,
    TagDeleteRequest,
    TagInfoRequest,
    TagNewRequest,
...
async def get_deployments_by_model(
    ...
    """
    from love_engine.types.router import Deployment, LOVE_ENGINE_Params, ModelInfo
    ...
    return [
        Deployment(
            ...
            love_engine_params=LOVE_ENGINE_Params(**deployment["love_engine_params"]),  # type: ignore
...
async def _add_tag_to_deployment(deployment: "Deployment", tag: str):
    ...
    try:
        ...
        # Prisma returns love_engine_params as dict (already parsed from JSON)
        existing_params = db_model.love_engine_params
        if isinstance(existing_params, str):
            # If it's a string, parse it
            existing_params = json.loads(existing_params)
        elif not isinstance(existing_params, dict):
            raise Exception(f"Unexpected love_engine_params type: {type(existing_params)}")
        ...
        await ModelRepository(prisma_client).table.update(
            where={"model_id": deployment.model_info.id},
            data={"love_engine_params": json.dumps(existing_params)},
        )
    except Exception as e:
...
async def info_tag(
    ...
    try:
        ...
        for tag_record in tag_records:
            ...
            if (
                hasattr(tag_record, "love_engine_budget_table")
                and tag_record.love_engine_budget_table
            ):
                tag_dict["love_engine_budget_table"] = tag_record.love_engine_budget_table
...
async def get_tag_daily_activity(
    ...
    return await get_daily_activity(
        ...
        page=page,
        page_size=page_size,
        # metadata_metrics_func=None because love_engine_dailytagspend rows are
