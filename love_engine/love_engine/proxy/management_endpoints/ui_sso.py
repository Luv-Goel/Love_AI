...
/sso/callback - returns JWT Redirect Response that redirects to LoveEngine UI
...
from love_engine.caching.dual_cache import DualCache
from love_engine.constants import (
    ...
    LOVE_ENGINE_CLI_SOURCE_IDENTIFIER,
...
from love_engine.love_engine_core_utils.dot_notation_indexing import get_nested_value
...
from love_engine.proxy.auth.auth_checks import ExperimentalUIJWTToken, get_user_object
...
from love_engine.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    show_missing_vars_in_env,
)
from love_engine.proxy.common_utils.html_forms.jwt_display_template import (
    jwt_display_template,
)
from love_engine.proxy.common_utils.html_forms.ui_login import html_form
from love_engine.proxy.common_utils.user_api_key_cache import UserApiKeyCache
from love_engine.proxy.management_endpoints.internal_user_endpoints import new_user
from love_engine.proxy.management_endpoints.sso import CustomMicrosoftSSO
from love_engine.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from love_engine.proxy.management_endpoints.team_endpoints import new_team, team_member_add
from love_engine.proxy.management_endpoints.types import (
    CustomOpenID,
    get_love_engine_user_role,
    is_valid_love_engine_user_role,
)
...
from love_engine.repositories.table_repositories import SSOConfigRepository
...
from love_engine.secret_managers.main import get_secret_bool, str_to_bool
from love_engine.types.proxy.management_endpoints.ui_sso import *  # noqa: F403
...
from love_engine.types.proxy.ui_sso import ParsedOpenIDResult
...
def _parse_cli_sso_claim_map() -> List[Tuple[str, str]]:
    """
    Parse CLI_SSO_CLAIM_MAP / LOVE_ENGINE_CLI_SSO_CLAIM_MAP.
    ...
    on the LoveEngine user's ``metadata`` JSON column.
    """
...
def build_cli_sso_attribution_metadata(
    ...
    Sources are configured via CLI_SSO_CLAIM_MAP / LOVE_ENGINE_CLI_SSO_CLAIM_MAP and
...
def _render_cli_sso_verification_page(
    ...
    <html>
      <head>
        <title>LoveEngine CLI Login</title>
        <style>
          body {{
...
async def cli_sso_start(request: Request):
    from love_engine.proxy.proxy_server import general_settings, user_api_key_cache
    ...
    verification_uri_complete: str | None = (
        (
            ...
            + urlencode(
                {
                    "source": LOVE_ENGINE_CLI_SOURCE_IDENTIFIER,
                    "key": login_id,
                    "user_code": user_code,
                }
            )
...
async def cli_sso_complete(request: Request, login_id: str):
    ...
    from love_engine.proxy.common_utils.html_forms.cli_sso_success import (
        render_cli_sso_success_page,
    )
    from love_engine.proxy.proxy_server import user_api_key_cache
        ...
        if existing_role is None:
            ...
            if user_role is None:
                ...
                if user_role_from_token is not None:
                    user_role = get_love_engine_user_role(user_role_from_token)
                    verbose_proxy_logger.debug(
...
async def google_login(
            ...
            if prisma_client is not None:
                ...
                if total_users and total_users > 5:
                    raise ProxyException(
                        message="You must be a LoveEngine Enterprise user to use SSO for more than 5 users. If you have a license please set `LOVE_ENGINE_LICENSE` in your env. If you want to obtain a license meet with us here: https://enterprise.love_engine.ai/demo You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
    ...
    if source == LOVE_ENGINE_CLI_SOURCE_IDENTIFIER:
    ...
    if user_custom_ui_sso_sign_in_handler is not None:
        try:
            from love_engine_enterprise.proxy.auth.custom_sso_handler import (  # type: ignore[import-untyped]
            ...
            raise ValueError(
                "Enterprise features are not available. Custom UI SSO sign-in requires LoveEngine Enterprise."
            )
...
def generic_response_convertor(
    ...
    if user_role is None:
        ...
        if user_role_from_sso is not None:
            role = get_love_engine_user_role(user_role_from_sso)
            if role is not None:
                user_role = role
                verbose_proxy_logger.debug(
                    f"Found valid LoveEngineUserRoles '{role.value}' from SSO attribute '{generic_user_role_attribute_name}'"
                )
...
async def _setup_team_mappings() -> Optional["TeamMappings"]:
    ...
    team_mappings: Optional["TeamMappings"] = None
    try:
        from love_engine.proxy.utils import get_prisma_client_or_throw
        ...
        if sso_db_record and sso_db_record.sso_settings:
            ...
            if team_mappings_data:
                from love_engine.types.proxy.management_endpoints.ui_sso import TeamMappings
...
async def _setup_role_mappings() -> Optional["RoleMappings"]:
    ...
    try:
        ...
        if sso_db_record and sso_db_record.sso_settings:
            ...
            if role_mappings_data:
                from love_engine.types.proxy.management_endpoints.ui_sso import RoleMappings
    ...
    if generic_role_mappings is not None:
        ...
        try:
            generic_user_role_mappings_data: Dict[LoveEngineUserRoles, List[str]] = (
...
def _handle_generic_sso_error(
    ...
    if not pkce_configured and (
        ...
        detailed_message = (
            ...
            f"but it's not enabled in your LoveEngine configuration.\n\n"
...
async def get_generic_sso_response(
    ...
    try:
        ...
        if code_verifier:
            ...
            # against the ``love_engine_oauth_state`` cookie set on the
            ...
            cookie_state = request.cookies.get("love_engine_oauth_state")
            if (
                not url_state
                or not cookie_state
...
async def create_team_member_add_task(team_id, user_info):
    ...
    try:
        ...
        return await team_member_add(
            ...
            user_api_key_dict=UserAPIKeyAuth(user_role=LoveEngineUserRoles.PROXY_ADMIN),
        )
    except Exception as e:
        verbose_proxy_logger.debug(
...
async def add_missing_team_member(
    user_info: Union[NewUserResponse, LOVE_ENGINE_UserTable], sso_teams: List[str]
):
    """
...
def get_disabled_non_admin_personal_key_creation():
    key_generation_settings = love_engine.key_generation_settings
...
) -> Optional[Union[LOVE_ENGINE_UserTable, NewUserResponse]]:
    try:
        potential_user_ids = []
        ...
        user_info: Optional[Union[LOVE_ENGINE_UserTable, NewUserResponse]] = None
        ...
        verbose_proxy_logger.debug(
            f"user_info: {user_info}; love_engine.default_internal_user_params: {love_engine.default_internal_user_params}"
        )

        # Upsert SSO User to LoveEngine DB
...
def _should_use_role_from_sso_response(sso_role: Optional[str]) -> bool:
    ...
    if not is_valid_love_engine_user_role(sso_role):
        verbose_proxy_logger.debug(
            f"SSO role '{sso_role}' is not a valid LoveEngine user role. "
            "Ignoring role from SSO response. See LoveEngineUserRoles enum for valid roles."
        )
        return False
    return True
...
def _build_sso_user_update_data(
    ...
    if sso_role is not None:
        ...
        sso_role_str = (
            sso_role.value if isinstance(sso_role, LoveEngineUserRoles) else sso_role
        )

        # Only include if it's a valid LoveEngine role
...
async def _sync_user_role_from_jwt_role_map(
    ...
    user_info: Optional[Union[LOVE_ENGINE_UserTable, NewUserResponse]],
    prisma_client: PrismaClient,
    user_api_key_cache: UserApiKeyCache,
    user_defined_values: Optional[SSOUserDefinedValues],
) -> None:
    """
    Apply jwt_love_engine_role_map during SSO login.

    When jwt_love_engine_role_map is configured with sync_user_role_and_teams=True,
    ...
    don't directly match LoveEngineUserRoles enum values.
    """
    if jwt_handler is None or received_response is None:
        return
    if not jwt_handler.love_engine_jwtauth.sync_user_role_and_teams:
        return
    if not jwt_handler.love_engine_jwtauth.jwt_love_engine_role_map:
        return

    mapped_role = jwt_handler.map_jwt_role_to_love_engine_role(received_response)
    if mapped_role is None:
        return

    verbose_proxy_logger.info(
        f"SSO jwt_love_engine_role_map matched role: {mapped_role.value}"
    )
    ...
    if _should_use_role_from_sso_response(sso_role):
        ...
        if user_info is None or user_info.user_role is None:
            user_defined_values["user_role"] = (
                LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY.value
            )
            verbose_proxy_logger.debug(
...
async def check_and_update_if_proxy_admin_id(
    ...
    if proxy_admin_id is not None and proxy_admin_id == user_id:
        if user_role and user_role == LoveEngineUserRoles.PROXY_ADMIN.value:
            return user_role

        if prisma_client:
            await UserRepository(prisma_client).table.update(
                where={"user_id": user_id},
                data={"user_role": LoveEngineUserRoles.PROXY_ADMIN.value},
            )

        user_role = LoveEngineUserRoles.PROXY_ADMIN.value
...
async def auth_callback(request: Request, state: Optional[str] = None):
    ...
    from love_engine.constants import LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX
    from love_engine.proxy._types import LOVE_ENGINE_JWTAuth
    from love_engine.proxy.auth.handle_jwt import JWTHandler
    from love_engine.proxy.proxy_server import (
        general_settings,
        jwt_handler,
    ...
    if state and state.startswith(f"{LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX}:"):
    ...
    cp_return_to: Optional[str] = request.cookies.get("love_engine_cp_return_to")
...
async def _build_cli_sso_user_defined_values(
    ...
    from love_engine.proxy.proxy_server import user_custom_sso
    ...
    return SSOUserDefinedValues(
        ...
        max_budget=love_engine.max_internal_user_budget,
        user_role=parsed_openid_result.get("user_role"),
        budget_duration=love_engine.internal_user_budget_duration,
    )
...
async def cli_poll_key(
    ...
    x_love_engine_cli_poll_secret: Optional[str] = Header(default=None),
):
    """
    ...
    """
    from love_engine.proxy.auth.auth_checks import ExperimentalUIJWTToken
    from love_engine.proxy.proxy_server import user_api_key_cache

    try:
        flow = _get_cli_sso_flow_or_raise(login_id=key_id, cache=user_api_key_cache)
        if not _verify_cli_sso_poll_secret(
            flow=flow, poll_secret=x_love_engine_cli_poll_secret
        ):
        ...
        if isinstance(session_data, dict):
            ...
            # Generate CLI JWT on-demand (expiration configurable via LOVE_ENGINE_CLI_JWT_EXPIRATION_HOURS)
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in LoveEngine DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): LoveEngine SSOValues / fields that were read
    ...
    if love_engine.default_internal_user_params:
        # Preserve the SSO-extracted role if it's a valid LoveEngine role,
        ...
        if _should_use_role_from_sso_response(sso_role):
            ...
            user_defined_values.update(love_engine.default_internal_user_params)  # type: ignore
    ...
    if user_defined_values.get("user_role") == LoveEngineUserRoles.INTERNAL_USER.value:
        if user_defined_values.get("max_budget") is None:
            user_defined_values["max_budget"] = love_engine.max_internal_user_budget
        if user_defined_values.get("budget_duration") is None:
            user_defined_values["budget_duration"] = (
                love_engine.internal_user_budget_duration
            )

    if user_defined_values["user_role"] is None:
        user_defined_values["user_role"] = LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY
...
async def get_ui_settings(request: Request):
    from love_engine.proxy.proxy_server import general_settings, proxy_state

    _proxy_base_url = os.getenv("PROXY_BASE_URL", None)
    _logout_url = os.getenv("PROXY_LOGOUT_URL", None)
    _api_doc_base_url = os.getenv("LOVE_ENGINE_UI_API_DOC_BASE_URL", None)
    ...
    return {
        ...
        "LOVE_ENGINE_UI_API_DOC_BASE_URL": _api_doc_base_url,
...
class SSOAuthenticationHandler:
    ...
    async def get_sso_login_redirect(
        ...
        Args:
            ...
            request: Optional FastAPI request, used to drive the ``Secure``
                attribute on the ``love_engine_oauth_state`` CSRF cookie.
        ...
        raise ValueError(
            "Unknown SSO provider. Please setup SSO with client IDs https://docs.love_engine.ai/docs/proxy/admin_ui_sso"
        )
    ...
    async def get_generic_sso_redirect_response(
        ...
        from love_engine.proxy.proxy_server import redis_usage_cache, user_api_key_cache

        with generic_sso:
            # State is bound to the caller's browser via a ``love_engine_oauth_state``
    ...
    def _get_generic_sso_redirect_params(
        ...
        if state:
            ...
            # the love_engine proxy cli sends the "state" parameter to the proxy server for auth. We should maintain the state parameter for the cli if it is provided
    ...
    async def upsert_sso_user(
        result: Optional[Union[CustomOpenID, OpenID, dict]],
        user_info: Optional[Union[NewUserResponse, LOVE_ENGINE_UserTable]],
        user_email: Optional[str],
        user_defined_values: Optional[SSOUserDefinedValues],
        prisma_client: PrismaClient,
    ):
        """
        Connects the SSO Users to the User Table in LoveEngine DB

        - If user on LoveEngine DB, update the user_email and user_role (if SSO provides valid role) with the SSO values
        - If user not on LoveEngine DB, insert the user into LoveEngine DB
        """
        try:
            if user_info is not None:
                ...
                verbose_proxy_logger.info(
                    "user not in DB, inserting user into LoveEngine DB"
                )
                # user not in DB, insert User into LoveEngine DB
            ...
            verbose_proxy_logger.exception(
                f"Error upserting SSO user into LoveEngine DB: {e}"
            )
            return user_info
    ...
    async def add_user_to_teams_from_sso_response(
        ...
        The `team_ids` field is populated by love_engine after processing the SSO response
        """
        if user_info is None:
            verbose_proxy_logger.debug(
                "User not found in LoveEngine DB, skipping team member addition"
            )
            return
    ...
    @staticmethod
    async def create_love_engine_team_from_sso_group(
        love_engine_team_id: str,
        love_engine_team_name: Optional[str] = None,
    ):
        """
        Creates a LoveEngine Team from a SSO Group ID

        Your SSO provider might have groups that should be created on LoveEngine

        Use this helper to create a LoveEngine Team from a SSO Group ID

        Args:
            love_engine_team_id (str): The ID of the LoveEngine Team
            love_engine_team_name (Optional[str]): The name of the LoveEngine Team
        """
        ...
        try:
            team_obj = await TeamRepository(prisma_client).table.find_first(
                where={"team_id": love_engine_team_id}
            )
            ...
            if team_obj:
                verbose_proxy_logger.debug(
                    f"Team already exists: {love_engine_team_id} - {love_engine_team_name}"
                )
                return

            team_request: NewTeamRequest = NewTeamRequest(
                team_id=love_engine_team_id,
                team_alias=love_engine_team_name,
            )
            if love_engine.default_team_params:
                team_request = SSOAuthenticationHandler._cast_and_deepcopy_love_engine_default_team_params(
                    default_team_params=love_engine.default_team_params,
                    love_engine_team_id=love_engine_team_id,
                    love_engine_team_name=love_engine_team_name,
            ...
            await new_team(
                ...
                user_api_key_dict=UserAPIKeyAuth(
                    token="",
                    key_alias=f"love_engine.{MicrosoftSSOHandler.__name__}",
                ),
            )
        except Exception as e:
            verbose_proxy_logger.exception(f"Error creating LoveEngine Team: {e}")

    @staticmethod
    def _cast_and_deepcopy_love_engine_default_team_params(
        ...
        """
        Casts and deepcopies the love_engine.default_team_params to a NewTeamRequest object

        - Ensures we create a new DefaultTeamSSOParams object
        - Handle the case where love_engine.default_team_params is a dict or a DefaultTeamSSOParams object
        - Adds the love_engine_team_id and love_engine_team_name to the DefaultTeamSSOParams object
        """
        if isinstance(default_team_params, dict):
            _team_request = deepcopy(default_team_params)
            _team_request["team_id"] = love_engine_team_id
            _team_request["team_alias"] = love_engine_team_name
            team_request = NewTeamRequest(**_team_request)
        elif isinstance(love_engine.default_team_params, DefaultTeamSSOParams):
            _default_team_params = deepcopy(love_engine.default_team_params)
    ...
    def _get_cli_state(
        ...
        """
        from love_engine.constants import (
            LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX,
        )

        if source == LOVE_ENGINE_CLI_SOURCE_IDENTIFIER and key:
            if _is_valid_cli_sso_user_code(user_code):
                return f"{LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX}:{key}:{user_code}"
            return f"{LOVE_ENGINE_CLI_SESSION_TOKEN_PREFIX}:{key}"
        else:
            return None
    ...
    def _get_user_email_and_id_from_result(
        ...
        if result is not None:
            ...
            if _user_role is not None:
                ...
                user_role = (
                    _user_role.value
                    if isinstance(_user_role, LoveEngineUserRoles)
                    else _user_role
                )
                verbose_proxy_logger.debug(
            ...
            if user_role is None:
                ...
                if _role_from_attr is not None:
                    ...
                    user_role = (
                        ...
                        if isinstance(_role_from_attr, LoveEngineUserRoles)
    ...
    async def get_redirect_response_from_openid(
        ...
        user_info = None
        user_id_models: List = []
        max_internal_user_budget = love_engine.max_internal_user_budget
        internal_user_budget_duration = love_engine.internal_user_budget_duration
        ...
        # Sync user role from JWT claims via jwt_love_engine_role_map (if configured).
        ...
        if user_defined_values is None:
            raise Exception(
                "Unable to map user identity to known values. 'user_defined_values' is None. File an issue - https://github.com/BerriAI/love_engine/issues"
            )
        ...
        user_role = (
            user_defined_values["user_role"]
            or LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY.value
        )
        ...
        returned_ui_token_object = ReturnedUITokenObject(
            ...
            key=key,
            user_email=user_email,
            user_role=user_role or LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY.value,
            login_method="sso",
        ...
        if return_to is not None and SSOAuthenticationHandler._validate_return_to(
            ...
            redirect_response.delete_cookie("love_engine_cp_return_to")
            return redirect_response

        if user_id is not None and isinstance(user_id, str):
            love_engine_dashboard_ui += "?login=success"
        verbose_proxy_logger.info(f"Redirecting to {love_engine_dashboard_ui}")
...
class MicrosoftSSOHandler:
    ...
    # used for debugging to show the user groups love_engine found from Graph API
    ...
    async def get_microsoft_callback_response(
        ...
        user_role: Optional[LoveEngineUserRoles] = None
        if app_roles:
            # Check if any app role is a valid LoveEngineUserRoles
            for role_str in app_roles:
                role = get_love_engine_user_role(role_str)
                if role is not None:
                    user_role = role
                    verbose_proxy_logger.debug(
                        f"Found valid LoveEngineUserRoles '{role.value}' in app_roles"
                    )
                    break
    ...
    def openid_from_response(
        ...
        team_ids: List[str],
        user_role: Optional[LoveEngineUserRoles],
    ) -> CustomOpenID:
        ...
        try:
            ...
            if service_principal_id:
                ...
                if len(service_principal_group_ids) > 0:
                    await MicrosoftSSOHandler.create_love_engine_teams_from_service_principal_team_ids(
    ...
    @staticmethod
    async def create_love_engine_teams_from_service_principal_team_ids(
        service_principal_teams: List[MicrosoftServicePrincipalTeam],
    ):
        """
        Creates LoveEngine Teams from the Service Principal Group IDs

        When a user sets a `SERVICE_PRINCIPAL_ID` in the env, love_engine will fetch groups under that service principal and create LoveEngine Teams from them
        """
        verbose_proxy_logger.debug(
            f"Creating LoveEngine Teams from Service Principal Teams: {service_principal_teams}"
        )
        for service_principal_team in service_principal_teams:
            love_engine_team_id: Optional[str] = service_principal_team.get("principalId")
            love_engine_team_name: Optional[str] = service_principal_team.get(
                "principalDisplayName"
            )
            if not love_engine_team_id:
                verbose_proxy_logger.debug(
                    f"Skipping team creation for {love_engine_team_name} because it has no principalId"
                )
                continue

            await SSOAuthenticationHandler.create_love_engine_team_from_sso_group(
                love_engine_team_id=love_engine_team_id,
                love_engine_team_name=love_engine_team_name,
            )
...
async def debug_sso_login(request: Request):
    ...
    if (
        ...
        if premium_user is not True:
            raise ProxyException(
                message="You must be a LoveEngine Enterprise user to use SSO. If you have a license please set `LOVE_ENGINE_LICENSE` in your env. If you want to obtain a license meet with us here: https://enterprise.love_engine.ai/demo You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
                type=ProxyErrorTypes.auth_error,
                param="premium_user",
                code=status.HTTP_403_FORBIDDEN,
            )
