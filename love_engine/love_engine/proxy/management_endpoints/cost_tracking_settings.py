...
from love_engine.cost_calculator import completion_cost
...
from love_engine.types.utils import LlmProvidersSet
...
def _resolve_model_for_cost_lookup(model: str) -> Tuple[str, Optional[str]]:
    ...
    underlying love_engine model name for cost lookup.
    ...
    if llm_router is not None:
        try:
            ...
            if deployments and len(deployments) > 0:
                ...
                love_engine_params = first_deployment.get("love_engine_params", {})
                model_info = first_deployment.get("model_info", {})

                # Check base_model first (needed for Azure custom deployment names)
                base_model = model_info.get("base_model") or love_engine_params.get(
                    "base_model"
                )
                if base_model:
                ...
                resolved_model = love_engine_params.get("model")
...
async def get_cost_discount_config(
    ...
    Returns the cost_discount_config from love_engine_settings.
    """
    ...
    try:
        ...
        # Get cost_discount_config from love_engine_settings
        love_engine_settings = config.get("love_engine_settings", {})
        cost_discount_config = love_engine_settings.get("cost_discount_config", {})
...
async def update_cost_discount_config(
    ...
    Updates the cost_discount_config in love_engine_settings.
    ...
    # Validate that all providers are valid LoveEngine providers
    invalid_providers = []
    ...
    if invalid_providers:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid provider(s): {', '.join(invalid_providers)}. Must be valid LoveEngine providers. See https://docs.love_engine.ai/docs/providers for the full list."
            },
        )
    ...
    try:
        ...
        # Ensure love_engine_settings exists
        if "love_engine_settings" not in config:
            config["love_engine_settings"] = {}

        # Update cost_discount_config
        config["love_engine_settings"]["cost_discount_config"] = cost_discount_config

        # Save the updated config to DB
        await proxy_config.save_config(new_config=config)

        # Update in-memory love_engine.cost_discount_config
        love_engine.cost_discount_config = cost_discount_config
...
async def get_cost_margin_config(
    ...
    Returns the cost_margin_config from love_engine_settings.
    """
    from love_engine.proxy.proxy_server import prisma_client, proxy_config
    ...
    try:
        ...
        # Get cost_margin_config from love_engine_settings
        love_engine_settings = config.get("love_engine_settings", {})
        cost_margin_config = love_engine_settings.get("cost_margin_config", {})
...
async def update_cost_margin_config(
    ...
    Updates the cost_margin_config in love_engine_settings.
    Margins can be:
    ...
    # Validate that all providers are valid LoveEngine providers (except "global")
    invalid_providers = []
    ...
    if invalid_providers:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid provider(s): {', '.join(invalid_providers)}. Must be valid LoveEngine providers or 'global'. See https://docs.love_engine.ai/docs/providers for the full list."
            },
        )
    ...
    try:
        ...
        config["love_engine_settings"]["cost_margin_config"] = cost_margin_config

        # Save the updated config to DB
        await proxy_config.save_config(new_config=config)

        # Update in-memory love_engine.cost_margin_config
        love_engine.cost_margin_config = cost_margin_config
...
async def estimate_cost(
    ...
    from love_engine.types.utils import ModelResponse, Usage
    ...
    love_engine_logging_obj = LoveEngineLoggingObj(
        ...
        start_time=None,
        love_engine_call_id="cost-estimate",
    ...
    try:
        model_info = love_engine.get_model_info(model=resolved_model)
        input_cost_per_token = model_info.get("input_cost_per_token")
        output_cost_per_token = model_info.get("output_cost_per_token")
        custom_llm_provider = model_info.get("love_engine_provider")
    except Exception:
