...
from love_engine.constants import (
    LENGTH_OF_LOVE_ENGINE_GENERATED_KEY,
    LOVE_ENGINE_PROXY_ADMIN_NAME,
    UI_SESSION_TOKEN_TEAM_ID,
)
from love_engine.love_engine_core_utils.duration_parser import duration_in_seconds
from love_engine.love_engine_core_utils.safe_json_dumps import safe_dumps
from love_engine.proxy._experimental.mcp_server.db import (
    rotate_mcp_server_credentials_master_key,
    rotate_mcp_user_credentials_master_key,
    rotate_mcp_user_env_vars_master_key,
)
from love_engine.proxy._types import *
from love_engine.proxy._types import LOVE_ENGINE_VerificationToken
...
from love_engine.proxy.auth.auth_utils import abbreviate_api_key
from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth
from love_engine.proxy.common_utils.callback_utils import (
    decrypt_callback_vars,
    encrypt_callback_vars,
)
from love_engine.proxy.common_utils.rbac_utils import check_org_admin_can_generate_keys
...
from love_engine.proxy.management_helpers.object_permission_utils import (
...
from love_engine.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from love_engine.proxy.management_helpers.utils import management_endpoint_wrapper
from love_engine.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from love_engine.types.proxy.management_endpoints.key_management_endpoints import (
...
from love_engine.types.router import Deployment
...
def _is_team_key(data: Union[GenerateKeyRequest, LOVE_ENGINE_VerificationToken]):
    return data.team_id is not None


def _get_user_in_team(
    team_table: LOVE_ENGINE_TeamTableCachedObj, user_id: Optional[str]
) -> Optional[Member]:
    if user_id is None:
        return None
...
def _set_key_rotation_fields(
    ...
    if auto_rotate and rotation_interval:
        if (
            love_engine._key_management_settings is not None
            and love_engine._key_management_settings.store_virtual_keys is True
...
def _is_allowed_to_make_key_request(
    ...
    Relevant issue: https://github.com/BerriAI/love_engine/issues/7336
    """
    ## BASE CASE - PROXY ADMIN
    if (
        user_api_key_dict.user_role is not None
        and user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
    ):
        return True
    ...
    if team_id is not None:
        if (
            ...
            return True  # handle https://github.com/BerriAI/love_engine/issues/7482
...
def _team_key_operation_team_member_check(
    ...
    team_table: LOVE_ENGINE_TeamTableCachedObj,
    user_api_key_dict: UserAPIKeyAuth,
...
def _team_key_generation_check(
    ...
    if user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value:
        return True
    if (
        love_engine.key_generation_settings is not None
        and "team_key_generation" in love_engine.key_generation_settings
    ):
        _team_key_generation = love_engine.key_generation_settings["team_key_generation"]
    else:
...
def _personal_key_membership_check(
    ...
    if user_api_key_dict.user_role not in personal_key_generation["allowed_user_roles"]:
        raise HTTPException(
            status_code=400,
            detail=f"Personal key creation has been restricted by admin. Allowed roles={love_engine.key_generation_settings['personal_key_generation']['allowed_user_roles']}. Your role={user_api_key_dict.user_role}",  # type: ignore
        )
...
def _personal_key_generation_check(
    ...
    if (
        love_engine.key_generation_settings is None
        or love_engine.key_generation_settings.get("personal_key_generation") is None
    ):
        return True

    _personal_key_generation = love_engine.key_generation_settings["personal_key_generation"]  # type: ignore
...
def key_generation_check(
    team_table: Optional[LOVE_ENGINE_TeamTableCachedObj],
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
    route: KeyManagementRoutes,
) -> bool:
    ...
    if is_team_key:
        if team_table is None and love_engine.key_generation_settings is not None:
            raise HTTPException(
                status_code=400,
...
def handle_key_type(data: GenerateKeyRequest, data_json: dict) -> dict:
    ...
    if key_type == LoveEngineKeyType.LLM_API:
        data_json["allowed_routes"] = ["llm_api_routes"]
    elif key_type == LoveEngineKeyType.MANAGEMENT:
        data_json["allowed_routes"] = ["management_routes"]
    elif key_type == LoveEngineKeyType.READ_ONLY:
...
def _validate_caller_can_change_key_ownership(
    ...
    ``_return_user_api_key_auth_obj`` resolves against ``love_engine_usertable``
...
def _enforce_upperbound_key_params(
    ...
    if love_engine.upperbound_key_generate_params is None:
        return

    for elem in data:
        key, value = elem
        upperbound_value = getattr(love_engine.upperbound_key_generate_params, key, None)
...
async def _common_key_generation_helper(
    ...
    from love_engine.proxy.proxy_server import (
        love_engine_proxy_admin_name,
        llm_router,
        premium_user,
        prisma_client,
    ...
    if love_engine.default_key_generate_params is not None:
        for elem in data:
            key, value = elem
            if value is None and key in [
                ...
                setattr(data, key, love_engine.default_key_generate_params.get(key, None))
            elif key == "models" and value == []:
                setattr(data, key, love_engine.default_key_generate_params.get(key, []))
    ...
    if (
        user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN.value
    ...
    try:
        from love_engine_enterprise.proxy.management_endpoints.key_management_endpoints import (
        ...
        verbose_proxy_logger.debug(
            "love_engine.proxy.proxy_server.generate_key_fn(): Enterprise key management params not applied - {}".format(
                str(e)
            )
        )

    # TODO: @ishaan-jaff: Migrate all budget tracking to use LOVE_ENGINE_BudgetTable
    _budget_id = data.budget_id
    if prisma_client is not None and data.soft_budget is not None:
        # create the Budget Row for the LoveEngine Verification Token
        budget_row = LOVE_ENGINE_BudgetTable(
    ...
    for field in LOVE_ENGINE_ManagementEndpoint_MetadataFields_Premium:
    ...
    if data.key is not None and not data.key.startswith("sk-"):
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid key format. LoveEngine Virtual Key must start with 'sk-'. Received: {_masked}"
            },
        )
...
def _check_key_model_specific_limits(
    keys: List[LOVE_ENGINE_VerificationToken],
    data: Union[GenerateKeyRequest, UpdateKeyRequest],
...
async def _check_project_key_limits(
    ...
    if project_obj.love_engine_budget_table is not None:
        project_max_budget = getattr(
            project_obj.love_engine_budget_table, "max_budget", None
        )
...
def check_org_key_model_specific_limits(
    keys: List[LOVE_ENGINE_VerificationToken],
    org_table: LOVE_ENGINE_OrganizationTable,
    data: Union[GenerateKeyRequest, UpdateKeyRequest],
) -> None:
    """
    ...
    if org_table.love_engine_budget_table is not None:
        entity_rpm_limit = org_table.love_engine_budget_table.rpm_limit
        entity_tpm_limit = org_table.love_engine_budget_table.tpm_limit
...
async def generate_key_fn(
    ...
    love_engine_changed_by: Optional[str] = Header(
        None,
        description="The love_engine-changed-by header enables tracking of actions performed by authorized users on behalf of other users, providing an audit trail for accountability",
    ),
):
    """
    Generate an API key based on the provided data.

    Docs: https://docs.love_engine.ai/docs/proxy/virtual_keys
    ...
    - aliases: Optional[dict] - Any alias mappings, on top of anything in the config.yaml model list. - https://docs.love_engine.ai/docs/proxy/virtual_keys#managing-auth---upgradedowngrade-models
    - config: Optional[dict] - any key-specific configs, overrides config in config.yaml
    - spend: Optional[int] - Amount spent by key. Default is 0. Will be updated by proxy whenever key is used. https://docs.love_engine.ai/docs/proxy/virtual_keys#managing-auth---tracking-spend
    ...
    - allowed_cache_controls: Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://docs.love_engine.ai/docs/proxy/caching#turn-on--off-caching-per-request
    - blocked: Optional[bool] - Whether the key is blocked.
    - rpm_limit: Optional[int] - Specify rpm limit for a given key (Requests per minute)
    - tpm_limit: Optional[int] - Specify tpm limit for a given key (Tokens per minute)
    - soft_budget: Optional[float] - Specify soft budget for a given key. Will trigger a slack alert when this soft budget is reached.
    - tags: Optional[List[str]] - Tags for [tracking spend](https://love_engine.vercel.app/docs/proxy/enterprise#tracking-spend-for-custom-tags) and/or doing [tag-based routing](https://love_engine.vercel.app/docs/proxy/tag_routing).
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    - enforced_params: Optional[List[str]] - List of enforced params for the key (Enterprise only). [Docs](https://docs.love_engine.ai/docs/proxy/enterprise#enforce-required-params-for-llm-requests)
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    ...
    - object_permission: Optional[LOVE_ENGINE_ObjectPermissionBase] - key-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"], "agents": ["agent_1", "agent_2"], "agent_access_groups": ["dev_group"]}. IF null or {} then no object permission.
    ...
    """
    try:
        from love_engine.proxy._types import CommonProxyErrors
        from love_engine.proxy.proxy_server import (
            prisma_client,
            user_api_key_cache,
        ...
        team_table: Optional[LOVE_ENGINE_TeamTableCachedObj] = None
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.generate_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def prepare_metadata_fields(
    ...
    """
    Check LOVE_ENGINE_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    for reserved_field in LOVE_ENGINE_Reserved_Metadata_Fields:
    ...
    try:
        for k, v in data_json.items():
            if k in LOVE_ENGINE_ManagementEndpoint_MetadataFields:
                if isinstance(v, datetime):
                    casted_metadata[k] = v.isoformat()
                else:
                    casted_metadata[k] = v
            if k in LOVE_ENGINE_ManagementEndpoint_MetadataFields_Premium:
                from love_engine.proxy.utils import _premium_user_check
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.prepare_metadata_fields(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def prepare_key_update_data(
    ...
    existing_key_row: LOVE_ENGINE_VerificationToken,
):
    ...
    for k, v in data_json.items():
        if (
            k in LOVE_ENGINE_ManagementEndpoint_MetadataFields
            or k in LOVE_ENGINE_ManagementEndpoint_MetadataFields_Premium
        ):
            continue
...
def is_different_team(
    data: UpdateKeyRequest, existing_key_row: LOVE_ENGINE_VerificationToken
) -> bool:
...
async def _get_and_validate_existing_key(
    ...
    Returns:
        LOVE_ENGINE_VerificationToken: The existing key row
...
async def _process_single_key_update(
    ...
    existing_key_row: Optional[LOVE_ENGINE_VerificationToken] = None,
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        love_engine_changed_by: Optional header for tracking who made the change
        prisma_client: Prisma client instance
    ...
    team_obj: Optional[LOVE_ENGINE_TeamTableCachedObj] = None
...
async def _validate_mcp_servers_for_key_update(
    ...
    team_obj: Optional["LOVE_ENGINE_TeamTableCachedObj"],
    existing_key_row: Any,
    prisma_client: Any,
...
async def _validate_update_key_data(
    ...
    _is_proxy_admin = user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
    ...
    if data.object_permission is not None:
        ...
        if normalized_object_permission is not None:
            data.object_permission = LOVE_ENGINE_ObjectPermissionBase(
...
async def update_key_fn(
    ...
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    - blocked: Optional[bool] - Whether the key is blocked
    - aliases: Optional[dict] - Model aliases for the key - [Docs](https://love_engine.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    try:
        ...
        if data.spend is not None:
            try:
                from love_engine.proxy.proxy_server import _invalidate_spend_counter
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.update_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def _build_failed_team_key_update(
    token: str,
    exception: Exception,
    existing_key_row: Optional[LOVE_ENGINE_VerificationToken],
) -> FailedKeyUpdate:
...
async def bulk_update_team_keys(
    ...
    if user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN.value:
        auth_anchor = (
            existing_keys[0]
            if existing_keys
            else LOVE_ENGINE_VerificationToken(
...
async def validate_key_team_change(
    key: LOVE_ENGINE_VerificationToken,
    ...
    member_object = _get_user_in_team(
        team_table=cast(LOVE_ENGINE_TeamTableCachedObj, team), user_id=key.user_id
    )
    ...
    if change_initiated_by.user_role == LoveEngineUserRoles.PROXY_ADMIN.value:
        return
    ...
    # this teams member permissions allow updating a
        ...
        team_table=cast(LOVE_ENGINE_TeamTableCachedObj, team),
...
async def delete_key_fn(
    ...
    try:
        ...
        # Normalize love_engine_changed_by: if it's a Header object or not a string, convert to None
        if love_engine_changed_by is not None and not isinstance(love_engine_changed_by, str):
            love_engine_changed_by = None
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.delete_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def info_key_fn_v2(
    ...
    **New endpoint**. Currently admin only.
    ...
    try:
        if prisma_client is None:
            raise Exception(
                "Database not connected. Connect a database to your proxy - https://docs.love_engine.ai/docs/simple_proxy#managing-auth---virtual-keys"
            )
        if data is None:
            raise HTTPException(
        ...
        for k in key_info:
            ...
            budget_table = k_dict.get("love_engine_budget_table") or {}
...
-H "Authorization: Bearer sk-test-example-key-123"
    ...
    try:
        ...
        budget_table = key_info.get("love_engine_budget_table") or {}
        if not model_max_budget and isinstance(budget_table, dict):
            model_max_budget = budget_table.get("model_max_budget") or {}
        ...
        if llm_router._is_model_access_group_for_wildcard_route(
            ...
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Setting a model access group on a wildcard model is only available for LoveEngine Enterprise users.{}".format(
...
async def generate_key_helper_fn(
    ...
    budget_id: Optional[float] = None,  # budget id <-> LOVE_ENGINE_BudgetTable
    ...
    object_permission_id: Optional[
        str
    ] = None,  # object_permission_id <-> LOVE_ENGINE_ObjectPermissionTable
    ...
    from love_engine.proxy.proxy_server import premium_user, prisma_client

    if prisma_client is None:
        raise Exception(
            "Connect Proxy to database to generate keys - https://docs.love_engine.ai/docs/proxy/virtual_keys "
        )
    ...
    if token is None:
        if key is not None:
            ...
            token = f"sk-{secrets.token_urlsafe(LENGTH_OF_LOVE_ENGINE_GENERATED_KEY)}"
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if (
                ...
                raise ValueError(
                    "get_spend_routes permission is only available for LoveEngine Enterprise users"
                )
        ...
        if prisma_client is not None:
            ...
            if table_name is not None and table_name == "user":
                ...
                # the LOVE_ENGINE_VerificationToken table will increase in size if we don't do this check
                return user_data
            ...
            key_data["love_engine_budget_table"] = getattr(
                create_key_response, "love_engine_budget_table", None
            )
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.generate_key_helper_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def _team_key_deletion_check(
    user_api_key_dict: UserAPIKeyAuth,
    key_info: LOVE_ENGINE_VerificationToken,
    prisma_client: PrismaClient,
    user_api_key_cache: UserApiKeyCache,
):
    ...
    if is_team_key and key_info.team_id is not None:
        ...
        if (
            ...
            _team_key_generation = love_engine.key_generation_settings[
                "team_key_generation"
            ]
        else:
            _team_key_generation = TeamUIKeyGenerationConfig(
...
async def can_modify_verification_token(
    ...
    if user_api_key_dict.api_key == LOVE_ENGINE_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME:
        return True
...
async def delete_verification_tokens(
    ...
    user_api_key_cache: UserApiKeyCache,
    user_api_key_dict: UserAPIKeyAuth,
    love_engine_changed_by: Optional[str] = None,
) -> Tuple[Optional[Dict], List[LOVE_ENGINE_VerificationToken]]:
    """
    ...
    Returns:
        Tuple[Optional[Dict], List[LOVE_ENGINE_VerificationToken]]:
            ...
            List[LOVE_ENGINE_VerificationToken]:
    ...
    try:
        if prisma_client:
            ...
            _keys_being_deleted: List[LOVE_ENGINE_VerificationToken] = (
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.delete_verification_tokens(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
def _transform_verification_tokens_to_deleted_records(
    ...
    for key in keys:
        ...
        deleted_record = LOVE_ENGINE_DeletedVerificationToken(
            **key_payload,
        ...
        for rel_key in (
            "love_engine_budget_table",
            "love_engine_organization_table",
            "object_permission",
            "id",
            "budget_limits",
        ):
...
async def _rotate_master_key(
    ...
    2. Decrypt the values
        - ModelTable
            - [{"model_name": "str", "love_engine_params": {}}]
        - ConfigTable
    ...
    from love_engine.proxy.proxy_server import proxy_config
    ...
    if models:
        ...
        for model in decrypted_models:
            ...
            if new_model:
                ...
                _dumped["love_engine_params"] = prisma.Json(_dumped["love_engine_params"])  # type: ignore[attr-defined]
        ...
        async with prisma_client.db.tx() as tx:
            await tx.love_engine_proxymodeltable.delete_many()
            verbose_proxy_logger.debug("Creating %s models", len(new_models))
            await tx.love_engine_proxymodeltable.create_many(
                data=new_models,
            )
    ...
    try:
        await rotate_mcp_server_credentials_master_key(
            prisma_client=prisma_client,
            touched_by=user_api_key_dict.user_id or LOVE_ENGINE_PROXY_ADMIN_NAME,
    ...
    try:
        ...
        credentials = None
    if credentials:
        from love_engine.proxy.credential_endpoints.endpoints import update_db_credential
    ...
    if data and data.new_key is not None:
        ...
        if not data.new_key.startswith("sk-"):
            raise HTTPException(
                ...
                detail={
                    "error": "New key must start with 'sk-'. This is to distinguish a key hash (used by love_engine for logging / internal logic) from the actual key."
                },
            )
    else:
        new_token = f"sk-{secrets.token_urlsafe(LENGTH_OF_LOVE_ENGINE_GENERATED_KEY)}"
    return new_token
...
async def _insert_deprecated_key(
    ...
    grace_period_value = grace_period or os.getenv(
        "LOVE_ENGINE_KEY_ROTATION_GRACE_PERIOD", ""
    )
...
async def _execute_virtual_key_regeneration(
    *,
    prisma_client: PrismaClient,
    key_in_db: LOVE_ENGINE_VerificationToken,
    hashed_api_key: str,
    key: str,
    ...
    from love_engine.proxy.proxy_server import hash_token
...
async def regenerate_key_fn(
    ...
    - data: Optional[RegenerateKeyRequest] - Request body containing optional parameters to update
        ...
        - guardrails: Optional[List[str]] - List of active guardrails for the key
        - blocked: Optional[bool] - Whether the key is blocked
        - grace_period: Optional[str] - Duration to keep old key valid after rotation (e.g. "24h", "2d"). Omitted = immediate revoke. Env: LOVE_ENGINE_KEY_ROTATION_GRACE_PERIOD
    ...
    try:
        ...
        if data is not None and data.access_group_ids:
            regenerate_team_table: Optional[LOVE_ENGINE_TeamTableCachedObj] = None
...
def _validate_reset_spend_value(
    reset_to: Any, key_in_db: LOVE_ENGINE_VerificationToken
) -> float:
    ...
    if key_in_db.love_engine_budget_table is not None:
        budget_max_budget = getattr(key_in_db.love_engine_budget_table, "max_budget", None)
...
async def reset_key_spend_fn(
    ...
    try:
        ...
        from love_engine.proxy.proxy_server import spend_counter_cache
...
async def validate_key_list_check(
    ...
    complete_user_info = LOVE_ENGINE_UserTable(**complete_user_info_db_obj.model_dump())
...
async def _fetch_user_team_objects(
    complete_user_info: Optional[LOVE_ENGINE_UserTable],
    prisma_client: PrismaClient,
) -> List[LOVE_ENGINE_TeamTable]:
    ...
    return [LOVE_ENGINE_TeamTable(**team.model_dump()) for team in teams]


def _get_admin_team_ids_from_objects(
    user_api_key_dict: UserAPIKeyAuth,
    team_objects: List[LOVE_ENGINE_TeamTable],
) -> List[str]:
...
async def list_keys(
    ...
    try:
        ...
        use_substring_matching = user_api_key_dict.user_role in [
            LoveEngineUserRoles.PROXY_ADMIN.value,
            LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY.value,
        ]
...
async def key_aliases(
    ...
    try:
        ...
        count_sql = f'SELECT COUNT(*) AS count FROM "LOVE_ENGINE_VerificationToken" WHERE {where_sql}'
        ...
        aliases_sql = (
            f"SELECT key_alias"
            f' FROM "LOVE_ENGINE_VerificationToken"'
            f" WHERE {where_sql}"
...
async def _list_key_helper(
    ...
    # Prepare response
    key_list: List[Union[str, UserAPIKeyAuth, LOVE_ENGINE_DeletedVerificationToken]] = []
    for key in keys:
        ...
        if return_full_object is True or (expand and "user" in expand):
            if use_deleted_table:
                ...
                key_list.append(LOVE_ENGINE_DeletedVerificationToken(**key_dict))
            else:
...
async def block_key(
...
) -> Optional[LOVE_ENGINE_VerificationToken]:
    """
    ...
    if love_engine.store_audit_logs is True:
        asyncio.create_task(
            create_audit_log_for_update(
                request_data=LOVE_ENGINE_AuditLogs(
                    ...
                    table_name=LoveEngineTableNames.KEY_TABLE_NAME,
...
async def _can_user_query_key_info(
    ...
    if (
        user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
        or user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY.value
    ):
        return True
...
async def test_key_logging(
    ...
    from love_engine.proxy.proxy_server import general_settings, proxy_config
    ...
    try:
        data = {
            "model": "openai/love_engine-key-health-test",
            "messages": [
                {
                    "role": "user",
                    "content": "Hello, this is a test from love_engine /key/health. No LLM API call was made for this",
                }
            ],
        }
...
def _validate_key_alias_format(key_alias: Optional[str]) -> None:
    ...
    Gated behind ``love_engine.enable_key_alias_format_validation`` (default **False**).
    ...
    """
    if not love_engine.enable_key_alias_format_validation:
        return
...
def validate_model_max_budget(model_max_budget: Optional[Dict]) -> None:
    ...
    try:
        ...
        if model_max_budget is not None:
            from love_engine.proxy.proxy_server import CommonProxyErrors, premium_user
        ...
        raise ValueError(
            f"Invalid model_max_budget: {str(e)}. Example of valid model_max_budget: https://docs.love_engine.ai/docs/proxy/users"
        )
