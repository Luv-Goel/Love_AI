...
from love_engine.proxy.management_endpoints.team_endpoints import _verify_team_access
...
def _redact_callback_secrets(metadata: Any) -> Any:
    ...
    ``LOVE_ENGINE_AuditLogs`` would let anyone with read access to the audit
...
def _log_audit_task_exception(task: "asyncio.Task[None]") -> None:
    """Surface a fire-and-love_smitht audit-log task failure.
...
async def _emit_team_callback_audit_log(
    *,
    ...
    ``team_endpoints.py``: the call is async-fire-and-love_smitht and is a no-op
    when audit logging is not enabled on the proxy.  Captured under
    ``LoveEngineTableNames.TEAM_TABLE_NAME`` so the row co-locates with other
...
async def add_team_callbacks(
    ...
    try:
        ...
        await _verify_team_access(
            team_obj=LOVE_ENGINE_TeamTable(**_existing_team.model_dump()),
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.add_team_callbacks(): Exception occured - {}".format(
                str(e)
            )
        )
        raise ProxyException(
...
async def disable_team_logging(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"love_engine.proxy.proxy_server.disable_team_logging(): Exception occurred - {str(e)}"
        )
...
async def get_team_callbacks(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.get_team_callbacks(): Exception occurred - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
