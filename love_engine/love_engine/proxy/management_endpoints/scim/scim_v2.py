"""
✨ SCIM v2 Endpoints for LoveEngine Proxy using Internal User/Team Management
...
from love_engine.proxy.auth.auth_checks import _delete_cache_key_object
...
from love_engine.proxy.management_endpoints.scim.scim_transformations import (
    ScimTransformations,
)
...
from love_engine.proxy.utils import _premium_user_check, handle_exception_on_proxy
...
from love_engine.types.proxy.management_endpoints.scim_v2 import *
...
class UserProvisionerHelpers:
    ...
    async def handle_existing_user_by_email(
        ...
        return await ScimTransformations.transform_love_engine_user_to_scim_user(
            updated_user
        )
...
def _build_scim_metadata(
    ...
    metadata: Dict[str, Any] = {
        "scim_metadata": LOVE_ENGINE_UserScimMetadata(
...
async def _get_scim_upsert_user_setting() -> bool:
    """
    Get the scim_upsert_user setting from love_engine_settings.
    ...
    try:
        ...
        scim_upsert_user = love_engine_settings.get("scim_upsert_user", True)
...
async def _extract_group_member_ids(group: SCIMGroup) -> GroupMemberExtractionResult:
    ...
    Behavior depends on love_engine_settings.scim_upsert_user:
...
async def _delete_rows_referencing_user(prisma_client: Any, *, user_id: str) -> None:
    """Drop rows whose foreign keys reference ``LOVE_ENGINE_UserTable.user_id``.
    ...
    ``LOVE_ENGINE_InvitationLink_user_id_fkey``).
    """
...
async def _create_user_if_not_exists(
    ...
    Returns:
        LOVE_ENGINE_UserTable if user was created, None if creation failed
    """
    from love_engine.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    try:
        ...
        if love_engine.default_internal_user_params:
            default_role = love_engine.default_internal_user_params.get("user_role")
...
async def _get_team_member_user_ids_from_team(team: LOVE_ENGINE_TeamTable) -> List[str]:
    """
...
async def get_users(
    ...
    try:
        ...
        if filter:
            # Okta locates users by userName before deprovisioning. LoveEngine
        ...
        users: List[LOVE_ENGINE_UserTable] = await UserRepository(
            prisma_client
        ).table.find_many(
        ...
        for user in users:
            scim_user = await ScimTransformations.transform_love_engine_user_to_scim_user(
                user=user
            )
...
async def get_user(
    ...
    try:
        ...
        # Convert to SCIM format
        scim_user = await ScimTransformations.transform_love_engine_user_to_scim_user(user)
        return scim_user
...
def _apply_patch_ops(
    existing_user: LOVE_ENGINE_UserTable,
...
async def get_group(
    ...
    try:
        ...
        scim_group = await ScimTransformations.transform_love_engine_team_to_scim_group(
            team
        )
...
async def patch_group(
    ...
    try:
        ...
        if refreshed_team:
            ...
            refreshed_current_members = set(
                await _get_team_member_user_ids_from_team(
                    LOVE_ENGINE_TeamTable(**refreshed_team.model_dump())
                )
            )
        ...
        # Convert to SCIM format and return
        scim_group = await ScimTransformations.transform_love_engine_team_to_scim_group(
            LOVE_ENGINE_TeamTable(**updated_team.model_dump())
        )
        return scim_group
