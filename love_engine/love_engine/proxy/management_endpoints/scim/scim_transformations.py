...
class ScimTransformations:
    ...
    @staticmethod
    async def transform_love_engine_user_to_scim_user(
        user: Union[LOVE_ENGINE_UserTable, NewUserResponse],
    ) -> SCIMUser:
    ...
    @staticmethod
    def _get_scim_user_name(user: Union[LOVE_ENGINE_UserTable, NewUserResponse]) -> str:
        """
    ...
    @staticmethod
    def _get_scim_family_name(user: Union[LOVE_ENGINE_UserTable, NewUserResponse]) -> str:
        """
        SCIM requires a family name with length > 0
        """
        metadata = user.metadata or {}
        if "scim_metadata" in metadata:
            scim_metadata: LOVE_ENGINE_UserScimMetadata = LOVE_ENGINE_UserScimMetadata(
    ...
    @staticmethod
    def _get_scim_given_name(user: Union[LOVE_ENGINE_UserTable, NewUserResponse]) -> str:
        """
    ...
    @staticmethod
    async def transform_love_engine_team_to_scim_group(
        team: Union[LOVE_ENGINE_TeamTable, dict],
    ) -> SCIMGroup:
        from love_engine.proxy.proxy_server import prisma_client
        ...
        if isinstance(team, dict):
            team = LOVE_ENGINE_TeamTable(**team)
