...
from love_engine.constants import LOVE_ENGINE_PROXY_ADMIN_NAME
...
from love_engine.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper
...
from love_engine.proxy.management_helpers.audit_logs import create_object_audit_log
from love_engine.proxy.utils import PrismaClient
from love_engine.repositories.model_repository import ModelRepository
from love_engine.repositories.table_repositories import ModelTableRepository
...
def update_db_model(
    ...
    merged_deployment_dict = DeploymentTypedDict(
        ...
        love_engine_params=LoveEngineParamsTypedDict(
            **db_model.love_engine_params.model_dump(exclude_none=True)  # type: ignore
        ),
    ...
    # update love_engine params
    if updated_patch.love_engine_params:
        # Encrypt any sensitive values
        encrypted_params = {
            k: encrypt_value_helper(v)
            for k, v in updated_patch.love_engine_params.model_dump(
                exclude_none=True
            ).items()
        }

        merged_deployment_dict["love_engine_params"].update(encrypted_params)  # type: ignore
    ...
    # silently undo a love_engine_params clear via .update().
    #
    ...
    # mirrored between love_engine_params and model_info by Deployment.__init__, so the
    # clear propagates to both blobs.
    if updated_patch.love_engine_params:
        for field in updated_patch.love_engine_params.model_fields_set:
            if (
                field in SPECIAL_MODEL_INFO_PARAMS
                and getattr(updated_patch.love_engine_params, field) is None
            ):
                merged_deployment_dict["love_engine_params"].pop(field, None)  # type: ignore
    ...
    if updated_patch.model_info:
        for field in updated_patch.model_info.model_fields_set:
            if (
                ...
                merged_deployment_dict.get("love_engine_params", {}).pop(field, None)  # type: ignore
    ...
    if "love_engine_params" in merged_deployment_dict:
        prisma_compatible_model_dict["love_engine_params"] = json.dumps(
            merged_deployment_dict["love_engine_params"]
        )
...
async def patch_model(
    ...
    try:
        ...
        update_data["updated_by"] = (
            user_api_key_dict.user_id or love_engine_proxy_admin_name
        )
        ...
        asyncio.create_task(
            create_object_audit_log(
                ...
                action="updated",
                user_api_key_dict=user_api_key_dict,
                table_name=LoveEngineTableNames.PROXY_MODEL_TABLE_NAME,
...
async def _add_model_to_db(
    ...
    # encrypt love_engine params #
    _love_engine_params_dict = model_params.love_engine_params.dict(exclude_none=True)
    _original_love_engine_model_name = model_params.love_engine_params.model
    for k, v in _love_engine_params_dict.items():
        encrypted_value = encrypt_value_helper(
            value=v, new_encryption_key=new_encryption_key
        )
        model_params.love_engine_params[k] = encrypted_value
    _data: dict = {
        "model_id": model_params.model_info.id,
        "model_name": model_params.model_name,
        "love_engine_params": model_params.love_engine_params.model_dump_json(exclude_none=True),  # type: ignore
    ...
    if should_create_model_in_db:
        ...
        model_response = LOVE_ENGINE_ProxyModelTable(**_data)
...
async def _update_team_model_in_db(
    ...
    if patch_data.model_info is None:
        from love_engine.types.router import ModelInfo
...
) -> List[LOVE_ENGINE_ProxyModelTable]:
    """
    ...
    async with prisma_client.db.tx() as tx:
        for team_id in team_ids:
            rows = await _get_team_deployments(
                team_id, prisma_client, table=tx.love_engine_proxymodeltable
            )
...
async def _remove_unbacked_team_models(
    ...
    # be a team alias value, so skip the full love_engine_modeltable scan for them.
    ...
    existing_team_row = await prisma_client.db.love_engine_teamtable.find_unique(
        where={"team_id": team_id}
    )
    if existing_team_row is None:
        return

    updated_team_row = await prisma_client.db.love_engine_teamtable.update(
        where={"team_id": team_id},
        data={
            "models": [
                model
...
class ModelManagementAuthChecks:
    ...
    def can_user_make_team_model_call(
        team_id: str,
        user_api_key_dict: UserAPIKeyAuth,
        team_obj: Optional[LOVE_ENGINE_TeamTable] = None,
    ...
    async def can_user_make_model_call(
        ...
        if (
            ...
            team_obj = LOVE_ENGINE_TeamTable(**team_obj_row.model_dump())
        ...
        elif user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN:
            raise HTTPException(
                status_code=403,
                detail={
...
#### [BETA] - This is a beta endpoint, format might change based on user feedback. - https://github.com/BerriAI/love_engine/issues/964
@router.post(
    "/model/delete",
    ...
    try:
        ...
        if prisma_client is None:
            raise HTTPException(
                status_code=500,
                detail={
                    "error": "No DB Connected. Here's how to do it - https://docs.love_engine.ai/docs/proxy/virtual_keys"
                },
            )
...
async def add_new_model(
    ...
    - love_engine_params: dict - LoveEngine-specific parameters (required)
        - model: str - The actual model identifier, e.g., "azure/my-deployment-name" (required - this is the only required field in love_engine_params)
    ...
    -H 'Authorization: Bearer LOVE_ENGINE_VIRTUAL_KEY' \
    ...
    try:
        ...
        model_response: Optional[LOVE_ENGINE_ProxyModelTable] = None
        # update DB
        if store_model_in_db is True:
            """
            - store model_list in db
            ...
            try:
                _original_love_engine_model_name = model_params.model_name
                ...
                if "slack" in _alerting:
                    ...
                    await proxy_logging_obj.slack_alerting_instance.model_added_alert(
                        model_name=model_params.model_name,
                        love_engine_model_name=_original_love_engine_model_name,
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.add_new_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_model(
    ...
    try:
        ...
        _existing_love_engine_params = await ModelRepository(
            prisma_client
        ).table.find_unique(where={"model_id": _model_id})

        if _existing_love_engine_params is None:
            if (
        ...
        deployment = Deployment(**_existing_love_engine_params.model_dump())
        ...
        # update DB
        if store_model_in_db is True:
            _existing_love_engine_params_dict = dict(
                _existing_love_engine_params.love_engine_params
            )

            if model_params.love_engine_params is None:
                raise Exception("love_engine_params not provided")

            _new_love_engine_params_dict = model_params.love_engine_params.dict(
                exclude_none=True
            )

            ### ENCRYPT PARAMS ###
            for k, v in _new_love_engine_params_dict.items():
                encrypted_value = encrypt_value_helper(value=v)
                model_params.love_engine_params[k] = encrypted_value

            ### MERGE WITH EXISTING DATA ###
            merged_dictionary = {}
            _mp = model_params.love_engine_params.dict()

            for key, value in _mp.items():
                if value is not None:
                    merged_dictionary[key] = value
                elif (
                    key in _existing_love_engine_params_dict
                    and _existing_love_engine_params_dict[key] is not None
                ):
                    merged_dictionary[key] = _existing_love_engine_params_dict[key]
                else:
                    pass

            _data: dict = {
                "love_engine_params": json.dumps(merged_dictionary),  # type: ignore
                "updated_by": user_api_key_dict.user_id or LOVE_ENGINE_PROXY_ADMIN_NAME,
            }
            ...
            asyncio.create_task(
                create_object_audit_log(
                    ...
                    table_name=LoveEngineTableNames.PROXY_MODEL_TABLE_NAME,
                    before_value=(
                        _existing_love_engine_params.model_dump_json(exclude_none=True)
                        if isinstance(_existing_love_engine_params, BaseModel)
                        else None
                    ),
                    after_value=(
                        model_response.model_dump_json(exclude_none=True)
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.update_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_public_model_groups(
    ...
    try:
        ...
        import love_engine
        from love_engine.proxy.proxy_server import proxy_config, store_model_in_db
        ...
        # Load existing config first (this may overwrite in-memory love_engine settings
        # from DB values via _update_config_from_db), so set the in-memory value AFTER
        config = await proxy_config.get_config()

        # Update config with new settings
        if "love_engine_settings" not in config:
            config["love_engine_settings"] = {}

        config["love_engine_settings"]["public_model_groups"] = request.model_groups
        ...
        love_engine.public_model_groups = request.model_groups
...
async def update_useful_links(
    ...
    try:
        ...
        config["love_engine_settings"]["public_model_groups_links"] = request.useful_links
        ...
        # Set in-memory value AFTER get_config() and save_config() to avoid
        # get_config() overwriting with stale DB value
        love_engine.public_model_groups_links = request.useful_links
...
def _deduplicate_love_engine_router_models(models: List[Dict]) -> List[Dict]:
    """
