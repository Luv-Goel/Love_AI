...
from love_engine.proxy.auth.auth_checks import can_user_call_model, get_user_object
...
from love_engine.proxy.management_endpoints.common_daily_activity import get_daily_activity
...
from love_engine.repositories.table_repositories import OrganizationMembershipRepository
...
async def _verify_org_access(
    ...
    from love_engine.proxy.proxy_server import proxy_logging_obj, user_api_key_cache
    ...
    for m in caller_user.organization_memberships or []:
        if (
            ...
            and m.user_role == LoveEngineUserRoles.ORG_ADMIN.value
        ):
            return
...
def handle_nested_budget_structure_in_organization_update_request(
    ...
    The UI sends nested budget data in 'love_engine_budget_table', but our
    model expects flat budget fields at the top level.
    """
    transformed_data = raw_data.copy()

    # Handle nested budget structure from UI
    if "love_engine_budget_table" in transformed_data:
        budget_data = transformed_data.pop("love_engine_budget_table", {})
        if budget_data:
            # Extract valid budget fields and merge into top level
            budget_fields = LOVE_ENGINE_BudgetTable.model_fields.keys()
...
async def new_organization(
    ...
    - object_permission: Optional[LOVE_ENGINE_ObjectPermissionBase] - organization-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"]}. IF null or {} then no object permission.
    ...
    user_object_correct_type: Optional[LOVE_ENGINE_UserTable] = None
    ...
    if user_api_key_dict.user_id is not None:
        try:
            ...
            user_object_correct_type = LOVE_ENGINE_UserTable(**user_object.model_dump())
        except Exception:
            pass
    ...
    if data.budget_id is None:
        ...
        """
        budget_params = LOVE_ENGINE_BudgetTable.model_fields.keys()

        # Only include Budget Params when creating an entry in love_engine_budgettable
        _json_data = data.json(exclude_none=True)
        _budget_data = {k: v for k, v in _json_data.items() if k in budget_params}
        budget_row = LOVE_ENGINE_BudgetTable(**_budget_data)
    ...
    organization_row = LOVE_ENGINE_OrganizationTable(
...
async def get_organization_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="love_engine_dailyorganizationspend",
...
async def _set_object_permission(
    ...
    """
    Creates the LOVE_ENGINE_ObjectPermissionTable record for the organization.
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LOVE_ENGINE_OrganizationTableWithMembers,
)
async def update_organization(
    ...
    data = LOVE_ENGINE_OrganizationTableUpdate(**raw_data_with_flat_budget_fields)
    ...
    budget_fields = {
        ...
        if k in LOVE_ENGINE_BudgetTable.model_fields.keys() and v is not None
    }
    ...
    for field in LOVE_ENGINE_BudgetTable.model_fields.keys():
    ...
    response = await OrganizationRepository(prisma_client).table.update(
        ...
        include={"members": True, "teams": True, "love_engine_budget_table": True},
    )
...
async def handle_update_object_permission(
    data_json: dict,
    existing_organization_row: LOVE_ENGINE_OrganizationTable,
) -> dict:
    """
    Handle the update of object permission for an organization.

    - Upserts the new object permission into the LOVE_ENGINE_ObjectPermissionTable
...
@router.delete(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[LOVE_ENGINE_OrganizationTableWithMembers],
)
...
async def info_organization(
    ...
    response_pydantic_obj = LOVE_ENGINE_OrganizationTableWithMembers(
        **response.model_dump()
    )
...
async def organization_member_add(
    ...
    - member: Union[List[Member], Member] (required)
        - role: Literal[LoveEngineUserRoles] (required)
    ...
    2. Creates a new Internal User if the user_id or user_email is not found in LOVE_ENGINE_UserTable
    3. Add Internal User to the `LOVE_ENGINE_OrganizationMembership` table
    """
    try:
        from love_engine.proxy.proxy_server import prisma_client
...
async def find_member_if_email(
    ...
    """
    Find a member if the user_email is in LOVE_ENGINE_UserTable
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Unique user not found for user_email={user_email}. Potential duplicate OR non-existent user_email in LOVE_ENGINE_UserTable. Use 'user_id' instead."
            },
        )
    existing_user_email_row_pydantic = LOVE_ENGINE_UserTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LOVE_ENGINE_OrganizationMembershipTable,
)
@management_endpoint_wrapper
async def organization_member_update(
    ...
    try:
        ...
        final_organization_membership_pydantic = LOVE_ENGINE_OrganizationMembershipTable(
...
async def add_member_to_organization(
    ...
    organization_id: str,
    prisma_client: PrismaClient,
) -> Tuple[LOVE_ENGINE_UserTable, LOVE_ENGINE_OrganizationMembershipTable]:
    """
    Add a member to an organization

    - Checks if member.user_id or member.user_email is in LOVE_ENGINE_UserTable
    - If not found, create a new user in LOVE_ENGINE_UserTable
    - Add user to organization in LOVE_ENGINE_OrganizationMembership
    """
    ...
    try:
        ...
        ## Check if user exists in LOVE_ENGINE_UserTable - user exists - either the user_id or user_email is in LOVE_ENGINE_UserTable
        ...
        if existing_user_id_row is None and member.user_email is not None:
            try:
                ...
                raise ValueError(
                    f"Potential NON-Existent or Duplicate user email in DB: Error finding a unique instance of user_email={member.user_email} in LOVE_ENGINE_UserTable.: {e}"
                )
        ...
        if existing_user_id_row is None and existing_user_email_row is None:
            ...
            if _returned_user is not None:
                user_object = LOVE_ENGINE_UserTable(**_returned_user.model_dump())
            ...
            user_object = LOVE_ENGINE_UserTable(**existing_user_email_row.model_dump())
        elif existing_user_id_row is not None:
            user_object = LOVE_ENGINE_UserTable(**existing_user_id_row.model_dump())
        else:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if user_object is None:
            raise ValueError(
                f"User does not exist in LOVE_ENGINE_UserTable. user_id={member.user_id} and user_email={member.user_email}"
            )
        ...
        organization_membership = LOVE_ENGINE_OrganizationMembershipTable(
