...
from love_engine.repositories.table_repositories import ConfigOverridesRepository
from love_engine.types.llms.custom_http import httpxSpecialProvider
from love_engine.types.proxy.management_endpoints.config_overrides import (
...
def _redact_config(config: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    ``client_key`` etc.  Persisting them verbatim into ``LOVE_ENGINE_AuditLogs``
...
async def _emit_hashicorp_vault_audit_log(
    *,
    ...
    ``LOVE_ENGINE_ConfigOverrides`` so the row co-locates with the table it
    mutates.
    """
    import love_engine

    if love_engine.store_audit_logs is not True:
        return
    ...
    task = asyncio.create_task(
        create_audit_log_for_update(
            request_data=LOVE_ENGINE_AuditLogs(
                ...
                changed_by=love_engine_changed_by
                or user_api_key_dict.user_id
                or love_engine_proxy_admin_name,
                changed_by_api_key=user_api_key_dict.api_key,
                table_name=LoveEngineTableNames.CONFIG_OVERRIDES_TABLE_NAME,
...
def _clear_hashicorp_vault_state(proxy_config: Any) -> None:
    ...
    _set_env_vars({})
    if love_engine._key_management_system == KeyManagementSystem.HASHICORP_VAULT:
        love_engine.secret_manager_client = None
        love_engine._key_management_system = None
...
async def test_hashicorp_vault_connection(
    ...
    client = love_engine.secret_manager_client
