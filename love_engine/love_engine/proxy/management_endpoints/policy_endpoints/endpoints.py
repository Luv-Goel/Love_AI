...
from love_engine.proxy.guardrails.guardrail_hooks.custom_code import (
...
from love_engine.types.proxy.policy_engine import (
...
from love_engine.types.utils import GenericGuardrailAPIInputs, ModelResponse
...
async def apply_policies(
    ...
    proxy_logging_obj: "LoveEngineLoggingObj",
...
async def test_policies_and_guardrails(
    ...
    """
    from love_engine.love_engine_core_utils.love_engine_logging import Logging as LoveEngineLoggingObj
    from love_engine.proxy.proxy_server import chat_completion, proxy_logging_obj
    ...
    try:
        logging_obj = cast(LoveEngineLoggingObj, proxy_logging_obj)
...
async def list_policies(
    ...
    - Inheritance chain
    """
    from love_engine.proxy.policy_engine.init_policies import get_policies_summary
...
POLICY_TEMPLATES_GITHUB_URL = (
    "https://raw.githubusercontent.com/BerriAI/love_engine/main/policy_templates.json"
)


def _load_policy_templates_from_local_backup() -> list:
    """Load policy templates from local backup file (love_engine/policy_templates_backup.json)."""
...
async def get_policy_templates(
    ...
    Set LOVE_ENGINE_LOCAL_POLICY_TEMPLATES=true to skip GitHub and use local backup only.
    """
    use_local = os.getenv("LOVE_ENGINE_LOCAL_POLICY_TEMPLATES", "").strip().lower() in (
        "true",
        "1",
        "yes",
    )
...
def _build_competitor_guardrail_definitions(
    ...
    for defn in enriched:
        ...
        if guardrail_name in blocked_words_map:
            defn["love_engine_params"]["blocked_words"] = blocked_words_map[guardrail_name]
...
async def suggest_policy_templates(
    ...
    """
    from love_engine.proxy.management_endpoints.policy_endpoints.ai_policy_suggester import (
        AiPolicySuggester,
    )
...
async def _test_guardrail_definitions(
    ...
    for guardrail_def in guardrail_definitions:
        ...
        love_engine_params = guardrail_def.get("love_engine_params", {})
        guardrail_type = love_engine_params.get("guardrail", "")

        if guardrail_type != "love_engine_content_filter":
            results.append(
        ...
        try:
            guardrail = ContentFilterGuardrail(
                guardrail_name=guardrail_name,
                patterns=love_engine_params.get("patterns"),
                blocked_words=love_engine_params.get("blocked_words"),
                categories=love_engine_params.get("categories"),
                pattern_redaction_format=love_engine_params.get("pattern_redaction_format"),
                default_on=love_engine_params.get("default_on", False),
            )
