...
from love_engine.proxy.db.exception_handler import call_with_db_reconnect_retry
from love_engine.repositories.table_repositories import CacheConfigRepository
from love_engine.types.management_endpoints import (
...
def _log_audit_task_exception(task: "asyncio.Task[None]") -> None:
    """Surface a fire-and-love_smitht audit-log task failure as a warning.
...
async def _emit_cache_settings_audit_log(
    *,
    ...
    ``team_callback_endpoints.py``: fire-and-love_smitht, no-op when audit
    logging is disabled, with a done-callback that surfaces any task
    exception.  Captured under ``LOVE_ENGINE_CacheConfig`` so the row
    ...
    task = asyncio.create_task(
        create_audit_log_for_update(
            request_data=LOVE_ENGINE_AuditLogs(
                ...
                table_name=LoveEngineTableNames.CACHE_CONFIG_TABLE_NAME,
    ...
    async def init_cache_settings_in_db(prisma_client, proxy_config):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.management_endpoints.cache_settings_endpoints.py::CacheSettingsManager::init_cache_settings_in_db - {}".format(
                    str(e)
                )
            )
...
async def update_cache_settings(
    ...
    2. Saves to LOVE_ENGINE_CacheConfig table
