...
from love_engine.repositories.table_repositories import AccessGroupRepository
from love_engine.types.access_group import (
...
def _record_to_access_group_table(record) -> LOVE_ENGINE_AccessGroupTable:
    """Convert a Prisma record to a LOVE_ENGINE_AccessGroupTable pydantic object for caching."""
    return LOVE_ENGINE_AccessGroupTable(**record.dict())
...
async def _sync_add_access_group_to_teams(
    ...
    for team_id in team_ids:
        team = await tx.love_engine_teamtable.find_unique(where={"team_id": team_id})
        if team is not None and access_group_id not in (team.access_group_ids or []):
            await tx.love_engine_teamtable.update(
...
async def _sync_add_access_group_to_keys(
    ...
    for token in key_tokens:
        key = await tx.love_engine_verificationtoken.find_unique(where={"token": token})
        if key is not None and access_group_id not in (key.access_group_ids or []):
            await tx.love_engine_verificationtoken.update(
                where={"token": token},
                data={
...
async def create_access_group(
    ...
    try:
        async with prisma_client.db.tx() as tx:
            existing = await tx.love_engine_accessgrouptable.find_unique(
            ...
            record = await tx.love_engine_accessgrouptable.create(
                data={
...
async def update_access_group(
    ...
    try:
        async with prisma_client.db.tx() as tx:
            ...
            record = await tx.love_engine_accessgrouptable.update(
                where={"access_group_id": access_group_id},
                data=update_data,
            )
...
async def delete_access_group(
    ...
    try:
        ...
        async with prisma_client.db.tx() as tx:
            ...
            teams_with_group = await tx.love_engine_teamtable.find_many(
            ...
            keys_with_group = await tx.love_engine_verificationtoken.find_many(
                where={"access_group_ids": {"hasSome": [access_group_id]}}
            )
            ...
            await tx.love_engine_accessgrouptable.delete(
                where={"access_group_id": access_group_id}
            )
