...
from love_engine.repositories.table_repositories import EndUserRepository
...
async def unblock_user(data: BlockUsers):
    ...
    if (
        not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in love_engine.callbacks)
        or love_engine.blocked_user_list is None
    ):
        raise HTTPException(
            status_code=400,
            detail={
    ...
    if isinstance(love_engine.blocked_user_list, list):
        for id in data.user_ids:
            love_engine.blocked_user_list.remove(id)
    else:
        raise HTTPException(
            status_code=500,
            detail={
    ...
    return {"blocked_users": love_engine.blocked_user_list}
...
async def _handle_customer_object_permission_update(
    ...
    end_user_table_data_typed: Optional[LOVE_ENGINE_EndUserTable],
...
async def new_end_user(
    ...
    - object_permission: Optional[LOVE_ENGINE_ObjectPermissionBase] - Customer-specific object permissions to control access to resources.
        Supported fields:
    ...
    try:
                ...
                budget_record = await BudgetRepository(prisma_client).table.create(
                    data={
                        **_new_budget.model_dump(exclude_unset=True),
                        "created_by": user_api_key_dict.user_id or love_engine_proxy_admin_name,  # type: ignore
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LOVE_ENGINE_EndUserTable,
)
@router.get(
    "/end_user/info",
    tags=["Customer Management"],
...
async def end_user_info(
    ...
    Example curl:
    ```
    curl -X GET 'http://localhost:4000/customer/info?end_user_id=test-love_engine-user-4' \
        -H 'Authorization: Bearer sk-1234'
    ```
    """
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.management_endpoints.customer_endpoints.end_user_info(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def update_end_user(
    ...
    try:
        ...
        end_user_table_data = await EndUserRepository(prisma_client).table.find_first(
            where={"user_id": data.user_id}, include={"love_engine_budget_table": True}
        )
        ...
        end_user_table_data_typed = LOVE_ENGINE_EndUserTable(
            **end_user_table_data.model_dump()
        )

        ## Get budget table data ##
        end_user_budget_table = end_user_table_data_typed.love_engine_budget_table
        ...
        for k, v in non_default_values.items():
            ...
            elif k in LOVE_ENGINE_BudgetTable.model_fields.keys():
                budget_table_data[k] = v

            elif k in LOVE_ENGINE_EndUserTable.model_fields.keys():
                update_end_user_table_data[k] = v
        ...
        if data.user_id is not None and len(data.user_id) > 0:
            ...
            response = await EndUserRepository(prisma_client).table.update(
                where={"user_id": data.user_id}, data=update_end_user_table_data, include={"love_engine_budget_table": True, "object_permission": True}  # type: ignore
            )
            if response is None:
                raise ValueError(
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.update_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def delete_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.delete_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[LOVE_ENGINE_EndUserTable],
)
@router.get(
    "/end_user/list",
    tags=["Customer Management"],
...
async def list_end_user(
    ...
    try:
        ...
        returned_response: List[LOVE_ENGINE_EndUserTable] = []
        for item in response:
            ...
            returned_response.append(LOVE_ENGINE_EndUserTable(**item_dict))
        return returned_response

    except Exception as e:
        verbose_proxy_logger.exception(
            "love_engine.proxy.management_endpoints.customer_endpoints.list_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
