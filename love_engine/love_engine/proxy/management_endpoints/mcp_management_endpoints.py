...
from love_engine.proxy.management_helpers.audit_logs import get_audit_log_changed_by
...
TEMPORARY_MCP_SERVER_REDIS_KEY_PREFIX = "love_engine:mcp:temporary_server"
...
if MCP_AVAILABLE:
    ...
    from love_engine.proxy._experimental.mcp_server.ui_session_utils import (
    ...
    from love_engine.types.mcp import MCPAuth, MCPCredentials
    from love_engine.types.mcp_server.mcp_server_manager import MCPServer
    ...
    async def _cache_temporary_mcp_server_in_redis(
        ...
        """
        if love_engine.cache is None or not hasattr(love_engine.cache, "cache"):
            return
        cache_backend = getattr(love_engine.cache, "cache", None)
    ...
    def _redact_mcp_credentials_list(
        mcp_servers: Iterable[LOVE_ENGINE_MCPServerTable],
    ) -> List[LOVE_ENGINE_MCPServerTable]:
        return [_redact_mcp_credentials(server) for server in mcp_servers]

    def _redact_global_env_var_values(mcp_server: LOVE_ENGINE_MCPServerTable) -> None:
    ...
    def _user_is_full_admin(user_api_key_dict: UserAPIKeyAuth) -> bool:
        ...
        """
        return user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN
    ...
    def _sanitize_mcp_server_for_non_admin(
        ...
        ``LOVE_ENGINE_MCPServerTable`` (``None`` for Optional fields,
    ...
    async def get_mcp_tools(
        ...
        """
        from love_engine.proxy._experimental.mcp_server.server import _list_mcp_tools
    ...
    async def _resolve_accessible_mcp_servers(
        ...
        aggregated: Dict[str, LOVE_ENGINE_MCPServerTable] = {}
    ...
    @router.get(
        ...
        dependencies=[Depends(user_api_key_auth)],
        response_model=List[LOVE_ENGINE_MCPServerTable],
    )
    async def fetch_all_mcp_servers(
        ...
        if love_engine.public_mcp_servers is not None:
            for server in redacted_mcp_servers:
                if server.server_id in love_engine.public_mcp_servers:
        ...
        from love_engine.proxy.proxy_server import prisma_client as _byok_prisma_client
    ...
    @router.post(
        ...
        dependencies=[Depends(user_api_key_auth)],
        response_model=LOVE_ENGINE_MCPServerTable,
    ...
    async def approve_mcp_server_submission(
        ...
        """
        if LoveEngineUserRoles.PROXY_ADMIN != user_api_key_dict.user_role:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={
    ...
    async def _mcp_oauth_user_api_key_auth(request: Request) -> UserAPIKeyAuth:
        ...
        from love_engine.proxy.proxy_server import master_key
        ...
        # exchange) must work without a LoveEngine session.  /authorize is opened
        ...
        # forward the request to the upstream issuer with LoveEngine's stored
        ...
        # us. Require normal LoveEngine auth for those.
        if not api_key:
            from love_engine.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415
                global_mcp_server_manager,
            )
                ...
                if (
                    ...
                    if path_lower.endswith("/token"):
                        ...
                        if grant_type != "authorization_code":
                            # Fall through to normal LoveEngine auth (will 401 if
                            # no key supplied).
                            pass
                        else:
    ...
    async def remove_mcp_server(
        ...
        if love_engine.store_audit_logs:
            pass
    ...
    def _compute_user_env_var_status(
        *,
        server: LOVE_ENGINE_MCPServerTable,
    ...
    async def make_mcp_servers_public(
        ...
        try:
            ...
            if love_engine.public_mcp_servers is None:
                love_engine.public_mcp_servers = []
            ...
            love_engine.public_mcp_servers = request.mcp_server_ids

            # Update config with new settings
            if "love_engine_settings" not in config or config["love_engine_settings"] is None:
            ...
            verbose_proxy_logger.debug(
                f"Updated public mcp servers to: {love_engine.public_mcp_servers} by user: {user_api_key_dict.user_id}"
            )

            return {
                "message": "Successfully updated public mcp servers",
                "public_mcp_servers": love_engine.public_mcp_servers,
    ...
    from love_engine.types.mcp_server.mcp_toolset import (
    ...
    async def add_mcp_toolset(
        ...
        user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
        love_engine_changed_by: Optional[str] = Header(None),
    ):
        ...
        touched_by = (
            ...
            or LOVE_ENGINE_PROXY_ADMIN_NAME
        )
        try:
