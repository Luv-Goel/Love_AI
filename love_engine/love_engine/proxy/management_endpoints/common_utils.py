...
from love_engine.proxy._types import (  # noqa: F401  re-exported
...
if TYPE_CHECKING:
    from love_engine.proxy._types import NewProjectRequest, UpdateProjectRequest
...
def _is_user_team_admin(
    user_api_key_dict: UserAPIKeyAuth, team_obj: LOVE_ENGINE_TeamTable
) -> bool:
...
async def _user_has_admin_privileges(
    ...
    from love_engine.caching import DualCache as DualCacheImport
    from love_engine.proxy.auth.auth_checks import get_user_object
    ...
    try:
        ...
        if user_obj.organization_memberships is not None:
            for membership in user_obj.organization_memberships:
                if membership.user_role == LoveEngineUserRoles.ORG_ADMIN.value:
                    return True
...
def _org_admin_can_invite_user(
    admin_user_obj: LOVE_ENGINE_UserTable,
    target_user_obj: LOVE_ENGINE_UserTable,
) -> bool:
    """
...
def _set_object_metadata_field(
    ...
    """
    if field_name in LOVE_ENGINE_ManagementEndpoint_MetadataFields_Premium and value:
...
async def _upsert_budget_and_membership(
    ...
    *,
    ...
    async def _disconnect():
        await tx.love_engine_teammembership.update(
            where={"user_id_team_id": {"user_id": user_id, "team_id": team_id}},
            data={"love_engine_budget_table": {"disconnect": True}},
        )

    if existing_budget_id is not None and not is_shared_default:
        existing_budget = await tx.love_engine_budgettable.find_unique(
        ...
        if not _has_meaningful_budget_limit(merged):
            await _disconnect()
            return
        await tx.love_engine_budgettable.update(
            where={"budget_id": existing_budget_id},
    ...
    if is_shared_default:
        default_budget_row = await tx.love_engine_budgettable.find_unique(
            where={"budget_id": existing_budget_id}
        )
    ...
    new_budget = await tx.love_engine_budgettable.create(
        data=create_data,
        include={"team_membership": True},
    )
    await tx.love_engine_teammembership.upsert(
        where={
            "user_id_team_id": {
                "user_id": user_id,
                "team_id": team_id,
...
def _update_metadata_field(updated_kv: dict, field_name: str) -> None:
    ...
    """
    if field_name in LOVE_ENGINE_ManagementEndpoint_MetadataFields_Premium:
