...
from love_engine.caching.redis_cache import RedisCache
from love_engine.types.llms.openai import ResponsesAPIResponse, ResponsesAPIStatus


class ResponsePollingHandler:
    """Handles polling-based responses with Redis cache"""

    CACHE_KEY_PREFIX = "love_engine:polling:response:"
    POLLING_ID_PREFIX = "love_engine_poll_"  # Clear prefix to identify polling IDs
...
def should_use_polling_for_request(
    ...
    Args:
        ...
        llm_router: LoveEngine router instance for looking up model deployments
        ...
        # Otherwise, check ALL deployments for this model_name in router
            ...
            try:
                ...
                for idx in indices:
                    ...
                    love_engine_params = deployment_dict.get("love_engine_params", {})

                    # Check custom_llm_provider first
                    dep_provider = love_engine_params.get("custom_llm_provider")

                    # Then try to extract from model (e.g., "openai/gpt-5")
                    if not dep_provider:
                        dep_model = love_engine_params.get("model", "")
                        if "/" in dep_model:
