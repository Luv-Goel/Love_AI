...
from love_engine.types.utils import SpecialEnums
...
def get_models_from_unified_file_id(unified_file_id: str) -> List[str]:
    ...
    Example:
    unified_file_id = "love_engine_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini,gemini-2.0-flash"
...
def get_model_id_from_unified_batch_id(file_id: str) -> Optional[str]:
    ...
    Expected format: love_engine_proxy;model_id:{};llm_batch_id:{};llm_output_file_id:{}
    """
...
def encode_file_id_with_model(
    ...
    Format: <prefix><base64(love_engine:<original_id>;model,<model_name>)>
    ...
    Args:
        ...
        model: Model name from model_list (e.g., "gpt-4o-love_engine")
    ...
    Examples:
        encode_file_id_with_model("file-abc123", "gpt-4o-love_engine")
    ...
    """
    encoded_str = f"love_engine:{file_id};model,{model}"
...
def decode_model_from_file_id(encoded_id: str) -> Optional[str]:
    ...
    try:
        ...
        decoded = base64.urlsafe_b64decode(padded).decode()
        if decoded.startswith("love_engine:") and ";model," in decoded:
...
def get_original_file_id(encoded_id: str) -> str:
    ...
    try:
        ...
        if decoded.startswith("love_engine:") and ";model," in decoded:
            match = re.search(r"love_engine:([^;]+);model,", decoded)
            if match:
...
def extract_model_from_sources(
    ...
    2. Request headers (x-love_engine-model)
...
def get_team_provider_credentials(
    ...
    from love_engine.proxy._types import SpecialModelNames
...
def prepare_data_with_credentials(
    ...
    if include_internal_credentials:
        data["_love_engine_internal_model_credentials"] = MappingProxyType(
            dict(credentials)
        )
...
def validate_managed_files_requirement(
    ...
    """
    Enforce proxy-level managed files when love_engine.require_managed_files is enabled.
    ...
    if love_engine.require_managed_files is not True:
        return
    ...
    if not target_model_names:
        raise HTTPException(
            ...
            detail=(
                ...
                "in love_engine_settings. Provide one or more model aliases via the "
    ...
    if model:
        raise HTTPException(
            ...
            detail=(
                ...
                "love_engine_settings. Uploads must go through managed files using "
    ...
    for file_attr in ("output_file_id", "error_file_id"):
        ...
        try:
            ...
            await managed_files_obj.store_unified_file_id(
                ...
                file_object=None,
                love_engine_parent_otel_span=getattr(
...
async def get_batch_from_database(
    ...
    Returns:
        ...
        - response_batch: Parsed LoveEngineBatch object (or None)
    """
    import json
    ...
    try:
        ...
        response = LoveEngineBatch(**batch_data)
