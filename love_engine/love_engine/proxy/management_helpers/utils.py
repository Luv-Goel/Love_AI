...
from love_engine.proxy._types import (  # key request types; user request types; team request types; customer request types
    BudgetNewRequest,
...
def get_new_internal_user_defaults(
    ...
    user_info = love_engine.default_internal_user_params or {}

    returned_dict: SSOUserDefinedValues = {
        "models": user_info.get("models") or [],
        "max_budget": user_info.get("max_budget", love_engine.max_internal_user_budget),
...
async def handle_budget_for_entity(
    ...
    Args:
        ...
        love_engine_proxy_admin_name: Admin name for audit trail
...
# Fields on LOVE_ENGINE_BudgetTable that represent the budget's *configuration*
...
async def add_new_member(
...
) -> Tuple[LOVE_ENGINE_UserTable, Optional[LOVE_ENGINE_TeamMembership]]:
    """
    ...
    """
    returned_user: Optional[LOVE_ENGINE_UserTable] = None
    returned_team_membership: Optional[LOVE_ENGINE_TeamMembership] = None
    ...
    if new_member.user_id is not None:
        ...
        if _returned_user is not None:
            returned_user = LOVE_ENGINE_UserTable(**_returned_user.model_dump())
    ...
    if _budget_id and returned_user is not None and returned_user.user_id is not None:
        ...
        returned_team_membership = LOVE_ENGINE_TeamMembership(
