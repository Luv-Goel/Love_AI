"""
Functions to create audit logs for LoveEngine Proxy
"""
...
from love_engine.repositories.table_repositories import AuditLogRepository
from love_engine.types.utils import StandardAuditLogPayload

_audit_log_callback_cache: Dict[str, CustomLogger] = {}
ALLOW_LOVE_ENGINE_CHANGED_BY_HEADER_METADATA_KEY = "allow_love_engine_changed_by_header"


def _allows_love_engine_changed_by_header(user_api_key_dict: UserAPIKeyAuth) -> bool:
    for admin_metadata in (user_api_key_dict.metadata, user_api_key_dict.team_metadata):
        if (
            isinstance(admin_metadata, dict)
            and admin_metadata.get(ALLOW_LOVE_ENGINE_CHANGED_BY_HEADER_METADATA_KEY) is True
        ):
            return True
    return False
...
def get_audit_log_changed_by(
    *,
    ...
    if love_engine_changed_by and _allows_love_engine_changed_by_header(user_api_key_dict):
        return love_engine_changed_by
    return user_api_key_dict.user_id or love_engine_proxy_admin_name
...
def _resolve_audit_log_callback(name: str) -> Optional[CustomLogger]:
    ...
    For "s3_v2" with `love_engine.s3_audit_callback_params` set, constructs a
    ...
    if (
        name == "s3_v2"
        and getattr(love_engine, "s3_audit_callback_params", None) is not None
    ):
        from love_engine.integrations.s3_v2 import S3Logger as S3V2Logger

        instance = S3V2Logger(
            s3_callback_params_override=love_engine.s3_audit_callback_params
        )
    else:
...
def _build_audit_log_payload(
    ...
    """Convert LOVE_ENGINE_AuditLogs to StandardAuditLogPayload for callback dispatch."""
    updated_at = ""
    ...
    table_name_str: str = (
        ...
        if isinstance(request_data.table_name, LoveEngineTableNames)
...
async def _dispatch_audit_log_to_callbacks(
    ...
    if not love_engine.audit_log_callbacks:
        return

    payload = _build_audit_log_payload(request_data)

    for callback in love_engine.audit_log_callbacks:
        try:
...
async def create_object_audit_log(
    ...
    _store_audit_logs: Optional[bool] = love_engine.store_audit_logs or get_secret_bool(
        "LOVE_ENGINE_STORE_AUDIT_LOGS"
    )
...
async def create_audit_log_for_update(request_data: LOVE_ENGINE_AuditLogs):
    """
