...
from love_engine.repositories.table_repositories import ManagedObjectRepository
...
_NEGATIVE_OWNER_SENTINEL = "__love_engine_container_no_owner__"
...
_NEGATIVE_STORED_ID_SENTINEL = "__love_engine_container_no_stored_id__"
...
# ``love_engine_managedobjecttable``. The cache key is the sorted owner-scope
...
async def get_container_forwarding_params(
    ...
    if not (isinstance(model_id, str) and model_id):
        # Native upstream IDs (e.g. Azure ``cntr_<hex>``) carry no LoveEngine
