# love_engine/proxy/guardrails/guardrail_registry.py
...
from love_engine.proxy.guardrails.guardrail_hooks.grayswan import (
    GraySwanGuardrail,
)
from love_engine.proxy.guardrails.guardrail_hooks.grayswan import (
...
def get_guardrail_initializer_from_hooks():
    ...
    try:
        ...
        for item in os.listdir(hooks_dir):
            ...
            module_path = f"love_engine.proxy.guardrails.guardrail_hooks.{item}"
            try:
                # Import the module
...
class GuardrailRegistry:
    ...
    async def add_guardrail_to_db(
        ...
        try:
            ...
            # Properly serialize LoveEngineParams Pydantic model to dict
            love_engine_params_obj: Any = guardrail.get("love_engine_params", {})
            if hasattr(love_engine_params_obj, "model_dump"):
                love_engine_params_dict = love_engine_params_obj.model_dump()
            else:
                love_engine_params_dict = (
                    dict(love_engine_params_obj) if love_engine_params_obj else {}
                )
            love_engine_params: str = safe_dumps(love_engine_params_dict)
...
class InMemoryGuardrailHandler:
    ...
    def initialize_guardrail(
        ...
        """
        Initialize a guardrail from a dictionary and add it to the love_engine callback manager
        ...
        love_engine_params_data = guardrail["love_engine_params"]
        verbose_proxy_logger.debug("love_engine_params= %s", love_engine_params_data)

        if isinstance(love_engine_params_data, dict):
            love_engine_params = LoveEngineParams(**love_engine_params_data)
        else:
            love_engine_params = love_engine_params_data

        if (
            "category_thresholds" in love_engine_params_data
            and love_engine_params_data["category_thresholds"]
        ):
            lakera_category_thresholds = LakeraCategoryThresholds(
                **love_engine_params_data["category_thresholds"]
            )
            love_engine_params.category_thresholds = lakera_category_thresholds

        if love_engine_params.api_key and love_engine_params.api_key.startswith("os.environ/"):
            love_engine_params.api_key = str(get_secret(love_engine_params.api_key))

        if love_engine_params.api_base and love_engine_params.api_base.startswith(
            "os.environ/"
        ):
            love_engine_params.api_base = str(get_secret(love_engine_params.api_base))

        guardrail_type = love_engine_params.guardrail
        ...
        if initializer:
            ...
            if "llm_router" in sig.parameters:
                custom_guardrail_callback = initializer(
                    love_engine_params, guardrail, llm_router  # type: ignore
                )
            else:
                custom_guardrail_callback = initializer(love_engine_params, guardrail)
        ...
        if custom_guardrail_callback is not None:
            setattr(
                ...
                getattr(love_engine_params, "skip_system_message_in_guardrail", None),
            )
            setattr(
                custom_guardrail_callback,
                "skip_tool_message_in_guardrail",
                getattr(love_engine_params, "skip_tool_message_in_guardrail", None),
            )
    ...
    def initialize_custom_guardrail(
        ...
        This initializes it by adding it to the love_engine callback manager
        """
        ...
        # Extract additional params from love_engine_params to pass to custom guardrail
        ...
        if hasattr(love_engine_params, "model_dump"):
            extra_params = love_engine_params.model_dump(exclude_none=True)
        else:
            extra_params = dict(love_engine_params) if love_engine_params else {}
        ...
        love_engine.logging_callback_manager.add_love_engine_callback(_guardrail_callback)  # type: ignore
    ...
    def update_in_memory_guardrail(
        ...
        - updates the guardrail params in love_engine.callback_manager
        """
        ...
        if custom_guardrail_callback:
            updated_love_engine_params = cast(
                LoveEngineParams, guardrail.get("love_engine_params", {})
            )
            custom_guardrail_callback.update_in_memory_love_engine_params(
                love_engine_params=updated_love_engine_params
            )

    def delete_in_memory_guardrail(self, guardrail_id: str) -> None:
        """
        Delete a guardrail in memory and remove from love_engine callbacks.

        The callback is purged from every callback list, not just
        love_engine.callbacks: request handling promotes guardrail callbacks into the
        success/failure/async lists, so removing it from only love_engine.callbacks
        ...
        love_engine.logging_callback_manager.remove_callback_from_all_lists(
            custom_guardrail_callback
        )
    ...
    @staticmethod
    def _normalize_love_engine_params_for_comparison(
        params: Optional[Any],
    ) -> Optional[Dict[str, Any]]:
        """
        Render love_engine_params to a canonical dict so an in-memory LoveEngineParams and
        the raw dict loaded from the DB compare equal when they describe the same
        config. The in-memory side is a LoveEngineParams whose model_dump() carries
        ...
        forever; normalizing both through LoveEngineParams keeps the diff meaningful.
        """
        if params is None:
            return None
        if isinstance(params, LoveEngineParams):
            return params.model_dump()
        if isinstance(params, dict):
            try:
                return LoveEngineParams(**params).model_dump()
            except ValidationError as e:
                verbose_proxy_logger.warning(
                    f"Could not normalize guardrail love_engine_params for comparison; "
    ...
    def _has_guardrail_params_changed(
        ...
        # Compare love_engine_params
        existing_dict = self._normalize_love_engine_params_for_comparison(
            existing.get("love_engine_params")
        )
        new_dict = self._normalize_love_engine_params_for_comparison(
            new_guardrail.get("love_engine_params")
        )
        ...
        if existing_dict is not None and new_dict is not None:
            ...
            changed_fields = {"love_engine_params": {"old": existing_dict, "new": new_dict}}
    ...
    def reinitialize_guardrail(
        ...
        Removes old callback from love_engine.callbacks and creates fresh instance.
        """
        guardrail_id = guardrail.get("guardrail_id")
        if not guardrail_id:
        ...
        # Initialize fresh (will add new callback to love_engine.callbacks)
...
########################################################
# In Memory Guardrail Handler for LoveEngine Proxy
########################################################
