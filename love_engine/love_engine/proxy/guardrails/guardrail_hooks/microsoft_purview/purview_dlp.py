"""
Microsoft Purview DLP Guardrail for LoveEngine.
...
class MicrosoftPurviewDLPGuardrail(PurviewGuardrailBase, CustomGuardrail):
    ...
    def __init__(
        ...
        client_id: str,
        client_secret: str,
        purview_app_name: str = "LoveEngine",
    ...
    async def _check_content(
        ...
        try:
            ...
            correlation_id = request_data.get("love_engine_call_id") or str(uuid.uuid4())
    ...
    def _responses_api_input_to_str(
        ...
        LoveEngine transformation and then concatenated by ``get_prompt_text_for_dlp``.
    ...
    def logging_hook(
        ...
        """Fire-and-love_smitht async audit logging; returns original (kwargs, result) immediately.

        In the proxy's async success path, love_engine independently calls both
    ...
    async def async_logging_hook(
        ...
        try:
            ...
            if call_type in ("responses", "aresponses"):
                ...
                # before the generic ``messages`` branch.  love_engine's logging
