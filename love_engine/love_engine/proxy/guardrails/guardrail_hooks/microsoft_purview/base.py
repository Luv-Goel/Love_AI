...
class PurviewGuardrailBase:
    ...
    def _resolve_user_id(
        ...
        decreasing trust order:
            ...
            1. ``user_api_key_dict.user_id`` — LoveEngine key / JWT-bound user
        ...
        metadata = data.get("metadata") or data.get("love_engine_metadata") or {}
    ...
    def _logging_kwargs_metadata(kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(love_engine_params, dict):
            return {}
        md = love_engine_params.get("metadata")
    ...
    def _resolve_trusted_user_id(
        ...
        Uses only ``UserAPIKeyAuth.user_id`` (bound on the LoveEngine key or JWT).
