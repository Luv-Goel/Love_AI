...
class PromptSecurityGuardrail(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        This method is called by LoveEngine's guardrail framework for ALL endpoints:
        - /chat/completions
        - /responses
        - /messages (Anthropic)
        - /embeddings
    ...
    def _resolve_key_alias_from_request_data(request_data: dict) -> Optional[str]:
        ...
        # Check love_engine_metadata first (set by guardrail framework)
        love_engine_metadata = request_data.get("love_engine_metadata", {})
        if love_engine_metadata:
            alias = love_engine_metadata.get("user_api_key_alias")
            if alias:
                return alias
    ...
    async def sanitize_file_content(
        ...
        if user_api_key_alias:
            headers["X-LoveEngine-Key-Alias"] = user_api_key_alias
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.prompt_security import (
