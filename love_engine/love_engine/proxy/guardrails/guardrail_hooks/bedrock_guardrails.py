...
from love_engine.exceptions import GuardrailInterventionNormalStringError
...
from love_engine.types.llms.openai import AllMessageValues, ChatCompletionUserMessage
from love_engine.types.proxy.guardrails.guardrail_hooks.bedrock_guardrails import (
    BedrockContentItem,
...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    def _build_response_content_items(
        ...
        if not isinstance(response, love_engine.ModelResponse):
            return items
        for choice in response.choices:
            if (
                isinstance(choice, love_engine.Choices)
    ...
    def convert_to_bedrock_format(
        ...
        """
        Convert the love_engine messages/response to the bedrock request format.
    ...
    async def make_bedrock_api_request(
        ...
        source: Literal["INPUT", "OUTPUT"],
        messages: Optional[List[AllMessageValues]] = None,
        response: Optional[Union[Any, love_engine.ModelResponse]] = None,
    ...
    def create_guardrail_blocked_response(self, response: str) -> ModelResponse:
        from love_engine.types.utils import Choices, Message, ModelResponse
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, love_engine.ModelResponse):
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        from love_engine.types.utils import TextCompletionResponse
    ...
    def _apply_masking_to_model_response(
        self, response: love_engine.ModelResponse, masked_texts: List[str]
    ) -> None:
        """
