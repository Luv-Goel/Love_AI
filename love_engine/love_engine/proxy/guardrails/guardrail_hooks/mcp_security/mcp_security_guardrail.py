"""
MCP Security Guardrail for LoveEngine.

Validates that MCP servers referenced in request tools are registered
on the LoveEngine gateway. Blocks or alerts when unregistered servers are found.
"""
...
class MCPSecurityGuardrail(CustomGuardrail):
    ...
    def _extract_mcp_server_names_from_tools(tools: List[dict]) -> Set[str]:
        """Extract MCP server names from tools with type=mcp and love_engine_proxy server_url."""
        ...
        for tool in tools:
            ...
            if not isinstance(server_url, str):
                continue
            if server_url.startswith(LOVE_ENGINE_PROXY_MCP_SERVER_URL_PREFIX):
                name = server_url[len(LOVE_ENGINE_PROXY_MCP_SERVER_URL_PREFIX) :]
                if name:
