...
class GraySwanGuardrail(CustomGuardrail):
    ...
    which automatically works with all LoveEngine endpoints:
    ...
    def _prepare_payload(
        ...
        if isinstance(love_engine_metadata, dict) and love_engine_metadata:
            cleaned_love_engine_metadata = dict(love_engine_metadata)
            # cleaned_love_engine_metadata.pop("user_api_key_auth", None)
            sanitized = safe_json_loads(
                safe_dumps(cleaned_love_engine_metadata), default={}
            )
            if isinstance(sanitized, dict) and sanitized:
                payload["love_engine_metadata"] = sanitized
