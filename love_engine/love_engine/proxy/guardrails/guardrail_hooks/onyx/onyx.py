...
class OnyxGuardrail(CustomGuardrail):
    ...
    async def _validate_with_guard_server(
        ...
        response = await self.async_handler.post(
            f"{self.api_base}/guard/evaluate/v1/{self.api_key}/love_engine",
            json={
                "payload": payload,
    ...
    async def apply_guardrail(
        ...
        conversation_id = (
            logging_obj.love_engine_call_id if logging_obj else str(uuid.uuid4())
        )
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.onyx import (
