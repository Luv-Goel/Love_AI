...
from love_engine.integrations.custom_guardrail import dc as global_cache
...
from love_engine.proxy.guardrails._content_utils import (
...
class LassoGuardrail(CustomGuardrail):
    """
    Lasso Security Guardrail integration for LoveEngine.
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, love_engine.ModelResponse):
            ...
            if response_messages:
                # Include love_engine_call_id from original data for conversation_id consistency
                response_data = {
                    "messages": response_messages,
                    "love_engine_call_id": data.get("love_engine_call_id"),
                }
    ...
    def _get_or_generate_conversation_id(self, data: dict, cache: DualCache) -> str:
        ...
        This method ensures session consistency by using love_engine_call_id as a cache key.
        ...
        Example:
            ...
            >>> data = {"love_engine_call_id": "call_123"}
            >>> conversation_id = guardrail._get_or_generate_conversation_id(data, cache)
            >>> # Returns consistent ID for same love_engine_call_id

        Args:
            data: The request data containing love_engine_call_id
        ...
        # Get the love_engine_call_id which is consistent across all hooks for this request
        love_engine_call_id = data.get("love_engine_call_id")

        if not love_engine_call_id:
            # Fallback to generating a new ULID if no love_engine_call_id available
            return self._generate_ulid()

        # Use love_engine_call_id as cache key for conversation_id
        cache_key = f"lasso_conversation_id:{love_engine_call_id}"
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.lasso import (
