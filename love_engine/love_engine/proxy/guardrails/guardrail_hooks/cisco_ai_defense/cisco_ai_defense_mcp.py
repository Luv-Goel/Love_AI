...
class _CiscoAIDefenseMcpMixin:
    ...
    def _build_mcp_request_payload(
        ...
        if data.get("jsonrpc") == "2.0":
            return {
                "jsonrpc": "2.0",
                "id": (data.get("id") or data.get("love_engine_call_id") or "love_engine-mcp"),
        ...
        return {
            "jsonrpc": "2.0",
            "id": data.get("love_engine_call_id") or "love_engine-mcp",
            "method": "tools/call",
            "params": {
                "name": tool_name,
    ...
    def _build_mcp_response_payload(
        ...
        if response_id not in (None, "love_engine-mcp"):
            payload["id"] = response_id
        elif payload.get("id") in (None, "love_engine-mcp"):
            request_id = request_data.get("love_engine_call_id") or request_data.get("id")
            if request_id:
    ...
    def _normalize_mcp_response(response: object) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(response, dict):
            ...
            if isinstance(response.get("result"), dict):
                return {
                    "jsonrpc": "2.0",
                    "id": response.get("id") or "love_engine-mcp",
