...
from love_engine.types.utils import LLMResponseTypes
...
class PillarGuardrail(CustomGuardrail):
    """
    Pillar Security Guardrail for LoveEngine.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Note:
            LoveEngine virtual key context (user_id, team_id, key_alias, etc.) is always
            automatically passed as X-LoveEngine-* headers to enable application/user tracking.
        """
    ...
    async def async_pre_call_hook(
        ...
        Args:
            ...
            cache: LoveEngine cache instance
            data: Request data
    ...
    def _prepare_headers(self, user_api_key_dict: UserAPIKeyAuth) -> Dict[str, str]:
        ...
        # Always add LoveEngine virtual key context headers (metadata excluded for security)
        context_mapping = {
            "X-LoveEngine-Key-Name": user_api_key_dict.key_name,
            "X-LoveEngine-Key-Alias": user_api_key_dict.key_alias,
            "X-LoveEngine-User-Id": user_api_key_dict.user_id,
            "X-LoveEngine-User-Email": user_api_key_dict.user_email,
            "X-LoveEngine-Team-Id": user_api_key_dict.team_id,
            "X-LoveEngine-Team-Name": user_api_key_dict.team_alias,
            "X-LoveEngine-Org-Id": user_api_key_dict.org_id,
        }
    ...
    def _extract_model_and_provider(self, data: dict) -> Tuple[str, str]:
        ...
        # Use LoveEngine's standard provider detection and model cleaning
        try:
    ...
    def _prepare_payload(self, data: dict) -> Dict[str, Any]:
        ...
        # User ID: use LoveEngine user field
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        ...
        """
        from love_engine.types.proxy.guardrails.guardrail_hooks.pillar import (
