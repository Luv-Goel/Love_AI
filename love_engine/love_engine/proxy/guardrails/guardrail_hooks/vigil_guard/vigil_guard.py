...
from love_engine.exceptions import Timeout as LoveEngineTimeout
...
class VigilGuardGuardrail(CustomGuardrail):
    ...
    def _collect_metadata(
        self, request_data: dict, logging_obj: Optional["LoveEngineLoggingObj"]
    ) -> Dict[str, Any]:
        sources: List[dict] = []
        if isinstance(request_data, dict):
            sources.append(request_data)
            for nested_key in ("metadata", "love_engine_metadata"):
        ...
        if call_id:
            collected["love_engine_call_id"] = call_id
    ...
    def _extract_call_id(
        request_data: dict, logging_obj: Optional["LoveEngineLoggingObj"]
    ) -> Optional[str]:
        ...
        if isinstance(request_data, dict):
            ...
            if isinstance(metadata, dict):
                nested = metadata.get("love_engine_call_id")
