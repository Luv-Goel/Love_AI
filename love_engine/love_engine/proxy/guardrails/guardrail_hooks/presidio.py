...
# |   https://github.com/BerriAI/love_engine/issues/  |
# +-----------------------------------------------+
#
...
from love_engine.types.proxy.guardrails.guardrail_hooks.presidio import (
    PresidioAnalyzeRequest,
    PresidioAnalyzeResponseItem,
)
from love_engine.types.utils import GuardrailStatus, StreamingChoices
...
class _OPTIONAL_PresidioPIIMasking(CustomGuardrail):
    ...
    def validate_environment(
        ...
        self.presidio_anonymizer_api_base: Optional[
            str
        ] = presidio_anonymizer_api_base or love_engine.get_secret(
    ...
    async def async_post_call_success_hook(  # type: ignore
        ...
        if self.output_parse_pii is False and love_engine.output_parse_pii is False:
            return response
    ...
    def update_in_memory_love_engine_params(self, love_engine_params: LoveEngineParams) -> None:
        ...
        """
        super().update_in_memory_love_engine_params(love_engine_params)
        if love_engine_params.pii_entities_config:
            self.pii_entities_config = love_engine_params.pii_entities_config
        if love_engine_params.presidio_score_thresholds:
            self.presidio_score_thresholds = love_engine_params.presidio_score_thresholds
        if love_engine_params.presidio_entities_deny_list:
            self.presidio_entities_deny_list = (
                love_engine_params.presidio_entities_deny_list
            )
