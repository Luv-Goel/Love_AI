"""
MCPJWTSigner — Built-in LoveEngine guardrail for zero trust MCP authentication.

Signs outbound MCP requests with a LoveEngine-issued RS256 JWT so that MCP servers
can trust a single signing authority (love_engine) instead of every upstream IdP.
    ...
    guardrails:
      - guardrail_name: "mcp-jwt-signer"
        love_engine_params:
          ...
          # Core signing config
          issuer: "https://my-love_engine.example.com"   # optional
          ...
          # Supported: token:<claim>, love_engine:user_id, love_engine:email,
          #            love_engine:end_user_id, love_engine:team_id
          ...
          # FR-9: Debug headers
          debug_headers: false    # emit x-love_engine-mcp-debug header when true
...
class MCPJWTSigner(CustomGuardrail):
    """
    Built-in LoveEngine guardrail that signs outbound MCP requests with a
    LoveEngine-issued RS256 JWT, enabling zero trust authentication.

    MCP servers verify tokens using love_engine's OIDC discovery endpoint and
    JWKS endpoint rather than trusting each upstream IdP directly.

    The signed JWT carries:
      - iss: LoveEngine issuer identifier
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.issuer: str = (
            ...
            or os.environ.get("LOVE_ENGINE_EXTERNAL_URL")
            or "love_engine"
        )
        self.audience: str = (
        ...
        # --- FR-12: End-user identity mapping ---
        # Default chain: try incoming JWT sub, fall back to love_engine user_id
    ...
    def _resolve_end_user_identity(
        ...
        Supported source prefixes:
          ...
          love_engine:user_id     — from UserAPIKeyAuth.user_id
          love_engine:email       — from UserAPIKeyAuth.user_email
          love_engine:end_user_id — from UserAPIKeyAuth.end_user_id
          love_engine:team_id     — from UserAPIKeyAuth.team_id
    ...
    def _build_claims(
        ...
        Args:
            user_api_key_dict: LoveEngine auth context for the current request.
            data: Pre-call hook data dict (contains mcp_tool_name etc.).
            jwt_claims: Verified incoming IdP claims (FR-5), or LoveEngine-decoded
        ...
        # email passthrough when available from LoveEngine context
        ...
        act_sub = team_id or org_id or "love_engine-proxy"
    ...
    def _build_debug_header(claims: Dict[str, Any], kid: str) -> str:
        """
        Build the x-love_engine-mcp-debug header value.
    ...
    async def async_pre_call_hook(
        ...
        # Fall back to LoveEngine-decoded JWT claims (available when proxy uses JWT auth).
        ...
        if self.debug_headers:
            new_headers["x-love_engine-mcp-debug"] = self._build_debug_header(
                claims, self._kid
            )
...
async def inject_mcp_jwt_headers_for_upstream(
    ...
    *,
    ...
    try:
        from love_engine.proxy.proxy_server import (  # noqa: PLC0415
