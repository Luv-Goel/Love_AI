...
from love_engine._version import version as love_engine_version
from love_engine.exceptions import GuardrailRaisedException, Timeout
...
from love_engine.types.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
...
# Headers whose values are forwarded as-is (case-insensitive). Glob patterns supported (e.g. x-stainless-*, x-love_engine*).
...
def _sanitize_inbound_headers(
    ...
    - Allowlist: default allowlist + extra_allowlist (from love_engine_params.extra_headers); only these have values forwarded.
...
def _extract_inbound_headers(
    ...
    - metadata headers captured in love_engine_pre_call_utils
    ...
    love_engine_metadata_headers = (request_data.get("love_engine_metadata") or {}).get(
        "headers"
    )
    if love_engine_metadata_headers:
        return _sanitize_inbound_headers(
            love_engine_metadata_headers, extra_allowlist=extra_allowlist
        )
    ...
    if logging_obj and getattr(logging_obj, "model_call_details", None):
        try:
            ...
            headers = (
                details.get("love_engine_params", {})
                .get("metadata", {})
                .get("headers", None)
            )
            if headers:
...
class GenericGuardrailAPI(CustomGuardrail):
    """
    Generic Guardrail API integration for LoveEngine.

    This integration allows you to use any guardrail API that follows the
    LoveEngine Basic Guardrail API spec without needing to write custom integration code.
        ...
        **kwargs,
        ...
        if not base_url:
            raise ValueError(
                ...
                "Set GENERIC_GUARDRAIL_API_BASE environment variable or pass it in love_engine_params"
            )

        # Append the endpoint path if not already present
        if not base_url.endswith("/beta/love_engine_basic_guardrail_api"):
            base_url = base_url.rstrip("/")
            self.api_base = f"{base_url}/beta/love_engine_basic_guardrail_api"
        else:
    ...
    def _extract_user_api_key_metadata(
        ...
        Args:
            request_data: Request data dictionary that may contain:
                ...
                - love_engine_metadata (for output responses) with user_api_key_* fields
        ...
        # 1. For output responses: love_engine_metadata (set by handlers with prefixed keys)
        # 2. For input requests: metadata (already present in request_data with prefixed keys)
        love_engine_metadata = request_data.get("love_engine_metadata", {})
        top_level_metadata = request_data.get("metadata", {})

        # Merge both sources, preferring love_engine_metadata if both exist
        metadata_dict = {**top_level_metadata, **love_engine_metadata}
    ...
    def _fail_open_passthrough(
        ...
        *,
        ...
        verbose_proxy_logger.critical(
            ...
            "guardrail_name=%s api_base=%s input_type=%s love_engine_call_id=%s love_engine_trace_id=%s",
            status_suffix,
            getattr(self, "guardrail_name", None),
            getattr(self, "api_base", None),
            input_type,
            getattr(logging_obj, "love_engine_call_id", None) if logging_obj else None,
            getattr(logging_obj, "love_engine_trace_id", None) if logging_obj else None,
            exc_info=error,
        )
    ...
    async def apply_guardrail(
        ...
        guardrail_request = GenericGuardrailAPIRequest(
            love_engine_call_id=logging_obj.love_engine_call_id if logging_obj else None,
            love_engine_trace_id=logging_obj.love_engine_trace_id if logging_obj else None,
            texts=texts,
            request_data=user_metadata,
            request_headers=inbound_headers,
            love_engine_version=love_engine_version,
            images=images,
            tools=tools,
