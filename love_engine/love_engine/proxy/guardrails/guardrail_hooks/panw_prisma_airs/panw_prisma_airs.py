#!/usr/bin/env python3
"""
Palo Alto Networks Prisma AI Runtime Security (AIRS) Guardrail Integration for LoveEngine
...
from love_engine.caching import DualCache
...
if TYPE_CHECKING:
    ...
    from love_engine.types.proxy.guardrails.guardrail_hooks.base import GuardrailConfigModel


class PanwPrismaAirsHandler(CustomGuardrail):
    """
    LoveEngine Built-in Guardrail for Palo Alto Networks Prisma AI Runtime Security (AIRS).
    ...
    Configuration:
        ...
        app_name: Application name for tracking in Prisma AIRS analytics (default: "LoveEngine")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Handle app_name: Default to "LoveEngine", or prefix user's app_name with "LoveEngine-"
        if app_name:
            self.app_name = f"LoveEngine-{app_name}"
        else:
            self.app_name = "LoveEngine"
    ...
    async def _call_panw_api(
        ...
        # Handle app_name: LoveEngine by default, or LoveEngine-{user_app_name} if user provides one
        user_app_name = metadata.get("app_name") if metadata else None
        if user_app_name:
            app_name_value = f"LoveEngine-{user_app_name}"
        else:
            app_name_value = self.app_name  # Defaults to "LoveEngine"

        panw_metadata = {
            "app_user": (
                (metadata.get("app_user") or metadata.get("user") or "love_engine_user")
                if metadata
                else "love_engine_user"
            ),
            "ai_model": metadata.get("model", "unknown") if metadata else "unknown",
            "app_name": app_name_value,
            "source": "love_engine_builtin_guardrail",
        }

        # Include user_ip if available (from LoveEngine metadata or request)
        ...
        # Forward love_engine_trace_id in AIRS metadata for session correlation
        if metadata and metadata.get("love_engine_trace_id"):
            panw_metadata["love_engine_trace_id"] = metadata["love_engine_trace_id"]
        ...
        # Use per-request love_engine_call_id as AIRS tr_id; keep love_engine_trace_id in metadata.
        if call_id:
        ...
        try:
            # Use LoveEngine's async HTTP client
    ...
    def _prepare_metadata_from_request(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - app_name: Application identifier (will be prefixed with "LoveEngine-")
        ...
        metadata = {
            "user": data.get("user") or "love_engine_user",
        ...
        # Include love_engine_trace_id for session tracking.
        # Sources (checked in priority order):
        #   1. data["love_engine_trace_id"]             — top-level body field
        #   2. metadata["love_engine_trace_id"]         — user passes in request metadata
        #   3. metadata["trace_id"]                 — x-love_engine-trace-id header
        #      (love_engine_pre_call_utils stores it as "trace_id", not "love_engine_trace_id")
        #   4. requester_metadata["love_engine_trace_id"] — deep copy for /v1/messages routes
        trace_id = (
            data.get("love_engine_trace_id")
            or user_metadata.get("love_engine_trace_id")
            or user_metadata.get("trace_id")
            or requester_meta.get("love_engine_trace_id")
        )
        if trace_id:
            metadata["love_engine_trace_id"] = trace_id
    ...
    async def _scan_raw_streaming_text(
        ...
        scan_result = await self._call_panw_api(
            ...
            is_response=True,
            metadata=metadata,
            call_id=request_data.get("love_engine_call_id"),
        )
    ...
    def _check_and_mark_scanned(self, data: dict, scan_type: str) -> bool:
        ...
        """
        call_id = data.get("love_engine_call_id")
        if not call_id:
            call_id = str(uuid.uuid4())
            data["love_engine_call_id"] = call_id
            verbose_proxy_logger.warning(
                "PANW Prisma AIRS: love_engine_call_id missing from request data, "
        ...
        love_engine_metadata = data.setdefault("love_engine_metadata", {})

        if love_engine_metadata.get(scan_key):
            verbose_proxy_logger.debug(
                f"PANW Prisma AIRS: Skipping duplicate {scan_type}-call scan"
            )
            return True  # Already scanned

        love_engine_metadata[scan_key] = True
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        """
        from love_engine.llms.base_llm.base_model_iterator import MockResponseIterator
    ...
    async def apply_guardrail(
        ...
        # Resolve love_engine_call_id: request_data first, then logging_obj fallback.
        # Post-call path reconstructs request_data as {"response": ...} without
        # love_engine_call_id, but logging_obj.love_engine_call_id is available.
        call_id = request_data.get("love_engine_call_id")
        if not call_id and logging_obj:
            call_id = getattr(logging_obj, "love_engine_call_id", None)
        if not call_id:
            ...
            if input_type == "request" and logging_obj is None and _mcp_tool:
                ...
                request_data["love_engine_call_id"] = call_id
                verbose_proxy_logger.debug(
                ...
                request_data["love_engine_call_id"] = call_id
                verbose_proxy_logger.warning(
                    "PANW Prisma AIRS: love_engine_call_id missing from empty "
                ...
                request_data["love_engine_call_id"] = call_id
                verbose_proxy_logger.warning(
                    "PANW Prisma AIRS: love_engine_call_id missing, synthesized %s "
                    "(input_type=%s)",
                    call_id,
                    input_type,
                )
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.panw_prisma_airs import (
