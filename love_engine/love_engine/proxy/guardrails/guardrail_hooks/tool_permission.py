...
from love_engine import ChatCompletionToolParam
from love_engine._logging import verbose_proxy_logger
from love_engine.caching.dual_cache import DualCache
from love_engine.exceptions import GuardrailRaisedException
...
from love_engine.types.guardrails import GuardrailEventHooks, LoveEngineParams
...
class ToolPermissionGuardrail(CustomGuardrail):
    ...
    def _load_rules(self, rules: Optional[List[Any]]) -> None:
        ...
        both ``__init__`` and ``update_in_memory_love_engine_params`` recompile from a
    ...
    def update_in_memory_love_engine_params(
        self, love_engine_params: Union[LoveEngineParams, dict]
    ) -> None:
        ...
        """
        # ``love_engine_params`` may arrive as the raw DB dict (the proxy ``cast()``s
        # it to ``LoveEngineParams`` without converting), so handle both shapes. The
