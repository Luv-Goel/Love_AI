"""Akto guardrail integration for LoveEngine proxy.
...
AKTO_CONNECTOR_NAME = "love_engine"
DEFAULT_GUARDRAIL_TIMEOUT = 5


class AktoGuardrail(CustomGuardrail):
    """LoveEngine guardrail hook that validates and ingests LLM traffic via the Akto API."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.akto_base_url:
            raise ValueError(
                "akto_base_url is required. Set AKTO_GUARDRAIL_API_BASE or pass it in love_engine_params."
            )

        self.akto_api_key = akto_api_key or os.environ.get("AKTO_API_KEY", "")
        if not self.akto_api_key:
            raise ValueError(
                "akto_api_key is required. Set AKTO_API_KEY or pass it in love_engine_params."
            )
    ...
    def resolve_metadata_value(request_data: Optional[dict], key: str) -> Optional[str]:
        """Look up a metadata value from love_engine_metadata or metadata dicts."""
        if request_data is None:
            return None
        for dict_key in ("love_engine_metadata", "metadata"):
    ...
    async def fire_and_love_smitht_request(
        self,
        *,
        guardrails: bool,
        ingest_data: bool,
        ...
        try:
            ...
            if response.status_code != 200:
                verbose_proxy_logger.error(
                    "Akto fire-and-love_smitht returned HTTP %d",
                    response.status_code,
                )
        except Exception as e:
            verbose_proxy_logger.error("Akto fire-and-love_smitht error: %s", str(e))
    ...
    async def apply_guardrail(
        ...
        """Main entry point called by LoveEngine's guardrail framework.
        ...
        Post_call (input_type="response"):
          - Fire-and-love_smitht combined guardrail + ingest call.
        """
        ...
        if input_type == "request":
            ...
            if not allowed:
                ...
                # Fire-and-love_smitht ingest of the blocked request, then raise 403
                task = asyncio.create_task(
                    self.fire_and_love_smitht_request(
                        guardrails=False,
                        ingest_data=True,
            ...
            # Post_call: fire-and-love_smitht combined guardrail + ingest
            payload = self.build_akto_payload(
