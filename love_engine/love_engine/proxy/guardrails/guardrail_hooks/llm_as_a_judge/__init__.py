...
from love_engine.types.utils import GenericGuardrailAPIInputs, GuardrailStatus
...
if TYPE_CHECKING:
    ...
    from love_engine.types.utils import StandardLoggingEvalInformation
...
def _get_love_engine_param(
    love_engine_params: "LoveEngineParams",
    guardrail: "Guardrail",
    key: str,
    default: Any = None,
) -> Any:
    val = getattr(love_engine_params, key, None)
    if val is not None:
        return val
...
def initialize_guardrail(
    ...
    judge_model = _get_love_engine_param(love_engine_params, guardrail, "judge_model")
    if not judge_model:
        raise ValueError(
            "llm_as_a_judge guardrail requires judge_model in love_engine_params"
        )

    criteria = _get_love_engine_param(love_engine_params, guardrail, "criteria") or []
    if not criteria:
    ...
    on_failure = _get_love_engine_param(love_engine_params, guardrail, "on_failure", "block")
    ...
    overall_threshold = float(
        _get_love_engine_param(love_engine_params, guardrail, "overall_threshold", 80.0)
    )

    mode = _get_love_engine_param(love_engine_params, guardrail, "mode")
    ...
    instance = LLMAsAJudgeGuardrail(
        ...
        default_on=bool(
            _get_love_engine_param(love_engine_params, guardrail, "default_on", False)
        ),
    )
