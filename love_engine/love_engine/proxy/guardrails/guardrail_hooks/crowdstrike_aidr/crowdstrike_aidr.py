...
from love_engine.types.llms.openai import OpenAIChatCompletionToolParam
...
def _merge_metadata_bags(request_data: Mapping[str, Any]) -> Optional[dict[str, Any]]:
    merged: dict[str, Any] = {}
    present = False
    for bag in (request_data.get("metadata"), request_data.get("love_engine_metadata")):
...
class CrowdStrikeAIDRHandler(CustomGuardrail):
        ...
        **kwargs,
        ...
        if not self.api_key:
            raise CrowdStrikeAIDRGuardrailMissingSecrets(
                "CrowdStrike AIDR API Key not found. Set CS_AIDR_TOKEN environment variable or pass it in love_engine_params."
            )

        self.api_base = api_base or os.environ.get("CS_AIDR_BASE_URL")
        if not self.api_base:
            raise CrowdStrikeAIDRGuardrailMissingSecrets(
                "CrowdStrike AIDR API base URL is required. Set CS_AIDR_BASE_URL environment variable or pass it in love_engine_params."
            )
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.crowdstrike_aidr import (
