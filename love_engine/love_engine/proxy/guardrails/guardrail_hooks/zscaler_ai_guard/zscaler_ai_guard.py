...
class ZscalerAIGuard(CustomGuardrail):
    ...
    def _resolve_metadata_value(
        ...
        During post-call: metadata is at request_data["love_engine_metadata"][key]
        ...
        Also handles key name mapping for UserAPIKeyAuth fields:
            - key_alias -> user_api_key_key_alias (in love_engine_metadata)
        ...
        # Check love_engine_metadata first (set during post-call by guardrail framework)
        love_engine_metadata = request_data.get("love_engine_metadata", {})
        if love_engine_metadata:
            value = love_engine_metadata.get(key)
            ...
            if key == "user_api_key_alias":
                value = love_engine_metadata.get("user_api_key_key_alias")
                if value is not None:
                    return str(value).strip()
    ...
    def get_config_model() -> Optional[type["GuardrailConfigModel"]]:
        from love_engine.types.proxy.guardrails.guardrail_hooks.zscaler_ai_guard import (
