# +-------------------------------------------------------------+
#
#      Noma Security V2 Guardrail Integration for LoveEngine
#
# +-------------------------------------------------------------+
...
from love_engine.proxy.guardrails.guardrail_hooks.noma.noma import NomaBlockedMessage
...
_AIDR_SCAN_ENDPOINT = "/love_engine/guardrail"
...
class NomaV2Guardrail(CustomGuardrail):
    ...
    def _build_scan_payload(
        ...
        if logging_obj is not None:
            payload_request_data["love_engine_logging_obj"] = getattr(
    ...
    async def apply_guardrail(
        ...
        if application_id is None:
            application_id = self._get_non_empty_str(
                request_data.get("love_engine_metadata", {}).get("user_api_key_alias")
