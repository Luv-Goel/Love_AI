...
from love_engine.proxy.proxy_server import LOVE_ENGINE_TeamTable, UserAPIKeyAuth
...
def can_modify_guardrails(team_obj: Optional[LOVE_ENGINE_TeamTable]) -> bool:
    if team_obj is None:
        return True
        ...
        if "guardrails" in data["metadata"]:
            ...
            if isinstance(request_guardrails, dict):
                ...
                for _guardrail_name, should_run in request_guardrails.items():
                    ...
                    guardrail_item: GuardrailItem = love_engine.guardrail_name_config_map[
                        _guardrail_name
                    ]
