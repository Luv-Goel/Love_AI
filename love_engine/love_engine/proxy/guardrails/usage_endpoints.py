...
def _guardrail_overview_rows(
    ...
    for g in guardrails:
        ...
        love_engine_params = (
            (g.love_engine_params or {}) if isinstance(g.love_engine_params, dict) else {}
        )
        provider = str(love_engine_params.get("guardrail", "Unknown"))
        guardrail_info = (
...
async def guardrails_usage_detail(
    ...
    _love_engine_params = getattr(guardrail, "love_engine_params", None) or (
        guardrail.get("love_engine_params") if isinstance(guardrail, dict) else None
    )
    love_engine_params = _love_engine_params if isinstance(_love_engine_params, dict) else {}
