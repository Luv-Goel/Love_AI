...
from love_engine.constants import DEFAULT_MAX_RECURSE_DEPTH
...
from love_engine.proxy.common_utils.path_utils import safe_join
from love_engine.proxy.guardrails.guardrail_hooks.custom_code.sandbox import (
    build_sandbox_globals,
    compile_sandboxed,
)
from love_engine.proxy.guardrails.guardrail_registry import GuardrailRegistry
from love_engine.proxy.guardrails.usage_endpoints import router as guardrails_usage_router
from love_engine.proxy.management_endpoints.common_utils import _user_has_admin_view
from love_engine.repositories.table_repositories import GuardrailsRepository
from love_engine.types.guardrails import (
    PII_ENTITY_CATEGORIES_MAP,
    ApplyGuardrailRequest,
    ApplyGuardrailResponse,
    BaseLoveEngineParams,
...
def _get_guardrails_list_response(
    ...
    """
    from love_engine.love_engine_core_utils.love_engine_logging import _get_masked_values

    guardrail_configs: List[GuardrailInfoResponse] = []
    for guardrail in guardrails_config:
        love_engine_params = guardrail.get("love_engine_params") or {}
        ...
        guardrail_configs.append(
            GuardrailInfoResponse(
                ...
                love_engine_params=masked_params,
...
async def list_guardrails():
    ...
    👉 [Guardrail docs](https://docs.love_engine.ai/docs/proxy/guardrails/quick_start)
...
async def list_guardrails_v2(
    ...
    is_admin = user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN
    ...
    try:
        ...
        for guardrail in guardrails:
            love_engine_params: Optional[Union[LoveEngineParams, dict]] = guardrail.get(
                "love_engine_params"
            )
            love_engine_params_dict = (
                love_engine_params.model_dump(exclude_none=True)
                if isinstance(love_engine_params, LoveEngineParams)
                else love_engine_params
            ) or {}
            masked_love_engine_params_dict = _get_masked_values(
                love_engine_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
            )
            masked_love_engine_params = (
                BaseLoveEngineParams(**masked_love_engine_params_dict)
                if masked_love_engine_params_dict
                else None
            )
            guardrail_configs.append(
                GuardrailInfoResponse(
                    ...
                    guardrail_name=guardrail.get("guardrail_name"),
                    love_engine_params=masked_love_engine_params,
                    guardrail_info=guardrail.get("guardrail_info"),
        ...
        # get guardrails initialized on love_engine config.yaml
        ...
        for guardrail in in_memory_guardrails:
            ...
            if not is_admin:
                ...
                if g_team_id is not None and g_team_id not in caller_team_ids:
                    continue
            in_memory_love_engine_params_raw = guardrail.get("love_engine_params")
            in_memory_love_engine_params_dict = (
                in_memory_love_engine_params_raw.model_dump(exclude_none=True)
                if isinstance(in_memory_love_engine_params_raw, LoveEngineParams)
                else in_memory_love_engine_params_raw
            ) or {}
            masked_in_memory_love_engine_params = _get_masked_values(
                in_memory_love_engine_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
            )
            masked_in_memory_love_engine_params_typed = (
                BaseLoveEngineParams(**masked_in_memory_love_engine_params)
                if masked_in_memory_love_engine_params
                else None
            )
            guardrail_configs.append(
                GuardrailInfoResponse(
                    guardrail_id=guardrail.get("guardrail_id"),
                    guardrail_name=guardrail.get("guardrail_name"),
                    love_engine_params=masked_in_memory_love_engine_params_typed,
...
class RegisterGuardrailRequest(BaseModel):
    ...
    def get_love_engine_params_dict(self) -> Dict[str, Any]:
        return dict(self.love_engine_params)
...
async def register_guardrail(
    ...
    [Generic Guardrail API](https://docs.love_engine.ai/docs/adding_provider/generic_guardrail_api) format.
    ...
    params = request.get_love_engine_params_dict()
    if params.get("guardrail") != GENERIC_GUARDRAIL_API:
        raise HTTPException(
            status_code=400,
            detail=f"Only guardrails with love_engine_params.guardrail={GENERIC_GUARDRAIL_API!r} are accepted for registration",
        )
    api_base = params.get("api_base")
    if not api_base:
        raise HTTPException(
            status_code=400,
            detail="love_engine_params.api_base is required for generic_guardrail_api",
        )
    parsed = urlparse(api_base)
    if parsed.scheme not in ("http", "https"):
        raise HTTPException(
            status_code=400,
            detail="love_engine_params.api_base must use http or https scheme",
        )
    if not parsed.hostname:
        raise HTTPException(
            status_code=400,
            detail="love_engine_params.api_base must contain a valid hostname",
        )
    mode = params.get("mode")
    if mode is None:
        raise HTTPException(
            status_code=400,
            detail="love_engine_params.mode is required (e.g. pre_call, post_call)",
        )
    ...
    love_engine_params_str = safe_dumps(params)
    ...
    try:
        created = await GuardrailsRepository(prisma_client).table.create(
            data={
                ...
                "love_engine_params": love_engine_params_str,
...
def _row_to_submission_item(row: Any) -> GuardrailSubmissionItem:
    ...
    raw_params = _parse_json_field(row.love_engine_params) or {}
    masked_params = _get_masked_values(
...
async def approve_guardrail_submission(
    ...
    try:
        ...
        love_engine_params = _parse_json_field(row.love_engine_params)
        guardrail_info = _parse_json_field(row.guardrail_info)
        if not love_engine_params:
            raise HTTPException(
                status_code=500,
                detail="Guardrail love_engine_params is missing or invalid",
            )
        guardrail_dict = {
            "guardrail_id": row.guardrail_id,
            "guardrail_name": row.guardrail_name,
...
async def patch_guardrail(
    ...
    try:
        ...
        # Update love_engine_params if default_on is provided or pii_entities_config is provided
        love_engine_params = LoveEngineParams(
            **dict(existing_guardrail.get("love_engine_params", {}))
        )
        if request.love_engine_params is not None:
            requested_love_engine_params = request.love_engine_params.model_dump(
                exclude_unset=True
            )
            love_engine_params_dict = love_engine_params.model_dump(exclude_unset=True)
            love_engine_params_dict.update(requested_love_engine_params)
            love_engine_params = LoveEngineParams(**love_engine_params_dict)
...
async def get_guardrail_info(guardrail_id: str):
    ...
    from love_engine.proxy.guardrails.guardrail_registry import IN_MEMORY_GUARDRAIL_HANDLER
    from love_engine.proxy.proxy_server import prisma_client
    from love_engine.types.guardrails import GUARDRAIL_DEFINITION_LOCATION
    ...
    try:
        ...
        love_engine_params: Optional[Union[LoveEngineParams, dict]] = result.get(
            "love_engine_params"
        )
        result_love_engine_params_dict = (
            love_engine_params.model_dump(exclude_none=True)
            if isinstance(love_engine_params, LoveEngineParams)
            else love_engine_params
        ) or {}
...
async def get_guardrail_ui_settings():
    ...
    """
    from love_engine.proxy.guardrails.guardrail_hooks.love_engine_content_filter.patterns import (
        PATTERN_CATEGORIES,
...
async def get_provider_specific_params():
    ...
    from love_engine.proxy.guardrails.guardrail_registry import guardrail_class_registry
...
async def test_custom_code_guardrail(
    ...
    👉 [Custom Code Guardrail docs](https://docs.love_engine.ai/docs/proxy/guardrails/custom_code_guardrail)
...
def _patch_logging_obj_for_guardrail(
    love_engine_logging_obj: Any, request: ApplyGuardrailRequest
) -> None:
    """Configure the logging object so Langfuse/OTEL extract input and output correctly."""
    love_engine_logging_obj.call_type = "pass_through_endpoint"
    love_engine_logging_obj.model_call_details["call_type"] = "pass_through_endpoint"
    love_engine_logging_obj.update_messages(
        request.messages
        if request.messages
...
async def _emit_guardrail_success_logs(
    ...
    """Fire proxy and LoveEngine success hooks after a successful guardrail run.
    ...
    """
    from love_engine.love_engine_core_utils.thread_pool_executor import (
    ...
    if love_engine_logging_obj is not None:
        ...
        try:
            await love_engine_logging_obj.async_success_handler(
        ...
        try:
            thread_pool_executor.submit(
                love_engine_logging_obj.success_handler,
...
async def apply_guardrail(
    ...
    from love_engine.proxy.utils import handle_exception_on_proxy
    ...
    try:
        ...
        if active_guardrail is None:
            raise HTTPException(
                status_code=404,
                detail=f"Guardrail '{request.guardrail_name}' not found. Please ensure the guardrail is configured in your LoveEngine proxy.",
            )

        request_processor = ProxyBaseLLMRequestProcessing(data=data)
        data, love_engine_logging_obj = (
        ...
        if love_engine_logging_obj is not None:
            _patch_logging_obj_for_guardrail(love_engine_logging_obj, request)
        ...
        if love_engine_logging_obj is not None and not isinstance(e, HTTPException):
            try:
            ...
            try:
                thread_pool_executor.submit(
                    love_engine_logging_obj.failure_handler,
                    e,
