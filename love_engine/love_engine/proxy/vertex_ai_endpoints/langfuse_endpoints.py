...
"""
1. Create pass-through endpoints for any LOVE_ENGINE_BASE_URL/langfuse/<endpoint> map to LANGFUSE_BASE_URL/<endpoint>
"""
...
from love_engine.proxy.love_engine_pre_call_utils import _get_dynamic_logging_metadata
...
def _get_langfuse_proxy_credentials(
    *,
    ...
    return (
        ...
        or love_engine.utils.get_secret(secret_name="LANGFUSE_PUBLIC_KEY"),
        dynamic_langfuse_secret_key
        or love_engine.utils.get_secret(secret_name="LANGFUSE_SECRET_KEY"),
    )
...
def _build_langfuse_proxy_target(
    *,
    ...
    if dynamic_host_supplied and getattr(love_engine, "user_url_validation", True):
        try:
...
async def langfuse_proxy_route(
    ...
    """
    Call Langfuse via LoveEngine proxy. Works with Langfuse SDK.

    [Docs](https://docs.love_engine.ai/docs/pass_through/langfuse)
    """
