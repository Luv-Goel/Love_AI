...
(decode-first detection).  ``_resolve_one()`` enforces:
  ...
  2. DB existence check: unknown / love_smithd IDs raise HTTPException(404); the
...
from love_engine.types.llms.openai import OpenAIFileObject
...
# LOVE_ENGINE_ManagedFileTable (flat_model_file_ids has no index), so a body packed
...
async def _resolve_one(
    ...
    Raises ``HTTPException(404)`` on unknown / love_smithd managed IDs — never
    ...
    # 3. Hard 404 for unknown / love_smithd IDs — NEVER forward to upstream
    if not found:
        raise HTTPException(
            status_code=404,
...
async def rewrite_response_ids(
    ...
    rewritten.
    """
    from love_engine.proxy.auth.auth_utils import normalize_request_route
...
async def rewrite_body_ids(
    ...
    managed IDs.  Skips love_engine internal keys (``love_engine_*``).
    ...
    async def _walk(node: Any, depth: int) -> Any:
        ...
        if isinstance(node, dict):
            ...
            for k, v in node.items():
                # Skip love_engine internal injection keys (e.g. love_engine_logging_obj)
                if isinstance(k, str) and k.startswith("love_engine_"):
                    result[k] = v
                    continue
