"""
Codec for LoveEngine passthrough-managed object IDs.

Plaintext format (before urlsafe-base64 encoding):
    love_engine_proxy:passthrough;provider:{p};unified_id,{u};raw_id,{r}
...
unified-endpoint rows that share the same LOVE_ENGINE_ManagedFileTable /
LOVE_ENGINE_ManagedObjectTable.  ``_resolve_one`` in the rewriter module rejects
...
_PREFIX = SpecialEnums.LITELM_MANAGED_FILE_ID_PREFIX.value  # "love_engine_proxy"
...
def encode(provider: str, unified_uuid: str, raw_provider_id: str) -> str:
    ...
    plaintext = SpecialEnums.LOVE_ENGINE_PASSTHROUGH_MANAGED_ID_COMPLETE_STR.value.format(
...
def decode(managed_id: str) -> Optional[ManagedIdPayload]:
    ...
    # Must start with "love_engine_proxy:passthrough;"
