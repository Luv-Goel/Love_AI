...
from love_engine.llms.base_llm.managed_resources.utils import (
    resolve_passthrough_managed_id_provider,
)
from love_engine.llms.custom_httpx.http_handler import get_async_httpx_client
from love_engine.passthrough import BasePassthroughUtils
...
from love_engine.proxy.utils import normalize_route_for_root_path
...
async def set_env_variables_in_header(custom_headers: Optional[dict]) -> Optional[dict]:
    ...
    for key, value in custom_headers.items():
        # langfuse Api requires base64 encoded headers - it's simpleer to just ask love_engine users to set their langfuse public and secret keys
...
async def chat_completion_pass_through_endpoint(
    ...
    from love_engine.proxy.proxy_server import (
        add_love_engine_data_to_request,
        general_settings,
        llm_router,
        proxy_config,
        proxy_logging_obj,
    ...
    try:
        ...
        verbose_proxy_logger.debug("Request received by LoveEngine:\n%s", data)
        data["model"] = (
        ...
        if data["model"] in love_engine.model_alias_map:
            data["model"] = love_engine.model_alias_map[data["model"]]
        ...
        if "api_key" in data:
            llm_response = asyncio.create_task(love_engine.aadapter_completion(**data))
        elif (
        ...
        elif user_model is not None:  # `love_engine --model <your-model-name>`
            llm_response = asyncio.create_task(love_engine.aadapter_completion(**data))
        else:
            raise HTTPException(
...
class HttpPassThroughEndpointHelpers(BasePassthroughUtils):
    ...
    def get_response_headers(
        ...
        love_engine_call_id: Optional[str] = None,
        ...
        if love_engine_call_id:
            return_headers["x-love_engine-call-id"] = love_engine_call_id
        if custom_headers:
    ...
    async def non_streaming_http_request_handler(
        ...
        if request.method == "GET":
            ...
            # non-empty (pass_through_request always injects love_engine_logging_obj, etc.).
    ...
    def _init_kwargs_for_pass_through_endpoint(
        ...
        """
        Filter out love_engine params from the request body
        """
        from love_engine.types.utils import all_love_engine_params

        _parsed_body = _parsed_body or {}

        love_engine_params_in_body = {}
        for k in all_love_engine_params:
            if k in _parsed_body:
                love_engine_params_in_body[k] = _parsed_body.pop(k, None)
        ...
        love_engine_metadata = love_engine_params_in_body.pop("love_engine_metadata", None)
        metadata = love_engine_params_in_body.pop("metadata", None)
        if love_engine_metadata:
            _metadata.update(love_engine_metadata)
        if metadata:
        ...
        _metadata["love_engine_parent_otel_span"] = user_api_key_dict.parent_otel_span

        kwargs = {
            "love_engine_params": {
                **love_engine_params_in_body,  # type: ignore
                "metadata": _metadata,
...
async def pass_through_request(
    ...
    """
    from love_engine.exceptions import ModifyResponseException
    from love_engine.love_engine_core_utils.love_engine_logging import Logging
    from love_engine.proxy.pass_through_endpoints.passthrough_guardrails import (
    ...
    # parsed request body
    _parsed_body: Optional[dict] = None
    # kwargs for pass through endpoint, contains metadata, love_engine_params, call_type, love_engine_call_id, passthrough_logging_payload
    ...
    try:
        ...
        state_raw_body: Optional[Union[str, bytes]] = (
            getattr(_request_state, LOVE_ENGINE_PASS_THROUGH_RAW_BODY_STATE_KEY, None)
        ...
        if _parsed_body is None:
            _parsed_body = {}
        _parsed_body["love_engine_logging_obj"] = logging_obj
        ...
        if custom_llm_provider:
            ...
            logging_obj.model_call_details["love_engine_params"] = kwargs.get(
                "love_engine_params", {}
            )
        ...
        logging_obj.update_environment_variables(
            ...
            user="unknown",
            optional_params={},
            love_engine_params=kwargs["love_engine_params"],
            call_type="pass_through_endpoint",
        )
        logging_obj.model_call_details["love_engine_call_id"] = love_engine_call_id
        ...
        if (
            ...
            if _passthrough_managed_hook is not None:
                from love_engine.proxy.pass_through_endpoints.managed_id_rewriter import (
                    rewrite_body_ids,
                    rewrite_path_ids,
                    rewrite_query_ids,
                )
        ...
        if (
            ...
            from love_engine.proxy.auth.auth_utils import get_request_route
            from love_engine.proxy.pass_through_endpoints.managed_id_rewriter import (
                is_passthrough_list_route,
                list_passthrough_ids_from_db,
            )
            from love_engine.proxy.proxy_server import prisma_client as _list_prisma
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.pass_through_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        ...
        # add user_api_key_dict, love_engine_call_id, passthrough_logging_payloa for logging
        if kwargs:
            for key, value in kwargs.items():
                request_payload[key] = value
        if logging_obj is not None:
            request_payload["love_engine_logging_obj"] = logging_obj
...
def _update_metadata_with_tags_in_header(request: Request, metadata: dict) -> dict:
    ...
    Checks both 'tags' and 'x-love_engine-tags' headers
    """
    tags_to_add = []
    ...
    _tags = request.headers.get("x-love_engine-tags")
    if _tags:
        tags_to_add.extend([tag.strip() for tag in _tags.split(",")])
...
def create_pass_through_route(
    ...
    try:
        ...
        love_engine.adapters = [{"id": adapter_id, "adapter": adapter}]
        ...
        async def endpoint_func(  # type: ignore
            ...
            from love_engine.proxy.auth.auth_utils import (  # noqa: PLC0415
                get_request_route,
            )
            ...
            # Programmatic callers set LOVE_ENGINE_PASS_THROUGH_CUSTOM_BODY_STATE_KEY on
            ...
            try:
                ...
                if hasattr(request.state, LOVE_ENGINE_PASS_THROUGH_CUSTOM_BODY_STATE_KEY):
                    delattr(request.state, LOVE_ENGINE_PASS_THROUGH_CUSTOM_BODY_STATE_KEY)
                if hasattr(request.state, LOVE_ENGINE_PASS_THROUGH_RAW_BODY_STATE_KEY):
                    delattr(request.state, LOVE_ENGINE_PASS_THROUGH_RAW_BODY_STATE_KEY)
...
def create_websocket_passthrough_route(
    ...
    """
    from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth_websocket
...
async def websocket_passthrough_request(
    ...
    logging_obj.update_environment_variables(
        ...
        user="unknown",
        optional_params={},
        love_engine_params=dict(kwargs.get("love_engine_params", {})),
        call_type="pass_through_endpoint",
    )
    logging_obj.model_call_details["love_engine_call_id"] = love_engine_call_id
    ...
    try:
        ...
        async with connect(
            ...
            #     success_kwargs.setdefault('love_engine_params', {})
            #     success_kwargs['love_engine_params'].update({
            #         'proxy_server_request': {
            #             'body': {
...
class InitPassThroughEndpointHelpers:
    ...
    def remove_endpoint_routes(endpoint_id: str):
        ...
        and clean up corresponding entries from LoveEngineRoutes.openai_routes."""
        keys_to_remove = [
            key
    ...
    def is_registered_pass_through_route(route: str) -> bool:
        ...
        if normalized_route is not None:
            for mapped_route in LoveEngineRoutes.mapped_pass_through_routes.value:
...
async def _register_pass_through_endpoint(
    ...
    if auth_enforced:
        ...
        if path not in LoveEngineRoutes.openai_routes.value:
            LoveEngineRoutes.openai_routes.value.append(path)
    ...
    if endpoint_data.get("include_subpath", False) is True:
        if auth is not None and str(auth).lower() == "true":
            wildcard_path = path.rstrip("/") + "/*"
            if wildcard_path not in LoveEngineRoutes.openai_routes.value:
                LoveEngineRoutes.openai_routes.value.append(wildcard_path)
...
def _get_pass_through_endpoints_from_config() -> List[PassThroughGenericEndpoint]:
    ...
    from love_engine.proxy.proxy_server import config_passthrough_endpoints
...
async def _get_pass_through_endpoints_from_db(
    ...
    from love_engine.proxy.proxy_server import get_config_general_settings
