...
from love_engine.love_engine_core_utils.love_engine_logging import use_custom_pricing_for_model
from love_engine.llms.anthropic import get_anthropic_config
...
from love_engine.proxy.auth.auth_utils import get_end_user_id_from_request_body
from love_engine.types.passthrough_endpoints.pass_through_endpoints import (
    PassthroughStandardLoggingPayload,
)
from love_engine.types.utils import LoveEngineBatch, ModelResponse, TextCompletionResponse

if TYPE_CHECKING:
    from love_engine.types.passthrough_endpoints.pass_through_endpoints import EndpointType
...
class AnthropicPassthroughLoggingHandler:
    ...
    def anthropic_passthrough_handler(
        ...
        **kwargs,
        ...
        love_engine_model_response: ModelResponse = anthropic_config().transform_response(
    ...
    @staticmethod
    def _resolve_costing_model(model: str, logging_obj: LoveEngineLoggingObj) -> str:
        if model and model != "unknown":
            return model
        love_engine_params = (getattr(logging_obj, "model_call_details", {}) or {}).get(
            "love_engine_params", {}
        ) or {}
        deployment_model = love_engine_params.get("model")
        if deployment_model and deployment_model != "unknown":
            return deployment_model
        model_group = (love_engine_params.get("metadata", {}) or {}).get("model_group")
        if model_group:
    ...
    def _create_anthropic_response_logging_payload(
        ...
        try:
            ...
            custom_pricing = use_custom_pricing_for_model(
                love_engine_params=(
                    logging_obj.love_engine_params
                    if hasattr(logging_obj, "love_engine_params")
                    else None
                )
            )
    ...
    def _handle_logging_anthropic_collected_chunks(
        ...
        """
        Takes raw chunks from Anthropic passthrough endpoint and logs them in love_engine callbacks
        ...
        if (
            not model
            and hasattr(love_engine_logging_obj, "model_call_details")
            and love_engine_logging_obj.model_call_details.get("model")
        ):
            model = cast(str, love_engine_logging_obj.model_call_details.get("model"))
    ...
    def batch_creation_handler(
        ...
        **kwargs,
        ...
        from love_engine.llms.anthropic.batches.transformation import AnthropicBatchesConfig
        from love_engine.types.utils import Choices, SpecialEnums
        ...
        try:
            ...
            if httpx_response.status_code == 200 and "id" in _json_response:
                # Transform Anthropic response to LoveEngine batch format
                anthropic_batches_config = AnthropicBatchesConfig()
                love_engine_batch_response = (
                ...
                love_engine_batch_response.status = "validating"
                ...
                love_engine_model_response.object = "batch"
                ...
                love_engine_model_response = ModelResponse()
                love_engine_model_response.id = str(uuid.uuid4())
                love_engine_model_response.model = "anthropic_batch"
                love_engine_model_response.object = "batch"
                love_engine_model_response.created = int(start_time.timestamp())
