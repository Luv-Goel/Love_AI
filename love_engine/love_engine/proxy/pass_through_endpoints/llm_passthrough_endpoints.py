...
Use love_engine with Anthropic SDK, Vertex AI SDK, Cohere SDK, etc.
"""
...
from love_engine.proxy.pass_through_endpoints.common_utils import get_love_engine_virtual_key
...
from love_engine.types.passthrough_endpoints.pass_through_endpoints import (
    LOVE_ENGINE_PASS_THROUGH_CUSTOM_BODY_STATE_KEY,
    LOVE_ENGINE_PASS_THROUGH_RAW_BODY_STATE_KEY,
)
from love_engine.proxy.utils import is_known_model
from love_engine.proxy.vector_store_endpoints.utils import (
    assert_user_can_access_vector_store,
    get_love_engine_managed_vector_store,
...
def is_passthrough_request_using_router_model(
    request_body: dict, llm_router: Optional[love_engine.Router]
) -> bool:
    """
...
async def gemini_proxy_route(
    ...
    """
    [Docs](https://docs.love_engine.ai/docs/pass_through/google_ai_studio)
    """
    ## CHECK FOR LOVE_ENGINE API KEY IN THE QUERY PARAMS - ?..key=LOVE_ENGINE_API_KEY
...
async def cohere_proxy_route(
    ...
    """
    [Docs](https://docs.love_engine.ai/docs/pass_through/cohere)
    """
...
async def vllm_proxy_route(
    ...
    """
    [Docs](https://docs.love_engine.ai/docs/pass_through/vllm)
    """
...
async def mistral_proxy_route(
    ...
    """
    [Docs](https://docs.love_engine.ai/docs/pass_through/mistral)
    """
...
async def milvus_proxy_route(
    ...
    if not love_engine.vector_store_index_registry or not love_engine.vector_store_registry:
        raise HTTPException(
            status_code=500,
    ...
    is_vector_store_index = love_engine.vector_store_index_registry.is_vector_store_index(
    ...
    if not is_vector_store_index:
        raise HTTPException(
            status_code=400,
            detail=f"Collection {collection_name} is not a love_engine managed vector store index. Only love_engine managed vector store indexes are supported.",
        )
    ...
    index_object = (
        (
            love_engine.vector_store_index_registry.get_vector_store_index_by_name(
                vector_store_index_name=collection_name
            )
        )
        if love_engine.vector_store_index_registry is not None
        else None
    )
    if index_object is None:
        raise Exception(f"Vector store index not found for {collection_name}")

    vector_store_name = index_object.love_engine_params.vector_store_name
    vector_store_index = index_object.love_engine_params.vector_store_index
    ...
    vector_store = love_engine.vector_store_registry.get_love_engine_managed_vector_store_from_registry_by_name(
    ...
    love_engine_params = vector_store.get("love_engine_params") or {}
    ...
    base_target_url = provider_config.get_complete_url(
        api_base=love_engine_params.get("api_base"), love_engine_params=love_engine_params
    )
...
async def anthropic_proxy_route(
    ...
    """
    [Docs](https://docs.love_engine.ai/docs/pass_through/anthropic_completion)
    """
    base_target_url = (
...
async def handle_bedrock_passthrough_router_model(
    ...
    from love_engine.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
...
async def handle_bedrock_count_tokens(
    ...
    from love_engine.llms.bedrock.count_tokens.handler import BedrockCountTokensHandler
    from love_engine.proxy.proxy_server import llm_router
    ...
    try:
        ...
        love_engine_params = {"user_api_key_dict": user_api_key_dict}
        ...
        if llm_router:
            ...
            if deployments and len(deployments) > 0:
                ...
                model_love_engine_params = deployment.get("love_engine_params", {})

                # Get the resolved model ID from the configuration
                if "model" in model_love_engine_params:
                    resolved_model = model_love_engine_params["model"]

                # Copy all love_engine_params - BaseAWSLLM will handle AWS credential discovery
                for key, value in model_love_engine_params.items():
                    if key != "user_api_key_dict":  # Don't overwrite user_api_key_dict
                        love_engine_params[key] = value  # type: ignore

        verbose_proxy_logger.debug(f"Count tokens love_engine_params: {love_engine_params}")
...
async def bedrock_proxy_route(
    ...
    [Docs](https://docs.love_engine.ai/docs/pass_through/bedrock)
    """
    ...
    aws_region_name = love_engine.utils.get_secret(secret_name="AWS_REGION_NAME")
    ...
    # Add or update query parameters
    from love_engine.llms.bedrock.chat import BedrockConverseLLM
    ...
    setattr(request.state, LOVE_ENGINE_PASS_THROUGH_CUSTOM_BODY_STATE_KEY, data)
    ...
    setattr(request.state, LOVE_ENGINE_PASS_THROUGH_RAW_BODY_STATE_KEY, prepped.body)
    received_value = await endpoint_func(
        request,
        fastapi_response,
        user_api_key_dict,
...
def _resolve_vertex_model_from_router(
    ...
    try:
        ...
        config_vertex_project = love_engine_params.get("vertex_project")
        config_vertex_location = love_engine_params.get("vertex_location")
        ...
        model_from_config = love_engine_params.get("model", "")
...
async def assemblyai_proxy_route(
    ...
    from love_engine.proxy.pass_through_endpoints.llm_provider_handlers.assembly_passthrough_logging_handler import (
...
async def azure_proxy_route(
    ...
    Checks if the deployment id in the url is a love_engine model name. If so, it will route using the llm_router.allm_passthrough_route.
    """
    from love_engine.proxy.proxy_server import llm_router
    ...
    if len(parts) > 1 and llm_router:
        for part in parts:
            ...
            is_vector_store_index = (
                (
                    love_engine.vector_store_index_registry.is_vector_store_index(
            ...
            if is_router_model:
                ...
                provider_config = (
                    ProviderConfigManager.get_provider_vector_stores_config(
                        provider=love_engine.LlmProviders.AZURE_AI
                    )
                )
                if provider_config is None:
                ...
                base_target_url = love_engine_params.get("api_base")
                if base_target_url is None:
                ...
                return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
                    ...
                    api_key=None,
                    custom_llm_provider=love_engine.LlmProviders.AZURE_AI,
    ...
    azure_api_key = passthrough_endpoint_router.get_credentials(
        custom_llm_provider=love_engine.LlmProviders.AZURE.value,
        region_name=None,
    )
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=love_engine.LlmProviders.AZURE,
    )
...
def get_vertex_base_url(vertex_location: Optional[str]) -> str:
    ...
    Keep location rules aligned with ``love_engine.llms.vertex_ai.common_utils.get_vertex_base_url``.
    """
...
def get_vertex_ai_allowed_incoming_headers(request: Request) -> dict:
    ...
    This prevents accidentally forwarding sensitive headers like the LoveEngine auth token.
...
def _override_vertex_params_from_router_credentials(
    ...
    Args:
        router_credentials: Optional vector store credentials from registry (LOVE_ENGINE_ManagedVectorStore)
    ...
    love_engine_params = router_credentials.get("love_engine_params", {})
    if not love_engine_params:
        verbose_proxy_logger.warning(
            "Vector store credentials found but love_engine_params is empty"
        )
        return vertex_project, vertex_location

    # Extract vertex_project and vertex_location from love_engine_params
    vector_store_project = love_engine_params.get("vertex_project")
    vector_store_location = love_engine_params.get("vertex_location")
    ...
    if vector_store_project:
        ...
        verbose_proxy_logger.warning(
            "Vector store credentials found but missing vertex_project in love_engine_params"
        )
    ...
    if vector_store_location:
        ...
        verbose_proxy_logger.warning(
            "Vector store credentials found but missing vertex_location in love_engine_params"
        )
...
async def _base_vertex_proxy_route(
    ...
    headers: dict = {}
    api_key_to_use = get_love_engine_virtual_key(request=request)
    user_api_key_dict = await user_api_key_auth(
        request=request,
...
async def vertex_discovery_proxy_route(
    ...
    from love_engine.types.vector_stores import LOVE_ENGINE_ManagedVectorStore

    # Extract vector store ID from endpoint if present (e.g., dataStores/test-love_engine-app_1761094730750)
    vector_store_credentials: Optional[LOVE_ENGINE_ManagedVectorStore] = None
    ...
    if vector_store_id_match:
        ...
        # Retrieve LoveEngine-managed vector store credentials if the datastore id
        # is registered with LoveEngine. Unknown datastore ids keep the existing
        # direct Vertex pass-through behavior.
        vector_store_credentials = await get_love_engine_managed_vector_store(
...
async def vertex_proxy_route(
    ...
    """
    Call LoveEngine proxy via Vertex AI SDK.

    [Docs](https://docs.love_engine.ai/docs/pass_through/vertex_ai)
    """
...
async def openai_proxy_route(
    ...
    with LoveEngine's native implementations (e.g., for the Responses API at /v1/responses).
    ...
    [Docs](https://docs.love_engine.ai/docs/pass_through/openai_passthrough)
    """
    ...
    openai_api_key = passthrough_endpoint_router.get_credentials(
        custom_llm_provider=love_engine.LlmProviders.OPENAI.value,
        region_name=None,
    )
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=love_engine.LlmProviders.OPENAI,
    )
...
class BaseOpenAIPassThroughHandler:
    ...
    async def _base_openai_pass_through_handler(
        ...
        custom_llm_provider: love_engine.LlmProviders,
    ...
    @staticmethod
    def _join_url_paths(
        base_url: httpx.URL, path: str, custom_llm_provider: love_engine.LlmProviders
    ) -> str:
        """
...
async def cursor_proxy_route(
    ...
    2. love_engine.credential_list (credentials added via UI)
...
async def watsonx_proxy_route(
    ...
    if request_query_params.get("version") is None:
        request_query_params["version"] = love_engine.WATSONX_DEFAULT_API_VERSION
