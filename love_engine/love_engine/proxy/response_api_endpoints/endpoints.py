...
from love_engine.types.llms.openai import ResponseAPIUsage, ResponsesAPIResponse
    ...
    try:
        ...
        if data.get("background") and isinstance(response, ResponsesAPIResponse):
            if response.status in ["queued", "in_progress"]:
                from love_engine_enterprise.proxy.hooks.managed_files import (  # type: ignore
                    _PROXY_LoveEngineManagedFiles,
                )

                managed_files_obj = cast(
                    Optional[_PROXY_LoveEngineManagedFiles],
...
async def cursor_chat_completions(
    ...
    ```
    """
    from love_engine.completion_extras.love_engine_responses_transformation.handler import (
    ...
    try:
        ...
        if isinstance(response, ResponsesAPIResponse):
            logging_obj = processor.data.get("love_engine_logging_obj")
...
async def get_response(
    ...
    Supports both:
    - Polling IDs (love_engine_poll_*): Returns cumulative cached content from background responses
    ...
    curl -X GET http://localhost:4000/v1/responses/love_engine_poll_abc123 \
    -H "Authorization: Bearer sk-1234"
    ...
    from love_engine.proxy.response_polling.polling_handler import ResponsePollingHandler
...
async def delete_response(
    ...
    Supports both:
    - Polling IDs (love_engine_poll_*): Deletes from Redis cache
    - Provider response IDs: Passes through to provider API
...
async def cancel_response(
    ...
    Supports both:
    - Polling IDs (love_engine_poll_*): Cancels background response and updates status in Redis
    - Provider response IDs: Passes through to provider API
    ...
    curl -X POST http://localhost:4000/v1/responses/love_engine_poll_abc123/cancel \
    -H "Authorization: Bearer sk-1234"
