...
class ResetBudgetJob:
    ...
    async def reset_budget(
        ...
        if self.prisma_client is not None:
            ### RESET KEY BUDGET ###
            await self.reset_budget_for_love_engine_keys()

            ### RESET USER BUDGET ###
            await self.reset_budget_for_love_engine_users()

            ## Reset Team Budget
            await self.reset_budget_for_love_engine_teams()

            ### RESET ENDUSER (Customer) BUDGET and corresponding Budget duration ###
            await self.reset_budget_for_love_engine_budget_table()
    ...
    async def _cascade_reset_spend_for_budget_link(
        self,
        budgets_to_reset: List[LOVE_ENGINE_BudgetTableFull],
        table: Any,
    ...
    async def reset_budget_for_love_engine_team_members(
        self, budgets_to_reset: List[LOVE_ENGINE_BudgetTableFull]
    ):
        """
        Resets the budget for all LoveEngine Team Members if their budget has expired
        """
    ...
    async def reset_budget_for_keys_linked_to_budgets(
        ...
        reset_budget_for_love_engine_keys() to avoid double-resetting.
        """
        return await self._cascade_reset_spend_for_budget_link(
            budgets_to_reset=budgets_to_reset,
    ...
    async def reset_budget_for_tags_linked_to_budgets(
        ...
        ``LOVE_ENGINE_TagTable.spend`` once the spend counter expires; without
    ...
    async def reset_budget_for_love_engine_budget_table(self):
        """
        Resets the budget for all LoveEngine End-Users (Customers), and Team Members if their budget has expired
        The corresponding Budget duration is also updated.
        """

        now = datetime.now(timezone.utc)
        start_time = time.time()
        endusers_to_reset: Optional[List[LOVE_ENGINE_EndUserTable]] = None
        budgets_to_reset: Optional[List[LOVE_ENGINE_BudgetTableFull]] = None
        updated_endusers: List[LOVE_ENGINE_EndUserTable] = []
        failed_endusers = []
        try:
            ...
            if budgets_to_reset is not None and len(budgets_to_reset) > 0:
                ...
                # default budget via love_engine.max_end_user_budget_id.  These
                # users are enforced in-memory but never had budget_id
                # persisted, so the query above misses them.
                if (
                    love_engine.max_end_user_budget_id is not None
                    and love_engine.max_end_user_budget_id in budget_ids_to_reset
                ):
                ...
                await self.reset_budget_for_love_engine_team_members(
                    budgets_to_reset=budgets_to_reset
                )
    ...
    async def _get_endusers_with_no_budget_id(
        ...
        rely on the default budget (love_engine.max_end_user_budget_id) applied
        ...
        return [LOVE_ENGINE_EndUserTable(**row.dict()) for row in rows]

    async def _write_key_reset_updates(
        self, updated_keys: List[LOVE_ENGINE_VerificationToken]
    ) -> None:
        """
        ...
        update input type for LOVE_ENGINE_VerificationToken, and the failure
    ...
    async def _write_user_reset_updates(
        self, updated_users: List[LOVE_ENGINE_UserTable]
    ) -> None:
        """
        ...
        for u in updated_users:
            ...
            if user_id is None:
                continue
            batcher.love_engine_usertable.update(
    ...
    async def _write_team_reset_updates(
        self, updated_teams: List[LOVE_ENGINE_TeamTable]
    ) -> None:
        """
    ...
    async def reset_budget_for_love_engine_keys(self):
        """
        Resets the budget for all the love_engine keys

        Catches Exceptions and logs them
        """
        now = datetime.utcnow()
        start_time = time.time()
        keys_to_reset: Optional[List[LOVE_ENGINE_VerificationToken]] = None
        try:
            ...
            updated_keys: List[LOVE_ENGINE_VerificationToken] = []
            failed_keys = []
    ...
    async def reset_budget_for_love_engine_users(self):
        """
        Resets the budget for all LoveEngine Internal Users if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        users_to_reset: Optional[List[LOVE_ENGINE_UserTable]] = None
        try:
    ...
    async def reset_budget_for_love_engine_teams(self):
        """
        Resets the budget for all LoveEngine Internal Teams if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        teams_to_reset: Optional[List[LOVE_ENGINE_TeamTable]] = None
        try:
            teams_to_reset = await self.prisma_client.get_data(
                table_name="team", query_type="find_all", reset_at=now
            )
            updated_teams: List[LOVE_ENGINE_TeamTable] = []
            failed_teams = []
    ...
    async def reset_budget_windows(self) -> None:
        ...
        try:
            key_rows = await self.prisma_client.db.query_raw(
                'SELECT token, budget_limits FROM "LOVE_ENGINE_VerificationToken" '
        ...
        try:
            team_rows = await self.prisma_client.db.query_raw(
                'SELECT team_id, budget_limits FROM "LOVE_ENGINE_TeamTable" '
                "WHERE budget_limits IS NOT NULL"
            )
    ...
    async def _reset_budget_common(
        item: Union[LOVE_ENGINE_TeamTable, LOVE_ENGINE_UserTable, LOVE_ENGINE_VerificationToken],
        ...
        try:
            ...
            if hasattr(item, "budget_duration") and item.budget_duration is not None:
                from love_engine.proxy.common_utils.timezone_utils import (
    ...
    async def _reset_budget_for_team(
        team: LOVE_ENGINE_TeamTable, current_time: datetime
    ...
    async def _reset_budget_for_user(
        user: LOVE_ENGINE_UserTable, current_time: datetime
    ...
    async def _reset_budget_for_enduser(
        enduser: LOVE_ENGINE_EndUserTable,
    ...
    async def _reset_budget_reset_at_date(
        budget: LOVE_ENGINE_BudgetTableFull, current_time: datetime
    ) -> LOVE_ENGINE_BudgetTableFull:
        try:
    ...
    async def _reset_budget_for_key(
        key: LOVE_ENGINE_VerificationToken, current_time: datetime
