...
def check_file_size_under_limit(
    ...
    if llm_router is not None and request_data["model"] in router_model_names:
        try:
            ...
            if (
                deployment
                and deployment.love_engine_params is not None
                and deployment.love_engine_params.max_file_size_mb is not None
            ):
                max_file_size_mb = deployment.love_engine_params.max_file_size_mb
        except Exception as e:
            verbose_proxy_logger.error(
...
async def convert_upload_files_to_file_data(
    ...
    """
    Convert FastAPI UploadFile objects to file data tuples for love_engine.

    Converts UploadFile objects to tuples of (filename, content, content_type)
    which is the format expected by httpx and love_engine's HTTP handlers.
...
def extract_nested_form_metadata(
    form_data: Dict[str, Any], prefix: str = "love_engine_metadata["
) -> Dict[str, Any]:
    """
    ...
    such as love_engine_metadata[spend_logs_metadata][owner] = "value".
    ...
    Args:
        ...
        prefix: The prefix to look for in form keys (default: "love_engine_metadata[")
    ...
    Example:
        Input form_data:
        {
            "love_engine_metadata[spend_logs_metadata][owner]": "john",
            "love_engine_metadata[spend_logs_metadata][team]": "engineering",
            "love_engine_metadata[tags]": "production",
            "other_field": "value"
        }
    ...
    for key, value in form_data.items():
        ...
        # Example: "love_engine_metadata[spend_logs_metadata][owner]" -> ["spend_logs_metadata", "owner"]
        try:
