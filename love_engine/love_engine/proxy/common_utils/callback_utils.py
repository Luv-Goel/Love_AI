...
import love_engine
from love_engine import get_secret
...
from love_engine.proxy._types import CommonProxyErrors, LoveEnginePromptInjectionParams
...
# avoid double-encrypting if `LOVE_ENGINE_SALT_KEY` is rotated between writes.
_CALLBACK_VAR_ENCRYPTED_PREFIX = "love_engine_enc::"
...
def initialize_callbacks_on_proxy(
    ...
    if isinstance(value, list):
        ...
        for callback in value:  # ["presidio", <my-custom-callback>]
            ...
            if (
                ...
                presidio_logging_only: Optional[bool] = love_engine_settings.get(
                ...
                try:
                    from love_engine_enterprise.enterprise_callbacks.llama_guard import (
                ...
                from love_engine.proxy.guardrails.guardrail_hooks.lakera_ai import (
                    lakeraAI_Moderation,
                )
                ...
                from love_engine.proxy.guardrails.guardrail_hooks.aporia_ai.aporia_ai import (
                    AporiaGuardrail,
                )
                ...
                try:
                    from love_engine_enterprise.enterprise_callbacks.llm_guard import (
                ...
                if "prompt_injection_params" in love_engine_settings:
                    prompt_injection_params_in_config = love_engine_settings[
                        "prompt_injection_params"
                    ]
                    prompt_injection_params = LoveEnginePromptInjectionParams(
                ...
                from love_engine.proxy.hooks.batch_redis_get import (
                ...
                from love_engine.proxy.hooks.azure_content_safety import (
                    _PROXY_AzureContentSafety,
                )

                azure_content_safety_params = love_engine_settings[
                ...
                from love_engine.integrations.datadog.datadog_cost_management import (
        ...
        if isinstance(love_engine.callbacks, list):
            love_engine.callbacks.extend(imported_list)
        else:
            love_engine.callbacks = imported_list  # type: ignore
    ...
    verbose_proxy_logger.debug(
        f"{blue_color_code} Initialized Callbacks - {love_engine.callbacks} {reset_color_code}"
    )


def get_model_group_from_love_engine_kwargs(kwargs: dict) -> Optional[str]:
    _love_engine_params = kwargs.get("love_engine_params", None) or {}
    _metadata = (
        _love_engine_params.get(get_metadata_variable_name_from_kwargs(kwargs)) or {}
    )
...
def get_remaining_tokens_and_requests_from_request_data(data: Dict) -> Dict[str, str]:
    """
    Helper function to return x-love_engine-key-remaining-tokens-{model_group} and x-love_engine-key-remaining-requests-{model_group}
    ...
    # Remaining Requests
    remaining_requests_variable_name = f"love_engine-key-remaining-requests-{model_group}"
    remaining_requests = _metadata.get(remaining_requests_variable_name, None)
    if remaining_requests:
        headers[f"x-love_engine-key-remaining-requests-{h11_model_group_name}"] = (
            remaining_requests
        )
    ...
    if remaining_tokens:
        headers[f"x-love_engine-key-remaining-tokens-{h11_model_group_name}"] = (
            remaining_tokens
        )
...
def get_logging_caching_headers(request_data: Dict) -> Optional[Dict]:
    ...
    love_engine_metadata_bucket = request_data.get("love_engine_metadata")
    if isinstance(metadata_bucket, dict):
        _metadata.update(metadata_bucket)
    if isinstance(love_engine_metadata_bucket, dict):
        # Batch/file routes store proxy tracking in love_engine_metadata while
        # user-facing metadata stays in metadata; merge both for headers.
        _metadata.update(love_engine_metadata_bucket)
    headers = {}
    if "applied_guardrails" in _metadata:
        headers["x-love_engine-applied-guardrails"] = ",".join(
            _metadata["applied_guardrails"]
        )

    if "applied_policies" in _metadata:
        headers["x-love_engine-applied-policies"] = ",".join(_metadata["applied_policies"])
    ...
    if "policy_sources" in _metadata:
        ...
        if isinstance(sources, dict) and sources:
            ...
            headers["x-love_engine-policy-sources"] = "; ".join(
                f"{name}={reason}" for name, reason in sources.items()
            )

    if "semantic-similarity" in _metadata:
        headers["x-love_engine-semantic-similarity"] = str(_metadata["semantic-similarity"])
...
def get_metadata_variable_name_from_kwargs(
    ...
    Context:
    - LoveEngine used `metadata` as an internal field for storing metadata
    - OpenAI then started using this field for their metadata
    - LoveEngine is now moving to using `love_engine_metadata` for our metadata
    """
    return "love_engine_metadata" if "love_engine_metadata" in kwargs else "metadata"


LOVE_ENGINE_PROXY_INTERNAL_METADATA_KEYS = frozenset(
    {
        "applied_policies",
        "applied_guardrails",
        "policy_sources",
...
def _get_or_create_proxy_metadata_bucket(
    request_data: Dict,
) -> tuple[Literal["metadata", "love_engine_metadata"], dict]:
    """
    Return the proxy-internal metadata bucket for this request.

    Batch/file routes store proxy state in ``love_engine_metadata`` so the OpenAI
...
def sanitize_openai_provider_metadata(
    ...
    Strips LoveEngine proxy-internal tracking fields that must not be forwarded to
    ...
    for key, value in metadata.items():
        if key in LOVE_ENGINE_PROXY_INTERNAL_METADATA_KEYS:
            continue
...
def add_policy_sources_to_metadata(request_data: Dict, policy_sources: Dict[str, str]):
    """
    Store policy match reasons in metadata for x-love_engine-policy-sources header.
...
def add_guardrail_response_to_standard_logging_object(
    love_engine_logging_obj: Optional["LoveEngineLogging"],
    ...
    standard_logging_object: Optional[StandardLoggingPayload] = (
        love_engine_logging_obj.model_call_details.get("standard_logging_object")
    )
...
def _encrypt_if_plaintext(key: str, value: Any) -> Any:
    ...
    try:
        ...
        # than crash the write. Dev environments without LOVE_ENGINE_SALT_KEY hit
