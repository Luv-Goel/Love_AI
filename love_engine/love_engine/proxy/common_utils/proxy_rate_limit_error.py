"""
ProxyRateLimitError — a unified rate-limit exception used by love_engine's
proxy-side hooks.

Background
----------
LoveEngine previously surfaced rate-limit conditions through *several* unrelated
exception types:

* :class:`love_engine.exceptions.RateLimitError` — raised by exception mapping when
...
* :class:`fastapi.HTTPException` (status 429) — raised directly by proxy hooks
  ...
  etc.
* :class:`love_engine.llms.base_llm.chat.transformation.BaseLLMException` (status
...
distinguish *where* the rate limit originated (vendor vs. love_engine, batch vs.
chat) without ad-hoc string-matching on the message.

This module provides a single proxy-side error class that:

1. Is a subclass of :class:`love_engine.exceptions.RateLimitError`, so user code
...
3. Carries a :attr:`category` field (one of
   :class:`love_engine.exceptions.RateLimitErrorCategory`) so callers can switch on
...
from love_engine.exceptions import RateLimitError, RateLimitErrorCategory, RateLimitType
...
class ProxyRateLimitError(HTTPException, RateLimitError):  # type: ignore[misc]
    """
    A 429 raised by love_engine's proxy-side rate limiting hooks.

    This class deliberately inherits from BOTH
    :class:`love_engine.exceptions.RateLimitError` and :class:`fastapi.HTTPException`
    ...
    * ``isinstance(e, HTTPException)`` (FastAPI / proxy_server.py route
    ...
    ``raise HTTPException(status_code=429, ...)`` for love_engine-internal rate
    limits.
    ...
    category:
        ...
        ``LOVE_ENGINE_RATE_LIMIT`` since this class is only used by love_engine's own
        proxy-side limiters; pass ``LOVE_ENGINE_BATCH_RATE_LIMIT`` for the batch
        limiter, etc.
    model / llm_provider:
    ...
    # value. Distinguishing vendor vs. love_engine 429s is now the job of the
    ...
    def __init__(
        ...
        ] = RateLimitErrorCategory.LOVE_ENGINE_RATE_LIMIT,
        rate_limit_type: Optional[Union[str, RateLimitType]] = None,
        model: Optional[str] = None,
        llm_provider: Optional[str] = "love_engine_proxy",
    ):
        ...
        model = model or ""
        llm_provider = llm_provider or "love_engine_proxy"
        ...
        # Now initialize the love_engine RateLimitError portion. We deliberately
