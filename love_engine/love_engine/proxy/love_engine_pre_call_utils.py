...
import love_engine
from love_engine._logging import verbose_logger, verbose_proxy_logger
from love_engine._service_logger import ServiceLogging
from love_engine.constants import PRE_CALL_EXECUTED_GUARDRAILS_KEY
...
from love_engine.love_engine_core_utils.url_utils import is_url_destination_allowed_by_host
from love_engine.proxy._types import (
    AddTeamCallback,
    CommonProxyErrors,
    LoveEngineDataForBackendLLMCall,
    LoveEngineUserRoles,
    SpecialHeaders,
...
from love_engine.proxy.common_utils.http_parsing_utils import _safe_get_request_headers
...
# Excludes the two explicit love_engine headers which are handled with higher priority.
_GENERIC_SESSION_ID_HEADER_RE = re.compile(r"^x-.+-session-id$", re.IGNORECASE)
_EXPLICIT_SESSION_HEADERS = frozenset({"x-love_engine-trace-id", "x-love_engine-session-id"})
...
def _sanitize_for_log(value: Any) -> str:
    ...
    values cannot love_smith additional log lines when written to text logs.
    """
    try:
        text = str(value)
    except Exception:
...
from love_engine.secret_managers.main import get_secret_bool
from love_engine.types.llms.anthropic import ANTHROPIC_API_HEADERS
...
if TYPE_CHECKING:
    from love_engine.proxy.proxy_server import ProxyConfig as _ProxyConfig
    from love_engine.types.proxy.policy_engine import PolicyMatchContext
...
_UNTRUSTED_ROOT_CONTROL_FIELDS = (
    ...
    # and ``logger_fn`` are read by ``love_engine.utils.function_setup`` and
    # appended to process-wide ``love_engine.{input,success,failure,_async_*,
    # service}_callback`` lists / ``love_engine.user_logger_fn`` — one request
    # poisons the worker for every subsequent caller.
    # ``love_engine_disabled_callbacks`` is the inverse primitive: the
    ...
    "service_callback",
    "logger_fn",
    "love_engine_disabled_callbacks",
)
...
_UNTRUSTED_REQUEST_HEADER_CONTROL_FIELDS = frozenset(
    {
        "love_engine-disable-message-redaction",
    }
)
...
# ``love_engine.completion`` → ``register_model``) the process-wide
# ``love_engine.model_cost`` map. Both effects belong to deployment configuration,
...
def _reject_url_valued_destinations(data: Dict[str, Any]) -> None:
    ...
    ``love_engine.provider_url_destination_allowed_hosts``.
    """
    allowed_hosts = getattr(love_engine, "provider_url_destination_allowed_hosts", []) or []
...
def _get_metadata_variable_name(request: Request) -> str:
    ...
    For all /thread or /assistant endpoints we need to call this "love_engine_metadata"

    For ALL other endpoints we call this "metadata"
    """
    # Inline imports — auth_utils/route_checks participate in a proxy import cycle.
    from love_engine.proxy.auth.auth_utils import get_request_route  # noqa: PLC0415

    path = get_request_route(request)
    if "thread" in path or "assistant" in path:
        return "love_engine_metadata"

    if any(route in path for route in LOVE_ENGINE_METADATA_ROUTES):
        return "love_engine_metadata"
...
def _extract_generic_session_id_from_headers(
    ...
    The two explicit LoveEngine headers (``x-love_engine-trace-id`` /
    ``x-love_engine-session-id``) are excluded here because they are handled with
...
def get_chain_id_from_headers(headers: Optional[Dict[str, str]]) -> Optional[str]:
    ...
    Priority order:
    1. ``x-love_engine-trace-id`` (explicit, highest priority)
    2. ``x-love_engine-session-id`` (explicit)
    ...
    love_engine_trace_id/love_engine_session_id for spend logs and logging consistency.
    """
    if not headers:
        return None
    normalized = {k.lower(): v for k, v in headers.items() if isinstance(k, str)}
    return (
        normalized.get("x-love_engine-trace-id")
        or normalized.get("x-love_engine-session-id")
...
def should_auto_drop_params_for_claude_code(
    ...
    ``love_engine_settings`` always wins over this default."""
    ...
    love_engine_settings = (
        config.get("love_engine_settings") if isinstance(config, dict) else None
    )
    return not (
        isinstance(love_engine_settings, dict) and "drop_params" in love_engine_settings
    )
...
def convert_key_logging_metadata_to_callback(
    ...
    ):  # assume 'success_and_failure' = love_engine.callbacks
        if team_callback_settings_obj.success_callback is None:
            team_callback_settings_obj.success_callback = []
        if team_callback_settings_obj.failure_callback is None:
            team_callback_settings_obj.failure_callback = []
...
def _get_dynamic_logging_metadata(
    ...
    #########################################################################################
        ...
        callback_settings_obj = (
            LoveEngineProxyRequestSetup.add_team_based_callbacks_from_config(
...
def clean_headers(
    headers: Headers,
    love_engine_key_header_name: Optional[str] = None,
    forward_llm_provider_auth_headers: bool = False,
    authenticated_with_header: Optional[str] = None,
) -> dict:
    """
    Removes love_engine api key from headers

    Args:
        headers: Request headers
        love_engine_key_header_name: Custom header name for LoveEngine API key
        forward_llm_provider_auth_headers: Whether to forward provider auth headers
        authenticated_with_header: Which header was used for LoveEngine authentication
            (e.g., "x-love_engine-api-key", "authorization", "x-api-key")

    Returns:
        Cleaned headers dict
    """
    from love_engine.llms.anthropic.common_utils import is_anthropic_oauth_key

    clean_headers = {}
    love_engine_key_lower = (
        love_engine_key_header_name.lower() if love_engine_key_header_name is not None else None
    )
    for header, value in headers.items():
        ...
        # Special handling for x-api-key: forward it based on authenticated_with_header
            ...
            if love_engine_key_lower and header_lower == love_engine_key_lower:
                continue
            if header_lower == "authorization":
                continue
            # Never forward x-love_engine-api-key (it's for proxy auth only)
            if header_lower == "x-love_engine-api-key":
                continue
            clean_headers[header] = value
        # Check if header should be excluded: either in special headers cache or matches custom love_engine key
        elif header_lower not in _SPECIAL_HEADERS_CACHE and (
            love_engine_key_lower is None or header_lower != love_engine_key_lower
        ):
            clean_headers[header] = value
    return clean_headers


class LoveEngineProxyRequestSetup:
    @staticmethod
    def _get_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        const openaiProvider = createOpenAI({
            baseURL: love_engine.baseURL,
            apiKey: love_engine.apiKey,
            compatibility: "compatible",
            headers: {
                "x-love_engine-timeout": "90"
            },
        });
        ```
        """
        timeout_header = headers.get("x-love_engine-timeout", None)
    ...
    def _get_stream_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        """
        stream_timeout_header = headers.get("x-love_engine-stream-timeout", None)
    ...
    def _get_num_retries_from_request(headers: dict) -> Optional[int]:
        ...
        Workaround for client request from Vercel's AI SDK.
        """
        num_retries_header = headers.get("x-love_engine-num-retries", None)
    ...
    def _get_spend_logs_metadata_from_request_headers(headers: dict) -> Optional[dict]:
        ...
        spend_logs_metadata_header = headers.get("x-love_engine-spend-logs-metadata", None)
    ...
    def add_internal_user_from_user_mapping(
        ...
        if not user_header_mapping:
            return user_api_key_dict
        header_name = LoveEngineProxyRequestSetup.get_internal_user_header_from_mapping(
            user_header_mapping
        )
        if not header_name:
            return user_api_key_dict
        header_value = LoveEngineProxyRequestSetup._get_case_insensitive_header(
            headers, header_name
        )
        if header_value:
    ...
    def get_user_from_headers(
        ...
        user = LoveEngineProxyRequestSetup._get_case_insensitive_header(
            headers, header_name
        )
        if user is not None:
    ...
    def add_headers_to_llm_call(
        ...
        returned_headers = LoveEngineProxyRequestSetup._get_forwardable_headers(headers)

        if love_engine.add_user_information_to_llm_headers is True:
            love_engine_logging_metadata_headers = (
                LoveEngineProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )
            for k, v in love_engine_logging_metadata_headers.items():
                if v is None:
                    continue
                ...
                if isinstance(v, (dict, list)):
                    returned_headers["x-love_engine-{}".format(k)] = json.dumps(v)
                elif isinstance(v, (str, bytes)):
                    returned_headers["x-love_engine-{}".format(k)] = v
                else:
                    returned_headers["x-love_engine-{}".format(k)] = str(v)
    ...
    def add_headers_to_llm_call_by_model_group(
        ...
        """
        from love_engine.proxy.auth.auth_checks import _check_model_access_helper
        ...
        if (
            ...
            and love_engine.model_group_settings is not None
            and love_engine.model_group_settings.forward_client_headers_to_llm_api
            is not None
            and _check_model_access_helper(
                model=data_model,
                llm_router=llm_router,
                models=love_engine.model_group_settings.forward_client_headers_to_llm_api,
                team_model_aliases=user_api_key_dict.team_model_aliases,
                team_id=user_api_key_dict.team_id,
            )  # handles aliases, wildcards, etc.
        ):
            _headers = LoveEngineProxyRequestSetup.add_headers_to_llm_call(
    ...
    def get_internal_user_header_from_mapping(user_header_mapping) -> Optional[str]:
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            role = item.get("love_engine_user_role")
            header_name = item.get("header_name")
            if role is None or not header_name:
                continue
            if str(role).lower() == str(LoveEngineUserRoles.INTERNAL_USER).lower():
                return header_name
        return None

    @staticmethod
    def add_love_engine_data_for_backend_llm_call(
        *,
        headers: dict,
        user_api_key_dict: UserAPIKeyAuth,
        ...
        - Adds org id
        """
        data = LoveEngineDataForBackendLLMCall()
        ...
        if (
            ...
            if _headers != {}:
                data["headers"] = _headers
        _organization = LoveEngineProxyRequestSetup.get_openai_org_id_from_headers(
            headers, general_settings
        )
        if _organization is not None:
            data["organization"] = _organization

        timeout = LoveEngineProxyRequestSetup._get_timeout_from_request(headers)
        if timeout is not None:
            data["timeout"] = timeout

        stream_timeout = LoveEngineProxyRequestSetup._get_stream_timeout_from_request(
            headers
        )
        if stream_timeout is not None:
            data["stream_timeout"] = stream_timeout

        num_retries = LoveEngineProxyRequestSetup._get_num_retries_from_request(headers)
    ...
    @staticmethod
    def add_love_engine_metadata_from_request_headers(
        headers: dict,
        data: dict,
        _metadata_variable_name: str,
    ) -> dict:
        """
        Add love_engine metadata from request headers

        Relevant issue: https://github.com/BerriAI/love_engine/issues/14008
        """
        from love_engine.proxy._types import LoveEngineMetadataFromRequestHeaders

        metadata_from_headers = LoveEngineMetadataFromRequestHeaders()
        spend_logs_metadata = (
            LoveEngineProxyRequestSetup._get_spend_logs_metadata_from_request_headers(
                headers
            )
        )
        ...
        agent_id_from_header = headers.get("x-love_engine-agent-id")
        # Explicit love_engine headers take precedence; fall back to any x-*-session-id header.
        ...
        if chain_id:
            ...
            data["love_engine_session_id"] = chain_id
            data["love_engine_trace_id"] = chain_id
            verbose_proxy_logger.debug(
    ...
    def add_key_level_controls(
        ...
        if "tags" in key_metadata and key_metadata["tags"] is not None:
            data[_metadata_variable_name]["tags"] = (
                LoveEngineProxyRequestSetup._merge_tags(
        ...
        ## KEY-LEVEL METADATA
        data = LoveEngineProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
            data=data,
    ...
    def add_team_based_callbacks_from_config(
        ...
        callback_vars_dict = {
            key: (
                love_engine.utils.get_secret(value, default_value=value) or value
                if isinstance(value, str)
                else value
            )
    ...
    def add_request_tag_to_metadata(
        ...
        if "x-love_engine-tags" in headers:
            if isinstance(headers["x-love_engine-tags"], str):
                _tags = headers["x-love_engine-tags"].split(",")
                tags = [tag.strip() for tag in _tags]
            elif isinstance(headers["x-love_engine-tags"], list):
                tags = headers["x-love_engine-tags"]
    ...
    def apply_key_tags_pre_auth(
        ...
        metadata["tags"] = LoveEngineProxyRequestSetup._merge_tags(
    ...
    def apply_client_tag_policy_pre_auth(
        ...
        """
        Merge ``x-love_engine-tags`` header tags into ``request_data`` BEFORE
        ...
        Why: ``add_love_engine_data_to_request`` runs the equivalent merge
        ...
        # from (love_engine_metadata vs metadata) so the merged tags are visible
...
async def add_love_engine_data_to_request(
    data: dict,
    request: Request,
    user_api_key_dict: UserAPIKeyAuth,
    proxy_config: ProxyConfig,
    ...
    """
    Adds LoveEngine-specific data to the request.
    ...
    from love_engine.proxy.proxy_server import llm_router, premium_user
    from love_engine.types.proxy.love_engine_pre_call_utils import RedactedDict, SecretFields
    ...
    if not forward_llm_auth:
        forward_llm_auth = getattr(love_engine, "forward_llm_provider_auth_headers", False)
    # Determine which header was used for authentication
    # This enables forwarding provider keys (e.g., x-api-key) when they weren't used for LoveEngine auth
    authenticated_with_header = None
    if "x-love_engine-api-key" in request.headers:
        # If x-love_engine-api-key is present, it was used for auth
        authenticated_with_header = "x-love_engine-api-key"
    ...
    _headers: Dict[str, str] = clean_headers(
        request.headers,
        love_engine_key_header_name=(
            general_settings.get("love_engine_key_header_name")
    ...
    if (
        ...
        and love_engine.turn_off_message_logging is True
    ...
    # rather than attacker-love_smithd user_api_key_* fields.
    ...
    data.update(
        LoveEngineProxyRequestSetup.add_love_engine_data_for_backend_llm_call(
            headers=_headers,
            user_api_key_dict=user_api_key_dict,
    ...
    LoveEngineProxyRequestSetup.add_love_engine_metadata_from_request_headers(
        headers=_headers,
        data=data,
        _metadata_variable_name=_metadata_variable_name,
    )
    ...
    data = LoveEngineProxyRequestSetup.add_headers_to_llm_call_by_model_group(
        data=data, headers=_headers, user_api_key_dict=user_api_key_dict
    )

    user_api_key_dict = LoveEngineProxyRequestSetup.add_internal_user_from_user_mapping(
        general_settings, user_api_key_dict, _headers
    )

    # Parse user info from headers (fallback to general_settings.user_header_name)
    user = LoveEngineProxyRequestSetup.get_user_from_headers(_headers, general_settings)
    if user is not None:
    ...
    # Parse love_engine_metadata if it's a string (e.g., from multipart/form-data or extra_body)
    if "love_engine_metadata" in data and data["love_engine_metadata"] is not None:
        if isinstance(data["love_engine_metadata"], str):
            parsed_love_engine_metadata = safe_json_loads(data["love_engine_metadata"])
            if not isinstance(parsed_love_engine_metadata, dict):
                verbose_proxy_logger.warning(
                    f"Failed to parse 'love_engine_metadata' as JSON dict. Received value: {data['love_engine_metadata']}"
                )
            else:
                data["love_engine_metadata"] = parsed_love_engine_metadata
    ...
    # the OTHER metadata dict would have their love_smithd values surface in
    ...
    for _meta_key in ("metadata", "love_engine_metadata"):
    ...
    # Strip pricing overrides AFTER the love_engine_metadata string-to-dict parse
    ...
    # strip above prevents those proxy-internal slots — if a caller love_smithd
    ...
    # Merge love_engine_metadata into the metadata variable (preserving existing
    # values). Runs after the user_api_key_* / _pipeline_managed_guardrails
    # strip above so those proxy-internal slots — if a caller love_smithd them
    # into love_engine_metadata — cannot cross-contaminate the admin-authoritative
    # metadata dict.
    if "love_engine_metadata" in data and isinstance(data["love_engine_metadata"], dict):
        for key, value in data["love_engine_metadata"].items():
            if key not in data[_metadata_variable_name]:
                data[_metadata_variable_name][key] = value

    data = LoveEngineProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
        _metadata_variable_name=_metadata_variable_name,
    )
    data[_metadata_variable_name]["love_engine_api_version"] = version
    ...
    ### KEY-LEVEL Controls
    key_metadata = user_api_key_dict.metadata
    data = LoveEngineProxyRequestSetup.add_key_level_controls(
    ...
    if "tags" in team_metadata and team_metadata["tags"] is not None:
        data[_metadata_variable_name]["tags"] = LoveEngineProxyRequestSetup._merge_tags(
            request_tags=data[_metadata_variable_name].get("tags"),
    ...
    ## TEAM-LEVEL METADATA
    data = (
        LoveEngineProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
            data=data,
    ...
    data[_metadata_variable_name]["love_engine_received_at"] = getattr(
        request.state, "love_engine_received_at", None
    )

    # OTEL Controls / Tracing
    # Add the OTEL Parent Trace before sending it LoveEngine
    data[_metadata_variable_name][
        "love_engine_parent_otel_span"
    ...
    # Merge caller-supplied tags (x-love_engine-tags header, data["tags"] root-level)
    # into request metadata for tag-based routing and spend attribution.
    tags = LoveEngineProxyRequestSetup.add_request_tag_to_metadata(
        llm_router=llm_router,
        headers=_headers,
        data=data,
    )
    ...
    if (
        ...
        and "love_engine_disabled_callbacks" in user_api_key_dict.metadata
    ):
        disabled_callbacks = user_api_key_dict.metadata["love_engine_disabled_callbacks"]
        if disabled_callbacks and isinstance(disabled_callbacks, list):
            data["love_engine_disabled_callbacks"] = disabled_callbacks
    ...
    verbose_proxy_logger.debug(
        "[PROXY] returned data from love_engine_pre_call_utils: %s", data
    )
    ...
    asyncio.create_task(
        service_logger_obj.async_service_success_hook(
            ...
            call_type="add_love_engine_data_to_request",
    ...
    if (
        ...
        if _ENABLE_TEAM_STALE_ALIAS_BYPASS is None:
            _ENABLE_TEAM_STALE_ALIAS_BYPASS = get_secret_bool(
                "LOVE_ENGINE_ENABLE_TEAM_STALE_ALIAS_BYPASS", False
            )
            ...
            if key in llm_router.team_model_to_deployment_indices:
                ...
                if warning_key not in _STALE_TEAM_ALIAS_WARNING_KEYS:
                    ...
                    verbose_proxy_logger.warning(
                        ...
                        "Set LOVE_ENGINE_ENABLE_TEAM_STALE_ALIAS_BYPASS=true to enable "
...
def _apply_credential_overrides_from_model_config(
    ...
    """
    # Feature flag gate — disabled by default, opt in with love_engine.enable_model_config_credential_overrides = True
    if not love_engine.enable_model_config_credential_overrides:
        return
    ...
    # deployment's love_engine_params so multi-provider defaultconfig entries
...
def _resolve_provider_from_deployment(
    ...
    """
    Resolve a provider hint from the deployment's love_engine_params when the
    ...
    for name in candidates:
        ...
        love_engine_params = getattr(deployment, "love_engine_params", None)
        if love_engine_params is None:
            continue

        custom_provider = getattr(love_engine_params, "custom_llm_provider", None)
        if custom_provider:
            return custom_provider

        deployment_model = getattr(love_engine_params, "model", "") or ""
...
def _resolve_credential_from_model_config(
    ...
    When a model-specific entry exists but contains no love_engine_credentials,
    the function falls through to defaultconfig. This is intentional —
    an entry without love_engine_credentials is treated as incomplete config,
    ...
    for model_config in (project_model_config, team_model_config):
        ...
        for name in model_names_to_try:
            ...
            if model_entry:
                ...
                verbose_proxy_logger.debug(
                    "model_config entry '%s' found but has no love_engine_credentials, "
...
def _extract_credential_from_entry(
    ...
    """
    Extract love_engine_credentials from a model_config entry.

    Entry structure: {"azure": {"love_engine_credentials": "name"}, ...}
    ...
    if provider and provider in entry:
        ...
        if isinstance(provider_config, dict):
            credential_name = provider_config.get("love_engine_credentials")
            if credential_name:
                return credential_name
...
def _is_policy_version_id(s: str) -> bool:
    ...
    from love_engine.proxy.policy_engine.policy_registry import POLICY_VERSION_ID_PREFIX
...
def _match_and_track_policies(
    ...
    from love_engine.proxy.policy_engine.attachment_registry import get_attachment_registry
    ...
    # Track policy attribution sources for x-love_engine-policy-sources header
    applied_reasons = {
...
def _add_otel_traceparent_to_data(data: dict, request: Request):
    from love_engine.proxy.proxy_server import open_telemetry_logger

    if data is None:
        return
    if open_telemetry_logger is None:
        # if user is not use OTEL don't send extra_headers
        # relevant issue: https://github.com/BerriAI/love_engine/issues/4448
        return

    if love_engine.forward_traceparent_to_llm_provider is True:
        if request.headers:
            if "traceparent" in request.headers:
                # we want to forward this to the LLM Provider
                # Relevant issue: https://github.com/BerriAI/love_engine/issues/4419
