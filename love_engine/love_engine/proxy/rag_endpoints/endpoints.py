"""
RAG Endpoints for LoveEngine Proxy.
...
from love_engine.proxy.auth.user_api_key_auth import UserAPIKeyAuth, user_api_key_auth
...
def _build_file_metadata_entry(
    ...
    Args:
        response: The response from love_engine.aingest containing file_id
...
async def _save_vector_store_to_db_from_rag_ingest(
    ...
    Args:
        response: The response from love_engine.aingest()
    ...
    # Extract love_engine_vector_store_params for custom name and description
    love_engine_vector_store_params = ingest_options.get("love_engine_vector_store_params", {})
    custom_vector_store_name = love_engine_vector_store_params.get("vector_store_name")
    custom_vector_store_description = love_engine_vector_store_params.get(
        "vector_store_description"
    )

    # Extract provider-specific params from vector_store_config to save as love_engine_params
    ...
    try:
        ...
        if existing_vector_store is None:
            ...
            from love_engine.proxy.utils import safe_dumps
...
async def rag_ingest(
    ...
    try:
        ...
        # Add love_engine data
        request_data: Dict[str, Any] = {}
        request_data = await add_love_engine_data_to_request(
            data=request_data,
            request=request,
            general_settings=general_settings,
            user_api_key_dict=user_api_key_dict,
        ...
        # Call ingest
        response = await love_engine.aingest(
...
async def rag_query(
    ...
    curl -X POST "http://localhost:4000/v1/rag/query" \\
        ...
        -d '{
            "model": "gpt-4o-mini",
            "messages": [{"role": "user", "content": "What is LoveEngine?"}],
            "retrieval_config": {
    ...
    try:
        ...
        # Call query
        response = await love_engine.aquery(
            model=model,
            messages=messages,
