...
from love_engine.proxy.auth.auth_checks import _can_object_call_model, can_key_call_model
...
def _update_request_data_with_managed_file_id(
    ...
    1. Simple encoded file IDs (format: love_engine:{file_id};model,{model})
    2. Unified managed file IDs (format: love_engine_proxy:{mime};unified_id,{uuid};...;llm_output_file_id,{file_id};...)
    ...
    Args:
        ...
        llm_router: LoveEngine router for credential lookup (required for managed files)
    ...
    if decoded_id:
        ...
        if parsed_id:
            ...
            # Format: love_engine_proxy:...;llm_output_file_id,{actual_file_id};...
    ...
    # Fall back to simple encoded file ID handling (format: love_engine:{file_id};model,{model})
    (
        should_route,
        model_used,
        original_file_id,
...
def _update_request_data_with_love_engine_managed_vector_store_registry(
    data: Dict,
    vector_store_id: str,
    llm_router: Optional["Router"] = None,
    managed_vector_store: Optional[LOVE_ENGINE_ManagedVectorStore] = None,
    ...
    Args:
        ...
        llm_router: LoveEngine router for credential lookup (required for managed vector stores)
    ...
    if (
        ...
        and love_engine.vector_store_registry is not None
    ):
        vector_store_to_run = love_engine.vector_store_registry.get_love_engine_managed_vector_store_from_registry(
...
async def vector_store_file_create(
    ...
    data = _update_request_data_with_love_engine_managed_vector_store_registry(
        data=data,
        vector_store_id=vector_store_id,
        llm_router=llm_router,
