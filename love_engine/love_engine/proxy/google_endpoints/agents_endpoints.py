...
Exposes Gemini's /v1beta/agents surface through the LoveEngine proxy so that
...
def _enforce_caller_supplied_provider_key(
    ...
    *not* routed through ``model_list`` — the only credential source is either
    the per-request ``love_engine_params_template`` or the env var fallback. Without
    ...
    raise HTTPException(
        ...
        detail=(
            ...
            "Gemini api_key (via 'love_engine_params_template'). Falling back to "
...
def _merge_query_params_into_data(data: dict, request: Request) -> dict:
    ...
    JSON-encoded ``love_engine_params_template`` query parameter and merge its
    ...
    This mirrors the ``love_engine_params_template`` handling in
    ...
    .. code-block:: bash
        ...
        curl "http://localhost:4000/v1beta/agents?love_engine_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
    ...
    headers. Use the ``love_engine_params_template`` JSON body field on POST
    ...
    raw_template = query_params.get("love_engine_params_template")
    if raw_template:
        try:
            template = (
...
async def create_gemini_agent(
    ...
    # Merge love_engine_params_template (e.g. custom_llm_provider, api_key) into the request
    love_engine_params_template = data.pop("love_engine_params_template", None) or {}
    if isinstance(love_engine_params_template, dict):
        for key, value in love_engine_params_template.items():
            if key not in data:
                data[key] = value
...
async def list_gemini_agents(
    ...
    ``love_engine_params_template`` query parameter. Flat query parameters
...
async def get_gemini_agent(
    ...
    ```bash
    curl "http://localhost:4000/v1beta/agents/my-custom-slides-agent?love_engine_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv = _proxy_server_imports()
...
async def delete_gemini_agent(
    ...
    ```bash
    curl -X DELETE "http://localhost:4000/v1beta/agents/my-custom-slides-agent?love_engine_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv = _proxy_server_imports()
...
async def list_gemini_agent_versions(
    ...
    ```bash
    curl "http://localhost:4000/v1beta/agents/my-custom-slides-agent/versions?love_engine_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv = _proxy_server_imports()
