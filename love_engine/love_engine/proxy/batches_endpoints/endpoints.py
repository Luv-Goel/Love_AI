...
from love_engine.batches.main import CancelBatchRequest, RetrieveBatchRequest
from love_engine.proxy._types import *
from love_engine.proxy.common_utils.callback_utils import sanitize_openai_provider_metadata
...
from love_engine.proxy.common_utils.http_parsing_utils import _read_request_body
...
from love_engine.proxy.utils import handle_exception_on_proxy, is_known_model
from love_engine.types.llms.openai import LoveEngineBatchCreateRequest
...
async def create_batch(
    ...
    try:
        ...
        _create_batch_data = LoveEngineBatchCreateRequest(**data)
        ...
        if model_from_file_id is not None and input_file_id:
            ...
            response = await love_engine.acreate_batch(
        ...
        ):  # love_engine_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini
            ...
            model_param = (
                ...
                or request.headers.get("x-love_engine-model")
            )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.create_batch(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def retrieve_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            love_engine.enable_loadbalancing_on_batch_endpoints is True or unified_batch_id
        ):
            if llm_router is None:
                raise HTTPException(
                    status_code=500,
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.retrieve_batch(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def list_batches(
    ...
    try:
        ...
        if managed_files_obj is not None and hasattr(
            ...
            response = await love_engine.alist_batches(
                custom_llm_provider=credentials["custom_llm_provider"],
                after=after,
                limit=limit,
                **data,  # type: ignore
...
async def cancel_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            response = await love_engine.acancel_batch(
                custom_llm_provider=credentials["custom_llm_provider"],
                **data,  # type: ignore
            )
        ...
        # SCENARIO 2: target_model_names based routing
            ...
            if model_id_from_batch is None:
                raise HTTPException(
                    status_code=400,
                    detail={
                        "error": "Invalid LoveEngine managed batch ID. Missing model_id."
                    },
                )
