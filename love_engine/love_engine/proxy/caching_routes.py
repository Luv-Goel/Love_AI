...
from love_engine.caching.caching import RedisCache
...
from love_engine.proxy._types import ProxyErrorTypes, ProxyException
from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth
from love_engine.types.caching import CachePingResponse, HealthCheckCacheParams
...
def _extract_cache_params() -> Dict[str, Any]:
    ...
    if love_engine.cache is None:
        return {}
    try:
        cache_params = vars(love_engine.cache.cache)
        cleaned_params = (
...
async def cache_ping():
    ...
    """
    love_engine_cache_params: Dict[str, Any] = {}
    cleaned_cache_params: Dict[str, Any] = {}
    if love_engine.cache is None:
        raise ProxyException(
            message=safe_dumps(
                {
                    "message": "Cache not initialized. love_engine.cache is None",
                    "love_engine_cache_params": "{}",
    ...
    try:
        love_engine_cache_params = masker.mask_dict(vars(love_engine.cache))
        # remove field that might reference itself
        love_engine_cache_params.pop("cache", None)
        cleaned_cache_params = _extract_cache_params()

        if love_engine.cache.type == "redis":
            ping_response = await love_engine.cache.ping()
            verbose_proxy_logger.debug(
                "/cache/ping: ping_response: " + str(ping_response)
            )
            # add cache does not return anything
            await love_engine.cache.async_add_cache(
                result="test_key",
                model="test-model",
                messages=[{"role": "user", "content": "test from love_engine"}],
            )
            verbose_proxy_logger.debug("/cache/ping: done with set_cache()")

            return CachePingResponse(
                status="healthy",
                cache_type=str(love_engine.cache.type),
                ping_response=True,
                set_cache_response="success",
                love_engine_cache_params=safe_dumps(love_engine_cache_params),
...
async def cache_delete(request: Request):
    """
    Endpoint for deleting a key from the cache. All responses from love_engine proxy have `x-love_engine-cache-key` in the headers
    ...
    try:
        if love_engine.cache is None:
            raise HTTPException(
                status_code=503, detail="Cache not initialized. love_engine.cache is None"
            )
        ...
        if love_engine.cache.type == "redis":
            await love_engine.cache.delete_cache_keys(keys=keys)
            return {
                "status": "success",
            }
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {love_engine.cache.type} does not support deleting a key. only `redis` is supported",
            )
...
async def cache_redis_info():
    ...
    try:
        ...
        if not (
            love_engine.cache.type == "redis"
            and isinstance(love_engine.cache.cache, RedisCache)
        ):
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {love_engine.cache.type} does not support redis info",
            )

        # Get client information (handles CLIENT LIST restrictions gracefully)
        client_list, num_clients = _get_redis_client_info(love_engine.cache.cache)

        # Get Redis server information
        redis_info = love_engine.cache.cache.info()
...
async def cache_flushall():
    ...
    try:
        ...
        if love_engine.cache.type == "redis" and isinstance(
            love_engine.cache.cache, RedisCache
        ):
            love_engine.cache.cache.flushall()
            return {
                "status": "success",
            }
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {love_engine.cache.type} does not support flushing",
            )
    except Exception as e:
        raise HTTPException(
            status_code=503,
