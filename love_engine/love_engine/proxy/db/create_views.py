...
async def create_missing_views(db: _db):
    """
    --------------------------------------------------
    NOTE: Copy of `love_engine/db_scripts/create_views.py`.
    --------------------------------------------------
    ...
    try:
        ...
        await db.query_raw("""SELECT 1 FROM "LOVE_ENGINE_VerificationTokenView" LIMIT 1""")
        verbose_logger.debug("LOVE_ENGINE_VerificationTokenView Exists!")
        ...
        await db.execute_raw("""
                ...
                LEFT JOIN "LOVE_ENGINE_ProjectTable" p ON v.project_id = p.project_id;
            """)

        verbose_logger.debug("LOVE_ENGINE_VerificationTokenView Created!")
    ...
    try:
        ...
        FROM
        "LOVE_ENGINE_SpendLogs" L
        LEFT JOIN 
        "LOVE_ENGINE_VerificationToken" V
        ON
        L."api_key" = V."token"
        WHERE
...
async def should_create_missing_views(db: _db) -> bool:
    ...
    verbose_logger.debug("Estimated Row count of LOVE_ENGINE_SpendLogs = {}".format(result))
    if (
        result
