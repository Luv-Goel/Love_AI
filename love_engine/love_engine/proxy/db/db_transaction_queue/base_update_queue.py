...
from love_engine.constants import (
    LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE,
...
class BaseUpdateQueue:
    ...
    def __init__(self):
        self.update_queue = asyncio.Queue(maxsize=LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE)
        self.MAX_SIZE_IN_MEMORY_QUEUE = MAX_SIZE_IN_MEMORY_QUEUE
        if MAX_SIZE_IN_MEMORY_QUEUE >= LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE:
            verbose_proxy_logger.warning(
                "Misconfigured queue thresholds: MAX_SIZE_IN_MEMORY_QUEUE (%d) >= LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE (%d). "
                "The spend aggregation check will never trigger because the asyncio.Queue blocks at %d items. "
                "Set MAX_SIZE_IN_MEMORY_QUEUE to a value less than LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE (recommended: 80%% of it).",
                MAX_SIZE_IN_MEMORY_QUEUE,
                LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE,
                LOVE_ENGINE_ASYNCIO_QUEUE_MAXSIZE,
            )
