...
from love_engine.proxy.db.db_transaction_queue.spend_logs_partition_manager import (
...
class SpendLogCleanup:
    ...
    When LOVE_ENGINE_SpendLogs is range-partitioned, expired data is reclaimed by
    ...
    def __init__(
        ...
        from love_engine.proxy.proxy_server import general_settings as default_settings
        ...
        while True:
            ...
            try:
                deleted_result = await prisma_client.db.execute_raw(
                    """
                    DELETE FROM "LOVE_ENGINE_SpendLogs"
                    WHERE ("request_id", "startTime") IN (
                        SELECT "request_id", "startTime" FROM "LOVE_ENGINE_SpendLogs"
