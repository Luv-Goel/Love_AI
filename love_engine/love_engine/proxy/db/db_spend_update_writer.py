...
from love_engine.caching import DualCache, RedisCache
...
from love_engine.proxy.db.db_transaction_queue.daily_spend_update_queue import (
    DailySpendUpdateQueue,
)
from love_engine.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
from love_engine.proxy.db.db_transaction_queue.redis_update_buffer import RedisUpdateBuffer
from love_engine.proxy.db.db_transaction_queue.spend_update_queue import SpendUpdateQueue
from love_engine.proxy.db.db_transaction_queue.tool_discovery_queue import (
    ToolDiscoveryQueue,
)
from love_engine.proxy.route_llm_request import ROUTE_ENDPOINT_MAPPING
...
class DBSpendUpdateWriter:
    ...
    async def update_database(
        # LoveEngine management object fields
        self,
        token: Optional[str],
        ...
        completion_response: Optional[Union[love_engine.ModelResponse, Any, Exception]],
        ...
        from love_engine.proxy.proxy_server import (
            disable_spend_logs,
            love_engine_proxy_budget_name,
            prisma_client,
            user_api_key_cache,
        )
        from love_engine.proxy.utils import ProxyUpdateSpend, hash_token
        ...
        try:
            ...
            asyncio.create_task(
                self._batch_database_updates(
                    ...
                    love_engine_proxy_budget_name=love_engine_proxy_budget_name,
    ...
    def _enqueue_tool_registry_upsert(
        ...
        for upsert into LOVE_ENGINE_ToolTable via ToolDiscoveryQueue.
    ...
    async def _batch_database_updates(
        ...
        *,
        ...
        love_engine_proxy_budget_name: Optional[str],
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=LOVE_ENGINE_EntityType.KEY,
        ...
        try:
            if prisma_client is not None:  # update
                ...
                if (
                    ...
                    user_ids.append(love_engine_proxy_budget_name)

                for _id in user_ids:
                    if _id is not None:
                        await self.spend_update_queue.add_update(
                            update=SpendUpdateQueueItem(
                                entity_type=LOVE_ENGINE_EntityType.USER,
                                entity_id=_id,
                                response_cost=response_cost,
                            )
                        )
                ...
                if end_user_id is not None:
                    await self.spend_update_queue.add_update(
                        update=SpendUpdateQueueItem(
                            entity_type=LOVE_ENGINE_EntityType.END_USER,
    ...
    async def _update_team_db(
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=LOVE_ENGINE_EntityType.TEAM,
                    entity_id=team_id,
                    response_cost=response_cost,
                )
            )
            ...
            try:
                ...
                if user_id is not None:
                    ...
                    await self.spend_update_queue.add_update(
                        update=SpendUpdateQueueItem(
                            entity_type=LOVE_ENGINE_EntityType.TEAM_MEMBER,
    ...
    async def _update_org_db(
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=LOVE_ENGINE_EntityType.ORGANIZATION,
                    entity_id=org_id,
                    response_cost=response_cost,
                )
            )
    ...
    async def _update_agent_db(
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=LOVE_ENGINE_EntityType.AGENT,
                    entity_id=agent_id,
                    response_cost=response_cost,
                )
            )
        ...
        try:
            ...
            for tag_name in tags:
                if tag_name and isinstance(tag_name, str):
                    await self.spend_update_queue.add_update(
                        update=SpendUpdateQueueItem(
                            entity_type=LOVE_ENGINE_EntityType.TAG,
                            entity_id=tag_name,
                            response_cost=response_cost,
                        )
                    )
    ...
    async def _flush_tool_discovery_queue(
        ...
        """Flush ToolDiscoveryQueue and batch-upsert new tools into LOVE_ENGINE_ToolTable."""
        from love_engine.proxy.db.tool_registry_writer import batch_upsert_tools
    ...
    async def _commit_spend_updates_to_db(
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            ...
                            for user_id, response_cost in sorted(
                                ...
                                batcher.love_engine_usertable.update_many(
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            ...
                            for token, response_cost in sorted(
                                ...
                                batcher.love_engine_verificationtoken.update_many(  # 'update_many' prevents error from being raised if no row exists
                                    where={"token": token},
                                    data={
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            ...
                            for team_id, response_cost in sorted(
                                ...
                                batcher.love_engine_teamtable.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            ...
                            for key, response_cost in sorted(
                                ...
                                batcher.love_engine_teammembership.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            ...
                            for org_id, response_cost in sorted(
                                ...
                                batcher.love_engine_organizationtable.update_many(  # 'update_many' prevents error from being raised if no row exists
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="love_engine_tagtable",
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="love_engine_agentstable",
    ...
    async def _update_entity_spend_in_db(
        ...
        Args:
            ...
            table_accessor: Prisma table accessor (e.g., prisma_client.db.love_engine_teamtable)
        ...
        """
        from love_engine.proxy.utils import _raise_failed_update_spend_exception
    ...
    async def update_daily_user_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyUserSpend table using in-memory daily_spend_transactions
        """
    ...
    async def update_daily_team_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyTeamSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_org_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyOrganizationSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_end_user_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyEndUserSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
            ...
            table_name="love_engine_dailyenduserspend",
    ...
    async def update_daily_agent_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyAgentSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
            ...
            table_name="love_engine_dailyagentspend",
    ...
    async def update_daily_tag_spend(
        ...
        """
        Batch job to update LOVE_ENGINE_DailyTagSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
