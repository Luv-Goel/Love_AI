...
from love_engine.constants import SPEND_COUNTER_RESEED_LOCKS_MAX_SIZE
...
class SpendCounterReseed:
    ...
    Counter key prefixes map to DB tables:
        spend:key:{token}                 -> LOVE_ENGINE_VerificationToken.spend
        spend:team:{team_id}              -> LOVE_ENGINE_TeamTable.spend
        spend:team_member:{uid}:{tid}     -> LOVE_ENGINE_TeamMembership.spend
        spend:user:{user_id}              -> LOVE_ENGINE_UserTable.spend
        spend:org:{org_id}                -> LOVE_ENGINE_OrganizationTable.spend
