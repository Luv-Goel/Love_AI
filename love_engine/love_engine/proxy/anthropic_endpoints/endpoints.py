...
from love_engine.anthropic_interface.exceptions import AnthropicExceptionMapping
...
def _strip_total_tokens_from_anthropic_response(response: Any) -> None:
    """Remove the OpenAI-flavored `usage.total_tokens` field that LoveEngine
...
async def anthropic_response(
    ...
    """
    Use `{PROXY_BASE_URL}/anthropic/v1/messages` instead - [Docs](https://docs.love_engine.ai/docs/pass_through/anthropic_completion).
    ...
    try:
        ...
        # LoveEngine adds internally. Anthropic's official /v1/messages spec
        # only defines input_tokens / output_tokens / cache_*_input_tokens;
        # total_tokens is an OpenAI convention. Default off
        # (`love_engine.strip_anthropic_total_tokens = False`) to preserve
        ...
        if love_engine.strip_anthropic_total_tokens:
        ...
        from love_engine.types.utils import AnthropicMessagesResponse
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.anthropic_response(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def count_tokens(
    ...
    """
    from love_engine.proxy.proxy_server import token_counter as internal_token_counter
    ...
    try:
        ...
        from love_engine.proxy._types import TokenCountRequest
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.anthropic_endpoints.count_tokens(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise HTTPException(
