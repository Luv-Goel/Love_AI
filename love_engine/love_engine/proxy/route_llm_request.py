...
# ``love_engine.types.router.MockRouterTestingParams`` by the test
...
# import time) avoids a cyclic import: ``love_engine.types.router`` imports
...
def _route_user_config_request(data: dict, route_type: str):
    ...
    valid_args = love_engine.Router.get_valid_args()
    filtered_config = {k: v for k, v in router_config.items() if k in valid_args}

    user_router = love_engine.Router(**filtered_config)
...
def _raise_if_model_fully_blocked(
    llm_router: LoveEngineRouter, model_name: Any, team_id: Optional[str]
) -> None:
    if not isinstance(model_name, str) or not model_name:
        return
    if not isinstance(llm_router, love_engine.Router):
        return
    deployments = (
        llm_router.get_model_list(model_name=model_name, team_id=team_id) or []
    )
    if llm_router._are_all_deployments_blocked(deployments):
        raise love_engine.PermissionDeniedError(
            ...
            response=httpx.Response(
                ...
                request=httpx.Request(
                    method="POST", url="https://github.com/BerriAI/love_engine"
                ),
            ),
        )
...
def get_team_id_from_data(data: dict) -> Optional[str]:
    """
    Get the team id from the data's metadata or love_engine_metadata params.
    """
    if (
        "metadata" in data
        ...
        and data["love_engine_metadata"] is not None
        and "user_api_key_team_id" in data["love_engine_metadata"]
    ):
        return data["love_engine_metadata"].get("user_api_key_team_id")
    return None
...
async def route_request(
    data: dict,
    llm_router: Optional[LoveEngineRouter],
    ...
    if "api_key" in data or "api_base" in data:
        if llm_router is not None:
            ...
            return getattr(love_engine, f"{route_type}")(**data)
        ...
        # Evals API: always route to love_engine directly (not through router)
            ...
            if model and llm_router:
                try:
                    ...
                    if not deployment_creds:
                        ...
                        if (
                            deployment
                            and deployment.love_engine_params
                            and not llm_router._is_deployment_blocked(deployment)
                        ):
                            deployment_creds = deployment.love_engine_params.model_dump(
                                exclude_none=True
                            )
        ...
        if team_model_name is not None:
            ...
            if llm_router.router_general_settings.pass_through_all_models:
                ...
                from love_engine.proxy.agent_endpoints.a2a_routing import (
