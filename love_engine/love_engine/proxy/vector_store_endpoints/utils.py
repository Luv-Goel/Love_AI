...
def _normalize_love_engine_params(
    ...
    if isinstance(love_engine_params, str):
        normalized = LOVE_ENGINE_ManagedVectorStore(**dict(vector_store))
        try:
            parsed = json.loads(love_engine_params)
            normalized["love_engine_params"] = parsed if isinstance(parsed, dict) else {}
        except (TypeError, ValueError):
            normalized["love_engine_params"] = {}
        return normalized
    return vector_store
    ...
    *,
    ...
    raise HTTPException(
        ...
        detail=(
            ...
            "Contact your LoveEngine administrator."
        ),
    )
...
async def can_user_access_vector_store(
    ...
    team_object_permission: Optional[LOVE_ENGINE_ObjectPermissionTable] = (
...
async def get_love_engine_managed_vector_store(
    vector_store_id: str,
) -> Optional[LOVE_ENGINE_ManagedVectorStore]:
    """
    Resolve a LoveEngine-managed vector store from the registry or shared cache.
    ...
    LoveEngine-managed multi-tenant stores.
    """
    ...
    if love_engine.vector_store_registry is not None:
        try:
            ...
            if vector_store is not None:
                return _normalize_love_engine_params(vector_store)
        except Exception as e:
    ...
    try:
        ...
        if not rows:
            return None
        return _normalize_love_engine_params(
            LOVE_ENGINE_ManagedVectorStore(**rows[0].model_dump())
        )
    except Exception as e:
        verbose_proxy_logger.warning(
...
async def assert_user_can_access_vector_store_id(
    ...
    """
    vector_store = await get_love_engine_managed_vector_store(
        vector_store_id=vector_store_id
    )
    if vector_store is not None:
