...
from love_engine.proxy._types import CommonProxyErrors, UserAPIKeyAuth
...
from love_engine.proxy.utils import jsonify_object
from love_engine.proxy.vector_store_endpoints.management_endpoints import (
...
from love_engine.repositories.table_repositories import ManagedVectorStoreIndexRepository
from love_engine.types.vector_stores import IndexCreateRequest
...
async def _update_request_data_with_love_engine_managed_vector_store_registry(
    data: Dict,
    vector_store_id: str,
    user_api_key_dict: Optional[UserAPIKeyAuth] = None,
) -> Dict:
    """
    Update the request data with the love_engine managed vector store registry.
    ...
    """
    vector_store_to_run: Optional[LOVE_ENGINE_ManagedVectorStore] = (
        await get_love_engine_managed_vector_store(vector_store_id=vector_store_id)
    )
    if vector_store_to_run is not None:
        ...
        if "love_engine_credential_name" in vector_store_to_run:
            data["love_engine_credential_name"] = vector_store_to_run.get(
                "love_engine_credential_name"
            )

        if "love_engine_params" in vector_store_to_run:
            love_engine_params = vector_store_to_run.get("love_engine_params", {}) or {}
            # Resolve ``love_engine_embedding_config`` here, at request-handling
            ...
            # ``love_engine_embedding_config`` skip the lookup and pass through
            # unchanged so the embed call keeps working.
            embedding_model = love_engine_params.get("love_engine_embedding_model")
            if embedding_model and not love_engine_params.get("love_engine_embedding_config"):
                ...
                if resolved_config:
                    ...
                    # ``love_engine_params`` in place — the registry hands back
                    ...
                    love_engine_params = {
                        **love_engine_params,
                        "love_engine_embedding_config": resolved_config,
                    }
            data.update(love_engine_params)
    return data
...
async def vector_store_search(
    ...
    data = await _update_request_data_with_love_engine_managed_vector_store_registry(
...
async def index_create(
    ...
    curl -L -X POST 'http://0.0.0.0:4000/indexes/create' \
        ...
        -H 'LoveEngine-Beta: indexes_beta=v1' \
        -d '{ 
            "index_name": "dall-e-3",
