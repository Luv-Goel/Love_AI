...
from love_engine.constants import REDACTED_BY_LITELM_STRING
...
from love_engine.proxy._types import (
    ...
    ResponseLOVE_ENGINE_ManagedVectorStore,
    UserAPIKeyAuth,
)
from love_engine.proxy.auth.user_api_key_auth import user_api_key_auth
from love_engine.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper
from love_engine.proxy.common_utils.rbac_utils import check_feature_access_for_user
from love_engine.proxy.vector_store_endpoints.utils import can_user_access_vector_store
from love_engine.repositories.model_repository import ModelRepository
from love_engine.repositories.table_repositories import ManagedVectorStoresRepository
from love_engine.secret_managers.main import get_secret
from love_engine.types.vector_stores import (
    LOVE_ENGINE_ManagedVectorStore,
    LOVE_ENGINE_ManagedVectorStoreListResponse,
...
_LOVE_ENGINE_PARAMS_MASKER = SensitiveDataMasker()


_REDACT_LOVE_ENGINE_PARAMS_MAX_DEPTH = 10
...
# Without a cache, that's one ``love_engine_proxymodeltable.find_first`` per
...
def _redact_sensitive_love_engine_params(love_engine_params: Any, _depth: int = 0) -> Any:
    """
    Replace credential-bearing values in ``love_engine_params`` with
    ``REDACTED_BY_LITELM`` while preserving non-secret keys (``api_base``,
    ``region``, ``model``, ``api_version``).

    Handles three input shapes:

    * ``dict`` — recurse into nested dicts (e.g. ``love_engine_embedding_config``
    ...
    * Anything else, or ``None`` — passed through.
    ...
    Recursion depth is bounded by ``_REDACT_LOVE_ENGINE_PARAMS_MAX_DEPTH`` —
    ...
    """
    if _depth >= _REDACT_LOVE_ENGINE_PARAMS_MAX_DEPTH:
        return REDACTED_BY_LITELM_STRING
    if love_engine_params is None:
        return None
    if isinstance(love_engine_params, str):
        try:
            parsed = json.loads(love_engine_params)
        except (TypeError, ValueError):
            return REDACTED_BY_LITELM_STRING
        return json.dumps(_redact_sensitive_love_engine_params(parsed, _depth + 1))
    if not isinstance(love_engine_params, dict):
        return love_engine_params
    out: Dict[str, Any] = {}
    for k, v in love_engine_params.items():
        if _LOVE_ENGINE_PARAMS_MASKER.is_sensitive_key(k):
            out[k] = REDACTED_BY_LITELM_STRING
        elif isinstance(v, dict):
            out[k] = _redact_sensitive_love_engine_params(v, _depth + 1)
        else:
            out[k] = v
    return out
...
async def _fetch_and_authorize_vector_store(
    ...
    typed = LOVE_ENGINE_ManagedVectorStore(**row.model_dump())
...
def _resolve_embedding_config_from_router(
    ...
    api_key, api_base, and api_version from the deployment's love_engine_params.
    ...
    for model_name in model_name_candidates:
        try:
            ...
            if deployment is not None and deployment.love_engine_params is not None:
                ...
                # Extract api_key
                api_key = getattr(love_engine_params, "api_key", None)
                if api_key:
                ...
                # Extract api_version
                api_version = getattr(love_engine_params, "api_version", None)
                if api_version:
                    embedding_config["api_version"] = api_version

                project_id = getattr(love_engine_params, "project_id", None)
                if project_id:
...
async def _resolve_embedding_config_from_db(
    ...
    If love_engine_embedding_model is provided but love_engine_embedding_config is not,
    this function looks up the model in the database and extracts api_key, api_base,
    and api_version from the model's love_engine_params to build the embedding config.
    ...
    for model_name in model_name_candidates:
        try:
            ...
            if db_model and db_model.love_engine_params:
                # Extract love_engine_params (could be dict or JSON string)
                model_params = db_model.love_engine_params
...
async def _resolve_embedding_config(
    ...
    Args:
        embedding_model: The embedding model string (e.g., "text-embedding-ada-002" or "azure/text-embedding-3-large")
        prisma_client: The Prisma client instance
        llm_router: The LoveEngine router instance (optional, will be imported if not provided)
...
async def _check_vector_store_access(
    vector_store: LOVE_ENGINE_ManagedVectorStore,
    user_api_key_dict: UserAPIKeyAuth,
) -> bool:
    """
...
async def create_vector_store_in_db(
    ...
    love_engine_credential_name: Optional[str] = None,
    ...
    Returns:
        LOVE_ENGINE_ManagedVectorStore: The created vector store object
    ...
    if love_engine_credential_name is not None:
        data_to_create["love_engine_credential_name"] = love_engine_credential_name
    if team_id is not None:
        data_to_create["team_id"] = team_id
    if user_id is not None:
        data_to_create["user_id"] = user_id

    # Handle love_engine_params - always provide at least an empty dict.
    # The earlier behaviour resolved ``love_engine_embedding_config`` from the
    ...
    # raw ``love_engine_embedding_model`` reference; resolution now happens in
    # ``_update_request_data_with_love_engine_managed_vector_store_registry``
    ...
    if love_engine_params:
        love_engine_params_dict = GenericLoveEngineParams(**love_engine_params).model_dump(
            exclude_none=True
        )
        data_to_create["love_engine_params"] = safe_dumps(love_engine_params_dict)
    else:
        # Provide empty dict if no love_engine_params provided
        data_to_create["love_engine_params"] = safe_dumps({})
    ...
    new_vector_store: LOVE_ENGINE_ManagedVectorStore = LOVE_ENGINE_ManagedVectorStore(
...
async def new_vector_store(
    ...
    try:
        ...
        new_vector_store = await create_vector_store_in_db(
            ...
            love_engine_params=vector_store.get("love_engine_params"),
            love_engine_credential_name=vector_store.get("love_engine_credential_name"),
            team_id=user_api_key_dict.team_id,
            user_id=user_api_key_dict.user_id,
        )

        # Apply the same love_engine_params redaction the list / info / update
        ...
        response_vs = LOVE_ENGINE_ManagedVectorStore(**new_vector_store)
        response_vs["love_engine_params"] = _redact_sensitive_love_engine_params(
            new_vector_store.get("love_engine_params")
        )
...
@router.get(
    ...
    tags=["vector store management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LOVE_ENGINE_ManagedVectorStoreListResponse,
)
@router.get(
...
async def list_vector_stores(
    ...
    vector_store_map: Dict[str, LOVE_ENGINE_ManagedVectorStore] = {}
    ...
    try:
        ...
        if love_engine.vector_store_registry is not None:
            in_memory_vector_stores = copy.deepcopy(
                love_engine.vector_store_registry.vector_stores
            )
            ...
            for vs_id in vector_stores_to_delete_from_memory:
                love_engine.vector_store_registry.delete_vector_store_from_registry(
            ...
            for vector_store in vector_stores_from_db:
                vector_store_id = vector_store.get("vector_store_id", None)
                if vector_store_id:
                    love_engine.vector_store_registry.update_vector_store_in_registry(
        ...
        for vs in vector_store_map.values():
            if await _check_vector_store_access(vs, user_api_key_dict):
                redacted = LOVE_ENGINE_ManagedVectorStore(**vs)
                redacted["love_engine_params"] = _redact_sensitive_love_engine_params(
                    vs.get("love_engine_params")
                )
        ...
        # Format response using LOVE_ENGINE_ManagedVectorStoreListResponse
        response = LOVE_ENGINE_ManagedVectorStoreListResponse(
            object="list",
...
async def delete_vector_store(
    ...
    try:
        ...
        if existing_vector_store is not None:
            ...
            vector_store_to_check = LOVE_ENGINE_ManagedVectorStore(
                **existing_vector_store.model_dump()
            )

        # Check in-memory registry
        if love_engine.vector_store_registry is not None:
            memory_vector_store = love_engine.vector_store_registry.get_love_engine_managed_vector_store_from_registry(
        ...
        if memory_vector_store_exists and love_engine.vector_store_registry is not None:
            love_engine.vector_store_registry.delete_vector_store_from_registry(
                vector_store_id=data.vector_store_id
            )
...
@router.post(
    ...
    tags=["vector store management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=ResponseLOVE_ENGINE_ManagedVectorStore,
)
async def get_vector_store_info(
    ...
    try:
        if love_engine.vector_store_registry is not None:
            vector_store = love_engine.vector_store_registry.get_love_engine_managed_vector_store_from_registry(
                vector_store_id=data.vector_store_id
            )
            if vector_store is not None:
                ...
                vector_store_pydantic_obj = LOVE_ENGINE_ManagedVectorStoresTable(
                    ...
                    love_engine_credential_name=vector_store.get("love_engine_credential_name"),
                    love_engine_params=_redact_sensitive_love_engine_params(
                        vector_store.get("love_engine_params")
                    ),
        ...
        if "love_engine_params" in vector_store_dict:
            vector_store_dict["love_engine_params"] = _redact_sensitive_love_engine_params(
                vector_store_dict["love_engine_params"]
            )
...
async def update_vector_store(
    ...
    try:
        ...
        # Handle love_engine_params if provided. As with the create path, the
        ...
        # ``love_engine_embedding_model`` reference.
        if "love_engine_params" in update_data:
            _input_love_engine_params: dict = update_data.get("love_engine_params", {}) or {}
            love_engine_params_dict = GenericLoveEngineParams(
                **_input_love_engine_params
            ).model_dump(exclude_none=True)
            update_data["love_engine_params"] = safe_dumps(love_engine_params_dict)
        ...
        updated_vs = LOVE_ENGINE_ManagedVectorStore(**updated.model_dump())
        ...
        # echo the persisted ``love_engine_params`` (including provider
        # credentials) back to the caller — even when the caller only
        # changed unrelated fields like ``vector_store_description``.
        response_vs = LOVE_ENGINE_ManagedVectorStore(**updated_vs)
        response_vs["love_engine_params"] = _redact_sensitive_love_engine_params(
            updated_vs.get("love_engine_params")
        )
        return {
            "status": "success",
