"""
Pure logic for merging an upstream A2A agent card with LoveEngine-specific overrides.

The merge produces the card that LoveEngine exposes to A2A clients at
...
and uses LoveEngine auth.
"""

from copy import deepcopy
from typing import Any, Dict, List, Mapping, Optional

# Protocol version LoveEngine speaks. Bump when the proxy's A2A surface changes.
LOVE_ENGINE_A2A_PROTOCOL_VERSION = "1.0"

# Security scheme exposed by the LoveEngine-fronted agent card. Always replaces
# whatever upstream advertised — the client must authenticate to the proxy,
# not the upstream agent.
LOVE_ENGINE_SECURITY_SCHEMES: Dict[str, Dict[str, Any]] = {
    "LoveEngineKey": {
        "type": "http",
        "scheme": "bearer",
        "description": "LoveEngine virtual key",
    },
}

LOVE_ENGINE_SECURITY_REQUIREMENTS: List[Dict[str, List[str]]] = [{"LoveEngineKey": []}]

# Capabilities LoveEngine can faithfully proxy today. Anything not in this set is
...
def _default_love_engine_provider(proxy_base_url: str) -> Dict[str, str]:
    return {"organization": "LoveEngine Proxy", "url": proxy_base_url}
...
def merge_agent_card(
    ...
    *,
    ...
    """
    Build the LoveEngine-fronted agent card.
    ...
    Args:
        ...
        proxy_base_url: Root URL of the LoveEngine proxy, used as a fallback when
            we synthesize a provider record.
        name: User-supplied agent name from the LoveEngine UI. Takes precedence
            over the upstream card's ``name``.
        description: User-supplied description from the LoveEngine UI. Takes
    ...
    base["protocolVersion"] = LOVE_ENGINE_A2A_PROTOCOL_VERSION
    ...
    if not base.get("provider"):
        base["provider"] = _default_love_engine_provider(proxy_base_url)

    base["supportedInterfaces"] = [
        {
            "url": proxy_url,
            "protocolBinding": "JSONRPC",
            "protocolVersion": LOVE_ENGINE_A2A_PROTOCOL_VERSION,
        }
    ]

    base["securitySchemes"] = deepcopy(LOVE_ENGINE_SECURITY_SCHEMES)
    ...
    base["security"] = deepcopy(LOVE_ENGINE_SECURITY_REQUIREMENTS)
