...
from love_engine.love_engine_core_utils.sensitive_data_masker import mask_sensitive_keys
...
# Extension point: packages outside OSS (e.g. love_engine_enterprise) can
...
# client gets a clear signal that the feature requires LoveEngine Enterprise.
...
async def _get_settings_with_schema(
    ...
    Args:
        settings_key: The key in love_engine_settings to get
        settings_class: The Pydantic class to use for schema
        config: The config dictionary
    """
    from pydantic import TypeAdapter

    love_engine_settings = config.get("love_engine_settings", {}) or {}
    settings_data = love_engine_settings.get(settings_key, {}) or {}
...
async def get_internal_user_settings():
    """
    Get all SSO settings from the love_engine_settings configuration.
...
async def update_default_team_member_budget(
    ...
    from love_engine.proxy.management_endpoints.team_endpoints import update_team
...
async def _update_love_engine_setting(
    settings: Union[
    ...
    """
    Common utility function to update `love_engine_settings` in both memory and config.

    Args:
        settings: The settings object to update
        settings_key: The key in love_engine_settings to update
    ...
    # because get_config() may overwrite love_engine.<key> with stale DB values
    # via LOVE_ENGINE_SETTINGS_SAFE_DB_OVERRIDES.
    config = await proxy_config.get_config()

    # Update the in-memory settings (after get_config to avoid stale override)
    setattr(love_engine, settings_key, in_memory_var)

    # Update config with new settings
    if "love_engine_settings" not in config:
        config["love_engine_settings"] = {}

    config["love_engine_settings"][settings_key] = in_memory_var
...
async def update_internal_user_settings(
    ...
    return await _update_love_engine_setting(
        settings=settings,
        settings_key="default_internal_user_params",
...
async def get_sso_settings():
    ...
    from love_engine.proxy.proxy_server import prisma_client, proxy_config
...
async def get_ui_theme_settings():
    """
    Get UI theme configuration from the love_engine_settings.
...
async def update_ui_theme_settings(theme_config: UIThemeConfig):
    ...
    # Store UI theme config in love_engine_settings (where it's retrieved from)
    if "love_engine_settings" not in config:
        config["love_engine_settings"] = {}
    config["love_engine_settings"]["ui_theme_config"] = theme_data
    ...
    # Update LOVE_ENGINE_FAVICON_URL environment variable if favicon_url is provided
    ...
    if (
        ...
        config["environment_variables"]["LOVE_ENGINE_FAVICON_URL"] = favicon_url
        os.environ["LOVE_ENGINE_FAVICON_URL"] = favicon_url
        verbose_proxy_logger.debug(f"Set LOVE_ENGINE_FAVICON_URL to: {favicon_url}")
    else:
        # Remove the environment variable to restore default favicon
        if "LOVE_ENGINE_FAVICON_URL" in config.get("environment_variables", {}):
            del config["environment_variables"]["LOVE_ENGINE_FAVICON_URL"]
            verbose_proxy_logger.debug("Removed LOVE_ENGINE_FAVICON_URL from config")
        if "LOVE_ENGINE_FAVICON_URL" in os.environ:
            del os.environ["LOVE_ENGINE_FAVICON_URL"]
            verbose_proxy_logger.debug("Removed LOVE_ENGINE_FAVICON_URL from environment")
...
async def update_mcp_semantic_filter_settings(
    ...
    """
    result = await _update_love_engine_setting(
        settings=settings,
        settings_key="mcp_semantic_tool_filter",
...
async def get_ui_settings():
    ...
    config: Dict[str, Any] = {"love_engine_settings": {"ui_settings": ui_settings}}
...
async def update_ui_settings(
    ...
    """
    from love_engine.proxy.proxy_server import prisma_client, store_model_in_db
    ...
    if blocked_enterprise_keys:
        raise HTTPException(
            ...
            detail={
                "error": (
                    f"Setting(s) {blocked_enterprise_keys} are a LoveEngine "
