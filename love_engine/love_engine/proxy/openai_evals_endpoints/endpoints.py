...
async def create_eval(
    ...
    - Pass model via header: `x-love_engine-model: gpt-4-account-1`
...
async def create_run(
    ...
    model = (
        request.headers.get("x-love_engine-model")
        or request.query_params.get("model")
        or data.get("model")
        or (
...
async def list_runs(
    ...
    model = request.headers.get("x-love_engine-model") or request.query_params.get("model")
    if model:
        data["model"] = model
