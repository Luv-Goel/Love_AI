...
from love_engine.proxy._types import ProviderBudgetResponse, ProviderBudgetResponseObject
...
from love_engine.proxy.spend_tracking.spend_tracking_utils import (
    get_spend_by_team_and_customer,
)
from love_engine.proxy.utils import handle_exception_on_proxy
from love_engine.repositories.table_repositories import SpendLogsRepository
...
@router.get(
    ...
    dependencies=[Depends(user_api_key_auth)],
    responses={
        200: {"model": List[LOVE_ENGINE_SpendLogs]},
    },
)
async def view_spend_tags(
    ...
    """
    LoveEngine Enterprise - View Spend Per Request Tag
...
async def get_global_activity(
    ...
    try:
        ...
        if (
            user_api_key_dict.user_role == LoveEngineUserRoles.INTERNAL_USER
            or user_api_key_dict.user_role == LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY
        ):
...
async def get_global_spend_provider(
    ...
    from love_engine.proxy.proxy_server import llm_router, prisma_client
        ...
        for row in db_response:
            ...
            if llm_router is not None:
                ...
                if _deployment is not None:
                    try:
                        _, _provider, _, _ = love_engine.get_llm_provider(
                            model=_deployment.love_engine_params.model,
                            custom_llm_provider=_deployment.love_engine_params.custom_llm_provider,
                            api_base=_deployment.love_engine_params.api_base,
                            love_engine_params=_deployment.love_engine_params,
                        )
...
async def global_view_spend_tags(
    ...
    """
    LoveEngine Enterprise - View Spend Per Request Tag. Used by LoveEngine UI
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "LOVE_ENGINE_SpendLogs" s
        LEFT JOIN
            "LOVE_ENGINE_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    Note: If you see a spend of $0.0 you need to set custom_pricing for your model: https://docs.love_engine.ai/docs/proxy/custom_pricing
    ...
    """
    try:
        from love_engine import completion_cost
        from love_engine.cost_calculator import CostPerToken
        ...
        if request.model is not None:
            ...
            1. user passes model, and model is defined on love_engine config.yaml or in DB. use info on config or in DB in this case
            2. user passes model, and model is not defined on love_engine config.yaml or in DB. Pass model as is to love_engine.completion_cost
            3. user passes completion_response
            
            """
            if _model_in_llm_router is not None:
                _love_engine_params = _model_in_llm_router.get("love_engine_params")
                _love_engine_model_name = _love_engine_params.get("model")
                input_cost_per_token = _love_engine_params.get("input_cost_per_token")
                output_cost_per_token = _love_engine_params.get("output_cost_per_token")
                if (
            ...
            _completion_response = love_engine.ModelResponse(**request.completion_response)
...
async def ui_view_spend_logs(
    ...
    model_id: Optional[str] = fastapi.Query(
        default=None,
        description="Filter logs by model ID (love_engine model deployment id)",
    ),
...
async def ui_view_request_response_for_request_id(
    ...
    custom_loggers = (
        love_engine.logging_callback_manager.get_active_additional_logging_utils_from_custom_logger()
    )
...
async def global_spend_reset():
    ...
    Globally reset spend for All API Keys and Teams, maintain LOVE_ENGINE_SpendLogs

    1. LOVE_ENGINE_SpendLogs will maintain the logs on spend, no data gets deleted from there
    2. LOVE_ENGINE_VerificationTokens spend will be set = 0
    3. LOVE_ENGINE_TeamTable spend will be set = 0
...
async def global_spend_logs(
    ...
    from love_engine.integrations.prometheus_helpers.prometheus_api import (
...
async def global_spend():
    ...
    try:
        ...
        return {"spend": total_spend, "max_budget": love_engine.max_budget}
    except Exception as e:
        error_trace = traceback.format_exc()
        error_str = str(e) + "\n" + error_trace
        if isinstance(e, HTTPException):
...
async def global_spend_key_internal_user(
    ...
    sql_query = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "LOVE_ENGINE_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query = """
    SELECT DISTINCT end_user FROM "LOVE_ENGINE_SpendLogs"
    """
...
async def provider_budgets() -> ProviderBudgetResponse:
    """
    Provider Budget Routing - Get Budget, Spend Details https://docs.love_engine.ai/docs/proxy/provider_budget_routing
    ...
    try:
        ...
        if provider_budget_config is None:
            raise ValueError(
                "No provider budget config found. Please set a provider budget config in the router settings. https://docs.love_engine.ai/docs/proxy/provider_budget_routing"
            )
...
async def ui_view_session_spend_logs(
    ...
    Returns:
        {
            "data": List[LOVE_ENGINE_SpendLogs],
            "total": int,
            "page": int,
            "page_size": int,
            "total_pages": int,
