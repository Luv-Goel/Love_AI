...
from love_engine.types.proxy.vantage_endpoints import (
...
def _get_registered_vantage_logger():
    """Return the VantageLogger already registered in love_engine.callbacks, if any."""
    from love_engine.integrations.vantage.vantage_logger import VantageLogger

    vantage_loggers = love_engine.logging_callback_manager.get_custom_loggers_for_type(
...
async def vantage_dry_run_export(
    ...
    try:
        ...
        from love_engine.integrations.focus.database import FocusLoveEngineDatabase
        from love_engine.integrations.focus.export_engine import FocusExportEngine
        from love_engine.integrations.focus.transformer import FocusTransformer

        database = FocusLoveEngineDatabase()
...
async def delete_vantage_settings(
    ...
    try:
        ...
        love_engine.logging_callback_manager.remove_callbacks_by_type(
            love_engine.callbacks, VantageLogger
        )
