...
from love_engine.proxy.auth.auth_utils import get_model_from_request
...
async def _get_end_user_budget_counter(
    ...
    if end_user_object is not None:
        ...
        if max_budget is None:
            budget_table = _get_value(end_user_object, "love_engine_budget_table")
...
async def _get_tag_budget_counters(
    ...
    from love_engine.proxy.auth.auth_checks import get_tag_objects_batch
    ...
    for tag_name in tag_names:
        ...
        if tag_object is None:
            continue
        budget_table = _get_value(tag_object, "love_engine_budget_table")
        max_budget = _to_float(_get_value(budget_table, "max_budget"))
        if max_budget is None or max_budget <= 0:
            continue
        counters.append(
...
async def _get_team_member_budget_counter(
    ...
    team_membership: Optional[LOVE_ENGINE_TeamMembership] = None
    if isinstance(cached_team_membership, LOVE_ENGINE_TeamMembership):
        team_membership = cached_team_membership
    elif isinstance(cached_team_membership, dict):
        team_membership = LOVE_ENGINE_TeamMembership(**cached_team_membership)

    team_member_budget: Optional[float] = None
    if team_membership is not None and team_membership.love_engine_budget_table is not None:
    ...
    team_member_spend = (
        cast(LOVE_ENGINE_TeamMembership, team_membership).spend
...
async def _get_org_budget_counter(
    ...
    org_budget_table = _get_value(org_table, "love_engine_budget_table")
...
def _get_model_cost_info(
    ...
    try:
        return dict(love_engine.get_model_info(model=model))
    except Exception:
        return None
...
def _estimate_input_tokens(
    ...
    try:
        if "messages" in request_body:
            return love_engine.token_counter(
                model=model,
...
def _count_text_tokens(model: str, text: Any) -> int:
    ...
    while stack:
        ...
        if isinstance(item, dict):
            token_count += love_engine.token_counter(model=model, text=json.dumps(item))
            continue
        token_count += love_engine.token_counter(model=model, text=str(item))
    return token_count
