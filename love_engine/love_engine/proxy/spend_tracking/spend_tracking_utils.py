...
from love_engine.constants import (
    LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD,
...
from love_engine.love_engine_core_utils.safe_json_dumps import safe_dumps, strip_null_bytes
from love_engine.proxy._types import SpendLogsMetadata, SpendLogsPayload
from love_engine.proxy.spend_tracking.spend_log_error_logger import spend_log_error
from love_engine.proxy.utils import PrismaClient, hash_token
...
def _get_spend_logs_metadata(
    ...
    love_engine_overhead_time_ms: Optional[float] = None,
    ...
    clean_metadata["love_engine_overhead_time_ms"] = love_engine_overhead_time_ms
...
def get_spend_logs_id(
    ...
    if call_type == "aretrieve_batch" or call_type == "acreate_file":
        ...
        id = cast(Optional[str], response_obj.get("id")) or cast(
            Optional[str], kwargs.get("love_engine_call_id")
        )
    return id
...
def get_logging_payload(kwargs, response_obj, start_time, end_time) -> SpendLogsPayload:
    ...
    metadata = get_love_engine_metadata_from_kwargs(kwargs)
    ...
    if call_type in ["ocr", "aocr"]:
        ...
        if isinstance(_usage, love_engine.Usage):
            usage = dict(_usage)
    ...
    if standard_logging_payload is not None:
        ...
        love_engine_overhead_time_ms = hidden_params.get("love_engine_overhead_time_ms")
    ...
    clean_metadata = _get_spend_logs_metadata(
        ...
        love_engine_overhead_time_ms=love_engine_overhead_time_ms,
        cost_breakdown=(
    ...
    if love_engine.cache is not None:
        cache_key = love_engine.cache.get_cache_key(**kwargs)
    else:
        cache_key = "Cache OFF"
    ...
    try:
        payload: SpendLogsPayload = SpendLogsPayload(
            ...
            proxy_server_request=_get_proxy_server_request_for_spend_logs_payload(
                metadata=metadata, love_engine_params=love_engine_params, kwargs=kwargs
            ),
...
def _get_session_id_for_spend_log(
    ...
    # Users can dynamically set the trace_id for each request by passing `love_engine_trace_id` in kwargs
    if kwargs.get("love_engine_trace_id") is not None:
        return str(kwargs.get("love_engine_trace_id"))
    ...
    def _sanitize_value(value: Any) -> Any:
        if isinstance(value, dict):
            ...
            if len(value) > max_string_length_prompt_in_db:
                ...
                truncated_value = (
                    f"{value[:start_chars]}"
                    f"... ({LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD} skipped {skipped_chars} chars. "
                    f"{LOVE_ENGINE_TRUNCATION_DB_SAFEGUARD_NOTE}) ..."
                    f"{value[-end_chars:]}"
                )
...
def _sanitize_error_information_for_spend_logs(
    ...
    """
    Sanitize ``error_information`` before it lands in ``LOVE_ENGINE_SpendLogs.metadata``.
    ...
    the untruncated error per ``LOVE_ENGINE_TRUNCATION_DB_SAFEGUARD_NOTE``.
    """
...
def _get_proxy_server_request_for_spend_logs_payload(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        _proxy_server_request = cast(
            Optional[dict], love_engine_params.get("proxy_server_request", {})
        )
        if _proxy_server_request is not None:
            ...
            if LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD in _request_body_json_str:
...
def _get_response_for_spend_logs_payload(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        ...
        if LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD in result_str:
            verbose_proxy_logger.info(
