...
from love_engine.types.utils import LoveEngineFineTuningJob

router = APIRouter()

from love_engine.types.llms.openai import LoveEngineFineTuningJobCreate
        ...
        if isinstance(element, dict):
            for key, value in element.items():
                if isinstance(value, str) and value.startswith("os.environ/"):
                    element[key] = love_engine.get_secret(value)
...
async def create_fine_tuning_job(
    ...
    fine_tuning_request: LoveEngineFineTuningJobCreate,
    ...
    try:
        ...
        response: Optional[LoveEngineFineTuningJob] = None
        if training_file:
        ...
        if unified_file_id:
            ...
            response = cast(
                LoveEngineFineTuningJob, await llm_router.acreate_fine_tuning_job(**data)
            )
        ...
        ## ELSE, Route based on custom_llm_provider
            ...
            response = await love_engine.acreate_fine_tuning_job(**data)

        if response is None:
            raise ValueError(
                "Invalid request, No love_engine managed file id or custom_llm_provider provided."
            )
        ...
        if _response is not None and isinstance(_response, LoveEngineFineTuningJob):
            response = _response
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.create_fine_tuning_job(): Exception occurred - {}".format(
                str(e)
            )
        )
...
async def retrieve_fine_tuning_job(
    ...
    - `custom_llm_provider`: Name of the LoveEngine provider
    ...
    try:
        ...
        if unified_finetuning_job_id:
            ...
            response = await love_engine.aretrieve_fine_tuning_job(
                **data,
            )
        ...
        if response is None:
            raise HTTPException(
                status_code=400,
                detail="Invalid request, No love_engine managed file id or custom_llm_provider provided.",
            )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.retrieve_fine_tuning_job(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def list_fine_tuning_jobs(
    ...
    try:
        ...
        if target_model_names and isinstance(target_model_names, str):
            ...
            response = await love_engine.alist_fine_tuning_jobs(
                **data,
                after=after,
                limit=limit,
            )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.list_fine_tuning_jobs(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def cancel_fine_tuning_job(
    ...
    try:
        ...
        if unified_finetuning_job_id:
            ...
            response = await love_engine.acancel_fine_tuning_job(
                **data,
            )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.cancel_fine_tuning_job(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
