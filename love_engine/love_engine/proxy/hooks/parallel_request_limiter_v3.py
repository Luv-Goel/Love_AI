...
from love_engine import DualCache
from love_engine._logging import verbose_proxy_logger
from love_engine.constants import DYNAMIC_RATE_LIMIT_ERROR_THRESHOLD_PER_MINUTE
...
from love_engine.proxy.auth.auth_utils import get_model_rate_limit_from_metadata
from love_engine.proxy.common_utils.proxy_rate_limit_error import (
    ProxyRateLimitError,
    map_v3_rate_limit_type,
)
from love_engine.proxy.hooks.rate_limiter_utils import resolve_llm_provider_for_rate_limit
from love_engine.types.caching import RedisPipelineIncrementOperation
from love_engine.types.llms.openai import BaseLoveEngineOpenAIResponseObject
...
if TYPE_CHECKING:
    ...
    from love_engine.proxy.utils import InternalUsageCache as _InternalUsageCache
    from love_engine.types.caching import RedisPipelineIncrementOperation
...
TPM_RESERVED_TOKENS_KEY = "_love_engine_tpm_reserved_tokens"
...
TPM_RESERVED_MODEL_KEY = "_love_engine_tpm_reserved_model"
...
TPM_RESERVED_SCOPES_KEY = "_love_engine_tpm_reserved_scopes"
...
TPM_RESERVATION_RELEASED_KEY = "_love_engine_tpm_reservation_released"
RATE_LIMIT_DESCRIPTORS_KEY = "_love_engine_rate_limit_descriptors"
...
_LOVE_ENGINE_STASH_KEYS: Tuple[str, ...] = (
...
class _PROXY_MaxParallelRequestsHandler_v3(CustomLogger):
    def __init__(
        ...
        self.window_size = int(os.getenv("LOVE_ENGINE_RATE_LIMIT_WINDOW_SIZE", 60))
        ...
        self.tpm_reservation_enabled = (
            os.getenv("LOVE_ENGINE_TPM_TOKEN_RESERVATION_ENABLED", "true").lower() == "true"
        )
    ...
    def _get_batch_rate_limiter(self) -> Optional[Any]:
        ...
        if self._batch_rate_limiter is None:
            try:
                from love_engine.proxy.hooks.batch_rate_limiter import (
    ...
    def _add_mcp_per_key_rate_limit_descriptor(
        ...
        """
        from love_engine.proxy.auth.auth_utils import get_key_mcp_rpm_limit
    ...
    def _add_mcp_per_team_rate_limit_descriptor(
        ...
        configured for the server being called.
        """
        from love_engine.proxy.auth.auth_utils import get_team_mcp_rpm_limit
    ...
    def _get_session_id_from_data(self, data: dict) -> Optional[str]:
        """Extract session_id from request metadata or love_engine_session_id."""
        session_id = data.get("love_engine_session_id")
        if session_id:
        ...
        session_id = love_engine_metadata.get("session_id")
        if session_id:
            return str(session_id)
        return None
    ...
    async def async_pre_call_hook(
        ...
        # a client can inject ``_love_engine_rate_limit_descriptors`` /
        # ``_love_engine_tpm_reserved_tokens`` in body ``metadata`` and have
        ...
        if descriptors:
            ...
            if response["overall_code"] == "OVER_LIMIT":
                ...
                data["love_engine_proxy_rate_limit_response"] = response
            ...
            if has_tpm_limits and self.tpm_reservation_enabled:
                ...
                if tpm_response["overall_code"] == "OVER_LIMIT":
                    ...
                    stored_response = data.get("love_engine_proxy_rate_limit_response")
                    if isinstance(stored_response, dict):
                        ...
                        data["love_engine_proxy_rate_limit_response"] = tpm_response
    ...
    def _strip_stash_keys_from_top_level(data: Any) -> None:
        if not isinstance(data, dict):
            return
        for stash_key in _LOVE_ENGINE_STASH_KEYS:
    ...
    def _strip_stash_keys_from_all_channels(cls, data: Any) -> None:
        ...
        for channel in ("metadata", "love_engine_metadata"):
    ...
    def _get_total_tokens_from_usage(
        ...
        rate limits. This aligns LoveEngine's TPM calculation with provider behavior.
        """
        total_tokens = 0
        cached_tokens = 0
    ...
    def _stash_value_in_metadata_channels(
        ...
        for channel in ("metadata", "love_engine_metadata"):
            ...
            if isinstance(existing, dict):
                ...
                # ``love_engine_metadata`` is owned by the router; don't conjure
                # it here.
    ...
    def _lookup_stashed_value(
        ...
        if isinstance(kwargs, dict):
            ...
            if isinstance(love_engine_params, dict):
                lp_metadata = love_engine_params.get("metadata")
    ...
    def _mark_reservation_released(data: Any) -> None:
        ...
        ``request_data["metadata"]`` and ``kwargs["love_engine_params"]["metadata"]``
        ...
        love_engine_params = data.get("love_engine_params")
        if isinstance(love_engine_params, dict):
            lp_metadata = love_engine_params.get("metadata")
            if isinstance(lp_metadata, dict):
    ...
    def _build_success_event_pipeline_operations(
        ...
        # 'metadata' and 'love_engine_metadata' fields from love_engine_params
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        love_engine_parent_otel_span: Union[Span, None] = _get_parent_otel_span_from_kwargs(
            kwargs
        )
        try:
            verbose_proxy_logger.debug(
    ...
    async def async_post_call_success_hook(
        ...
        try:
            ...
            love_engine_proxy_rate_limit_response = cast(
                Optional[RateLimitResponse],
                data.get("love_engine_proxy_rate_limit_response", None),
            )

            if love_engine_proxy_rate_limit_response is not None:
                ...
                if _hidden_params is not None and (
                    ...
                    for status in love_engine_proxy_rate_limit_response["statuses"]:
    ...
    async def async_post_call_failure_hook(
        ...
        reservations are stranded — async_log_failure_event is a love_engine
        ...
        try:
            ...
            if ops:
                ...
                await self.internal_usage_cache.dual_cache.async_increment_cache_pipeline(
                    increment_list=ops,
                    love_engine_parent_otel_span=user_api_key_dict.parent_otel_span,
                )
