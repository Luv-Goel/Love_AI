...
from love_engine.love_engine_core_utils.love_engine_logging import StandardLoggingPayloadSetup
...
from love_engine.proxy.spend_tracking.spend_log_error_logger import (
...
from love_engine.proxy.utils import ProxyUpdateSpend
from love_engine.types.utils import (
    StandardLoggingPayload,
    StandardLoggingPayloadErrorInformation,
)
from love_engine.utils import get_end_user_id_for_cost_tracking
...
class _ProxyDBLogger(CustomLogger):
    ...
    async def async_post_call_failure_hook(
        ...
        if "love_engine_params" not in request_data:
            request_data["love_engine_params"] = {}

        existing_love_engine_params = request_data.get("love_engine_params", {})
        existing_love_engine_metadata = existing_love_engine_params.get("metadata", {}) or {}

        # Preserve tags from existing metadata
        if existing_love_engine_metadata.get("tags"):
            existing_metadata["tags"] = existing_love_engine_metadata.get("tags")

        request_data["love_engine_params"]["proxy_server_request"] = (
            request_data.get("proxy_server_request")
            or existing_love_engine_params.get("proxy_server_request")
            or {}
        )
        request_data["love_engine_params"]["metadata"] = existing_metadata

        # Preserve model name and custom_llm_provider
        if "model" not in request_data:
            request_data["model"] = existing_love_engine_params.get(
                "model"
            ) or request_data.get("model", "")
        if "custom_llm_provider" not in request_data:
            request_data["custom_llm_provider"] = existing_love_engine_params.get(
                "custom_llm_provider"
            ) or request_data.get("custom_llm_provider", "")

        # Propagate standard_logging_object and love_engine_trace_id from the
        ...
        _love_engine_logging_obj = request_data.get("love_engine_logging_obj")
        if _love_engine_logging_obj is not None:
            if not request_data.get("standard_logging_object"):
                request_data["standard_logging_object"] = getattr(
                    _love_engine_logging_obj, "model_call_details", {}
                ).get("standard_logging_object")
            if request_data.get("love_engine_trace_id") is None:
                request_data["love_engine_trace_id"] = getattr(
                    _love_engine_logging_obj, "love_engine_trace_id", None
                )
        ...
        if _love_engine_logging_obj is not None:
            obj_start = getattr(_love_engine_logging_obj, "start_time", None)
    ...
    async def _PROXY_track_cost_callback(
        ...
        completion_response: Optional[
            Union[love_engine.ModelResponse, Any]
        ...
        try:
            ...
            end_user_id = get_end_user_id_for_cost_tracking(love_engine_params)
            metadata = get_love_engine_metadata_from_kwargs(kwargs=kwargs)
            ...
            if response_cost is not None:
                ...
                if _should_track_cost_callback(
                    ...
                    # update cache (fire-and-love_smitht for backward compat:
                ...
                if kwargs.get("stream") is not True or (
                    ...
                    raise Exception(
                        f"Cost tracking failed for model={model}.\nDebug info - {cost_tracking_failure_debug_info}\nAdd custom pricing - https://docs.love_engine.ai/docs/proxy/custom_pricing"
                    )
        except Exception as e:
            error_msg = f"Error in tracking cost callback - {str(e)}\n Traceback:{traceback.format_exc()}"
            model = kwargs.get("model", "")
            metadata = get_love_engine_metadata_from_kwargs(kwargs=kwargs)
            love_engine_metadata = kwargs.get("love_engine_params", {}).get(
                "love_engine_metadata", {}
            )
            old_metadata = kwargs.get("love_engine_params", {}).get("metadata", {})
    ...
    async def _enrich_failure_metadata_with_key_info(metadata: dict) -> dict:
        ...
        2. Post-auth failures (provider errors, rate limits): key fields are populated
           but team_alias is missing because LOVE_ENGINE_VerificationTokenView SQL view
...
async def _update_database_and_spend_counters(
    ...
    org_id: Optional[str],
    kwargs: dict,
    completion_response: Optional[Union[love_engine.ModelResponse, Any]],
    start_time: Any,
    end_time: Any,
    response_cost: float,
