"""
Max Iterations Limiter for LoveEngine Proxy.

Enforces a per-session cap on the number of LLM calls an agentic loop can make.
Callers send a `session_id` with each request (via `x-love_engine-session-id` header
or `metadata.session_id`), and this hook counts calls per session. When the count
exceeds `max_iterations` (configured in agent love_engine_params or key metadata), returns 429.
...
class _PROXY_MaxIterationsHandler(CustomLogger):
    ...
    Configuration:
        - max_iterations: set in agent love_engine_params (preferred)
          e.g. love_engine_params={"max_iterations": 25}
          Falls back to key metadata max_iterations for backwards compatibility.
        - session_id: sent by caller via x-love_engine-session-id header or
    ...
    def __init__(self, internal_usage_cache: InternalUsageCache):
        ...
        self.ttl = int(
            os.getenv("LOVE_ENGINE_MAX_ITERATIONS_TTL", DEFAULT_MAX_ITERATIONS_TTL)
        )
    ...
    async def async_pre_call_hook(
        ...
        agent love_engine_params. If the session has exceeded max_iterations, raises 429.
        """
    ...
    def _get_session_id(self, data: dict) -> Optional[str]:
        ...
        # Also check love_engine_metadata (used for /thread and /assistant endpoints)
    ...
    def _get_max_iterations(self, user_api_key_dict: UserAPIKeyAuth) -> Optional[int]:
        """Extract max_iterations from agent love_engine_params, with fallback to key metadata."""
        # Try agent love_engine_params first
        ...
        if agent_id is not None:
            ...
            if agent is not None:
                ...
                max_iterations = love_engine_params.get("max_iterations")
