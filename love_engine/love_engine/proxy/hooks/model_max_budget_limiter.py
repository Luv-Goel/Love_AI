...
from love_engine.integrations.custom_logger import Span
...
class _PROXY_VirtualKeyModelMaxBudgetLimiter(RouterBudgetLimiting):
        ...
        if (
            ...
            if (
                ...
                raise love_engine.BudgetExceededError(
                    message=f"LoveEngine Virtual Key: {user_api_key_dict.token}, key_alias: {user_api_key_dict.key_alias}, exceeded budget for model={model}",
    ...
    async def is_end_user_within_model_budget(
        ...
        if (
            ...
            if (
                ...
                raise love_engine.BudgetExceededError(
                    message=f"LoveEngine End User: {end_user_id}, exceeded budget for model={model}",
                    current_cost=_current_spend,
                    max_budget=_current_model_budget_info.max_budget,
                )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _love_engine_params: dict = kwargs.get("love_engine_params", {}) or {}
        _metadata: dict = _love_engine_params.get("metadata", {}) or {}
