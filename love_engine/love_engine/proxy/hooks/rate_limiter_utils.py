...
from love_engine.types.utils import PriorityReservationDict

PROXY_LLM_PROVIDER_FALLBACK = "love_engine_proxy"
...
def resolve_llm_provider_for_rate_limit(
    ...
    :func:`love_engine.get_llm_provider` is invoked anywhere else in the request
    lifecycle — so the raised 429 would otherwise have an empty
    ``llm_provider`` field, making the resulting Prometheus
    ``love_engine_proxy_failed_requests_metric`` show up with
    ``exception_class="RateLimitError"`` and no provider attribution.

    Resolution order:

    1. ``love_engine.get_llm_provider(model)`` — covers raw provider/model
    ...
    2. **Router alias fallback** — nearly every real proxy deployment
       ...
       ``"tpm-locked"`` → ``love_engine_params.model: openai/gpt-4o-mini``).
       ``get_llm_provider`` doesn't know router aliases, so without this
       step every alias call ended up labeled ``"love_engine_proxy"``,
       defeating the field's purpose for the most common case.
    3. Defensive fallback to ``("", "love_engine_proxy")`` — used only when
    ...
    try:
        resolved_model, custom_llm_provider, _, _ = love_engine.get_llm_provider(
            model=model,
        )
        return (
...
def _resolve_provider_from_router_alias(
    ...
    registered, the deployment has no usable ``love_engine_params.model``, or
    any underlying lookup raises. Callers fall through to the defensive
    ``love_engine_proxy`` fallback in that case — never raising secondary
    ...
    try:
        ...
        for deployment in model_list:
            ...
            if deployment.get("model_name") != model:
                continue
            params = deployment.get("love_engine_params")
