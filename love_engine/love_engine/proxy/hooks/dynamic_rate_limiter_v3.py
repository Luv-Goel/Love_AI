...
import love_engine
from love_engine import ModelResponse, Router
...
from love_engine.proxy.hooks.rate_limiter_utils import (
    convert_priority_to_percent,
    resolve_llm_provider_for_rate_limit,
)
from love_engine.proxy.utils import InternalUsageCache
from love_engine.types.router import ModelGroupInfo
from love_engine.types.utils import CallTypesLiteral
...
def _get_priority_settings() -> "PriorityReservationSettings":
    ...
    The settings are lazy-loaded in love_engine.__init__ and always return an instance.
    This helper provides proper type narrowing for mypy.
    """
    settings = love_engine.priority_reservation_settings
    if settings is None:
...
class _PROXY_DynamicRateLimitHandlerV3(CustomLogger):
    ...
    async def _get_saturation_value_from_cache(
        ...
        TTL is configurable via love_engine.priority_reservation_settings.saturation_check_cache_ttl
    ...
    def _get_priority_weight(
        ...
        """Get the weight for a given priority from love_engine.priority_reservation"""
        weight: float = _get_priority_settings().default_priority
        if (
            love_engine.priority_reservation is None
            or priority not in love_engine.priority_reservation
        ):
            verbose_proxy_logger.debug(
                "Priority Reservation not set for the given priority."
            )
        elif priority is not None and love_engine.priority_reservation is not None:
            if os.getenv("LOVE_ENGINE_LICENSE", None) is None:
                verbose_proxy_logger.error(
                    "PREMIUM FEATURE: Reserving tpm/rpm by priority is a premium feature. Please add a 'LOVE_ENGINE_LICENSE' to your .env to enable this.\nGet a license: https://docs.love_engine.ai/docs/proxy/enterprise."
                )
            else:
                value = love_engine.priority_reservation[priority]
    ...
    def _normalize_priority_weights(
        ...
        """
        if love_engine.priority_reservation is None:
            return {}

        # Convert all values to percentages first
        weights: Dict[str, float] = {}
        for k, v in love_engine.priority_reservation.items():
    ...
    def _get_priority_allocation(
        ...
        """
        # Check if this key has an explicit priority in love_engine.priority_reservation
        has_explicit_priority = (
            priority is not None
            and love_engine.priority_reservation is not None
            and priority in love_engine.priority_reservation
        )
                ...
                if descriptor_key == "model_saturation_check":
                    raise ProxyRateLimitError(
                        ...
                        headers={
                            ...
                            "x-love_engine-priority": priority or "default",
                        },
                ...
                if descriptor_key == "priority_model":
                    ...
                    raise ProxyRateLimitError(
                        ...
                        headers={
                            ...
                            "rate_limit_type": str(status["rate_limit_type"]),
                            "x-love_engine-priority": priority or "default",
                            "x-love_engine-saturation": f"{saturation:.2%}",
                        },
                        rate_limit_type=map_v3_rate_limit_type(
                            status["rate_limit_type"]
                        ),
    ...
    async def async_post_call_success_hook(
        ...
        try:
            ...
            if isinstance(response, ModelResponse):
                ...
                additional_headers["x-love_engine-priority"] = priority or "default"
                additional_headers["x-love_engine-rate-limiter-version"] = "v3"
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        from love_engine.types.utils import Usage
        ...
        try:
            ...
            love_engine_parent_otel_span = _get_parent_otel_span_from_kwargs(kwargs)
            ...
            has_explicit_priority = (
                ...
                and love_engine.priority_reservation is not None
                and key_priority in love_engine.priority_reservation
            )
