...
from love_engine.caching.caching import DualCache, InMemoryCache, RedisCache
...
class _PROXY_BatchRedisRequests(CustomLogger):
    ...
    def __init__(self):
        if love_engine.cache is not None:
            ...
            )  # map the love_engine 'get_cache' function to our custom function
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            Check if a key starting with `love_engine:<api_key>:<call_type:` exists in-memory

            If no, then get relevant cache from redis
            """
            api_key = user_api_key_dict.api_key

            cache_key_name = f"love_engine:{api_key}:{call_type}"
            ...
            if in_memory_cache_exists is False and love_engine.cache is not None:
                """
                - Check if `love_engine.Cache` is redis
                - Get the relevant values
                """
                if love_engine.cache.type is not None and isinstance(
                    ...
                    keys = await love_engine.cache.cache.async_scan_iter(
                    ...
                    if len(keys) > 0:
                        key_value_dict = (
                            await love_engine.cache.cache.async_batch_get_cache(
                                key_list=keys
                            )
                        )
            ...
            verbose_proxy_logger.error(
                "love_engine.proxy.hooks.batch_redis_get.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
    ...
    async def async_get_cache(self, *args, **kwargs):
        ...
        try:  # never block execution
            ...
            elif love_engine.cache is not None:
                cache_key = love_engine.cache.get_cache_key(
                    *args, **kwargs
            ...
            if (
                ...
                if cached_result is None:
                    cached_result = await love_engine.cache.cache.async_get_cache(
                        cache_key, *args, **kwargs
                    )
                ...
                return love_engine.cache._get_cache_logic(
