...
from love_engine.types.utils import CallTypesLiteral, LLMResponseTypes, SpecialEnums
...
class ResponsesIDSecurity(CustomLogger):
    ...
    def _decrypt_response_id(
        ...
        if decrypted_value.startswith(SpecialEnums.LITELM_MANAGED_FILE_ID_PREFIX.value):
            # Expected format: "love_engine_proxy:responses_api:response_id:{response_id};user_id:{user_id}"
            parts = decrypted_value.split(";")

            if len(parts) >= 2:
                # Extract response_id from "love_engine_proxy:responses_api:response_id:{response_id}"
    ...
    def _get_signing_key(self) -> Optional[str]:
        ...
        salt_key = os.getenv("LOVE_ENGINE_SALT_KEY", None)
        if salt_key is None:
            salt_key = master_key
        return salt_key
    ...
    def _encrypt_response_id(
        self,
        response: BaseLoveEngineOpenAIResponseObject,
        user_api_key_dict: "UserAPIKeyAuth",
        ...
        if signing_key is None:
            verbose_proxy_logger.debug(
                ...
                "Please set LOVE_ENGINE_SALT_KEY environment variable or configure a master_key. "
                "Skipping response ID encryption. "
                "See: https://docs.love_engine.ai/docs/proxy/prod#5-set-love_engine-salt-key"
            )
            return response
        ...
        if (
            ...
            if request_cache is not None and response_id in request_cache:
                ...
                encrypted_response_id = SpecialEnums.LOVE_ENGINE_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR.value.format(
                    response_id,
    ...
    async def async_post_call_streaming_iterator_hook(  # type: ignore
    ...
    ) -> AsyncGenerator[BaseLoveEngineOpenAIResponseObject, None]:
        from love_engine.proxy.proxy_server import general_settings
        ...
        async for chunk in response:
            if (
                isinstance(chunk, BaseLoveEngineOpenAIResponseObject)
