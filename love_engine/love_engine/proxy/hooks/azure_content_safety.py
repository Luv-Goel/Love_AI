...
class _PROXY_AzureContentSafety(
    CustomLogger
):  # https://docs.love_engine.ai/docs/observability/custom_callback#callback-class
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            verbose_proxy_logger.error(
                "love_engine.proxy.hooks.azure_content_safety.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            verbose_proxy_logger.debug(traceback.format_exc())
    ...
    async def async_post_call_success_hook(
        ...
        for choice in response.choices:
            if not isinstance(choice, love_engine.utils.Choices):
                continue
