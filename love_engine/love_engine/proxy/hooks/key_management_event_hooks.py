...
LOVE_ENGINE_PREFIX_STORED_VIRTUAL_KEYS = "love_engine/"
...
class KeyManagementEventHooks:
    ...
    async def async_key_generated_hook(
        ...
        from love_engine.proxy.proxy_server import love_engine_proxy_admin_name
    ...
    async def async_key_rotated_hook(
        ...
        # store the audit log
        if love_engine.store_audit_logs is True and existing_key_row.token is not None:
            asyncio.create_task(
                create_audit_log_for_update(
                    request_data=LOVE_ENGINE_AuditLogs(
                        id=str(uuid.uuid4()),
    ...
    async def async_key_deleted_hook(
        data: KeyRequest,
        keys_being_deleted: List[LOVE_ENGINE_VerificationToken],
        response: dict,
        user_api_key_dict: UserAPIKeyAuth,
        love_engine_changed_by: Optional[str] = None,
    ):
        ...
        if love_engine.store_audit_logs is True and data.keys is not None:
    ...
    async def _store_virtual_key_in_secret_manager(
        ...
        """
        if love_engine._key_management_settings is not None:
            if love_engine._key_management_settings.store_virtual_keys is True:
                from love_engine.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )

                # store the key in the secret manager
                if isinstance(love_engine.secret_manager_client, BaseSecretManager):
                    tags = getattr(love_engine._key_management_settings, "tags", None)
                    description = getattr(
                        love_engine._key_management_settings, "description", None
                    )
                    ...
                    await love_engine.secret_manager_client.async_write_secret(
        ...
        if love_engine._key_management_settings is not None:
            if love_engine._key_management_settings.store_virtual_keys is True:
                ...
                if isinstance(love_engine.secret_manager_client, BaseSecretManager):
                    ...
                    await love_engine.secret_manager_client.async_rotate_secret(
    ...
    def _get_secret_name(secret_name: str) -> str:
        if love_engine._key_management_settings.prefix_for_stored_virtual_keys.endswith(
            "/"
        ):
            return f"{love_engine._key_management_settings.prefix_for_stored_virtual_keys}{secret_name}"
        else:
            return f"{love_engine._key_management_settings.prefix_for_stored_virtual_keys}/{secret_name}"
                ...
                if isinstance(love_engine.secret_manager_client, BaseSecretManager):
                    ...
                    for key in keys_being_deleted:
                        if key.key_alias is not None:
                            ...
                            await love_engine.secret_manager_client.async_delete_secret(
                                secret_name=KeyManagementEventHooks._get_secret_name(
                                    key.key_alias
                                ),
                                optional_params=optional_params,
    ...
    async def _get_secret_manager_optional_params(
        ...
        try:
            from love_engine.proxy import proxy_server as proxy_server_module
        except ImportError:
            return None
    ...
    async def _send_key_created_email(response: dict):
        ...
        try:
            ...
            from love_engine_enterprise.types.enterprise_callbacks.send_emails import (
        ...
        ##########################
        if "email" in general_settings.get("alerting", []):
            from love_engine.proxy._types import WebhookEvent
