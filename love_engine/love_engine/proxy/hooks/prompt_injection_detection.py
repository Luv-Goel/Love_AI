...
from love_engine.constants import DEFAULT_PROMPT_INJECTION_SIMILARITY_THRESHOLD
...
from love_engine.proxy._types import LoveEnginePromptInjectionParams, UserAPIKeyAuth
from love_engine.router import Router
from love_engine.utils import get_formatted_prompt
...
class _OPTIONAL_PromptInjectionDetection(CustomLogger):
    ...
    def __init__(
        self,
        prompt_injection_params: Optional[LoveEnginePromptInjectionParams] = None,
    ):
    ...
    async def async_moderation_hook(  # type: ignore
        ...
        if (
            ...
            if isinstance(response, love_engine.ModelResponse) and isinstance(
                response.choices[0], love_engine.Choices
            ):
