...
import love_engine
from love_engine import DualCache, EmbeddingResponse, ModelResponse, TextCompletionResponse
from love_engine._logging import verbose_proxy_logger
from love_engine.integrations.custom_logger import CustomLogger
from love_engine.love_engine_core_utils.core_helpers import _get_parent_otel_span_from_kwargs
from love_engine.proxy._types import CommonProxyErrors, CurrentItemRateLimit, UserAPIKeyAuth
from love_engine.exceptions import RateLimitType
from love_engine.proxy.auth.auth_utils import (
    get_key_model_rpm_limit,
    get_key_model_tpm_limit,
)
from love_engine.proxy.common_utils.proxy_rate_limit_error import ProxyRateLimitError
...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    def raise_rate_limit_error(
        ...
        """
        Raise a 429 with a retry-after header for love_engine-proxy parallel-request limits.
        ...
        :class:`love_engine.RateLimitError` (so callers can catch by category) and a
        ...
        callbacks). Falls back to ``llm_provider="love_engine_proxy"`` when the
    ...
    async def async_pre_call_hook(
        ...
        if _tpm_limit_for_key_model is not None or _rpm_limit_for_key_model is not None:
            ...
            _remaining_limits_data = {
                f"love_engine-key-remaining-tokens-{_model}": _remaining_tokens,
                f"love_engine-key-remaining-requests-{_model}": _remaining_requests,
            }
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            global_max_parallel_requests = kwargs["love_engine_params"]["metadata"].get(
                "global_max_parallel_requests", None
            )
            user_api_key = kwargs["love_engine_params"]["metadata"]["user_api_key"]
            user_api_key_user_id = kwargs["love_engine_params"]["metadata"].get(
                "user_api_key_user_id", None
            )
            user_api_key_team_id = kwargs["love_engine_params"]["metadata"].get(
                "user_api_key_team_id", None
            )
            user_api_key_model_max_budget = kwargs["love_engine_params"]["metadata"].get(
                "user_api_key_model_max_budget", None
            )
            user_api_key_end_user_id = kwargs.get("user")

            user_api_key_metadata = (
                kwargs["love_engine_params"]["metadata"].get("user_api_key_metadata", {})
                or {}
            )
            user_api_key_team_metadata = kwargs["love_engine_params"]["metadata"].get(
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            _metadata = kwargs["love_engine_params"].get("metadata", {}) or {}
    ...
    async def get_internal_user_object(
        ...
        It uses the `get_user_object` function from `love_engine.proxy.auth.auth_checks`
