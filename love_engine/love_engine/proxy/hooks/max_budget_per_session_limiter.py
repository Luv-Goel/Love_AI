"""
Per-Session Budget Limiter for LoveEngine Proxy.

Enforces a dollar-amount cap per session (identified by `session_id` /
`x-love_engine-trace-id`). After each successful LLM call the response cost is
accumulated against the session. When the accumulated spend exceeds
`max_budget_per_session` (configured in agent love_engine_params), subsequent
...
class _PROXY_MaxBudgetPerSessionHandler(CustomLogger):
    ...
    Configuration (set in agent love_engine_params):
    ...
    def __init__(self, internal_usage_cache: InternalUsageCache):
        ...
        self.ttl = int(
            os.getenv(
                "LOVE_ENGINE_MAX_BUDGET_PER_SESSION_TTL",
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            max_budget = agent_love_engine_params.get("max_budget_per_session")
    ...
    def _get_max_budget_per_session(
        ...
        """Extract max_budget_per_session from agent love_engine_params."""
        ...
        max_budget = love_engine_params.get("max_budget_per_session")
