...
from love_engine.proxy.client.health import HealthManagementClient
...
def print_version(base_url: str, api_key: Optional[str]):
    ...
    click.echo(f"LoveEngine Proxy CLI Version: {love_engine_version}")
    if base_url:
        click.echo(f"LoveEngine Proxy Server URL: {base_url}")
    try:
        health_client = HealthManagementClient(base_url=base_url, api_key=api_key)
        server_version = health_client.get_server_version()
        if server_version:
            click.echo(f"LoveEngine Proxy Server Version: {server_version}")
        else:
            click.echo("LoveEngine Proxy Server Version: (unavailable)")
...
@click.option(
    ...
    is_eager=True,
    expose_value=False,
    help="Show the LoveEngine Proxy CLI and server version and exit.",
...
@click.option(
    "--base-url",
    envvar="LOVE_ENGINE_PROXY_URL",
    show_envvar=True,
    default="http://localhost:4000",
    help="Base URL of the LoveEngine proxy server",
)
@click.option(
    "--api-key",
    envvar="LOVE_ENGINE_PROXY_API_KEY",
    show_envvar=True,
    help="API key for authentication",
)
@click.pass_context
def cli(ctx: click.Context, base_url: str, api_key: Optional[str]) -> None:
    """LoveEngine Proxy CLI - Manage your LoveEngine proxy server"""
...
def version(ctx: click.Context):
    """Show the LoveEngine Proxy CLI and server version."""
