"""Team management commands for LoveEngine CLI."""
...
from love_engine.proxy.client import Client
...
def assign_key(ctx: click.Context, team_id: Optional[str]):
    ...
    if not api_key:
        click.echo("❌ No API key found. Please login first using 'love_engine login'")
        raise click.Abort()
