...
CODEX_PROXY_PROVIDER = "love_engine"
...
def _codex_proxy_args(base_url: str) -> List[str]:
    ...
    return [
        ...
        "-c",
        f'{provider}.name="LoveEngine proxy"',
        "-c",
...
def verify_proxy_key(
    ...
    *,
    ...
    try:
        ...
        raise AgentRunError(
            f"Could not reach the LoveEngine proxy at {base_url.rstrip('/')}: {e}. "
            "Is it running, and is --base-url (or LOVE_ENGINE_PROXY_URL) correct?"
        )
    if resp.status_code in (401, 403):
        raise AgentRunError(
            f"LoveEngine rejected your key (HTTP {resp.status_code}). "
...
def _resolve_api_key(ctx: click.Context) -> str:
    ...
    if not _is_interactive():
        raise click.ClickException(
            "No LoveEngine key found. Set LOVE_ENGINE_PROXY_API_KEY (or pass --api-key) for "
            "non-interactive use, or run `lite login` from a terminal."
        )

    click.echo("No LoveEngine credentials found; starting login...")
    ctx.invoke(login)
...
def _launch(
    ...
    click.echo(
        f"love_engine: routing {display_name} through proxy at {base_url.rstrip('/')}"
    )
...
def _make_agent_command(binary: str, display_name: str) -> click.Command:
    @click.command(
        ...
        short_help=f"Run {display_name} through your LoveEngine proxy",
    )
    ...
    _command.help = (
        f"Run {display_name} routed through your LoveEngine proxy.\n\n"
        f"Logs in with LoveEngine if needed, verifies your key against the proxy, "
