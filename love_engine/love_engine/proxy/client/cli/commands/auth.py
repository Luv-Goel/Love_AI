...
from love_engine.constants import CLI_JWT_EXPIRATION_HOURS
...
def get_token_file_path() -> str:
    ...
    home_dir = Path.home()
    config_dir = home_dir / ".love_engine"
...
def get_stored_api_key(expected_base_url: Optional[str] = None) -> Optional[str]:
    ...
    return get_love_engine_gateway_api_key(expected_base_url=expected_base_url)
...
def _get_cli_sso_poll_headers(poll_secret: str) -> Dict[str, str]:
    return {"x-love_engine-cli-poll-secret": poll_secret}
...
def login(ctx: click.Context):
    """Login to LoveEngine proxy using SSO authentication"""
    from love_engine.constants import LOVE_ENGINE_CLI_SOURCE_IDENTIFIER
    from love_engine.proxy.client.cli.interface import show_commands
    ...
    try:
        ...
        sso_url = f"{base_url}/sso/key/generate?" + urlencode(
            {"source": LOVE_ENGINE_CLI_SOURCE_IDENTIFIER, "key": key_id}
        )
