...
class Client:
    """Main client for interacting with the LoveEngine proxy API."""
    ...
    def __init__(
        ...
        """
        Initialize the LoveEngine proxy client.

        Args:
            base_url (str): The base URL of the LoveEngine proxy server (e.g., "http://localhost:4000")
            api_key (Optional[str]): API key for authentication. If provided, it will be sent as a Bearer token.
            timeout: Request timeout in seconds (default: 30)
        """
        self._base_url = base_url.rstrip("/")
        # Only use the stored CLI key when it was issued for this server.
        self._api_key = api_key or get_love_engine_gateway_api_key(
