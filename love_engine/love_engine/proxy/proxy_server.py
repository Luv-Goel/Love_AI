...
from love_engine.constants import (
    ...
    LOVE_ENGINE_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS,
    LOVE_ENGINE_SETTINGS_SAFE_DB_OVERRIDES,
    LOVE_ENGINE_UI_ALLOW_HEADERS,
    LOVE_ENGINE_UI_SESSION_DURATION,
)
from love_engine.love_engine_core_utils.love_engine_logging import (
    _init_custom_logger_compatible_class,
)
from love_engine.love_engine_core_utils.safe_json_dumps import safe_dumps
from love_engine.proxy._types import (
    ...
    LOVE_ENGINE_TeamTableCachedObj,
...
from love_engine.proxy.common_utils.cache_pydantic_utils import CacheCodec
from love_engine.proxy.common_utils.callback_utils import (
    normalize_callback_names,
    process_callback,
)
from love_engine.proxy.common_utils.realtime_utils import _realtime_request_body
...
sys.path.insert(
    0, os.path.abspath("../..")
)  # Adds the parent directory to the system path - for love_engine local dev
...
try:
    ...
    raise ImportError(f"Missing dependency {e}. Run `pip install 'love_engine[proxy]'`")
...
def generate_feedback_box():
    ...
    print(  # noqa: T201
        "\033[1;37m"
        + "# {:^59} #\033[0m".format("https://github.com/BerriAI/love_engine/issues/new")
    )
    print("\033[1;37m" + "#" + " " * box_width + "#\033[0m")  # noqa: T201
    print("\033[1;37m" + "#" + "-" * box_width + "#\033[0m")  # noqa: T201
    print()  # noqa: T201
    print(" Love AI Inference Engine Initialized!")  # noqa: T201
    print()  # noqa: T201
    print()  # noqa: T201
    print()  # noqa: T201
    print(  # noqa: T201
        "\033[1;31mGive Feedback / Get Help: https://github.com/BerriAI/love_engine/issues/new\033[0m"
    )
    print()  # noqa: T201
    print()  # noqa: T201
...
from love_engine._logging import verbose_proxy_logger, verbose_router_logger
from love_engine.caching.caching import DualCache, RedisCache
from love_engine.caching.redis_cluster_cache import RedisClusterCache
from love_engine.constants import (
...
from love_engine.exceptions import RejectedRequestError
from love_engine.integrations.custom_guardrail import ModifyResponseException
from love_engine.integrations.custom_logger import CustomLogger
from love_engine.integrations.SlackAlerting.slack_alerting import SlackAlerting
from love_engine.love_engine_core_utils.core_helpers import (
    _get_parent_otel_span_from_kwargs,
    get_love_engine_metadata_from_kwargs,
)
from love_engine.love_engine_core_utils.credential_accessor import CredentialAccessor
from love_engine.love_engine_core_utils.love_engine_logging import Logging as LoveEngineLoggingObj
from love_engine.love_engine_core_utils.sensitive_data_masker import (
    SensitiveDataMasker,
    mask_sensitive_keys,
)
...
from love_engine.proxy._lazy_features import attach_lazy_features
from love_engine.proxy._types import *
from love_engine.proxy.analytics_endpoints.analytics_endpoints import (
    router as analytics_router,
)
from love_engine.proxy.auth.auth_checks import (
    ExperimentalUIJWTToken,
    can_key_call_resolved_model,
    get_team_object,
    log_db_metrics,
)
from love_engine.proxy.auth.auth_utils import (
    check_response_size_is_safe,
    is_request_body_safe,
    warn_once_if_custom_auth_skips_common_checks,
)
from love_engine.proxy.auth.handle_jwt import JWTHandler
from love_engine.proxy.auth.love_engine_license import LicenseCheck
from love_engine.proxy.auth.model_checks import (
    get_all_fallbacks,
...
from love_engine.proxy.auth.user_api_key_auth import (
    _fetch_global_spend_with_event_coordination,
    user_api_key_auth,
    user_api_key_auth_websocket,
)
from love_engine.proxy.batches_endpoints.endpoints import router as batches_router

## Import All Misc routes here ##
from love_engine.proxy.caching_routes import router as caching_router
from love_engine.proxy.common_request_processing import (
    ProxyBaseLLMRequestProcessing,
    _is_azure_model_router_request,
    create_response,
)
from love_engine.proxy.common_utils.callback_utils import initialize_callbacks_on_proxy
from love_engine.proxy.common_utils.debug_utils import init_verbose_loggers
from love_engine.proxy.common_utils.debug_utils import router as debugging_endpoints_router
from love_engine.proxy.common_utils.encrypt_decrypt_utils import (
    decrypt_value_helper,
    encrypt_value_helper,
)
from love_engine.proxy.common_utils.html_forms.ui_login import build_ui_login_form
from love_engine.proxy.common_utils.http_parsing_utils import (
    _read_request_body,
    _safe_get_request_headers,
    check_file_size_under_limit,
    get_form_data,
)
from love_engine.proxy.common_utils.load_config_utils import (
    get_config_file_contents_from_gcs,
    get_file_contents_from_s3,
)
from love_engine.proxy.common_utils.model_listing_utils import TeamModelNameTranslator
from love_engine.proxy.common_utils.openai_endpoint_utils import (
    remove_sensitive_info_from_deployment,
)
from love_engine.proxy.common_utils.proxy_state import ProxyState
from love_engine.proxy.common_utils.reset_budget_job import ResetBudgetJob
from love_engine.proxy.common_utils.swagger_utils import ERROR_RESPONSES
from love_engine.proxy.common_utils.timezone_utils import get_budget_reset_time
from love_engine.proxy.common_utils.user_api_key_cache import UserApiKeyCache
from love_engine.proxy.container_endpoints.endpoints import router as container_router
from love_engine.proxy.credential_endpoints.endpoints import router as credential_router
from love_engine.proxy.db.db_transaction_queue.spend_log_cleanup import SpendLogCleanup
from love_engine.proxy.db.exception_handler import (
    PrismaDBExceptionHandler,
    call_with_db_reconnect_retry,
)
from love_engine.proxy.db.spend_counter_reseed import SpendCounterReseed
from love_engine.proxy.discovery_endpoints import ui_discovery_endpoints_router
from love_engine.proxy.fine_tuning_endpoints.endpoints import router as fine_tuning_router
from love_engine.proxy.fine_tuning_endpoints.endpoints import set_fine_tuning_config
from love_engine.proxy.google_endpoints.endpoints import router as google_router
from love_engine.proxy.guardrails.init_guardrails import (
    init_guardrails_v2,
    initialize_guardrails,
)
from love_engine.proxy.health_check import (
    health_check_filter_kwargs_from_general_settings,
    perform_health_check,
)
from love_engine.proxy.health_endpoints._health_endpoints import router as health_router
from love_engine.proxy.hooks.model_max_budget_limiter import (
    _PROXY_VirtualKeyModelMaxBudgetLimiter,
)
from love_engine.proxy.hooks.prompt_injection_detection import (
    _OPTIONAL_PromptInjectionDetection,
)
from love_engine.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from love_engine.proxy.image_endpoints.endpoints import router as image_router
from love_engine.proxy.love_engine_pre_call_utils import add_love_engine_data_to_request
from love_engine.proxy.management_endpoints.budget_management_endpoints import (
    router as budget_management_router,
)
from love_engine.proxy.management_endpoints.cache_settings_endpoints import (
    router as cache_settings_router,
)
from love_engine.proxy.management_endpoints.callback_management_endpoints import (
    router as callback_management_endpoints_router,
)
from love_engine.proxy.management_endpoints.common_utils import (
    _user_has_admin_privileges,
    _user_has_admin_view,
    admin_can_invite_user,
)
from love_engine.proxy.management_endpoints.cost_tracking_settings import (
    router as cost_tracking_settings_router,
)
from love_engine.proxy.management_endpoints.customer_endpoints import (
    router as customer_router,
)
from love_engine.proxy.management_endpoints.fallback_management_endpoints import (
    router as fallback_management_router,
)
from love_engine.proxy.management_endpoints.internal_user_endpoints import (
...
from love_engine.proxy.management_endpoints.model_access_group_management_endpoints import (
    router as model_access_group_management_router,
)
from love_engine.proxy.management_endpoints.model_management_endpoints import (
    _add_model_to_db,
    _add_team_model_to_db,
    _deduplicate_love_engine_router_models,
)
from love_engine.proxy.management_endpoints.model_management_endpoints import (
    router as model_management_router,
)
from love_engine.proxy.management_endpoints.organization_endpoints import (
    router as organization_router,
)
from love_engine.proxy.management_endpoints.router_settings_endpoints import (
    router as router_settings_router,
)
from love_engine.proxy.management_endpoints.tag_management_endpoints import (
    router as tag_management_router,
)
from love_engine.proxy.management_endpoints.team_callback_endpoints import (
    router as team_callback_router,
)
from love_engine.proxy.management_endpoints.team_endpoints import router as team_router
from love_engine.proxy.management_endpoints.team_endpoints import (
    update_team,
    validate_membership,
)
from love_engine.proxy.management_endpoints.ui_sso import (
    get_disabled_non_admin_personal_key_creation,
)
from love_engine.proxy.management_endpoints.ui_sso import router as ui_sso_router
from love_engine.proxy.management_endpoints.user_agent_analytics_endpoints import (
    router as user_agent_analytics_router,
)
from love_engine.proxy.management_endpoints.workflow_management_endpoints import (
...
from love_engine.proxy.memory.memory_endpoints import router as memory_router
from love_engine.proxy.middleware.in_flight_requests_middleware import (
    InFlightRequestsMiddleware,
)
from love_engine.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
from love_engine.proxy.middleware.request_size_limit_middleware import (
    RequestSizeLimitMiddleware,
)
from love_engine.proxy.ocr_endpoints.endpoints import router as ocr_router
from love_engine.proxy.openai_files_endpoints.files_endpoints import (
    router as openai_files_router,
)
from love_engine.proxy.openai_files_endpoints.files_endpoints import (
    set_files_config,
)
from love_engine.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
...
from love_engine.proxy.pass_through_endpoints.pass_through_endpoints import (
    initialize_pass_through_endpoints,
)
from love_engine.proxy.pass_through_endpoints.pass_through_endpoints import (
    router as pass_through_router,
)
from love_engine.proxy.public_endpoints import router as public_endpoints_router
from love_engine.proxy.rag_endpoints.endpoints import router as rag_router
from love_engine.proxy.rerank_endpoints.endpoints import router as rerank_router
from love_engine.proxy.response_api_endpoints.endpoints import router as response_router
from love_engine.proxy.route_llm_request import route_request
from love_engine.proxy.search_endpoints.endpoints import router as search_router
from love_engine.proxy.shutdown.graceful_shutdown_manager import GracefulShutdownManager
from love_engine.proxy.spend_tracking.spend_management_endpoints import (
    router as spend_management_router,
)
from love_engine.proxy.spend_tracking.spend_tracking_utils import get_logging_payload
from love_engine.proxy.types_utils.utils import get_instance_fn
from love_engine.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
    router as ui_crud_endpoints_router,
)
from love_engine.proxy.utils import (
    PrismaClient,
    ProxyLogging,
    ProxyUpdateSpend,
    _cache_user_row,
    ...
    love_engine_config_cache,
    migrate_passwords_to_scrypt_async,
    model_dump_with_preserved_fields,
    prefetch_config_params,
    update_spend,
)
from love_engine.proxy.video_endpoints.endpoints import router as video_router
from love_engine.repositories.credentials_repository import CredentialsRepository
from love_engine.router import (
    AssistantsTypedDict,
    Deployment,
    LOVE_ENGINE_Params,
    ModelGroupInfo,
)
from love_engine.scheduler import FlowItem, Scheduler
from love_engine.secret_managers.aws_secret_manager import load_aws_kms
from love_engine.secret_managers.google_kms import load_google_kms
from love_engine.secret_managers.main import (
    get_secret,
    get_secret_bool,
    get_secret_str,
...
from love_engine.types.integrations.slack_alerting import SlackAlertingArgs
...
from love_engine.types.llms.openai import HttpxBinaryResponseContent
from love_engine.types.proxy.control_plane_endpoints import WorkerRegistryEntry
from love_engine.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from love_engine.types.proxy.management_endpoints.ui_sso import (
    DefaultTeamSSOParams,
    LOVE_ENGINE_UpperboundKeyGenerateParams,
)
from love_engine.types.realtime import RealtimeQueryParams
from love_engine.types.router import (
    DeploymentTypedDict,
)
from love_engine.types.router import ModelInfo as RouterModelInfo
from love_engine.types.router import (
    RouterGeneralSettings,
    SearchToolTypedDict,
    updateDeployment,
)
from love_engine.types.scheduler import DefaultPriorities
from love_engine.types.secret_managers.main import (
    KeyManagementSettings,
    KeyManagementSystem,
)
from love_engine.types.utils import CredentialItem, CustomHuggingfaceTokenizer
from love_engine.types.utils import ModelInfo as ModelMapInfo
from love_engine.types.utils import RawRequestTypedDict, StandardLoggingPayload
from love_engine.utils import _add_custom_logger_callback_to_specific_event

try:
    from love_engine._version import version
except Exception:
    version = "0.0.0"
love_engine.suppress_debug_info = True
import json
...
from love_engine.types.agents import AgentConfig

# import enterprise folder
enterprise_router = APIRouter()
try:
    # when using love_engine cli
    import love_engine.proxy.enterprise as enterprise
except Exception:
    # when using love_engine docker image
    try:
...
try:
    from love_engine_enterprise.proxy.enterprise_routes import router as _enterprise_router
    from love_engine_enterprise.proxy.proxy_server import EnterpriseProxyConfig
...
global_max_parallel_request_retries_env: Optional[str] = os.getenv(
    "LOVE_ENGINE_GLOBAL_MAX_PARALLEL_REQUEST_RETRIES"
)
...
global_max_parallel_request_retry_timeout_env: Optional[str] = os.getenv(
    "LOVE_ENGINE_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT"
)
...
ui_message = f"👉 [```LoveEngine Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO. Having issues? Try [```Fallback Login```]({fallback_login_link})"
ui_message += "\n\n💸 [```LoveEngine Model Cost Map```](https://models.love_engine.ai/)."

ui_message += f"\n\n🔎 [```LoveEngine Model Hub```]({model_hub_link}). See available models on the proxy. [**Docs**](https://docs.love_engine.ai/docs/proxy/ai_hub)"

custom_swagger_message = "[**Customize Swagger Docs**](https://docs.love_engine.ai/docs/proxy/enterprise#swagger-docs---custom-routes--branding)"

### CUSTOM BRANDING [ENTERPRISE FEATURE] ###
_title = os.getenv("DOCS_TITLE", "LoveEngine API") if premium_user else "LoveEngine API"
_description = (
    os.getenv(
        "DOCS_DESCRIPTION",
...
async def proxy_shutdown_event():
    ...
    verbose_proxy_logger.info("Shutting down LoveEngine Proxy Server")
    if prisma_client:
        verbose_proxy_logger.debug("Disconnecting from Prisma")
        await prisma_client.disconnect()

    if love_engine.cache is not None:
        await love_engine.cache.disconnect()
    ...
    if "langfuse" in love_engine.success_callback:
        try:
            # flush langfuse logs on shutdow
            from love_engine.utils import langFuseLogger
...
async def proxy_startup_event(app: FastAPI):
    global prisma_client, master_key, use_background_health_checks, llm_router, llm_model_list, general_settings, proxy_budget_rescheduler_min_time, proxy_budget_rescheduler_max_time, love_engine_proxy_admin_name, db_writer_client, store_model_in_db, premium_user, _license_check, proxy_batch_polling_interval, shared_aiohttp_session
    import json

    init_verbose_loggers()

    ## RUN WORKER STARTUP HOOKS (e.g., gflags initialization) ##
    _startup_hooks_env = os.environ.get("LOVE_ENGINE_WORKER_STARTUP_HOOKS", "")
    ...
    verbose_proxy_logger.debug(
        "love_engine.proxy.proxy_server.py::startup() - CHECKING PREMIUM USER - {}".format(
            premium_user
        )
    )
    ...
    master_key = get_secret_str("LOVE_ENGINE_MASTER_KEY")
    ### LOAD CONFIG ###
    ...
    if env_config_yaml is not None:
        ...
        elif os.environ.get("LOVE_ENGINE_CONFIG_BUCKET_NAME") is not None and isinstance(
            worker_config, str
        ):
            (
                llm_router,
    ...
    try:
        from love_engine.integrations.otel.model.config import is_otel_v2_enabled

        if is_otel_v2_enabled():
            from opentelemetry import trace as _otel_trace

            from love_engine.integrations.otel.logger import OpenTelemetryV2

            _otel_v2_logger = (
                next(
                    (
                        cb
                        for cb in love_engine.service_callback
    ...
    # Read love_engine_settings from config for semantic filter initialization
    try:
        verbose_proxy_logger.debug("About to initialize semantic tool filter")
        _config = proxy_config.get_config_state()
        _love_engine_settings = _config.get("love_engine_settings", {})
        verbose_proxy_logger.debug(
            f"love_engine_settings keys = {list(_love_engine_settings.keys())}"
        )
        await ProxyStartupEvent._initialize_semantic_tool_filter(
            llm_router=llm_router,
            love_engine_settings=_love_engine_settings,
        )
    ...
    if prisma_client is not None and love_engine.max_budget > 0:
        ProxyStartupEvent._add_proxy_budget_to_db(
            love_engine_proxy_budget_name=love_engine_proxy_admin_name
        )
        asyncio.create_task(
    ...
    ## [Optional] Initialize Pyroscope continuous profiling (env: LOVE_ENGINE_ENABLE_PYROSCOPE=true)
...
## LOVE_ENGINE_OTEL_V2). This MUST run at app-creation time — once the lifespan runs,
## the middleware stack is frozen and ``instrument_app`` raises "Cannot add
## middleware after an application has started". See
## ``love_engine.integrations.otel.mount`` for the full rationale; the call is a safe
## no-op when the gate is off or the instrumentation package is unavailable.
from love_engine.integrations.otel.mount import instrument_fastapi_app
...
def get_openapi_schema():
    ...
    from love_engine.proxy.common_utils.openapi_schema_compat import (
    ...
    from love_engine.proxy.common_utils.custom_openapi_spec import CustomOpenAPISpec
    ...
    from love_engine.proxy._lazy_features import inject_lazy_stubs
...
def custom_openapi():
    ...
    openai_routes = LoveEngineRoutes.openai_routes.value
...
def _get_cors_config(
    ...
    Args:
        cors_origins_env: Value of LOVE_ENGINE_CORS_ORIGINS (defaults to os.getenv).
        cors_credentials_env: Value of LOVE_ENGINE_CORS_ALLOW_CREDENTIALS (defaults to os.getenv).
    ...
    _origins_raw = (
        ...
        else os.getenv("LOVE_ENGINE_CORS_ORIGINS")
    )
    ...
    # Set LOVE_ENGINE_CORS_ALLOW_CREDENTIALS=true to explicitly restore the old behaviour
    ...
    _credentials_raw = (
        ...
        else os.getenv("LOVE_ENGINE_CORS_ALLOW_CREDENTIALS")
    )
...
try:
    ...
    love_engine_asset_prefix = "/love_engine-asset-prefix"
    ...
    def _is_ui_pre_restructured(ui_dir: str) -> bool:
        ...
        Returns True if:
        1. Marker file .love_engine_ui_ready exists (created by Dockerfile), OR
        ...
        marker_file = os.path.join(ui_dir, ".love_engine_ui_ready")
    ...
    is_non_root = os.getenv("LOVE_ENGINE_NON_ROOT", "").lower() == "true"

    # Determine runtime UI path
    # Priority: LOVE_ENGINE_UI_PATH env var > default path based on is_non_root
    if is_non_root:
        default_runtime_ui_path = "/var/lib/love_engine/ui"
    else:
        default_runtime_ui_path = packaged_ui_path

    runtime_ui_path = os.getenv("LOVE_ENGINE_UI_PATH", default_runtime_ui_path)
        ...
        # Case 4: Runtime UI missing - try to populate
            ...
            if success:
                ...
                verbose_proxy_logger.warning(
                    ...
                    f"or set LOVE_ENGINE_UI_PATH to a writable emptyDir volume."
                )
                ui_path = packaged_ui_path
    else:
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        modified_content = content.replace(
                            f"{love_engine_asset_prefix}",
                            f"{server_root_path}",
                        )

                        # Replace the /.well-known/love_engine-ui-config with the server root path
                        modified_content = modified_content.replace(
                            "/love_engine/.well-known/love_engine-ui-config",
                            f"{server_root_path}/.well-known/love_engine-ui-config",
                        )
    ...
    app.mount(
        f"{love_engine_asset_prefix}/_next",
        StaticFiles(directory=os.path.join(ui_path, "_next")),
        name="next_static",
    )
    ...
    def _restructure_ui_html_files(ui_root: str) -> None:
        ...
        for current_root, _, files in os.walk(ui_root):
            ...
            if first_segment in {"_next", "love_engine-asset-prefix"}:
                continue
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=LOVE_ENGINE_UI_ALLOW_HEADERS,
)
...
def mount_swagger_ui():
    ...
    from love_engine.proxy._lazy_features import lazy_tag_to_prefix
...
love_engine.logging_callback_manager.add_love_engine_callback(model_max_budget_limiter)
...
# Tests that need to reset it can patch 'love_engine.proxy.proxy_server._pkce_no_redis_warning_emitted'.
...
queue: List = []
love_engine_proxy_budget_name = "love_engine-proxy-budget"
love_engine_proxy_admin_name = LOVE_ENGINE_PROXY_ADMIN_NAME
...
proxy_batch_write_at = PROXY_BATCH_WRITE_AT
love_engine_master_key_hash = None
...
def load_from_azure_key_vault(use_azure_key_vault: bool = False):
    ...
    try:
        ...
        love_engine.secret_manager_client = client
        love_engine._key_management_system = KeyManagementSystem.AZURE_KEY_VAULT
    except Exception as e:
        _error_str = str(e)
        verbose_proxy_logger.exception(
...
def cost_tracking():
    ...
    if prisma_client is not None:
        love_engine.logging_callback_manager.add_love_engine_callback(_ProxyDBLogger())
            ...
            if isinstance(key_budget_limits, list):
                for window in key_budget_limits:
                    ...
                    if key_window_counter not in reserved_counter_keys:
                        from love_engine.proxy.spend_tracking.budget_reservation import (
...
async def update_cache(
    ...
    async def _update_key_cache(token: str, response_cost: float):
        ...
        if (
            ...
            call_info = CallInfo(
                ...
                event_group=LOVE_ENGINE_EntityType.KEY,
            )
            # alert user
            asyncio.create_task(
        ...
        try:
            for _id in user_ids:
                ...
                existing_spend_obj = CacheCodec.deserialize(
                    cached_user, LOVE_ENGINE_UserTable
                )
                if existing_spend_obj is None:
                    return
                verbose_proxy_logger.debug(
                ...
                values_to_update_in_cache.append(
                    (
                        ...
                        CacheCodec.serialize(
                            existing_spend_obj, model_type=LOVE_ENGINE_UserTable
                        ),
                    )
                )
            ## UPDATE GLOBAL PROXY ##
            global_proxy_spend = await user_api_key_cache.async_get_cache(
                key="{}:spend".format(love_engine_proxy_admin_name)
            )
            if global_proxy_spend is None:
                ...
                values_to_update_in_cache.append(
                    ("{}:spend".format(love_engine_proxy_admin_name), increment)
                )
        except Exception as e:
            verbose_proxy_logger.warning(
    ...
    async def _update_end_user_cache():
        ...
        try:
            ...
            if cached_end_user is None:
                # if user does not exist in LOVE_ENGINE_UserTable, create a new user
                # do nothing if end-user not in api key cache
                return
            existing_spend_obj = CacheCodec.deserialize(
                cached_end_user, LOVE_ENGINE_EndUserTable
            )
            if existing_spend_obj is None:
                return
            verbose_proxy_logger.debug(
            ...
            values_to_update_in_cache.append(
                (
                    _id,
                    CacheCodec.serialize(
                        existing_spend_obj, model_type=LOVE_ENGINE_EndUserTable
                    ),
                )
            )
        except Exception as e:
    ...
    async def _update_team_cache():
        ...
        try:
            ...
            if cached_team is None:
                ...
                return
            existing_spend_obj: Optional[LOVE_ENGINE_TeamTableCachedObj] = (
                CacheCodec.deserialize(cached_team, LOVE_ENGINE_TeamTableCachedObj)
            )
            if existing_spend_obj is None:
                return
            verbose_proxy_logger.debug(
            ...
            values_to_update_in_cache.append(
                (
                    _id,
                    CacheCodec.serialize(
                        existing_spend_obj, model_type=LOVE_ENGINE_TeamTableCachedObj
                    ),
                )
            )
        except Exception as e:
    ...
    async def _update_tag_cache():
        ...
        try:
            for tag_name in tags:
                ...
                existing_tag_obj = CacheCodec.deserialize(cached_tag, LOVE_ENGINE_TagTable)
                ...
                values_to_update_in_cache.append(
                    (
                        cache_key,
                        CacheCodec.serialize(
                            existing_tag_obj, model_type=LOVE_ENGINE_TagTable
                        ),
                    )
                )
        except Exception as e:
    ...
    asyncio.create_task(
        user_api_key_cache.async_set_cache_pipeline(
            ...
            ttl=60,
            love_engine_parent_otel_span=parent_otel_span,
        )
    )
...
def run_ollama_serve():
    try:
        ...
        verbose_proxy_logger.debug(f"""
            LoveEngine Warning: proxy started with `ollama` model\n`ollama serve` failed with Exception{e}. \nEnsure you run `ollama serve`
        """)
...
def _schedule_background_health_check_db_save(
    ...
    """Fire-and-love_smitht: persist health check results to DB if prisma is available."""
    if prisma_client is None:
        return
    import time as time_module

    from love_engine.proxy.health_endpoints._health_endpoints import (
...
def _write_health_state_to_router_cache(
    ...
    """
    from love_engine.proxy.health_check import build_deployment_health_states
    from love_engine.router_utils.cooldown_handlers import _set_cooldown_deployments
    from love_engine.router_utils.router_callbacks.track_deployment_metrics import (
...
async def _run_background_health_check():
    ...
    if use_shared_health_check and redis_usage_cache is not None:
        from love_engine.proxy.health_check_utils.shared_health_check_manager import (
...
# Fields in ``love_engine_settings`` / ``general_settings`` whose values flow
...
_DB_OVERLAY_REMOTE_MODULE_STR_FIELDS: Dict[str, Tuple[str, ...]] = {
    "love_engine_settings": ("post_call_rules",),
    "general_settings": (
        "custom_auth",
        "custom_key_generate",
        "custom_key_update",
...
def _scrub_guardrail_inner(inner: Dict[str, Any]) -> None:
    ...
    if isinstance(cbs, list):
        ...
        if len(cleaned) != len(cbs):
            verbose_proxy_logger.warning(
                ...
                "love_engine_settings.guardrails[...].callbacks",
                len(cbs) - len(cleaned),
            )
    ...
    if _is_remote_module_url(inner.get("guardrail")):
        verbose_proxy_logger.warning(
            ...
            "love_engine_settings.guardrails[...].guardrail: %r",
            inner.get("guardrail"),
        )
...
def _scrub_db_overlay_remote_module_loads(section: str, db_value: Any) -> Any:
    ...
    # walk it explicitly.
    if section == "love_engine_settings":
            ...
            for item in cpm:
                if isinstance(item, dict) and _is_remote_module_url(
                    ...
                    verbose_proxy_logger.warning(
                        ...
                        "love_engine_settings.custom_provider_map: %r",
                        item.get("custom_handler"),
                    )
                    item["custom_handler"] = None
    # ``love_engine_settings.guardrails`` is a list of single-key dicts in
    # v1 ({guardrail_name: {callbacks: [...], default_on: bool}}) or a
    # list of v2 entries ({guardrail_name, love_engine_params: {guardrail:
    ...
    if section == "love_engine_settings":
        ...
        if isinstance(guardrails, list):
            for entry in guardrails:
                ...
                lp = entry.get("love_engine_params")
                if isinstance(lp, dict):
                    _scrub_guardrail_inner(lp)

    # ``general_settings.love_engine_jwtauth.custom_validate`` is a nested
    # string field.
    if section == "general_settings":
        jwt = sanitized.get("love_engine_jwtauth")
        if isinstance(jwt, dict) and _is_remote_module_url(jwt.get("custom_validate")):
            verbose_proxy_logger.warning(
                ...
                "general_settings.love_engine_jwtauth: %r",
...
class ProxyConfig:
    ...
    async def save_config(self, new_config: dict):
        ...
        - if api_key is passed, save that to the local environment or connected secret manage (maybe expose `love_engine.save_secret()`)
        """
    ...
    def load_team_config(self, team_id: str):
        ...
        ## LOVE_ENGINE MODULE SETTINGS (e.g. love_engine.drop_params=True,..)
        love_engine_settings = config.get("love_engine_settings", {})
        all_teams_config = love_engine_settings.get("default_team_settings", None)
    ...
    def _init_cache(
        ...
        from love_engine import Cache

        if "default_in_memory_ttl" in cache_params:
            love_engine.default_in_memory_ttl = cache_params["default_in_memory_ttl"]

        if "default_redis_ttl" in cache_params:
            love_engine.default_redis_ttl = cache_params["default_redis_ttl"]

        love_engine.cache = Cache(**cache_params)

        if love_engine.cache is not None and isinstance(
            love_engine.cache.cache, (RedisCache, RedisClusterCache)
        ):
            ## INIT PROXY REDIS USAGE CLIENT ##
            redis_usage_cache = love_engine.cache.cache
            spend_counter_cache.attach_redis_cache(
                redis_usage_cache,
                default_redis_ttl=love_engine.default_redis_ttl,
            )
            ...
            if enable_redis_auth_cache is True:
                ...
                verbose_proxy_logger.info(
                    ...
                    "love_engine_settings.enable_redis_auth_cache: true to share "
                    "the auth cache across workers and reduce DB load."
                )
            love_engine_config_cache.redis_cache = redis_usage_cache
            # Note: PKCE verifier storage uses redis_usage_cache directly (not
            # user_api_key_cache) to avoid routing all API-key lookups through Redis.
        elif love_engine_config_cache.redis_cache is None:
            verbose_proxy_logger.info(
                "love_engine_config_cache: no Redis configured; cluster-wide cache sharing disabled."
            )
    ...
    def switch_on_llm_response_caching(self):
        ...
        Router passes caching=self.cache_responses to love_engine.completion()
        """
        global llm_router
        import love_engine
    ...
    async def get_config(self, config_file_path: Optional[str] = None) -> dict:
        ...
        if os.environ.get("LOVE_ENGINE_CONFIG_BUCKET_NAME") is not None:
            bucket_name = os.environ.get("LOVE_ENGINE_CONFIG_BUCKET_NAME")
            object_key = os.environ.get("LOVE_ENGINE_CONFIG_BUCKET_OBJECT_KEY")
            bucket_type = os.environ.get("LOVE_ENGINE_CONFIG_BUCKET_TYPE")
            verbose_proxy_logger.debug(
    ...
    def parse_search_tools(self, config: dict) -> Optional[List[SearchToolTypedDict]]:
        ...
        print(  # noqa: T201
            "\033[32mLoveEngine: Proxy initialized with Search Tools:\033[0m"
        )

        for search_tool in search_tools_raw:
            # Display loaded search tool
            search_tool_name = search_tool.get("search_tool_name", "")
            search_provider = search_tool.get("love_engine_params", {}).get(
                "search_provider", ""
            )
            print(  # noqa: T201
            ...
            # Handle os.environ/ variables in love_engine_params
            love_engine_params = search_tool.get("love_engine_params", {})
            if love_engine_params:
                for k, v in love_engine_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _v = v.replace("os.environ/", "")
                        v = get_secret(_v)
                        love_engine_params[k] = v
                search_tool["love_engine_params"] = love_engine_params
    ...
    def _load_environment_variables(self, config: dict):
        ...
        if environment_variables:
            ...
            # check if love_engine_license in general_settings
            if "LOVE_ENGINE_LICENSE" in environment_variables:
                _license_check.license_str = os.getenv("LOVE_ENGINE_LICENSE", None)
                premium_user = _license_check.is_premium()
        return

    async def load_config(
        self, router: Optional[love_engine.Router], config_file_path: str
    ):
        """
        Load config values into proxy global state
        """
        global master_key, user_config_file_path, otel_logging, user_custom_auth, user_custom_auth_path, user_custom_key_generate, user_custom_key_update, user_custom_sso, user_custom_ui_sso_sign_in_handler, use_background_health_checks, use_shared_health_check, health_check_interval, health_check_concurrency, use_queue, proxy_budget_rescheduler_max_time, proxy_budget_rescheduler_min_time, ui_access_mode, love_engine_master_key_hash, proxy_batch_write_at, disable_spend_logs, prompt_injection_detection_obj, redis_usage_cache, store_model_in_db, premium_user, open_telemetry_logger, health_check_details, proxy_batch_polling_interval, config_passthrough_endpoints
        ...
        if callback_settings:
            love_engine.callback_settings = callback_settings

        ## LOVE_ENGINE MODULE SETTINGS (e.g. love_engine.drop_params=True,..)
        love_engine_settings = config.get("love_engine_settings", None)
        if love_engine_settings is None:
            love_engine_settings = {}
        if love_engine_settings:
            # ANSI escape code for blue text
            blue_color_code = "\033[94m"
            reset_color_code = "\033[0m"
            for key, value in love_engine_settings.items():
                if key == "cache" and value is True:
                    print(f"{blue_color_code}\nSetting Cache on Proxy")  # noqa: T201
                    from love_engine.caching.caching import Cache

                    cache_params = {}
                    if "cache_params" in love_engine_settings:
                        cache_params_in_config = love_engine_settings["cache_params"]
                    ...
                    self._init_cache(
                        ...
                        enable_redis_auth_cache=love_engine_settings.get(
                    ...
                    if love_engine.cache is not None:
                        verbose_proxy_logger.debug(
                            f"{blue_color_code}Set Cache on LoveEngine Proxy{reset_color_code}"
                        )
                    ...
                    love_engine.guardrail_name_config_map = guardrail_name_config_map

                elif key == "global_prompt_directory":
                    from love_engine.integrations.dotprompt import (
                        set_global_prompt_directory,
                    )

                    set_global_prompt_directory(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global Prompt Directory on LoveEngine Proxy{reset_color_code}"
                    )
                elif key == "global_bitbucket_config":
                    from love_engine.integrations.bitbucket import (
                        set_global_bitbucket_config,
                    )

                    set_global_bitbucket_config(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global BitBucket Config on LoveEngine Proxy{reset_color_code}"
                    )
                elif key == "global_gitlab_config":
                    from love_engine.integrations.gitlab import set_global_gitlab_config

                    set_global_gitlab_config(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global Gitlab Config on LoveEngine Proxy{reset_color_code}"
                    )
                elif key == "priority_reservation_settings":
                    from love_engine.types.utils import PriorityReservationSettings

                    love_engine.priority_reservation_settings = PriorityReservationSettings(
                        **value
                    )
                elif key == "callbacks":
                    ...
                    from love_engine.types.router import ModelGroupSettings

                    love_engine.model_group_settings = ModelGroupSettings(**value)

                elif key == "post_call_rules":
                    love_engine.post_call_rules = [
                        get_instance_fn(value=value, config_file_path=config_file_path)
                    ]
                    verbose_proxy_logger.debug(
                        f"love_engine.post_call_rules: {love_engine.post_call_rules}"
                    )
                elif key == "max_budget":
                    love_engine.max_budget = float(value)
                elif key == "max_internal_user_budget":
                    love_engine.max_internal_user_budget = float(value)  # type: ignore
                elif key == "default_max_internal_user_budget":
                    love_engine.default_max_internal_user_budget = float(value)
                    if love_engine.max_internal_user_budget is None:
                        love_engine.max_internal_user_budget = (
                            love_engine.default_max_internal_user_budget
                        )
                elif key == "custom_provider_map":
                    from love_engine.utils import custom_llm_setup

                    love_engine.custom_provider_map = [
                        {
                    ...
                    for callback in value:
                        ...
                        # these are love_engine callbacks - "langfuse", "sentry", "wandb"
                        else:
                            love_engine.logging_callback_manager.add_love_engine_success_callback(
                                callback
                            )
                            if "prometheus" in callback:
                                from love_engine.integrations.prometheus import (
                                    PrometheusLogger,
                                )
                    ...
                    print(  # noqa: T201
                        f"{blue_color_code} Initialized Success Callbacks - {love_engine.success_callback} {reset_color_code}"
                    )
                    ...
                    print(  # noqa: T201
                        f"{blue_color_code} Initialized Failure Callbacks - {love_engine.failure_callback} {reset_color_code}"
                    )
                    ...
                    reset_audit_log_callback_cache()
                    love_engine.audit_log_callbacks = []

                    for callback in value:
                        if "." in callback:
                            love_engine.audit_log_callbacks.append(
                                get_instance_fn(
                                    value=callback,
                                    config_file_path=config_file_path,
                                )
                            ...
                            love_engine.audit_log_callbacks.append(callback)

                    _store_audit_logs = love_engine_settings.get(
                        "store_audit_logs", love_engine.store_audit_logs
                    )
                    if _store_audit_logs:
                        print(  # noqa: T201
                            f"{blue_color_code} Initialized Audit Log Callbacks - {love_engine.audit_log_callbacks} {reset_color_code}"
                        )
                    else:
                        verbose_proxy_logger.warning(
                            "'audit_log_callbacks' is configured but 'store_audit_logs' is not enabled. "
                            "Audit log callbacks will not fire until 'store_audit_logs: true' is added to love_engine_settings."
                        )
                elif key == "cache_params":
                    # this is set in the cache branch
                    # see usage here: https://docs.love_engine.ai/docs/proxy/caching
                    pass
                elif key == "responses":
                    ...
                    verbose_proxy_logger.debug(
                        f"{blue_color_code} setting love_engine.{key}={value}{reset_color_code}"
                    )
                    setattr(love_engine, key, value)
                    ...
                    if value is not None and isinstance(value, dict):
                        ...
                        love_engine.upperbound_key_generate_params = (
                            LOVE_ENGINE_UpperboundKeyGenerateParams(**value)
                        )
                    else:
                        raise Exception(
                    ...
                    love_engine.json_logs = True
                    love_engine._turn_on_json()
                    verbose_proxy_logger.debug(
                    ...
                    if key in {"s3_audit_callback_params", "s3_callback_params"}:
                        from love_engine.integrations.s3_v2 import S3Logger as S3V2Logger
                        from love_engine.love_engine_core_utils.love_engine_logging import (
                            _in_memory_loggers,
                        )
                        from love_engine.proxy.management_helpers.audit_logs import (
        ...
        ## GENERAL SERVER SETTINGS (e.g. master key,..) # do this after initializing love_engine, to ensure sentry logging works for proxylogging
        general_settings = config.get("general_settings", {})
        ...
        if general_settings:
            ...
            if key_management_settings is not None:
                love_engine._key_management_settings = KeyManagementSettings(
            ...
            if database_url and database_url.startswith("os.environ/"):
                verbose_proxy_logger.debug("GOING INTO LOVE_ENGINE.GET_SECRET!")
                database_url = get_secret(database_url)
                verbose_proxy_logger.debug("RETRIEVED DB URL: %s", database_url)
            ### MASTER KEY ###
            master_key = general_settings.get(
                "master_key", get_secret("LOVE_ENGINE_MASTER_KEY", None)
            )
            ...
            if master_key is not None and isinstance(master_key, str):
                love_engine_master_key_hash = hash_token(master_key)
            else:
                verbose_proxy_logger.critical(
                    "LOVE_ENGINE_MASTER_KEY is not set! All requests will be treated as INTERNAL_USER with no admin access. Set LOVE_ENGINE_MASTER_KEY for production use."
                )
            ...
            if user_api_key_cache_ttl is not None:
                ...
                # Mirror TTL on Redis as well when ``love_engine_settings.enable_redis_auth_cache``
            ...
            if use_pkce and redis_usage_cache is None:
                ...
                if not _pkce_no_redis_warning_emitted:
                    ...
                    verbose_proxy_logger.warning(
                        "GENERIC_CLIENT_USE_PKCE=true but Redis is not configured for LoveEngine caching. "
            ...
            if cp_url and redis_usage_cache is None:
                ...
                if not _cp_no_redis_warning_emitted:
                    ...
                    verbose_proxy_logger.warning(
                        "control_plane_url is configured but Redis is not configured for LoveEngine caching. "
            ...
            if allowed_ips is not None and premium_user is False:
                raise ValueError(
                    "allowed_ips is an Enterprise Feature. Please add a valid LOVE_ENGINE_LICENSE to your envionment."
                )
            ## BUDGET RESCHEDULER ##
            ...
            if _use_legacy_interactions_schema is not None:
                if isinstance(_use_legacy_interactions_schema, str):
                    love_engine.use_legacy_interactions_schema = (
                        _use_legacy_interactions_schema.lower() == "true"
                    )
                else:
                    love_engine.use_legacy_interactions_schema = bool(
            ...
            # check if love_engine_license in general_settings
            if "love_engine_license" in general_settings:
                _license_check.license_str = general_settings["love_engine_license"]
                premium_user = _license_check.is_premium()

        router_params: dict = {
            "cache_responses": love_engine.cache
        ...
        if model_list:
            ...
            print(  # noqa: T201
                "\033[32mLoveEngine: Proxy initialized with Config, Set models:\033[0m"
            )
            for model in model_list:
                ### LOAD FROM os.environ/ ###
                for k, v in model["love_engine_params"].items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        model["love_engine_params"][k] = get_secret(v)
                print(f"\033[32m    {model.get('model_name', '')}\033[0m")  # noqa: T201
                love_engine_model_name = model["love_engine_params"]["model"]
                love_engine_model_api_base = model["love_engine_params"].get("api_base", None)
                if "ollama" in love_engine_model_name and love_engine_model_api_base is None:
                    run_ollama_serve()
        ...
        if assistant_settings:
            for k, v in assistant_settings["love_engine_params"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    _v = v.replace("os.environ/", "")
                    v = os.getenv(_v)
                    assistant_settings["love_engine_params"][k] = v
        ...
        if router_settings and isinstance(router_settings, dict):
            ...
            available_args = [
                x for x in love_engine.Router.get_valid_args() if x not in exclude_args
            ]
        ...
        if prompts:
            from love_engine.proxy.prompts.init_prompts import init_prompts
        ...
        love_engine.credential_list = credential_list_dict
    ...
    async def _init_non_llm_configs(
        ...
        if mcp_tools_config:
            from love_engine.proxy._experimental.mcp_server.tool_registry import (
        ...
        if agent_config:
            from love_engine.proxy.agent_endpoints.agent_registry import (
        ...
        if mcp_servers_config:
            from love_engine.proxy._experimental.mcp_server.mcp_server_manager import (
                global_mcp_server_manager,
            )

            # Get mcp_aliases from love_engine_settings if available
            love_engine_settings = config.get("love_engine_settings", {})
            mcp_aliases = love_engine_settings.get("mcp_aliases", None)
        ...
        if vector_store_registry_config:
            from love_engine.vector_stores.vector_store_registry import VectorStoreRegistry

            if love_engine.vector_store_registry is None:
                love_engine.vector_store_registry = VectorStoreRegistry()

            # Load vector stores from config
            love_engine.vector_store_registry.load_vector_stores_from_config(
    ...
    async def _init_policy_engine(
        ...
        from love_engine.proxy.policy_engine.init_policies import init_policies
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        for _alert in _alerting_callbacks:
            if _alert == "slack":
                ...
                if _alert in love_engine._known_custom_logger_compatible_callbacks:
                    ...
                    if _logger is not None:
                        love_engine.logging_callback_manager.add_love_engine_callback(_logger)
        pass
    ...
    def initialize_secret_manager(
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                from love_engine.secret_managers.aws_secret_manager_v2 import (
                    AWSSecretsManagerV2,
                )

                AWSSecretsManagerV2.load_aws_secret_manager(
                    use_aws_secret_manager=True,
                    key_management_settings=love_engine._key_management_settings,
                )
                ...
                from love_engine.secret_managers.google_secret_manager import (
                    GoogleSecretManager,
                )

                GoogleSecretManager()
            elif key_management_system == KeyManagementSystem.HASHICORP_VAULT.value:
                from love_engine.secret_managers.hashicorp_secret_manager import (
                    HashicorpSecretManager,
                )

                HashicorpSecretManager()
            elif key_management_system == KeyManagementSystem.CYBERARK.value:
                from love_engine.secret_managers.cyberark_secret_manager import (
                ...
                from love_engine.secret_managers.custom_secret_manager_loader import (
    ...
    def get_model_info_with_id(self, model, db_model=False) -> RouterModelInfo:
        ...
        if premium_user is True:
            # seeing "created_at", "updated_at", "created_by", "updated_by" is a LoveEngine Enterprise Feature
            ...
            for model in model_list:
                ...
                if model_id is None:
                    ...
                    model_id = llm_router._generate_model_id(
                        ...
                        love_engine_params=model["love_engine_params"],
                    )
                else:
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        ## ADD MODEL LOGIC
        for m in db_models:
            _love_engine_params = m.love_engine_params
            if isinstance(_love_engine_params, dict):
                # decrypt values
                for k, v in _love_engine_params.items():
                    if isinstance(v, str):
                        ...
                        _love_engine_params[k] = _value
                _love_engine_params = LOVE_ENGINE_Params(**_love_engine_params)

            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid love_engine params. love_engine_params={_love_engine_params}"
                )
            ...
            added = llm_router.upsert_deployment(
                deployment=Deployment(
                    ...
                    love_engine_params=_love_engine_params,
    ...
    def decrypt_model_list_from_db(self, new_models: list) -> list:
        ...
        for m in new_models:
            _love_engine_params = m.love_engine_params
            if isinstance(_love_engine_params, BaseModel):
                _love_engine_params = _love_engine_params.model_dump()
            if isinstance(_love_engine_params, dict):
                # decrypt values
                for k, v in _love_engine_params.items():
                    ...
                    _love_engine_params[k] = decrypted_value
                _love_engine_params = LOVE_ENGINE_Params(**_love_engine_params)
            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid love_engine params. love_engine_params={_love_engine_params}"
    ...
    def _add_callback_from_db_to_in_memory_love_engine_callbacks(
        self,
        callback: str,
        ...
        """
        Helper method to add a single callback to love_engine for specified event types.
        ...
        """
        if callback in love_engine._known_custom_logger_compatible_callbacks:
            ...
            if event_types == ["success"]:
                love_engine.logging_callback_manager.add_love_engine_success_callback(callback)
            elif event_types == ["failure"]:
                love_engine.logging_callback_manager.add_love_engine_failure_callback(callback)
            else:  # Both success and failure
                love_engine.logging_callback_manager.add_love_engine_callback(callback)

    def _add_callbacks_from_db_config(self, config_data: dict) -> None:
        """
        Adds callbacks from DB config to love_engine
        """
        love_engine_settings = config_data.get("love_engine_settings", {}) or {}
        success_callbacks = love_engine_settings.get("success_callback", None)
        failure_callbacks = love_engine_settings.get("failure_callback", None)
        callbacks = love_engine_settings.get("callbacks", None)

        if success_callbacks is not None and isinstance(success_callbacks, list):
            for success_callback in success_callbacks:
                self._add_callback_from_db_to_in_memory_love_engine_callbacks(
                    callback=success_callback,
                    event_types=["success"],
                    existing_callbacks=love_engine.success_callback,
                )
        ...
        if failure_callbacks is not None and isinstance(failure_callbacks, list):
            for failure_callback in failure_callbacks:
                self._add_callback_from_db_to_in_memory_love_engine_callbacks(
                    ...
                    event_types=["failure"],
                    existing_callbacks=love_engine.failure_callback,
                )
        ...
        if callbacks is not None and isinstance(callbacks, list):
            for callback in callbacks:
                self._add_callback_from_db_to_in_memory_love_engine_callbacks(
                    ...
                    existing_callbacks=love_engine.callbacks,
                )
    ...
    async def _add_router_settings_from_db_config(
        ...
        """
        Adds router settings from DB config to love_engine proxy
        ...
        if llm_router is not None and prisma_client is not None:
            ...
            if (
                ...
                from love_engine.utils import _update_dictionary
    ...
    def _add_general_settings_from_db_config(
        ...
        """
        Adds general settings from DB config to love_engine proxy
    ...
    async def _reschedule_spend_log_cleanup_job(self):
        ...
        if retention_period is not None:
            from love_engine.proxy.db.db_transaction_queue.spend_log_cleanup import (
                SpendLogCleanup,
            )
            ...
            if cleanup_cron:
                ...
                from love_engine.love_engine_core_utils.duration_parser import (
                    duration_in_seconds,
                )
    ...
    def _update_config_fields(
        ...
        elif param_name == "love_engine_settings" and isinstance(db_param_value, dict):
            for key, value in db_param_value.items():
                if (
                    key in LOVE_ENGINE_SETTINGS_SAFE_DB_OVERRIDES
    ...
    async def _get_models_from_db(self, prisma_client: PrismaClient) -> Optional[list]:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy_server.py::add_deployment() - Error getting new models from DB - {}".format(
                    str(e)
                )
            )
            return None
    ...
    async def add_deployment(
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:add_deployment - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_semantic_filter_settings_in_db(self, prisma_client: PrismaClient):
        ...
        import love_engine
        from love_engine.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook

        try:
            config_record = await get_config_param(prisma_client, "love_engine_settings")

            if config_record is None or config_record.param_value is None:
                return

            love_engine_settings = config_record.param_value
            if isinstance(love_engine_settings, str):
                love_engine_settings = json.loads(love_engine_settings)

            mcp_semantic_filter_config = love_engine_settings.get(
            ...
            if hasattr(self, "_last_semantic_filter_config"):
                if self._last_semantic_filter_config == mcp_semantic_filter_config:
                    ...
                    active_hooks = (
                        love_engine.logging_callback_manager.get_custom_loggers_for_type(
            ...
            love_engine.logging_callback_manager.remove_callbacks_by_type(
                love_engine.callbacks, SemanticToolFilterHook
            )
            ...
            if mcp_semantic_filter_config.get("enabled", False):
                ...
                if hook:
                    love_engine.logging_callback_manager.add_love_engine_callback(hook)
                    verbose_proxy_logger.info(
    ...
    async def _init_sso_settings_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_sso_settings_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_hashicorp_vault_config_override(self, prisma_client: PrismaClient):
        ...
        """
        from love_engine.proxy.management_endpoints.config_override_endpoints import (
    ...
    async def _check_and_reload_model_cost_map(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if should_reload:
                # Perform the reload
                from love_engine.love_engine_core_utils.get_model_cost_map import (
                    get_model_cost_map,
                )

                model_cost_map_url = love_engine.model_cost_map_url
                new_model_cost_map = get_model_cost_map(url=model_cost_map_url)
                love_engine.model_cost = new_model_cost_map
                # Invalidate case-insensitive lookup map since model_cost was replaced
                _invalidate_model_cost_lowercase_map()
                # Repopulate provider model sets (e.g. love_engine.anthropic_models) so that
                # wildcard patterns like "anthropic/*" include any newly added models.
                love_engine.add_known_models(model_cost_map=new_model_cost_map)
    ...
    async def _check_and_reload_anthropic_beta_headers(
        ...
        try:
            ...
            if should_reload:
                # Perform the reload
                from love_engine.anthropic_beta_headers_manager import (
    ...
    def _get_prompt_spec_for_db_prompt(self, db_prompt):
        ...
        """
        from love_engine.proxy.prompts.prompt_endpoints import create_versioned_prompt_spec

        return create_versioned_prompt_spec(db_prompt=db_prompt)

    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        from love_engine.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - {}".format(
                    str(e)
                )
            )

    async def _init_guardrails_in_db(self, prisma_client: PrismaClient):
        from love_engine.proxy.guardrails.guardrail_registry import (
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_guardrails_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_policies_in_db(self, prisma_client: PrismaClient):
        ...
        from love_engine.proxy.policy_engine.policy_registry import get_policy_registry
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_policies_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_tool_policy_in_db(self, prisma_client: PrismaClient):
        ...
        """
        from love_engine.proxy.db.tool_registry_writer import get_tool_policy_registry
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_tool_policy_in_db - {}".format(
                    str(e)
                )
            )
        ...
        try:
            ...
            if love_engine.vector_store_registry is None:
                ...
                for vector_store in vector_stores:
                    love_engine.vector_store_registry.add_vector_store_to_registry(
                        vector_store=vector_store
                    )
        except Exception as e:
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - {}".format(
                    str(e)
                )
            )

    async def _init_vector_store_indexes_in_db(self, prisma_client: PrismaClient):
        from love_engine.vector_stores.vector_store_registry import VectorStoreIndexRegistry
        ...
        try:
            ...
            if love_engine.vector_store_index_registry is None:
                love_engine.vector_store_index_registry = VectorStoreIndexRegistry(
                    vector_store_indexes=vector_store_indexes
                )
            else:
                for vector_store_index in vector_store_indexes:
                    love_engine.vector_store_index_registry.upsert_vector_store_index(
    ...
    async def _init_mcp_servers_in_db(self):
        from love_engine.proxy._experimental.mcp_server.utils import is_mcp_available
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_agents_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_agents_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_search_tools_in_db(self, prisma_client: PrismaClient):
        ...
        from love_engine.proxy.search_endpoints.search_tool_registry import (
            SearchToolRegistry,
        )
        from love_engine.router_utils.search_api_router import SearchAPIRouter
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.py::ProxyConfig:_init_search_tools_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def delete_credentials(self, db_credentials: List[CredentialItem]):
        ...
        Compare to the love_engine.credential_list
        Delete any from love_engine.credential_list that are not in the all-up list
        """
        ## CONFIG credentials ##
        config = await self.get_config(config_file_path=user_config_file_path)
        ...
        ## DELETE ##
        idx_to_delete = []
        for idx, credential in enumerate(love_engine.credential_list):
            if credential.credential_name not in [
                cred.credential_name for cred in combined_list
            ]:
                idx_to_delete.append(idx)
        for idx in sorted(idx_to_delete, reverse=True):
            love_engine.credential_list.pop(idx)
    ...
    async def get_credentials(self, prisma_client: PrismaClient):
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy_server.py::get_credentials() - Error getting credentials from DB - {}".format(
                    str(e)
                )
            )
            return []
...
async def initialize(
    ...
    from love_engine.proxy.common_utils.banner import show_banner

    show_banner()
    if os.getenv("LOVE_ENGINE_DONT_SHOW_FEEDBACK_BOX", "").lower() != "true":
    ...
    if debug is True:  # this needs to be first, so users can see Router init debugg
        ...
        from love_engine._logging import (
            verbose_logger,
    ...
    if detailed_debug is True:
        ...
        # users can control proxy debugging using env variable = 'LOVE_ENGINE_LOG'
        love_engine_log_setting = os.environ.get("LOVE_ENGINE_LOG", "")
        if love_engine_log_setting is not None:
            if love_engine_log_setting.upper() == "INFO":
                import logging
            ...
            elif love_engine_log_setting.upper() == "DEBUG":
                import logging
    ...
    if api_version:
        os.environ["AZURE_API_VERSION"] = (
            api_version  # set this for azure - love_engine can read this from the env
        )
    ...
    if drop_params is True:  # love_engine-specific param
        love_engine.drop_params = True
        dynamic_config["general"]["drop_params"] = True
    if add_function_to_prompt is True:  # love_engine-specific param
        love_engine.add_function_to_prompt = True
        dynamic_config["general"]["add_function_to_prompt"] = True
    if max_budget:  # love_engine-specific param
        love_engine.max_budget = float(max_budget)
        dynamic_config["general"]["max_budget"] = love_engine.max_budget
    if experimental:
        pass
...
async def async_assistants_data_generator(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.async_assistants_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        ...
        verbose_proxy_logger.debug(
            f"\033[1;31mAn error occurred: {e}\n\n Debug this by setting `--debug`, e.g. `love_engine --model gpt-3.5-turbo --debug`"
        )
...
def _get_client_requested_model_for_streaming(request_data: dict) -> str:
    ...
    """
    requested_model = request_data.get("_love_engine_client_requested_model")
...
def _restamp_streaming_chunk_model(
    *,
    ...
    if not model_mismatch_logged and downstream_model != requested_model_from_client:
        verbose_proxy_logger.debug(
            "love_engine_call_id=%s: streaming chunk model mismatch - requested=%r downstream=%r. Overriding model to requested.",
            request_data.get("love_engine_call_id"),
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine_call_id=%s: failed to override chunk.model=%r on chunk_type=%s. error=%s",
            request_data.get("love_engine_call_id"),
            requested_model_from_client,
            type(chunk),
            str(e),
...
async def _apply_streaming_chunk_hooks(
    *,
    ...
    if isinstance(chunk, (ModelResponse, ModelResponseStream)):
        response_str = love_engine.get_response_string(response_obj=chunk)
...
async def async_data_generator(
    ...
    try:
        ...
        is_raw_sse_stream = bool(request_data.get("_love_engine_raw_sse_stream"))
        raw_sse_buffer = ""
        ...
        if not request_data.get("_love_engine_skip_openai_stream_done"):
            done_message = "[DONE]"
            yield f"data: {done_message}\n\n"
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.async_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        await proxy_logging_obj.post_call_failure_hook(
...
def get_love_engine_model_info(model: dict = {}):
    model_info = model.get("model_info", {})
    model_to_lookup = model.get("love_engine_params", {}).get("model", None)
    try:
        if "azure" in model_to_lookup or model_info.get("base_model"):
            model_to_lookup = model_info.get("base_model", None)
        love_engine_model_info = love_engine.get_model_info(model_to_lookup)
        return love_engine_model_info
    except Exception:
        # this should not block returning on /model/info
        # if love_engine does not have info on the model it should return {}
        return {}
...
class ProxyStartupEvent:
    ...
    def _validate_redis_transaction_buffer_config(
        ...
        a Redis cache is properly configured in love_engine_settings.
        """
        from love_engine.secret_managers.main import str_to_bool
        ...
        if _use_redis_transaction_buffer and redis_usage_cache is None:
            raise ValueError(
                ...
                "to not be tracked. Add a Redis cache in love_engine_settings:\n\n"
                "love_engine_settings:\n"
                "  cache: true\n"
                "  cache_params:\n"
                "    type: redis\n"
    ...
    def _get_transaction_buffer_redis_cache(
        ...
        """
        from love_engine._redis import _redis_kwargs_from_environment
        from love_engine.secret_managers.main import str_to_bool
    ...
    async def _initialize_semantic_tool_filter(
        cls,
        llm_router: Optional[Router],
        love_engine_settings: Dict[str, Any],
    ):
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("love_engine_jwtauth", None) is not None:
            for k, v in general_settings["love_engine_jwtauth"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    general_settings["love_engine_jwtauth"][k] = get_secret(v)
            ...
            love_engine_jwtauth = LOVE_ENGINE_JWTAuth(
                config_file_path=user_config_file_path,
                **general_settings["love_engine_jwtauth"],
            )
        else:
            love_engine_jwtauth = LOVE_ENGINE_JWTAuth()
    ...
    @classmethod
    def _add_proxy_budget_to_db(cls, love_engine_proxy_budget_name: str):
        """Adds a global proxy budget to db"""
        if love_engine.budget_duration is None:
            raise Exception(
                "budget_duration not set on Proxy. budget_duration is required to use max_budget."
            )

        asyncio.create_task(
            cls._upsert_proxy_budget_with_reset_at_backfill(love_engine_proxy_budget_name)
        )

    @classmethod
    async def _upsert_proxy_budget_with_reset_at_backfill(
        cls, love_engine_proxy_budget_name: str
    ) -> None:
        """
        ...
        await generate_key_helper_fn(  # type: ignore
            request_type="user",
            table_name="user",
            user_id=love_engine_proxy_budget_name,
            duration=None,
            models=[],
            aliases={},
            config={},
            ...
            budget_duration=love_engine.budget_duration,
        ...
        # took the UPDATE path, and reset_budget_for_love_engine_users never
        # matches them (NULL < now() is unknown in SQL) — so the proxy-wide
        # spend cap blocks forever once it's hit.
        if prisma_client is not None and love_engine.budget_duration is not None:
            try:
    ...
    async def _warm_global_spend_cache(
        cls,
        love_engine_proxy_admin_name: str,
        user_api_key_cache: UserApiKeyCache,
        prisma_client: PrismaClient,
    ) -> None:
        """Warm global spend cache once at startup to reduce impact of first wave of requests."""
        try:
            cache_key = "{}:spend".format(love_engine_proxy_admin_name)
    ...
    async def _update_default_team_member_budget(cls):
        ...
        if love_engine.default_internal_user_params is None:
            return

        _teams = love_engine.default_internal_user_params.get("teams") or []
    ...
    async def initialize_scheduled_background_jobs(
        ...
        from love_engine.proxy.utils import update_daily_tag_spend
        ...
        if general_settings.get("disable_spend_logs", False) is False:
            from love_engine.proxy.utils import _monitor_spend_logs_queue
        ...
        if llm_router is not None and PROXY_BATCH_POLLING_ENABLED:
            try:
                from love_engine_enterprise.proxy.common_utils.check_batch_cost import (
                    CheckBatchCost,
                )
                ...
                verbose_proxy_logger.debug(
                    "Checking batch cost for LoveEngine Managed Files is an Enterprise Feature. Skipping..."
                )
                pass
        ...
        if llm_router is not None and PROXY_BATCH_POLLING_ENABLED:
            try:
                from love_engine_enterprise.proxy.common_utils.check_responses_cost import (
                    CheckResponsesCost,
                )
                ...
                verbose_proxy_logger.debug(
                    "Checking responses cost for LoveEngine Managed Files is an Enterprise Feature. Skipping..."
                )
                pass
    ...
    async def _initialize_spend_tracking_background_jobs(
        ...
        from love_engine.proxy.spend_tracking.cloudzero_endpoints import is_cloudzero_setup
        ...
        from love_engine.proxy.spend_tracking.vantage_endpoints import (
        ...
        ########################################################
        from love_engine.integrations.mavvrik_focus.mavvrik_focus_logger import (  # noqa: PLC0415
            MavvrikFocusLogger,
        )
        ...
        ########################################################
        if love_engine.prometheus_initialize_budget_metrics is True:
        ...
        ########################################################
        from love_engine.constants import (
            LOVE_ENGINE_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
            LOVE_ENGINE_KEY_ROTATION_ENABLED,
        )

        key_rotation_enabled: Optional[bool] = str_to_bool(LOVE_ENGINE_KEY_ROTATION_ENABLED)
        verbose_proxy_logger.debug(f"key_rotation_enabled: {key_rotation_enabled}")

        if key_rotation_enabled is True:
            try:
                from love_engine.proxy.common_utils.key_rotation_manager import (
                    KeyRotationManager,
                )
                ...
                if prisma_client is not None:
                    ...
                    verbose_proxy_logger.debug(
                        f"Key rotation background job scheduled every {LOVE_ENGINE_KEY_ROTATION_CHECK_INTERVAL_SECONDS} seconds (LOVE_ENGINE_KEY_ROTATION_ENABLED=true)"
                    )
                    scheduler.add_job(
                        key_rotation_manager.process_rotations,
                        "interval",
                        seconds=LOVE_ENGINE_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
                        id="key_rotation_job",
                    )
                else:
                    verbose_proxy_logger.warning(
            ...
            verbose_proxy_logger.debug(
                "Key rotation disabled (set LOVE_ENGINE_KEY_ROTATION_ENABLED=true to enable)"
            )
    ...
    async def _initialize_expired_ui_session_key_cleanup_background_job(
        ...
        from love_engine.constants import (
            ...
            LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED,
            LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS,
        )
        ...
        if expired_ui_session_key_cleanup_enabled is True:
            try:
                from love_engine.proxy.common_utils.expired_ui_session_key_cleanup_manager import (
                ...
                if prisma_client is not None:
                    ...
                    verbose_proxy_logger.debug(
                        ...
                        "every "
                        f"{LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS} "
                        "seconds "
                        "(LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED=true)"
                    )
                    scheduler.add_job(
                        expired_ui_session_key_cleanup_manager.cleanup_expired_keys,
                        "interval",
                        seconds=LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS,
            ...
            verbose_proxy_logger.debug(
                ...
                "LOVE_ENGINE_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED=true to enable)"
            )
    ...
    def _init_dd_tracer(cls):
        ...
        """
        from love_engine.love_engine_core_utils.dd_tracing import (
    ...
    def _init_pyroscope(cls):
        ...
        Off by default. Enable with LOVE_ENGINE_ENABLE_PYROSCOPE=true.
        ...
        """
        if not get_secret_bool("LOVE_ENGINE_ENABLE_PYROSCOPE", False):
            verbose_proxy_logger.debug(
                "LoveEngine: Pyroscope profiling is disabled (set LOVE_ENGINE_ENABLE_PYROSCOPE=true to enable)."
            )
            return
        try:
            import pyroscope
            ...
            msg = (
                f"LoveEngine: Pyroscope profiling started (app_name={app_name}, server_address={server_address}). "
            ...
            verbose_proxy_logger.warning(
                "LoveEngine: LOVE_ENGINE_ENABLE_PYROSCOPE is set but the 'pyroscope-io' package is not installed. "
...
async def model_list(
    ...
    if should_expand_scope:
        ...
        from love_engine.proxy.auth.model_checks import get_complete_model_list
...
async def model_info(
    ...
    # Use the actual love_engine model from the deployment to get provider info
    _, provider, _, _ = love_engine.get_llm_provider(model=deployment.love_engine_params.model)
...
async def chat_completion(
    ...
    try:
        ...
        _chat_response = love_engine.ModelResponse()
        ...
        if data.get("stream", None) is not None and data["stream"] is True:
            _iterator = love_engine.utils.ModelResponseIterator(
                model_response=_chat_response, convert_to_delta=True
            )
            _streaming_response = love_engine.CustomStreamWrapper(
                completion_stream=_iterator,
                model=e.model,
                custom_llm_provider="cached_response",
                logging_obj=data.get("love_engine_logging_obj", None),
            )
        ...
        _usage = love_engine.Usage(prompt_tokens=0, completion_tokens=0, total_tokens=0)
...
async def completion(
    ...
    try:
        ...
        if _data.get("stream", None) is not None and _data["stream"] is True:
            _text_response = love_engine.ModelResponse()
            ...
            _streaming_response = love_engine.TextCompletionStreamWrapper(
                completion_stream=_iterator,
                model=e.model,
            )
            ...
            _response = love_engine.TextCompletionResponse()
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.completion(): Exception occured - {}".format(
                str(e)
            )
        )
        error_msg = f"{str(e)}"
        ...
        if (
            ...
            if llm_router is not None and data.get("model") in router_model_names:
                ...
                if deployment is not None:
                    love_engine_params = deployment.get("love_engine_params", {}) or {}
                    love_engine_model = love_engine_params.get("model", "")
                    # Check if this provider supports token arrays
                    supports_token_arrays = love_engine_model in love_engine.open_ai_embedding_models or any(
                        love_engine_model.startswith(provider)
                        for provider in LOVE_ENGINE_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS
                    )
                    if not supports_token_arrays:
                        ...
                        for i in data["input"]:
                            input_list.append(
                                love_engine.decode(model="gpt-3.5-turbo", tokens=i)
                            )
...
async def moderations(
    ...
    try:
        ...
        data = await add_love_engine_data_to_request(
            data=data,
            request=request,
            general_settings=general_settings,
            user_api_key_dict=user_api_key_dict,
        ...
        asyncio.create_task(
            proxy_logging_obj.update_request_status(
                love_engine_call_id=data.get("love_engine_call_id", ""), status="success"
            )
        )
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.moderations(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def audio_speech(
    ...
    try:
        ...
        love_engine_call_id = hidden_params.get("love_engine_call_id", None) or ""
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.audio_speech(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def audio_transcriptions(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.audio_transcription(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def get_assistants(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as LoveEngine adds more providers for acreate_batch
        if llm_router is None:
            raise HTTPException(
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.get_assistants(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.create_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.delete_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_threads(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.create_threads(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.get_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def add_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.add_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.get_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def run_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.run_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
from love_engine.llms.base_llm.base_utils import BaseTokenCounter
...
def _get_provider_token_counter(
    ...
    full_model = deployment.get("love_engine_params", {}).get("model", "")
    model: Optional[str] = None
    custom_llm_provider: Optional[str] = None

    try:
        # Use existing LoveEngine logic to determine provider
        model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
            model=full_model,
            custom_llm_provider=deployment.get("love_engine_params", {}).get(
                "custom_llm_provider"
            ),
            api_base=deployment.get("love_engine_params", {}).get("api_base"),
            api_key=deployment.get("love_engine_params", {}).get("api_key"),
        )
...
async def token_counter(request: TokenCountRequest, call_endpoint: bool = False):
    ...
    Returns:
        TokenCountResponse
    """
    from love_engine import token_counter
    ...
    if llm_router is not None:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "love_engine.proxy.proxy_server.token_counter(): Exception occured while getting deployment"
            )
            pass
    if deployment is not None:
        love_engine_model_name = deployment.get("love_engine_params", {}).get("model")
        model_info = deployment.get("model_info", {})
        load_credentials_from_list(deployment.get("love_engine_params", {}))
        # remove the custom_llm_provider_prefix in the love_engine_model_name
        if "/" in love_engine_model_name:
            love_engine_model_name = love_engine_model_name.split("/", 1)[1]

    model_to_use: str = (
        love_engine_model_name or request.model
    )  # use love_engine model name, if it's not avalable then fallback to request.model
    ...
    # Default LoveEngine token counting
    ...
    _tokenizer_used = love_engine.utils._select_tokenizer(
...
async def supported_openai_params(model: str):
    """
    Returns supported openai params for a given love_engine model name
    ...
    """
    try:
        model, custom_llm_provider, _, _ = love_engine.get_llm_provider(model=model)
        return {
...
async def transform_request(request: TransformRequestBody):
    from love_engine.utils import return_raw_request
...
def _check_if_model_is_team_model(
    models: List[DeploymentTypedDict], user_row: LOVE_ENGINE_UserTable
) -> List[Dict]:
    """
...
async def non_admin_all_models(
    ...
    unique_models = _deduplicate_love_engine_router_models(models=all_models)
    return unique_models


def _add_team_models_to_all_models(
    team_db_objects_typed: List[LOVE_ENGINE_TeamTable],
    llm_router: Router,
) -> Dict[str, Set[str]]:
    """
...
async def _add_access_group_models_to_team_models(
    ...
    eligible_teams: List[LOVE_ENGINE_TeamTable] = []
...
async def get_all_team_models(
    ...
    if user_teams == "*":
        ...
        team_db_objects_typed = [
            LOVE_ENGINE_TeamTable(**team_db_object.model_dump())
...
def get_direct_access_models(
    user_db_object: LOVE_ENGINE_UserTable,
    llm_router: Router,
) -> List[str]:
    """
...
async def _populate_team_access_on_models(
    ...
    direct_access_models: List[str] = []
    if user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN:
        user_teams = "*"
        ...
        if user_db_object is not None:
            user_object = LOVE_ENGINE_UserTable(**user_db_object.model_dump())
...
def _enrich_model_info_with_love_engine_data(
    model: Dict[str, Any], debug: bool = False, llm_router: Optional[Router] = None
) -> Dict[str, Any]:
    """
    Enrich a model dictionary with love_engine model info (pricing, context window, etc.)
    ...
    # read love_engine model_prices_and_context_window.json to get the following:
    # input_cost_per_token, output_cost_per_token, max_tokens
    love_engine_model_info = get_love_engine_model_info(model=model)

    # 2nd pass on the model, try seeing if we can find model in love_engine model_cost map
    if love_engine_model_info == {}:
        # use love_engine_param model_name to get model_info
        love_engine_params = model.get("love_engine_params", {})
        love_engine_model = love_engine_params.get("model", None)
        try:
            love_engine_model_info = love_engine.get_model_info(model=love_engine_model)
        except Exception:
            love_engine_model_info = {}
    # 3rd pass on the model, try seeing if we can find model but without the "/" in model cost map
    if love_engine_model_info == {}:
        # use love_engine_param model_name to get model_info
        love_engine_params = model.get("love_engine_params", {})
        love_engine_model = love_engine_params.get("model", None)
        if love_engine_model:
            split_model = love_engine_model.split("/")
            if len(split_model) > 0:
                love_engine_model = split_model[-1]
            try:
                love_engine_model_info = love_engine.get_model_info(
                    model=love_engine_model, custom_llm_provider=split_model[0]
                )
            except Exception:
                love_engine_model_info = {}
    for k, v in love_engine_model_info.items():
    ...
    model = remove_sensitive_info_from_deployment(
        model, excluded_keys={"love_engine_credential_name"}
    )
    return model
...
async def _get_caller_byok_team_scope(
    ...
    if user_api_key_dict.user_role in (
        LoveEngineUserRoles.PROXY_ADMIN,
        LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY,
    ):
        return None
...
async def _load_team_object_for_model_filter(
...
) -> Optional[LOVE_ENGINE_TeamTable]:
    ...
    try:
        ...
        if team_db_object is None:
            ...
            return None
        return LOVE_ENGINE_TeamTable(**team_db_object.model_dump())
    except Exception as e:
...
async def _gather_team_accessible_model_ids(
    team_object: LOVE_ENGINE_TeamTable,
    team_id: str,
    prisma_client: PrismaClient,
    llm_router: Router,
) -> Set[str]:
...
async def model_info_v2(
    ...
    Query parameters:
        ...
        modelId: Return a single deployment by LoveEngine model id.
    ...
    ```json
    {
        "data": [
            {
                "model_name": "gpt-4",
                "love_engine_params": {"model": "openai/gpt-4.1"},
                "model_info": {
                    "id": "abc123",
    ...
    if modelId is not None:
        ...
        if user_model is not None:
            # if user does not use a config.yaml, https://github.com/BerriAI/love_engine/issues/2061
    ...
    # Fill in model info based on config.yaml and love_engine model_prices_and_context_window.json
    ...
    for i, _model in enumerate(all_models):
        all_models[i] = _enrich_model_info_with_love_engine_data(
            model=_model,
    ...
    from love_engine.proxy.agent_endpoints.model_list_helpers import (
...
async def model_metrics_exceptions(
    ...
    sql_query = """
        WITH cte AS (
            SELECT
                CASE WHEN api_base = '' THEN love_engine_model_name ELSE CONCAT(love_engine_model_name, '-', api_base) END AS combined_model_api_base,
                exception_type,
                COUNT(*) AS num_rate_limit_exceptions
            FROM "LOVE_ENGINE_ErrorLogs"
            WHERE
...
def _get_proxy_model_info(model: dict) -> dict:
    ...
    # don't return the llm credentials
    model = remove_sensitive_info_from_deployment(
        deployment_dict=model, excluded_keys={"love_engine_credential_name"}
    )
...
async def model_info_v1(
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    love_engine_model_id: Optional[str] = None,
    include_team_models: Optional[bool] = fastapi.Query(
        False,
    ...
    Parameters:
        love_engine_model_id: Optional[str] = None (this is the value of `x-love_engine-model-id` returned in response headers)

        - When love_engine_model_id is passed, it will return the info for that specific model
        - When love_engine_model_id is not passed, it will return the info for all models
    ...
    if user_model is not None:
        # user is trying to get specific model from love_engine router
        try:
            model_info: Dict = cast(Dict, love_engine.get_model_info(model=user_model))
        except Exception:
            model_info = {}
        ...
        _deployment_info_dict = remove_sensitive_info_from_deployment(
            ...
            excluded_keys={"love_engine_credential_name"},
        )
    ...
    if llm_model_list is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Model List not loaded in. Make sure you passed models in your config.yaml or on the LoveEngine Admin UI. - https://docs.love_engine.ai/docs/proxy/configs"
            },
        )
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Router is not loaded in. Make sure you passed models in your config.yaml or on the LoveEngine Admin UI. - https://docs.love_engine.ai/docs/proxy/configs"
            },
        )
    ...
    if love_engine_model_id is not None:
        # user is trying to get specific model from love_engine router
        deployment_info = llm_router.get_deployment(model_id=love_engine_model_id)
        if deployment_info is None:
            raise HTTPException(
                status_code=400,
                detail={
                    "error": f"Model id = {love_engine_model_id} not found on love_engine proxy"
                },
            )
    ...
    all_models = [
        _translate_model_name_for_response(
            _enrich_model_info_with_love_engine_data(model=model, llm_router=llm_router)
        )
        for model in all_models
    ]
...
def _get_model_group_info(
    ...
    if love_engine.public_model_groups is not None:
        for mg in model_groups:
            if mg.model_group in love_engine.public_model_groups:
...
async def model_group_info(
    ...
    """
    Get information about all the deployments on love_engine proxy, including config.yaml descriptions (except api key and api base)
    ...
    Learn how to use and set wildcard models [here](https://docs.love_engine.ai/docs/wildcard_routing)
    ...
    from love_engine.proxy.utils import get_available_models_for_user
...
async def model_settings():
    ...
    for provider in love_engine.provider_list:
        returned_list.append(
            ProviderInfo(
                name=provider,
                fields=love_engine.get_provider_fields(custom_llm_provider=provider),
            )
        )
...
async def async_queue_request(
    ...
    try:
        ...
        fastapi_response.headers.update({"x-love_engine-priority": str(data["priority"])})
        return response
    except Exception as e:
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict, original_exception=e, request_data=data
...
async def fallback_login(request: Request):
    ...
    PROXY_BASE_URL should be the your deployed proxy endpoint, e.g. PROXY_BASE_URL="https://love_engine-production-7002.up.railway.app/"
    Example:
    """
    from love_engine.proxy.proxy_server import ui_link
...
async def login(request: Request):
    ...
    from love_engine.proxy.auth.login_utils import authenticate_user, create_ui_token_object
    from love_engine.proxy.utils import get_custom_url
    ...
    # Build redirect URL
    love_engine_dashboard_ui = get_custom_url(str(request.base_url))
    if love_engine_dashboard_ui.endswith("/"):
        love_engine_dashboard_ui += "ui/"
    else:
        love_engine_dashboard_ui += "/ui/"
    love_engine_dashboard_ui += "?login=success"

    # Create redirect response with cookie
    redirect_response = RedirectResponse(url=love_engine_dashboard_ui, status_code=303)
...
async def login_v2(request: Request):
    ...
    try:
        ...
        json_response = JSONResponse(
            content={"redirect_url": love_engine_dashboard_ui, "token": jwt_token},
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.login_v2(): Exception occurred - {}".format(
                str(e)
            )
        )
...
async def login_v3(request: Request):
    ...
    try:
        ...
        cache_value = {"token": jwt_token, "redirect_url": love_engine_dashboard_ui}
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.login_v3(): Exception occurred - {}".format(
                str(e)
            )
        )
        if isinstance(e, ProxyException):
...
async def login_v3_exchange(request: Request):
    try:
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.login_v3_exchange(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise ProxyException(
...
async def onboarding(invite_link: str, request: Request):
    ...
    from love_engine.types.proxy.ui_sso import ReturnedUITokenObject

    if master_key is None:
        raise ProxyException(
            message="Master Key not set for Proxy. Please set Master Key to use Admin UI. Set `LOVE_ENGINE_MASTER_KEY` in .env or set general_settings:master_key in config.yaml.  https://docs.love_engine.ai/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
            type=ProxyErrorTypes.auth_error,
            param="master_key",
            code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    ...
    utc_now_date = love_engine.utils.get_utc_datetime().date()
    ...
    love_engine_dashboard_ui = get_custom_url(str(request.base_url))
    if love_engine_dashboard_ui.endswith("/"):
        love_engine_dashboard_ui += "ui/onboarding"
    else:
        love_engine_dashboard_ui += "/ui/onboarding"
    import jwt
    ...
    onboarding_token = jwt.encode(  # type: ignore
        {
            "token_type": "love_engine_onboarding",
            "invitation_link": invite_link,
            "user_id": user_obj.user_id,
            "exp": love_engine.utils.get_utc_datetime() + timedelta(minutes=15),
        },
        master_key,
        algorithm="HS256",
    )
    ...
    returned_ui_token_object = ReturnedUITokenObject(
        ...
        auth_header_name=general_settings.get(
            "love_engine_key_header_name", "Authorization"
        ),
    ...
    love_engine_dashboard_ui += "?token={}&user_email={}".format(jwt_token, user_email)
    return {
        "login_url": love_engine_dashboard_ui,
        "token": jwt_token,
...
def _get_onboarding_claims_from_request(request: Request) -> dict:
    ...
    auth_header_name = general_settings.get("love_engine_key_header_name", "Authorization")
    ...
    try:
        await InvitationLinkRepository(prisma_client).table.update_many(
            ...
            data={
                "accepted_at": None,
                "is_accepted": False,
                "updated_at": love_engine.utils.get_utc_datetime(),
                "updated_by": user_id,
            },
        )
    except Exception:
...
async def _generate_onboarding_ui_session_token(user_obj: Any) -> str:
    ...
    response = await generate_key_helper_fn(
        ...
        **{
            "user_role": user_obj.user_role,
            "duration": LOVE_ENGINE_UI_SESSION_DURATION,
            "key_max_budget": love_engine.max_ui_session_budget,
            "models": [],
            "aliases": {},
            "config": {},
            "spend": 0,
...
async def claim_onboarding_link(data: InvitationClaim, request: Request):
    ...
    if (
        onboarding_claims.get("token_type") != "love_engine_onboarding"
    ...
    async with prisma_client.db.tx() as tx:
        updated_count = await tx.love_engine_invitationlink.update_many(
        ...
        ### UPDATE USER OBJECT ###
        user_obj = await tx.love_engine_usertable.update(
        ...
        #### MARK LINK AS USED
        current_time = love_engine.utils.get_utc_datetime()
        await tx.love_engine_invitationlink.update(
            where={"id": data.invitation_link},
            data={
...
async def get_image():
    ...
    # Priority: LOVE_ENGINE_ASSETS_PATH env var > default based on is_non_root
    default_assets_dir = "/var/lib/love_engine/assets" if is_non_root else current_dir
    assets_dir = os.getenv("LOVE_ENGINE_ASSETS_PATH", default_assets_dir)
    ...
    from love_engine.proxy.common_utils.static_asset_utils import (
...
async def get_favicon():
    ...
    favicon_url = os.getenv("LOVE_ENGINE_FAVICON_URL", "")
    ...
    if favicon_url.startswith(("http://", "https://")):
        ...
        verbose_proxy_logger.warning(
            "LOVE_ENGINE_FAVICON_URL %r is not a supported image file or does not "
...
async def new_invitation(
    ...
    curl -X POST 'http://localhost:4000/invitation/new' \
        -H 'Content-Type: application/json' \
        -d '{
            "user_id": "1234" // 👈 id of user in 'LOVE_ENGINE_UserTable'
        }'
    ```
    """
    try:
        from love_engine.proxy.management_helpers.user_invitation import (
        ...
        has_access = (
            user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN
...
async def invitation_update(
    ...
    curl -X POST 'http://localhost:4000/invitation/update' \
        -H 'Content-Type: application/json' \
        -d '{
            "invitation_id": "1234" // 👈 id of invitation in 'LOVE_ENGINE_InvitationTable'
...
async def invitation_delete(
    ...
    is_proxy_admin = user_api_key_dict.user_role == LoveEngineUserRoles.PROXY_ADMIN
...
async def update_config(
    ...
    Writes only the sections present in the request body to LOVE_ENGINE_Config rows
    ...
    try:
        ...
        # love_engine_settings: merge existing + request, request wins (matching
        ...
        if config_info.love_engine_settings is not None:
            existing = await _read_section("love_engine_settings")
            updated_love_engine_settings = dict(config_info.love_engine_settings)

            incoming_cb = updated_love_engine_settings.get("success_callback")
            if isinstance(incoming_cb, list):
                updated_love_engine_settings["success_callback"] = normalize_callback_names(
                    incoming_cb
                )

            merged = {**existing, **updated_love_engine_settings}
            ...
            await _upsert_section("love_engine_settings", merged)
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.update_config(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def update_config_general_settings(
    ...
    """
    Update a specific field in love_engine general settings
    """
    global prisma_client
    ## VALIDATION ##
    """
...
async def delete_config_general_settings(
    ...
    """
    Delete the db value of this field in love_engine general settings. Resets it to it's initial default value on love_engine.
    """
    global prisma_client
    ## VALIDATION ##
    """
...
async def delete_callback(
    ...
    try:
        ...
        love_engine_settings = config.get("love_engine_settings", {})
        success_callbacks = love_engine_settings.get("success_callback", [])
        ...
        config.setdefault("love_engine_settings", {})[
            "success_callback"
        ] = success_callbacks
        ...
        verbose_proxy_logger.error(
            f"love_engine.proxy.proxy_server.delete_callback(): Exception occurred - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise ProxyException(
...
async def get_config():
    ...
    try:
        ...
        config_data = await proxy_config.get_config()
        _love_engine_settings = config_data.get("love_engine_settings", {})
        _general_settings = config_data.get("general_settings", {})
        environment_variables = config_data.get("environment_variables", {})

        _success_callbacks = _love_engine_settings.get("success_callback", [])
        _failure_callbacks = _love_engine_settings.get("failure_callback", [])
        _success_and_failure_callbacks = _love_engine_settings.get("callbacks", [])
        ...
        verbose_proxy_logger.exception(
            "love_engine.proxy.proxy_server.get_config(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def config_yaml_endpoint(config_info: ConfigYAML):
    ...
    - **model_list**: *Optional[List[ModelParams]]* - A list of supported models on the server, along with model-specific configurations. ModelParams includes "model_name" (name of the model), "love_engine_params" (love_engine-specific parameters for the model), and "model_info" (additional info about the model such as id, mode, cost per token, etc).

    - **love_engine_settings**: *Optional[dict]*: Settings for the love_engine module. You can specify multiple properties like "drop_params", "set_verbose", "api_base", "cache".
...
async def reload_model_cost_map(
    ...
    try:
        ...
        from love_engine.love_engine_core_utils.get_model_cost_map import get_model_cost_map
...
async def get_model_cost_map_source(
    ...
    - is_env_forced: true if LOVE_ENGINE_LOCAL_MODEL_COST_MAP=True forced local usage
    ...
    try:
        ...
        model_count = len(love_engine.model_cost) if love_engine.model_cost else 0
...
async def reload_anthropic_beta_headers(
    ...
    try:
        ...
        from love_engine.anthropic_beta_headers_manager import reload_beta_headers_config
...
async def get_routes():
    ...
    """
    from love_engine.proxy.common_utils.get_routes import GetRoutes
...
#         scopes=["love_engine_proxy_admin"],
#     )
...
async def toolset_mcp_route(toolset_name: str, request: Request):
    ...
    try:
        ...
        from love_engine.proxy._experimental.mcp_server.server import (
...
async def _resolve_mcp_csv_tokens(
    ...
    """
    from love_engine.constants import DEFAULT_MCP_NAMESPACE_CSV_MAX_TOKENS
    from love_engine.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
...
async def _is_mcp_access_group_cached(name: str) -> bool:
    ...
    from love_engine.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import (
        MCPRequestHandler,
    )
...
async def dynamic_mcp_route(mcp_server_name: str, request: Request):
    ...
    try:
        ...
        from love_engine.proxy.auth.ip_address_utils import IPAddressUtils
