...
from love_engine.types.proxy.discovery_endpoints.ui_discovery_endpoints import (
...
@router.get("/.well-known/love_engine-ui-config", response_model=UiDiscoveryEndpoints)
@router.get(
    "/love_engine/.well-known/love_engine-ui-config", response_model=UiDiscoveryEndpoints
)  # if mounted at root path
async def get_ui_config():
    from love_engine.proxy.auth.auth_utils import _has_user_setup_sso
    from love_engine.proxy.utils import get_proxy_base_url, get_server_root_path
    ...
    hide_default_credentials_hint = bool(
        os.getenv("LOVE_ENGINE_HIDE_DEFAULT_CREDENTIALS_HINT", "false").lower() == "true"
