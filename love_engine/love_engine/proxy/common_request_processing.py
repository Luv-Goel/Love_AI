...
from love_engine.constants import (
    ...
    LOVE_ENGINE_DETAILED_TIMING,
    LOVE_ENGINE_HTTP_STATUS_CLIENT_DISCONNECTED,
    MAX_PAYLOAD_SIZE_FOR_DEBUG_LOG,
    STREAM_SSE_DATA_PREFIX,
)
from love_engine.integrations.custom_guardrail import CustomGuardrail
from love_engine.love_engine_core_utils.dd_tracing import NullTracer, tracer
...
from love_engine.proxy._types import ProxyException, UserAPIKeyAuth
from love_engine.proxy.auth.auth_utils import check_response_size_is_safe
from love_engine.proxy.common_utils.callback_utils import (
    get_logging_caching_headers,
    get_remaining_tokens_and_requests_from_request_data,
)
from love_engine.proxy.dd_span_tagger import DDSpanTagger
from love_engine.proxy.route_llm_request import route_request
from love_engine.proxy.utils import ProxyLogging
from love_engine.router import Router
from love_engine.types.guardrails import GuardrailEventHooks
from love_engine.types.router import RouterRateLimitError
from love_engine.types.utils import ServerToolUse
...
_CLIENT_DISCONNECTED_ERROR_INFORMATION: StandardLoggingPayloadErrorInformation = {
    "error_code": str(LOVE_ENGINE_HTTP_STATUS_CLIENT_DISCONNECTED),
...
async def _record_streaming_client_disconnect_if_needed(
    ...
    if logging_obj is not None:
        love_engine_params = logging_obj.model_call_details.setdefault("love_engine_params", {})
        _apply_client_disconnect_metadata(love_engine_params.setdefault("metadata", {}))
    ...
    love_engine_params = request_data.setdefault("love_engine_params", {})
    _apply_client_disconnect_metadata(love_engine_params.setdefault("metadata", {}))

    verbose_proxy_logger.debug(
        "Recorded streaming client disconnect with error_code=499 for love_engine_call_id=%s",
...
async def _authorize_response_file_search_vector_stores(
    ...
    from love_engine.proxy.vector_store_endpoints.utils import (
...
def _override_openai_response_model(
    *,
    ...
    LoveEngine internally prefixes some provider/deployment model identifiers (e.g. `hosted_vllm/...`).
    ...
    Exceptions:
    1. If a fallback occurred (indicated by x-love_engine-attempted-fallbacks header),
    ...
    if isinstance(hidden_params, dict):
        ...
        attempted_fallbacks = fallback_headers.get(
            "x-love_engine-attempted-fallbacks", None
        )
        ...
        if hidden_params.get("fastest_response_batch_completion"):
            winning_model = fallback_headers.get("x-love_engine-model-group")
            if winning_model:
                verbose_proxy_logger.debug(
...
def _get_cost_breakdown_from_logging_obj(
    love_engine_logging_obj: Optional[LoveEngineLoggingObj],
    ...
    """
    if not love_engine_logging_obj or not hasattr(love_engine_logging_obj, "cost_breakdown"):
        return None, None, None, None

    cost_breakdown = love_engine_logging_obj.cost_breakdown
...
def _has_attribute_error_in_chain(exc: Exception) -> bool:
    ...
    Checks __cause__, __context__, and the love_engine-specific original_exception
...
def _log_llm_api_exception(e: Exception) -> None:
    if (
        ...
        verbose_proxy_logger.info(
            "love_engine.proxy.proxy_server._handle_llm_api_exception(): client disconnected, upstream LLM request cancelled"
        )
        return
    verbose_proxy_logger.exception(
        f"love_engine.proxy.proxy_server._handle_llm_api_exception(): Exception occured - {str(e)}"
    )
...
class ProxyBaseLLMRequestProcessing:
    ...
    def get_custom_headers(
        *,
        ...
        love_engine_logging_obj: Optional[LoveEngineLoggingObj] = None,
        **kwargs,
    ) -> dict:
        ...
        headers = {
            "x-love_engine-call-id": call_id,
            "x-love_engine-model-id": model_id,
            "x-love_engine-cache-key": cache_key,
            "x-love_engine-model-api-base": (
                api_base.split("?")[0] if api_base else None
            ),  # don't include query params, risk of leaking sensitive info
            "x-love_engine-version": version,
            "x-love_engine-model-region": model_region,
            "x-love_engine-response-cost": str(response_cost),
            "x-love_engine-response-cost-original": (
                str(original_cost) if original_cost is not None else None
            ),
            "x-love_engine-response-cost-discount-amount": (
                str(discount_amount) if discount_amount is not None else None
            ),
            "x-love_engine-response-cost-margin-amount": (
                str(margin_total_amount) if margin_total_amount is not None else None
            ),
            "x-love_engine-response-cost-margin-percent": (
                str(margin_percent) if margin_percent is not None else None
            ),
            "x-love_engine-key-tpm-limit": str(user_api_key_dict.tpm_limit),
            "x-love_engine-key-rpm-limit": str(user_api_key_dict.rpm_limit),
            "x-love_engine-key-max-budget": str(user_api_key_dict.max_budget),
            "x-love_engine-key-spend": str(updated_spend),
            "x-love_engine-response-duration-ms": str(
                hidden_params.get("_response_ms", None)
            ),
            "x-love_engine-overhead-duration-ms": str(
                hidden_params.get("love_engine_overhead_time_ms", None)
            ),
            "x-love_engine-callback-duration-ms": str(
                hidden_params.get("callback_duration_ms", None)
            ),
            **(
                {
                    "x-love_engine-timing-pre-processing-ms": str(
                        hidden_params.get("timing_pre_processing_ms", None)
                    ),
                    "x-love_engine-timing-llm-api-ms": str(
                        hidden_params.get("timing_llm_api_ms", None)
                    ),
                    "x-love_engine-timing-post-processing-ms": str(
                        hidden_params.get("timing_post_processing_ms", None)
                    ),
                    "x-love_engine-timing-message-copy-ms": str(
                        hidden_params.get("timing_message_copy_ms", None)
                    ),
                }
                if LOVE_ENGINE_DETAILED_TIMING
                else {}
            ),
            "x-love_engine-fastest_response_batch_completion": (
                str(fastest_response_batch_completion)
                if fastest_response_batch_completion is not None
                else None
            ),
            "x-love_engine-timeout": str(timeout) if timeout is not None else None,
    ...
    @staticmethod
    async def build_love_engine_proxy_success_headers_from_llm_response(
        *,
        response: Any,
        request_data: dict,
        request: Request,
        ...
        """
        Build LoveEngine proxy response headers for routes that call the LLM directly
        ...
        custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=logging_obj.love_engine_call_id,
            model_id=model_id,
            cache_key=cache_key,
            api_base=api_base,
            version=version,
            ...
            love_engine_logging_obj=logging_obj,
            **additional_headers,
        )
    ...
    async def common_processing_pre_call_logic(
    ...
    ) -> Tuple[dict, LoveEngineLoggingObj]:
        start_time = datetime.now()  # start before calling guardrail hooks

        self.data = await add_love_engine_data_to_request(
            data=self.data,
            request=request,
        ...
        # Calculate request queue time after add_love_engine_data_to_request
        ...
        # Store queue time in metadata after add_love_engine_data_to_request to ensure it's preserved
        if queue_time_seconds is not None:
            from love_engine.proxy.love_engine_pre_call_utils import _get_metadata_variable_name
        ...
        if (
            ...
            and self.data["model"] in love_engine.model_alias_map
        ):
            self.data["model"] = love_engine.model_alias_map[self.data["model"]]
        ...
        self.data["love_engine_call_id"] = request.headers.get(
            "x-love_engine-call-id", str(uuid.uuid4())
        )
        DDSpanTagger.tag_call_id(self.data.get("love_engine_call_id"))
        ...
        logging_obj, self.data = love_engine.utils.function_setup(
        ...
        self.data["love_engine_logging_obj"] = logging_obj
    ...
    def _get_model_id_from_response(hidden_params: dict, data: dict) -> str:
        """Extract model_id from hidden_params with fallback to love_engine_metadata."""
        model_id = hidden_params.get("model_id", None) or ""
        if not model_id:
            love_engine_metadata = data.get("love_engine_metadata", {}) or {}
            model_info = love_engine_metadata.get("model_info", {}) or {}
    ...
    def _debug_log_request_payload(self) -> None:
        ...
        if len(_payload_str) > MAX_PAYLOAD_SIZE_FOR_DEBUG_LOG:
            verbose_proxy_logger.debug(
                "Request received by LoveEngine: payload too large to log (%d bytes, limit %d). Keys: %s",
                len(_payload_str),
                MAX_PAYLOAD_SIZE_FOR_DEBUG_LOG,
                (
                    list(self.data.keys())
            ...
            verbose_proxy_logger.debug(
                "Request received by LoveEngine:\n%s",
                _payload_str,
            )
    ...
    async def base_process_llm_request(
        ...
        if skip_pre_call_logic:
            logging_obj = self.data.get("love_engine_logging_obj")
            if logging_obj is None:
                raise ValueError(
                    "skip_pre_call_logic=True requires love_engine_logging_obj to be set in data. "
        ...
        # Cache the result to avoid scanning love_engine.callbacks twice.
        ...
        try:
            ...
            asyncio.create_task(
                proxy_logging_obj.update_request_status(
                    love_engine_call_id=self.data.get("love_engine_call_id", ""),
                    status="success",
                )
            )
            if self._is_streaming_request(
                ...
                if requested_model_from_client:
                    self.data["_love_engine_client_requested_model"] = (
                ...
                from love_engine.love_engine_core_utils.streaming_handler import (
                    CustomStreamWrapper,
                )
        ...
        if requested_model_from_client:
            _override_openai_response_model(
                ...
                log_context=f"love_engine_call_id={logging_obj.love_engine_call_id}",
            )
    ...
    async def _record_container_owners_from_responses_if_needed(
        ...
        from love_engine.proxy.container_endpoints.ownership import (
    ...
    async def _wrap_responses_stream_for_container_ownership(
        ...
        ``LOVE_ENGINE_ManagedObjectTable`` row for any container created
    ...
    def _has_post_call_guardrails_for_passthrough(self) -> bool:
        ...
        from love_engine.proxy.utils import _check_and_merge_model_level_guardrails
    ...
    def _flush_deferred_async_logging(
        ...
        returns before the deferred block in love_engine/utils.py), so this is a
        no-op there.
    ...
    async def _run_deferred_stream_guardrails(
        ...
        try:
            from love_engine.proxy.proxy_server import llm_router as _global_llm_router
            from love_engine.proxy.utils import _check_and_merge_model_level_guardrails
            ...
            try:
                ...
                # _is_sync_love_engine_request (which only recognizes a subset of
                # async markers stored in love_engine_params).
                asyncio.create_task(
    ...
    async def _handle_llm_api_exception(
        ...
        model_id = self.maybe_get_model_id(_love_engine_logging_obj)

        custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=(
                _love_engine_logging_obj.love_engine_call_id
                if _love_engine_logging_obj
                else self.data.get("love_engine_call_id")
            ),
            model_id=model_id,
            version=version,
            response_cost=0,
            ...
            request_data=self.data,
            timeout=timeout,
            love_engine_logging_obj=_love_engine_logging_obj,
        )
    ...
    async def async_streaming_data_generator(
        ...
        *,
        ...
        cost_injection_enabled = bool(
            getattr(love_engine, "include_cost_in_streaming_usage", False)
        )
        fast_path = (
    ...
    def _process_chunk_with_cost_injection(chunk: Any, model_name: str) -> Any:
        ...
        """
        if not getattr(love_engine, "include_cost_in_streaming_usage", False):
            return chunk
    ...
    def _inject_cost_into_usage_dict(obj: dict, model_name: str) -> Optional[dict]:
        ...
        if obj.get("type") == "message_delta" and isinstance(obj.get("usage"), dict):
            ...
            try:
                cost_val = love_engine.completion_cost(
    ...
    def maybe_get_model_id(
        self, _logging_obj: Optional[LoveEngineLoggingObj]
    ) -> Optional[str]:
        """
        ...
        model_id = None
        if _logging_obj:
            # 1. Try getting from love_engine_params (updated during call)
            if hasattr(_logging_obj, "love_engine_params") and _logging_obj.love_engine_params:
                # First check direct model_info path (set by router.py with selected deployment)
                model_info = _logging_obj.love_engine_params.get("model_info") or {}
                model_id = model_info.get("id", None)

                # Fallback to nested metadata path
                if not model_id:
                    metadata = _logging_obj.love_engine_params.get("metadata") or {}
            ...
            if not model_id:
                ...
                if _kwargs:
                    love_engine_params = _kwargs.get("love_engine_params", {})
                    # First check direct model_info path
                    model_info = love_engine_params.get("model_info") or {}
                    model_id = model_info.get("id", None)
        ...
        # 3. Final fallback to self.data["love_engine_metadata"] (for routes like /v1/responses that populate data before error)
        if not model_id:
            love_engine_metadata = self.data.get("love_engine_metadata", {}) or {}
            model_info = love_engine_metadata.get("model_info", {}) or {}
            model_id = model_info.get("id", None)
