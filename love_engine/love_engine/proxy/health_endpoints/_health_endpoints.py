...
from love_engine.constants import HEALTH_CHECK_TIMEOUT_SECONDS
from love_engine.love_engine_core_utils.custom_logger_registry import CustomLoggerRegistry
...
from love_engine.proxy.health_check import (
    ...
    _update_love_engine_params_for_health_check,
...
async def health_services_endpoint(
    ...
    try:
        ...
        if service in love_engine.success_callback:
            service_in_success_callbacks = True
        else:
            for cb in love_engine.success_callback:
        ...
        if (
            ...
            _ = await love_engine.acompletion(
                model="openai/love_engine-mock-response-model",
                messages=[{"role": "user", "content": "Hey, how's it going?"}],
                user="love_engine:/health/services",
            ...
            from love_engine.integrations.datadog.datadog_metrics import (
            ...
            from love_engine.integrations.langfuse.langfuse import LangFuseLogger

            langfuse_logger = LangFuseLogger()
            langfuse_logger.Langfuse.auth_check()
            _ = love_engine.completion(
                model="openai/love_engine-mock-response-model",
                messages=[{"role": "user", "content": "Hey, how's it going?"}],
                user="love_engine:/health/services",
                mock_response="This is a mock response",
        ...
        if service == "slack" or service == "slack_budget_alerts":
            if "slack" in general_settings.get("alerting", []):
                # test_message = f"""\n🚨 `ProjectedLimitExceededError` 💸\n\n`Key Alias:` love_engine-ui-test-alert \n`Expected Day of Error`: 28th March \n`Current Spend`: $100.00 \n`Projected Spend at end of month`: $1000.00 \n`Soft Limit`: $700"""
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.health_services_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
def _build_model_param_to_info_mapping(model_list: list) -> dict:
    ...
    for model in model_list:
        ...
        model_param = love_engine_params.get("model")
...
def _is_proxy_admin(user_api_key_dict: UserAPIKeyAuth) -> bool:
    ...
    user_role on UserAPIKeyAuth can be either a LoveEngineUserRoles enum or its
...
def _resolve_targeted_model_ids(
    ...
    ``love_engine_params.model`` provider string. ``model_id`` matches
    ``model_info.id``.
    ...
    for m in model_list:
        ...
        if model_id and deployment_id == model_id:
            ...
            continue
        if model:
            love_engine_model = (m.get("love_engine_params") or {}).get("model")
            if m.get("model_name") == model or love_engine_model == model:
                target_ids.add(deployment_id)
    return target_ids
...
async def health_endpoint(
    ...
    See more 👉 https://docs.love_engine.ai/docs/proxy/health
    ...
    def _post_process(result: dict) -> dict:
        ...
        if is_admin:
            return result
        response.headers["LoveEngine-Health-Field-Notice"] = (
    ...
    try:
        if llm_model_list is None:
            # if no router set, check if user set a model using love_engine --model ollama/llama2
        ...
        verbose_proxy_logger.error(
            "love_engine.proxy.proxy_server.py::health_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def shared_health_check_status_endpoint(
    ...
    """
    from love_engine.proxy.proxy_server import redis_usage_cache, use_shared_health_check
...
def _read_license_data() -> Optional[Dict[str, Any]]:
    from love_engine.proxy.proxy_server import _license_check, premium_user_data
...
async def health_license_endpoint(
    ...
    """Return metadata about the configured LoveEngine license without exposing the key."""
    from love_engine.proxy.proxy_server import _license_check, premium_user
...
async def active_callbacks():
    """
    Returns a list of love_engine level settings
    ...
    {
        "alerting": _alerting,
        "love_engine.callbacks": love_engine_callbacks,
        "love_engine.input_callback": love_engine_input_callbacks,
        "love_engine.failure_callback": love_engine_failure_callbacks,
        "love_engine.success_callback": love_engine_success_callbacks,
        "love_engine._async_success_callback": love_engine_async_success_callbacks,
        "love_engine._async_failure_callback": love_engine_async_failure_callbacks,
        "love_engine._async_input_callback": love_engine_async_input_callbacks,
        "all_love_engine_callbacks": all_love_engine_callbacks,
        "num_callbacks": len(all_love_engine_callbacks),
        "num_alerting": _num_alerting,
        "love_engine.request_timeout": love_engine.request_timeout,
    }
    ```
    """
    ...
    love_engine_callbacks = [str(x) for x in love_engine.callbacks]
    love_engine_input_callbacks = [str(x) for x in love_engine.input_callback]
    love_engine_failure_callbacks = [str(x) for x in love_engine.failure_callback]
    love_engine_success_callbacks = [str(x) for x in love_engine.success_callback]
    love_engine_async_success_callbacks = [str(x) for x in love_engine._async_success_callback]
    love_engine_async_failure_callbacks = [str(x) for x in love_engine._async_failure_callback]
    love_engine_async_input_callbacks = [str(x) for x in love_engine._async_input_callback]

    all_love_engine_callbacks = (
        love_engine_callbacks
        + love_engine_input_callbacks
        + love_engine_failure_callbacks
        + love_engine_success_callbacks
        + love_engine_async_success_callbacks
        + love_engine_async_failure_callbacks
        + love_engine_async_input_callbacks
    )
...
async def _get_health_readiness_details(
    ...
    """
    from love_engine.proxy.proxy_server import prisma_client, version
    ...
    try:
        ...
        try:
            ...
            success_callback_names = [
                callback_name(x) for x in love_engine.success_callback
            ]
        except AttributeError:
            # don't let this block the /health/readiness response, if we can't convert to str -> return love_engine.success_callback
            success_callback_names = love_engine.success_callback

        # check Cache
        cache_type: Any = None
        if love_engine.cache is not None:
            from love_engine.caching.caching import RedisSemanticCache

            cache_type = love_engine.cache.type

            if isinstance(love_engine.cache.cache, RedisSemanticCache):
                # ping the cache
                # TODO: @ishaan-jaff - we should probably not ping the cache on every /health/readiness check
                index_info: Any
                try:
                    index_info = await love_engine.cache.cache._index_info()
                except Exception as e:
...
async def health_backlog():
    ...
    for the event loop to get to them, adding latency before LoveEngine even starts
    its own timer.
    """
...
@router.get(
    "/health/liveliness",  # Historical LoveEngine name; doesn't match k8s terminology but kept for backwards compatibility
    tags=["health"],
)
@router.get(
...
async def test_model_connection(
    ...
    love_engine_params: Dict = fastapi.Body(
        None,
        description="Parameters for love_engine.completion, love_engine.embedding for the health check",
    ),
    ...
    from love_engine.proxy.proxy_server import llm_router, premium_user, prisma_client
    from love_engine.types.router import Deployment, LOVE_ENGINE_Params
    ...
    try:
        ...
        # Get model name from love_engine_params
        request_love_engine_params = love_engine_params or {}
        ...
        _reject_os_environ_references(request_love_engine_params)
        model_name = request_love_engine_params.get("model")

        # Look up model configuration from router if model name is provided
        # This gets the love_engine_params from proxy config (with resolved env vars)
        config_love_engine_params: dict = {}
        if llm_router is not None:
            ...
            try:
                ...
                if deployment_by_id is not None:
                    config_love_engine_params = deployment_by_id.love_engine_params.model_dump(
                        exclude_none=True
                    )
                elif model_name:
                    ...
                    # If not found, try to find by love_engine model name
                    # (e.g., "azure/gpt-4o")
                    if not deployments or len(deployments) == 0:
                        ...
                        if all_deployments:
                            for deployment in all_deployments:
                                if (
                                    deployment.get("love_engine_params", {}).get("model")
                                    == model_name
                                ):
                    ...
                    if deployments and len(deployments) > 0:
                        # Use the first deployment's love_engine_params as base
                        # config. These already have resolved environment
                        # variables from proxy config.
                        config_love_engine_params = dict(
                            deployments[0].get("love_engine_params", {})
                        )
            except Exception as e:
        ...
        love_engine_params = {**config_love_engine_params, **request_love_engine_params}

        ## Auth check
        await ModelManagementAuthChecks.can_user_make_model_call(
            model_params=Deployment(
                model_name="test_model",
                love_engine_params=LOVE_ENGINE_Params(**love_engine_params),
        ...
        love_engine_params = _update_love_engine_params_for_health_check(
            model_info={},
            love_engine_params=love_engine_params,
        )
        mode = mode or love_engine_params.pop("mode", None)

        result = await run_with_timeout(
            love_engine.ahealth_check(
                model_params=love_engine_params,
                mode=mode,
                prompt="test from love_engine",
                input=["test from love_engine"],
            ),
        ...
        cleaned_result = _clean_endpoint_data(
            {**love_engine_params, **result}, details=True
        )
        ...
        verbose_proxy_logger.debug(
            f"love_engine.proxy.health_endpoints.test_model_connection(): Exception occurred - {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
