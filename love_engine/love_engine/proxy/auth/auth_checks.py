...
from love_engine.proxy._types import (
    ...
    LOVE_ENGINE_ProjectTableCachedObj,
...
from love_engine.proxy.spend_tracking.budget_reservation import get_budget_window_start
...
from love_engine.proxy.db.exception_handler import PrismaDBExceptionHandler
from love_engine.proxy.guardrails.tool_name_extraction import (
    TOOL_CAPABLE_CALL_TYPES,
    extract_request_tool_names,
)
from love_engine.proxy.route_llm_request import route_request
from love_engine.proxy.utils import PrismaClient, ProxyLogging, log_db_metrics
from love_engine.repositories.budget_repository import BudgetRepository
from love_engine.repositories.object_permission_repository import ObjectPermissionRepository
...
from love_engine.utils import get_utc_datetime
...
all_routes = LoveEngineRoutes.openai_routes.value + LoveEngineRoutes.management_routes.value
...
def _is_model_cost_zero(
    ...
    Args:
        ...
        llm_router: The LoveEngine router instance
    ...
    for model_name in model_list:
        ...
        try:
            ...
            # See: https://github.com/BerriAI/love_engine/issues/24770
...
def _is_cost_explicitly_configured(model: str, llm_router: "Router") -> bool:
    ...
    set in its love_engine.model_cost entry.
    ...
    for deployment in llm_router.model_list:
        ...
        if model_id is None:
            continue
        raw_entry = love_engine.model_cost.get(model_id, {})
...
async def _run_project_checks(
    project_object: Optional[LOVE_ENGINE_ProjectTableCachedObj],
...
def _enforce_user_param_check(
    ...
    is_mcp_route = (
        route in LoveEngineRoutes.mcp_routes.value
        or RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.mcp_routes.value
        )
    )
...
def _global_proxy_budget_check(
    ...
    if (
        ...
        if (
            math.isfinite(love_engine.max_budget)
            and global_proxy_spend > love_engine.max_budget
        ):
            raise love_engine.BudgetExceededError(
                current_cost=global_proxy_spend, max_budget=love_engine.max_budget
            )
...
def _guardrail_modification_check(
    request_body: dict, team_object: Optional[LOVE_ENGINE_TeamTable]
) -> None:
    """
    ...
    """
    from love_engine.proxy.guardrails.guardrail_helpers import can_modify_guardrails
    ...
    def _coerce_to_dict(container: Any) -> Optional[dict]:
        ...
        no-modification, and ``add_love_engine_data_to_request`` parses the string
    ...
    modifies = (
        ...
        or _user_requested_modification(request_body.get("love_engine_metadata"))
...
async def check_tools_allowlist(
    ...
    """
    from love_engine.love_engine_core_utils.api_route_to_call_types import (
...
async def common_checks(
    ...
    project_object: Optional[LOVE_ENGINE_ProjectTableCachedObj] = None,
) -> bool:
    """
    ...
    7. [OPTIONAL] If 'love_engine.max_budget' is set (>0), is proxy under budget
    ...
    if _model and team_object:
        with tracer.trace("love_engine.proxy.auth.common_checks.can_team_access_model"):
            try:
    ...
    if _model and team_object and valid_token and valid_token.user_id:
        with tracer.trace(
            "love_engine.proxy.auth.common_checks.check_team_member_model_access"
        ):
    ...
    if valid_token is not None and valid_token.agent_id:
        from love_engine.proxy.agent_endpoints.agent_registry import global_agent_registry
        from love_engine.proxy.love_engine_pre_call_utils import get_chain_id_from_headers

        agent = global_agent_registry.get_agent_by_id(agent_id=valid_token.agent_id)
        if agent is not None:
            require_trace_id = (agent.love_engine_params or {}).get(
            ...
            if require_trace_id:
                ...
                if not trace_id:
                    raise ProxyException(
                        message="Requests made with this agent's key must include the x-love_engine-trace-id header.",
                        type=ProxyErrorTypes.bad_request_error,
                        param=None,
                        code=status.HTTP_400_BAD_REQUEST,
                    )
    ...
    if _model and team_object is None and user_object is not None:
        with tracer.trace("love_engine.proxy.auth.common_checks.can_user_call_model"):
    ...
    with tracer.trace("love_engine.proxy.auth.common_checks.run_project_checks"):
    ...
    if not skip_budget_checks:
        # 3. If team is in budget
        with tracer.trace("love_engine.proxy.auth.common_checks.team_max_budget_check"):
        ...
        with tracer.trace("love_engine.proxy.auth.common_checks.team_multi_budget_check"):
            await _team_multi_budget_check(team_object=team_object)

        # 3.2. Multi-window budget check for key
        with tracer.trace(
            "love_engine.proxy.auth.common_checks.virtual_key_multi_budget_check"
        ):
        ...
        with tracer.trace("love_engine.proxy.auth.common_checks.team_soft_budget_check"):
        ...
        with tracer.trace(
            "love_engine.proxy.auth.common_checks.organization_max_budget_check"
        ):
        ...
        if valid_token is not None:
            ...
            LoveEngineProxyRequestSetup.apply_key_tags_pre_auth(
                request_data=request_body,
                user_api_key_dict=valid_token,
            )

        with tracer.trace("love_engine.proxy.auth.common_checks.tag_max_budget_check"):
        ...
        if (
            ...
            from love_engine.proxy.proxy_server import get_current_spend
        ...
        with tracer.trace("love_engine.proxy.auth.common_checks.check_team_member_budget"):
        ...
        if (
            ...
            and end_user_object.love_engine_budget_table is not None
        ):
    ...
    # 11. [OPTIONAL] Vector store checks - is the object allowed to access the vector store
    with tracer.trace("love_engine.proxy.auth.common_checks.vector_store_access_check"):
    ...
    with tracer.trace("love_engine.proxy.auth.common_checks.check_tools_allowlist"):
...
def _get_user_role(
    user_obj: Optional[LOVE_ENGINE_UserTable],
    ...
    try:
        role = LoveEngineUserRoles(_user_role)
    except ValueError:
        return LoveEngineUserRoles.INTERNAL_USER
...
def _is_api_route_allowed(
    ...
    request_data: dict,
    valid_token: Optional[UserAPIKeyAuth],
    user_obj: Optional[LOVE_ENGINE_UserTable] = None,
) -> bool:
    """
...
def _is_user_proxy_admin(user_obj: Optional[LOVE_ENGINE_UserTable]):
    if user_obj is None:
        return False

    if (
        user_obj.user_role is not None
        and user_obj.user_role == LoveEngineUserRoles.PROXY_ADMIN.value
    ):
        return True
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|LoveEngineRoutes] - the list of allowed routes for the user.
    """
    from starlette.routing import compile_path

    for allowed_route in allowed_routes:
        if allowed_route in LoveEngineRoutes.__members__:
            for template in LoveEngineRoutes[allowed_route].value:
...
def allowed_routes_check(
    user_role: LoveEngineUserRoles,
    user_route: str,
    love_engine_proxy_roles: LOVE_ENGINE_JWTAuth,
) -> bool:
    """
    Check if user -> not admin - allowed to access these routes
    """

    if user_role == LoveEngineUserRoles.PROXY_ADMIN:
        is_allowed = _allowed_routes_check(
            user_route=user_route,
            allowed_routes=love_engine_proxy_roles.admin_allowed_routes,
        )
        return is_allowed

    elif user_role == LoveEngineUserRoles.TEAM:
        if love_engine_proxy_roles.team_allowed_routes is None:
            """
            ...
            return is_allowed
        elif love_engine_proxy_roles.team_allowed_routes is not None:
            is_allowed = _allowed_routes_check(
                user_route=user_route,
                allowed_routes=love_engine_proxy_roles.team_allowed_routes,
            )
            return is_allowed
    return False
...
def allowed_route_check_inside_route(
    ...
    if (
        ...
        and user_api_key_dict.user_role != LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY
    ):
        ret_val = False
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = LoveEngineRoutes[route_name].value
...
async def get_default_end_user_budget(
...
) -> Optional[LOVE_ENGINE_BudgetTable]:
    """
    Fetches the default end user budget from the database if love_engine.max_end_user_budget_id is configured.
    ...
    Returns:
        LOVE_ENGINE_BudgetTable if configured and found, None otherwise
    """
    if prisma_client is None or love_engine.max_end_user_budget_id is None:
        return None

    cache_key = f"default_end_user_budget:{love_engine.max_end_user_budget_id}"

    # Check cache first
    cached_budget = await user_api_key_cache.async_get_cache(
        key=cache_key,
        model_type=LOVE_ENGINE_BudgetTable,
    )
    ...
    try:
        budget_record = await BudgetRepository(prisma_client).table.find_unique(
            where={"budget_id": love_engine.max_end_user_budget_id}
        )

        if budget_record is None:
            verbose_proxy_logger.warning(
                f"Default end user budget not found in database: {love_engine.max_end_user_budget_id}"
            )
            return None

        _budget_obj = LOVE_ENGINE_BudgetTable(**budget_record.dict())
...
async def get_team_member_default_budget(
    ...
    Returns:
        LOVE_ENGINE_BudgetTable if found, None otherwise
    """
    ...
    if isinstance(cached_budget, LOVE_ENGINE_BudgetTable):
        return cached_budget
    if isinstance(cached_budget, dict):
        return LOVE_ENGINE_BudgetTable(**cached_budget)
    ...
    try:
        ...
        return LOVE_ENGINE_BudgetTable(**budget_record.dict())
...
async def _apply_default_budget_to_end_user(
    end_user_obj: LOVE_ENGINE_EndUserTable,
    ...
    if end_user_obj.love_engine_budget_table is not None:
        return end_user_obj

    # If no default budget configured, return as-is
    if love_engine.max_end_user_budget_id is None:
        return end_user_obj
    ...
    if default_budget is not None:
        ...
        end_user_obj.love_engine_budget_table = default_budget
        verbose_proxy_logger.debug(
            f"Applied default budget {love_engine.max_end_user_budget_id} to end user {end_user_obj.user_id}"
        )
...
async def _check_end_user_budget(
    ...
    Raises:
        love_engine.BudgetExceededError: If end user has exceeded their budget
    """
    if RouteChecks.is_info_route(route):
        return

    if end_user_obj.love_engine_budget_table is None:
        return

    end_user_budget = end_user_obj.love_engine_budget_table.max_budget
...
async def get_end_user_object(
    ...
    Returns:
        LOVE_ENGINE_EndUserTable if found, None otherwise
    """
    if prisma_client is None:
    ...
    cached_user_obj = await user_api_key_cache.async_get_cache(
        key=_key,
        model_type=LOVE_ENGINE_EndUserTable,
    )
    ...
    try:
        response = await EndUserRepository(prisma_client).table.find_unique(
            ...
            include={"love_engine_budget_table": True, "object_permission": True},
        )

        if response is None:
            raise Exception

        # Convert to LOVE_ENGINE_EndUserTable object
        _response = LOVE_ENGINE_EndUserTable(**response.dict())
...
async def resolve_and_validate_end_user_id(
    ...
    Default: pass-through. LoveEngine's documented pattern is that the `user`
    field is an arbitrary caller-supplied identifier, so validation is
    opt-in behind ``love_engine.validate_end_user_id_in_db`` to preserve
    backwards compatibility.

    When the flag is set: accept the id when it matches any of
      - LOVE_ENGINE_EndUserTable.user_id
      - LOVE_ENGINE_UserTable.user_id
      - LOVE_ENGINE_UserTable.user_email (case-insensitive)

    If the id doesn't match but ``love_engine.max_end_user_budget_id`` is set,
    ...
    if raw_end_user_id is None:
        return None
    if not love_engine.validate_end_user_id_in_db:
    ...
    if cached == "valid":
        return raw_end_user_id
    if cached == "invalid":
        return raw_end_user_id if love_engine.max_end_user_budget_id else None
    ...
    if is_valid:
        return raw_end_user_id
    # Preserve id so the caller can still apply love_engine.max_end_user_budget_id.
    if love_engine.max_end_user_budget_id:
        return raw_end_user_id
    return None
...
async def get_tag_objects_batch(
    ...
    Returns:
        Dictionary mapping tag_name to LOVE_ENGINE_TagTable object
    """
    if prisma_client is None:
        return {}
    ...
    for tag_name in tag_names:
        ...
        cached_tag = await user_api_key_cache.async_get_cache(
            key=cache_key,
            model_type=LOVE_ENGINE_TagTable,
        )
    ...
    if uncached_tags:
        try:
            ...
            for db_tag in db_tags:
                ...
                cache_key = f"tag:{tag_name}"
                _tag_obj = LOVE_ENGINE_TagTable(**db_tag.dict())
                await user_api_key_cache.async_set_cache(
                    key=cache_key,
                    value=_tag_obj,
                    model_type=LOVE_ENGINE_TagTable,
...
async def get_tag_object(
    ...
    Returns:
        LOVE_ENGINE_TagTable object if found, None otherwise
    """
...
async def get_team_membership(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional["LOVE_ENGINE_TeamMembership"]:
    """
    ...
    """
    from love_engine.proxy._types import LOVE_ENGINE_TeamMembership
    ...
    cached_membership_obj = await user_api_key_cache.async_get_cache(
        key=_key,
        model_type=LOVE_ENGINE_TeamMembership,
    )
    ...
    try:
        ...
        _response = LOVE_ENGINE_TeamMembership(**response.dict())
        await user_api_key_cache.async_set_cache(
            key=_key,
            value=_response,
            model_type=LOVE_ENGINE_TeamMembership,
...
async def get_user_object(
    ...
    - if valid, return LOVE_ENGINE_UserTable object with defined limits
    ...
    if not check_db_only:
        cached_user_obj = await user_api_key_cache.async_get_cache(
            key=user_id,
            model_type=LOVE_ENGINE_UserTable,
        )
        if cached_user_obj is not None:
    ...
    try:
        ...
        if response is None:
            if user_id_upsert:
                ...
                if love_engine.default_internal_user_params is not None:
                    new_user_params.update(love_engine.default_internal_user_params)
        ...
        if (
            ...
            # dump each organization membership to type LOVE_ENGINE_OrganizationMembershipTable
            _dumped_memberships = [
                LOVE_ENGINE_OrganizationMembershipTable(**membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = LOVE_ENGINE_UserTable(**dict(response))
...
async def _cache_team_object(
    ...
    await _cache_management_object(
        ...
        user_api_key_cache=user_api_key_cache,
        proxy_logging_obj=proxy_logging_obj,
        model_type=LOVE_ENGINE_TeamTableCachedObj,
    )
...
async def _get_team_db_check(
    ...
    if response is None and team_id_upsert:
        from love_engine.proxy.management_endpoints.team_endpoints import new_team

        new_team_data = NewTeamRequest(team_id=team_id)

        mock_request = Request(scope={"type": "http"})
        system_admin_user = UserAPIKeyAuth(user_role=LoveEngineUserRoles.PROXY_ADMIN)
        ...
        response = LOVE_ENGINE_TeamTable(**created_team_dict)
    return response
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response = LOVE_ENGINE_TeamTableCachedObj(**response.dict())
...
) -> Optional[LOVE_ENGINE_TeamTableCachedObj]:
    ...
    if (
        ...
        if cached_raw is not None:
            from_internal = CacheCodec.deserialize(
                cached_raw, LOVE_ENGINE_TeamTableCachedObj
            )
...
async def get_team_object(
    ...
    - if valid, return LOVE_ENGINE_TeamTable object with defined limits
    - if not, then raise an error
...
async def _cache_access_object(
    access_group_id: str,
    access_group_table: LOVE_ENGINE_AccessGroupTable,
    user_api_key_cache: UserApiKeyCache,
    proxy_logging_obj: Optional[ProxyLogging] = None,
):
    ...
    await user_api_key_cache.async_set_cache(
        ...
        model_type=LOVE_ENGINE_AccessGroupTable,
...
async def get_access_object(
    ...
    - if valid, return LOVE_ENGINE_AccessGroupTable object
    - if not, then raise an error
    ...
    try:
        ...
        _response = LOVE_ENGINE_AccessGroupTable(**response.dict())
...
async def get_team_object_by_alias(
    ...
    Returns:
        LOVE_ENGINE_TeamTableCachedObj: The team object if found
    ...
    try:
        ...
        team = teams[0]
        team_obj = LOVE_ENGINE_TeamTableCachedObj(**team.model_dump())
...
async def get_org_object_by_alias(
    ...
    Returns:
        LOVE_ENGINE_OrganizationTable if found, None otherwise
    ...
    cached_org_obj = await user_api_key_cache.async_get_cache(
        key=cache_key,
        model_type=LOVE_ENGINE_OrganizationTable,
    )
    ...
    try:
        ...
        org = orgs[0]
        org_obj = LOVE_ENGINE_OrganizationTable(**org.model_dump())
...
class ExperimentalUIJWTToken:
    @staticmethod
    def get_experimental_ui_login_jwt_auth_token(user_info: LOVE_ENGINE_UserTable) -> str:
        ...
        # Experimental UI flow uses fixed 10-min expiry for security (does not use LOVE_ENGINE_UI_SESSION_DURATION)
        ...
        valid_token = UserAPIKeyAuth(
            ...
            key_name="ui-token",
            key_alias="ui-token",
            max_budget=love_engine.max_ui_session_budget,
            rpm_limit=100,  # allow user to have a conversation on test key pane of UI
            expires=expires,
            user_id=user_info.user_id,
            team_id="love_engine-dashboard",
            models=user_info.models,
            max_parallel_requests=None,
            user_role=LoveEngineUserRoles(user_info.user_role),
        )
    ...
    def get_cli_jwt_auth_token(
        user_info: LOVE_ENGINE_UserTable,
        ...
        The expiration time can be controlled via the LOVE_ENGINE_CLI_JWT_EXPIRATION_HOURS
        ...
        # Calculate expiration time (configurable via LOVE_ENGINE_CLI_JWT_EXPIRATION_HOURS env var)
    ...
    def get_key_object_from_ui_hash_key(
        ...
        from love_engine.proxy.auth.user_api_key_auth import UserAPIKeyAuth
        from love_engine.proxy.common_utils.encrypt_decrypt_utils import (
...
async def get_object_permission(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[LOVE_ENGINE_ObjectPermissionTable]:
    """
    - Check if object permission id in proxy ObjectPermissionTable
    - if valid, return LOVE_ENGINE_ObjectPermissionTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    deserialized_perm = await user_api_key_cache.async_get_cache(
        key=key,
        model_type=LOVE_ENGINE_ObjectPermissionTable,
    )
    ...
    try:
        ...
        _perm_obj = LOVE_ENGINE_ObjectPermissionTable(**response.dict())
        await user_api_key_cache.async_set_cache(
            key=key,
            value=_perm_obj,
            model_type=LOVE_ENGINE_ObjectPermissionTable,
...
async def get_managed_vector_store_rows_by_uuids(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> List[LOVE_ENGINE_ManagedVectorStoresTable]:
    """
    ...
    result: List[LOVE_ENGINE_ManagedVectorStoresTable] = []
    ...
    for uuid in uuids:
        ...
        deserialized_vs = await user_api_key_cache.async_get_cache(
            key=key,
            model_type=LOVE_ENGINE_ManagedVectorStoresTable,
        )
    ...
    for row in rows:
        ...
        if not row_dict:
            continue
        cached_obj = LOVE_ENGINE_ManagedVectorStoresTable(**row_dict)
...
async def get_org_object(
    ...
    - if valid, return LOVE_ENGINE_OrganizationTable object
    - if not, then raise an error
    ...
    Args:
        ...
        parent_otel_span: Optional OpenTelemetry span
        proxy_logging_obj: Optional proxy logging object
        include_budget_table: If True, includes love_engine_budget_table in the query
    """
    if prisma_client is None:
        raise Exception(
            "No DB Connected. See - https://docs.love_engine.ai/docs/proxy/virtual_keys"
    ...
    try:
        ...
        if include_budget_table:
            query_kwargs["include"] = {"love_engine_budget_table": True}
        ...
        _org_obj = LOVE_ENGINE_OrganizationTable(**response.model_dump())
        # Cache the result
        await user_api_key_cache.async_set_cache(
            key=cache_key,
            value=_org_obj,
...
async def _get_resources_from_access_groups(
    ...
    if prisma_client is None or user_api_key_cache is None:
        from love_engine.proxy.proxy_server import prisma_client as _prisma_client
        from love_engine.proxy.proxy_server import proxy_logging_obj as _proxy_logging_obj
        from love_engine.proxy.proxy_server import user_api_key_cache as _user_api_key_cache
...
def _can_object_call_model(
    ...
    if model in love_engine.model_alias_map:
        potential_models.append(love_engine.model_alias_map[model])
...
async def can_key_call_model(
    ...
    valid_token: UserAPIKeyAuth,
    llm_router: Optional[love_engine.Router],
) -> Literal[True]:
    """
    Checks if token can call a given model
...
async def can_key_call_resolved_model(
    ...
    team_object: Optional[LOVE_ENGINE_TeamTableCachedObj] = None
    ...
    if valid_token.team_id is not None:
        try:
            ...
            team_object = LOVE_ENGINE_TeamTableCachedObj(
                team_id=valid_token.team_id,
...
async def get_authorized_resources_from_key_access_groups(
    ...
    """
    For each access_group_id on the key, fetch the LOVE_ENGINE_AccessGroupTable row
...
def can_project_access_model(
    model: Union[str, List[str]],
    project_object: LOVE_ENGINE_ProjectTableCachedObj,
    llm_router: Optional[Router],
) -> Literal[True]:
    """
...
def _search_tool_names_from_object_permission(
    object_permission: Optional[LOVE_ENGINE_ObjectPermissionTable],
) -> List[str]:
    ...
    if (
        ...
        alert_email_config: Optional[Dict[str, List[str]]] = (
            _merge_budget_alert_email_configs(
                global_cfg=love_engine.default_key_max_budget_alert_emails,
...
async def _check_team_member_budget(
    ...
    if (
        ...
        if (
            ...
            and team_membership.love_engine_budget_table is not None
            and team_membership.love_engine_budget_table.max_budget is not None
        ):
            team_member_budget = team_membership.love_engine_budget_table.max_budget
        else:
...
async def _check_team_member_model_access(
    ...
    if (
        ...
        or team_membership.love_engine_budget_table is None
        or not team_membership.love_engine_budget_table.allowed_models
    ):
        return  # no per-member restriction — inherit team-level check

    member_allowed_models: List[str] = (
        team_membership.love_engine_budget_table.allowed_models
    )
    try:
        _can_object_call_model(
            model=model,
        ...
        if math.isfinite(team_object.max_budget) and spend > team_object.max_budget:
            if valid_token:
                call_info = CallInfo(
                    ...
                    team_alias=valid_token.team_alias,
                    organization_id=valid_token.org_id,
                    event_group=LOVE_ENGINE_EntityType.TEAM,
                )
                asyncio.create_task(
                    proxy_logging_obj.budget_alerts(
                        type="team_budget",
...
async def _project_max_budget_check(
    ...
    max_budget = None
    if project_object.love_engine_budget_table is not None:
        max_budget = project_object.love_engine_budget_table.max_budget
    ...
    if (
        ...
        if valid_token:
            call_info = CallInfo(
                ...
                team_alias=valid_token.team_alias,
                organization_id=valid_token.org_id,
                event_group=LOVE_ENGINE_EntityType.PROJECT,
            )
            asyncio.create_task(
                proxy_logging_obj.budget_alerts(
                    type="project_budget",
...
async def _project_soft_budget_check(
    ...
    soft_budget = None
    if project_object.love_engine_budget_table is not None:
        soft_budget = project_object.love_engine_budget_table.soft_budget
...
async def get_project_object(
    ...
    deserialized_project = await user_api_key_cache.async_get_cache(
        key=cache_key,
        model_type=LOVE_ENGINE_ProjectTableCachedObj,
    )
    ...
    project_obj = LOVE_ENGINE_ProjectTableCachedObj(**project_row.model_dump())
...
async def _organization_max_budget_check(
    ...
    if org_table.love_engine_budget_table is not None:
        org_max_budget = org_table.love_engine_budget_table.max_budget
    ...
    if math.isfinite(org_max_budget) and org_spend >= org_max_budget:
        ...
        call_info = CallInfo(
            ...
            event_group=LOVE_ENGINE_EntityType.ORGANIZATION,
        )
        asyncio.create_task(
            proxy_logging_obj.budget_alerts(
...
async def _tag_max_budget_check(
    ...
    """
    from love_engine.proxy.common_utils.http_parsing_utils import get_tags_from_request_body
    ...
    for tag_name in tags:
        ...
        if (
            tag_object.love_engine_budget_table is not None
            and tag_object.love_engine_budget_table.max_budget is not None
        ):
            from love_engine.proxy.proxy_server import get_current_spend
            ...
            tag_spend = await get_current_spend(
                ...
                max_budget=tag_object.love_engine_budget_table.max_budget,
                fallback_authoritative=True,
            )
            if tag_spend <= tag_object.love_engine_budget_table.max_budget:
                continue
            raise love_engine.BudgetExceededError(
                current_cost=tag_spend,
                max_budget=tag_object.love_engine_budget_table.max_budget,
                message=f"Budget has been exceeded! Tag={tag_name} Current cost: {tag_spend}, Max budget: {tag_object.love_engine_budget_table.max_budget}",
            )
...
async def vector_store_access_check(
    ...
    vector_store_ids_to_run = love_engine.vector_store_registry.get_vector_store_ids_to_run(
...
def _can_object_call_vector_stores(
    ...
    object_permissions: Optional[LOVE_ENGINE_ObjectPermissionTable],
):
    """
    Raises ProxyException if the object (key, team, org) cannot access the specific vector store.
    """
