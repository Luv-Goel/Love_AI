...
import love_engine
from love_engine.constants import LOVE_ENGINE_PROXY_ADMIN_NAME, LOVE_ENGINE_UI_SESSION_DURATION
...
from love_engine.proxy.management_endpoints.internal_user_endpoints import user_update
...
def get_ui_credentials(master_key: Optional[str]) -> tuple[str, str]:
    ...
    if ui_password is None:
        raise ProxyException(
            message="set Proxy master key to use UI. https://docs.love_engine.ai/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
...
async def authenticate_user(
    ...
    _user_row: Optional[LOVE_ENGINE_UserTable] = None
    user_role: Optional[
        Literal[
    ...
    if secrets.compare_digest(
        ...
        key_user_id = LOVE_ENGINE_PROXY_ADMIN_NAME
        if (
            os.getenv("PROXY_ADMIN_ID", None) is not None
            and os.environ["PROXY_ADMIN_ID"] == user_id
        ) or user_id == LOVE_ENGINE_PROXY_ADMIN_NAME:
            # checks if user is admin
            key_user_id = os.getenv("PROXY_ADMIN_ID", LOVE_ENGINE_PROXY_ADMIN_NAME)
        ...
        if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
            ...
            user_info: Optional[LOVE_ENGINE_UserTable] = None
        ...
        user_role = getattr(
            _user_row, "user_role", LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY
        )
