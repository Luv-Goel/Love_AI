...
def organization_role_based_access_check(
    ...
    2. IF route is a LoveEngineRoutes.org_admin_only_routes, then check if user is an Org Admin for that organization
    ...
    if route == "/organization/new":
        if user_object.user_role != LoveEngineUserRoles.PROXY_ADMIN.value:
    ...
    if route in LoveEngineRoutes.org_admin_only_routes.value:
        (
            _user_organizations,
        ...
        user_role: Optional[LoveEngineUserRoles] = _user_organization_role_mapping.get(
        ...
        if user_role != LoveEngineUserRoles.ORG_ADMIN.value:
            raise ProxyException(
        ...
        if (
            ...
            if _user_role_in_passed_org != LoveEngineUserRoles.ORG_ADMIN.value:
                raise ProxyException(
...
def get_user_organization_info(
...
) -> Tuple[List[str], Dict[str, Optional[LoveEngineUserRoles]]]:
    """
    Helper function to extract user organization information.

    Args:
        user_object (LOVE_ENGINE_UserTable): The user object containing organization memberships.

    Returns:
        Tuple[List[str], Dict[str, Optional[LoveEngineUserRoles]]]: A tuple containing:
    ...
    _user_organization_role_mapping: Dict[str, Optional[LoveEngineUserRoles]] = {}
...
def _user_is_org_admin(
    ...
    admin_org_ids = {
        ...
        for _membership in user_object.organization_memberships
        if _membership.user_role == LoveEngineUserRoles.ORG_ADMIN.value
