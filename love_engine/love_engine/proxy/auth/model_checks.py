...
from love_engine.proxy._types import SpecialModelNames, UserAPIKeyAuth
...
from love_engine.router_utils.fallback_event_handlers import get_fallback_model_group
from love_engine.types.router import CredentialLoveEngineParams, LOVE_ENGINE_Params
from love_engine.types.utils import LlmProviders
from love_engine.utils import get_valid_models

_CREDENTIAL_LOVE_ENGINE_PARAM_FIELDS = set(CredentialLoveEngineParams.model_fields)
...
def get_provider_models(
    provider: str, love_engine_params: Optional[LOVE_ENGINE_Params] = None
) -> Optional[List[str]]:
    """
    Returns the list of known models by provider
    """
    if provider == "*":
        return get_valid_models(love_engine_params=love_engine_params)

    if provider in love_engine.models_by_provider:
        provider_models = get_valid_models(
            custom_llm_provider=provider, love_engine_params=love_engine_params
        )
...
def _hydrate_love_engine_credential_name(
    love_engine_params: Optional[LOVE_ENGINE_Params],
) -> Optional[LOVE_ENGINE_Params]:
    if love_engine_params is None or love_engine_params.love_engine_credential_name is None:
        return love_engine_params

    credential_values = CredentialAccessor.get_credential_values(
        love_engine_params.love_engine_credential_name
    )
    if not credential_values:
        return love_engine_params

    love_engine_params = love_engine_params.model_copy()
    for key, value in credential_values.items():
        if (
            key in _CREDENTIAL_LOVE_ENGINE_PARAM_FIELDS
            and getattr(love_engine_params, key, None) is None
        ):
            setattr(love_engine_params, key, value)
    love_engine_params.love_engine_credential_name = None
    return love_engine_params


def get_known_models_from_wildcard(
    wildcard_model: str, love_engine_params: Optional[LOVE_ENGINE_Params] = None
) -> List[str]:
    try:
    ...
    # Use provider from love_engine_params when available, otherwise from wildcard prefix
    # (e.g., "openai" from "openai/*" - needed for BYOK where wildcard isn't in router)
    if love_engine_params is not None:
        try:
            provider = love_engine_params.model.split("/", 1)[0]
        except ValueError:
            provider = wildcard_provider_prefix
    else:
        provider = wildcard_provider_prefix

    love_engine_params = _hydrate_love_engine_credential_name(love_engine_params)

    wildcard_models = get_provider_models(
        provider=provider, love_engine_params=love_engine_params
    )
...
def _get_wildcard_models(
    ...
    for model in unique_models:
        if _check_wildcard_routing(model=model):
            ...
            ## get love_engine params from model
            if llm_router is not None:
                ...
                if model_list:
                    for router_model in model_list:
                        wildcard_models = get_known_models_from_wildcard(
                            ...
                            love_engine_params=LOVE_ENGINE_Params(
                                **router_model["love_engine_params"]  # type: ignore
                            ),
                        )
                    ...
                    wildcard_models = get_known_models_from_wildcard(
                        wildcard_model=model, love_engine_params=None
                    )
                    if wildcard_models:
