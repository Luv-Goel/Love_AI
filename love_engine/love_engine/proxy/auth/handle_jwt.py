...
JWT token must have 'love_engine_proxy_admin' in scope.
"""
...
from love_engine.constants import DEFAULT_MANAGEMENT_OBJECT_IN_MEMORY_CACHE_TTL
from love_engine.love_engine_core_utils.dot_notation_indexing import get_nested_value
from love_engine.llms.custom_httpx.httpx_handler import HTTPHandler
...
from love_engine.proxy.auth.auth_checks import can_team_access_model
...
from love_engine.proxy.utils import PrismaClient, ProxyLogging
...
class JWTHandler:
    ...
    - if role="love_engine_proxy_user" -> allow making calls + info. Can not edit budgets
    """
    ...
    LOVE_ENGINE_JWT_ISSUER_CLAIM = "_love_engine_jwt_issuer"
    LOVE_ENGINE_USER_ID_CLAIM = "_love_engine_user_id"
    LOVE_ENGINE_USER_EMAIL_CLAIM = "_love_engine_user_email"
    LOVE_ENGINE_TEAM_ID_CLAIM = "_love_engine_team_id"
    LOVE_ENGINE_TEAM_IDS_CLAIM = "_love_engine_team_ids"
    LOVE_ENGINE_ORG_ID_CLAIM = "_love_engine_org_id"
    LOVE_ENGINE_END_USER_ID_CLAIM = "_love_engine_end_user_id"
    LOVE_ENGINE_INTERNAL_CLAIMS = (
        LOVE_ENGINE_JWT_ISSUER_CLAIM,
        LOVE_ENGINE_USER_ID_CLAIM,
        LOVE_ENGINE_USER_EMAIL_CLAIM,
        LOVE_ENGINE_TEAM_ID_CLAIM,
        LOVE_ENGINE_TEAM_IDS_CLAIM,
        LOVE_ENGINE_ORG_ID_CLAIM,
        LOVE_ENGINE_END_USER_ID_CLAIM,
    )
    ...
    def update_environment(
        ...
        self.love_engine_jwtauth = love_engine_jwtauth
        self.leeway = leeway
    ...
    def _rbac_role_from_role_mapping(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        """
        if self.love_engine_jwtauth.role_mappings is None:
            return None
        ...
        for role_mapping in self.love_engine_jwtauth.role_mappings:
    ...
    def get_rbac_role(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        Resolves: https://github.com/BerriAI/love_engine/issues/6793
    ...
    def is_admin(self, scopes: list) -> bool:
        if self.love_engine_jwtauth.admin_jwt_scope in scopes:
            return True
        return False

    def _is_trusted_issuer_normalized_token(self, token: dict) -> bool:
        issuer = token.get(self.LOVE_ENGINE_JWT_ISSUER_CLAIM)
        if not isinstance(issuer, str) or not issuer:
            return False

        love_engine_jwtauth = getattr(self, "love_engine_jwtauth", None)
        issuer_configs = getattr(love_engine_jwtauth, "issuers", None) or []
    ...
    def get_team_ids_from_jwt(self, token: dict) -> List[str]:
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_TEAM_IDS_CLAIM
        ):
            issuer_team_ids = token.get(self.LOVE_ENGINE_TEAM_IDS_CLAIM)
        ...
        if self.love_engine_jwtauth.team_ids_jwt_field is not None:
            team_ids: Optional[List[str]] = get_nested_value(
                data=token,
                key_path=self.love_engine_jwtauth.team_ids_jwt_field,
                default=[],
            )
            return team_ids or []
    ...
    def get_all_jwt_team_ids(self, token: dict) -> List[str]:
        ...
        singular: Any = None
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_TEAM_ID_CLAIM
        ):
            singular = token.get(self.LOVE_ENGINE_TEAM_ID_CLAIM)
        elif self.love_engine_jwtauth.team_id_jwt_field is not None:
            singular = get_nested_value(
                data=token,
                key_path=self.love_engine_jwtauth.team_id_jwt_field,
                default=None,
            )
    ...
    def get_end_user_id(
        ...
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_END_USER_ID_CLAIM
        ):
            return token.get(self.LOVE_ENGINE_END_USER_ID_CLAIM)

        try:
            if self.love_engine_jwtauth.end_user_id_jwt_field is not None:
                user_id = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.end_user_id_jwt_field,
    ...
    def is_required_team_id(self) -> bool:
        ...
        """
        if (
            self.love_engine_jwtauth.team_id_jwt_field is None
            and self.love_engine_jwtauth.team_alias_jwt_field is None
        ):
            return False
        return True
    ...
    def is_enforced_email_domain(self) -> bool:
        ...
        if self.love_engine_jwtauth.user_allowed_email_domain is not None and isinstance(
            self.love_engine_jwtauth.user_allowed_email_domain, str
        ):
            return True
        return False
    ...
    def get_team_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        if self._has_trusted_issuer_normalized_claim(
            ...
            team_id = token.get(self.LOVE_ENGINE_TEAM_ID_CLAIM)
            if isinstance(team_id, list):
                return team_id[0] if team_id else default_value
            return team_id

        try:
            if self.love_engine_jwtauth.team_id_jwt_field is not None:
                ...
                if team_id is sentinel:
                    ...
                    if self.love_engine_jwtauth.team_id_default is not None:
                        return self.love_engine_jwtauth.team_id_default
                    else:
                ...
                if isinstance(team_id, list):
                    if not team_id:
                        return default_value
                    verbose_proxy_logger.debug(
                        f"JWT Auth: team_id_jwt_field '{self.love_engine_jwtauth.team_id_jwt_field}' "
                        f"returned a list {team_id}; using first element '{team_id[0]}' automatically."
                    )
                    team_id = team_id[0]
                return team_id  # type: ignore[return-value]
            elif self.love_engine_jwtauth.team_id_default is not None:
                team_id = self.love_engine_jwtauth.team_id_default
            else:
                team_id = None
        except KeyError:
    ...
    def get_team_alias(
        ...
        """
        try:
            if self.love_engine_jwtauth.team_alias_jwt_field is not None:
                team_alias = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.team_alias_jwt_field,
                    default=default_value,
                )
                return team_alias
            else:
    ...
    def is_upsert_user_id(self, valid_user_email: Optional[bool] = None) -> bool:
        ...
        if valid_user_email is False:
            return False
        return self.love_engine_jwtauth.user_id_upsert

    def get_user_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_USER_ID_CLAIM
        ):
            return token.get(self.LOVE_ENGINE_USER_ID_CLAIM)

        try:
            if self.love_engine_jwtauth.user_id_jwt_field is not None:
                user_id = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.user_id_jwt_field,
                    default=default_value,
                )
            else:
                user_id = default_value
    ...
    def get_user_roles(
        ...
        """
        try:
            if self.love_engine_jwtauth.user_roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.user_roles_jwt_field,
                    default=default_value,
                )
            else:
    ...
    def map_jwt_role_to_love_engine_role(self, token: dict) -> Optional[LoveEngineUserRoles]:
        """Map roles from JWT to LoveEngine user roles"""
        if not self.love_engine_jwtauth.jwt_love_engine_role_map:
            return None
        ...
        for mapping in self.love_engine_jwtauth.jwt_love_engine_role_map:
            for role in jwt_roles:
                if fnmatch.fnmatch(role, mapping.jwt_role):
                    return mapping.love_engine_role
        return None
    ...
    def get_jwt_role(
        ...
        """
        try:
            if self.love_engine_jwtauth.roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.roles_jwt_field,
                    default=default_value,
                )
            else:
                user_roles = default_value
    ...
    def is_allowed_user_role(self, user_roles: Optional[List[str]]) -> bool:
        ...
        if (
            ...
            and self.love_engine_jwtauth.user_allowed_roles is not None
            and any(
                role in self.love_engine_jwtauth.user_allowed_roles for role in user_roles
            )
        ):
            return True
        return False
    ...
    def get_user_email(
        ...
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_USER_EMAIL_CLAIM
        ):
            return token.get(self.LOVE_ENGINE_USER_EMAIL_CLAIM)

        try:
            if self.love_engine_jwtauth.user_email_jwt_field is not None:
                user_email = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.user_email_jwt_field,
                    default=default_value,
                )
            else:
                user_email = None
    ...
    def get_object_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.love_engine_jwtauth.object_id_jwt_field is not None:
                object_id = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.object_id_jwt_field,
                    default=default_value,
                )
            else:
    ...
    def get_org_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        if self._has_trusted_issuer_normalized_claim(
            token=token, claim=self.LOVE_ENGINE_ORG_ID_CLAIM
        ):
            return token.get(self.LOVE_ENGINE_ORG_ID_CLAIM)

        try:
            if self.love_engine_jwtauth.org_id_jwt_field is not None:
                org_id = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.org_id_jwt_field,
                    default=default_value,
                )
            else:
                org_id = None
    ...
    def get_org_alias(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        ...
        """
        try:
            if self.love_engine_jwtauth.org_alias_jwt_field is not None:
                org_alias = get_nested_value(
                    data=token,
                    key_path=self.love_engine_jwtauth.org_alias_jwt_field,
                    default=default_value,
                )
                return org_alias
            else:
    ...
    async def _resolve_jwks_url(self, url: str) -> str:
        ...
        cache_key = f"love_engine_oidc_discovery_{url}"
    ...
    def _get_public_key_cache_ttl(self) -> float:
        love_engine_jwtauth = getattr(self, "love_engine_jwtauth", None)
        if love_engine_jwtauth is None:
            return 600
        return love_engine_jwtauth.public_key_ttl
    ...
    async def _get_public_key_from_jwks_url(
        ...
        cache_key = f"love_engine_jwt_auth_keys_{resolved_jwks_url}"
    ...
    def is_allowed_domain(self, user_email: str) -> bool:
        if self.love_engine_jwtauth.user_allowed_email_domain is None:
            return True

        email_domain = user_email.split("@")[-1]  # Extract domain from email
        if email_domain == self.love_engine_jwtauth.user_allowed_email_domain:
            return True
        else:
            return False
    ...
    async def get_oidc_userinfo(self, token: str) -> dict:
        ...
        """
        if not self.love_engine_jwtauth.oidc_userinfo_endpoint:
            raise Exception(
        ...
        verbose_proxy_logger.debug(
            f"Calling OIDC UserInfo endpoint: {self.love_engine_jwtauth.oidc_userinfo_endpoint}"
        )

        try:
            # Call the UserInfo endpoint with the access token
            response = await self.http_handler.get(
                url=self.love_engine_jwtauth.oidc_userinfo_endpoint,
                headers={
            ...
            await self.user_api_key_cache.async_set_cache(
                key=cache_key,
                value=userinfo,
                ttl=self.love_engine_jwtauth.oidc_userinfo_cache_ttl,
            )
    ...
    def _build_decode_kwargs(cls) -> dict:
        ...
        ``LOVE_ENGINE_JWTAuth.issuers``: tokens whose ``iss`` does not match any
    ...
    def _get_configured_issuer(self, token: str) -> Optional[JWTIssuerConfig]:
        ...
        issuer_configs = love_engine_jwtauth.issuers
    ...
    def _get_claim_value_for_issuer_mapping(self, token: dict, claim_field: str) -> Any:
        ...
        behave like the global ``love_engine_jwtauth`` path — present claims override
    ...
    def _apply_issuer_claim_mappings(
        ...
        normalized: dict = {
            k: v for k, v in token.items() if k not in self.LOVE_ENGINE_INTERNAL_CLAIMS
        }
        normalized[self.LOVE_ENGINE_JWT_ISSUER_CLAIM] = issuer_config.issuer
        claim_mappings = [
            (issuer_config.user_id_jwt_field, self.LOVE_ENGINE_USER_ID_CLAIM),
            (issuer_config.user_email_jwt_field, self.LOVE_ENGINE_USER_EMAIL_CLAIM),
            (issuer_config.team_id_jwt_field, self.LOVE_ENGINE_TEAM_ID_CLAIM),
            (issuer_config.team_ids_jwt_field, self.LOVE_ENGINE_TEAM_IDS_CLAIM),
            (issuer_config.org_id_jwt_field, self.LOVE_ENGINE_ORG_ID_CLAIM),
            (issuer_config.end_user_id_jwt_field, self.LOVE_ENGINE_END_USER_ID_CLAIM),
        ]
        ...
        if public_key is not None:
            try:
                ...
                return {
                    ...
                    if k not in self.LOVE_ENGINE_INTERNAL_CLAIMS
                }
...
class JWTAuthManager:
    ...
    async def check_rbac_role(
        ...
        if jwt_handler.love_engine_jwtauth.enforce_rbac is True:
    ...
    async def check_admin_access(
        ...
        is_allowed = allowed_routes_check(
            ...
            user_route=route,
            love_engine_proxy_roles=jwt_handler.love_engine_jwtauth,
        )
        if not is_allowed:
            allowed_routes: List[Any] = jwt_handler.love_engine_jwtauth.admin_allowed_routes
    ...
    async def find_and_validate_specific_team_id(
    ...
    ) -> Tuple[Optional[str], Optional[LOVE_ENGINE_TeamTable]]:
        ...
        team_object: Optional[LOVE_ENGINE_TeamTable] = None
        ...
        if individual_team_id:
            try:
                team_object = await get_team_object(
                    ...
                    team_id_upsert=jwt_handler.love_engine_jwtauth.team_id_upsert,
                )
                return individual_team_id, team_object
            except HTTPException as e:
                if (
                    e.status_code != 404
                    or not jwt_handler.love_engine_jwtauth.team_claim_fallback
                ):
                    raise
        ...
        if jwt_handler.is_required_team_id() is True:
            team_id_field = jwt_handler.love_engine_jwtauth.team_id_jwt_field
            team_alias_field = jwt_handler.love_engine_jwtauth.team_alias_jwt_field
            hint = ""
            if team_id_field:
                ...
                if "." in team_id_field:
                    ...
                    if parts[-1].isdigit():
                        ...
                        hint = (
                            ...
                            f"supported. Use '{base_field}' instead — LoveEngine automatically "
                ...
                # "roles[0]" — bracket-notation indexing is also not supported in get_nested_value
                    ...
                    if m:
                        ...
                        hint = (
                            ...
                            f"in team_id_jwt_field. Use '{base_field}' instead — LoveEngine "
    ...
    async def find_team_with_model_access(
        ...
        if not team_ids:
            if jwt_handler.love_engine_jwtauth.enforce_team_based_model_access:
                raise HTTPException(
                    status_code=403,
            ...
            try:
                ...
                if team_object and team_object.models is not None:
                    ...
                    if isinstance(team_models, list) and (
                        ...
                        is_allowed = allowed_routes_check(
                            user_role=LoveEngineUserRoles.TEAM,
                            user_route=route,
                            love_engine_proxy_roles=jwt_handler.love_engine_jwtauth,
                        )
                        if (
    ...
    async def get_objects(
        ...
        org_object: Optional[LOVE_ENGINE_OrganizationTable] = None
        if org_id:
            org_object = (
        ...
        if valid_user_email is False:
            raise ProxyException(
                message=f"Email domain not allowed. User email: {user_email}. Allowed domain: {jwt_handler.love_engine_jwtauth.user_allowed_email_domain}",
                type=ProxyErrorTypes.auth_error,
                param="user_email",
                code=403,
            )

        user_object: Optional[LOVE_ENGINE_UserTable] = None
        if user_id:
            user_object = (
        ...
        team_membership_object: Optional[LOVE_ENGINE_TeamMembership] = None
    ...
    def get_team_id_from_header(
        ...
        """
        Extract team_id from x-love_engine-team-id header if present.
        ...
        header_team_id = normalized_headers.get("x-love_engine-team-id")
        ...
        if header_team_id not in allowed_team_ids:
            raise HTTPException(
                status_code=403,
                detail=f"Team '{header_team_id}' from x-love_engine-team-id header is not in your JWT's allowed teams. Allowed teams: {list(allowed_team_ids)}",
            )

        verbose_proxy_logger.debug(
            f"Using team_id from x-love_engine-team-id header: {header_team_id}"
        )
    ...
    async def map_user_to_teams(
        ...
        """
        from love_engine.proxy.management_endpoints.team_endpoints import team_member_add
    ...
    async def sync_user_role_and_teams(
        ...
        1. The user role on LoveEngine DB is in sync with the IDP provider role
        ...
        # Update user role
        new_role = jwt_handler.map_jwt_role_to_love_engine_role(jwt_valid_token)
        ...
        if teams_to_add or teams_to_remove:
            from love_engine.proxy.management_endpoints.scim.scim_v2 import (
    ...
    async def _attach_team_from_header_for_admin(
        ...
        """Attach team context from x-love_engine-team-id to an admin result.
        ...
        """
        header_team_id = (
            request_headers.get("x-love_engine-team-id") if request_headers else None
        )
        ...
        try:
            ...
            verbose_proxy_logger.warning(
                "admin x-love_engine-team-id=%r on route=%s could not be resolved (%s); "
    ...
    async def _resolve_single_team_fallback(
        ...
        in LoveEngine, load that team (and membership when user_id is set) so that
    ...
    async def auth_builder(
        ...
        if (
            jwt_handler.love_engine_jwtauth.oidc_userinfo_enabled
        ...
        if jwt_handler.love_engine_jwtauth.custom_validate:
            if not jwt_handler.love_engine_jwtauth.custom_validate(jwt_valid_token):
                raise HTTPException(
                    status_code=403,
        ...
        if (
            jwt_handler.love_engine_jwtauth.enforce_scope_based_access
            and jwt_handler.love_engine_jwtauth.scope_mappings
        ):
            JWTAuthManager.check_scope_based_access(
                scope_mappings=jwt_handler.love_engine_jwtauth.scope_mappings,
                scopes=scopes,
        ...
        if rbac_role and object_id:
            if rbac_role == LoveEngineUserRoles.TEAM:
                team_id = object_id
            elif rbac_role == LoveEngineUserRoles.INTERNAL_USER:
                user_id = object_id
        ...
        ## Check if team_id is specified via x-love_engine-team-id header
        ...
        is_proxy_admin = bool(
            user_object and user_object.user_role == LoveEngineUserRoles.PROXY_ADMIN
        )
