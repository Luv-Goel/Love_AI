...
import love_engine
from love_engine import Router, provider_list
from love_engine._logging import verbose_proxy_logger
from love_engine.constants import STANDARD_CUSTOMER_ID_HEADERS
...
from love_engine.types.router import CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS
from love_engine.types.utils import CustomPricingLoveEngineParams
...
def check_complete_credentials(request_body: dict) -> bool:
    ...
    if getattr(love_engine, "user_url_validation", False):
...
# API calls. ``love_engine_embedding_config`` is consumed by the Milvus
...
# ``love_engine_embedding_config``.
_NESTED_CONFIG_KEYS: Tuple[str, ...] = ("love_engine_embedding_config", "extra_body")
...
_NESTED_METADATA_KEYS: Tuple[str, ...] = ("metadata", "love_engine_metadata")
...
def _build_banned_observability_params() -> FrozenSet[str]:
    ...
    ``love_engine/love_engine_core_utils/initialize_dynamic_callback_params.py`` is
...
_BANNED_REQUEST_BODY_PARAMS: Tuple[str, ...] = (
    ...
    *sorted(CustomPricingLoveEngineParams.model_fields.keys()),
)
...
def is_request_body_safe(
    ...
    ``love_engine_embedding_config.api_base`` (VERIA-6) without exposing a
...
def _coerce_metadata_to_dict(value: Any) -> Optional[Dict[str, Any]]:
    ...
    Multipart/form-data and ``extra_body`` callers send ``love_engine_metadata``
    as a JSON-encoded string; the proxy parses it into a dict later in
    ``add_love_engine_data_to_request``, but the auth-time bouncer runs first
...
async def pre_db_read_auth_checks(
    ...
    """
    from love_engine.proxy.proxy_server import general_settings, llm_router, premium_user
...
def route_in_additonal_public_routes(current_route: str):
    ...
    In order to use this the love_engine config.yaml should have the following in general_settings:

    ```yaml
    general_settings:
        master_key: sk-1234
        public_routes: ["LoveEngineRoutes.public_routes", "/spend/calculate", "/api/*"]
    ```
    """
    from love_engine.proxy.auth.route_checks import RouteChecks
    from love_engine.proxy.proxy_server import general_settings, premium_user
...
def _get_deployment_default_limit(model_name: str, field: str) -> Optional[int]:
    ...
    for deployment in deployments:
        raw = deployment.get("love_engine_params", {}).get(field)
        if raw is not None:
            try:
...
def custom_auth_common_checks_warning(
    *,
    ...
    return (
        ...
        "Docs: https://docs.love_engine.ai/docs/proxy/custom_auth"
    )
...
def get_customer_user_header_from_mapping(user_id_mapping) -> Optional[list]:
    ...
    for item in items:
        ...
        if role is None or not header_name:
            continue
        if str(role).lower() == str(LoveEngineUserRoles.CUSTOMER).lower():
...
def _coerce_user_id_to_str(value: Any) -> Optional[str]:
    ...
    are only rejected when ``love_engine.validate_end_user_id_in_db`` is enabled
    ...
    if isinstance(value, str):
        ...
        if love_engine.validate_end_user_id_in_db and stripped[:1] in ("{", "["):
...
def get_end_user_id_from_request_body(
    ...
    def _as_dict(value: Any) -> dict:
        # metadata / love_engine_metadata can arrive as JSON strings from
    ...
    # Check 4: 'love_engine_metadata.user' in request_body (commonly Anthropic)
    love_engine_metadata = _as_dict(request_body.get("love_engine_metadata"))
    user_id_str = _coerce_user_id_to_str(love_engine_metadata.get("user"))
    if user_id_str:
        return user_id_str
...
MODEL_ROUTING_HEADER_NAME = "x-love_engine-model"
...
def _extract_models_from_managed_resource_id(
    ...
    try:
        from love_engine.llms.base_llm.managed_resources.utils import parse_unified_id
    ...
    if resource_id_field in ("video_id", "character_id"):
        try:
            from love_engine.types.videos.utils import (
