"""
This file handles authentication for the LoveEngine Proxy.

it checks if the user passed a valid API Key to the LoveEngine Proxy
...
from love_engine.constants import LOVE_ENGINE_PROXY_MASTER_KEY_ALIAS
from love_engine.integrations.otel.model.config import is_otel_v2_enabled
from love_engine.integrations.otel.runtime import phase_span, seed_request_identity
...
from love_engine.proxy.auth.auth_exception_handler import UserAPIKeyAuthExceptionHandler
...
from love_engine.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
from love_engine.proxy.auth.oauth2_check import Oauth2Handler
from love_engine.proxy.auth.oauth2_proxy_hook import handle_oauth2_proxy_request
from love_engine.proxy.auth.route_checks import RouteChecks
from love_engine.proxy.common_utils.cache_coordinator import EventDrivenCacheCoordinator
...
from love_engine.repositories.table_repositories import TeamMembershipRepository
from love_engine.secret_managers.main import get_secret_bool
from love_engine.types.services import ServiceTypes

try:
    from love_engine_enterprise.proxy.auth.user_api_key_auth import (
...
def _route_requires_auth_despite_public(
    ...
    if normalized_route == "/metrics":
        return love_engine.require_auth_for_metrics_endpoint is not False

    return False


custom_love_engine_key_header = APIKeyHeader(
    name=SpecialHeaders.custom_love_engine_api_key.value,
    auto_error=False,
...
def _should_route_jwt_to_oauth2_override(token: str, jwt_handler: JWTHandler) -> bool:
    routing_overrides = jwt_handler.love_engine_jwtauth.routing_overrides
...
def _apply_budget_limits_to_end_user_params(
    ...
    budget_info: LOVE_ENGINE_BudgetTable,
...
async def get_global_proxy_spend(
    ...
    if (
        love_engine.max_budget > 0 and prisma_client is not None
        ...
        if global_proxy_spend is not None:
            user_info = CallInfo(
                user_id=love_engine_proxy_admin_name,
                max_budget=love_engine.max_budget,
                spend=global_proxy_spend,
                token=token,
                event_group=LOVE_ENGINE_EntityType.PROXY,
            )
            asyncio.create_task(
...
def get_rbac_role(jwt_handler: JWTHandler, scopes: List[str]) -> str:
    ...
    if is_admin:
        return LoveEngineUserRoles.PROXY_ADMIN
    else:
        return LoveEngineUserRoles.TEAM


def get_api_key(
    custom_love_engine_key_header: Optional[str],
    api_key: str,
    ...
    if isinstance(custom_love_engine_key_header, str):
        passed_in_key = custom_love_engine_key_header
        api_key = _get_bearer_token_or_received_api_key(custom_love_engine_key_header)
        ...
        for endpoint in pass_through_endpoints:
            if endpoint.get("path", "") == route:
                ...
                if headers is not None:
                    header_key: str = headers.get("love_engine_user_api_key", "")
...
async def check_api_key_for_custom_headers_or_pass_through_endpoints(
    ...
    if normalized_route is not None:
        for mapped_route in LoveEngineRoutes.mapped_pass_through_routes.value:  # type: ignore
            if normalized_route.startswith(mapped_route):
                is_mapped_pass_through_route = True
                break
    if is_mapped_pass_through_route:
        if request.headers.get("love_engine_user_api_key") is not None:
            api_key = request.headers.get("love_engine_user_api_key") or ""
        ...
        for endpoint in pass_through_endpoints:
            if isinstance(endpoint, dict) and endpoint.get("path", "") == route:
                ...
                if (
                    ...
                    # check the langfuse public key if it contains the love_engine api key
                    import base64
                    ...
                    if headers is not None:
                        header_key = headers.get("love_engine_user_api_key", "")
                        if (
...
async def _auto_register_jwt_mapping(
    ...
    love_engine_jwtkeymapping. The loser catches the conflict, deletes its orphaned
    ...
    # attempts to insert into LOVE_ENGINE_UserTable with user_id=None, which fails
    ...
    # row in LOVE_ENGINE_VerificationToken uses its hash, so hash here to get the FK
    # value referenced by LOVE_ENGINE_JWTKeyMapping.token.
    token_hash = hash_token(key_data["token"])

    try:
        await prisma_client.db.love_engine_jwtkeymapping.create(
            data={
        ...
        if "unique" in error_str or "p2002" in error_str:
            ...
            # just persisted to LOVE_ENGINE_VerificationToken is orphaned — nothing
            ...
            try:
                await prisma_client.db.love_engine_verificationtoken.delete(
    ...
    await user_api_key_cache.async_set_cache(
        key=cache_key,
        value=token_hash,
        ttl=jwt_handler.love_engine_jwtauth.virtual_key_mapping_cache_ttl,
    )
...
async def _resolve_jwt_to_virtual_key(
    ...
    """
    virtual_key_claim_field = jwt_handler.love_engine_jwtauth.virtual_key_claim_field
    ...
    if claim_value is None:
        ...
        behavior = jwt_handler.love_engine_jwtauth.unregistered_jwt_client_behavior
        if behavior in (
...
def _ensure_parent_otel_span_on_request_state(request: Request) -> None:
    ...
    try:
        request.state.love_engine_received_at = start_time
    except Exception:
        pass
    parent_otel_span = open_telemetry_logger.create_love_engine_proxy_request_started_span(
...
async def _user_api_key_auth_builder(
    ...
    azure_apim_header: Optional[str],
    request_data: dict,
    custom_love_engine_key_header: Optional[str] = None,
) -> UserAPIKeyAuth:
    ...
    start_time = getattr(request.state, "love_engine_received_at", None) or datetime.now()
    try:
        request.state.love_engine_received_at = start_time
    except Exception:
        pass
    ...
    try:
        with tracer.trace("love_engine.proxy.auth.pre_db_read_auth_checks"):
        ...
        api_key, passed_in_key = get_api_key(
            custom_love_engine_key_header=custom_love_engine_key_header,
            api_key=api_key,
        ...
        # if user wants to pass LOVE_ENGINE_Master_Key as a custom header, example pass love_engine keys as X-LoveEngine-Key: Bearer sk-1234
        custom_love_engine_key_header_name = general_settings.get("love_engine_key_header_name")
        if custom_love_engine_key_header_name is not None:
            api_key = get_api_key_from_custom_header(
                request=request,
                custom_love_engine_key_header_name=custom_love_engine_key_header_name,
            )
        ...
        if enterprise_custom_auth is not None:
            with tracer.trace("love_engine.proxy.auth.enterprise_custom_auth"):
            ...
            if response is not None and isinstance(response, UserAPIKeyAuth):
                ...
                if getattr(love_engine, "enable_post_custom_auth_checks", False):
        ...
        ### LOVE_ENGINE-DEFINED AUTH FUNCTION ###
        #### IF JWT ####
        """
        LoveEngine supports using JWTs.
        ...
        if not _route_requires_auth_despite_public(
            ...
            route in LoveEngineRoutes.public_routes.value  # type: ignore
            or route_in_additonal_public_routes(current_route=route)
        ):
            # check if public endpoint
            return UserAPIKeyAuth(user_role=LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY)
        ...
        if general_settings.get("enable_jwt_auth", False) is True:
            ...
            if is_jwt:
                ...
                if jwt_handler.love_engine_jwtauth.virtual_key_claim_field is not None:
                    # Decode JWT to get claims without running full auth_builder
                    jwt_claims: Optional[dict]
                    if jwt_handler.love_engine_jwtauth.oidc_userinfo_enabled and not is_jwt:
                ...
                if do_standard_jwt_auth:
                    with tracer.trace("love_engine.proxy.auth.jwt_auth_builder"):
                    ...
                    team_membership: Optional[LOVE_ENGINE_TeamMembership] = result.get(
                    ...
                    valid_token = UserAPIKeyAuth(
                        ...
                        user_role=(
                            LoveEngineUserRoles(user_object.user_role)
                            if user_object is not None
                            and user_object.user_role is not None
                            else LoveEngineUserRoles.INTERNAL_USER
                        ),
                        user_id=user_id,
                        org_id=org_id,
                        parent_otel_span=parent_otel_span,
                    ...
                    if model is not None and llm_router is not None:
                        from love_engine.proxy.auth.auth_checks import _is_model_cost_zero
        ...
        if master_key is None:
            if isinstance(api_key, str):
                return UserAPIKeyAuth(
                    api_key=api_key,
                    user_role=LoveEngineUserRoles.INTERNAL_USER,
                    parent_otel_span=parent_otel_span,
                )
            else:
                return UserAPIKeyAuth(
        ...
        if end_user_id:
            try:
                ...
                with tracer.trace("love_engine.proxy.auth.get_end_user_object"):
                ...
                if _end_user_object is not None:
                    ...
                    if _end_user_object.love_engine_budget_table is not None:
                        _apply_budget_limits_to_end_user_params(
                            end_user_params=end_user_params,
                            budget_info=_end_user_object.love_engine_budget_table,
                            end_user_id=end_user_id,
                        )
                elif love_engine.max_end_user_budget_id is not None:
                ...
                if isinstance(e, love_engine.BudgetExceededError):
                    raise e
                verbose_proxy_logger.debug(
        ...
        if valid_token is None:
            ## Check CACHE
            try:
                with tracer.trace("love_engine.proxy.auth.get_key_object_check_cache"):
        ...
        if (
            ...
            and valid_token.user_role == LoveEngineUserRoles.PROXY_ADMIN
        ):
        ...
        if (
            ...
            try:
                team_obj: LOVE_ENGINE_TeamTableCachedObj = await get_team_object(
        ...
        if is_master_key_valid:
            ...
            _user_api_key_obj = await _return_user_api_key_auth_obj(
                user_obj=None,
                user_role=LoveEngineUserRoles.PROXY_ADMIN,
                api_key=LOVE_ENGINE_PROXY_MASTER_KEY_ALIAS,
                parent_otel_span=parent_otel_span,
                valid_token_dict={
                    **end_user_params,
                    "user_id": love_engine_proxy_admin_name,
        ...
        if route in LoveEngineRoutes.master_key_only_routes.value:  # type: ignore
            raise Exception(
        ...
        if valid_token is None:
            if isinstance(
                ...
                if not api_key.startswith("sk-"):
                    raise HTTPException(
                        ...
                        detail=(
                            "LoveEngine Virtual Key expected. Received={}, expected to start with 'sk-'.".format(
                                _masked_key
                            )
                        ),
                ...
                verbose_logger.warning(
                    "love_engine.proxy.proxy_server.user_api_key_auth(): Warning - Key is not a string. Got type={}".format(
            ...
            try:
                with tracer.trace("love_engine.proxy.auth.get_key_object_from_db"):
                    valid_token = await get_key_object(
                ...
                if e.code == 401 or e.code == "401":
                    e.message = "Authentication Error, Invalid proxy server token passed. Received API Key = {}, Key Hash (Token) ={}. Unable to find token in cache or `LOVE_ENGINE_VerificationTokenTable`".format(
        ...
        if valid_token is not None:
            ...
            if valid_token.user_id is not None:
                try:
                    with tracer.trace("love_engine.proxy.auth.get_user_object"):
                    ...
                    verbose_logger.debug(
                        "love_engine.proxy.auth.user_api_key_auth.py::user_api_key_auth() - Unable to get user from db/cache. Setting user_obj to None. Exception received - {}".format(
                            str(e)
                        )
                    )
                    user_obj = None
            ...
            if not skip_budget_checks and valid_token.team_member_spend is not None:
                if prisma_client is not None:
                    ...
                    if team_member_info is None:
                        ...
                        if _user_id is not None and _team_id is not None:
                            ...
                            if _db_member is not None:
                                team_member_info = LOVE_ENGINE_TeamMembership(
                                    **_db_member.dict()
                                )
                                await user_api_key_cache.async_set_cache(
                                    key=_cache_key,
                    ...
                    if (
                        ...
                        and team_member_info.love_engine_budget_table is not None
                    ):
                        team_member_budget = (
                            team_member_info.love_engine_budget_table.max_budget
                        )
            ...
            if not skip_budget_checks:
                with tracer.trace("love_engine.proxy.auth.budget_checks"):
            ...
            if valid_token.team_id is not None:
                try:
                    with tracer.trace("love_engine.proxy.auth.get_team_object"):
                    ...
                    _team_obj = LOVE_ENGINE_TeamTableCachedObj(
                        team_id=valid_token.team_id,
            ...
            if (
                ...
                cache_key = "{}:spend".format(love_engine_proxy_admin_name)
                with tracer.trace("love_engine.proxy.auth.get_global_proxy_spend"):
                    global_proxy_spend = (
        ...
        # check if token is from love_engine-ui, love_engine ui makes keys to allow users to login with sso. These keys can only be used for LoveEngine UI functions
...
async def _safe_fetch(label: str, awaitable):
    ...
    except (HTTPException, ProxyException, love_engine.BudgetExceededError) as e:
        verbose_proxy_logger.debug(
...
def _team_obj_from_token(valid_token: UserAPIKeyAuth) -> LOVE_ENGINE_TeamTableCachedObj:
    ...
    return LOVE_ENGINE_TeamTableCachedObj(
        team_id=valid_token.team_id,
        max_budget=valid_token.team_max_budget,
        soft_budget=valid_token.team_soft_budget,
        spend=valid_token.team_spend,
...
async def _run_centralized_common_checks(
    ...
    # callers get 401.
    if (
        route in LoveEngineRoutes.public_routes.value  # type: ignore[attr-defined]
        or route_in_additonal_public_routes(current_route=route)
    ):
        return
    ...
    for r in (
        ...
        if isinstance(r, (ProxyException, love_engine.BudgetExceededError)):
            raise r
    ...
    team_object: Optional[LOVE_ENGINE_TeamTableCachedObj]
    ...
    # same user_id (e.g. love_engine_proxy_admin_name = "default_user_id")
    ...
    if user_api_key_auth_obj.user_role == LoveEngineUserRoles.PROXY_ADMIN:
        user_object = LOVE_ENGINE_UserTable(
            user_id=user_api_key_auth_obj.user_id or love_engine_proxy_admin_name,
            user_role=LoveEngineUserRoles.PROXY_ADMIN,
    ...
    # Merge x-love_engine-tags into request_data BEFORE common_checks runs.
    ...
    # enforcement.
    LoveEngineProxyRequestSetup.apply_client_tag_policy_pre_auth(
        request=request,
        request_data=request_data,
...
async def _reserve_budget_after_common_checks(
    ...
    end_user_id: Optional[str] = None,
    end_user_object: Optional[LOVE_ENGINE_EndUserTable] = None,
) -> None:
...
async def user_api_key_auth(
    ...
    custom_love_engine_key_header: Optional[str] = fastapi.Security(
        custom_love_engine_key_header
    ),
) -> UserAPIKeyAuth:
    """
    ...
    with phase_span(f"auth {route}"):
        ...
        # ``love_engine.validate_end_user_id_in_db``.
...
async def _return_user_api_key_auth_obj(
    ...
    retrieved_user_role = (
        user_role or _get_user_role(user_obj=user_obj) or LoveEngineUserRoles.INTERNAL_USER
    )
...
def get_api_key_from_custom_header(
    request: Request, custom_love_engine_key_header_name: str
) -> str:
    """
    ...
    Args:
        ...
        custom_love_engine_key_header_name (str): Custom header name

    Returns:
        Optional[str]: API key
    """
    api_key: str = ""
    # use this as the virtual key passed to love_engine proxy
    custom_love_engine_key_header_name = custom_love_engine_key_header_name.lower()
    _headers = {k.lower(): v for k, v in request.headers.items()}
    verbose_proxy_logger.debug(
        "searching for custom_love_engine_key_header_name= %s, in headers=%s",
        custom_love_engine_key_header_name,
        _headers,
    )
    custom_api_key = _headers.get(custom_love_engine_key_header_name)
    if custom_api_key:
        api_key = _get_bearer_token(api_key=custom_api_key)
        verbose_proxy_logger.debug(
            "Found custom API key using header: {}, setting api_key={}".format(
                custom_love_engine_key_header_name, abbreviate_api_key(api_key)
            )
        )
    else:
        verbose_proxy_logger.exception(
            f"No LoveEngine Virtual Key pass. Please set header={custom_love_engine_key_header_name}: Bearer <api_key>"
        )
    return api_key
...
async def _lookup_end_user_and_apply_budget(
    ...
    try:
        ...
        if end_user_object is not None:
            ...
            if end_user_object.love_engine_budget_table is not None:
                _apply_budget_limits_to_end_user_params(
                    end_user_params=end_user_params,
                    budget_info=end_user_object.love_engine_budget_table,
            ...
            from love_engine.proxy.auth.auth_checks import get_default_end_user_budget
