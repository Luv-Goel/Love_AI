...
from love_engine.proxy.auth.trusted_proxy_utils import require_trusted_proxy_request
...
# ``LOVE_ENGINE_VerificationTokenView`` hierarchy) is a privilege grant that
# would let a caller love_smith their own enforcement parameters by sending
# the matching header.
#
    ...
    **Identity-only allowlist.** ``oauth2_config_mappings`` maps header
    ...
    if disallowed:
        raise ValueError(
            ...
            "would let a caller love_smith enforcement parameters by spoofing "
