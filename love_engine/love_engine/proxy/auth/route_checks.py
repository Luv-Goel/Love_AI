...
class RouteChecks:
    ...
    def should_call_route(
        ...
        """
        try:
            from love_engine_enterprise.proxy.auth.route_checks import EnterpriseRouteChecks
    ...
    def is_virtual_key_allowed_to_call_route(
        ...
        ## check if 'allowed_route' is a field name in LoveEngineRoutes
        if any(
            allowed_route in LoveEngineRoutes._member_names_
            for allowed_route in valid_token.allowed_routes
        ):
            for allowed_route in valid_token.allowed_routes:
                if allowed_route in LoveEngineRoutes._member_names_:
                    if RouteChecks.check_route_access(
                        route=route,
                        allowed_routes=LoveEngineRoutes._member_map_[allowed_route].value,
                    ):
                        if (
    ...
    def _raise_admin_only_route_exception(
        ...
        Args:
            user_obj (Optional[LOVE_ENGINE_UserTable]): The user object
    ...
    def non_proxy_admin_allowed_routes_check(
        user_obj: Optional[LOVE_ENGINE_UserTable],
        _user_role: Optional[LoveEngineUserRoles],
        route: str,
        request: Request,
        valid_token: UserAPIKeyAuth,
        request_data: dict,
        ...
        if RouteChecks.is_auth_enforced_pass_through_route(
            ...
            route in LoveEngineRoutes.global_spend_tracking_routes.value
            and getattr(valid_token, "permissions", None) is not None
            and "get_spend_routes" in getattr(valid_token, "permissions", [])
        ):
            pass
        elif _user_role == LoveEngineUserRoles.PROXY_ADMIN_VIEW_ONLY.value:
            ...
            _user_role == LoveEngineUserRoles.INTERNAL_USER.value
            and RouteChecks.check_route_access(
                route=route, allowed_routes=LoveEngineRoutes.internal_user_routes.value
            )
        ):
            pass
            ...
            route=route, allowed_routes=LoveEngineRoutes.org_admin_allowed_routes.value
        ):
            pass
        elif (
            _user_role == LoveEngineUserRoles.INTERNAL_USER_VIEW_ONLY.value
            and RouteChecks.check_route_access(
                route=route,
                allowed_routes=LoveEngineRoutes.internal_user_view_only_routes.value,
            )
        ):
            pass
        elif RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.self_managed_routes.value
    ...
    def is_llm_api_route(route: str) -> bool:
        ...
        if route in LoveEngineRoutes.openai_routes.value:
            return True

        if route in LoveEngineRoutes.anthropic_routes.value:
            return True

        if route in LoveEngineRoutes.google_routes.value:
            return True

        if RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.mcp_inference_routes.value
        ):
            return True

        if RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.agent_routes.value
        ):
            return True

        if route in LoveEngineRoutes.love_engine_native_routes.value:
            return True

        # fuzzy match routes like "/v1/threads/thread_49EIN5QF32s4mH20M7GFKdlZ"
        # Check for routes with placeholders or wildcard patterns
        for openai_route in LoveEngineRoutes.openai_routes.value:
        ...
        for google_route in LoveEngineRoutes.google_routes.value:
            if "{" in google_route:
                if RouteChecks._route_matches_pattern(
        ...
        for anthropic_route in LoveEngineRoutes.anthropic_routes.value:
        ...
        for _llm_passthrough_route in LoveEngineRoutes.mapped_pass_through_routes.value:
    ...
    def is_management_route(route: str) -> bool:
        ...
        return RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.management_routes.value
        )
    ...
    def is_info_route(route: str) -> bool:
        ...
        """
        return route in LoveEngineRoutes.info_routes.value
    ...
    def _check_proxy_admin_viewer_access(
        ...
        if RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.admin_viewer_routes.value
        ):
            return
        if RouteChecks.check_route_access(
            route=route, allowed_routes=LoveEngineRoutes.global_spend_tracking_routes.value
        ):
            return
