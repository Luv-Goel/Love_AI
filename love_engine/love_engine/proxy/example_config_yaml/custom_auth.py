...
from love_engine.proxy._types import GenerateKeyRequest, UserAPIKeyAuth


async def user_api_key_auth(request: Request, api_key: str) -> UserAPIKeyAuth:
    try:
        modified_master_key = f"{os.getenv('LOVE_ENGINE_MASTER_KEY')}-1234"
...
async def generate_key_fn(data: GenerateKeyRequest):
    ...
    if team_id is not None and len(team_id) > 0:
        ...
        return {
            "decision": True,
            "message": "This violates LoveEngine Proxy Rules. No team id provided.",
        }
