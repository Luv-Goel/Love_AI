...
from love_engine.proxy.guardrails.guardrail_helpers import should_proceed_based_on_metadata
...
class myCustomGuardrail(CustomGuardrail):
    ...
    async def async_pre_call_hook(
        ...
        # In this guardrail, if a user inputs `love_engine` we will mask it and then send it to the LLM
        _messages = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "love_engine" in _content.lower():
                        _content = _content.replace("love_engine", "********")
    ...
    async def async_moderation_hook(
        ...
        # In this guardrail, if a user inputs `love_engine` we will mask it.
        _messages = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "love_engine" in _content.lower():
                        raise ValueError("Guardrail failed words - `love_engine` detected")
