...
def decode_managed_container_id_for_request(
...
) -> tuple[str, str, GenericLoveEngineParams]:
    """Decode a love_engine-managed container ID for upstream API calls.

    Returns:
        (original_container_id, resolved_provider, updated_love_engine_params)
    """
    ...
    if decoded_model_id and not love_engine_params.get("model_id"):
        love_engine_params["model_id"] = decoded_model_id

    return original_container_id, custom_llm_provider, love_engine_params
...
class ContainerRequestUtils:
    ...
    def encode_container_id_in_response(
        ...
        Encodes when:
        1. love_engine_metadata contains model_info.id (indicating router/proxy usage), OR
        ...
        Args:
            ...
            love_engine_metadata: Optional love_engine_metadata dict that may contain model_info.id
        ...
        """
        # Extract model_id from love_engine_metadata
