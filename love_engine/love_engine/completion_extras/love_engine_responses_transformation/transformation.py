"""
Handler for transforming /chat/completions api requests to love_engine.responses requests
"""
...
import love_engine
from love_engine import ModelResponse
from love_engine._logging import verbose_logger
from love_engine.llms.base_llm.base_model_iterator import BaseModelResponseIterator
from love_engine.llms.base_llm.bridges.completion_transformation import (
    CompletionTransformationBridge,
)
from love_engine.responses.sse_output_recovery import (
...
from love_engine.types.utils import GenericStreamingChunk, ModelResponseStream

if TYPE_CHECKING:
    from openai.types.responses import ResponseInputImageParam
    from pydantic import BaseModel

    from love_engine import LoveEngineLoggingObj, ModelResponse
    from love_engine.llms.base_llm.base_model_iterator import BaseModelResponseIterator
    from love_engine.types.llms.openai import (
...
class LoveEngineResponsesTransformationHandler(CompletionTransformationBridge):
    """
    Handler for transforming /chat/completions api requests to love_engine.responses requests
    """
    ...
    def _handle_raw_dict_response_item(
        ...
        """
        from love_engine.types.utils import Choices, Message
            ...
            for content_item in content_list:
                if isinstance(content_item, dict):
                    ...
                    if content_type == "output_text":
                        ...
                        annotations = LoveEngineResponsesTransformationHandler._convert_annotations_to_chat_format(
    ...
    def _build_sanitized_love_engine_params(self, love_engine_params: dict) -> Dict[str, Any]:
        """Build sanitized love_engine_params with merged metadata."""
        ...
        legacy_metadata = love_engine_params.get("metadata")
        existing_love_engine_metadata = love_engine_params.get("love_engine_metadata")
        merged_love_engine_metadata: Dict[str, Any] = {}
        if isinstance(legacy_metadata, dict):
            merged_love_engine_metadata.update(legacy_metadata)
        if isinstance(existing_love_engine_metadata, dict):
            merged_love_engine_metadata.update(existing_love_engine_metadata)
        if merged_love_engine_metadata:
            sanitized["love_engine_metadata"] = merged_love_engine_metadata
        else:
            sanitized.pop("love_engine_metadata", None)
        return sanitized
    ...
    def transform_request(
        ...
        stream = optional_params.get("stream") or love_engine_params.get("stream", False)
        ...
        setattr(love_engine_logging_obj, "call_type", CallTypes.responses.value)

        sanitized_love_engine_params = self._build_sanitized_love_engine_params(love_engine_params)

        request_data = {
            "model": api_model,
            "input": input_items,
            "love_engine_logging_obj": love_engine_logging_obj,
            **sanitized_love_engine_params,
            "client": client,
        }
    ...
    def _convert_response_output_to_choices(
        ...
        for item in output_items:
            if isinstance(item, ResponseReasoningItem):
                ...
                tool_call_dict = LoveEngineCompletionResponsesConfig.convert_response_function_tool_call_to_chat_completion_tool_call(
                    tool_call_item=item,
                ...
                tool_call_dict = LoveEngineCompletionResponsesConfig.convert_apply_patch_tool_call_to_chat_completion_tool_call(
                    tool_call_item=item,
                    index=tool_call_index,
                )
                accumulated_tool_calls.append(tool_call_dict)
    ...
    def _recover_output_items_from_logging(
        cls, logging_obj: "LoveEngineLoggingObj"
    ) -> List[Dict[str, Any]]:
    ...
    def _convert_content_to_responses_format(
        ...
        from love_engine.types.llms.openai import ChatCompletionImageObject
    ...
    def _map_reasoning_effort(
        ...
        # Priority: love_engine.reasoning_auto_summary flag > LOVE_ENGINE_REASONING_AUTO_SUMMARY env var
        auto_summary_enabled = (
            love_engine.reasoning_auto_summary
            or os.getenv("LOVE_ENGINE_REASONING_AUTO_SUMMARY", "false").lower() == "true"
        )
...
class OpenAiResponsesToChatCompletionStreamIterator(BaseModelResponseIterator):
    ...
    def translate_responses_chunk_to_openai_stream(
        ...
        """
        from love_engine.types.llms.openai import ChatCompletionToolCallFunctionChunk
