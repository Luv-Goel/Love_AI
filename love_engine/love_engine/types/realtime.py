...
class RealtimeClientSecretRequest(BaseModel):
    ...
    LoveEngine also accepts a top-level `model` field for routing when
    session.model is absent (LoveEngine extension, not forwarded to OpenAI).
    """

    expires_after: Optional[RealtimeExpiresAfter] = None
    session: Optional[RealtimeSessionConfig] = None
    # LoveEngine-only routing hint — stripped before forwarding upstream
    model: Optional[str] = None
...
class RealtimeTranscriptionSessionRequest(BaseModel):
    ...
    for routing is taken from the LoveEngine-only top-level `model` hint, falling
