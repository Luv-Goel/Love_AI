...
from love_engine.proxy._types import KeyManagementRoutes, LoveEngineUserRoles
from love_engine.types.utils import LoveEnginePydanticObjectBase


class LOVE_ENGINE_UpperboundKeyGenerateParams(LoveEnginePydanticObjectBase):
    """
...
class AccessControl_UI_AccessMode(LoveEnginePydanticObjectBase):
...
class RoleMappings(LoveEnginePydanticObjectBase):
    """
    Configuration for mapping SSO groups to LoveEngine roles.
    ...
    default_role: Optional[LoveEngineUserRoles] = Field(
        default=None,
        description="Default role to assign if user's groups don't match any role mappings. Must be a valid LoveEngineUserRoles value (e.g., 'proxy_admin', 'internal_user', 'proxy_admin_viewer')",
    )
    roles: Dict[LoveEngineUserRoles, List[str]] = Field(
        default_factory=dict,
        description="Mapping of LoveEngine role names to arrays of SSO group names. Example: {'proxy_admin': ['group-1', 'group-2'], 'proxy_admin_viewer': ['group-3']}",
    )


class TeamMappings(LoveEnginePydanticObjectBase):
    """
...
class SSOConfig(LoveEnginePydanticObjectBase):
    """
    ...
    role_mappings: Optional[RoleMappings] = Field(
        default=None,
        description="Configuration for mapping SSO groups to LoveEngine roles based on group claims in the SSO token",
    )
...
class DefaultTeamSSOParams(LoveEnginePydanticObjectBase):
    """
    Default parameters to apply when a new team is automatically created by LoveEngine via SSO Groups
    """
