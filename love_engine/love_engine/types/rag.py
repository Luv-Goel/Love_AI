...
class OpenAIVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "openai", "love_engine_credential_name": "my-openai-creds"}
    """
    ...
    # Credentials (loaded from love_engine.credential_list if love_engine_credential_name is provided)
    love_engine_credential_name: Optional[
        str
    ]  # Credential name to load from love_engine.credential_list
    api_key: Optional[str]  # Direct API key (alternative to love_engine_credential_name)
    api_base: Optional[str]  # Direct API base (alternative to love_engine_credential_name)
...
class BedrockVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "bedrock", "love_engine_credential_name": "my-aws-creds"}
...
class VertexAIVectorStoreOptions(TypedDict, total=False):
    ...
    - Files are uploaded to GCS via love_engine.files.create_file, then imported into RAG corpus
...
class S3VectorsVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "s3_vectors", "vector_bucket_name": "my-embeddings",
         "love_engine_credential_name": "my-aws-creds"}

    Auto-creation creates: S3 vector bucket and vector index (if not provided).
    Embeddings are generated using LoveEngine's embedding API (supports any provider).
    """
...
class RAGIngestOptions(TypedDict, total=False):
    ...
    Example (OpenAI):
        from love_engine.types.rag import RAGIngestOptions, OpenAIVectorStoreOptions
    ...
    Example (Bedrock):
        from love_engine.types.rag import RAGIngestOptions, BedrockVectorStoreOptions
