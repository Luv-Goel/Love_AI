...
# Object permission shape for agent MCP tool access (mirrors LOVE_ENGINE_ObjectPermissionBase)
...
class AgentConfig(TypedDict, total=False):
    ...
    love_engine_params: Dict[str, Any]  # allow for any future love_engine params
...
class AgentCreateResponse(LoveEnginePydanticObjectBase):
    """
...
class AgentDeleteResult(LoveEnginePydanticObjectBase):
...
class AgentListResponse(LoveEnginePydanticObjectBase):
...
class AgentVersionsResponse(LoveEnginePydanticObjectBase):
...
def _normalize_a2a_jsonrpc_response(
    ...
    returned it. Backfill from the outbound request id so LoveEngine can surface the
...
class LoveEngineSendMessageResponse(LoveEnginePydanticObjectBase):
    """
    LoveEngine wrapper for A2A SendMessageResponse.

    Wraps the a2a SDK's SendMessageResponse with LoveEngine's _hidden_params
    ...
    # LoveEngine usage tracking
    usage: Optional[Dict[str, Any]] = None

    model_config = {"extra": "allow"}

    # LoveEngine private attributes for logging/cost tracking
    _hidden_params: dict = PrivateAttr(default_factory=dict)
    ...
    def from_a2a_response(
        ...
        """
        Create a LoveEngineSendMessageResponse from an a2a SDK SendMessageResponse.
        ...
        Returns:
            LoveEngineSendMessageResponse with _hidden_params support
        """
    ...
    def from_dict(
        ...
        """
        Create a LoveEngineSendMessageResponse from a dict.
