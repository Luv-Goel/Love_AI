"""
love_engine A2A SDK functions.

Provides standalone functions with @client decorator for love_engine logging integration.
"""
...
from love_engine.a2a_protocol.streaming_iterator import A2AStreamingIterator
from love_engine.a2a_protocol.utils import A2ARequestUtils
from love_engine.constants import DEFAULT_A2A_AGENT_TIMEOUT
...
from love_engine.utils import client
...
from love_engine.a2a_protocol.card_resolver import LoveEngineA2ACardResolver
from love_engine.a2a_protocol.exception_mapping_utils import (
    handle_a2a_localhost_retry,
    map_a2a_exception,
)
from love_engine.a2a_protocol.exceptions import A2ALocalhostURLError

# Use our custom resolver instead of the default A2A SDK resolver
A2ACardResolver = LoveEngineA2ACardResolver
...
def _set_usage_on_logging_obj(
    ...
    """
    Set usage on love_engine_logging_obj for standard logging payload.

    Args:
        kwargs: The kwargs dict containing love_engine_logging_obj
        prompt_tokens: Number of input tokens
        completion_tokens: Number of output tokens
    """
    love_engine_logging_obj = kwargs.get("love_engine_logging_obj")
    if love_engine_logging_obj is not None:
        usage = love_engine.Usage(
            prompt_tokens=prompt_tokens,
            completion_tokens=completion_tokens,
            total_tokens=prompt_tokens + completion_tokens,
        )
        love_engine_logging_obj.model_call_details["usage"] = usage
...
def _set_agent_id_on_logging_obj(
    ...
    """
    Set agent_id on love_engine_logging_obj for SpendLogs tracking.
    ...
    if love_engine_logging_obj is not None:
        ...
        love_engine_logging_obj.model_call_details["agent_id"] = agent_id
...
def _get_a2a_model_info(a2a_client: Any, kwargs: Dict[str, Any]) -> str:
    ...
    # Set on love_engine_logging_obj if available (for standard logging payload)
    love_engine_logging_obj = kwargs.get("love_engine_logging_obj")
    if love_engine_logging_obj is not None:
        love_engine_logging_obj.model = model
        love_engine_logging_obj.custom_llm_provider = custom_llm_provider
...
async def _send_message_via_completion_bridge(
    ...
    """
    Route a send_message through the love_engine completion bridge (e.g. LangGraph, Bedrock AgentCore).
    ...
    return LoveEngineSendMessageResponse.from_dict(
...
async def asend_message(
    ...
    **kwargs: Any,
    ...
    Uses the @client decorator for love_engine logging and tracking.
    If love_engine_params contains custom_llm_provider, routes through the completion bridge.
    ...
    Args:
        ...
        love_engine_params: Optional dict with custom_llm_provider, model, etc. for completion bridge
        agent_id: Optional agent ID for tracking in SpendLogs
        **kwargs: Additional arguments passed to the client decorator

    Returns:
        LoveEngineSendMessageResponse (wraps a2a SendMessageResponse with _hidden_params)

    Example (standard A2A):
        ```python
        from love_engine.a2a_protocol import asend_message, create_a2a_client
    ...
    Example (completion bridge with LangGraph):
        ```python
        from love_engine.a2a_protocol import asend_message
        from a2a.types import SendMessageRequest, MessageSendParams
        from uuid import uuid4
        ...
        response = await asend_message(
            ...
            love_engine_params={"custom_llm_provider": "langgraph", "model": "agent"},
        )
        ```
    """
    love_engine_params = love_engine_params or {}
    logging_obj = kwargs.get("love_engine_logging_obj")
    trace_id = getattr(logging_obj, "love_engine_trace_id", None) if logging_obj else None
    custom_llm_provider = love_engine_params.get("custom_llm_provider")
    ...
    if a2a_client is None:
        ...
        extra_headers: Dict[str, str] = {"X-love_engine-Trace-Id": trace_id}
        if agent_id:
            extra_headers["X-love_engine-Agent-Id"] = agent_id
        # Overlay agent-level headers (agent headers take precedence over love_engine internal ones)
    ...
    agent_card = getattr(a2a_client, "_love_engine_agent_card", None) or getattr(
    ...
    # Wrap in love_engine response type for _hidden_params support
    response = LoveEngineSendMessageResponse.from_a2a_response(
...
def send_message(
    ...
    request: "SendMessageRequest",
    **kwargs: Any,
) -> Union[LoveEngineSendMessageResponse, Coroutine[Any, Any, LoveEngineSendMessageResponse]]:
    """
...
def _build_streaming_logging_obj(
    ...
    logging_obj = Logging(
        ...
        love_engine_call_id=str(request.id),
    ...
    _love_engine_params = love_engine_params.copy() if love_engine_params else {}
    if metadata:
        _love_engine_params["metadata"] = metadata
    if proxy_server_request:
        _love_engine_params["proxy_server_request"] = proxy_server_request

    logging_obj.love_engine_params = _love_engine_params
    logging_obj.optional_params = _love_engine_params
    logging_obj.model_call_details["love_engine_params"] = _love_engine_params
...
async def asend_message_streaming(
    ...
    Example (completion bridge with LangGraph):
        ```python
        from love_engine.a2a_protocol import asend_message_streaming
    ...
    if a2a_client is None:
        ...
        streaming_extra_headers: Dict[str, str] = {
            "X-love_engine-Trace-Id": str(request.id),
        }
        if agent_id:
            streaming_extra_headers["X-love_engine-Agent-Id"] = agent_id
        if agent_extra_headers:
...
async def create_a2a_client(
    ...
    Example:
        ```python
        from love_engine.a2a_protocol import create_a2a_client, asend_message
    ...
    a2a_client._love_engine_agent_card = agent_card  # type: ignore[attr-defined]
...
async def aget_agent_card(
    ...
    # Use love_engine's cached httpx client
