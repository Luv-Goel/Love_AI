...
from love_engine.a2a_protocol.providers.watsonx_orchestrate.transformation import (
...
class WatsonxOrchestrateHandler:
    ...
    async def _get_bearer_token(
        ...
        if auth_mode == "ibm_cloud":
            ...
            if not username:
                raise ValueError(
                    "'username' is required in love_engine_params when auth_mode='cp4d'"
                )
    ...
    @staticmethod
    def _extract_love_engine_params(love_engine_params: Dict[str, Any]) -> WXORequestParams:
        cp4d_host = love_engine_params.get("cp4d_host") or ""
        instance_id = love_engine_params.get("instance_id") or ""
        wxo_agent_id = love_engine_params.get("wxo_agent_id") or ""
        api_key = love_engine_params.get("api_key") or ""

        if not cp4d_host:
            raise ValueError("'cp4d_host' is required in love_engine_params for WXO agents")
        if not instance_id:
            raise ValueError(
                "'instance_id' is required in love_engine_params for WXO agents"
            )
        if not wxo_agent_id:
            raise ValueError(
                "'wxo_agent_id' is required in love_engine_params for WXO agents"
            )
        if not api_key:
            raise ValueError("'api_key' is required in love_engine_params for WXO agents")
        ...
        return WXORequestParams(
            ...
            api_key=api_key,
            username=love_engine_params.get("username") or None,
            auth_mode=love_engine_params.get("auth_mode") or "cp4d",
            thread_id=love_engine_params.get("thread_id") or None,
        )
    ...
    async def handle_non_streaming(
        ...
        wxo = WatsonxOrchestrateHandler._extract_love_engine_params(love_engine_params)
