...
from love_engine.llms.bedrock.chat.agentcore.transformation import AmazonAgentCoreConfig
...
# ``runtimeSessionId`` / ``runtimeUserId`` in the agent's ``love_engine_params``;
...
class BedrockAgentCoreA2ATransformation:
    ...
    def get_url_and_signed_request(
        ...
        Args:
            ...
            love_engine_params: Agent's love_engine_params (model, api_key, etc.)
            ...
            agent_extra_headers: Per-request headers (from x-a2a-{agent}-* rewrite and
                ...
                ``api_key`` / ``runtimeUserId`` / ``runtimeSessionId`` in love_engine_params
        ...
        # Build optional_params from love_engine_params (everything except model and custom_llm_provider)
        optional_params = {
            k: v
