"""
Handler for A2A to love_engine completion bridge.

Routes A2A requests through love_engine.acompletion based on custom_llm_provider.
...
from love_engine.a2a_protocol.providers.config_manager import A2AProviderConfigManager
from love_engine.interactions.agents.utils import merge_agent_headers

# love_engine_params key carrying the authenticated principal (hashed virtual key) so
...
A2A_USER_API_KEY_HASH_PARAM = "love_engine_a2a_user_api_key_hash"

# Agent metadata fields stored in love_engine_params that are not valid love_engine.acompletion() kwargs
...
class A2ACompletionBridgeHandler:
    """
    Static methods for handling A2A requests via love_engine completion.
    """
    ...
    async def handle_non_streaming(
        ...
        *,
        ...
        """
        Handle non-streaming A2A request via love_engine.acompletion.

        Args:
            request_id: A2A JSON-RPC request ID
            params: A2A MessageSendParams containing the message
            love_engine_params: Agent's love_engine_params (custom_llm_provider, model, etc.)
        ...
        custom_llm_provider = love_engine_params.get("custom_llm_provider")
        model = love_engine_params.get("model", "agent")
        ...
        # Add love_engine_params (contains api_key, client_id, client_secret, tenant_id, etc.)
        love_engine_params_to_add = {
            k: v
            for k, v in love_engine_params.items()
            if k not in ("model", "custom_llm_provider") and k not in _AGENT_ONLY_PARAMS
        }
        completion_params.update(love_engine_params_to_add)
        # Apply forward metadata AFTER the love_engine_params merge so the helper
        ...
        # Call love_engine.acompletion
        response = await love_engine.acompletion(**completion_params)
    ...
    async def handle_streaming(
        ...
        *,
        ...
        """
        Handle streaming A2A request via love_engine.acompletion with stream=True.
        ...
        # Call love_engine.acompletion with streaming
        response = await love_engine.acompletion(**completion_params)
