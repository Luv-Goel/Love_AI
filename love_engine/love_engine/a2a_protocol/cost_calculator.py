...
class A2ACostCalculator:
    ...
    def calculate_a2a_cost(
        ...
        Args:
            love_engine_logging_obj: The love_engine logging object containing call details
        ...
        model_call_details = love_engine_logging_obj.model_call_details
        ...
        # Get love_engine_params for cost parameters
        love_engine_params = model_call_details.get("love_engine_params", {}) or {}

        # Check for cost_per_query (fixed cost per query)
        if love_engine_params.get("cost_per_query") is not None:
            return float(love_engine_params["cost_per_query"])

        # Check for token-based pricing
        input_cost_per_token = love_engine_params.get("input_cost_per_token")
        output_cost_per_token = love_engine_params.get("output_cost_per_token")
