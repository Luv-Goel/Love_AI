...
from love_engine.constants import CACHED_STREAMING_CHUNK_DELAY
from love_engine.love_engine_core_utils.model_param_helper import ModelParamHelper
from love_engine.types.caching import *
from love_engine.types.utils import EmbeddingResponse, all_love_engine_params
...
#### love_engine.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: Optional[LoveEngineCacheType] = LoveEngineCacheType.LOCAL,
        mode: Optional[
            CacheMode
        ...
        **kwargs,
        ...
        Returns:
            None. Cache is set as a love_engine param
        """
        if type == LoveEngineCacheType.REDIS:
            # Check REDIS_CLUSTER_NODES env var if no explicit startup nodes
            if not redis_startup_nodes:
                _env_cluster_nodes = love_engine.get_secret("REDIS_CLUSTER_NODES")
        ...
        elif type == LoveEngineCacheType.REDIS_SEMANTIC:
        ...
        elif type == LoveEngineCacheType.QDRANT_SEMANTIC:
        ...
        elif type == LoveEngineCacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == LoveEngineCacheType.S3:
            self.cache = S3Cache(
        ...
        elif type == LoveEngineCacheType.GCS:
            self.cache = GCSCache(
                bucket_name=gcs_bucket_name,
                path_service_account=gcs_path_service_account,
                gcs_path=gcs_path,
            )
        elif type == LoveEngineCacheType.AZURE_BLOB:
            self.cache = AzureBlobCache(
                account_url=azure_account_url,
                container=azure_blob_container,
            )
        elif type == LoveEngineCacheType.DISK:
            self.cache = DiskCache(disk_cache_dir=disk_cache_dir)
        if "cache" not in love_engine.input_callback:
            love_engine.input_callback.append("cache")
        if "cache" not in love_engine.success_callback:
            love_engine.logging_callback_manager.add_love_engine_success_callback("cache")
        if "cache" not in love_engine._async_success_callback:
            love_engine.logging_callback_manager.add_love_engine_async_success_callback("cache")
        ...
        if self.type == LoveEngineCacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == LoveEngineCacheType.REDIS
            or self.type == LoveEngineCacheType.REDIS_SEMANTIC
    ...
    def get_cache_key(self, **kwargs) -> str:
        ...
        Args:
            **kwargs: kwargs to love_engine.completion() or embedding()
        ...
        love_engine_param_kwargs = all_love_engine_params
        for param in kwargs:
            if param in combined_kwargs:
                ...
                param not in love_engine_param_kwargs
            ):  # check if user passed in optional param - e.g. top_k
                if (
                    love_engine.enable_caching_on_provider_specific_optional_params is True
    ...
    def _get_model_param_value(self, kwargs: dict) -> str:
        ...
        1. If caching groups are set, then return the caching group as the model https://docs.love_engine.ai/docs/routing#caching-across-model-groups
        2. Else if a model_group is set, then return the model_group as the model. This is used for all requests sent through the love_engine.Router()
        3. Else use the `model` passed in kwargs
        """
        metadata: Dict = kwargs.get("metadata", {}) or {}
        love_engine_params: Dict = kwargs.get("love_engine_params", {}) or {}
        metadata_in_love_engine_params: Dict = love_engine_params.get("metadata", {}) or {}
        model_group: Optional[str] = metadata.get(
            "model_group"
        ) or metadata_in_love_engine_params.get("model_group")
    ...
    def _get_file_param_value(self, kwargs: dict) -> str:
        ...
        return (
            ...
            or love_engine_params.get("file_name")
        )

    def _get_preset_cache_key_from_kwargs(self, **kwargs) -> Optional[str]:
        """
        Get the preset cache key from kwargs["love_engine_params"]
        ...
        """
        if kwargs:
            if "love_engine_params" in kwargs:
                return kwargs["love_engine_params"].get("preset_cache_key", None)
        return None
    ...
    def _set_preset_cache_key_in_kwargs(self, preset_cache_key: str, **kwargs) -> None:
        ...
        Placed in kwargs["love_engine_params"]
        """
        if kwargs:
            if "love_engine_params" in kwargs:
                kwargs["love_engine_params"]["preset_cache_key"] = preset_cache_key
    ...
    def get_cache(self, dynamic_cache_object: Optional[BaseCache] = None, **kwargs):
        ...
        Args:
            *args: args to love_engine.completion() or embedding()
            **kwargs: kwargs to love_engine.completion() or embedding()
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception(f"love_engine Cache: Excepton add_cache: {str(e)}")
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("love_engine: Enabling Cache")
    if "cache" not in love_engine.input_callback:
        love_engine.input_callback.append("cache")
    if "cache" not in love_engine.success_callback:
        love_engine.logging_callback_manager.add_love_engine_success_callback("cache")
    ...
    print_verbose(f"love_engine: Cache enabled, love_engine.cache={love_engine.cache}")
    print_verbose(f"love_engine Cache: {vars(love_engine.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    Update the cache for love_engine.
    ...
    """
    print_verbose("love_engine: Updating Cache")
    love_engine.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"love_engine: Cache Updated, love_engine.cache={love_engine.cache}")
    print_verbose(f"love_engine Cache: {vars(love_engine.cache)}")


def disable_cache():
    """
    Disable the cache used by love_engine.

    This function disables the cache used by the love_engine module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the love_engine.cache attribute to None.
    ...
    print_verbose("love_engine: Disabling Cache")
    with suppress(ValueError):
        love_engine.input_callback.remove("cache")
        love_engine.success_callback.remove("cache")
        love_engine._async_success_callback.remove("cache")

    love_engine.cache = None
    print_verbose(f"love_engine: Cache disabled, love_engine.cache={love_engine.cache}")
