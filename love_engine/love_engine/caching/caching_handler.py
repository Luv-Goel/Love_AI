...
from love_engine.caching.caching import S3Cache
from love_engine.love_engine_core_utils.llm_response_utils.response_metadata import (
    update_response_metadata,
)
from love_engine.love_engine_core_utils.logging_utils import (
    _assemble_complete_response_from_streaming_chunks,
)
from love_engine.types.caching import CachedEmbedding
...
if TYPE_CHECKING:
    ...
    from love_engine.types.utils import PromptTokensDetailsWrapper
else:
...
class LLMCachingHandler:
    def __init__(
        ...
        if love_engine.cache is not None and isinstance(love_engine.cache.cache, RedisCache):
            self.dual_cache: Optional[DualCache] = DualCache(
                redis_cache=love_engine.cache.cache,
    ...
    async def _async_get_cache(
        ...
        if (
            (kwargs.get("caching", None) is None and love_engine.cache is not None)
            ...
            if love_engine.cache is not None and self._is_call_type_supported_by_cache(
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    self._update_love_engine_logging_obj_environment(
                    ...
                    cache_key = (
                        ...
                        or love_engine.cache.get_cache_key(**self.request_kwargs)
                    )
                    ...
                    and not isinstance(
                        love_engine.cache.cache, S3Cache
    ...
    def _sync_get_cache(
        ...
        if love_engine.cache is not None and self._is_call_type_supported_by_cache(
            ...
            if new_kwargs.get("stream") is True and "cache_key" not in new_kwargs:
                new_kwargs["cache_key"] = love_engine.cache.get_cache_key(**new_kwargs)
            self.request_kwargs = new_kwargs
            print_verbose("Checking Sync Cache")
            cached_result = love_engine.cache.get_cache(**new_kwargs)
            ...
            for val in non_null_list:
                ...
                if cr is not None:
                    ...
                    if cached_prompt_tokens is not None:
                        ...
                        from love_engine.utils import token_counter
    ...
    def _async_log_cache_hit_on_callbacks(
        ...
        Args:
            logging_obj (LoveEngineLoggingObj): The logging object.
    ...
    async def _retrieve_from_cache(
        ...
        if call_type == CallTypes.aembedding.value:
            ...
            for idx, i in enumerate(new_kwargs["input"]):
                preset_cache_key = love_engine.cache.get_cache_key(
                    **{**new_kwargs, "input": i}
                )
                tasks.append(
                    love_engine.cache.async_get_cache(
            ...
            if love_engine.cache._supports_async() is True:
                ## check if dual cache is supported ##
                self.preset_cache_key = (
                    request_cache_key or love_engine.cache.get_cache_key(**request_kwargs)
                )
                cached_result = await love_engine.cache.async_get_cache(
                    dynamic_cache_object=self.dual_cache,
                ...
                cached_result = love_engine.cache.get_cache(
                    dynamic_cache_object=self.dual_cache,
                    cache_key=self.preset_cache_key,
                    **request_kwargs,
                )
    ...
    def _convert_cached_result_to_model_response(
        ...
        Returns:
            Optional[Any]:
        """
        from love_engine.utils import convert_to_model_response_object
    ...
    async def async_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            if (
                isinstance(result, love_engine.ModelResponse)
                or isinstance(result, love_engine.EmbeddingResponse)
                ...
                if (
                    ...
                    asyncio.create_task(
                        love_engine.cache.async_add_cache_pipeline(
                ...
                asyncio.create_task(love_engine.cache.async_add_cache(result, **new_kwargs))
    ...
    def sync_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            love_engine.cache.add_cache(result, **new_kwargs)
    ...
    def _should_store_result_in_cache(
        ...
        """
        return (
            (love_engine.cache is not None)
            and love_engine.cache.supported_call_types is not None
            and (str(original_function.__name__) in love_engine.cache.supported_call_types)
    ...
    def _is_call_type_supported_by_cache(
        ...
        Defined on `love_engine.types.utils.CallTypes`
    ...
    async def _add_streaming_response_to_cache(self, processed_chunk: ModelResponse):
        ...
        - If 'streaming_chunk' has a 'finish_reason' then assemble a love_engine.ModelResponse object
    ...
    def _update_love_engine_logging_obj_environment(
        self,
        logging_obj: LoveEngineLoggingObj,
        model: str,
        kwargs: Dict[str, Any],
        ...
        """
        Helper function to update the LoveEngineLoggingObj environment variables.

        Args:
            logging_obj (LoveEngineLoggingObj): The logging object to update.
        ...
        if love_engine.cache is not None:
            love_engine_params["preset_cache_key"] = (
                love_engine.cache._get_preset_cache_key_from_kwargs(**kwargs)
            )
        else:
            love_engine_params["preset_cache_key"] = None
