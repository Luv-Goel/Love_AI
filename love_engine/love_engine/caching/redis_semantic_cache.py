"""
Redis Semantic Cache implementation for love_engine
...
import love_engine
from love_engine._logging import print_verbose
from love_engine.love_engine_core_utils.prompt_templates.common_utils import (
    get_str_from_messages,
)
from love_engine.types.utils import EmbeddingResponse
...
class RedisSemanticCache(BaseCache):
    ...
    DEFAULT_REDIS_INDEX_NAME: str = "love_engine_semantic_cache_index"
    CACHE_KEY_FIELD_NAME: str = "love_engine_cache_key"
    ...
    def _cache_hit_matches_key(self, cache_hit: Dict[str, Any], key: str) -> bool:
        # Pre-isolation entries with no ``love_engine_cache_key`` field cannot be
    ...
    async def _get_async_embedding(self, prompt: str, **kwargs) -> List[float]:
        ...
        Returns:
            List[float]: The embedding vector
        """
        from love_engine.proxy.proxy_server import llm_model_list, llm_router
        ...
        try:
            if llm_router is not None and self.embedding_model in router_model_names:
                ...
                embedding_response = await love_engine.aembedding(
                    model=self.embedding_model,
                    input=prompt,
                    cache={"no-store": True, "no-cache": True},
                )
