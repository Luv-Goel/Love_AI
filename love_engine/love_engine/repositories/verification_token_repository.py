"""
VerificationToken repository for database operations on LOVE_ENGINE_VerificationToken.
"""
...
from love_engine.models.verification_token import (
    LOVE_ENGINE_VerificationToken,
)
from love_engine.repositories.base_repository import BaseRepository


class VerificationTokenRepository(BaseRepository[LOVE_ENGINE_VerificationToken]):
    """Repository for verification token (API key) database operations."""

    @property
    def table(self) -> Any:
        return self.prisma_client.db.love_engine_verificationtoken

    @property
    def deleted_table(self) -> Any:
        return self.prisma_client.db.love_engine_deletedverificationtoken

    @property
    def model_class(self) -> Type[LOVE_ENGINE_VerificationToken]:
        return LOVE_ENGINE_VerificationToken

    def _to_model(self, record: Any) -> Optional[LOVE_ENGINE_VerificationToken]:
        ...
        return LOVE_ENGINE_VerificationToken(**data)
    ...
    async def find_by_user_id(self, user_id: str) -> List[LOVE_ENGINE_VerificationToken]:
        """Find all tokens belonging to a user."""
        records = await self.table.find_many(where={"user_id": user_id})
        return self._to_model_list(records)

    async def find_by_team_id(self, team_id: str) -> List[LOVE_ENGINE_VerificationToken]:
    ...
    async def find_active_tokens(self) -> List[LOVE_ENGINE_VerificationToken]:
    ...
    async def delete_token(
        ...
        archive_data["love_engine_changed_by"] = love_engine_changed_by
        archive_data["deleted_at"] = datetime.utcnow()

        async with self.prisma_client.db.tx() as tx:
            await tx.love_engine_deletedverificationtoken.create(data=archive_data)
            await tx.love_engine_verificationtoken.delete(where={"token": token})

        return token_record

    def _build_archive_data(self, token: LOVE_ENGINE_VerificationToken) -> Dict[str, Any]:
        """Build archive data with only columns present in LOVE_ENGINE_DeletedVerificationToken.
        ...
        for field in ("object_permission", "love_engine_budget_table", "budget_limits"):
            data.pop(field, None)
