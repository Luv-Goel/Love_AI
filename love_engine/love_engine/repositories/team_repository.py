"""
Team repository for database operations on LOVE_ENGINE_TeamTable.
"""
...
class TeamRepository(BaseRepository[LOVE_ENGINE_TeamTable]):
    """Repository for team database operations."""

    @property
    def table(self) -> Any:
        return self.prisma_client.db.love_engine_teamtable

    @property
    def deleted_table(self) -> Any:
        return self.prisma_client.db.love_engine_deletedteamtable

    @property
    def model_class(self) -> Type[LOVE_ENGINE_TeamTable]:
        return LOVE_ENGINE_TeamTable

    def _to_model(self, record: Any) -> Optional[LOVE_ENGINE_TeamTable]:
        ...
        return LOVE_ENGINE_TeamTable(**data)
    ...
    async def find_by_alias(self, team_alias: str) -> Optional[LOVE_ENGINE_TeamTable]:
    ...
    async def find_by_member(self, user_id: str) -> List[LOVE_ENGINE_TeamTable]:
        """Find all teams where user is a member."""
        records = await self.table.find_many(where={"members": {"has": user_id}})
        return self._to_model_list(records)

    async def find_by_admin(self, user_id: str) -> List[LOVE_ENGINE_TeamTable]:
    ...
    async def delete_team(
        ...
        async with self.prisma_client.db.tx() as tx:
            await tx.love_engine_deletedteamtable.create(data=archive_data)
            await tx.love_engine_teamtable.delete(where={"team_id": team_id})

        return team

    def _build_archive_data(self, team: LOVE_ENGINE_TeamTable) -> Dict[str, Any]:
        """Build archive data dict with only columns that exist in LOVE_ENGINE_DeletedTeamTable."""
