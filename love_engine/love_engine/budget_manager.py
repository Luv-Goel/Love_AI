...
class BudgetManager:
    def __init__(
        ...
        self.api_base = api_base or "https://api.love_engine.ai"
    ...
    def projected_cost(self, model: str, messages: list, user: str):
        ...
        prompt_tokens = love_engine.token_counter(model=model, text=text)
        prompt_cost, _ = love_engine.cost_per_token(
    ...
    def update_cost(
        ...
        if model and input_text and output_text:
            ...
            cost = love_engine.completion_cost(completion_response=completion_obj)
            model = completion_obj[
                "model"
            ]  # if this throws an error try, model = completion_obj['model']
        else:
            raise ValueError(
                "Either a chat completion object or the text response needs to be passed in. Learn more - https://docs.love_engine.ai/docs/budget_manager"
            )
