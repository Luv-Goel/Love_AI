...
from love_engine.utils import get_optional_params
...
def batch_completion(
    ...
    **kwargs,
    ...
    """
    Batch love_engine.completion function for a given model.
    ...
    if model.split("/", 1)[0] in love_engine.provider_list:
            ...
            for sub_batch in chunks(batch_messages, 100):
                for message_list in sub_batch:
                    ...
                    future = executor.submit(
                        love_engine.completion, **kwargs_modified, **original_kwargs
                    )
        ...
        with ThreadPoolExecutor(max_workers=len(models)) as executor:
            for model in models:
                futures[model] = executor.submit(
                    love_engine.completion, *args, model=model, **kwargs
                )
...
def batch_completion_models_all_responses(*args, **kwargs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(models)) as executor:
        futures = [
            executor.submit(love_engine.completion, *args, model=model, **kwargs)
            for model in models
        ]
