...
def _get_otel_v2_class() -> Optional[type]:
    ...
    Imported lazily: ``love_engine.integrations.otel.logger`` imports the OpenTelemetry
    SDK at module scope, so importing it eagerly would break installs without the
    SDK. The V2 logger only exists when ``LOVE_ENGINE_OTEL_V2`` is enabled (which
...
class ServiceLogging(CustomLogger):
    """
    Separate class used for monitoring health of love_engine-adjacent services (redis/postgres).
    """
    ...
    def __init__(self, mock_testing: bool = False) -> None:
        ...
        if "prometheus_system" in love_engine.service_callback:
    ...
    def _resolve_otel_service_logger(self, callback: Any) -> Optional[Any]:
        ...
        showed up under ``LOVE_ENGINE_OTEL_V2``. Match both the legacy and V2 types,
    ...
    async def async_service_success_hook(
        ...
        for callback in love_engine.service_callback:
    ...
    async def async_post_call_failure_hook(
        ...
        """
        Hook to track failed love_engine-service calls
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        """
        Hook to track latency for love_engine proxy llm api calls
        """
        try:
            ...
            verbose_logger.exception(
                f"ServiceLogger: async_log_success_event Error - {str(e)}"
            )
