"""
love_engine Proxy uses this MCP Client to connnect to other MCP servers.
"""
...
from love_engine.constants import MCP_CLIENT_TIMEOUT, MCP_NPM_CACHE_DIR
from love_engine.llms.custom_httpx.http_handler import get_ssl_configuration
from love_engine.types.llms.custom_http import VerifyTypes
...
class MCPSigV4Auth(httpx.Auth):
    ...
    def _assume_role(
        ...
        session_name = (
            aws_session_name or f"love_engine-mcp-{int(__import__('time').time())}"
        )
...
class MCPClient:
    ...
    def _create_transport_context(
        ...
        headers = self._get_auth_headers()
        httpx_client_factory = self._create_httpx_client_factory()
        verbose_logger.debug("love_engine headers for streamable_http_client: %s", headers)
    ...
    def _get_safe_stdio_env(
        ...
        to avoid leaking sensitive love_engine keys (OPENAI_API_KEY, etc.) to sub-processes.
        """
    ...
    def _create_httpx_client_factory(self) -> Callable[..., httpx.AsyncClient]:
        """
        Create a custom httpx client factory that uses love_engine's SSL configuration.
        ...
        def factory(
            *,
            ...
            """Create an httpx.AsyncClient with love_engine's SSL configuration."""
