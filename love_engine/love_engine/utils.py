"""Utility helpers for love_engine core request handling and provider support."""
...
# What is this?
## Generic utils.py file. Problem-specific utils (e.g. 'cost calculation), should all be in `love_engine_core_utils/`.
import sys
import textwrap
import threading
import time
...
import love_engine
import love_engine.love_engine_core_utils

# audio_utils.utils is lazy-loaded - only imported when needed for transcription calls
import love_engine.love_engine_core_utils.json_validation_rule
from love_engine._internal_context import is_internal_call
from love_engine._lazy_imports import (
...
def _get_cached_custom_guardrail():
    ...
    if _CustomGuardrail is None:
        from love_engine.integrations.custom_guardrail import CustomGuardrail
...
def _get_cached_caching_handler_response():
    ...
    if _CachingHandlerResponse is None:
        from love_engine.caching.caching_handler import CachingHandlerResponse
...
def _get_cached_llm_caching_handler():
    ...
    if _LLMCachingHandler is None:
        from love_engine.caching.caching_handler import LLMCachingHandler
...
def _get_cached_audio_utils():
    ...
    if _audio_utils_module is None:
        import love_engine.love_engine_core_utils.audio_utils.utils

        _audio_utils_module = love_engine.love_engine_core_utils.audio_utils.utils
...
from love_engine.types.utils import FileTypes  # type: ignore
...
# | https://github.com/BerriAI/love_engine/issues/new |
# |                                               |
# +-----------------------------------------------+
#
...
try:
    # Python 3.9+
    with (
        resources.files("love_engine.love_engine_core_utils.tokenizers")
    ...
    with resources.open_text(
        "love_engine.love_engine_core_utils.tokenizers", "anthropic_tokenizer.json"
    ) as f:
        json_data = json.load(f)
...
from love_engine.llms.base_llm.base_utils import (
    BaseLLMModelInfo,
...
if TYPE_CHECKING:
    ...
    # their modules at runtime during `love_engine` import.
    from love_engine.caching.caching_handler import (
        CachingHandlerResponse,
        LLMCachingHandler,
    )
    from love_engine.integrations.custom_logger import CustomLogger
    from love_engine.llms.base_llm.files.transformation import BaseFilesConfig
    from love_engine.llms.base_llm.realtime.http_transformation import (
        BaseRealtimeHTTPConfig,
    )
    from love_engine.proxy._types import AllowedModelRegion
    ...
    from love_engine.love_engine_core_utils.exception_mapping_utils import exception_type
    from love_engine.love_engine_core_utils.get_llm_provider_logic import (
        _is_non_openai_azure_model,
        get_llm_provider,
    )
    from love_engine.love_engine_core_utils.get_supported_openai_params import (
        get_supported_openai_params,
    )
    from love_engine.love_engine_core_utils.llm_response_utils.convert_dict_to_response import (
    ...
    from love_engine.love_engine_core_utils.llm_response_utils.get_api_base import get_api_base
    from love_engine.love_engine_core_utils.llm_response_utils.response_metadata import (
        ResponseMetadata,
    )
    from love_engine.love_engine_core_utils.prompt_templates.common_utils import (
        _parse_content_for_reasoning,
    )
    from love_engine.love_engine_core_utils.redact_messages import (
        LoveEngineLoggingObject,
        redact_message_input_output_from_logging,
    )
    from love_engine.love_engine_core_utils.streaming_handler import CustomStreamWrapper
    from love_engine.llms.base_llm.google_genai.transformation import (
        BaseGoogleGenAIGenerateContentConfig,
    )
    from love_engine.llms.base_llm.ocr.transformation import BaseOCRConfig
    from love_engine.llms.base_llm.search.transformation import BaseSearchConfig
    from love_engine.llms.base_llm.text_to_speech.transformation import (
        BaseTextToSpeechConfig,
    )
    from love_engine.llms.bedrock.common_utils import BedrockModelInfo
    from love_engine.llms.cohere.common_utils import CohereModelInfo
    from love_engine.llms.mistral.ocr.transformation import MistralOCRConfig

    # Type stubs for lazy-loaded functions and classes
    from love_engine.love_engine_core_utils.cached_imports import (
        get_coroutine_checker,
        get_love_engine_logging_class,
        get_set_callbacks,
    )
    from love_engine.love_engine_core_utils.core_helpers import (
        get_love_engine_metadata_from_kwargs,
        map_finish_reason,
        process_response_headers,
    )
    from love_engine.love_engine_core_utils.dot_notation_indexing import (
        delete_nested_value,
        is_nested_path,
    )
    from love_engine.love_engine_core_utils.get_love_engine_params import (
        _get_base_model_from_love_engine_call_metadata,
        get_love_engine_params,
    )
    from love_engine.love_engine_core_utils.llm_request_utils import _ensure_extra_body_is_safe
    from love_engine.love_engine_core_utils.llm_response_utils.get_formatted_prompt import (
        get_formatted_prompt,
    )
    from love_engine.love_engine_core_utils.llm_response_utils.get_headers import (
        get_response_headers,
    )
    from love_engine.love_engine_core_utils.llm_response_utils.response_metadata import (
        update_response_metadata,
    )
    from love_engine.love_engine_core_utils.rules import Rules
    from love_engine.love_engine_core_utils.thread_pool_executor import executor
    from love_engine.llms.base_llm.anthropic_messages.transformation import (
    ...
    from love_engine.router_utils.get_retry_from_policy import (
        get_num_retries_from_retry_policy,
        reset_retry_policy,
    )

    # Type stubs for lazy-loaded config classes and types
    from love_engine.llms.base_llm.batches.transformation import BaseBatchesConfig
    from love_engine.llms.base_llm.containers.transformation import BaseContainerConfig
    from love_engine.llms.base_llm.embedding.transformation import BaseEmbeddingConfig
    from love_engine.llms.base_llm.image_edit.transformation import BaseImageEditConfig
    from love_engine.llms.base_llm.image_generation.transformation import (
        BaseImageGenerationConfig,
    )
    from love_engine.llms.base_llm.image_variations.transformation import (
    ...
    from love_engine.llms.base_llm.rerank.transformation import BaseRerankConfig
    from love_engine.llms.base_llm.vector_store.transformation import BaseVectorStoreConfig
    from love_engine.llms.base_llm.vector_store_files.transformation import (
        BaseVectorStoreFilesConfig,
    )
    from love_engine.llms.base_llm.videos.transformation import BaseVideoConfig
    from love_engine.types.llms.anthropic import (
        ANTHROPIC_API_ONLY_HEADERS,
        AnthropicThinkingParam,
    )
    from love_engine.types.rerank import RerankResponse
    from love_engine.types.llms.openai import (
        ChatCompletionDeltaToolCallChunk,
        ChatCompletionToolCallChunk,
        ChatCompletionToolCallFunctionChunk,
    )
    from love_engine.types.router import LOVE_ENGINE_Params

from love_engine.secret_managers.main import get_secret

from love_engine.llms.base_llm.chat.transformation import BaseConfig
from love_engine.llms.base_llm.completion.transformation import BaseTextCompletionConfig
from love_engine.llms.base_llm.evals.transformation import BaseEvalsAPIConfig
from love_engine.llms.base_llm.responses.transformation import BaseResponsesAPIConfig
...
if TYPE_CHECKING:
    from love_engine import MockException
...
# All love_engine Model responses will be in this format, Follows the OpenAI Format
# https://docs.love_engine.ai/docs/completion/output
# {
#   'choices': [
#      {
#         'finish_reason': 'stop',
...
def print_verbose(
    ...
    try:
        ...
        if love_engine.set_verbose is True and logger_only is False:
...
def custom_llm_setup():
    ...
    """
    for custom_llm in love_engine.custom_provider_map:
        if custom_llm["provider"] not in love_engine.provider_list:
            love_engine.provider_list.append(custom_llm["provider"])

        if custom_llm["provider"] not in love_engine._custom_providers:
            love_engine._custom_providers.append(custom_llm["provider"])
...
def _add_custom_logger_callback_to_specific_event(
    ...
    if callback not in love_engine._known_custom_logger_compatible_callbacks:
        verbose_logger.debug(
            f"Callback {callback} is not a valid custom logger compatible callback. Known list - {love_engine._known_custom_logger_compatible_callbacks}"
        )
        return
    ...
    if callback_class:
        if (
            ...
            love_engine.logging_callback_manager.add_love_engine_success_callback(
                callback_class
            )
            love_engine.logging_callback_manager.add_love_engine_async_success_callback(
                callback_class
            )
            if callback in love_engine.success_callback:
                love_engine.success_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in love_engine._async_success_callback:
                love_engine._async_success_callback.remove(
                    callback
                )  # remove the string from the callback list
        elif (
            ...
            love_engine.logging_callback_manager.add_love_engine_failure_callback(
                callback_class
            )
            love_engine.logging_callback_manager.add_love_engine_async_failure_callback(
                callback_class
            )
            if callback in love_engine.failure_callback:
                love_engine.failure_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in love_engine._async_failure_callback:
                love_engine._async_failure_callback.remove(
                    callback
                )  # remove the string from the callback list
...
def _custom_logger_class_exists_in_success_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in love_engine.success_callback or love_engine._async_success_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in love_engine.success_callback or love_engine._async_success_callback

    Prevents double adding a custom logger callback to the love_engine callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in love_engine.success_callback + love_engine._async_success_callback
    )
...
def _custom_logger_class_exists_in_failure_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in love_engine.failure_callback or love_engine._async_failure_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in love_engine.failure_callback or love_engine._async_failure_callback

    Prevents double adding a custom logger callback to the love_engine callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in love_engine.failure_callback + love_engine._async_failure_callback
    )
...
def get_applied_guardrails(kwargs: Dict[str, Any]) -> List[str]:
    ...
    for callback in love_engine.callbacks:
...
def load_credentials_from_list(kwargs: dict):
    ...
    credential_name = kwargs.get("love_engine_credential_name")
    if credential_name and love_engine.credential_list:
...
def get_dynamic_callbacks(
    ...
    returned_callbacks = love_engine.callbacks.copy()
...
def function_setup(
    ...
    ### NOTICES ###
    if love_engine.set_verbose is True:
        verbose_logger.warning(
            "`love_engine.set_verbose` is deprecated. Please set `os.environ['LOVE_ENGINE_LOG'] = 'DEBUG'` for debug logs."
        )
    try:
        ...
        if len(all_callbacks) > 0:
            for callback in all_callbacks:
                ...
                if isinstance(callback, str):
                    callback = love_engine.love_engine_core_utils.love_engine_logging._init_custom_logger_compatible_class(  # type: ignore
                        callback, internal_usage_cache=None, llm_router=None  # type: ignore
                    )
                    if callback is None or any(
                        isinstance(cb, type(callback))
                        for cb in love_engine._async_success_callback
                    ):  # don't double add a callback
                        continue
                if callback not in love_engine.input_callback:
                    love_engine.input_callback.append(callback)  # type: ignore
                if callback not in love_engine.success_callback:
                    love_engine.logging_callback_manager.add_love_engine_success_callback(callback)  # type: ignore
                if callback not in love_engine.failure_callback:
                    love_engine.logging_callback_manager.add_love_engine_failure_callback(callback)  # type: ignore
                if callback not in love_engine._async_success_callback:
                    love_engine.logging_callback_manager.add_love_engine_async_success_callback(callback)  # type: ignore
                if callback not in love_engine._async_failure_callback:
                    love_engine.logging_callback_manager.add_love_engine_async_failure_callback(callback)  # type: ignore
            print_verbose(
                f"Initialized love_engine callbacks, Async Success Callbacks: {love_engine._async_success_callback}"
            )

        if (
            len(love_engine.input_callback) > 0
            or len(love_engine.success_callback) > 0
            or len(love_engine.failure_callback) > 0
        ) and len(
            callback_list  # type: ignore
        ) == 0:  # type: ignore
            callback_list = list(
                set(
                    love_engine.input_callback  # type: ignore
                    + love_engine.success_callback
                    + love_engine.failure_callback
                )
            )
        ...
        if len(love_engine.input_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(love_engine.input_callback):  # type: ignore
                if coroutine_checker.is_async_callable(callback):
                    love_engine._async_input_callback.append(callback)
                    removed_async_items.append(index)

            # Pop the async items from input_callback in reverse order to avoid index issues
            for index in reversed(removed_async_items):
                love_engine.input_callback.pop(index)
        if len(love_engine.success_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(love_engine.success_callback):  # type: ignore
                if coroutine_checker.is_async_callable(callback):
                    love_engine.logging_callback_manager.add_love_engine_async_success_callback(
                        callback
                    )
                    ...
                    callback in love_engine._known_custom_logger_compatible_callbacks
            ...
            for index in reversed(removed_async_items):
                love_engine.success_callback.pop(index)

        if len(love_engine.failure_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(love_engine.failure_callback):  # type: ignore
                if coroutine_checker.is_async_callable(callback):
                    love_engine.logging_callback_manager.add_love_engine_async_failure_callback(
                        callback
                    )
            ...
            for index in reversed(removed_async_items):
                love_engine.failure_callback.pop(index)
        ### DYNAMIC CALLBACKS ###
        ...
        if add_breadcrumb:
            try:
                from love_engine.love_engine_core_utils.core_helpers import safe_deep_copy

                details_to_log = safe_deep_copy(kwargs)
            except Exception:
                details_to_log = kwargs

            if love_engine.turn_off_message_logging:
                # make a copy of the _model_Call_details and log it
                details_to_log.pop("messages", None)
                details_to_log.pop("input", None)
                details_to_log.pop("prompt", None)
            add_breadcrumb(
                category="love_engine.llm_call",
        ...
        if (
            ...
            if isinstance(messages, list) and len(messages) > 0:
                try:
                    ...
                    from love_engine.love_engine_core_utils.prompt_templates.factory import (
            ...
            try:
                from love_engine.google_genai.adapters.transformation import (
                    GoogleGenAIAdapter,
                )
                from love_engine.love_engine_core_utils.prompt_templates.common_utils import (
        ...
        if _is_streaming_request(
            ...
            stream = True
        get_love_engine_logging_class = getattr(
            sys.modules[__name__], "get_love_engine_logging_class"
        )
        logging_obj = get_love_engine_logging_class()(  # Victim for object pool
            model=model,  # type: ignore
            messages=messages,
            stream=stream,
            love_engine_call_id=kwargs["love_engine_call_id"],
            love_engine_trace_id=kwargs.get("love_engine_trace_id"),
        ...
        love_engine_params: Dict[str, Any] = {"api_base": ""}
        if "metadata" in kwargs:
            love_engine_params["metadata"] = kwargs["metadata"]
        if "love_engine_metadata" in kwargs and isinstance(
            kwargs["love_engine_metadata"], dict
        ):
            love_engine_params["love_engine_metadata"] = kwargs["love_engine_metadata"].copy()
            # For endpoints like /v1/messages that use "love_engine_metadata" instead
            # of "metadata" (to avoid conflicting with provider API metadata fields),
            # populate love_engine_params["metadata"] so callbacks (e.g. Langfuse) that
            # read API key info from love_engine_params["metadata"] see the fields.
            if not love_engine_params.get("metadata"):
                love_engine_params["metadata"] = kwargs["love_engine_metadata"].copy()
        ...
        verbose_logger.exception(
            "love_engine.utils.py::function_setup() - [Non-Blocking] Error in function_setup"
        )
        raise e


async def _client_async_logging_helper(
    logging_obj: LoveEngineLoggingObject,
    result,
    start_time,
    end_time,
    ...
    if (
    ...
    ):  # don't log the parent event love_engine.completion_with_fallbacks as a 'log_success_event', this will lead to double logging the same call - https://github.com/BerriAI/love_engine/issues/7477
        print_verbose(
        ...
        ################################################
        from love_engine.love_engine_core_utils.logging_worker import GLOBAL_LOGGING_WORKER
...
def _get_wrapper_num_retries(
    ...
    if num_retries is None:
        num_retries = love_engine.num_retries
                    ...
                    if (
                        ...
                        if model_response is not None:
                            ...
                            _enable_json_schema_validation = (
                                ...
                                else love_engine.enable_json_schema_validation
                            )
                                ...
                                try:
                                    if (
                                        ...
                                        if json_response_format is not None:
                                            love_engine.love_engine_core_utils.json_validation_rule.validate_schema(
...
def client(original_function):
    ...
    def wrapper(*args, **kwargs):
        ...
        if _is_async_request(kwargs):
            ...
            if love_engine.num_retries_per_request is not None:
                # check if previous_models passed in as ['love_engine_params']['metadata]['previous_models']
                previous_models = (kwargs.get("metadata") or {}).get(
                    "previous_models", None
                )
                if previous_models is not None:
                    if love_engine.num_retries_per_request <= len(previous_models):
            ...
            if _is_streaming_request(
                ...
                if (
                    ...
                    for idx, chunk in enumerate(result):
                        chunks.append(chunk)
                    return love_engine.stream_chunk_builder(
        ...
        # Prints Exactly what was passed to love_engine function - don't execute any logic here - it should just print
        print_args_passed_to_love_engine(original_function, args, kwargs)
        start_time = datetime.datetime.now()
        result = None
        logging_obj: Optional[LoveEngineLoggingObject] = kwargs.get(
            "love_engine_logging_obj", None
        )

        # only set love_engine_call_id if its not in kwargs
        if "love_engine_call_id" not in kwargs:
            kwargs["love_engine_call_id"] = str(uuid.uuid4())
        ...
        try:
            ...
            kwargs["love_engine_logging_obj"] = logging_obj
            ...
            if love_engine.max_budget:
                if love_engine._current_cost > love_engine.max_budget:
                    raise BudgetExceededError(
                        current_cost=love_engine._current_cost,
                        max_budget=love_engine.max_budget,
                    )
            ...
            print_verbose(
                f"SYNC kwargs[caching]: {kwargs.get('caching', False)}; love_engine.cache: {love_engine.cache}; kwargs.get('cache')['no-cache']: {kwargs.get('cache', {}).get('no-cache', False)}"
            )
                ...
                (
                    (
                        (
                            ...
                            and love_engine.cache is not None
                        )
            ...
            if call_type == CallTypes.completion.value:
                num_retries = (
                    kwargs.get("num_retries", None) or love_engine.num_retries or None
                )
                if kwargs.get("retry_policy", None):
                    get_num_retries_from_retry_policy = getattr(
                        sys.modules[__name__], "get_num_retries_from_retry_policy"
                ...
                _is_love_engine_router_call = "model_group" in (
                    kwargs.get("metadata") or {}
                )  # check if call from love_engine.router/proxy
                if (
                    num_retries and not _is_love_engine_router_call
                ):  # only enter this if call is not from love_engine router/proxy. router has it's own logic for retrying
                    if (
                        ...
                        return love_engine.completion_with_retries(*args, **kwargs)
                elif (
                    isinstance(e, love_engine.exceptions.ContextWindowExceededError)
                    and context_window_fallback_dict
                    and model in context_window_fallback_dict
                    and not _is_love_engine_router_call
                ):
                    if len(args) > 0:
                ...
                if (
                    ...
                    if (
                        ...
                        kwargs["num_retries"] = num_retries
                        return love_engine.responses_with_retries(*args, **kwargs)
    ...
    async def wrapper_async(*args, **kwargs):
        ...
        kwargs.pop("_is_love_engine_internal_call", None)  # discard if injected
        _is_love_engine_internal_call = is_internal_call.get()
        ...
        try:
            ...
            if _is_debugging_on():
                print_verbose(
                    f"ASYNC kwargs[caching]: {kwargs.get('caching', False)}; love_engine.cache: {love_engine.cache}; kwargs.get('cache'): {kwargs.get('cache', None)}"
                )
            ...
            if not _is_love_engine_internal_call:
            ...
            traceback_exception = traceback.format_exc()
            end_time = datetime.datetime.now()
            if logging_obj and not _is_love_engine_internal_call:
                try:
                    logging_obj.failure_handler(
                        e, traceback_exception, start_time, end_time
                    )  # DO NOT MAKE THREADED - router retry fallback relies on this!
            ...
            if call_type == CallTypes.acompletion.value:
                ...
                if (
                    ...
                    try:
                        ...
                        return await love_engine.acompletion_with_retries(*args, **kwargs)
                    except Exception:
                        pass
                elif (
                    isinstance(e, love_engine.exceptions.ContextWindowExceededError)
                ...
                if (
                    ...
                    try:
                        ...
                        if isinstance(
                            ...
                            kwargs["retry_strategy"] = "constant_retry"
                        return await love_engine.aresponses_with_retries(*args, **kwargs)
                    except Exception:
                        pass
...
def _is_async_request(
    ...
    eg. love_engine.acompletion, love_engine.aimage_generation, love_engine.acreate_batch, love_engine._arealtime

    Args:
        kwargs (dict): The kwargs passed to the love_engine function
...
def _is_streaming_request(
    ...
    Returns True if:
        - if "stream=True" in kwargs  (love_engine chat completion, love_engine text completion, love_engine messages)
        - if call_type is generate_content_stream or agenerate_content_stream (love_engine google genai)
    """
...
def _select_tokenizer_helper(model: str) -> SelectTokenizerResponse:
    if love_engine.disable_hf_tokenizer_download is True:
...
def _return_huggingface_tokenizer(model: str) -> Optional[SelectTokenizerResponse]:
    if model in love_engine.cohere_models and "command-r" in model:
        # cohere
    ...
    # anthropic
    elif model in love_engine.anthropic_models and "claude-3" not in model:
...
def decode(
    ...
    Args:
        skip_special_tokens: For HuggingFace tokenizers, keep the historical
            love_engine round-trip behavior by omitting special tokens by default.
...
def token_counter(
    ...
    """
    The same as `love_engine.love_engine_core_utils.token_counter`.
    ...
    #########################################################
    if love_engine.disable_token_counter is True:
        return 0
...
def supports_native_streaming(model: str, custom_llm_provider: Optional[str]) -> bool:
    ...
    """
    try:
        model, custom_llm_provider, _, _ = love_engine.get_llm_provider(
...
def supports_response_schema(
    ...
    PROVIDERS_GLOBALLY_SUPPORT_RESPONSE_SCHEMA = [
        love_engine.LlmProviders.PREDIBASE,
        love_engine.LlmProviders.FIREWORKS_AI,
        love_engine.LlmProviders.LM_STUDIO,
        love_engine.LlmProviders.NEBIUS,
        love_engine.LlmProviders.DATABRICKS,
    ]
    ...
    try:
        ...
        if model_info.get(key, False) is True:
            ...
            if bare_model_key is not None:
                bare_entry = love_engine.model_cost.get(bare_model_key) or {}
...
def get_supported_regions(
    ...
    try:
        ...
        model_cost_data = love_engine.model_cost.get(model_key, {})
...
def _resolve_builtin_model_cost_entry(
    ...
    for candidate in candidates:
        entry = love_engine.model_cost.get(candidate)
        if entry is not None and entry.get("love_engine_provider") is not None:
            return dict(entry)
    return None
...
def register_model(model_cost: Union[str, dict]):
    ...
    if isinstance(model_cost, dict):
        ...
        loaded_model_cost = love_engine.get_model_cost_map(url=model_cost)
    ...
    for key, value in loaded_model_cost.items():
        ...
        # ``get_model_info`` returns ``love_engine_provider: None`` when the
        # provider is unknown (e.g. custom deployments registered via
        # ``Router.add_deployment``). Persisting that None into
        # ``love_engine.model_cost`` causes ``_check_provider_match`` to drop
        # custom pricing on subsequent cost lookups.
        if existing_model.get("love_engine_provider") is None:
            existing_model.pop("love_engine_provider", None)
        ...
        _raw_entry = love_engine.model_cost.get(model_cost_key)
        if _raw_entry is None:
            _raw_entry = love_engine.model_cost.get(key)
        if _raw_entry is None:
            _raw_entry = {}
        ...
        love_engine.model_cost.setdefault(model_cost_key, {}).update(updated_dictionary)

        # Invalidate case-insensitive lookup map since model_cost was modified
        _invalidate_model_cost_lowercase_map()

        verbose_logger.debug(
            f"added/updated model={model_cost_key} in love_engine.model_cost: {model_cost_key}"
        )
        # add new model names to provider lists
        if value.get("love_engine_provider") == "openai":
            if key not in love_engine.open_ai_chat_completion_models:
                love_engine.open_ai_chat_completion_models.add(key)
        elif value.get("love_engine_provider") == "text-completion-openai":
            if key not in love_engine.open_ai_text_completion_models:
                love_engine.open_ai_text_completion_models.add(key)
        elif value.get("love_engine_provider") == "cohere":
            if key not in love_engine.cohere_models:
                love_engine.cohere_models.add(key)
        elif value.get("love_engine_provider") == "anthropic":
            if key not in love_engine.anthropic_models:
                love_engine.anthropic_models.add(key)
        elif value.get("love_engine_provider") == "openrouter":
            split_string = key.split("/", 1)
            if split_string[-1] not in love_engine.openrouter_models:
                love_engine.openrouter_models.add(split_string[-1])
        elif value.get("love_engine_provider") == "vercel_ai_gateway":
            if key not in love_engine.vercel_ai_gateway_models:
                love_engine.vercel_ai_gateway_models.add(key)
        elif value.get("love_engine_provider") == "vertex_ai-text-models":
            if key not in love_engine.vertex_text_models:
                love_engine.vertex_text_models.add(key)
        elif value.get("love_engine_provider") == "vertex_ai-code-text-models":
            if key not in love_engine.vertex_code_text_models:
                love_engine.vertex_code_text_models.add(key)
        elif value.get("love_engine_provider") == "vertex_ai-chat-models":
            if key not in love_engine.vertex_chat_models:
                love_engine.vertex_chat_models.add(key)
        elif value.get("love_engine_provider") == "vertex_ai-code-chat-models":
            if key not in love_engine.vertex_code_chat_models:
                love_engine.vertex_code_chat_models.add(key)
        elif value.get("love_engine_provider") == "ai21":
            if key not in love_engine.ai21_models:
                love_engine.ai21_models.add(key)
        elif value.get("love_engine_provider") == "nlp_cloud":
            if key not in love_engine.nlp_cloud_models:
                love_engine.nlp_cloud_models.add(key)
        elif value.get("love_engine_provider") == "aleph_alpha":
            if key not in love_engine.aleph_alpha_models:
                love_engine.aleph_alpha_models.add(key)
        elif value.get("love_engine_provider") == "bedrock":
            if key not in love_engine.bedrock_models:
                love_engine.bedrock_models.add(key)
        elif value.get("love_engine_provider") == "novita":
            if key not in love_engine.novita_models:
                love_engine.novita_models.add(key)
    return model_cost
...
def get_optional_params_transcription(
    ...
    **kwargs,
    ...
    from love_engine.constants import OPENAI_TRANSCRIPTION_PARAMS
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    drop_params is True or love_engine.drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting user/encoding format is not supported by {custom_llm_provider}. To drop it from the call, set `love_engine.drop_params = True`.",
                    )
            return non_default_params
    ...
    if custom_llm_provider == "openai" or custom_llm_provider == "azure":
        ...
        supported_params = love_engine.GroqSTTConfig().get_supported_openai_params_stt()
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.GroqSTTConfig().map_openai_params_stt(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    love_engine.drop_params is True or drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                    ...
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting `{k}` is not supported by {custom_llm_provider}, {model}. To drop it from the call, set `love_engine.drop_params = True`.",
                    )
            return non_default_params
    ...
    if provider_config is not None:
        ...
        or custom_llm_provider in love_engine.openai_compatible_providers
    ):
        optional_params = non_default_params
    elif custom_llm_provider == "bedrock":
        config_class = love_engine.BedrockImageGeneration.get_config_class(model=model)
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: Optional[list]):
        ...
        if unsupported_params:
            if love_engine.drop_params is True or (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {unsupported_params}, for model={model}. To drop these, set `love_engine.drop_params=True` or for proxy:\n\n`love_engine_settings:\n drop_params: true`\n",
                )
    ...
    ## raise exception if non-default value passed for non-openai/azure embedding calls
        ...
        if (
            ...
            # Honor drop_params (per-call) and love_engine.drop_params (global) the same
            # way `_check_valid_arg` does above. The raised error message itself
            # tells users to set `drop_params=True`, so respect it here.
            if love_engine.drop_params is True or drop_params is True:
                non_default_params.pop("dimensions", None)
            else:
                raise UnsupportedParamsError(
                    status_code=500,
                    message="Setting dimensions is not supported for OpenAI `text-embedding-3` and later models. To drop it from the call, set `love_engine.drop_params = True`.",
                )
        optional_params = non_default_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.TritonEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.DatabricksEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.nvidiaNimEmbeddingConfig.map_openai_params(
        ...
        (
            optional_params,
            kwargs,
        ) = love_engine.VertexAITextEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}, kwargs=kwargs
        )
    elif custom_llm_provider == "lm_studio":
        supported_params = (
            love_engine.LmStudioEmbeddingConfig().get_supported_openai_params()
        )
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.LmStudioEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
    elif custom_llm_provider == "bedrock":
        ...
        if "amazon.titan-embed-text-v1" in model:
            object: Any = love_engine.AmazonTitanG1Config()
        elif "amazon.titan-embed-image-v1" in model:
            object = love_engine.AmazonTitanMultimodalEmbeddingG1Config()
        elif "amazon.titan-embed-text-v2:0" in model:
            object = love_engine.AmazonTitanV2Config()
        elif "cohere.embed-multilingual-v3" in model or "cohere.embed-v4" in model:
            object = love_engine.BedrockCohereEmbeddingConfig()
        elif "twelvelabs" in model or "marengo" in model:
            object = love_engine.TwelveLabsMarengoEmbeddingConfig()
        elif "nova" in model.lower():
            object = love_engine.AmazonNovaEmbeddingConfig()
        else:  # unmapped model
            supported_params = []
            _check_valid_arg(supported_params=supported_params)
            final_params = {**kwargs}
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.MistralEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.JinaAIEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        if love_engine.VoyageContextualEmbeddingConfig.is_contextualized_embeddings(model):
            optional_params = (
                love_engine.VoyageContextualEmbeddingConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params={},
                    model=model,
                    drop_params=drop_params if drop_params is not None else False,
        ...
        elif love_engine.VoyageMultimodalEmbeddingConfig.is_multimodal_embeddings(model):
            optional_params = (
                love_engine.VoyageMultimodalEmbeddingConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params={},
                    model=model,
                    drop_params=drop_params if drop_params is not None else False,
            ...
            optional_params = love_engine.VoyageEmbeddingConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params={},
                model=model,
                drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.GenAIHubEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.InfinityEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.FireworksAIEmbeddingConfig().map_openai_params(
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.SambaNovaEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = love_engine.OVHCloudEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        if len(non_default_params.keys()) > 0:
            if (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"Setting {non_default_params} is not supported by {custom_llm_provider}. To drop it from the call, set `love_engine.drop_params = True`.",
                )
    elif (
        custom_llm_provider != "openai"
        and custom_llm_provider != "azure"
        and custom_llm_provider not in love_engine.openai_compatible_providers
    ):
        if len(non_default_params.keys()) > 0:
            if (
                love_engine.drop_params is True or drop_params is True
...
def _remove_additional_properties(schema):
    ...
    Relevant Issues: https://github.com/BerriAI/love_engine/issues/6136, https://github.com/BerriAI/love_engine/issues/6088
    """
...
def filter_out_love_engine_params(kwargs: dict) -> dict:
    """
    Filter out love_engine internal parameters from kwargs dict.

    Returns a new dict containing only non-love_engine parameters that should be
    passed to external provider APIs.

    Args:
        kwargs: Dictionary that may contain love_engine internal parameters

    Returns:
        Dictionary with love_engine internal parameters filtered out

    Example:
        >>> kwargs = {"query": "test", "shared_session": session_obj, "metadata": {}}
        >>> filtered = filter_out_love_engine_params(kwargs)
        >>> # filtered = {"query": "test"}
    """

    return {
        key: value for key, value in kwargs.items() if key not in all_love_engine_params
    }
    ...
    def base_pre_process_non_default_params(
        ...
        for k, v in special_params.items():
            if k == "aws_bedrock_project_id":
                # sent as a request header (read from love_engine_params by the
...
def pre_process_non_default_params(
    ...
    ):  # fixes https://github.com/BerriAI/love_engine/issues/4933
...
def pre_process_optional_params(
    ...
    common_auth_dict = love_engine.common_cloud_provider_auth_params
    if custom_llm_provider in common_auth_dict["providers"]:
        ...
        if custom_llm_provider == "azure":
            optional_params = love_engine.AzureOpenAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "bedrock":
            optional_params = (
                love_engine.AmazonBedrockGlobalConfig().map_special_auth_params(
                    non_default_params=passed_params, optional_params=optional_params
                )
            )
        elif (
            ...
            optional_params = love_engine.VertexAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "watsonx":
            optional_params = love_engine.IBMWatsonXAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
    ...
    if (
        ...
        if (
            ...
            if custom_llm_provider == "ollama":
                ...
                love_engine.add_function_to_prompt = (
...
def get_optional_params(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: List[str]):
        ...
        Args:
            supported_params: List[str] - supported params from the love_engine config
        """
        verbose_logger.info(
            f"\nlove_engine completion() model= {model}; provider = {custom_llm_provider}"
        )
        verbose_logger.debug(
            f"\nlove_engine: Params passed to completion() {passed_params}"
        )
        verbose_logger.debug(
            f"\nlove_engine: Non-Default params passed to completion() {non_default_params}"
        )
        unsupported_params = {}
        for k in non_default_params.keys():
            if k not in supported_params:
        ...
        if unsupported_params:
            if love_engine.drop_params is True or (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {list(unsupported_params.keys())}, for model={model}. To drop these, set `love_engine.drop_params=True` or for proxy:\n\n`love_engine_settings:\n drop_params: true`\n. \n If you want to use these params dynamically send allowed_openai_params={list(unsupported_params.keys())} in your request.",
                )
    ...
    if custom_llm_provider == "anthropic":
        ...
        optional_params = love_engine.AnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = love_engine.AnthropicTextConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = love_engine.CohereChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.TritonConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        optional_params = love_engine.MaritalkConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.ReplicateConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.PredibaseConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.HuggingFaceChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.TogetherAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        model in love_engine.vertex_chat_models
        or model in love_engine.vertex_code_chat_models
        or model in love_engine.vertex_text_models
        or model in love_engine.vertex_code_text_models
        or model in love_engine.vertex_language_models
        or model in love_engine.vertex_vision_models
    ):
        optional_params = love_engine.VertexGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.GoogleAIStudioGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
    ...
    elif love_engine.VertexAIAnthropicConfig.is_supported_model(
        model=model, custom_llm_provider=custom_llm_provider
    ):
        optional_params = love_engine.VertexAIAnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        if model in love_engine.vertex_mistral_models:
            if "codestral" in model:
                optional_params = (
                    love_engine.CodestralTextCompletionConfig().map_openai_params(
                        model=model,
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        drop_params=(
                ...
                optional_params = love_engine.MistralConfig().map_openai_params(
                    model=model,
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    drop_params=(
        ...
        elif model in love_engine.vertex_ai_ai21_models:
            optional_params = love_engine.VertexAIAi21Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
            ...
            optional_params = love_engine.VertexAILlama3Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
        ...
        optional_params = love_engine.SagemakerConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if bedrock_route == "converse" or bedrock_route == "converse_like":
            optional_params = love_engine.AmazonConverseConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=(
            ...
            optional_params = love_engine.AmazonBedrockOpenAIConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=(
            ...
            if (
                bedrock_base_model
                in love_engine.AmazonAnthropicConfig.get_legacy_anthropic_model_names()
            ):
                optional_params = love_engine.AmazonAnthropicConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    model=model,
                    drop_params=(
                ...
                optional_params = (
                    love_engine.AmazonAnthropicClaudeConfig().map_openai_params(
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        model=model,
                        drop_params=(
        ...
        optional_params = love_engine.CloudflareChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = love_engine.OllamaConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.OllamaChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = love_engine.NLPCloudConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.PetalsConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.DeepInfraConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.CodestralTextCompletionConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.InceptionTextCompletionConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.DatabricksConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.NvidiaNimConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = love_engine.CerebrasConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.XAIChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
        )
    elif custom_llm_provider == "ai21_chat" or custom_llm_provider == "ai21":
        optional_params = love_engine.AI21ChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.FireworksAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.VolcEngineConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.HostedVLLMChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.VLLMConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.GroqChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.BedrockMantleChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.DeepSeekChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.OpenrouterConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.IBMWatsonXChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        for param in passed_params.keys():
            if love_engine.IBMWatsonXAIConfig().is_watsonx_text_param(param):
                raise ValueError(
                    f"love_engine now defaults to Watsonx's `/text/chat` endpoint. Please use the `watsonx_text` provider instead, to call the `/text/generation` endpoint. Param: {param}"
                )
    elif custom_llm_provider == "watsonx_text":
        optional_params = love_engine.IBMWatsonXAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.OpenAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = love_engine.NebiusConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if love_engine.AzureOpenAIO1Config().is_o_series_model(
            ...
            optional_params = love_engine.AzureOpenAIO1Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=_azure_detection_model,
                drop_params=(
            ...
            optional_params = love_engine.AzureOpenAIGPT5Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=_azure_detection_model,
                drop_params=(
            ...
            verbose_logger.debug(
                "Azure optional params - api_version: api_version={}, love_engine.api_version={}, os.environ['AZURE_API_VERSION']={}".format(
                    api_version, love_engine.api_version, get_secret("AZURE_API_VERSION")
                )
            )
            api_version = (
                api_version
                or love_engine.api_version
                or get_secret("AZURE_API_VERSION")
                or love_engine.AZURE_DEFAULT_API_VERSION
            )
            optional_params = love_engine.AzureOpenAIConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=_azure_detection_model,
        ...
        optional_params = love_engine.OpenAILikeChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
...
def add_provider_specific_params_to_optional_params(
    ...
    if (
        ...
        + love_engine.openai_compatible_providers
    ):
...
def _apply_openai_param_overrides(
    ...
    See https://github.com/BerriAI/love_engine/issues/25697
    """
...
def _get_deployment_order(deployment: Union[Dict, Any]) -> Optional[int]:
    ...
    Checks love_engine_params first (static config), then model_info (dynamic/team
    models added via API where order lives in model_info, not love_engine_params).
    """
    order = deployment.get("love_engine_params", {}).get("order")
    if order is None:
...
def _get_model_region(
    custom_llm_provider: str, love_engine_params: LOVE_ENGINE_Params
) -> Optional[str]:
    """
    ...
    if custom_llm_provider == "vertex_ai":
        ...
        vertex_ai_location = (
            love_engine_params.vertex_location
        ...
        aws_region_name = love_engine_params.aws_region_name
        if aws_region_name is not None:
            return aws_region_name
    elif custom_llm_provider == "watsonx":
        watsonx_region_name = love_engine_params.watsonx_region_name
        if watsonx_region_name is not None:
            return watsonx_region_name
    return love_engine_params.region_name


def _infer_model_region(love_engine_params: LOVE_ENGINE_Params) -> Optional[AllowedModelRegion]:
    """
    ...
    """
    model, custom_llm_provider, _, _ = love_engine.get_llm_provider(
        model=love_engine_params.model, love_engine_params=love_engine_params
    )

    model_region = _get_model_region(
        custom_llm_provider=custom_llm_provider, love_engine_params=love_engine_params
    )

    if model_region is None:
        verbose_logger.debug(
            "Cannot infer model region for model: {}".format(love_engine_params.model)
        )
        return None

    if custom_llm_provider == "azure":
        eu_regions = love_engine.AzureOpenAIConfig().get_eu_regions()
        us_regions = love_engine.AzureOpenAIConfig().get_us_regions()
    elif custom_llm_provider == "vertex_ai":
        eu_regions = love_engine.VertexAIConfig().get_eu_regions()
        us_regions = love_engine.VertexAIConfig().get_us_regions()
    elif custom_llm_provider == "bedrock":
        eu_regions = love_engine.AmazonBedrockGlobalConfig().get_eu_regions()
        us_regions = love_engine.AmazonBedrockGlobalConfig().get_us_regions()
    elif custom_llm_provider == "watsonx":
        eu_regions = love_engine.IBMWatsonXAIConfig().get_eu_regions()
        us_regions = love_engine.IBMWatsonXAIConfig().get_us_regions()
    else:
        eu_regions = []
        us_regions = []
...
def _is_region_eu(love_engine_params: LOVE_ENGINE_Params) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if love_engine_params.region_name == "eu":
        return True

    ## Else - try and infer from model region
    model_region = _infer_model_region(love_engine_params=love_engine_params)
    if model_region is not None and model_region == "eu":
        return True
    return False


def _is_region_us(love_engine_params: LOVE_ENGINE_Params) -> bool:
    """
    Return true/false if a deployment is in the US
    """
    if love_engine_params.region_name == "us":
        return True
...
def is_region_allowed(
    love_engine_params: LOVE_ENGINE_Params, allowed_model_region: str
) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if love_engine_params.region_name == allowed_model_region:
        return True
    return False


def get_model_region(
    love_engine_params: LOVE_ENGINE_Params, mode: Optional[str]
) -> Optional[str]:
    """
    Pass the love_engine params for an azure model, and get back the region
    """
    if (
        "azure" in love_engine_params.model
        and isinstance(love_engine_params.api_key, str)
        and isinstance(love_engine_params.api_base, str)
    ):
        _model = love_engine_params.model.replace("azure/", "")
        response: dict = love_engine.AzureChatCompletion().get_headers(
            model=_model,
            api_key=love_engine_params.api_key,
            api_base=love_engine_params.api_base,
            api_version=love_engine_params.api_version or love_engine.AZURE_DEFAULT_API_VERSION,
            timeout=10,
            mode=mode or "chat",
        )
...
def get_api_key(llm_provider: str, dynamic_api_key: Optional[str]):
    api_key = dynamic_api_key or love_engine.api_key
    # openai
    if llm_provider == "openai" or llm_provider == "text-completion-openai":
        api_key = api_key or love_engine.openai_key or get_secret("OPENAI_API_KEY")
    # anthropic
    elif llm_provider == "anthropic" or llm_provider == "anthropic_text":
        api_key = api_key or love_engine.anthropic_key or get_secret("ANTHROPIC_API_KEY")
    # ai21
    elif llm_provider == "ai21":
        api_key = api_key or love_engine.ai21_key or get_secret("AI211_API_KEY")
    # aleph_alpha
    elif llm_provider == "aleph_alpha":
        api_key = (
            api_key or love_engine.aleph_alpha_key or get_secret("ALEPH_ALPHA_API_KEY")
        )
    # baseten
    elif llm_provider == "baseten":
        api_key = api_key or love_engine.baseten_key or get_secret("BASETEN_API_KEY")
    # cohere
    elif llm_provider == "cohere" or llm_provider == "cohere_chat":
        api_key = api_key or love_engine.cohere_key or get_secret("COHERE_API_KEY")
    # huggingface
    elif llm_provider == "huggingface":
        api_key = (
            api_key or love_engine.huggingface_key or get_secret("HUGGINGFACE_API_KEY")
        )
    # nlp_cloud
    elif llm_provider == "nlp_cloud":
        api_key = api_key or love_engine.nlp_cloud_key or get_secret("NLP_CLOUD_API_KEY")
    # replicate
    elif llm_provider == "replicate":
        api_key = api_key or love_engine.replicate_key or get_secret("REPLICATE_API_KEY")
    # together_ai
    elif llm_provider == "together_ai":
        api_key = (
            api_key
            or love_engine.togetherai_api_key
            or get_secret("TOGETHERAI_API_KEY")
            or get_secret("TOGETHER_AI_TOKEN")
        )
    # nebius
    elif llm_provider == "nebius":
        api_key = api_key or love_engine.nebius_key or get_secret("NEBIUS_API_KEY")
    # wandb
    elif llm_provider == "wandb":
        api_key = api_key or love_engine.wandb_key or get_secret("WANDB_API_KEY")
    return api_key
...
def get_max_tokens(model: str) -> Optional[int]:
    ...
    def _get_max_position_embeddings(model_name):
        ...
        try:
            ...
            response = love_engine.module_level_client.get(config_url)
    ...
    try:
        if model in love_engine.model_cost:
            if "max_output_tokens" in love_engine.model_cost[model]:
                return love_engine.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in love_engine.model_cost[model]:
                return love_engine.model_cost[model]["max_tokens"]
        get_llm_provider = getattr(sys.modules[__name__], "get_llm_provider")
        model, custom_llm_provider, _, _ = get_llm_provider(model=model)
        if custom_llm_provider == "huggingface":
            max_tokens = _get_max_position_embeddings(model_name=model)
            return max_tokens
        if model in love_engine.model_cost:  # check if extracted model is in model_list
            if "max_output_tokens" in love_engine.model_cost[model]:
                return love_engine.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in love_engine.model_cost[model]:
                return love_engine.model_cost[model]["max_tokens"]
        ...
        raise Exception(
            f"Model {model} isn't mapped yet. Add it here - https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json"
        )
...
def _invalidate_model_cost_lowercase_map() -> None:
    ...
    Call this whenever love_engine.model_cost is modified to ensure the map is rebuilt.
...
def _rebuild_model_cost_lowercase_map() -> Dict[str, str]:
    ...
    _model_cost_lowercase_map = {k.lower(): k for k in love_engine.model_cost}
...
def _handle_stale_map_entry_rebuild(
    ...
    if matched_key is not None and matched_key in love_engine.model_cost:
        return matched_key
    return None
...
def _handle_new_key_with_scan(
    ...
    """
    global _model_cost_lowercase_map
    for key in love_engine.model_cost:
...
def _get_model_cost_key(potential_key: str) -> Optional[str]:
    ...
    # Exact match (O(1))
    if potential_key in love_engine.model_cost:
...
def _get_model_info_from_model_cost(key: str) -> dict:
    return love_engine.model_cost[key]
...
def _check_provider_match(model_info: dict, custom_llm_provider: Optional[str]) -> bool:
    ...
    A missing ``love_engine_provider`` key and a ``love_engine_provider`` set to
    ...
    if custom_llm_provider and (
        model_info.get("love_engine_provider") is not None
        and model_info["love_engine_provider"] != custom_llm_provider
    ):
        if custom_llm_provider == "vertex_ai" and model_info[
            "love_engine_provider"
            ...
            custom_llm_provider == "love_engine_proxy"
        ):  # love_engine_proxy is a special case, it's not a provider, it's a proxy for the provider
            return True
        elif custom_llm_provider == "azure_ai" and model_info["love_engine_provider"] in (
            "azure",
            "openai",
        ):
...
def _get_model_info_helper(
    ...
    """
    try:
        azure_llms = {**love_engine.azure_llms, **love_engine.azure_embedding_models}
        if model in azure_llms:
            model = azure_llms[model]
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai_beta":
            custom_llm_provider = "vertex_ai"
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
            if "meta/" + model in love_engine.vertex_llama3_models:
                model = "meta/" + model
            elif model + "@latest" in love_engine.vertex_mistral_models:
                model = model + "@latest"
            elif model + "@latest" in love_engine.vertex_ai_ai21_models:
                model = model + "@latest"
        ##########################
        ...
        verbose_logger.debug(
            f"checking potential_model_names in love_engine.model_cost: {potential_model_names}"
        )
        ...
        if custom_llm_provider == "huggingface":
            ...
            return ModelInfoBase(
                ...
                love_engine_provider="huggingface",
                mode="chat",
            ...
            1. 'custom_llm_provider/model' in love_engine.model_cost. Checks "groq/llama3-8b-8192" if model="llama3-8b-8192" and custom_llm_provider="groq"
            2. 'model' in love_engine.model_cost. Checks "gemini-1.5-pro-002" in  love_engine.model_cost if model="gemini-1.5-pro-002" and custom_llm_provider=None
            3. 'combined_stripped_model_name' in love_engine.model_cost. Checks if 'gemini/gemini-1.5-flash' in model map, if 'gemini/gemini-1.5-flash-001' given.
            4. 'stripped_model_name' in love_engine.model_cost. Checks if 'ft:gpt-3.5-turbo' in model map, if 'ft:gpt-3.5-turbo:my-org:custom_suffix:id' given.
            5. 'split_model' in love_engine.model_cost. Checks "llama3-8b-8192" in love_engine.model_cost if model="groq/llama3-8b-8192"
            """
            ...
            if _model_info is None or key is None:
                raise ValueError(
                    "This model isn't mapped yet. Add it here - https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json"
                )
            ...
            return ModelInfoBase(
                ...
                love_engine_provider=_model_info.get(
                    "love_engine_provider", custom_llm_provider
                ),
        ...
        raise Exception(
            "This model isn't mapped yet. model={}, custom_llm_provider={}. Add it here - https://github.com/BerriAI/love_engine/blob/main/model_prices_and_context_window.json.".format(
                model, custom_llm_provider
            )
        )
...
def _build_model_info(
    ...
    supported_openai_params = love_engine.get_supported_openai_params(
        model=model, custom_llm_provider=custom_llm_provider
    )
...
def get_model_info(
    ...
    Parameters:
    - model (str): The name of the model.
    - custom_llm_provider (str | null): the provider used for the model. If provided, used to check if the love_engine model info is for that provider.

    Returns:
        dict: A dictionary containing the following information:
            key: Required[str] # the key in love_engine.model_cost which is returned
            ...
            love_engine_provider: Required[str]
            mode: Required[
                Literal[
...
def function_to_dict(input_function) -> dict:  # noqa: C901
    ...
    -------
    dictionnary
        A dictionnary to add to the list passed to `functions` parameter of `love_engine.completion`
    """
...
def get_provider_fields(custom_llm_provider: str) -> List[ProviderField]:
    ...
    if custom_llm_provider == "databricks":
        return love_engine.DatabricksConfig().get_required_params()

    elif custom_llm_provider == "ollama":
        return love_engine.OllamaConfig().get_required_params()

    elif custom_llm_provider == "azure_ai":
        return love_engine.AzureAIStudioConfig().get_required_params()
...
def validate_environment(
    ...
    if custom_llm_provider:
        ...
        if (
            model in love_engine.open_ai_chat_completion_models
            or model in love_engine.open_ai_text_completion_models
            or model in love_engine.open_ai_embedding_models
            or model in love_engine.openai_image_generation_models
            ...
            if "OPENAI_API_KEY" in os.environ:
                ...
                missing_keys.append("OPENAI_API_KEY")
        ## anthropic
        elif model in love_engine.anthropic_models:
            if (
                "ANTHROPIC_API_KEY" in os.environ
                or "ANTHROPIC_AUTH_TOKEN" in os.environ
            ):
                ...
                missing_keys.append("ANTHROPIC_API_KEY")
        ## cohere
        elif model in love_engine.cohere_models:
            if "COHERE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("COHERE_API_KEY")
        ## replicate
        elif model in love_engine.replicate_models:
            if "REPLICATE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("REPLICATE_API_KEY")
        ## openrouter
        elif model in love_engine.openrouter_models:
            if "OPENROUTER_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("OPENROUTER_API_KEY")
        ## vercel_ai_gateway
        elif model in love_engine.vercel_ai_gateway_models:
            if "VERCEL_AI_GATEWAY_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("VERCEL_AI_GATEWAY_API_KEY")
        ## datarobot
        elif model in love_engine.datarobot_models:
            if "DATAROBOT_API_TOKEN" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("DATAROBOT_API_TOKEN")
        ## vertex - text + chat models
            ...
            model in love_engine.vertex_chat_models
            or model in love_engine.vertex_text_models
            or model in love_engine.models_by_provider["vertex_ai"]
        ):
            if "VERTEXAI_PROJECT" in os.environ and "VERTEXAI_LOCATION" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.extend(["VERTEXAI_PROJECT", "VERTEXAI_LOCATION"])
        ## huggingface
        elif model in love_engine.huggingface_models:
            if "HUGGINGFACE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("HUGGINGFACE_API_KEY")
        ## ai21
        elif model in love_engine.ai21_models:
            if "AI21_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("AI21_API_KEY")
        ## together_ai
        elif model in love_engine.together_ai_models:
            if "TOGETHERAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("TOGETHERAI_API_KEY")
        ## aleph_alpha
        elif model in love_engine.aleph_alpha_models:
            if "ALEPH_ALPHA_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("ALEPH_ALPHA_API_KEY")
        ## baseten
        elif model in love_engine.baseten_models:
            if "BASETEN_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("BASETEN_API_KEY")
        ## nlp_cloud
        elif model in love_engine.nlp_cloud_models:
            if "NLP_CLOUD_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NLP_CLOUD_API_KEY")
        elif model in love_engine.novita_models:
            if "NOVITA_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NOVITA_API_KEY")
        elif model in love_engine.nebius_models:
            if "NEBIUS_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NEBIUS_API_KEY")
        elif model in love_engine.wandb_models:
            if "WANDB_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("WANDB_API_KEY")
...
def acreate(*args, **kwargs):  ## Thin client to handle the acreate langchain call
    return love_engine.acompletion(*args, **kwargs)
...
def valid_model(model):
    try:
        ...
        if (
            ...
            love_engine.completion(model=model, messages=messages)
    except Exception:
...
def check_valid_key(model: str, api_key: str):
    """
    Checks if a given API key is valid for a specific model by making a love_engine.completion call with max_tokens=10
...
def register_prompt_template(
    ...
    ```
    import love_engine
    love_engine.register_prompt_template(
            model="llama-2",
    ...
    if tokenizer_config:
        for m in potential_models:
            love_engine.known_tokenizer_config[m] = {
                "tokenizer": tokenizer_config,
                "status": "success",
            }
    else:
        for m in potential_models:
            love_engine.custom_prompt_dict[m] = {
                "roles": roles,
                "initial_prompt_value": initial_prompt_value,
                "final_prompt_value": final_prompt_value,
            }

    return love_engine.custom_prompt_dict
...
def mock_completion_streaming_obj(
    ...
    if isinstance(mock_response, love_engine.MockException):
        raise mock_response
    ...
    for i in range(0, len(mock_response), 3):
        ...
        if n is None:
            ...
            _all_choices = []
            for j in range(n):
                _streaming_choice = love_engine.utils.StreamingChoices(
                    index=j,
                    delta=love_engine.utils.Delta(
...
def trim_messages(
    ...
    Args:
        ...
        model: The love_engine model being used (determines the token limit).
    ...
    try:
        if max_tokens is None:
            ...
            if model in love_engine.model_cost:
                max_tokens_for_model = love_engine.model_cost[model].get(
                    "max_input_tokens", love_engine.model_cost[model]["max_tokens"]
                )
                max_tokens = int(max_tokens_for_model * trim_ratio)
            else:
                # if user did not specify max (input) tokens
                # or passed an llm love_engine does not know
        ...
        ## Handle Tool Call ## - check if last message is a tool response, return as is - https://github.com/BerriAI/love_engine/issues/4931
        tool_messages = []
...
from love_engine.caching.in_memory_cache import InMemoryCache
...
class AvailableModelsCache(InMemoryCache):
    ...
    def _get_cache_key(
        self,
        custom_llm_provider: Optional[str],
        love_engine_params: Optional[LOVE_ENGINE_Params],
    ) -> str:
        valid_str = ""

        if love_engine_params is not None:
            valid_str = love_engine_params.model_dump_json()
        if custom_llm_provider is not None:
    ...
    def get_cached_model_info(
        self,
        custom_llm_provider: Optional[str] = None,
        love_engine_params: Optional[LOVE_ENGINE_Params] = None,
    ) -> Optional[List[str]]:
        """Get cached model info"""
        # Check if environment has changed
        if love_engine_params is None and self._check_env_changed():
            self.cache_dict.clear()
            return None

        cache_key = self._get_cache_key(custom_llm_provider, love_engine_params)
...
def _infer_valid_provider_from_env_vars(
    ...
    for provider in love_engine.provider_list:
        if custom_llm_provider and provider != custom_llm_provider:
            continue

        # edge case love_engine has together_ai as a provider, it should be togetherai
        env_provider_1 = provider.replace("_", "")
        env_provider_2 = provider

        # love_engine standardizes expected provider keys to
...
def _get_valid_models_from_provider_api(
    ...
    try:
        cached_result = _model_cache.get_cached_model_info(
            custom_llm_provider, love_engine_params
        )

        if cached_result is not None:
            return cached_result
        models = provider_config.get_models(
            api_key=love_engine_params.api_key if love_engine_params is not None else None,
            api_base=love_engine_params.api_base if love_engine_params is not None else None,
        )

        _model_cache.set_cached_model_info(custom_llm_provider, love_engine_params, models)
        return models
    except Exception as e:
...
def get_valid_models(
    ...
    try:
        ################################
        # init love_engine_params
        #################################
        from love_engine.types.router import LOVE_ENGINE_Params

        if love_engine_params is None:
            love_engine_params = LOVE_ENGINE_Params(model="")
        if api_key is not None:
            love_engine_params.api_key = api_key
        if api_base is not None:
            love_engine_params.api_base = api_base
        #################################

        check_provider_endpoint = (
            check_provider_endpoint or love_engine.check_provider_endpoint
        )
        # get keys set in .env
        ...
        for provider in valid_providers:
            ...
            if provider == "azure":
                ...
                models_for_provider = copy.deepcopy(
                    love_engine.models_by_provider.get(provider, [])
                )
...
def print_args_passed_to_love_engine(original_function, args, kwargs):
    ...
    try:
        ...
        print_verbose(
            "\033[92mRequest to love_engine:\033[0m",
        )
        if args and kwargs:
            print_verbose(
                f"\033[92mlove_engine.{original_function.__name__}({args_str}, {kwargs_str})\033[0m"
            )
        elif args:
            print_verbose(
                f"\033[92mlove_engine.{original_function.__name__}({args_str})\033[0m"
            )
        elif kwargs:
            print_verbose(
                f"\033[92mlove_engine.{original_function.__name__}({kwargs_str})\033[0m"
            )
        else:
            print_verbose(f"\033[92mlove_engine.{original_function.__name__}()\033[0m")
        print_verbose("\n")  # new line after
    except Exception:
...
def _get_base_model_from_metadata(model_call_details=None):
    if model_call_details is None:
        return None
    love_engine_params = model_call_details.get("love_engine_params", {})
    if love_engine_params is not None:
        _base_model = love_engine_params.get("base_model", None)
        if _base_model is not None:
            return _base_model
        metadata = love_engine_params.get("metadata") or {}

        _get_base_model_from_love_engine_call_metadata = getattr(
            sys.modules[__name__], "_get_base_model_from_love_engine_call_metadata"
        )
        base_model_from_metadata = _get_base_model_from_love_engine_call_metadata(
            metadata=metadata
        )
        if base_model_from_metadata is not None:
            return base_model_from_metadata

        # Also check love_engine_metadata (used by Responses API and other generic API calls)
        love_engine_metadata = love_engine_params.get("love_engine_metadata", {})
        _get_base_model_from_love_engine_call_metadata = getattr(
            sys.modules[__name__], "_get_base_model_from_love_engine_call_metadata"
        )
        return _get_base_model_from_love_engine_call_metadata(metadata=love_engine_metadata)
    return None
...
def is_cached_message(message: AllMessageValues) -> bool:
    ...
    Can be disabled globally by setting love_engine.disable_anthropic_gemini_context_caching_transform = True
    """
    # Check if context caching is disabled globally
    if love_engine.disable_anthropic_gemini_context_caching_transform is True:
        return False
...
def any_assistant_message_has_thinking_blocks(
    ...
    Related issue: https://github.com/BerriAI/love_engine/issues/18926
    """
    for message in messages:
...
def last_assistant_with_tool_calls_has_no_thinking_blocks(
    ...
    Related issues: https://github.com/BerriAI/love_engine/issues/14194, https://github.com/BerriAI/love_engine/issues/9020
    """
...
def add_dummy_tool(custom_llm_provider: str) -> List[ChatCompletionToolParam]:
    ...
    Relevent Issues: https://github.com/BerriAI/love_engine/issues/5388, https://github.com/BerriAI/love_engine/issues/5747
    """
    return [
        ChatCompletionToolParam(
            type="function",
...
def validate_chat_completion_tool_choice(
    ...
    Prevents user errors like: https://github.com/BerriAI/love_engine/issues/7483
    """
    from love_engine.types.llms.openai import (
...
def validate_openai_optional_params(
    ...
    if (
        ...
        and not love_engine.disable_stop_sequence_limit
    ):
...
def _get_bundled_model_cost_map() -> Dict[str, Any]:
    try:
        model_cost_path = resources.files("love_engine").joinpath(
...
def _get_model_cost_entry_for_provider_config(
    ...
    for model_key in candidate_keys:
        model_info = love_engine.model_cost.get(model_key)
...
class ProviderConfigManager:
    ...
    def _build_provider_config_map() -> dict[LlmProviders, tuple[Callable, bool]]:
        ...
        return {
            ...
            LlmProviders.OPENAI: (lambda: love_engine.OpenAIGPTConfig(), False),
            LlmProviders.ANTHROPIC: (lambda: love_engine.AnthropicConfig(), False),
            ...
            LlmProviders.DEEPSEEK: (lambda: love_engine.DeepSeekChatConfig(), False),
            LlmProviders.GROQ: (lambda: love_engine.GroqChatConfig(), False),
            LlmProviders.BEDROCK_MANTLE: (
                lambda: love_engine.BedrockMantleChatConfig(),
                False,
            ),
            LlmProviders.A2A: (lambda: love_engine.A2AConfig(), False),
            LlmProviders.BYTEZ: (lambda: love_engine.BytezChatConfig(), False),
            LlmProviders.DATABRICKS: (lambda: love_engine.DatabricksConfig(), False),
            LlmProviders.XAI: (lambda: love_engine.XAIChatConfig(), False),
            LlmProviders.ZAI: (lambda: love_engine.ZAIChatConfig(), False),
            LlmProviders.LAMBDA_AI: (lambda: love_engine.LambdaAIChatConfig(), False),
            LlmProviders.INCEPTION: (lambda: love_engine.InceptionChatConfig(), False),
            LlmProviders.LLAMA: (lambda: love_engine.LlamaAPIConfig(), False),
            LlmProviders.TEXT_COMPLETION_OPENAI: (
                lambda: love_engine.OpenAITextCompletionConfig(),
                False,
            ),
            LlmProviders.SNOWFLAKE: (lambda: love_engine.SnowflakeConfig(), False),
            LlmProviders.CLARIFAI: (lambda: love_engine.ClarifaiConfig(), False),
            LlmProviders.ANTHROPIC_TEXT: (lambda: love_engine.AnthropicTextConfig(), False),
            LlmProviders.VERTEX_AI_BETA: (lambda: love_engine.VertexGeminiConfig(), False),
            LlmProviders.CLOUDFLARE: (lambda: love_engine.CloudflareChatConfig(), False),
            LlmProviders.SAGEMAKER_CHAT: (lambda: love_engine.SagemakerChatConfig(), False),
            LlmProviders.SAGEMAKER_NOVA: (lambda: love_engine.SagemakerNovaConfig(), False),
            LlmProviders.SAGEMAKER: (lambda: love_engine.SagemakerConfig(), False),
            LlmProviders.FIREWORKS_AI: (lambda: love_engine.FireworksAIConfig(), False),
            LlmProviders.FRIENDLIAI: (lambda: love_engine.FriendliaiChatConfig(), False),
            LlmProviders.WATSONX: (lambda: love_engine.IBMWatsonXChatConfig(), False),
            LlmProviders.WATSONX_TEXT: (lambda: love_engine.IBMWatsonXAIConfig(), False),
            LlmProviders.EMPOWER: (lambda: love_engine.EmpowerChatConfig(), False),
            LlmProviders.MINIMAX: (lambda: love_engine.MinimaxChatConfig(), False),
            LlmProviders.GITHUB: (lambda: love_engine.GithubChatConfig(), False),
            LlmProviders.COMPACTIFAI: (lambda: love_engine.CompactifAIChatConfig(), False),
            LlmProviders.GITHUB_COPILOT: (lambda: love_engine.GithubCopilotConfig(), False),
            LlmProviders.CHATGPT: (lambda: love_engine.ChatGPTConfig(), False),
            LlmProviders.GIGACHAT: (lambda: love_engine.GigaChatConfig(), False),
            LlmProviders.RAGFLOW: (lambda: love_engine.RAGFlowConfig(), False),
            LlmProviders.CUSTOM: (lambda: love_engine.OpenAILikeChatConfig(), False),
            LlmProviders.CUSTOM_OPENAI: (lambda: love_engine.OpenAILikeChatConfig(), False),
            LlmProviders.OPENAI_LIKE: (lambda: love_engine.OpenAILikeChatConfig(), False),
            LlmProviders.AIOHTTP_OPENAI: (
                lambda: love_engine.AiohttpOpenAIChatConfig(),
                False,
            ),
            LlmProviders.HOSTED_VLLM: (lambda: love_engine.HostedVLLMChatConfig(), False),
            LlmProviders.LLAMAFILE: (lambda: love_engine.LlamafileChatConfig(), False),
            LlmProviders.LM_STUDIO: (lambda: love_engine.LMStudioChatConfig(), False),
            LlmProviders.GALADRIEL: (lambda: love_engine.GaladrielChatConfig(), False),
            LlmProviders.REPLICATE: (lambda: love_engine.ReplicateConfig(), False),
            LlmProviders.HUGGINGFACE: (lambda: love_engine.HuggingFaceChatConfig(), False),
            LlmProviders.TOGETHER_AI: (lambda: love_engine.TogetherAIConfig(), False),
            LlmProviders.OPENROUTER: (lambda: love_engine.OpenrouterConfig(), False),
            LlmProviders.VERCEL_AI_GATEWAY: (
                lambda: love_engine.VercelAIGatewayConfig(),
                False,
            ),
            LlmProviders.COMETAPI: (lambda: love_engine.CometAPIConfig(), False),
            LlmProviders.DATAROBOT: (lambda: love_engine.DataRobotConfig(), False),
            LlmProviders.GEMINI: (lambda: love_engine.GoogleAIStudioGeminiConfig(), False),
            LlmProviders.AI21: (lambda: love_engine.AI21ChatConfig(), False),
            LlmProviders.AI21_CHAT: (lambda: love_engine.AI21ChatConfig(), False),
            LlmProviders.AZURE_TEXT: (lambda: love_engine.AzureOpenAITextConfig(), False),
            LlmProviders.NLP_CLOUD: (lambda: love_engine.NLPCloudConfig(), False),
            LlmProviders.OOBABOOGA: (lambda: love_engine.OobaboogaConfig(), False),
            LlmProviders.OLLAMA_CHAT: (lambda: love_engine.OllamaChatConfig(), False),
            LlmProviders.DEEPINFRA: (lambda: love_engine.DeepInfraConfig(), False),
            LlmProviders.PERPLEXITY: (lambda: love_engine.PerplexityChatConfig(), False),
            LlmProviders.MISTRAL: (lambda: love_engine.MistralConfig(), False),
            LlmProviders.CODESTRAL: (lambda: love_engine.MistralConfig(), False),
            LlmProviders.NVIDIA_NIM: (lambda: love_engine.NvidiaNimConfig(), False),
            LlmProviders.CEREBRAS: (lambda: love_engine.CerebrasConfig(), False),
            LlmProviders.BASETEN: (lambda: love_engine.BasetenConfig(), False),
            LlmProviders.VOLCENGINE: (lambda: love_engine.VolcEngineConfig(), False),
            LlmProviders.TEXT_COMPLETION_CODESTRAL: (
                lambda: love_engine.CodestralTextCompletionConfig(),
                False,
            ),
            LlmProviders.TEXT_COMPLETION_INCEPTION: (
                lambda: love_engine.InceptionTextCompletionConfig(),
                False,
            ),
            LlmProviders.SAMBANOVA: (lambda: love_engine.SambanovaConfig(), False),
            LlmProviders.MARITALK: (lambda: love_engine.MaritalkConfig(), False),
            LlmProviders.VLLM: (lambda: love_engine.VLLMConfig(), False),
            LlmProviders.OLLAMA: (lambda: love_engine.OllamaConfig(), False),
            LlmProviders.PREDIBASE: (lambda: love_engine.PredibaseConfig(), False),
            LlmProviders.TRITON: (lambda: love_engine.TritonConfig(), False),
            LlmProviders.PETALS: (lambda: love_engine.PetalsConfig(), False),
            LlmProviders.SAP_GENERATIVE_AI_HUB: (
                lambda: love_engine.GenAIHubOrchestrationConfig(),
                False,
            ),
            LlmProviders.FEATHERLESS_AI: (lambda: love_engine.FeatherlessAIConfig(), False),
            LlmProviders.NOVITA: (lambda: love_engine.NovitaConfig(), False),
            LlmProviders.NEBIUS: (lambda: love_engine.NebiusConfig(), False),
            LlmProviders.WANDB: (lambda: love_engine.WandbConfig(), False),
            LlmProviders.DASHSCOPE: (lambda: love_engine.DashScopeChatConfig(), False),
            LlmProviders.MODELSCOPE: (lambda: love_engine.ModelScopeChatConfig(), False),
            LlmProviders.MOONSHOT: (lambda: love_engine.MoonshotChatConfig(), False),
            LlmProviders.DOCKER_MODEL_RUNNER: (
                lambda: love_engine.DockerModelRunnerChatConfig(),
                False,
            ),
            LlmProviders.V0: (lambda: love_engine.V0ChatConfig(), False),
            LlmProviders.MORPH: (lambda: love_engine.MorphChatConfig(), False),
            LlmProviders.LOVE_ENGINE_PROXY: (
                lambda: love_engine.LoveEngineProxyChatConfig(),
                False,
            ),
            LlmProviders.GRADIENT_AI: (lambda: love_engine.GradientAIConfig(), False),
            LlmProviders.NSCALE: (lambda: love_engine.NscaleConfig(), False),
            LlmProviders.HEROKU: (lambda: love_engine.HerokuChatConfig(), False),
            LlmProviders.OCI: (lambda: love_engine.OCIChatConfig(), False),
            LlmProviders.HYPERBOLIC: (lambda: love_engine.HyperbolicChatConfig(), False),
            LlmProviders.OVHCLOUD: (lambda: love_engine.OVHCloudChatConfig(), False),
            LlmProviders.AMAZON_NOVA: (lambda: love_engine.AmazonNovaChatConfig(), False),
    ...
    def _get_azure_config(model: str, base_model: Optional[str] = None) -> BaseConfig:
        ...
        if love_engine.AzureOpenAIO1Config().is_o_series_model(model=detection_model):
            return love_engine.AzureOpenAIO1Config()
        if love_engine.AzureOpenAIGPT5Config.is_model_gpt_5_model(model=detection_model):
            return love_engine.AzureOpenAIGPT5Config()
        return love_engine.AzureOpenAIConfig()
    ...
    def _get_vertex_ai_config(model: str) -> BaseConfig:
        ...
        if "gemini" in model:
            return love_engine.VertexGeminiConfig()
        elif "claude" in model:
            return love_engine.VertexAIAnthropicConfig()
        elif "gpt-oss" in model:
            from love_engine.llms.vertex_ai.vertex_ai_partner_models.gpt_oss.transformation import (
                VertexAIGPTOSSTransformation,
            )

            return VertexAIGPTOSSTransformation()
        elif model in love_engine.vertex_mistral_models:
            if "codestral" in model:
                return love_engine.CodestralTextCompletionConfig()
            return love_engine.MistralConfig()
        elif model in love_engine.vertex_ai_ai21_models:
            return love_engine.VertexAIAi21Config()
        else:
            return love_engine.VertexAILlama3Config()

    @staticmethod
    def _get_bedrock_config(model: str) -> BaseConfig:
        """Get Bedrock config based on model."""
        from love_engine.llms.bedrock.common_utils import get_bedrock_chat_config
    ...
    def _get_cohere_config(model: str) -> BaseConfig:
        ...
        if route == "v2":
            return love_engine.CohereV2ChatConfig()
        return love_engine.CohereChatConfig()
    ...
    def _get_langflow_config() -> BaseConfig:
        """Get LangFlow config."""
        from love_engine.llms.langflow.chat.transformation import LangFlowConfig
    ...
    def get_provider_chat_config(
        ...
        if provider == LlmProviders.OPENAI:
            if love_engine.openaiOSeriesConfig.is_model_o_series_model(model=model):
                return love_engine.openaiOSeriesConfig
            if love_engine.OpenAIGPT5Config.is_model_gpt_5_model(model=model):
                return love_engine.OpenAIGPT5Config()
        ...
        from love_engine.llms.openai_like.dynamic_config import create_config_class
        from love_engine.llms.openai_like.json_loader import JSONProviderRegistry
    ...
    def get_provider_embedding_config(
        ...
        if (
            love_engine.LlmProviders.VOYAGE == provider
            and love_engine.VoyageContextualEmbeddingConfig.is_contextualized_embeddings(
                model
            )
        ):
            return love_engine.VoyageContextualEmbeddingConfig()
        elif (
            love_engine.LlmProviders.VOYAGE == provider
            and love_engine.VoyageMultimodalEmbeddingConfig.is_multimodal_embeddings(model)
        ):
            return love_engine.VoyageMultimodalEmbeddingConfig()
        elif love_engine.LlmProviders.VOYAGE == provider:
            return love_engine.VoyageEmbeddingConfig()
        elif love_engine.LlmProviders.TRITON == provider:
            return love_engine.TritonEmbeddingConfig()
        elif love_engine.LlmProviders.WATSONX == provider:
            return love_engine.IBMWatsonXEmbeddingConfig()
        elif love_engine.LlmProviders.SAP_GENERATIVE_AI_HUB == provider:
            return love_engine.GenAIHubEmbeddingConfig()
        elif love_engine.LlmProviders.INFINITY == provider:
            return love_engine.InfinityEmbeddingConfig()
        elif love_engine.LlmProviders.SAMBANOVA == provider:
            return love_engine.SambaNovaEmbeddingConfig()
        elif love_engine.LlmProviders.OCI == provider:
            from love_engine.llms.oci.embed.transformation import OCIEmbedConfig

            return OCIEmbedConfig()
        elif (
            love_engine.LlmProviders.COHERE == provider
            or love_engine.LlmProviders.COHERE_CHAT == provider
        ):
            from love_engine.llms.cohere.embed.transformation import CohereEmbeddingConfig

            return CohereEmbeddingConfig()
        elif love_engine.LlmProviders.JINA_AI == provider:
            from love_engine.llms.jina_ai.embedding.transformation import (
                JinaAIEmbeddingConfig,
            )

            return JinaAIEmbeddingConfig()
        elif love_engine.LlmProviders.VOLCENGINE == provider:
            from love_engine.llms.volcengine.embedding.transformation import (
                VolcEngineEmbeddingConfig,
            )

            return VolcEngineEmbeddingConfig()
        elif love_engine.LlmProviders.DASHSCOPE == provider:
            from love_engine.llms.dashscope.embed.transformation import (
                DashScopeEmbeddingConfig,
            )

            return DashScopeEmbeddingConfig()
        elif love_engine.LlmProviders.OVHCLOUD == provider:
            return love_engine.OVHCloudEmbeddingConfig()
        elif love_engine.LlmProviders.SNOWFLAKE == provider:
            return love_engine.SnowflakeEmbeddingConfig()
        elif love_engine.LlmProviders.COMETAPI == provider:
            return love_engine.CometAPIEmbeddingConfig()
        elif love_engine.LlmProviders.GITHUB_COPILOT == provider:
            return love_engine.GithubCopilotEmbeddingConfig()
        elif love_engine.LlmProviders.OPENROUTER == provider:
            from love_engine.llms.openrouter.embedding.transformation import (
                OpenrouterEmbeddingConfig,
            )

            return OpenrouterEmbeddingConfig()
        elif love_engine.LlmProviders.VERCEL_AI_GATEWAY == provider:
            from love_engine.llms.vercel_ai_gateway.embedding.transformation import (
                VercelAIGatewayEmbeddingConfig,
            )

            return VercelAIGatewayEmbeddingConfig()
        elif love_engine.LlmProviders.GIGACHAT == provider:
            return love_engine.GigaChatEmbeddingConfig()
        elif love_engine.LlmProviders.HOSTED_VLLM == provider:
            return love_engine.HostedVLLMEmbeddingConfig()
        elif love_engine.LlmProviders.SAGEMAKER == provider:
            from love_engine.llms.sagemaker.embedding.transformation import (
                SagemakerEmbeddingConfig,
            )

            return SagemakerEmbeddingConfig.get_model_config(model)
        elif love_engine.LlmProviders.PERPLEXITY == provider:
            return love_engine.PerplexityEmbeddingConfig()
        return None
    ...
    def get_provider_rerank_config(
        ...
        if (
            ...
            if should_use_cohere_v1_client(api_base, present_version_params):
                return love_engine.CohereRerankConfig()
            else:
                return love_engine.CohereRerankV2Config()
        elif love_engine.LlmProviders.AZURE_AI == provider:
            return love_engine.AzureAIRerankConfig()
        elif love_engine.LlmProviders.INFINITY == provider:
            return love_engine.InfinityRerankConfig()
        elif love_engine.LlmProviders.JINA_AI == provider:
            return love_engine.JinaAIRerankConfig()
        elif love_engine.LlmProviders.HOSTED_VLLM == provider:
            return love_engine.HostedVLLMRerankConfig()
        elif love_engine.LlmProviders.HUGGINGFACE == provider:
            return love_engine.HuggingFaceRerankConfig()
        elif love_engine.LlmProviders.DEEPINFRA == provider:
            return love_engine.DeepinfraRerankConfig()
        elif love_engine.LlmProviders.NVIDIA_NIM == provider:
            from love_engine.llms.nvidia_nim.rerank.common_utils import (
                get_nvidia_nim_rerank_config,
            )

            return get_nvidia_nim_rerank_config(model)
        elif love_engine.LlmProviders.VERTEX_AI == provider:
            return love_engine.VertexAIRerankConfig()
        elif love_engine.LlmProviders.FIREWORKS_AI == provider:
            return love_engine.FireworksAIRerankConfig()
        elif love_engine.LlmProviders.VOYAGE == provider:
            return love_engine.VoyageRerankConfig()
        elif love_engine.LlmProviders.WATSONX == provider:
            return love_engine.IBMWatsonXRerankConfig()
        elif love_engine.LlmProviders.DASHSCOPE == provider:
            from love_engine.llms.dashscope.rerank.transformation import (
    ...
    def _get_provider_anthropic_messages_config_cached(
        ...
        if love_engine.LlmProviders.ANTHROPIC == provider:
            return love_engine.AnthropicMessagesConfig()
        ...
        # This mapping ensures that the correct configuration is returned for BEDROCK.
        elif love_engine.LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.common_utils import BedrockModelInfo

            return BedrockModelInfo.get_bedrock_provider_config_for_messages_api(model)
        elif love_engine.LlmProviders.VERTEX_AI == provider:
            if "claude" in model_lower:
                from love_engine.llms.vertex_ai.vertex_ai_partner_models.anthropic.experimental_pass_through.transformation import (
            ...
            if "claude" in model_lower:
                from love_engine.llms.azure_ai.anthropic.messages_transformation import (
                    AzureAnthropicMessagesConfig,
                )

                return AzureAnthropicMessagesConfig()
        elif love_engine.LlmProviders.MINIMAX == provider:
            from love_engine.llms.minimax.messages.transformation import (
                MinimaxMessagesConfig,
            )

            return MinimaxMessagesConfig()
        elif love_engine.LlmProviders.DEEPSEEK == provider:
            from love_engine.llms.deepseek.messages.transformation import (
    ...
    def get_provider_audio_transcription_config(
        ...
        if (
            love_engine.LlmProviders.AZURE == provider
            and model_cost_entry.get("audio_transcription_config") == "azure_speech"
        ):
            from love_engine.llms.azure.audio_transcription.transformation import (
                AzureSpeechAudioTranscriptionConfig,
            )

            return AzureSpeechAudioTranscriptionConfig()
        if love_engine.LlmProviders.FIREWORKS_AI == provider:
            return love_engine.FireworksAIAudioTranscriptionConfig()
        elif love_engine.LlmProviders.DEEPGRAM == provider:
            return love_engine.DeepgramAudioTranscriptionConfig()
        elif love_engine.LlmProviders.ELEVENLABS == provider:
        ...
        elif love_engine.LlmProviders.OPENAI == provider:
            if "gpt-4o" in model:
                return love_engine.OpenAIGPTAudioTranscriptionConfig()
            else:
                return love_engine.OpenAIWhisperAudioTranscriptionConfig()
        elif love_engine.LlmProviders.HOSTED_VLLM == provider:
            from love_engine.llms.hosted_vllm.transcriptions.transformation import (
                HostedVLLMAudioTranscriptionConfig,
            )

            return HostedVLLMAudioTranscriptionConfig()
        elif love_engine.LlmProviders.WATSONX == provider:
            from love_engine.llms.watsonx.audio_transcription.transformation import (
        ...
        elif love_engine.LlmProviders.SCALEWAY == provider:
        ...
        elif love_engine.LlmProviders.MISTRAL == provider:
        ...
        elif love_engine.LlmProviders.NVIDIA_RIVA == provider:
            from love_engine.llms.nvidia_riva.audio_transcription.transformation import (
                NvidiaRivaAudioTranscriptionConfig,
            )

            return NvidiaRivaAudioTranscriptionConfig()
        elif love_engine.LlmProviders.SONIOX == provider:
            from love_engine.llms.soniox.audio_transcription.transformation import (
    ...
    def get_provider_responses_api_config(
        ...
        from love_engine.llms.openai_like.dynamic_config import (
    ...
    def _get_python_responses_api_config(
        ...
        if love_engine.LlmProviders.OPENAI == provider:
            return love_engine.OpenAIResponsesAPIConfig()
        elif love_engine.LlmProviders.AZURE == provider:
            ...
            if is_o_series:
                return love_engine.AzureOpenAIOSeriesResponsesAPIConfig()
            else:
                return love_engine.AzureOpenAIResponsesAPIConfig()
        elif love_engine.LlmProviders.XAI == provider:
            return love_engine.XAIResponsesAPIConfig()
        elif love_engine.LlmProviders.GITHUB_COPILOT == provider:
            from love_engine.llms.github_copilot.responses.transformation import (
                github_copilot_supports_responses_api,
            )

            if model is None or github_copilot_supports_responses_api(model=model):
                return love_engine.GithubCopilotResponsesAPIConfig()
            return None
        elif love_engine.LlmProviders.CHATGPT == provider:
            return love_engine.ChatGPTResponsesAPIConfig()
        elif love_engine.LlmProviders.LOVE_ENGINE_PROXY == provider:
            return love_engine.LoveEngineProxyResponsesAPIConfig()
        elif love_engine.LlmProviders.VOLCENGINE == provider:
            return love_engine.VolcEngineResponsesAPIConfig()
        elif love_engine.LlmProviders.MANUS == provider:
            return love_engine.ManusResponsesAPIConfig()
        elif love_engine.LlmProviders.PERPLEXITY == provider:
            return love_engine.PerplexityResponsesConfig()
        elif love_engine.LlmProviders.DATABRICKS == provider:
            # Databricks Responses API is only compatible with OpenAI GPT models
            if model and "gpt" in model.lower():
                return love_engine.DatabricksResponsesAPIConfig()
            return None
        elif love_engine.LlmProviders.OPENROUTER == provider:
            return love_engine.OpenRouterResponsesAPIConfig()
        elif love_engine.LlmProviders.HOSTED_VLLM == provider:
            return love_engine.HostedVLLMResponsesAPIConfig()
        elif love_engine.LlmProviders.BEDROCK_MANTLE == provider:
            ...
            if not model:
                return None
            model_lower = model.lower()
            entry = love_engine.model_cost.get(f"bedrock_mantle/{model}", {})
            ...
            if on_openai_path or name_is_frontier:
                return love_engine.BedrockMantleResponsesAPIConfig(use_openai_path=True)
            try:
                if get_model_info(model, "bedrock_mantle").get("mode") == "responses":
                    return love_engine.BedrockMantleResponsesAPIConfig(
    ...
    def get_provider_skills_api_config(
        ...
        """
        if love_engine.LlmProviders.ANTHROPIC == provider:
            return love_engine.AnthropicSkillsConfig()
        return None
    ...
    def get_provider_evals_api_config(
        ...
        """
        if love_engine.LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.evals.transformation import OpenAIEvalsConfig
    ...
    def get_provider_text_completion_config(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            return love_engine.FireworksAITextCompletionConfig()
        elif LlmProviders.TOGETHER_AI == provider:
            return love_engine.TogetherAITextCompletionConfig()
        elif LlmProviders.TEXT_COMPLETION_INCEPTION == provider:
            return love_engine.InceptionTextCompletionConfig()
        return love_engine.OpenAITextCompletionConfig()
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            return love_engine.FireworksAIConfig()
        elif LlmProviders.OPENAI == provider:
            return love_engine.OpenAIGPTConfig()
        elif LlmProviders.GEMINI == provider:
            return love_engine.GeminiModelInfo()
        elif LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.common_utils import VertexAIModelInfo

            return VertexAIModelInfo()
        elif LlmProviders.LOVE_ENGINE_PROXY == provider:
            return love_engine.LoveEngineProxyChatConfig()
        elif LlmProviders.TOPAZ == provider:
            return love_engine.TopazModelInfo()
        elif LlmProviders.ANTHROPIC == provider:
            return love_engine.AnthropicModelInfo()
        elif LlmProviders.XAI == provider:
            return love_engine.XAIModelInfo()
        elif LlmProviders.OLLAMA == provider or LlmProviders.OLLAMA_CHAT == provider:
            # Dynamic model listing for Ollama server
            from love_engine.llms.ollama.common_utils import OllamaModelInfo

            return OllamaModelInfo()
        elif LlmProviders.VLLM == provider or LlmProviders.HOSTED_VLLM == provider:
            from love_engine.llms.vllm.common_utils import (
            ...
            return love_engine.LemonadeChatConfig()
        elif LlmProviders.CLARIFAI == provider:
            return love_engine.ClarifaiConfig()
    ...
    def get_provider_passthrough_config(
        ...
        if LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.passthrough.transformation import (
                BedrockPassthroughConfig,
            )

            return BedrockPassthroughConfig()
        elif LlmProviders.VLLM == provider or LlmProviders.HOSTED_VLLM == provider:
            from love_engine.llms.vllm.passthrough.transformation import (
                VLLMPassthroughConfig,
            )

            return VLLMPassthroughConfig()
        elif LlmProviders.AZURE == provider:
            from love_engine.llms.azure.passthrough.transformation import (
                AzurePassthroughConfig,
            )

            return AzurePassthroughConfig()
        elif LlmProviders.WATSONX == provider:
            from love_engine.llms.watsonx.passthrough.transformation import (
    ...
    def get_provider_image_variation_config(
        ...
        if LlmProviders.OPENAI == provider:
            return love_engine.OpenAIImageVariationConfig()
        elif LlmProviders.TOPAZ == provider:
            return love_engine.TopazImageVariationConfig()
        return None
    ...
    def get_provider_files_config(
        ...
        if LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.files.transformation import (
                GoogleAIStudioFilesHandler,  # experimental approach, to reduce bloat on __init__.py
            )

            return GoogleAIStudioFilesHandler()
        elif LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.files.transformation import VertexAIFilesConfig

            return VertexAIFilesConfig()
        elif LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.files.transformation import BedrockFilesConfig

            return BedrockFilesConfig()
        elif LlmProviders.MANUS == provider:
            from love_engine.llms.manus.files.transformation import ManusFilesConfig

            return ManusFilesConfig()
        elif LlmProviders.ANTHROPIC == provider:
            from love_engine.llms.anthropic.files.transformation import AnthropicFilesConfig
    ...
    def get_provider_batches_config(
        ...
        if LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.batches.transformation import BedrockBatchesConfig
    ...
    def get_provider_vector_store_config(
        ...
        from love_engine.integrations.vector_store_integrations.bedrock_vector_store import (
            BedrockVectorStore,
        )
    ...
    def get_provider_vector_stores_config(
        ...
        """
        if love_engine.LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.vector_stores.transformation import (
                OpenAIVectorStoreConfig,
            )

            return OpenAIVectorStoreConfig()
        elif love_engine.LlmProviders.AZURE == provider:
            from love_engine.llms.azure.vector_stores.transformation import (
            ...
            if api_type == "rag_api" or api_type is None:  # default to rag_api
                from love_engine.llms.vertex_ai.vector_stores.rag_api.transformation import (
                    VertexVectorStoreConfig,
                )

                return VertexVectorStoreConfig()
            elif api_type == "search_api":
                from love_engine.llms.vertex_ai.vector_stores.search_api.transformation import (
                    VertexSearchAPIVectorStoreConfig,
                )

                return VertexSearchAPIVectorStoreConfig()
        elif love_engine.LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.vector_stores.transformation import (
                BedrockVectorStoreConfig,
            )

            return BedrockVectorStoreConfig()
        elif love_engine.LlmProviders.PG_VECTOR == provider:
            from love_engine.llms.pg_vector.vector_stores.transformation import (
                PGVectorStoreConfig,
            )

            return PGVectorStoreConfig()
        elif love_engine.LlmProviders.AZURE_AI == provider:
            from love_engine.llms.azure_ai.vector_stores.transformation import (
                AzureAIVectorStoreConfig,
            )

            return AzureAIVectorStoreConfig()
        elif love_engine.LlmProviders.MILVUS == provider:
            from love_engine.llms.milvus.vector_stores.transformation import (
                MilvusVectorStoreConfig,
            )

            return MilvusVectorStoreConfig()
        elif love_engine.LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.vector_stores.transformation import (
                GeminiVectorStoreConfig,
            )

            return GeminiVectorStoreConfig()
        elif love_engine.LlmProviders.RAGFLOW == provider:
            from love_engine.llms.ragflow.vector_stores.transformation import (
                RAGFlowVectorStoreConfig,
            )

            return RAGFlowVectorStoreConfig()
        elif love_engine.LlmProviders.S3_VECTORS == provider:
            from love_engine.llms.s3_vectors.vector_stores.transformation import (
    ...
    def get_provider_vector_store_files_config(
        ...
        if love_engine.LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.vector_store_files.transformation import (
    ...
    def get_provider_image_generation_config(
        ...
        if LlmProviders.OPENAI == provider:
            ...
            from love_engine.llms.xinference.image_generation import (
            ...
            from love_engine.llms.runwayml.image_generation import (
            ...
            from love_engine.llms.black_forest_labs.image_generation import (
                get_black_forest_labs_image_generation_config,
            )

            return get_black_forest_labs_image_generation_config(model)
        elif LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.image_generation import (
            ...
            from love_engine.llms.openrouter.image_generation import (
            ...
            from love_engine.llms.dashscope.image_generation import (
            ...
            from love_engine.llms.modelscope.image_generation import (
    ...
    def get_provider_video_config(
        ...
        if LlmProviders.OPENAI == provider:
            ...
            from love_engine.llms.azure.videos.transformation import AzureVideoConfig

            return AzureVideoConfig()
        elif LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.videos.transformation import GeminiVideoConfig

            return GeminiVideoConfig()
        elif LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.videos.transformation import VertexAIVideoConfig

            return VertexAIVideoConfig()
        elif LlmProviders.RUNWAYML == provider:
            from love_engine.llms.runwayml.videos.transformation import RunwayMLVideoConfig
    ...
    def get_provider_container_config(
        ...
        if LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.containers.transformation import (
                OpenAIContainerConfig,
            )

            return OpenAIContainerConfig()
        if provider in (LlmProviders.AZURE, LlmProviders.AZURE_TEXT):
            from love_engine.llms.azure.containers.transformation import (
    ...
    def get_provider_realtime_config(
        ...
        if LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.realtime.transformation import GeminiRealtimeConfig
    ...
    def get_provider_realtime_http_config(
        ...
        if LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.realtime.http_transformation import (
                OpenAIRealtimeHTTPConfig,
            )

            return OpenAIRealtimeHTTPConfig()
        if LlmProviders.AZURE == provider:
            from love_engine.llms.azure.realtime.http_transformation import (
    ...
    def get_provider_image_edit_config(
        ...
        if LlmProviders.OPENAI == provider:
            from love_engine.llms.openai.image_edit import get_openai_image_edit_config

            return get_openai_image_edit_config(model=model)
        elif LlmProviders.AZURE == provider:
            from love_engine.llms.azure.image_edit.transformation import (
                AzureImageEditConfig,
            )

            return AzureImageEditConfig()
        elif LlmProviders.RECRAFT == provider:
            from love_engine.llms.recraft.image_edit.transformation import (
                RecraftImageEditConfig,
            )

            return RecraftImageEditConfig()
        elif LlmProviders.BLACK_FOREST_LABS == provider:
            from love_engine.llms.black_forest_labs.image_edit.transformation import (
                BlackForestLabsImageEditConfig,
            )

            return BlackForestLabsImageEditConfig()
        elif LlmProviders.AZURE_AI == provider:
            from love_engine.llms.azure_ai.image_edit import get_azure_ai_image_edit_config

            return get_azure_ai_image_edit_config(model)
        elif LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.image_edit import get_gemini_image_edit_config

            return get_gemini_image_edit_config(model)
        elif LlmProviders.LOVE_ENGINE_PROXY == provider:
            from love_engine.llms.love_engine_proxy.image_edit.transformation import (
                LoveEngineProxyImageEditConfig,
            )

            return LoveEngineProxyImageEditConfig()
        elif LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.image_edit import (
                get_vertex_ai_image_edit_config,
            )

            return get_vertex_ai_image_edit_config(model)
        elif LlmProviders.STABILITY == provider:
            from love_engine.llms.stability.image_edit import (
                get_stability_image_edit_config,
            )

            return get_stability_image_edit_config(model)
        elif LlmProviders.BEDROCK == provider:
            from love_engine.llms.bedrock.image_edit.amazon_nova_canvas_image_edit_transformation import (
                get_bedrock_image_edit_config_for_model,
            )

            return get_bedrock_image_edit_config_for_model(model)
        elif LlmProviders.OPENROUTER == provider:
            from love_engine.llms.openrouter.image_edit import (
    ...
    def get_provider_ocr_config(
        ...
        """
        from love_engine.llms.vertex_ai.ocr.transformation import VertexAIOCRConfig

        # Special handling for Azure AI - distinguish between Mistral OCR and Document Intelligence
        if provider == love_engine.LlmProviders.AZURE_AI:
            from love_engine.llms.azure_ai.ocr.common_utils import get_azure_ai_ocr_config

            return get_azure_ai_ocr_config(model=model)

        if provider == love_engine.LlmProviders.VERTEX_AI:
            from love_engine.llms.vertex_ai.ocr.common_utils import get_vertex_ai_ocr_config

            return get_vertex_ai_ocr_config(model=model)

        if provider == love_engine.LlmProviders.REDUCTO:
            from love_engine.llms.reducto.ocr.transformation import (
        ...
        PROVIDER_TO_CONFIG_MAP = {
            love_engine.LlmProviders.MISTRAL: MistralOCRConfig,
        }
    ...
    def get_provider_search_config(
        ...
        """
        from love_engine.llms.apiserpent.search.transformation import (
            APISerpentSearchConfig,
        )
        from love_engine.llms.brave.search.transformation import BraveSearchConfig
        from love_engine.llms.dataforseo.search.transformation import DataForSEOSearchConfig
        from love_engine.llms.duckduckgo.search.transformation import DuckDuckGoSearchConfig
        from love_engine.llms.exa_ai.search.transformation import ExaAISearchConfig
        from love_engine.llms.fastcrw.search.transformation import FastCRWSearchConfig
        from love_engine.llms.firecrawl.search.transformation import FirecrawlSearchConfig
        from love_engine.llms.google_pse.search.transformation import GooglePSESearchConfig
        from love_engine.llms.linkup.search.transformation import LinkupSearchConfig
        from love_engine.llms.parallel_ai.search.transformation import (
            ParallelAISearchConfig,
        )
        from love_engine.llms.perplexity.search.transformation import PerplexitySearchConfig
        from love_engine.llms.searchapi.search.transformation import SearchAPIConfig
        from love_engine.llms.searxng.search.transformation import SearXNGSearchConfig
        from love_engine.llms.serper.search.transformation import SerperSearchConfig
        from love_engine.llms.tavily.search.transformation import TavilySearchConfig
        from love_engine.llms.tinyfish.search.transformation import TinyfishSearchConfig
        from love_engine.llms.you_com.search.transformation import YouComSearchConfig
    ...
    def get_provider_text_to_speech_config(
        ...
        if love_engine.LlmProviders.AZURE == provider:
            ...
            if model.startswith("speech/"):
                from love_engine.llms.azure.text_to_speech.transformation import (
                    AzureAVATextToSpeechConfig,
                )

                return AzureAVATextToSpeechConfig()
        elif love_engine.LlmProviders.ELEVENLABS == provider:
            from love_engine.llms.elevenlabs.text_to_speech.transformation import (
                ElevenLabsTextToSpeechConfig,
            )

            return ElevenLabsTextToSpeechConfig()
        elif love_engine.LlmProviders.RUNWAYML == provider:
            from love_engine.llms.runwayml.text_to_speech.transformation import (
                RunwayMLTextToSpeechConfig,
            )

            return RunwayMLTextToSpeechConfig()
        elif love_engine.LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.text_to_speech.transformation import (
                VertexAITextToSpeechConfig,
            )

            return VertexAITextToSpeechConfig()
        elif love_engine.LlmProviders.MINIMAX == provider:
            from love_engine.llms.minimax.text_to_speech.transformation import (
                MinimaxTextToSpeechConfig,
            )

            return MinimaxTextToSpeechConfig()
        elif love_engine.LlmProviders.AWS_POLLY == provider:
            from love_engine.llms.aws_polly.text_to_speech.transformation import (
    ...
    def get_provider_google_genai_generate_content_config(
        ...
        if love_engine.LlmProviders.GEMINI == provider:
            from love_engine.llms.gemini.google_genai.transformation import (
                GoogleGenAIConfig,
            )

            return GoogleGenAIConfig()
        elif love_engine.LlmProviders.VERTEX_AI == provider:
            from love_engine.llms.vertex_ai.google_genai.transformation import (
                VertexAIGoogleGenAIConfig,
            )
            from love_engine.llms.vertex_ai.vertex_ai_partner_models.main import (
                VertexAIPartnerModels,
            )

            #########################################################
            # If Vertex Partner models like Anthropic, Mistral, etc. are used,
            # return None as we want this to go through the love_engine.completion() adapter
...
def get_end_user_id_for_cost_tracking(
    ...
    service_type: Literal["love_engine_logging", "prometheus"] = "love_engine_logging",
) -> Optional[str]:
    """
    Used for enforcing `disable_end_user_cost_tracking` param.

    service_type: "love_engine_logging" or "prometheus" - used to allow prometheus only disable cost tracking.
    """
    get_love_engine_metadata_from_kwargs = getattr(
        sys.modules[__name__], "get_love_engine_metadata_from_kwargs"
    )
    _metadata = cast(
        dict, get_love_engine_metadata_from_kwargs(dict(love_engine_params=love_engine_params))
    )

    end_user_id = cast(
        Optional[str],
        love_engine_params.get("user_api_key_end_user_id")
        or _metadata.get("user_api_key_end_user_id"),
    )
    if love_engine.disable_end_user_cost_tracking:
        return None

    #######################################
    # By default we don't track end_user on prometheus since we don't want to increase cardinality
    # by default love_engine.enable_end_user_cost_tracking_prometheus_only is None, so we don't track end_user on prometheus
    #######################################
    if service_type == "prometheus":
        if love_engine.enable_end_user_cost_tracking_prometheus_only is not True:
            return None
    return end_user_id
...
def get_standard_openai_params(params: dict) -> dict:
    return {
        ...
        if k in love_engine.OPENAI_CHAT_COMPLETION_PARAMS and v is not None
    }


def get_non_default_completion_params(kwargs: dict) -> dict:
    openai_params = love_engine.OPENAI_CHAT_COMPLETION_PARAMS
    default_params = openai_params + all_love_engine_params
    non_default_params = {
...
def get_non_default_transcription_params(kwargs: dict) -> dict:
    ...
    default_params = OPENAI_TRANSCRIPTION_PARAMS + all_love_engine_params
...
def return_raw_request(endpoint: CallTypes, kwargs: dict) -> RawRequestTypedDict:
    ...
    This is currently in BETA, and tested for `/chat/completions` -> `love_engine.completion` calls.
    """
    from datetime import datetime
    ...
    love_engine_logging_obj = Logging(
        ...
        love_engine_call_id="1234",
        start_time=datetime.now(),
        function_id="1234",
        log_raw_request_response=True,
    )

    llm_api_endpoint = getattr(love_engine, endpoint.value)
    ...
    raw_request_typed_dict = love_engine_logging_obj.model_call_details.get(
...
def jsonify_tools(tools: List[Any]) -> List[Dict]:
    """
    Fixes https://github.com/BerriAI/love_engine/issues/9321
...
def __getattr__(name: str) -> Any:
    ...
    from love_engine._lazy_imports import _get_lazy_import_registry
