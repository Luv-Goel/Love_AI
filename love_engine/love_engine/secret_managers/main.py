...
from love_engine.secret_managers.secret_manager_handler import get_secret_from_manager
...
def _get_oidc_allowed_credential_dirs() -> list[str]:
    ...
    override via the ``LOVE_ENGINE_OIDC_ALLOWED_CREDENTIAL_DIRS`` environment
    variable (comma-separated list of absolute paths).
    """
    override = os.getenv("LOVE_ENGINE_OIDC_ALLOWED_CREDENTIAL_DIRS")
    raw_dirs = (
...
def _resolve_oidc_file_path(requested_path: str) -> str:
    ...
    raise ValueError(
        ...
        "Set LOVE_ENGINE_OIDC_ALLOWED_CREDENTIAL_DIRS to extend the allowlist."
    )
...
def get_secret(
    ...
    key_management_system = love_engine._key_management_system
    ...
    try:
        if (
            ...
            and love_engine.secret_manager_client is not None
        ):
            try:
...
def _should_read_secret_from_secret_manager() -> bool:
    ...
    """
    if love_engine.secret_manager_client is not None:
        if love_engine._key_management_settings is not None:
            if (
                love_engine._key_management_settings.access_mode == "read_only"
                or love_engine._key_management_settings.access_mode == "read_and_write"
            ):
                return True
    return False
