...
Relevant issue: https://github.com/BerriAI/love_engine/issues/1883
...
import love_engine
from love_engine.proxy._types import KeyManagementSystem
...
def load_aws_kms(use_aws_kms: Optional[bool]):
    ...
    try:
        ...
        love_engine.secret_manager_client = kms_client
        love_engine._key_management_system = KeyManagementSystem.AWS_KMS
...
class AWSKeyManagementService_V2:
    ...
    def validate_environment(
        ...
        is_love_engine_license_in_env: bool = False

        if os.getenv("LOVE_ENGINE_LICENSE", None) is not None:
            is_love_engine_license_in_env = True
        elif os.getenv("LOVE_ENGINE_SECRET_AWS_KMS_LOVE_ENGINE_LICENSE", None) is not None:
            is_love_engine_license_in_env = True
        if is_love_engine_license_in_env is False:
            raise ValueError(
                "AWSKeyManagementService V2 is an Enterprise Feature. Please add a valid LOVE_ENGINE_LICENSE to your envionment."
            )
