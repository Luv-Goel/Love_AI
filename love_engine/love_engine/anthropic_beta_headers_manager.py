...
- Local file: Set LOVE_ENGINE_LOCAL_ANTHROPIC_BETA_HEADERS=True to use bundled config only

Environment Variables:
- LOVE_ENGINE_LOCAL_ANTHROPIC_BETA_HEADERS: Set to "True" to disable remote fetching
- LOVE_ENGINE_ANTHROPIC_BETA_HEADERS_URL: Custom URL for remote config (optional)
"""
    ...
    def _check_is_valid_dict(fetched_config: dict) -> bool:
        ...
        if not isinstance(fetched_config, dict):
            verbose_logger.warning(
                "love_engine: Fetched beta headers config is not a dict (type=%s). "
        ...
        if len(fetched_config) == 0:
            verbose_logger.warning(
                "love_engine: Fetched beta headers config is empty. "
                "Falling back to local backup.",
            )
            return False
        ...
        if not has_provider:
            verbose_logger.warning(
                "love_engine: Fetched beta headers config missing provider keys. "
                "Falling back to local backup.",
            )
            return False
...
def get_beta_headers_config(url: str) -> dict:
    ...
    1. If ``LOVE_ENGINE_LOCAL_ANTHROPIC_BETA_HEADERS`` is set, uses the local backup only.
    ...
    if os.getenv("LOVE_ENGINE_LOCAL_ANTHROPIC_BETA_HEADERS", "").lower() == "true":
        # verbose_logger.debug("Using local Anthropic beta headers config (LOVE_ENGINE_LOCAL_ANTHROPIC_BETA_HEADERS=True)")
    ...
    try:
        ...
        verbose_logger.warning(
            "love_engine: Failed to fetch remote beta headers config from %s: %s. "
            "Falling back to local backup.",
            url,
            str(e),
        )
    ...
    if not GetAnthropicBetaHeadersConfig.validate_beta_headers_config(
        ...
        verbose_logger.warning(
            "love_engine: Fetched beta headers config failed integrity check. "
...
def _load_beta_headers_config() -> Dict:
    ...
    from love_engine import anthropic_beta_headers_url
