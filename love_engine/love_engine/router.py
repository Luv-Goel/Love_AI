...
import love_engine.love_engine_core_utils.exception_mapping_utils
from love_engine import get_secret_str
from love_engine._logging import verbose_router_logger
from love_engine._uuid import uuid
from love_engine.caching.caching import (
    DualCache,
    InMemoryCache,
    RedisCache,
    RedisClusterCache,
...
from love_engine.love_engine_core_utils.asyncify import run_async_function
...
from love_engine.love_engine_core_utils.dd_tracing import tracer
from love_engine.love_engine_core_utils.love_engine_logging import Logging as LoveEngineLogging
from love_engine.love_engine_core_utils.sensitive_data_masker import SensitiveDataMasker
from love_engine.llms.openai_like.json_loader import JSONProviderRegistry
from love_engine.router_strategy.budget_limiter import RouterBudgetLimiting
from love_engine.router_strategy.least_busy import LeastBusyLoggingHandler
from love_engine.router_strategy.lowest_cost import LowestCostLoggingHandler
from love_engine.router_strategy.lowest_latency import LowestLatencyLoggingHandler
from love_engine.router_strategy.lowest_tpm_rpm import LowestTPMLoggingHandler
from love_engine.router_strategy.lowest_tpm_rpm_v2 import LowestTPMLoggingHandler_v2
from love_engine.router_strategy.simple_shuffle import simple_shuffle
from love_engine.router_strategy.tag_based_routing import get_deployments_for_tag
from love_engine.router_utils.add_retry_fallback_headers import (
    add_fallback_headers_to_response,
    add_retry_headers_to_response,
)
from love_engine.router_utils.batch_utils import (
    _get_router_metadata_variable_name,
    replace_model_in_jsonl,
    should_replace_model_in_jsonl,
)
from love_engine.router_utils.client_initalization_utils import InitalizeCachedClient
from love_engine.router_utils.clientside_credential_handler import (
    get_dynamic_love_engine_params,
    is_clientside_credential,
)
from love_engine.router_utils.common_utils import (
    filter_team_based_models,
    filter_web_search_deployments,
)
from love_engine.router_utils.cooldown_cache import CooldownCache
from love_engine.router_utils.cooldown_handlers import (
...
from love_engine.router_utils.fallback_event_handlers import (
...
from love_engine.router_utils.handle_error import (
    async_raise_no_deployment_exception,
    send_llm_exception_alert,
)
from love_engine.router_utils.health_state_cache import DeploymentHealthCache
from love_engine.router_utils.pre_call_checks.deployment_affinity_check import (
    DeploymentAffinityCheck,
)
from love_engine.router_utils.pre_call_checks.model_rate_limit_check import (
    ModelRateLimitingCheck,
)
from love_engine.router_utils.pre_call_checks.prompt_caching_deployment_check import (
...
from love_engine.types.router import (
    ...
    VALID_LOVE_ENGINE_ENVIRONMENTS,
    AlertingConfig,
    AllowedFailsPolicy,
    AssistantsTypedDict,
    CredentialLoveEngineParams,
...
from love_engine.types.services import ServiceTypes
from love_engine.types.utils import (
    CustomPricingLoveEngineParams,
    GenericBudgetConfigType,
    LoveEngineBatch,
)
from love_engine.types.utils import ModelInfo
...
if TYPE_CHECKING:
    ...
    from love_engine.router_strategy.auto_router.auto_router import (
        AutoRouter,
        PreRoutingHookResponse,
    )
    from love_engine.router_strategy.complexity_router.complexity_router import (
        ComplexityRouter,
    )
    from love_engine.router_strategy.adaptive_router.adaptive_router import (
        AdaptiveRouter,
    )
    from love_engine.router_strategy.quality_router.quality_router import (
        QualityRouter,
    )
    from love_engine.responses.streaming_iterator import (
        BaseResponsesAPIStreamingIterator,
    )
    from love_engine.types.llms.base import BaseLoveEngineOpenAIResponseObject
    from love_engine.types.llms.openai import (
        ResponseAPIUsage,
        ResponseInputParam,
...
class Router:
    ...
    def __init__(
        ...
        default_love_engine_params: Optional[
            dict
        ...
        Args:
            ...
            default_love_engine_params (dict): Default parameters for Router.chat.completion.create. Defaults to {}.
        ...
        Returns:
            Router: An instance of the love_engine.Router class.
        ...
        model_list = [
        {
            ...
            "love_engine_params": { # params for love_engine completion/embedding call
        ...
        from love_engine._service_logger import ServiceLogging

        self.service_logger_obj: ServiceLogging = ServiceLogging()
        love_engine.suppress_debug_info = True  # prevents 'Give Feedback/Get help' message from being emitted on Router - Relevant Issue: https://github.com/BerriAI/love_engine/issues/5942
        ...
        self.deployment_names: List = (
            []
        )  # names of models under love_engine_params. ex. azure/chatgpt-v-2
        ...
        if cache_responses:
            if love_engine.cache is None:
                # the cache can be initialized on the proxy server. We should not overwrite it
                love_engine.cache = love_engine.Cache(type=cache_type, **cache_config)  # type: ignore
        ...
        # ``love_engine.proxy.auth.auth_checks._is_model_cost_zero``.
        ...
        if model_list is not None:
            ...
            for m in model_list:
                if "model" in m["love_engine_params"]:
                    self.deployment_latency_map[m["love_engine_params"]["model"]] = 0
        else:
        ...
        if allowed_fails is not None:
            ...
            self.allowed_fails = love_engine.allowed_fails
        ...
        elif love_engine.num_retries is not None:
            self.num_retries = love_engine.num_retries
        else:
            self.num_retries = openai.DEFAULT_MAX_RETRIES

        if max_fallbacks is not None:
            self.max_fallbacks = max_fallbacks
        elif love_engine.max_fallbacks is not None:
            self.max_fallbacks = love_engine.max_fallbacks
        else:
            self.max_fallbacks = love_engine.ROUTER_MAX_FALLBACKS

        self._explicit_timeout = timeout  # None when user did not pass timeout
        self.timeout = timeout or love_engine.request_timeout
        ...
        _fallbacks = fallbacks or love_engine.fallbacks

        self.validate_fallbacks(fallback_param=_fallbacks)
        ### set fallbacks
        self.fallbacks = _fallbacks

        if default_fallbacks is not None or love_engine.default_fallbacks is not None:
            _fallbacks = default_fallbacks or love_engine.default_fallbacks
        ...
        self.context_window_fallbacks = (
            context_window_fallbacks or love_engine.context_window_fallbacks
        )

        _content_policy_fallbacks = (
            content_policy_fallbacks or love_engine.content_policy_fallbacks
        )
        ...
        default_love_engine_params = default_love_engine_params or {}
        self.chat = love_engine.Chat(params=default_love_engine_params, router_obj=self)

        # default love_engine args
        self.default_love_engine_params = default_love_engine_params
        self.default_love_engine_params.setdefault("timeout", timeout)
        self.default_love_engine_params.setdefault("max_retries", 0)
        self.default_love_engine_params.setdefault("metadata", {}).update(
        ...
        ## USAGE TRACKING ##
        if isinstance(love_engine._async_success_callback, list):
        ...
        if self.model_group_affinity_config and not any(
            ...
            love_engine.logging_callback_manager.add_love_engine_callback(affinity_callback)
    ...
    def discard(self):
        ...
        """
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine._async_success_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine.success_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine._async_failure_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine.failure_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine.input_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine.service_callback, self
        )
        love_engine.logging_callback_manager.remove_callback_from_list_by_object(
            love_engine.callbacks, self
        )
        ...
        if self.optional_callbacks is not None:
            for callback in self.optional_callbacks:
                love_engine.logging_callback_manager.remove_callback_from_list_by_object(
                    love_engine.callbacks, callback, require_self=False
                )
    ...
    def _validate_routing_strategy(
        ...
        # See: https://github.com/BerriAI/love_engine/issues/11330
    ...
    def _build_strategy_selector(
        ...
        match self._normalize_strategy(strategy):
            case RoutingStrategy.LEAST_BUSY.value:
                ...
                if register_callbacks:
                    if isinstance(love_engine.input_callback, list):
                        love_engine.input_callback.append(selector)  # type: ignore
                    else:
                        love_engine.input_callback = [selector]  # type: ignore
        ...
        if (
            ...
            and isinstance(love_engine.callbacks, list)
        ):
            love_engine.logging_callback_manager.add_love_engine_callback(selector)  # type: ignore

        return selector

    def _unregister_router_selectors(self, selectors: List[Any]) -> None:
        """
        Drop router-owned strategy selectors from love_engine's global callback
        ...
        if not selector_ids:
            return
        if isinstance(love_engine.callbacks, list):
            love_engine.callbacks = [
                c for c in love_engine.callbacks if id(c) not in selector_ids
            ]
        if isinstance(love_engine.input_callback, list):
            love_engine.input_callback = [
                c for c in love_engine.input_callback if id(c) not in selector_ids
            ]
    ...
    def initialize_assistants_endpoint(self):
        ...
        self.acreate_assistants = self.factory_function(love_engine.acreate_assistants)
        self.adelete_assistant = self.factory_function(love_engine.adelete_assistant)
        self.aget_assistants = self.factory_function(love_engine.aget_assistants)
        self.acreate_thread = self.factory_function(love_engine.acreate_thread)
        self.aget_thread = self.factory_function(love_engine.aget_thread)
        self.a_add_message = self.factory_function(love_engine.a_add_message)
        self.aget_messages = self.factory_function(love_engine.aget_messages)
        self.arun_thread = self.factory_function(love_engine.arun_thread)

    def _initialize_core_endpoints(self):
        """Helper to initialize core router endpoints."""
        self.amoderation = self.factory_function(
            love_engine.amoderation, call_type="moderation"
        )
        self.aanthropic_messages = self.factory_function(
            love_engine.anthropic_messages, call_type="anthropic_messages"
        )
        self.anthropic_messages = self.factory_function(
            love_engine.anthropic_messages, call_type="anthropic_messages"
        )
        self.agenerate_content = self.factory_function(
            love_engine.agenerate_content, call_type="agenerate_content"
        )
        self.aadapter_generate_content = self.factory_function(
            love_engine.aadapter_generate_content, call_type="aadapter_generate_content"
        )
        self.aresponses = self.factory_function(
            love_engine.aresponses, call_type="aresponses"
        )
        self.afile_delete = self.factory_function(
            love_engine.afile_delete, call_type="afile_delete"
        )
        self.afile_content = self.factory_function(
            love_engine.afile_content, call_type="afile_content"
        )
        self.responses = self.factory_function(love_engine.responses, call_type="responses")
        self.aget_responses = self.factory_function(
            love_engine.aget_responses, call_type="aget_responses"
        )
        self.acancel_responses = self.factory_function(
            love_engine.acancel_responses, call_type="acancel_responses"
        )
        self.acompact_responses = self.factory_function(
            love_engine.acompact_responses, call_type="acompact_responses"
        )
        self.adelete_responses = self.factory_function(
            love_engine.adelete_responses, call_type="adelete_responses"
        )
        self.alist_input_items = self.factory_function(
            love_engine.alist_input_items, call_type="alist_input_items"
        )
        self._arealtime = self.factory_function(
            love_engine._arealtime, call_type="_arealtime"
        )
        self._aresponses_websocket = self.factory_function(
            love_engine._aresponses_websocket, call_type="_aresponses_websocket"
        )
        self.acreate_fine_tuning_job = self.factory_function(
            love_engine.acreate_fine_tuning_job, call_type="acreate_fine_tuning_job"
        )
        self.acancel_fine_tuning_job = self.factory_function(
            love_engine.acancel_fine_tuning_job, call_type="acancel_fine_tuning_job"
        )
        self.alist_fine_tuning_jobs = self.factory_function(
            love_engine.alist_fine_tuning_jobs, call_type="alist_fine_tuning_jobs"
        )
        self.aretrieve_fine_tuning_job = self.factory_function(
            love_engine.aretrieve_fine_tuning_job, call_type="aretrieve_fine_tuning_job"
        )
        self.afile_list = self.factory_function(
            love_engine.afile_list, call_type="alist_files"
        )
        self.aimage_edit = self.factory_function(
            love_engine.aimage_edit, call_type="aimage_edit"
        )
        self.allm_passthrough_route = self.factory_function(
            love_engine.allm_passthrough_route, call_type="allm_passthrough_route"
        )
    ...
    def _initialize_vector_store_endpoints(self):
        ...
        from love_engine.vector_stores.main import (
            adelete,
            alist,
            aretrieve,
            asearch,
    ...
    def _initialize_vector_store_file_endpoints(self):
        ...
        from love_engine.vector_store_files.main import (
            acreate as avector_store_file_create_fn,
        )
        from love_engine.vector_store_files.main import (
            adelete as avector_store_file_delete_fn,
        )
        from love_engine.vector_store_files.main import alist as avector_store_file_list_fn
        from love_engine.vector_store_files.main import (
        ...
        from love_engine.vector_store_files.main import list as vector_store_file_list_fn
        from love_engine.vector_store_files.main import (
    ...
    def _initialize_google_genai_endpoints(self):
        ...
        from love_engine.google_genai import (
            agenerate_content,
    ...
    def _initialize_video_endpoints(self):
        ...
        from love_engine.videos import (
            avideo_content,
    ...
    def _initialize_container_endpoints(self):
        ...
        from love_engine.containers import (
            acreate_container,
            adelete_container,
            alist_containers,
            aretrieve_container,
        ...
        from love_engine.containers.endpoint_factory import (
    ...
    def _initialize_skills_endpoints(self):
        ...
        self.acreate_skill = self.factory_function(
            love_engine.acreate_skill, call_type="acreate_skill"
        )
        self.alist_skills = self.factory_function(
            love_engine.alist_skills, call_type="alist_skills"
        )
        self.aget_skill = self.factory_function(
            love_engine.aget_skill, call_type="aget_skill"
        )
        self.adelete_skill = self.factory_function(
            love_engine.adelete_skill, call_type="adelete_skill"
        )

    def _initialize_interactions_endpoints(self):
        """Initialize Google Interactions API endpoints."""
        from love_engine.interactions import acancel as acancel_interaction
        from love_engine.interactions import acreate as acreate_interaction
        from love_engine.interactions import adelete as adelete_interaction
        from love_engine.interactions import aget as aget_interaction
        from love_engine.interactions import cancel as cancel_interaction
        from love_engine.interactions import create as create_interaction
        from love_engine.interactions import delete as delete_interaction
        from love_engine.interactions import get as get_interaction
    ...
    def _initialize_managed_agents_endpoints(self):
        ...
        from love_engine.interactions.agents import acreate as acreate_agent
        from love_engine.interactions.agents import adelete as adelete_agent
        from love_engine.interactions.agents import aget as aget_agent
        from love_engine.interactions.agents import alist as alist_agents
        from love_engine.interactions.agents import alist_versions as alist_agent_versions
        from love_engine.interactions.agents import create as create_agent
        from love_engine.interactions.agents import delete as delete_agent
        from love_engine.interactions.agents import get as get_agent
        from love_engine.interactions.agents import list as list_agents
        from love_engine.interactions.agents import list_versions as list_agent_versions
    ...
    def _add_encrypted_content_affinity_check(
        ...
        from love_engine.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        ...
        if (
            ...
            if existing_ec_callback is not None:
                ...
                love_engine.logging_callback_manager.add_love_engine_callback(ec_callback)

            _move_before_deployment_affinity(self.optional_callbacks, ec_callback)
            _move_before_deployment_affinity(love_engine.callbacks, ec_callback)
    ...
    def print_deployment(self, deployment: dict):
        ...
        try:
            ...
            love_engine_params: dict = _deployment_copy["love_engine_params"]

            if love_engine.redact_user_api_key_info:
                masker = SensitiveDataMasker(visible_prefix=2, visible_suffix=0)
                _deployment_copy["love_engine_params"] = masker.mask_dict(love_engine_params)
            elif "api_key" in love_engine_params:
                love_engine_params["api_key"] = love_engine_params["api_key"][:2] + "*" * 10
    ...
    def _completion(
        ...
        try:
            ...
            # Make a local copy of love_engine_params to avoid mutating the Router's state
            love_engine_params = deployment["love_engine_params"].copy()
            silent_model = love_engine_params.pop("silent_model", None)
            ...
            model_name = love_engine_params["model"]
            ...
            response = love_engine.completion(**input_kwargs)
            verbose_router_logger.info(
                f"love_engine.completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            ...
            if isinstance(response, ModelResponse):
                ...
                if _should_raise:
                    raise love_engine.ContentPolicyViolationError(
            ...
            verbose_router_logger.info(
                f"love_engine.completion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    def _get_silent_experiment_kwargs(self, **kwargs) -> dict:
        ...
        silent_kwargs["metadata"].pop("love_engine_parent_otel_span", None)
        ...
        silent_kwargs.pop("love_engine_call_id", None)
        silent_kwargs.pop("love_engine_logging_obj", None)
        ...
        try:
            ...
            try:
                ...
                async def _run_silent_completion():
                    ...
                    # Drain any fire-and-love_smitht tasks (e.g. alerting hooks)
    ...
    async def acompletion(
        ...
        **kwargs,
        ...
        try:
            ...
            asyncio.create_task(
                send_llm_exception_alert(
                    love_engine_router_instance=self,
                    request_kwargs=kwargs,
    ...
    def _combine_fallback_usage(
        ...
        from love_engine.cost_calculator import BaseTokenUsageProcessor
    ...
    async def _acompletion_streaming_iterator(
        ...
        """
        from love_engine.exceptions import MidStreamFallbackError
        ...
        class FallbackStreamWrapper(CustomStreamWrapper):
            def __init__(self, async_generator: AsyncGenerator):
                ...
                # Preserve hidden params (including love_engine_overhead_time_ms) from original response
        ...
        async def stream_with_fallbacks():
            ...
            try:
                ...
                from love_engine.main import stream_chunk_builder
    ...
    def _extract_partial_responses_usage(
        ...
        Two sources, in priority order:
          1. The bridge path (LoveEngineCompletionStreamingIterator) accumulates
        ...
        if isinstance(source_iterator, LoveEngineCompletionStreamingIterator):
    ...
    def _combine_responses_fallback_usage(
        fallback_item: "BaseLoveEngineOpenAIResponseObject",
    ...
    async def _aresponses_streaming_iterator(
        ...
        LoveEngineCompletionStreamingIterator when the Responses API is
        ...
        class FallbackResponsesStreamWrapper(BaseResponsesAPIStreamingIterator):
            ...
            def __init__(self, async_generator: AsyncGenerator):
                ...
                #
                # The bridge path (LoveEngineCompletionStreamingIterator used by
                ...
                # bridge stores the logging object as `love_engine_logging_obj`.
                # base class declares non-Optional types for these
                # fields but the bridge path (LoveEngineCompletionStreamingIterator)
                ...
                self.logging_obj = getattr(  # type: ignore[assignment]
                    source_iterator,
                    "logging_obj",
                    getattr(source_iterator, "love_engine_logging_obj", None),
                )
                self.finished = False
                ...
                self.love_engine_metadata = getattr(
                    source_iterator, "love_engine_metadata", None
                )
        ...
        async def stream_with_fallbacks():
            ...
            try:
                ...
                try:
                    ...
                    # under "love_engine_metadata" (not the default "metadata") —
                    # see _ageneric_api_call_with_fallbacks. Mirroring that
                    # here ensures model_group, model_group_alias, and trace
                    # ids land in the same key love_engine.aresponses reads from.
                    self._update_kwargs_before_fallbacks(
                        model=model_group,
                        kwargs=initial_kwargs,
                        metadata_variable_name="love_engine_metadata",
                    )
                    fallback_response = (
                        await self.async_function_with_fallbacks_common_utils(
                            e=e,
    ...
    async def _acompletion(
        ...
        try:
            ...
            _response = love_engine.acompletion(**input_kwargs)

            logging_obj: Optional[LoveEngineLogging] = kwargs.get(
            ...
            verbose_router_logger.info(
                f"love_engine.acompletion(model={model_name})\033[32m 200 OK\033[0m"
            )
            # debug how often this deployment picked
            self._track_deployment_metrics(
                deployment=deployment,
            ...
            return response
        except love_engine.Timeout as e:
            ...
            if love_engine.expose_router_debug_in_errors:
            ...
            verbose_router_logger.info(
                f"love_engine.acompletion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    def _update_kwargs_before_fallbacks(
        ...
        - num_retries
        - love_engine_trace_id
        - metadata
        """
        kwargs["num_retries"] = kwargs.get("num_retries", self.num_retries)
        kwargs.setdefault("love_engine_trace_id", str(uuid.uuid4()))
    ...
    def _set_deployment_num_retries_on_exception(
        ...
        """
        Set num_retries from deployment love_engine_params on the exception.
        ...
        love_engine_params = deployment.get("love_engine_params", {})
        dep_num_retries = love_engine_params.get("num_retries")
    ...
    def _update_kwargs_with_default_love_engine_params(
        self, kwargs: dict, metadata_variable_name: Optional[str] = "metadata"
    ) -> None:
        """
        Adds default love_engine params to kwargs, if set.

        Handles inserting this as either "metadata" or "love_engine_metadata" depending on the metadata_variable_name
        """
        # 1) copy your defaults and pull out metadata
        defaults = self.default_love_engine_params.copy()
        ...
        # 3) merge in metadata, this handles inserting this as either "metadata" or "love_engine_metadata"
    ...
    def _handle_clientside_credential(
        ...
        love_engine_params = deployment["love_engine_params"].copy()
        dynamic_love_engine_params = get_dynamic_love_engine_params(
            love_engine_params=love_engine_params, request_kwargs=kwargs
        )
        ...
        _model_id = self._generate_model_id(
            model_group=model_group, love_engine_params=dynamic_love_engine_params
        )
        ...
        deployment_pydantic_obj = Deployment(
            ...
            love_engine_params=LOVE_ENGINE_Params(**dynamic_love_engine_params),
    ...
    def _merge_tools_from_deployment(deployment: dict, kwargs: dict) -> None:
        """
        Merge tools from deployment love_engine_params with request kwargs.
        ...
        """
        dep_params_raw = deployment.get("love_engine_params", {}) or {}
    ...
    def _update_kwargs_with_deployment(
        ...
        model_info = deployment.get("model_info", {}).copy()
        deployment_love_engine_model_name = deployment["love_engine_params"]["model"]
        deployment_api_base = deployment["love_engine_params"].get("api_base")
        ...
        if is_clientside_credential(request_kwargs=kwargs):
            ...
            deployment_love_engine_model_name = deployment_pydantic_obj.love_engine_params.model
            deployment_api_base = deployment_pydantic_obj.love_engine_params.api_base
        ...
        kwargs.setdefault(metadata_variable_name, {}).update(
            {
                "deployment": deployment_love_engine_model_name,
                "model_info": model_info,
        ...
        deployment_tags = deployment.get("love_engine_params", {}).get("tags")
        if deployment_tags:
        ...
        credential_name = deployment.get("love_engine_params", {}).get(
            "love_engine_credential_name"
        )
        if credential_name:
        ...
        if function_name == "_ageneric_api_call_with_fallbacks":
            from love_engine.passthrough.timeout_utils import (
            ...
            kwargs["timeout"] = resolve_llm_passthrough_timeout(
                kwargs=kwargs,
                love_engine_params=deployment["love_engine_params"],
                router_timeout=_router_timeout,
            )
        else:
            kwargs["timeout"] = self._get_timeout(
                kwargs=kwargs, data=deployment["love_engine_params"]
            )

        self._update_kwargs_with_default_love_engine_params(
    ...
    def _get_stream_timeout(
        ...
        return (
            ...
            or data.get(
                "stream_timeout", None
            )  # timeout set on love_engine_params for this deployment
            or self.stream_timeout  # timeout set on router
            or self.default_love_engine_params.get("stream_timeout", None)
        )
    ...
    def _get_non_stream_timeout(
        ...
        timeout = (
            ...
            or self.default_love_engine_params.get("timeout", None)
        )
        return timeout
    ...
    async def abatch_completion(
        ...
        **kwargs,
        ...
        1. Batch Process 1 request to N models on love_engine.Router. Pass messages as List[Dict[str, str]] to use this
        2. Batch Process N requests to M models on love_engine.Router. Pass messages as List[List[Dict[str, str]]] to use this
        ...
        ```
            response = await router.abatch_completion(
                ...
                messages=[
                    {"role": "user", "content": "is love_engine becoming a better product ?"}
                ],
                max_tokens=15,
            )
        ```
    ...
    async def abatch_completion_one_model_multiple_requests(
        ...
        """
        Async Batch Completion - Batch Process multiple Messages to one model_group on love_engine.Router
        ...
        if make_request:
            try:
                ...
                _response._hidden_params["additional_headers"].update(
                    {"x-love_engine-request-prioritization-used": True}
                )
                return _response
            except Exception as e:
    ...
    def _is_prompt_management_model(self, model: str) -> bool:
        ...
        love_engine_model = model_list[0]["love_engine_params"].get("model", None)
        if love_engine_model is None or "/" not in love_engine_model:
            return False

        split_love_engine_model = love_engine_model.split("/")[0]
        return split_love_engine_model in love_engine._known_custom_logger_compatible_callbacks
    ...
    async def _prompt_management_factory(
        ...
        love_engine_logging_object = kwargs.get("love_engine_logging_obj", None)
        if love_engine_logging_object is None:
            love_engine_logging_object, kwargs = function_setup(
                **{
        ...
        love_engine_logging_object = cast(LoveEngineLogging, love_engine_logging_object)
        ...
        data = prompt_management_deployment["love_engine_params"].copy()

        love_engine_model = data.get("model", None)

        # love_engine_agent/ prefix only strips the model name, no prompt_id needed
        is_love_engine_agent_model = isinstance(
            love_engine_model, str
        ) and love_engine_model.startswith("love_engine_agent/")
        ...
        prompt_variables = kwargs.get(
            "prompt_variables"
        ) or prompt_management_deployment["love_engine_params"].get(
        ...
        if not is_love_engine_agent_model and (
        ...
        (
            ...
            messages,
            optional_params,
        ) = love_engine_logging_object.get_chat_completion_prompt(
            model=love_engine_model,
            messages=messages,
        ...
        kwargs["model"] = model
        kwargs["messages"] = messages
        kwargs["love_engine_logging_obj"] = love_engine_logging_object
        ...
        if _model_list is None or len(_model_list) == 0:  # if direct call to model
            ...
            return await love_engine.acompletion(**kwargs)
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self._update_kwargs_with_deployment(deployment=deployment, kwargs=kwargs)
            data = deployment["love_engine_params"].copy()
            ...
            response = love_engine.image_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.image_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.image_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aimage_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = love_engine.aimage_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.aimage_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.aimage_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _atranscription(self, file: FileTypes, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = love_engine.atranscription(
                **{
                    **data,
                    "file": file,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.atranscription(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.atranscription(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aspeech(self, model: str, input: str, voice: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.aspeech(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.aspeech(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _arerank(self, model: str, **kwargs):
        ...
        try:
            ...
            response = await love_engine.arerank(
                **{
                    **data,
                    "caching": self.cache_responses,
                    "client": model_client,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.arerank(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.arerank(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            data = deployment["love_engine_params"].copy()
            for k, v in self.default_love_engine_params.items():
                if (
                    k not in kwargs
            ...
            # call via love_engine.completion()
            return love_engine.text_completion(**{**data, "prompt": prompt, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
            raise e
    ...
    async def _atext_completion(self, model: str, prompt: str, **kwargs):
        try:
            ...
            response = love_engine.atext_completion(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.atext_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.atext_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    async def _aadapter_completion(self, adapter_id: str, model: str, **kwargs):
        try:
            ...
            response = love_engine.aadapter_completion(
                **{
                    **data,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.aadapter_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.aadapter_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _aguardrail_helper(
        ...
        **kwargs,
        ...
        verbose_router_logger.debug(
            f"Selected guardrail deployment: {selected_guardrail.get('love_engine_params', {}).get('guardrail')}"
        )
    ...
    async def _ageneric_api_call_with_fallbacks(
        ...
        """
        Helper function to make a generic LLM API call through the router, this allows you to use retries/fallbacks with love_engine router
        """
        try:
            kwargs["model"] = model
            kwargs["original_generic_function"] = original_function
            ...
            self._update_kwargs_before_fallbacks(
                model=model, kwargs=kwargs, metadata_variable_name="love_engine_metadata"
            )
            verbose_router_logger.debug(
    ...
    def _add_deployment_model_to_endpoint_for_llm_passthrough_route(
        ...
        if "endpoint" in kwargs and kwargs["endpoint"]:
            ...
            from love_engine import get_llm_provider
    ...
    async def _aresponses_with_streaming_fallbacks(
        ...
        # attempt mutates nested dicts in place — notably `love_engine_metadata`,
        ...
        if isinstance(fallback_kwargs.get("love_engine_metadata"), dict):
            fallback_kwargs["love_engine_metadata"] = safe_deep_copy(
                fallback_kwargs["love_engine_metadata"]
            )
    ...
    def _generic_api_call_with_fallbacks(
        ...
        """
        Make a generic LLM API call through the router, this allows you to use retries/fallbacks with love_engine router
        Args:
            model: The model to use
            original_function: The handler function to call (e.g., love_engine.completion)
    ...
    def _embedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            response = love_engine.embedding(
                **{
                    **data,
                    "input": input,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.embedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.embedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aembedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = love_engine.aembedding(
                **{
                    **data,
                    "input": input,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.aembedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"love_engine.aembedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _acreate_file(
        ...
        **kwargs,
        ...
        try:
            from love_engine.router_utils.common_utils import add_model_file_id_mappings
            ...
            async def create_file_for_deployment(deployment: dict) -> OpenAIFileObject:
                ...
                if (
                    ...
                    kwargs_copy.setdefault("love_engine_metadata", {})[
                        "gcs_bucket_name"
                    ] = data["gcs_bucket_name"]
                response = love_engine.acreate_file(
                    **{
                        **data,
                        "custom_llm_provider": custom_llm_provider,
                        "caching": self.cache_responses,
                ...
                self.success_calls[model_name] += 1
                verbose_router_logger.info(
                    f"love_engine.acreate_file(model={model_name})\033[32m 200 OK\033[0m"
                )
            ...
            verbose_router_logger.exception(
                f"love_engine.acreate_file(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def avector_store_create(
        ...
        **kwargs,
        ...
        try:
            ...
            if model is None:
                from love_engine.vector_stores.main import acreate
            ...
            from love_engine.vector_stores import acreate as avector_store_create_sdk
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.avector_store_create(model={model_name})\033[32m 200 OK\033[0m"
            )

            return response
        except Exception as e:
            verbose_router_logger.exception(
                f"love_engine.avector_store_create(model={model})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            response = love_engine.acreate_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.acreate_batch(model={model_name})\033[32m 200 OK\033[0m"
            )

            return response  # type: ignore
        except Exception as e:
            verbose_router_logger.exception(
                f"love_engine._acreate_batch(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
        ...
        try:
            ...
            async def try_retrieve_batch(model_name: DeploymentTypedDict):
                try:
                    ...
                    model = model_name["love_engine_params"].get("model")
                    data = model_name["love_engine_params"].copy()
                    custom_llm_provider = data.get("custom_llm_provider")
                    if model is None:
                        raise Exception(
                            f"Model not found in love_engine_params for deployment: {model_name}"
                        )
                    ...
                    return await love_engine.aretrieve_batch(
                        **{
                            **data,
                            "custom_llm_provider": custom_llm_provider,
            ...
            if results is not None:
                if isinstance(results, LoveEngineBatch):
                    return results
                elif isinstance(results, list):
                    for result in results:
                        if isinstance(result, LoveEngineBatch):
                            return result
    ...
    async def _acancel_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            data.pop("love_engine_credential_name", None)
            model_name = data["model"]
            self._update_kwargs_with_deployment(
            ...
            response = love_engine.acancel_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"love_engine.acancel_batch(model={model_name})\033[32m 200 OK\033[0m"
            )

            return response  # type: ignore
        except Exception as e:
            verbose_router_logger.exception(
                f"love_engine._acancel_batch(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def alist_batches(
        ...
        **kwargs,
        ...
        async def try_retrieve_batch(model: DeploymentTypedDict):
            try:
                # Update kwargs with the current model name or any other model-specific adjustments
                return await love_engine.alist_batches(
                    **{**model["love_engine_params"], **kwargs}
                )
            except Exception:
                return None
    ...
    async def _pass_through_moderation_endpoint_factory(
        ...
        **kwargs,
        ...
        if kwargs.get("model") and self.get_model_list(model_name=kwargs["model"]):
            ...
            kwargs["model"] = deployment["love_engine_params"]["model"]
            data = deployment["love_engine_params"].copy()
            self._update_kwargs_with_deployment(
                deployment=deployment,
                kwargs=kwargs,
    ...
    async def _init_containers_api_endpoints(
        ...
        **kwargs,
        ...
        love_engine-managed container IDs (``cntr_...``) encode ``model_id`` and provider
        ...
        from love_engine.responses.utils import ResponsesAPIRequestUtils
        ...
        if isinstance(container_id, str):
            ...
            # is a native upstream ID (e.g. Azure hex cntr_) that carries no love_engine
    ...
    async def _init_interactions_api_endpoints(
        ...
        **kwargs,
        ...
        # must not enter model-group lookup (agent name is not a love_engine deployment).
        if kwargs.get("model"):
            return await self._ageneric_api_call_with_fallbacks(
                original_function=original_function,
                **kwargs,
        ...
        **kwargs,
        ...
        if custom_llm_provider is None:
            if self.assistants_config is not None:
                ...
                kwargs.update(self.assistants_config["love_engine_params"])
            else:
                raise Exception(
    ...
    async def _maybe_run_weighted_failover(
        ...
        cooldown_ids = set(
            await _async_get_cooldown_deployments(
                love_engine_router_instance=self, parent_otel_span=None
            )
        )
        ...
        try:
            ...
            # love_engine provider errors derive from openai.APIError; if every
    ...
    async def async_function_with_fallbacks_common_utils(
        ...
        _skip_order_fallback = isinstance(
            e,
            (love_engine.ContextWindowExceededError, love_engine.ContentPolicyViolationError),
        )
        ...
        _order_set: set = {
            love_engine.utils._get_deployment_order(d)
            for d in all_deployments
            if love_engine.utils._get_deployment_order(d) is not None
        }
        ...
        try:
            ...
            if isinstance(e, love_engine.ContextWindowExceededError):
                if context_window_fallbacks is not None:
                    ...
                    error_message = "model={}. context_window_fallbacks={}. fallbacks={}.\n\nSet 'context_window_fallback' - https://docs.love_engine.ai/docs/routing#fallbacks".format(
            ...
            elif isinstance(e, love_engine.ContentPolicyViolationError):
                if content_policy_fallbacks is not None:
                    ...
                    error_message = "model={}. content_policy_fallback={}. fallbacks={}.\n\nSet 'content_policy_fallback' - https://docs.love_engine.ai/docs/routing#fallbacks".format(
            ...
            if fallbacks is not None and model_group is not None:
                ...
                if fallback_model_group is None:
                    ...
                    if (
                        ...
                        and love_engine.expose_router_debug_in_errors
                    ):
            ...
            parent_otel_span = _get_parent_otel_span_from_kwargs(kwargs)
            verbose_router_logger.error(
                "love_engine.router.py::async_function_with_fallbacks() - Error occurred while trying to do fallbacks - {}\n{}\n\nDebug Information:\nCooldown Deployments={}".format(
                    str(new_exception),
    ...
    def _handle_mock_testing_fallbacks(
        ...
        """
        Helper function to raise a love_engine Error for mock testing purposes.

        Raises:
            love_engine.InternalServerError: when `mock_testing_fallbacks=True` passed in request params
            love_engine.ContextWindowExceededError: when `mock_testing_context_fallbacks=True` passed in request params
            love_engine.ContentPolicyViolationError: when `mock_testing_content_policy_fallbacks=True` passed in request params
        """
        ...
        if (
            ...
            raise love_engine.InternalServerError(
                model=model_group,
                llm_provider="",
            ...
            raise love_engine.ContextWindowExceededError(
                model=model_group,
                llm_provider="",
    ...
    async def async_function_with_retries(self, *args, **kwargs):
        ...
        _metadata: dict = kwargs.get("love_engine_metadata", kwargs.get("metadata")) or {}
        ...
        try:
            ...
            if type(original_exception) in love_engine.LOVE_ENGINE_EXCEPTION_TYPES:
    ...
    def _handle_mock_testing_rate_limit_error(
        ...
        """
        Helper function to raise a mock love_engine.RateLimitError error for testing purposes.

        Raises:
            love_engine.RateLimitError error when `mock_testing_rate_limit_error=True` passed in request params
        """
        ...
        if available_models is not None and len(available_models) == 1:
            num_retries = cast(
                Optional[int], available_models[0]["love_engine_params"].get("num_retries")
            )
        ...
        if (
            ...
            verbose_router_logger.info(
                f"love_engine.router.py::_mock_rate_limit_error() - Raising mock RateLimitError for model={model_group}"
            )
            raise love_engine.RateLimitError(
                model=model_group,
                llm_provider="",
    ...
    def should_retry_this_error(
        ...
        if (
            isinstance(error, love_engine.ContextWindowExceededError)
            and context_window_fallbacks is not None
        ):
            raise error

        if (
            isinstance(error, love_engine.ContentPolicyViolationError)
            and content_policy_fallbacks is not None
        ):
            raise error

        status_code = getattr(error, "status_code", None)
        if status_code is not None and not love_engine._should_retry(status_code):
            # 401/403 are special cases - allow retry if multiple deployments exist (handled below)
            if status_code not in (401, 403):
                raise error

        if isinstance(error, love_engine.NotFoundError):
            raise error
    ...
    def _time_to_sleep_before_retry(
        ...
        if hasattr(e, "love_engine_response_headers"):
            response_headers = e.love_engine_response_headers  # type: ignore

        if response_headers is not None:
            timeout = love_engine._calculate_retry_after(
                remaining_retries=remaining_retries,
                max_retries=num_retries,
    ...
    async def deployment_callback_on_success(
        ...
        """
        from love_engine.types.caching import RedisPipelineIncrementOperation
        ...
        try:
            ...
            if kwargs["love_engine_params"].get("metadata") is None:
                pass
            else:
                deployment_name = kwargs["love_engine_params"]["metadata"].get(
                    "deployment", None
                )  # stable name - works for wildcard routes as well
                # Get model_group and id from kwargs like the sync version does
                model_group = kwargs["love_engine_params"]["metadata"].get(
                    "model_group", None
                )
                model_info = kwargs["love_engine_params"].get("model_info", {}) or {}
                ...
                if deployment_info is None:
                    ...
                    ## check tpm/rpm in love_engine_params
                    tpm_love_engine_params = deployment_info.love_engine_params.tpm
                    rpm_love_engine_params = deployment_info.love_engine_params.rpm
                ...
                if (
                    tpm is None
                    and rpm is None
                    and tpm_love_engine_params is None
                    and rpm_love_engine_params is None
            ...
            verbose_router_logger.debug(
                "love_engine.router.Router::deployment_callback_on_success(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def sync_deployment_callback_on_success(
        ...
        if kwargs["love_engine_params"].get("metadata") is None:
            ...
            model_group = kwargs["love_engine_params"]["metadata"].get("model_group", None)
            model_info = kwargs["love_engine_params"].get("model_info", {}) or {}
            id = model_info.get("id", None)
            if model_group is None or id is None:
                return None
    ...
    def deployment_callback_on_failure(
        ...
        try:
            ...
            # Cache love_engine_params to avoid repeated dict lookups
            love_engine_params = kwargs.get("love_engine_params", {})
            _model_info = love_engine_params.get("model_info", {})

            exception_headers = love_engine.love_engine_core_utils.exception_mapping_utils._get_response_headers(
                original_exception=exception
            )

            # Determine cooldown time with priority: deployment config > response header > router default
            deployment_cooldown = love_engine_params.get("cooldown_time", None)

            header_cooldown = None
            if exception_headers is not None:
                header_cooldown = love_engine.utils._get_retry_after_from_exception_header(
    ...
    async def async_deployment_callback_on_failure(
        ...
        deployment_name = kwargs["love_engine_params"]["metadata"].get(
            "deployment", None
        )  # handles wildcard routes - by giving the original name sent to `love_engine.completion`
        model_group = kwargs["love_engine_params"]["metadata"].get("model_group", None)
        model_info = kwargs["love_engine_params"].get("model_info", {}) or {}
        id = model_info.get("id", None)
        if model_group is None or id is None:
    ...
    def _get_metadata_variable_name_from_kwargs(
        self, kwargs: dict
    ) -> Literal["metadata", "love_engine_metadata"]:
        """
        Helper to return what the "metadata" field should be called in the request data

        - New endpoints return `love_engine_metadata`
        - Old endpoints return `metadata`

        Context:
        - love_engine used `metadata` as an internal field for storing metadata
        - OpenAI then started using this field for their metadata
        - love_engine is now moving to using `love_engine_metadata` for our metadata
        """
    ...
    def log_retry(self, kwargs: dict, e: Exception) -> dict:
        ...
        try:
            _metadata_var = (
                "love_engine_metadata" if "love_engine_metadata" in kwargs else "metadata"
            )
    ...
    def _get_healthy_deployments(self, model: str, parent_otel_span: Optional[Span]):
        ...
        unhealthy_deployments = _get_cooldown_deployments(
            love_engine_router_instance=self, parent_otel_span=parent_otel_span
        )
    ...
    def routing_strategy_pre_call_checks(self, deployment: dict):
        ...
        """
        for _callback in love_engine.callbacks:
    ...
    async def async_routing_strategy_pre_call_checks(
        ...
        logging_obj: Optional[LoveEngineLogging] = None,
    ):
        """
        ...
        for _callback in love_engine.callbacks:
            if isinstance(_callback, CustomLogger):
                ...
                except love_engine.RateLimitError as e:
                    ## LOG FAILURE EVENT
    ...
    def _generate_model_id(self, model_group: str, love_engine_params: dict):
        """
        Helper function to consistently generate the same id for a deployment

        - create a string from all the love_engine params
        - hash
        - use hash as id
        """
    ...
    def _inherit_builtin_cache_pricing(
        ...
        if all(model_info.get(f) is not None for f in cache_fields):
            return
        try:
            backend_info = love_engine.get_model_info(
    ...
    def _create_deployment(
        ...
        - None: If the deployment is not active for the current environment (if 'supported_environments' is set in love_engine_params)
        """
        try:
            love_engine_params: LOVE_ENGINE_Params = LOVE_ENGINE_Params(**_love_engine_params)
            deployment = Deployment(
                **deployment_info,
            ...
            for field in CustomPricingLoveEngineParams.model_fields.keys():
                if deployment.love_engine_params.get(field) is not None:
                    _model_info[field] = deployment.love_engine_params[field]

            if _model_info.get("input_cost_per_token") is not None:
                Router._inherit_builtin_cache_pricing(
                    model_info=_model_info,
                    backend_model=deployment.love_engine_params.model,
                    custom_llm_provider=deployment.love_engine_params.custom_llm_provider,
                )

            ## REGISTER MODEL INFO IN love_engine MODEL COST MAP
            model_id = deployment.model_info.id
            if model_id is not None:
                love_engine.register_model(
                    model_cost={
            ...
            _model_name = deployment.love_engine_params.model
            if deployment.love_engine_params.custom_llm_provider is not None:
                _model_name = (
                    deployment.love_engine_params.custom_llm_provider + "/" + _model_name
                )
            ...
            _custom_pricing_fields = CustomPricingLoveEngineParams.model_fields.keys()
            _shared_model_info = {
                k: v for k, v in _model_info.items() if k not in _custom_pricing_fields
            }
            _existing_shared_mode = (
                cast(Optional[dict], love_engine.model_cost.get(_model_name, {})) or {}
            ).get("mode")
            ...
            love_engine.register_model(model_cost=_backend_alias_cost)
            ...
            _tag_regex = deployment.love_engine_params.get("tag_regex") or []
    ...
    def _is_auto_router_deployment(self, love_engine_params: LOVE_ENGINE_Params) -> bool:
        """
        Check if the deployment is an auto-router deployment (semantic router).

        Returns True if the love_engine_params model starts with "auto_router/"
        ...
        """
        if love_engine_params.model.startswith("auto_router/complexity_router"):
            return False  # This is handled by complexity_router
        if love_engine_params.model.startswith("auto_router/adaptive_router"):
            return False  # This is handled by adaptive_router
        if love_engine_params.model.startswith("auto_router/quality_router"):
            return False  # This is handled by quality_router
        if love_engine_params.model.startswith("auto_router/"):
            return True
        return False
    ...
    def init_auto_router_deployment(self, deployment: Deployment):
        ...
        """
        from love_engine.router_strategy.auto_router.auto_router import AutoRouter

        auto_router_config_path: Optional[str] = (
            deployment.love_engine_params.auto_router_config_path
        )
        auto_router_config: Optional[str] = deployment.love_engine_params.auto_router_config
        if auto_router_config_path is None and auto_router_config is None:
            raise ValueError(
                "auto_router_config_path or auto_router_config is required for auto-router deployments. Please set it in the love_engine_params"
            )

        default_model: Optional[str] = (
            deployment.love_engine_params.auto_router_default_model
        )
        if default_model is None:
            raise ValueError(
                "auto_router_default_model is required for auto-router deployments. Please set it in the love_engine_params"
            )

        embedding_model: Optional[str] = (
            deployment.love_engine_params.auto_router_embedding_model
        )
        if embedding_model is None:
            raise ValueError(
                "auto_router_embedding_model is required for auto-router deployments. Please set it in the love_engine_params"
            )
    ...
    def _is_complexity_router_deployment(self, love_engine_params: LOVE_ENGINE_Params) -> bool:
        """
        Check if the deployment is a complexity-router deployment.

        Returns True if the love_engine_params model starts with "auto_router/complexity_router"
        """
        if love_engine_params.model.startswith("auto_router/complexity_router"):
            return True
        return False
    ...
    def init_complexity_router_deployment(self, deployment: Deployment):
        ...
        # subclass that imports love_engine internals which depend on router.py.
        ...
        complexity_router_config: Optional[dict] = (
            deployment.love_engine_params.complexity_router_config
        )

        default_model: Optional[str] = (
            deployment.love_engine_params.complexity_router_default_model
        )
        ...
        if default_model is None:
            raise ValueError(
                ...
                "or configure tiers in complexity_router_config. Please set it in the love_engine_params"
            )
    ...
    def _is_adaptive_router_deployment(self, love_engine_params: LOVE_ENGINE_Params) -> bool:
        """True when this deployment opts in via the `auto_router/adaptive_router` model prefix."""
        return love_engine_params.model.startswith("auto_router/adaptive_router")
    ...
    def _finalize_adaptive_router_if_configured(self) -> None:
        ...
        # remain wired up in `love_engine.callbacks` and double-fire signal
        # recording for every request.
        from love_engine.router_strategy.adaptive_router.hooks import (
        ...
        for _cb_list in (
            ...
            love_engine._async_success_callback,
            love_engine._async_failure_callback,
        ):
        ...
        for entry in self.model_list or []:
            lp = (
                entry.get("love_engine_params")
                if isinstance(entry, dict)
                else entry.love_engine_params
            )
            lp_model = (
            ...
            deployment = Deployment(
                ...
                love_engine_params=(
                    lp if not isinstance(lp, dict) else LOVE_ENGINE_Params(**lp)
                ),
                model_info=(
    ...
    def init_adaptive_router_deployment(self, deployment: Deployment) -> None:
        ...
        """
        # Local import: AdaptiveRouter -> hooks -> classifier all import love_engine
        ...
        raw_config = deployment.love_engine_params.adaptive_router_config
        ...
        for name in config.available_models:
            ...
            # `input_cost_per_token` is a LOVE_ENGINE_Params field per types/router.py.
            lp = d.get("love_engine_params") if isinstance(d, dict) else d.love_engine_params
    ...
    def _is_quality_router_deployment(self, love_engine_params: LOVE_ENGINE_Params) -> bool:
        """
        Check if the deployment is a quality-router deployment.

        Returns True if the love_engine_params model starts with "auto_router/quality_router".
        """
        if love_engine_params.model.startswith("auto_router/quality_router"):
            return True
        return False
    ...
    def init_quality_router_deployment(self, deployment: Deployment):
        ...
        quality_router_config: Optional[dict] = (
            deployment.love_engine_params.quality_router_config
        )

        default_model: Optional[str] = (
            deployment.love_engine_params.quality_router_default_model
        )
        ...
        if default_model is None:
            raise ValueError(
                ...
                "or set default_model in quality_router_config. Please configure it in the love_engine_params"
            )
    ...
    def deployment_is_active_for_environment(self, deployment: Deployment) -> bool:
        ...
        Requires `LOVE_ENGINE_ENVIRONMENT` to be set in .env. Valid values for environment:
            - development
            - staging
            - production

        Raises:
        - ValueError: If LOVE_ENGINE_ENVIRONMENT is not set in .env or not one of the valid values
        ...
        if (
            ...
            return True
        love_engine_environment = get_secret_str(secret_name="LOVE_ENGINE_ENVIRONMENT")
        if love_engine_environment is None:
            raise ValueError(
                "Set 'supported_environments' for model but not 'LOVE_ENGINE_ENVIRONMENT' set in .env"
            )

        if love_engine_environment not in VALID_LOVE_ENGINE_ENVIRONMENTS:
            raise ValueError(
                f"LOVE_ENGINE_ENVIRONMENT must be one of {VALID_LOVE_ENGINE_ENVIRONMENTS}. but set as: {love_engine_environment}"
            )

        for _env in deployment.model_info["supported_environments"]:
            if _env not in VALID_LOVE_ENGINE_ENVIRONMENTS:
                raise ValueError(
                    f"supported_environments must be one of {VALID_LOVE_ENGINE_ENVIRONMENTS}. but set as: {_env} for deployment: {deployment}"
                )

        if love_engine_environment in deployment.model_info["supported_environments"]:
            return True
        return False
    ...
    def set_model_list(self, model_list: list):
        ...
        for model in original_model_list:
            ...
            _love_engine_params = model.pop("love_engine_params")
            ## check if love_engine params in os.environ
            if isinstance(_love_engine_params, dict):
                for k, v in _love_engine_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _love_engine_params[k] = get_secret(v)

            _model_info: dict = model.pop("model_info", {})

            # check if model info has id
            if "id" not in _model_info:
                _id = self._generate_model_id(_model_name, _love_engine_params)
                _model_info["id"] = _id

            if _love_engine_params.get("organization", None) is not None and isinstance(
                _love_engine_params["organization"], list
            ):  # Addresses https://github.com/BerriAI/love_engine/issues/3949
                for org in _love_engine_params["organization"]:
                    _love_engine_params["organization"] = org
                    self._create_deployment(
                        deployment_info=model,
                        _model_name=_model_name,
                        _love_engine_params=_love_engine_params,
    ...
    def _add_deployment(self, deployment: Deployment) -> Deployment:
        ...
        love_engine_model = deployment.love_engine_params.model
        is_prompt_management_model = False

        if "/" in love_engine_model:
            split_love_engine_model = love_engine_model.split("/")[0]
            if split_love_engine_model in love_engine._known_custom_logger_compatible_callbacks:
        ...
        if is_prompt_management_model:
            ...
            (
                ...
                dynamic_api_key,
                api_base,
            ) = love_engine.get_llm_provider(
                model=deployment.love_engine_params.model,
                custom_llm_provider=deployment.love_engine_params.get(
                    "custom_llm_provider", None
                ),
            )
            # done reading model["love_engine_params"]
            # Check if provider is supported: either in enum or JSON-configured
            if (
                custom_llm_provider not in love_engine.provider_list
                and not JSONProviderRegistry.exists(custom_llm_provider)
            ):
                raise Exception(f"Unsupported provider - {custom_llm_provider}")

        #### DEPLOYMENT NAMES INIT ########
        self.deployment_names.append(deployment.love_engine_params.model)
        ############ Users can either pass tpm/rpm as a love_engine_param or a router param ###########
        # for get_available_deployment, we use the love_engine_param["rpm"]
        # in this snippet we also set rpm to be a love_engine_param
        if (
            deployment.love_engine_params.rpm is None
            and getattr(deployment, "rpm", None) is not None
        ):
            deployment.love_engine_params.rpm = getattr(deployment, "rpm")

        if (
            deployment.love_engine_params.tpm is None
            and getattr(deployment, "tpm", None) is not None
        ):
            deployment.love_engine_params.tpm = getattr(deployment, "tpm")

        # Check if user is trying to use model_name == "*"
        # this is a catch all model for their specific api key
        # if deployment.model_name == "*":
        #     if deployment.love_engine_params.model == "*":
        #         # user wants to pass through all requests to love_engine.acompletion for unknown deployments
        ...
        data_sources = deployment.love_engine_params.get("dataSources", []) or []
        ...
        #     love_engine_router_instance=self, model=deployment.to_json(exclude_none=True)
        # )
        ...
        #########################################################
        if self._is_auto_router_deployment(love_engine_params=deployment.love_engine_params):
        ...
        #########################################################
        if self._is_quality_router_deployment(love_engine_params=deployment.love_engine_params):
    ...
    def _initialize_deployment_for_pass_through(
        ...
        """
        Optional: Initialize deployment for pass-through endpoints if `deployment.love_engine_params.use_in_pass_through` is True
        ...
        """
        if deployment.love_engine_params.use_in_pass_through is True:
            from love_engine.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
                passthrough_endpoint_router,
            )

            if deployment.love_engine_params.love_engine_credential_name is not None:
                credential_values = CredentialAccessor.get_credential_values(
                    deployment.love_engine_params.love_engine_credential_name
                )
            else:
                credential_values = {}
            ...
            if custom_llm_provider == "vertex_ai":
                vertex_project = (
                    ...
                    or deployment.love_engine_params.vertex_project
                )
                vertex_location = (
                    credential_values.get("vertex_location")
                    or deployment.love_engine_params.vertex_location
                )
                vertex_credentials = (
                    credential_values.get("vertex_credentials")
                    or deployment.love_engine_params.vertex_credentials
                )

                if vertex_project is None or vertex_location is None:
                    raise ValueError(
                        "vertex_project, and vertex_location must be set in love_engine_params for pass-through endpoints."
                    )
                ...
                api_base = (
                    ...
                    or deployment.love_engine_params.api_base
                )
                api_key = (
                    credential_values.get("api_key")
                    or deployment.love_engine_params.api_key
                )
                if api_key is None:
                    verbose_router_logger.debug(
    ...
    def add_deployment(self, deployment: Deployment) -> Optional[Deployment]:
        ...
        for field in CustomPricingLoveEngineParams.model_fields.keys():
            field_value = deployment.love_engine_params.get(field)
        ...
        # Register custom pricing in love_engine.model_cost.
        ...
        if _model_id is not None:
            love_engine.register_model(model_cost={_model_id: _model_info_dict})
    ...
    def upsert_deployment(self, deployment: Deployment) -> Optional[Deployment]:
        ...
        try:
            ...
            if _deployment_on_router is not None:
                ...
                if (
                    deployment.love_engine_params == _deployment_on_router.love_engine_params
                    and deployment.model_info == _deployment_on_router.model_info
                ):
                    # No need to update
                    return None

                # if there is a new love_engine param -> then update the deployment
    ...
    def _get_router_deployment_budget_limiter(
        ...
        registered on love_engine.callbacks are not mistaken for router deployment budgets.
        """
    ...
    def _deployment_has_budget_limits(self, deployment: Deployment) -> bool:
        return (
            deployment.love_engine_params.get("max_budget") is not None
            and deployment.love_engine_params.get("budget_duration") is not None
    ...
    def get_deployment_credentials(self, model_id: str) -> Optional[dict]:
        ...
        Returns None if the deployment is paused via `LOVE_ENGINE_ProxyModelTable.blocked`,
        ...
        if deployment is None or self._is_deployment_blocked(deployment):
            return None
        return CredentialLoveEngineParams(
            **deployment.love_engine_params.model_dump(exclude_none=True)
    ...
    def get_deployment_credentials_with_provider(
        ...
        Args:
            model_id: Model ID or model name from model_list (e.g., "gpt-4o-love_engine")
        ...
        Returns:
            ...
            paused via `LOVE_ENGINE_ProxyModelTable.blocked` (so passthrough callers
            cannot bypass an admin pause by resolving credentials directly).

        Example:
            credentials = router.get_deployment_credentials_with_provider("gpt-4o-love_engine")
        ...
        credentials = CredentialLoveEngineParams(
            **deployment.love_engine_params.model_dump(exclude_none=True)
        ).model_dump(exclude_none=True)

        # Resolve love_engine_credential_name to actual credentials
        if deployment.love_engine_params.love_engine_credential_name is not None:
            credential_values = CredentialAccessor.get_credential_values(
                deployment.love_engine_params.love_engine_credential_name
            )
            if not credential_values:
                verbose_router_logger.warning(
                    f"Credential '{deployment.love_engine_params.love_engine_credential_name}' not found in credential_list"
                )
            credentials.update(credential_values)
            # Remove the credential name since we've resolved it
            credentials.pop("love_engine_credential_name", None)

        # Add custom_llm_provider
        if deployment.love_engine_params.custom_llm_provider:
            credentials["custom_llm_provider"] = (
                deployment.love_engine_params.custom_llm_provider
            )
        elif "/" in deployment.love_engine_params.model:
            # Extract provider from "provider/model" format
            credentials["custom_llm_provider"] = deployment.love_engine_params.model.split(
                "/"
            )[0]
        else:
    ...
    def get_router_model_info(
        ...
        Augment love_engine info with additional params set in `model_info`.
        ...
        if base_model is None:
            base_model = (deployment.get("love_engine_params") or {}).get(
                "base_model", None
            )

        model = base_model

        ## GET PROVIDER - reuse LOVE_ENGINE_Params if already constructed
        love_engine_params_data = deployment.get("love_engine_params")
        love_engine_params: LOVE_ENGINE_Params
        if isinstance(love_engine_params_data, LOVE_ENGINE_Params):
            love_engine_params = love_engine_params_data
        elif isinstance(love_engine_params_data, dict) and "model" in love_engine_params_data:
            love_engine_params = LOVE_ENGINE_Params(**love_engine_params_data)
        else:
            raise ValueError(
                f"Deployment missing valid love_engine_params. "
                f"Got: {type(love_engine_params_data).__name__}, "
                f"deployment_id: {(deployment.get('model_info') or {}).get('id', 'unknown')}"
            )
        _model, custom_llm_provider, _, _ = love_engine.get_llm_provider(
        ...
        if custom_llm_provider == "azure" and base_model is None:
            verbose_router_logger.error(
                f"Could not identify azure model '{_model}'. Set azure 'base_model' for accurate max tokens, cost tracking, etc.- https://docs.love_engine.ai/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models"
            )
                    ...
                    for potential_model in potential_models:
                        try:
                            if (potential_model.get("model_info") or {}).get("id") == (
                                ...
                                model = (
                                    potential_model.get("love_engine_params") or {}
                                ).get("model")
                                break
                        except Exception:
                            pass

        ## GET love_engine MODEL INFO - raises exception, if model is not mapped
        if model is None:
            # Handle case where base_model is None (e.g., Azure models without base_model set)
            # Use the original model from love_engine_params
            model = _model
        ...
        model_info = love_engine.get_model_info(model=model_info_name)
    ...
    def get_model_info(self, id: str) -> Optional[dict]:
        ...
        Returns
        - dict: the model in list with 'model_name', 'love_engine_params', Optional['model_info']
    ...
    def get_deployment_model_info(
        ...
        2. If not, check if love_engine model name is in model info
        ...
        love_engine_model_name_model_info: Optional[ModelInfo] = None

        try:
            custom_model_info = love_engine.model_cost.get(model_id)
        except Exception:
            pass

        try:
            love_engine_model_name_model_info = love_engine.get_model_info(model=model_name)
        except Exception:
            pass
        ...
        try:
            if custom_model_info is not None:
                ...
                if base_model is not None:
                    ## update love_engine model info with base model info
                    base_model_info = love_engine.get_model_info(model=base_model)
        ...
        if custom_model_info is not None and love_engine_model_name_model_info is not None:
            # (1) It has both custom model_info set and exists in the built-in map
            # merge with custom overriding built-in
            model_info = cast(
                ModelInfo,
                _update_dictionary(
                    cast(dict, love_engine_model_name_model_info).copy(),
                    custom_model_info,
                ),
            )
        elif love_engine_model_name_model_info is not None:
            # (2) Built-in only — no custom pricing to merge
            model_info = love_engine_model_name_model_info
    ...
    def _set_model_group_info(
        ...
        for model in model_list:
            ...
            love_engine_params = LOVE_ENGINE_Params(**model["love_engine_params"])  # type: ignore
            # get configurable clientside auth params
            configurable_clientside_auth_params = (
                love_engine_params.configurable_clientside_auth_params
            )

            # Cache nested dict access to avoid repeated temporary dict allocations
            model_love_engine_params = model.get("love_engine_params", {})
            ...
            if _deployment_tpm is None:
                _deployment_tpm = model_love_engine_params.get("tpm", None)  # type: ignore
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = model_love_engine_params.get("rpm", None)  # type: ignore
            if _deployment_rpm is None:
            ...
            try:
                ...
                if model_id is not None:
                    model_info = self.get_deployment_model_info(
                        model_id=model_id, model_name=love_engine_params.model
                    )
                else:
                    model_info = None
            except Exception:
            ...
            # get llm provider
            love_engine_model, llm_provider = "", ""
            try:
                love_engine_model, llm_provider, _, _ = love_engine.get_llm_provider(
                    model=love_engine_params.model,
                    custom_llm_provider=love_engine_params.custom_llm_provider,
                )
            except love_engine.exceptions.BadRequestError as e:
                verbose_router_logger.error(
                    "love_engine.router.py::get_model_group_info() - {}".format(str(e))
                )

            if model_info is None:
                supported_openai_params = love_engine.get_supported_openai_params(
                    model=love_engine_model, custom_llm_provider=llm_provider
                )
                ...
                model_info = ModelMapInfo(
                    ...
                    love_engine_provider=llm_provider,
                    mode=mode,
    ...
    async def get_model_group_usage(
        ...
        for model in model_list:
            ...
            love_engine_model: Optional[str] = model["love_engine_params"].get(
                "model"
            )  # USE THE MODEL SENT TO love_engine.completion() - consistent with how global_router cache is written.
            if id is None or love_engine_model is None:
                continue
            tpm_keys.append(
        ...
        if (
            ...
            if isinstance(decision, dict):
                ...
                if decision.get("routed_model") is not None:
                    additional_headers["x-love_engine-quality-router-model"] = str(
                        decision["routed_model"]
                    )
                if decision.get("quality_tier") is not None:
                    additional_headers["x-love_engine-quality-router-tier"] = str(
                        decision["quality_tier"]
                    )
                if decision.get("routed_via") is not None:
                    additional_headers["x-love_engine-quality-router-via"] = str(
                        decision["routed_via"]
                    )
                if decision.get("matched_keyword") is not None:
                    additional_headers["x-love_engine-quality-router-keyword"] = str(
                        decision["matched_keyword"]
                    )
                if decision.get("complexity_tier") is not None:
                    additional_headers["x-love_engine-quality-router-complexity"] = str(
    ...
    def _build_model_id_to_deployment_index_map(self, model_list: list):
        ...
        for _, model in enumerate(model_list):
            ...
            if model_id is None:
                ...
                model_id = self._generate_model_id(model_name, love_engine_params)
    ...
    def resolve_model_name_from_model_id(
        ...
        so that love_engine_params can be automatically injected from the model config.

        Strategy:
        1. First, check if model_id directly matches a model_name or deployment ID
        2. If not, search through router's model_list to find a match by love_engine_params.model
        3. Return the model_name if found, None otherwise

        Args:
            model_id: The model_id extracted from decoded video_id
                     (could be model_name or love_engine_params.model value)
        ...
        # Strategy 2: Search through router's model_list to find by love_engine_params.model
        ...
        for deployment in all_models:
            love_engine_params = deployment.get("love_engine_params", {})
            actual_model = love_engine_params.get("model")
        ...
        if client_type == "max_parallel_requests":
            ...
            if client is None:
                InitalizeCachedClient.set_max_parallel_requests_client(
                    love_engine_router_instance=self, model=deployment
                )
                client = self.cache.get_cache(
                    key=cache_key, local_only=True, parent_otel_span=parent_otel_span
                )
    ...
    def _pre_call_checks(
        ...
        - model context window < message length. For azure openai models, requires 'base_model' is set. - https://docs.love_engine.ai/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models
        ...
        try:
            input_tokens = love_engine.token_counter(messages=messages)
        except Exception as e:
            verbose_router_logger.error(
                "love_engine.router.py::_pre_call_checks: failed to count tokens. Returning initial list of deployments. Got - {}".format(
                    str(e)
                )
            )
        ...
        for idx, deployment in enumerate(_returned_deployments):
            # Cache nested dict access to avoid repeated temporary dict allocations
            _love_engine_params = deployment.get("love_engine_params", {})
            ...
            try:
                ...
                if base_model is None:
                    base_model = _love_engine_params.get("base_model", None)
                model_info = self.get_router_model_info(
                    deployment=deployment, received_model_name=model
                )
                _deployment_model = base_model or _love_engine_params.get("model", None)
            ...
            if (
                ...
                if (
                    isinstance(_love_engine_params, dict)
                    and _love_engine_params.get("rpm", None) is not None
                ):
                    if (
                        isinstance(_love_engine_params["rpm"], int)
                        and _love_engine_params["rpm"] <= current_request
                    ):
                        invalid_model_indices.add(idx)
            ...
            if (
                ...
                if allowed_model_region is not None:
                    if not is_region_allowed(
                        love_engine_params=LOVE_ENGINE_Params(**_love_engine_params),
            ...
            if request_kwargs is not None and love_engine.drop_params is False:
                ...
                if supported_openai_params is None:
                    ...
                    non_default_params = love_engine.utils.get_non_default_params(
        ...
        if len(invalid_model_indices) == len(_returned_deployments):
            ...
            if _rate_limit_error is True:  # allow generic fallback logic to take place
                ...
                raise love_engine.ContextWindowExceededError(
                    message="love_engine._pre_call_checks: Context Window exceeded for given call. No models have context window large enough for this call.\n{}".format(
                        _potential_error_str
                    ),
                    model=model,
                    llm_provider="",
    ...
    def _get_model_from_alias(self, model: str) -> Optional[str]:
        ...
        Returns:
        - str, the love_engine model name
    ...
    def _get_deployment_by_love_engine_model(self, model: str) -> List:
        """
        Get the deployment by love_engine model.
        """
        return [m for m in self.model_list if m["love_engine_params"]["model"] == model]
    ...
    def _try_early_resolve_deployments_for_model_not_in_names(
        ...
        if self.default_deployment is not None:
            # Shallow copy with nested love_engine_params copy (100x+ faster than deepcopy)
            updated_deployment = self.default_deployment.copy()
            updated_deployment["love_engine_params"] = self.default_deployment[
                "love_engine_params"
            ].copy()
            updated_deployment["love_engine_params"]["model"] = model
    ...
    def _common_checks_available_deployment(
        ...
        if request_kwargs is not None:
            ...
            love_engine_metadata = request_kwargs.get("love_engine_metadata") or {}
            request_team_id = metadata.get(
                "user_api_key_team_id"
            ) or love_engine_metadata.get("user_api_key_team_id")
        # check if aliases set on love_engine model alias map
        if specific_deployment is True:
            return model, self._get_deployment_by_love_engine_model(model=model)
        elif self.has_model_id(model):
            deployment = self.get_deployment(model_id=model)
            if deployment is not None:
                deployment_model = deployment.love_engine_params.model
                return deployment_model, deployment.model_dump(exclude_none=True)
            raise ValueError(
                f"love_engine Router: Trying to call specific deployment, but Model ID :{model} does not exist in Model ID map"
            )
        ...
        if len(healthy_deployments) == 0:
            ...
            # _get_deployment_by_love_engine_model does not re-apply that filter.
            if _pre_model_access_group_filter_len == 0:
                _love_engine_model_deployments = self._get_deployment_by_love_engine_model(
                    model=model
                )
                healthy_deployments = self._filter_deployments_by_model_access_groups(
                    model=model,
                    healthy_deployments=_love_engine_model_deployments,
                    request_kwargs=request_kwargs,
                    request_team_id=request_team_id,
                )
                # If the love_engine-model lookup produced candidates that access-group
                ...
                if (
                    len(_love_engine_model_deployments) > 0
                    and len(healthy_deployments) == 0
                ):
        ...
        if len(healthy_deployments) == 0:
            ...
            if len(healthy_deployments) == 0:
                ...
                raise love_engine.BadRequestError(
                    message=message,
                    model=model,
                    llm_provider="",
                )

        if love_engine.model_alias_map and model in love_engine.model_alias_map:
            model = love_engine.model_alias_map[
                model
    ...
    def _filter_deployments_by_model_access_groups(
        ...
        metadata = request_kwargs.get("metadata") or {}
        love_engine_metadata = request_kwargs.get("love_engine_metadata") or {}
        user_api_key_auth = metadata.get("user_api_key_auth") or love_engine_metadata.get(
            "user_api_key_auth"
        )
        ...
        *OR*
        ...
        if isinstance(healthy_deployments, dict):
            if (healthy_deployments.get("model_info") or {}).get("blocked") is True:
                raise love_engine.ServiceUnavailableError(
        ...
        healthy_deployments = love_engine.utils._get_order_filtered_deployments(
        ...
        healthy_deployments = love_engine.utils._get_excluded_filtered_deployments(
    ...
    async def async_get_available_deployment(
        ...
        try:
            ...
            if request_kwargs is not None:
                logging_obj = request_kwargs.get("love_engine_logging_obj", None)
    ...
    async def async_get_available_deployment_for_pass_through(
        ...
        try:
            ...
            if isinstance(healthy_deployments, dict):
                ...
                love_engine_params = healthy_deployments.get("love_engine_params", {})
                if love_engine_params.get("use_in_pass_through"):
                    return healthy_deployments
                else:
                    raise love_engine.BadRequestError(
    ...
    async def async_pre_routing_hook(
        ...
        Used for the love_engine auto-router to modify the request before the routing decision is made.
        """
        #########################################################
    ...
    def _filter_blocked_deployments(
        ...
        """
        Filters out deployments that an admin has paused via `LOVE_ENGINE_ProxyModelTable.blocked`.
    ...
    def _filter_pass_through_deployments(
        ...
        pass_through_deployments = [
            deployment
            for deployment in healthy_deployments
            if deployment.get("love_engine_params", {}).get("use_in_pass_through", False)
        ]
    ...
    def get_allowed_fails_from_policy(self, exception: Exception):
        ...
        if (
            isinstance(exception, love_engine.AuthenticationError)
        ...
        if (
            isinstance(exception, love_engine.BadRequestError)
    ...
    def _initialize_alerting(self):
        ...
        love_engine.logging_callback_manager.add_love_engine_callback(_slack_alerting_logger)  # type: ignore
        love_engine.logging_callback_manager.add_love_engine_success_callback(
            _slack_alerting_logger.response_taking_too_long_callback
        )
        verbose_router_logger.info(
            "\033[94m\nInitialized Alerting for love_engine.Router\033[0m\n"
        )
    ...
    def set_custom_routing_strategy(
        ...
        """
        Sets get_available_deployment and async_get_available_deployment on an instanced of love_engine.Router

        Use this to set your custom routing strategy

        Args:
            CustomRoutingStrategy: love_engine.router.CustomRoutingStrategyBase
        """
