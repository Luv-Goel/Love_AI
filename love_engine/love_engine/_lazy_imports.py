...
This module implements lazy loading for love_engine attributes. Instead of importing
everything when the module loads, we only import things when they're actually used.

How it works:
1. When someone accesses `love_engine.some_attribute`, Python calls __getattr__ in __init__.py
...
This makes importing love_engine much faster because we don't load heavy dependencies
...
def _get_love_engine_globals() -> dict:
    """
    Get the globals dictionary of the love_engine module.

    This is where we cache imported attributes so we don't import them twice.
    When you do `love_engine.some_function`, it gets stored in this dictionary.
    """
    return sys.modules["love_engine"].__dict__
...
def _get_utils_globals() -> dict:
    ...
    This is where we cache imported attributes so we don't import them twice.
    When you do `love_engine.utils.some_function`, it gets stored in this dictionary.
    """
    return sys.modules["love_engine.utils"].__dict__
...
def _get_default_encoding() -> Any:
    ...
    This avoids importing `love_engine.love_engine_core_utils.default_encoding` (and thus tiktoken)
    at `love_engine` import time. The encoding is cached after the first import.
    ...
    if _default_encoding is None:
        from love_engine.love_engine_core_utils.default_encoding import encoding
...
def _get_modified_max_tokens() -> Any:
    ...
    This avoids importing `love_engine.love_engine_core_utils.token_counter` at `love_engine` import time.
    ...
    if _get_modified_max_tokens_func is None:
        from love_engine.love_engine_core_utils.token_counter import (
...
def _get_lazy_import_registry() -> dict[str, Callable[[str], Any]]:
    ...
    if _LAZY_IMPORT_REGISTRY is None:
        ...
        for name in LOVE_ENGINE_LOGGING_NAMES:
            _LAZY_IMPORT_REGISTRY[name] = _lazy_import_love_engine_logging
...
def _generic_lazy_import(
    ...
    # If module_path starts with ".", it's a relative import (needs package="love_engine")
    # Otherwise it's an absolute import (like "love_engine.caching.caching")
    if module_path.startswith("."):
        module = importlib.import_module(module_path, package="love_engine")
    else:
...
def _lazy_import_types_utils(name: str) -> Any:
    """Handler for types from love_engine.types.utils (BudgetConfig, ImageObject, etc.)"""
...
def _lazy_import_love_engine_logging(name: str) -> Any:
    """Handler for love_engine_logging module (Logging, modify_integration)"""
    return _generic_lazy_import(name, _LOVE_ENGINE_LOGGING_IMPORT_MAP, "love_engine logging")
...
def _lazy_import_utils_module(name: str) -> Any:
    ...
    _get_utils_globals() instead of _get_love_engine_globals() for caching.
    """
...
def _lazy_import_llm_client_cache(name: str) -> Any:
    ...
    # Import the class
    module = importlib.import_module("love_engine.caching.llm_caching_handler")
...
def _lazy_import_http_handlers(name: str) -> Any:
    ...
    if name == "module_level_aclient":
        ...
        provider_id = cast(Any, "love_engine_module_level_client")
    ...
    if name == "module_level_client":
        ...
        from love_engine.llms.custom_httpx.http_handler import HTTPHandler
