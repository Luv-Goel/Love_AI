...
from love_engine.types.router import LoveEngineParamsTypedDict
...
async def run_async_fallback(
    *args: Tuple[Any],
    love_engine_router: LoveEngineRouter,
    ...
    **kwargs,
    ...
    Args:
        love_engine_router: The love_engine router instance.
    ...
    for mg in fallback_model_group:
        ...
        try:
            # LOGGING
            kwargs = love_engine_router.log_retry(kwargs=kwargs, e=original_exception)
            ...
            response = await love_engine_router.async_function_with_fallbacks(
                *args, **kwargs
            )
...
async def log_success_fallback_event(
    ...
    custom_loggers = love_engine.logging_callback_manager.get_custom_loggers_for_type(
        CustomLogger
    )
...
def _check_non_standard_fallback_format(fallbacks: Optional[List[Any]]) -> bool:
    ...
    - List[Dict[<LoveEngineParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for item in fallbacks:
            for key in LoveEngineParamsTypedDict.__annotations__.keys():
                if key in item:
