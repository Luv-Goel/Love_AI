...
from love_engine.router_utils.cooldown_callbacks import router_cooldown_event_callback
...
def _is_cooldown_required(
    ...
    try:
        ...
        ):  # don't cooldown on love_engine api connection errors errors
...
def _should_run_cooldown_logic(
    ...
    - deployment is in love_engine_router_instance.provider_default_deployment_ids
    - exception_status is not one that should be immediately retried (e.g. 401)
    """
    if (
        deployment is None
        or love_engine_router_instance.get_model_group(id=deployment) is None
    ):
    ...
    if love_engine_router_instance.disable_cooldowns:
        verbose_router_logger.debug(
    ...
    if not _is_cooldown_required(
        love_engine_router_instance=love_engine_router_instance,
        model_id=deployment,
    ...
    if deployment in love_engine_router_instance.provider_default_deployment_ids:
        verbose_router_logger.debug(
...
def _should_cooldown_deployment(
    ...
    cooldown if:
        ...
        - got 401 Auth error, 404 NotFounder - checked by love_engine._should_retry()
    ...
    model_group = love_engine_router_instance.get_model_group(id=deployment)
    ...
    if (
        love_engine_router_instance.allowed_fails_policy is None
        ...
        num_successes_this_minute = get_deployment_successes_for_current_minute(
            love_engine_router_instance=love_engine_router_instance, deployment_id=deployment
        )
...
def _set_cooldown_deployments(
    ...
    if _should_cooldown_deployment(
        ...
        love_engine_router_instance.cooldown_cache.add_deployment_to_cooldown(
            model_id=deployment,
            original_exception=original_exception,
...
async def _async_get_cooldown_deployments(
    ...
    """
    model_ids = love_engine_router_instance.get_model_ids()
    cooldown_models = (
        await love_engine_router_instance.cooldown_cache.async_get_active_cooldowns(
            model_ids=model_ids,
...
def _get_cooldown_deployments(
    love_engine_router_instance: LoveEngineRouter, parent_otel_span: Optional[Span]
) -> List[str]:
    """
    ...
    cooldown_models = love_engine_router_instance.cooldown_cache.get_active_cooldowns(
        model_ids=model_ids, parent_otel_span=parent_otel_span
    )
...
def should_cooldown_based_on_allowed_fails_policy(
    ...
    """
    allowed_fails = (
        love_engine_router_instance.get_allowed_fails_from_policy(
            exception=original_exception,
        )
        or love_engine_router_instance.allowed_fails
    )
    cooldown_time = (
        love_engine_router_instance.cooldown_time or DEFAULT_COOLDOWN_TIME_SECONDS
    )

    current_fails = love_engine_router_instance.failed_calls.get_cache(key=deployment) or 0
    updated_fails = current_fails + 1

    if updated_fails > allowed_fails:
        return True
    else:
        love_engine_router_instance.failed_calls.set_cache(
...
def _is_allowed_fails_set_on_router(
    ...
    """
    if love_engine_router_instance.allowed_fails is None:
        return False
    if love_engine_router_instance.allowed_fails != love_engine.allowed_fails:
        return True
    return False
