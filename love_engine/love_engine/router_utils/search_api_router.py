...
class SearchAPIRouter:
    """
    Static utility class for routing search API calls through the LoveEngine router.
    ...
    def _resolve_search_provider_credentials(
        *,
        tool_love_engine_params: Dict[str, Any],
    ) -> Tuple[Optional[str], Optional[str]]:
        """
        Resolve search provider credentials from tool configuration ONLY.

        Credentials are stored only in search_tool.love_engine_params, never in team/key metadata.
        This ensures secrets are not exposed in team/key API responses.

        Args:
            tool_love_engine_params: Search tool love_engine_params with credentials

        Returns:
            Tuple of (api_key, api_base) from tool configuration
        """
        resolved_api_key: Optional[str] = tool_love_engine_params.get("api_key")
        resolved_api_base: Optional[str] = tool_love_engine_params.get("api_base")
    ...
    async def update_router_search_tools(router_instance: Any, search_tools: list):
        ...
        """
        try:
            from love_engine.types.router import SearchToolTypedDict
            ...
            for tool in search_tools:
                ...
                router_search_tool: SearchToolTypedDict = {  # type: ignore
                    ...
                    "love_engine_params": tool.get("love_engine_params", {}),
    ...
    async def async_search_with_fallbacks(
        ...
        **kwargs,
        ...
        Args:
            router_instance: The Router instance
            original_function: The original love_engine.asearch function
        ...
        try:
            ...
            from love_engine.router_utils.handle_error import send_llm_exception_alert

            asyncio.create_task(
                send_llm_exception_alert(
                    love_engine_router_instance=router_instance,
    ...
    async def async_search_with_fallbacks_helper(
        ...
        **kwargs,
        ...
        Args:
            ...
            original_generic_function: The original love_engine.asearch function
        ...
        try:
            ...
            # Extract search provider and other params from love_engine_params
            love_engine_params = selected_tool.get("love_engine_params", {})
            search_provider = love_engine_params.get("search_provider")
            if not search_provider:
                raise ValueError(
                    f"search_provider not found in love_engine_params for search tool '{search_tool_name}'"
                )

            api_key, api_base = SearchAPIRouter._resolve_search_provider_credentials(
                tool_love_engine_params=love_engine_params,
            )
