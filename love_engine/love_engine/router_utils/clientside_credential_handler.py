...
def _admin_config_fields_to_clear_on_base_override() -> List[str]:
    ...
    Built dynamically from ``CredentialLoveEngineParams.model_fields`` so any
    ...
    """
    from love_engine.types.router import CredentialLoveEngineParams

    typed_fields = [
        f
        for f in CredentialLoveEngineParams.model_fields
        if f not in clientside_credential_keys
    ]
    kwargs_only_fields = [
        # Caller-supplied via **kwargs, not declared on CredentialLoveEngineParams.
        "organization",
        "extra_body",
        "extra_headers",
        "default_headers",
        ...
        "aws_role_name",
        # OCI provider — consumed by love_engine/llms/oci/* via optional_params
        # and not declared on CredentialLoveEngineParams. Without these here,
...
def get_dynamic_love_engine_params(love_engine_params: dict, request_kwargs: dict) -> dict:
    """
    ...
    """
    # update love_engine_params with clientside credentials
    for key in clientside_credential_keys:
        if key in request_kwargs:
            love_engine_params[key] = request_kwargs[key]
