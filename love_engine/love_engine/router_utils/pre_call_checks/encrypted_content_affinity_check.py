...
2. **Items without IDs** (Codex): Wrap the encrypted_content with model_id metadata
   (e.g., ``love_engine_enc:{base64_metadata};{original_encrypted_content}``)
...
from love_engine.router_utils.cooldown_cache import CooldownCacheValue
...
class EncryptedContentAffinityCheck(CustomLogger):
    ...
    The ``model_id`` is decoded directly from the love_engine-encoded item IDs –
    ...
    def _extract_model_id_from_input(request_input: Any) -> Optional[str]:
        """
        Scan ``input`` items for love_engine-encoded encrypted-content markers and
        return the ``model_id`` embedded in the first one found.

        Checks both:
        1. Encoded item IDs (encitem_...) - for clients that send IDs
        2. Wrapped encrypted_content (love_engine_enc:...) - for clients like Codex that don't send IDs
    ...
    def _encryption_boundary_key(
        ...
        ``LOVE_ENGINE_Params``-style Pydantic instances, which define a custom
        ...
        """
        getter = getattr(love_engine_params, "get", None)
    ...
    def _find_deployments_on_same_encryption_boundary(
        ...
        boundary = self._encryption_boundary_key(
            originating.love_engine_params.model_dump(exclude_none=True)
        )
        if boundary is None:
            return [], originating
        matches = [
            ...
            if self._encryption_boundary_key(d.get("love_engine_params", {})) == boundary
        ]
    ...
    async def async_filter_deployments(
        ...
        """
        If the request ``input`` contains love_engine-encoded item IDs, decode the
        ...
        # love_engine_metadata already exists (Responses API path).  Using
        # setdefault would create an empty love_engine_metadata dict for chat
        # completions / embeddings, which breaks tag-based routing because
        # _get_metadata_variable_name_from_kwargs would pick "love_engine_metadata"
        # over "metadata" where tags are actually stored.
        if "love_engine_metadata" in request_kwargs:
            request_kwargs["love_engine_metadata"][
        ...
        if cooldown is not None and str(cooldown.get("status_code")) == "429":
            ...
            return RateLimitError(
                ...
                response=httpx.Response(
                    ...
                    request=httpx.Request("POST", "https://love_engine.ai/"),
                ),
            )
