...
class DeploymentAffinityCheck(CustomLogger):
    ...
    @staticmethod
    def _get_model_map_key_from_love_engine_model_name(
        ...
        """
        if not love_engine_model_name:
            return None

        if "/" not in love_engine_model_name:
            return love_engine_model_name

        provider_prefix, remainder = love_engine_model_name.split("/", 1)
    ...
    def _get_model_map_key_from_deployment(deployment: dict) -> Optional[str]:
        ...
        parsing `love_engine_params.model`.
        """
        ...
        if isinstance(love_engine_params, dict):
            base_model = love_engine_params.get("base_model")
            if isinstance(base_model, str) and base_model:
                return base_model
            love_engine_model_name = love_engine_params.get("model")
            if isinstance(love_engine_model_name, str) and love_engine_model_name:
                return (
                    DeploymentAffinityCheck._get_model_map_key_from_love_engine_model_name(
    ...
    def _iter_metadata_dicts(request_kwargs: dict) -> List[dict]:
        ...
        Depending on the endpoint, Router may populate `metadata` or `love_engine_metadata`.
    ...
    async def async_pre_call_deployment_hook(
        ...
        Why pre-call?
        - LoveEngine runs async success callbacks via a background logging worker for performance.
