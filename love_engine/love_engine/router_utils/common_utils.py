...
def get_love_engine_params_sensitive_credential_hash(love_engine_params: dict) -> str:
    """
    Hash of the credential params, used for mapping the file id to the right model
    """
    sensitive_params = CredentialLoveEngineParams(**love_engine_params)
...
def filter_team_based_models(
    ...
    request_team_id = metadata.get("user_api_key_team_id") or love_engine_metadata.get(
...
def _deployment_supports_web_search(deployment: Dict) -> bool:
    ...
    Note: Ideally we'd fall back to love_engine.supports_web_search() but
