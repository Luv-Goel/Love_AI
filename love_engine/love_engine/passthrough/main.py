...
from love_engine.passthrough.utils import CommonUtils
...
def llm_passthrough_route(
    ...
    **kwargs,
    ...
    if client is None:
        ...
        from love_engine.passthrough.timeout_utils import resolve_llm_passthrough_timeout
    ...
    # Add model_id to love_engine_params if present in kwargs (for Bedrock Application Inference Profiles)
    if "model_id" in kwargs:
        love_engine_params_dict["model_id"] = kwargs["model_id"]
    ...
    love_engine_logging_obj.stream = is_streaming_request
    ...
    try:
        if _is_async:
            ...
            if (
                ...
                return _sync_streaming(response, love_engine_logging_obj, provider_config)
            else:
...
def _sync_streaming(
    ...
    from love_engine.utils import executor
    ...
    try:
        ...
        if not flush_scheduled and raw_bytes:
            ...
            try:
                executor.submit(
                    love_engine_logging_obj.flush_passthrough_collected_chunks,
                    raw_bytes=raw_bytes,
                    provider_config=provider_config,
                )
            except Exception as e:
...
async def _async_streaming(
    ...
    try:
        ...
        if not flush_scheduled and raw_bytes:
            ...
            try:
                asyncio.create_task(
                    love_engine_logging_obj.async_flush_passthrough_collected_chunks(
                        raw_bytes=raw_bytes,
                        provider_config=provider_config,
                    )
                )
