...
from love_engine.vector_store_files.main import acreate as vector_store_file_acreate
from love_engine.vector_stores.main import acreate as vector_store_acreate
...
class OpenAIRAGIngestion(BaseRAGIngestion):
    ...
    async def store(
        ...
        # Get credentials from vector_store_config (loaded from love_engine_credential_name if provided)
        ...
        if not vector_store_id:
            ...
            create_response = await vector_store_acreate(
                name=self.ingest_name or "love_engine-rag-ingest",
        ...
        if file_content and filename and vector_store_id:
            ...
            file_response = await love_engine.acreate_file(
                file=(
                    filename,
                    file_content,
