...
class VertexAIRAGIngestion(BaseRAGIngestion, VertexBase):
    ...
    def __init__(
        ...
        love_engine_params = dict(self.vector_store_config)

        # Get project, location, and credentials using VertexBase methods
        self.project_id = self.safe_get_vertex_ai_project(love_engine_params)
        self.location = self.get_vertex_ai_location(love_engine_params) or "us-central1"
        self.vertex_credentials = self.safe_get_vertex_ai_credentials(love_engine_params)
    ...
    async def store(
        ...
        if not rag_corpus_id:
            rag_corpus_id = await self._create_rag_corpus(
                display_name=self.ingest_name or "love_engine-rag-corpus",
