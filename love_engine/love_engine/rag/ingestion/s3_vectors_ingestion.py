...
2. Uses LoveEngine's embedding API (supports any provider)
...
class S3VectorsRAGIngestion(BaseRAGIngestion, BaseAWSLLM):
    ...
    3. Generate embeddings using LoveEngine (supports any provider)
    ...
    async def _get_dimension_from_embedding_request(self) -> int:
        ...
        try:
            ...
            if self.router:
                ...
                response = await love_engine.aembedding(
                    model=model_name, input=[test_input]
                )
    ...
    async def _ensure_config_initialized(self):
        ...
        if not self.index_name:
            ...
            self.index_name = f"love_engine-index-{unique_id}"
    ...
    async def _sign_and_execute_request(
        ...
        Pattern from love_engine/integrations/s3_v2.py
        """
        try:
            import requests
    ...
    async def embed(
        ...
        """
        Generate embeddings using LoveEngine's embedding API.
    ...
    async def query_vector_store(
        ...
        response = await love_engine.aembedding(model=embedding_model, input=[query])
