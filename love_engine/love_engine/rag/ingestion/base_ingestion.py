...
from love_engine._uuid import uuid4
from love_engine.constants import DEFAULT_CHUNK_OVERLAP, DEFAULT_CHUNK_SIZE
from love_engine.llms.custom_httpx.http_handler import (
    get_async_httpx_client,
    httpxSpecialProvider,
)
from love_engine.love_engine_core_utils.url_utils import async_safe_get
from love_engine.rag.ingestion.file_parsers import extract_text_from_pdf
from love_engine.rag.text_splitters import RecursiveCharacterTextSplitter
from love_engine.types.rag import RAGIngestOptions, RAGIngestResponse
...
class BaseRAGIngestion(ABC):
    ...
    def __init__(
        ...
        # Load credentials from love_engine_credential_name if provided in vector_store config
    ...
    def _load_credentials_from_config(self) -> None:
        ...
        which will be resolved from love_engine.credential_list. When a stored
        ...
        credential_name = self.vector_store_config.get("love_engine_credential_name")
    ...
    async def ocr(
        ...
        if self.router is not None:
            ...
            ocr_response = await love_engine.aocr(
                model=ocr_model,
                document={"type": doc_type, url_key: data_url},
            )
    ...
    async def embed(
        ...
        if self.router is not None:
            ...
            response = await love_engine.aembedding(model=embedding_model, input=chunks)
