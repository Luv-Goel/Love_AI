...
from love_engine.rag.rag_query import RAGQuery
from love_engine.types.rag import (
    RAGIngestOptions,
    RAGIngestResponse,
)
...
async def _execute_ingest_pipeline(
    ...
    Args:
        ...
        router: Optional LoveEngine router for load balancing
        ...
        response = await love_engine.aingest(
            ingest_options={
                "vector_store": {
                    ...
                    "love_engine_credential_name": "my-openai-creds",  # optional
                }
            },
...
async def _execute_query_pipeline(
    ...
    **kwargs,
    ...
    if rerank and rerank.get("enabled"):
        ...
        if documents:
            rerank_response = await love_engine.arerank(
                model=rerank["model"],
                query=query_text,
                documents=documents,
