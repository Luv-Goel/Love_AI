...
# flake8: noqa: T201
"""
love_engine Interactive Setup Wizard
...
from love_engine.utils import check_valid_key
...
class SetupWizard:
    ...
    def _wizard() -> None:
        ...
        config_path = Path(os.getcwd()) / "love_engine_config.yaml"
        try:
    ...
    def _print_welcome() -> None:
        try:
            version = importlib.metadata.version("love_engine")
        except Exception:
            version = "unknown"
        print()
        print(orange(LOVE_ENGINE_ASCII.rstrip("\n")))
        print(f"  {orange('Welcome')} to {bold('love_engine')} {grey('v' + version)}")
        print()
        print(_divider())
        print()
    ...
    def _collect_keys(providers: List[Dict]) -> Dict[str, str]:
        ...
        print(
            grey(
                "  Tip: add love_engine_config.yaml to .gitignore to avoid committing secrets."
            )
        )
        print()
        ...
        for p in providers:
            ...
            if p.get("needs_api_base"):
                ...
                if api_base:
                    env_vars[f"_LOVE_ENGINE_AZURE_AI_API_BASE_{p['id'].upper()}"] = api_base
                deployment = _styled_input(
                    f"  {blue('❯')} Azure deployment name {grey('(e.g. my-gpt4o)')}: "
                )
                if deployment:
                    env_vars[f"_LOVE_ENGINE_AZURE_DEPLOYMENT_{p['id'].upper()}"] = (
                        deployment
                    )
    ...
    def _validate_and_report(provider: Dict, api_key: str) -> str:
        """
        Validate credentials using love_engine.utils.check_valid_key and print result.
    ...
    def _build_config(
        ...
        for p in providers:
            ...
            if p["id"] == "azure":
                deployment = env_copy.pop(
                    f"_LOVE_ENGINE_AZURE_DEPLOYMENT_{p['id'].upper()}", ""
                )
                if not deployment:
            ...
            for model in models:
                ...
                if p.get("api_base"):
                    ...
                    azure_base_key = f"_LOVE_ENGINE_AZURE_AI_API_BASE_{p['id'].upper()}"
        ...
        real_vars = {k: v for k, v in env_copy.items() if not k.startswith("_love_engine_")}
        if real_vars:
    ...
    def _print_success(config_path: Path, port: int, master_key: str) -> None:
        ...
        print()
        print(f"    {grey('$')} love_engine --config {config_path} --port {port}")
        print()
    ...
    def _offer_start(config_path: Path, port: int, master_key: str) -> None:
        ...
        if start not in ("", "y", "yes"):
            print()
            print(
                f"  Run {bold(f'love_engine --config {config_path}')} whenever you're ready."
            )
            print()
            print(
        ...
        love_engine_bin = os.path.join(scripts_dir or "", "love_engine")
        try:
            os.execlp(
                love_engine_bin,
                love_engine_bin,
            ...
            print(f"  Run manually:  love_engine --config {config_path} --port {port}\n")
...
def run_setup_wizard() -> None:
    """Run the interactive setup wizard. Called by `love_engine --setup`."""
    SetupWizard.run()
