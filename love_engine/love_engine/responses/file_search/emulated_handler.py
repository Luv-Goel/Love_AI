...
Flow:
  ...
  3. If the provider issues a file_search function_call, execute vector search
     via love_engine.vector_stores.main.asearch().
...
from love_engine.types.llms.openai import ResponseOutputItem, ResponsesAPIResponse
from love_engine.types.vector_stores import VectorStoreSearchResult

# Keep ToolParam broad so we stay compatible with both dict and Pydantic forms
ToolParam = Any

FILE_SEARCH_FUNCTION_NAME = "love_engine_file_search"
...
def _build_function_tool(vector_store_ids: List[str]) -> Dict[str, Any]:
    ...
    file_search); LoveEngine runs the actual vector search against the configured
    vector stores.
    ...
    LoveEngineCompletionResponsesConfig transformation picks up name and description.
    """
    return {
        "type": "function",
...
async def _run_vector_searches(
    ...
    """
    import love_engine.vector_stores.main as vs_main
...
async def aresponses_with_emulated_file_search(
    ...
    **kwargs: Any,
    ...
    # the parent love_engine_logging_obj (propagated via kwargs) fires once at the end.
