...
from love_engine.responses.love_engine_completion_transformation.handler import (
    LoveEngineCompletionTransformationHandler,
)
...
if TYPE_CHECKING:
    from love_engine.types.llms.openai import ResponseText  # type: ignore
else:
...
from love_engine.llms.openai.data_residency import infer_openai_data_residency
...
love_engine_completion_transformation_handler = LoveEngineCompletionTransformationHandler()
#################################################
...
async def aresponses_api_with_mcp(
    ...
    **kwargs,
    ...
    When MCP tools with server_url="love_engine_proxy" are provided, this function will:
    ...
    (
        ...
        other_tools,
    ) = LOVE_ENGINE_Proxy_MCP_Handler._parse_mcp_tools(tools)

    # Process MCP tools through the complete pipeline (fetch + filter + deduplicate + transform)
    # Extract user_api_key_auth from love_engine_metadata (where it's added by add_user_api_key_auth_to_request_metadata)
    ...
    if stream and mcp_tools_with_love_engine_proxy:
        ...
        return LOVE_ENGINE_Proxy_MCP_Handler._create_mcp_streaming_response(
            input=input,
            model=model,
            all_tools=all_tools,
            mcp_tools_with_love_engine_proxy=mcp_tools_with_love_engine_proxy,
    ...
    should_auto_execute = bool(
        mcp_tools_with_love_engine_proxy
    ) and LOVE_ENGINE_Proxy_MCP_Handler._should_auto_execute_tools(
        mcp_tools_with_love_engine_proxy=mcp_tools_with_love_engine_proxy
    )

    # Prepare parameters for the initial call
    initial_call_params = LOVE_ENGINE_Proxy_MCP_Handler._prepare_initial_call_params(
    ...
    if should_auto_execute and isinstance(
        ...
        tool_calls = LOVE_ENGINE_Proxy_MCP_Handler._extract_tool_calls_from_response(
            response=response
        )

        if tool_calls:
            user_api_key_auth = kwargs.get("love_engine_metadata", {}).get(
                "user_api_key_auth"
            )
            ...
            tool_results = await LOVE_ENGINE_Proxy_MCP_Handler._execute_tool_calls(
                tool_server_map=tool_server_map,
            ...
            if tool_results:
                follow_up_input = LOVE_ENGINE_Proxy_MCP_Handler._create_follow_up_input(
                ...
                follow_up_call_params = (
                    LOVE_ENGINE_Proxy_MCP_Handler._prepare_follow_up_call_params(
                ...
                if stream:
                    tool_execution_events = (
                        LOVE_ENGINE_Proxy_MCP_Handler._create_tool_execution_events(
                            tool_calls=tool_calls, tool_results=tool_results
                        )
                    )

                final_response = await LOVE_ENGINE_Proxy_MCP_Handler._make_follow_up_call(
                ...
                # Add custom output elements to the final response (for non-streaming)
                    ...
                    final_response = (
                        LOVE_ENGINE_Proxy_MCP_Handler._add_mcp_output_elements_to_response(
...
async def aresponses(
    ...
    **kwargs,
    ...
    try:
        ...
        ) and love_engine_logging_obj.should_run_prompt_management_hooks(
        ...
        if isinstance(response, ResponsesAPIResponse):
            ...
            # (mirrors love_engine/main.py:1371 for chat completions)
...
def _resolve_model_provider_for_responses(
    ...
    if custom_llm_provider is not None and not love_engine_params.custom_llm_provider:
        love_engine_params.custom_llm_provider = custom_llm_provider
    (
        model,
        custom_llm_provider,
        dynamic_api_key,
    ...
    if dynamic_api_key is not None:
        love_engine_params.api_key = dynamic_api_key
    if dynamic_api_base is not None:
        love_engine_params.api_base = dynamic_api_base
...
def _responses_try_dispatch_mcp_gateway(
    *,
    ...
    if not LOVE_ENGINE_Proxy_MCP_Handler._should_use_love_engine_mcp_gateway(tools=tools):
        return None
    mcp_call_kwargs = {
        "input": input,
        "model": model,
...
def _responses_try_dispatch_emulated_file_search(
    *,
    ...
    if not _has_file_search_tool(tools) or not (
        ...
        return None
    from love_engine.responses.file_search.emulated_handler import (
        aresponses_with_emulated_file_search,
    )

    _internal_skip = {"love_engine_call_id", "aresponses"}
    emulated_kwargs = {
        "include": include,
        "instructions": instructions,
        "max_output_tokens": max_output_tokens,
...
def responses(
    ...
    **kwargs,
    ...
    try:
        ...
        # MOCK RESPONSE LOGIC
        #########################################################
        if love_engine_params.mock_response and isinstance(
            love_engine_params.mock_response, str
        ):
            return mock_responses_api_response(
                mock_response=love_engine_params.mock_response
            )
        ...
        # Map reasoning_effort (from love_engine_params/proxy config) to reasoning when not set
        if reasoning is None and "reasoning_effort" in local_vars:
            _mapped = LoveEngineResponsesTransformationHandler()._map_reasoning_effort(
        ...
        if responses_api_provider_config is None or use_chat_completions_api is True:
            return love_engine_completion_transformation_handler.response_api_handler(
                model=model,
                input=input,
            ...
            love_engine_params={
                **responses_api_request_params,
                ...
                "data_residency": infer_openai_data_residency(
                    custom_llm_provider, love_engine_params.api_base
                ),
                "metadata": (
                    kwargs["love_engine_metadata"]
                    if "love_engine_metadata" in kwargs
        ...
        # Decode any love_engine-encoded encrypted-content item IDs back to their original IDs
    ...
    **kwargs,
    ...
    try:
        ...
        love_engine_logging_obj.update_from_kwargs(
            ...
            love_engine_params={"love_engine_call_id": love_engine_call_id},
            custom_llm_provider=custom_llm_provider,
        )
...
def _build_love_engine_metadata_for_ws(kwargs: dict) -> dict:
...
async def _aresponses_websocket(
    ...
    **kwargs,
    ...
    love_engine_params_dict["data_residency"] = infer_openai_data_residency(
        _custom_llm_provider,
        dynamic_api_base or love_engine_params.api_base or love_engine.api_base,
    )
    ...
    if _custom_llm_provider is not None:
        responses_api_provider_config = (
            ProviderConfigManager.get_provider_responses_api_config(
                model=model,
                provider=love_engine.LlmProviders(_custom_llm_provider),
            )
        )

    resolved_api_base = (
        dynamic_api_base or love_engine_params.api_base or love_engine.api_base or None
    )
    resolved_api_key = (
        dynamic_api_key
        or love_engine_params.api_key
    ...
    await base_llm_http_handler.async_responses_websocket(
        ...
        love_engine_metadata=_build_love_engine_metadata_for_ws(kwargs),
        custom_llm_provider=_custom_llm_provider,
        **remaining_kwargs,
    )
