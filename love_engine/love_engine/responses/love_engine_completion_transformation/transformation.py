"""
Handles transforming from Responses API -> LoveEngine completion  (Chat Completion API)
"""
...
from love_engine.caching import InMemoryCache
from love_engine.love_engine_core_utils.love_engine_logging import Logging as LoveEngineLoggingObj
from love_engine.responses.love_engine_completion_transformation.session_handler import (
...
from love_engine.types.responses.main import (
...
class ChatCompletionSession(TypedDict, total=False):
    ...
    love_engine_session_id: Optional[str]


########### End of Initialize Classes used for Responses API  ###########


class LoveEngineCompletionResponsesConfig:
    @staticmethod
    def get_supported_openai_params(model: str) -> list:
        """
        LoveEngine Adapter from OpenAI Responses API to Chat Completion API supports a subset of OpenAI Responses API params
        """
        return [
            "input",
            "model",
    ...
    def transform_responses_api_request_to_chat_completion_request(
        ...
        **kwargs,
        ...
        (
            tools,
            web_search_options,
        ) = LoveEngineCompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
        ...
        if text_param:
            response_format = LoveEngineCompletionResponsesConfig._transform_text_format_to_response_format(
                text_param
            )
        ...
        love_engine_completion_request: dict = {
            "messages": LoveEngineCompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=input,
                responses_api_request=responses_api_request,
            ),
            "model": model,
            "tool_choice": LoveEngineCompletionResponsesConfig._transform_tool_choice(
        ...
        # Responses API `Completed` events require usage, we pass `stream_options` to love_engine.completion to include usage
        if stream is True:
            stream_options = {
                "include_usage": True,
            }
            love_engine_completion_request["stream_options"] = stream_options
            love_engine_logging_obj: Optional[LoveEngineLoggingObj] = kwargs.get(
                "love_engine_logging_obj"
            )
            if love_engine_logging_obj:
                love_engine_logging_obj.stream_options = stream_options

        # only pass non-None values
        love_engine_completion_request = {
            k: v for k, v in love_engine_completion_request.items() if v is not None
        }
        return love_engine_completion_request
    ...
    def transform_responses_api_input_to_messages(
        ...
        if responses_api_request.get("instructions"):
            messages.append(
                LoveEngineCompletionResponsesConfig.transform_instructions_to_system_message(
                    responses_api_request.get("instructions")
                )
            )

        messages.extend(
            LoveEngineCompletionResponsesConfig._transform_response_input_param_to_chat_completion_message(
                input=input,
            )
        )
    ...
    async def async_responses_api_session_handler(
        ...
        chat_completion_session = ChatCompletionSession(
            messages=[], love_engine_session_id=None
        )
        ...
        _messages = love_engine_completion_request.get("messages") or []
        ...
        tools = love_engine_completion_request.get("tools") or []
        combined_messages = LoveEngineCompletionResponsesConfig._ensure_tool_results_have_corresponding_tool_calls(
        ...
        if not combined_messages:
            ...
            if original_new_messages:
                ...
                raise love_engine.BadRequestError(
                    ...
                    model=love_engine_completion_request.get("model", ""),
                    llm_provider=love_engine_completion_request.get(
                        "custom_llm_provider", ""
                    ),
                )

        love_engine_completion_request["messages"] = combined_messages
        love_engine_completion_request["love_engine_trace_id"] = chat_completion_session.get(
            "love_engine_session_id"
        )
        return love_engine_completion_request
    ...
    def _transform_response_input_param_to_chat_completion_message(
        ...
        if isinstance(input, str):
            ...
            for _input in input:
                chat_completion_messages = LoveEngineCompletionResponsesConfig._transform_responses_api_input_item_to_chat_completion_message(
                    input_item=_input
                )

                if LoveEngineCompletionResponsesConfig._is_input_item_function_call(
                    input_item=_input
                ):
                        ...
                        if last_role == "assistant":
                            for new_msg in chat_completion_messages:
                                ...
                                if new_role == "assistant":
                                    ...
                                    for tc in new_tcs:
                                        LoveEngineCompletionResponsesConfig._add_tool_call_to_assistant(
                                            last_msg, tc
                                        )
                            continue
                ...
                #########################################################
                if LoveEngineCompletionResponsesConfig._is_input_item_tool_call_output(
                    input_item=_input
                ):
    ...
    def _create_tool_call_chunk(
        ...
        function_raw = LoveEngineCompletionResponsesConfig._get_mapping_or_attr_value(
            tool_use_definition, "function"
        )
        function_name_raw = LoveEngineCompletionResponsesConfig._get_mapping_or_attr_value(
            function_raw, "name"
        )
        function_arguments_raw = (
            LoveEngineCompletionResponsesConfig._get_mapping_or_attr_value(
        ...
        tool_use_id_raw = LoveEngineCompletionResponsesConfig._get_mapping_or_attr_value(
            tool_use_definition, "id"
        )
        tool_use_id: str = (
            str(tool_use_id_raw) if tool_use_id_raw is not None else str(tool_call_id)
        )
        tool_use_type_raw = LoveEngineCompletionResponsesConfig._get_mapping_or_attr_value(
    ...
    def _ensure_tool_results_have_corresponding_tool_calls(
        ...
        for i, message in enumerate(fixed_messages):
            ...
            prev_assistant_idx = (
                LoveEngineCompletionResponsesConfig._find_previous_assistant_idx(
                    fixed_messages, i
                )
            )
            ...
            if not tool_call_id and prev_assistant_idx is not None:
                ...
                tool_call_id = LoveEngineCompletionResponsesConfig._recover_tool_call_id_from_assistant(
            ...
            if prev_assistant_idx is not None and tool_call_id:
                prev_assistant = fixed_messages[prev_assistant_idx]
                tool_calls = LoveEngineCompletionResponsesConfig._get_tool_calls_list(
                    prev_assistant
                )

                if not LoveEngineCompletionResponsesConfig._check_tool_call_exists(
                    tool_calls, tool_call_id
                ):
                    _tool_use_definition = TOOL_CALLS_CACHE.get_cache(key=tool_call_id)

                    if not _tool_use_definition and tools:
                        _tool_use_definition = LoveEngineCompletionResponsesConfig._reconstruct_tool_call_from_tools(
                            tool_call_id, tools
                        )

                    normalized_tool_use_definition = (
                        LoveEngineCompletionResponsesConfig._normalize_tool_use_definition(
                    ...
                    if normalized_tool_use_definition:
                        tool_call_chunk = (
                            LoveEngineCompletionResponsesConfig._create_tool_call_chunk(
                                normalized_tool_use_definition,
                                tool_call_id,
                                len(tool_calls),
                            )
    ...
    def _transform_responses_api_input_item_to_chat_completion_message(
        ...
        - ItemReference
        """
        if LoveEngineCompletionResponsesConfig._is_input_item_tool_call_output(input_item):
            # handle executed tool call results
            return LoveEngineCompletionResponsesConfig._transform_responses_api_tool_call_output_to_chat_completion_message(
                tool_call_output=input_item
            )
        elif LoveEngineCompletionResponsesConfig._is_input_item_function_call(input_item):
            # handle function call input items
            return LoveEngineCompletionResponsesConfig._transform_responses_api_function_call_to_chat_completion_message(
            ...
            return [
                GenericChatCompletionMessage(
                    ...
                    content=LoveEngineCompletionResponsesConfig._transform_responses_api_content_to_chat_completion_content(
                        content
                    ),
                )
            ]
    ...
    def _transform_input_file_item_to_file_item(item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        file_id = LoveEngineCompletionResponsesConfig._resolve_file_id(item)
        if file_id:
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    if item.get("type") == "input_file":
                        content_list.append(
                            LoveEngineCompletionResponsesConfig._transform_input_file_item_to_file_item(
                                item
                            )
                        )
                    elif item.get("type") == "input_image":
                        image_block = dict(
                            LoveEngineCompletionResponsesConfig._transform_input_image_item_to_image_item(
                                item
                            )
                        )
                        if "cache_control" in item:
                        ...
                        content_block: Dict[str, Any] = {
                            "type": LoveEngineCompletionResponsesConfig._get_chat_completion_request_content_type(
    ...
    def convert_response_function_tool_call_to_chat_completion_tool_call(
        ...
        tool_call_dict: Dict[str, Any] = {
            "id": LoveEngineCompletionResponsesConfig._tool_call_id_from_responses_item(
    ...
    def transform_chat_completion_response_to_responses_api_response(
        ...
        responses_api_response: ResponsesAPIResponse = ResponsesAPIResponse(
            ...
            output=LoveEngineCompletionResponsesConfig._transform_chat_completion_choices_to_responses_output(
            ...
            reasoning=None,
            status=LoveEngineCompletionResponsesConfig._map_chat_completion_finish_reason_to_responses_status(
                finish_reason
            ),
            text={},
            truncation=getattr(chat_completion_response, "truncation", None),
            usage=LoveEngineCompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
                chat_completion_response=chat_completion_response
            ),
    ...
    def _transform_chat_completion_choices_to_responses_output(
        ...
        responses_output.extend(
            LoveEngineCompletionResponsesConfig._extract_reasoning_output_items(
                chat_completion_response, choices
            )
        )
        responses_output.extend(
            LoveEngineCompletionResponsesConfig._extract_message_output_items(
                chat_completion_response, choices
            )
        )
        responses_output.extend(
            LoveEngineCompletionResponsesConfig.transform_chat_completion_tools_to_responses_tools(
                chat_completion_response=chat_completion_response
            )
        )
        ...
        tool_result_items = (
            LoveEngineCompletionResponsesConfig._extract_tool_result_output_items(
    ...
    def _extract_image_generation_output_items(
        ...
        for idx, image_item in enumerate(images):
            ...
            base64_data = (
                LoveEngineCompletionResponsesConfig._extract_base64_from_data_url(
                    image_url
                )
            )
            ...
            if base64_data:
                image_generation_items.append(
                    OutputImageGenerationCall(
                        ...
                        status=LoveEngineCompletionResponsesConfig._map_finish_reason_to_image_generation_status(
                            choice.finish_reason
                        ),
                        result=base64_data,
                    )
    ...
    def _extract_message_output_items(
        ...
        for choice in choices:
            ...
            if hasattr(choice.message, "images") and choice.message.images:
                ...
                image_generation_items = LoveEngineCompletionResponsesConfig._extract_image_generation_output_items(
                    chat_completion_response=chat_completion_response,
                    choice=choice,
                )
                ...
                message_output_items.append(
                    GenericResponseOutputItem(
                        ...
                        content=[
                            LoveEngineCompletionResponsesConfig._transform_chat_message_to_response_output_text(
                                choice.message
                            )
                        ],
                    )
    ...
    def _transform_responses_api_outputs_to_chat_completion_messages(
        ...
        for _output_item in output_items:
            ...
            if output_item.get("type") == "function_call":
                ...
                messages.append(
                    LoveEngineCompletionResponsesConfig._transform_responses_output_tool_call_to_chat_completion_output_tool_call(
    ...
    def _transform_chat_message_to_response_output_text(
        ...
        transformed_annotations = LoveEngineCompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
