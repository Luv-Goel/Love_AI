...
from love_engine.proxy._types import SpendLogsPayload
from love_engine.proxy.spend_tracking.cold_storage_handler import ColdStorageHandler
...
from love_engine.types.utils import ChatCompletionMessageToolCall, Message, ModelResponse
...
class ResponsesSessionHandler:
    ...
    async def get_chat_completion_message_history_for_previous_response_id(
        ...
        if len(all_spend_logs) > 0:
            love_engine_session_id = all_spend_logs[0].get("session_id")
    ...
    async def extend_chat_completion_message_with_spend_log_payload(
        ...
        if response_input_param:
            chat_completion_messages = LoveEngineCompletionResponsesConfig.transform_responses_api_input_to_messages(
    ...
    def _should_check_cold_storage_for_full_payload(
        ...
        1. `LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD` is in the proxy server request dict
        2. `love_engine.cold_storage_custom_logger` is not None
        """
        from love_engine.constants import LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD

        configured_cold_storage_custom_logger = love_engine.cold_storage_custom_logger
        ...
        if len(proxy_server_request_dict) == 0:
            return True
        if LOVE_ENGINE_TRUNCATED_PAYLOAD_FIELD in str(proxy_server_request_dict):
            return True
        return False
