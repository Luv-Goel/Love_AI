...
from love_engine.responses.streaming_iterator import ResponsesAPIStreamingIterator
...
from love_engine.types.utils import Delta as ChatCompletionDelta
...
class LoveEngineCompletionStreamingIterator(ResponsesAPIStreamingIterator):
    """
    ...
    def __init__(
        self,
        model: str,
        love_engine_custom_stream_wrapper: love_engine.CustomStreamWrapper,
        request_input: Union[str, ResponseInputParam],
        responses_api_request: ResponsesAPIOptionalRequestParams,
        custom_llm_provider: Optional[str] = None,
        love_engine_metadata: Optional[dict] = None,
    ):
        self.model: str = model
        self.love_engine_custom_stream_wrapper: love_engine.CustomStreamWrapper = (
            love_engine_custom_stream_wrapper
        )
        ...
        self.love_engine_metadata: Optional[dict] = love_engine_metadata or {}
        ...
        self.love_engine_logging_obj = love_engine_custom_stream_wrapper.logging_obj
        ...
        self.love_engine_model_response: Optional[
            Union[ModelResponse, TextCompletionResponse]
        ] = None
        self.final_text: str = ""
        self._cached_item_id: Optional[str] = None
        self._cached_response_id: Optional[str] = None
        self._pending_tool_events: List[BaseLoveEngineOpenAIResponseObject] = []
        ...
        self._pending_response_events: List[BaseLoveEngineOpenAIResponseObject] = []
        ...
        for tc in tool_calls:
            ...
            if fn_args_delta:
                ...
                for i in range(0, len(fn_args_delta), chunk_size):
                    ...
                    self._sequence_number += 1
                    delta_event: BaseLoveEngineOpenAIResponseObject = (
                        FunctionCallArgumentsDeltaEvent(
                    ...
                    # Add sequence_number as extra field (BaseLoveEngineOpenAIResponseObject allows extra fields)
    ...
    def _queue_final_tool_call_done_events(
        self, love_engine_complete_object: ModelResponse
    ) -> None:
        """
        ...
        try:
            message = love_engine_complete_object.choices[0].message  # type: ignore
    ...
    def _default_response_created_event_data(self) -> dict:
        ...
        if "tool_choice" in self.responses_api_request:
            ...
            response_created_event_data["tool_choice"] = (
                LoveEngineCompletionResponsesConfig._transform_tool_choice(
    ...
    def create_love_engine_model_response(self) -> Optional[ModelResponse]:
        response = cast(
            Optional[ModelResponse],
            stream_chunk_builder(
                chunks=self.collected_chat_completion_chunks,
                logging_obj=self.love_engine_logging_obj,
            ),
        )
    ...
    def create_output_text_done_event(
        ...
        return OutputTextDoneEvent(
            ...
            output_index=0,
            content_index=0,
            text=getattr(love_engine_complete_object.choices[0].message, "content", "")  # type: ignore
            or "",
        )
    ...
    def create_output_content_part_done_event(
        ...
        text = getattr(love_engine_complete_object.choices[0].message, "content", "") or ""  # type: ignore
        reasoning_content = getattr(love_engine_complete_object.choices[0].message, "reasoning_content", "") or ""  # type: ignore
        annotations = getattr(love_engine_complete_object.choices[0].message, "annotations", None)  # type: ignore
        ...
        if reasoning_content:
            ...
            response_annotations = LoveEngineCompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
    ...
    def create_output_item_done_event(
        ...
        text = self.love_engine_model_response.choices[0].message.content or ""  # type: ignore
        annotations = getattr(self.love_engine_model_response.choices[0].message, "annotations", None)  # type: ignore
    ...
    def common_done_event_logic(
        ...
        if not self.love_engine_model_response or isinstance(
            self.love_engine_model_response, TextCompletionResponse
        ):
            self.love_engine_model_response = self.create_love_engine_model_response()
        if self.love_engine_model_response:
            # If tool calls exist, emit tool events before finishing/response.completed.
            if isinstance(self.love_engine_model_response, ModelResponse):
                self._queue_final_tool_call_done_events(self.love_engine_model_response)
            if self._pending_tool_events:
                return self._pending_tool_events.pop(0)

            done_event = self.return_default_done_events(self.love_engine_model_response)
            if done_event:
                return done_event
        else:
            if sync_mode:
        ...
        response_completed_event = self._emit_response_completed_event(
            self.love_engine_model_response
        )
    ...
    async def __anext__(
        ...
        try:
            while True:
                ...
                try:
                    chunk = await self.love_engine_custom_stream_wrapper.__anext__()
        ...
        try:
            while True:
                ...
                if self._pending_tool_events:
                    return self._pending_tool_events.pop(0)
                try:
                    chunk = self.love_engine_custom_stream_wrapper.__next__()
                    self._ensure_output_item_for_chunk(chunk)
                    # Accumulate provider_specific_fields from chunk and delta
                    for src in (
                        getattr(chunk, "provider_specific_fields", None),
    ...
    def _get_delta_string_from_streaming_choices(
        ...
        It's unclear how users expect love_engine to translate multiple-choices-per-chunk to the responses API output.
        """
        choice = choices[0]
    ...
    def _emit_response_completed_event(
        self, love_engine_model_response: ModelResponse
    ) -> Optional[ResponseCompletedEvent]:
        if love_engine_model_response:
            # Add cost to usage object if include_cost_in_streaming_usage is True
            if (
                love_engine.include_cost_in_streaming_usage
                and self.love_engine_logging_obj is not None
            ):
                usage = getattr(love_engine_model_response, "usage", None)
                if usage is not None:
                    setattr(
                        usage,
                        "cost",
                        self.love_engine_logging_obj._response_cost_calculator(
                            result=love_engine_model_response
                        ),
                    )

            # Transform the response
            responses_api_response = LoveEngineCompletionResponsesConfig.transform_chat_completion_response_to_responses_api_response(
                request_input=self.request_input,
                chat_completion_response=love_engine_model_response,
            ...
            encoded_response = ResponsesAPIRequestUtils._update_responses_api_response_id_with_model_id(
                ...
                love_engine_metadata=self.love_engine_metadata,
            )
