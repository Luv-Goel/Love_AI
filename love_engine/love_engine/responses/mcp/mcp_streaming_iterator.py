...
async def create_mcp_list_tools_events(
    ...
    try:
        ...
        from love_engine.types.llms.openai import OutputItemDoneEvent

        # Extract server label from the first MCP tool config
        server_label = ""
        if mcp_tools_with_love_engine_proxy:
            first_tool = mcp_tools_with_love_engine_proxy[0]
...
class MCPEnhancedStreamingIterator(BaseResponsesAPIStreamingIterator):
    ...
    def __init__(
        ...
        mcp_tools_with_love_engine_proxy: Optional[List[Any]] = None,
        user_api_key_auth: Any = None,
        original_request_params: Optional[Dict[str, Any]] = None,
    ):
        # MCP setup
        self.mcp_tools_with_love_engine_proxy = mcp_tools_with_love_engine_proxy or []
        ...
        self.love_engine_metadata = {}
        self.custom_llm_provider = self.original_request_params.get(
            "custom_llm_provider", None
        )
        self.love_engine_call_id = self.original_request_params.get("love_engine_call_id")
        self.love_engine_trace_id = self.original_request_params.get("love_engine_trace_id")
    ...
    def _should_auto_execute_tools(self) -> bool:
        ...
        return LOVE_ENGINE_Proxy_MCP_Handler._should_auto_execute_tools(
            self.mcp_tools_with_love_engine_proxy
        )
    ...
    async def _create_initial_response_iterator(self) -> None:
        ...
        try:
            ...
            if hasattr(response, "__aiter__") or hasattr(response, "__iter__"):
                ...
                self.love_engine_metadata = getattr(response, "love_engine_metadata", {})
    ...
    async def _generate_tool_execution_events(self) -> None:
        ...
        try:
            ...
            if self.collected_response is not None:
                tool_calls = LOVE_ENGINE_Proxy_MCP_Handler._extract_tool_calls_from_response(self.collected_response)  # type: ignore[arg-type]
            else:
                tool_calls = []
            if not tool_calls:
                return
                ...
                output_item_done_event = OutputItemDoneEvent(
                    ...
                    item=BaseLoveEngineOpenAIResponseObject(
                        **{
                            ...
                            "name": tool_name,
                            "output": result_text,
                            "server_label": "love_engine",  # or extract from tool config
                        }
                    ),
                )
