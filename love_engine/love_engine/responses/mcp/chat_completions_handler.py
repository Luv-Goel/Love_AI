...
async def acompletion_with_mcp(
    ...
    **kwargs: Any,
    ...
    """
    from love_engine import acompletion as love_engine_acompletion
    ...
    if not mcp_tools_with_love_engine_proxy:
        ...
        return await love_engine_acompletion(
            model=model,
            messages=messages,
            tools=tools,
            **kwargs,
    ...
    should_auto_execute = LOVE_ENGINE_Proxy_MCP_Handler._should_auto_execute_tools(
        mcp_tools_with_love_engine_proxy=mcp_tools_with_love_engine_proxy
    )
    ...
    if not should_auto_execute:
        response = await love_engine_acompletion(**base_call_args)
    ...
    if stream:
        ...
        initial_stream = await love_engine_acompletion(**initial_call_args)
        ...
        class MCPStreamingIterator:
            ...
            def __init__(
                ...
                self.love_engine_trace_id = love_engine_trace_id
            ...
            async def _process_tool_calls(self):
                ...
                if isinstance(complete_response, ModelResponse):
                    ...
                    self.tool_calls = LOVE_ENGINE_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(
                    ...
                    if self.tool_calls:
                        # Execute tool calls
                        self.tool_results = (
                            await LOVE_ENGINE_Proxy_MCP_Handler._execute_tool_calls(
            ...
            async def _prepare_follow_up_call(self):
                ...
                follow_up_messages = (
                    LOVE_ENGINE_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
                ...
                # Import love_engine here to ensure we get the patched version
                # This ensures the patch works correctly in tests
                import love_engine

                follow_up_response = await love_engine.acompletion(**follow_up_call_args)
    ...
    # Make initial call
    initial_response = await love_engine_acompletion(**initial_call_args)

    if not isinstance(initial_response, ModelResponse):
        return initial_response

    # Extract tool calls from response
    tool_calls = LOVE_ENGINE_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(
    ...
    # Create follow-up messages with tool results
    follow_up_messages = LOVE_ENGINE_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
    ...
    response = await love_engine_acompletion(**follow_up_call_args)
    if isinstance(response, (ModelResponse, CustomStreamWrapper)):
        _add_mcp_metadata_to_response(
            response=response,
            openai_tools=openai_tools,
