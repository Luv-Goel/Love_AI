...
from love_engine.proxy._experimental.mcp_server.utils import split_server_prefix_from_name
...
from love_engine.utils import Rules, function_setup
...
LOVE_ENGINE_PROXY_MCP_SERVER_URL = "love_engine_proxy"
LOVE_ENGINE_PROXY_MCP_SERVER_URL_PREFIX = f"{LOVE_ENGINE_PROXY_MCP_SERVER_URL}/mcp/"
...
class LOVE_ENGINE_Proxy_MCP_Handler:
    """
    Helper class with static methods for MCP integration with Responses API.

    This handles when a user passes mcp server_url="love_engine_proxy" in their tools.
    """

    @staticmethod
    def _should_use_love_engine_mcp_gateway(tools: Optional[Iterable[ToolParam]]) -> bool:
        """
        Returns True if any MCP tool should be handled via the love_engine proxy MCP gateway.
        This includes tools with server_url="love_engine_proxy" as well as URLs ending in /mcp/<name>.
        """
        if tools:
            for tool in tools:
                if isinstance(tool, dict) and tool.get("type") == "mcp":
                    ...
                    if isinstance(server_url, str) and server_url.startswith(
                        LOVE_ENGINE_PROXY_MCP_SERVER_URL
                    ):
                        return True
    ...
    def _parse_mcp_tools(
        ...
        """
        Parse tools and separate MCP tools with love_engine_proxy from other tools.

        Returns:
            Tuple of (mcp_tools_with_love_engine_proxy, other_tools)
        """
        mcp_tools_with_love_engine_proxy: List[ToolParam] = []
            ...
            for tool in tools:
                if isinstance(tool, dict) and tool.get("type") == "mcp":
                    ...
                    if isinstance(server_url, str) and server_url.startswith(
                        ...
                        mcp_tools_with_love_engine_proxy.append(tool)
                    elif isinstance(server_url, str):
                        # Also intercept URLs like http://localhost:4000/mcp/atlassian_test
                        # by rewriting them to the internal love_engine_proxy format.
                        m = _PROXY_MCP_PATH_RE.match(server_url)
                        if m:
                            rewritten = {
                                **tool,
                                "server_url": f"{LOVE_ENGINE_PROXY_MCP_SERVER_URL_PREFIX}{m.group(1)}",
                            }
                            mcp_tools_with_love_engine_proxy.append(rewritten)
                        else:
        ...
        return mcp_tools_with_love_engine_proxy, other_tools
    ...
    async def _apply_toolset_permissions(
        ...
        from love_engine.proxy._types import LOVE_ENGINE_ObjectPermissionTable
        ...
        try:
            ...
            if existing_op is not None:
                ...
                updated_op = LOVE_ENGINE_ObjectPermissionTable(
    ...
    async def _get_mcp_tools_from_manager(
        user_api_key_auth: Any,
        mcp_tools_with_love_engine_proxy: Optional[Iterable[ToolParam]],
        ...
        Args:
            ...
            mcp_tools_with_love_engine_proxy: ToolParam objects with server_url starting with "love_engine_proxy"
        ...
        if mcp_tools_with_love_engine_proxy:
            for _tool in mcp_tools_with_love_engine_proxy:
                # if user specifies servers as server_url: love_engine_proxy/mcp/zapier,github then return zapier,github
                server_url = (
                    _tool.get("server_url", "") if isinstance(_tool, dict) else ""
                )
                if isinstance(server_url, str) and server_url.startswith(
                    LOVE_ENGINE_PROXY_MCP_SERVER_URL_PREFIX
                ):
        ...
        if resolved_toolset_ids and user_api_key_auth is not None:
            user_api_key_auth = (
                await LOVE_ENGINE_Proxy_MCP_Handler._apply_toolset_permissions(
        ...
        tools = await _get_tools_from_mcp_servers(
            ...
            love_engine_trace_id=love_engine_trace_id,
        )
    ...
    def _filter_mcp_tools_by_allowed_tools(
        mcp_tools: List[MCPTool], mcp_tools_with_love_engine_proxy: List[ToolParam]
    ) -> List[MCPTool]:
        ...
        for tool_config in mcp_tools_with_love_engine_proxy:
    ...
    async def _process_mcp_tools_to_openai_format(
        ...
        Args:
            user_api_key_auth: User authentication info for access control
            mcp_tools_with_love_engine_proxy: ToolParam objects with server_url starting with "love_engine_proxy"
            love_engine_trace_id: Optional trace ID for linking list_mcp_tools spend logs to parent request
        ...
        (
            ...
            tool_server_map,
        ) = await LOVE_ENGINE_Proxy_MCP_Handler._process_mcp_tools_without_openai_transform(
            user_api_key_auth,
            mcp_tools_with_love_engine_proxy,
            love_engine_trace_id=love_engine_trace_id,
        )

        openai_tools = LOVE_ENGINE_Proxy_MCP_Handler._transform_mcp_tools_to_openai(
            deduplicated_mcp_tools
        )
    ...
    async def _process_mcp_tools_without_openai_transform(
        ...
        """
        if not mcp_tools_with_love_engine_proxy:
            return [], {}
        ...
        (
            mcp_tools_fetched,
            allowed_mcp_servers,
        ) = await LOVE_ENGINE_Proxy_MCP_Handler._get_mcp_tools_from_manager(
            user_api_key_auth=user_api_key_auth,
            mcp_tools_with_love_engine_proxy=mcp_tools_with_love_engine_proxy,
            love_engine_trace_id=love_engine_trace_id,
            mcp_auth_header=mcp_auth_header,
            mcp_server_auth_headers=mcp_server_auth_headers,
        )
        ...
        filtered_mcp_tools = (
            LOVE_ENGINE_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        ...
        (
            deduplicated_mcp_tools,
            tool_server_map,
        ) = LOVE_ENGINE_Proxy_MCP_Handler._deduplicate_mcp_tools(
    ...
    def _transform_mcp_tools_to_openai(
        ...
        from love_engine.experimental_mcp_client.tools import (
    ...
    def _should_auto_execute_tools(
        mcp_tools_with_love_engine_proxy: Union[List[Dict[str, Any]], List[ToolParam]],
    ) -> bool:
        ...
        """
        for tool in mcp_tools_with_love_engine_proxy:
            if isinstance(tool, dict):
        ...
        for tool_call in tool_calls:
            ...
            try:
                (
                    ...
                    tool_arguments,
                    tool_call_id,
                ) = LOVE_ENGINE_Proxy_MCP_Handler._extract_tool_call_details(tool_call)

                if not tool_name:
                    verbose_logger.warning(f"Tool call missing name: {tool_call}")
                    continue

                parsed_arguments = LOVE_ENGINE_Proxy_MCP_Handler._parse_tool_arguments(
                    tool_arguments
                )
                ...
                tool_logging_call_id = love_engine_call_id or str(uuid.uuid4())
                logging_request_data = {
                    ...
                    "love_engine_call_id": tool_logging_call_id,
                ...
                if love_engine_trace_id:
                    logging_request_data["love_engine_trace_id"] = love_engine_trace_id
                ...
                try:
                    love_engine_logging_obj, _ = function_setup(
                ...
                logging_request_data["love_engine_logging_obj"] = love_engine_logging_obj
                ...
                if love_engine_logging_obj:
                    love_engine_logging_obj.model_call_details["mcp_tool_call_metadata"] = (
                        standard_logging_mcp_tool_call
                    )
                    love_engine_logging_obj.model = f"MCP: {tool_name}"
                    love_engine_logging_obj.call_type = CallTypes.call_mcp_tool.value
                ...
                if love_engine_logging_obj:
                    try:
                        love_engine_logging_obj.post_call(original_response=result)
                        end_time = datetime.now()
                        await love_engine_logging_obj.async_post_mcp_tool_call_hook(
                ...
                result_text = LOVE_ENGINE_Proxy_MCP_Handler._parse_mcp_result(result)
                tool_results.append(
                    {
                        "tool_call_id": tool_call_id,
                        "result": result_text,
                ...
                await LOVE_ENGINE_Proxy_MCP_Handler._log_mcp_tool_failure(
                    proxy_logging_obj=proxy_logging_obj,
                    user_api_key_auth=user_api_key_auth,
    ...
    def _create_follow_up_messages_for_chat(
        ...
        from love_engine.utils import convert_list_message_to_dict
    ...
    def _create_mcp_streaming_response(
        ...
        model: str,
        all_tools: Optional[List[Any]],
        mcp_tools_with_love_engine_proxy: List[Any],
        ...
        **kwargs,
        ...
        """
        from love_engine.responses.mcp.mcp_streaming_iterator import (
            MCPEnhancedStreamingIterator,
        )

        # Build the complete request parameters by merging all sources
        request_params = LOVE_ENGINE_Proxy_MCP_Handler._build_request_params(
            input=input,
            model=model,
            all_tools=all_tools,
        ...
        return MCPEnhancedStreamingIterator(
            ...
            or kwargs.get("love_engine_metadata", {}).get("user_api_key_auth"),
    ...
    def _create_tool_execution_events(
        ...
        """
        from love_engine._uuid import uuid
        from love_engine.responses.mcp.mcp_streaming_iterator import create_mcp_call_events
    ...
    def _add_mcp_output_elements_to_response(
        ...
        from love_engine._uuid import uuid
        from love_engine.types.responses.main import GenericResponseOutputItem, OutputText
