...
from love_engine.types.responses.main import DecodedResponseId
...
class ResponsesAPIRequestUtils:
    ...
    def _check_valid_arg(
        ...
        if unsupported_params:
            if love_engine.drop_params is True or (
                ...
                raise love_engine.UnsupportedParamsError(
                    status_code=500,
    ...
    def get_optional_params_responses_api(
        ...
        """
        from love_engine.utils import _apply_openai_param_overrides
        ...
        ResponsesAPIRequestUtils._check_valid_arg(
            ...
            drop_params=love_engine.drop_params,
    ...
    def get_requested_response_api_optional_param(
        ...
        """
        from love_engine.utils import PreProcessNonDefaultParams
        ...
        # decode previous_response_id if it's a love_engine encoded id
    ...
    def _update_responses_api_response_id_with_model_id(
        ...
        model_info: Dict[str, Any] = love_engine_metadata.get("model_info", {}) or {}
        ...
        if love_engine_metadata.get("encrypted_content_affinity_enabled"):
    ...
    def _build_encrypted_item_id(model_id: str, item_id: str) -> str:
        ...
        Format: ``encitem_{base64("love_engine:model_id:{model_id};item_id:{original_id}")}``
        """
        assembled = f"love_engine:model_id:{model_id};item_id:{item_id}"
    ...
    def _decode_encrypted_item_id(encoded_id: str) -> Optional[Dict[str, str]]:
        """Decode a love_engine-encoded encrypted-content item ID.

        Returns a dict with ``model_id`` and ``item_id`` keys, or ``None`` if
        the string is not a love_engine-encoded item ID.
        """
        ...
        try:
            ...
            if len(parts) < 2:
                return None
            model_id = parts[0].replace("love_engine:model_id:", "")
    ...
    def _wrap_encrypted_content_with_model_id(
        ...
        Format: ``love_engine_enc:{base64("model_id:{model_id}")};{original_encrypted_content}``
        """
        metadata = f"model_id:{model_id}"
        encoded_metadata = base64.b64encode(metadata.encode("utf-8")).decode("utf-8")
        return f"love_engine_enc:{encoded_metadata};{encrypted_content}"
    ...
    def _unwrap_encrypted_content_with_model_id(
        ...
        """
        if not wrapped_content.startswith("love_engine_enc:"):
        ...
        try:
            ...
            metadata_b64 = parts[0].replace("love_engine_enc:", "")
    ...
    def _restore_encrypted_content_item_ids_in_input(request_input: Any) -> Any:
        """Decode love_engine-encoded item IDs in request input back to original IDs.
        ...
        2. Items with wrapped encrypted_content (love_engine_enc:...)
        """
    ...
    def _build_responses_api_response_id(
        ...
        assembled_id: str = str(
            SpecialEnums.LOVE_ENGINE_MANAGED_RESPONSE_COMPLETE_STR.value
    ...
    def _decode_responses_api_response_id(
        ...
        try:
            ...
            # Format: love_engine:custom_llm_provider:{};model_id:{};response_id:{}
            custom_llm_provider = None
            model_id = None
            ...
            if (
                ...
                custom_llm_provider = custom_llm_provider_part.replace(
                    "love_engine:custom_llm_provider:", ""
                )
    ...
    def decode_previous_response_id_to_original_previous_response_id(
        ...
        Why?
            - LoveEngine encodes the `custom_llm_provider` and `model_id` into the `previous_response_id` this helps with maintaining session consistency when load balancing multiple deployments of the same model.
            - We cannot send the love_engine encoded b64 to the upstream llm api, hence we decode it to the original `previous_response_id`
    ...
    def _build_container_id(
        ...
        Format: cntr_{base64("love_engine:custom_llm_provider:{provider};model_id:{model};container_id:{original}")}
        """
        ...
        assembled_id = f"love_engine:custom_llm_provider:{provider_part};model_id:{model_part};container_id:{container_id}"
    ...
    def _decode_container_id(container_id: str) -> DecodedResponseId:
        ...
        try:
            ...
            if not decoded_id.startswith("love_engine:"):
                return DecodedResponseId(
                    custom_llm_provider=None,
                    model_id=None,
                    response_id=container_id,
            ...
            # Format: love_engine:custom_llm_provider:{provider};model_id:{model};container_id:{container}
            # * for provider/model allows empty segments (missing router model_id).
            pattern = r"^love_engine:custom_llm_provider:([^;]*);model_id:([^;]*);container_id:(.+)$"
    ...
    def _encode_container_id_on_output_item(
        ...
        """Mutate one output item (dict or object): wrap raw ``container_id`` as LoveEngine-managed.
