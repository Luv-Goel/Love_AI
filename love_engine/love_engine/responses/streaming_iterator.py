...
from love_engine.types.llms.openai import ResponsesAPIStreamEvents
from love_engine.types.utils import CallTypes
from love_engine.utils import async_post_call_success_deployment_hook


@lru_cache(maxsize=1)
def _get_openai_response_types():
    from love_engine.types.llms import openai as openai_types
...
class BaseResponsesAPIStreamingIterator:
    ...
    def __init__(
        ...
        self.love_engine_metadata = love_engine_metadata
        self.custom_llm_provider = custom_llm_provider
        self.request_data: Dict[str, Any] = request_data or {}
        self.call_type: Optional[str] = call_type

        # set hidden params for response headers (e.g., x-love_engine-model-id)
        # This matches the stream wrapper in love_engine/love_engine_core_utils/streaming_handler.py
        ...
        _model_info: Dict = (
            love_engine_metadata.get("model_info", {}) if love_engine_metadata else {}
        )
        self._hidden_params = {
    ...
    def _check_max_streaming_duration(self) -> None:
        """Raise love_engine.Timeout if the stream has exceeded LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS."""
        if LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS is None:
            return
        elapsed = time.time() - self._stream_created_time
        if elapsed > LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS:
            raise love_engine.Timeout(
                message=f"Stream exceeded max streaming duration of {LOVE_ENGINE_MAX_STREAMING_DURATION_SECONDS}s (elapsed {elapsed:.1f}s)",
                model=self.model or "",
    ...
    def _process_chunk(self, chunk) -> Optional[Any]:
        ...
        try:
            ...
            if isinstance(parsed_chunk, dict):
                ...
                _stream_model_id = (
                    self.love_engine_metadata.get("model_info", {}).get("id")
                    if self.love_engine_metadata
                    else None
                )
                if _event_type in (
                ...
                if self.love_engine_metadata and self.love_engine_metadata.get(
                    ...
                    if event_type in (
                        ...
                        if item:
                            ...
                            if encrypted_content and isinstance(encrypted_content, str):
                                model_id = (
                                    self.love_engine_metadata.get("model_info", {}).get(
                                        "id"
                                    )
                                    if self.love_engine_metadata
                                    else None
    ...
    def _handle_logging_failed_response(self):
        ...
        exception = love_engine.APIError(
            status_code=500,
    ...
    def _persist_completed_response_to_cache(self, *, is_async: bool) -> None:
        ...
        if is_async:
            cache_write_task = asyncio.create_task(
                love_engine.cache.async_add_cache(
                    cached_response,
    ...
    async def _call_post_streaming_deployment_hook(self, chunk):
        ...
        try:
            ...
            callbacks = getattr(love_engine, "callbacks", None) or []
            hooks_ran = False
    ...
    def _run_post_success_hooks(self, end_time: datetime):
        ...
        try:
            ...
            pass
        if "love_engine_params" not in request_payload:
            try:
                request_payload["love_engine_params"] = getattr(
                    self.logging_obj, "model_call_details", {}
                ).get("love_engine_params", {})
            except Exception:
                request_payload["love_engine_params"] = {}
...
def _build_content_part_done_event(
    *,
    ...
    if part_type == "output_text":
        annotations = [
            openai_types.BaseLoveEngineOpenAIResponseObject(**annotation)
    ...
    for output_index, output_item in enumerate(
        ...
        events.append(
            openai_types.OutputItemAddedEvent(
                ...
                output_index=output_index,
                item=openai_types.BaseLoveEngineOpenAIResponseObject(
                    **output_item_payload
                ),
            )
        )
            ...
            for content_index, part in enumerate(
                ...
                events.append(
                    openai_types.ContentPartAddedEvent(
                        ...
                        output_index=output_index,
                        content_index=content_index,
                        part=openai_types.BaseLoveEngineOpenAIResponseObject(
                            **part_payload
                        ),
                    )
                )
...
from love_engine.love_engine_core_utils.thread_pool_executor import executor as _ws_executor
...
class ManagedResponsesWebSocketHandler:
    ...
    - Makes HTTP streaming calls via ``love_engine.aresponses(stream=True)``
    ...
    This makes every provider that LoveEngine can reach over HTTP available on
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.logging_obj = logging_obj
        self.user_api_key_dict = user_api_key_dict
        self.love_engine_metadata: Dict[str, Any] = love_engine_metadata or {}
        self.model_group: Optional[str] = self.love_engine_metadata.get(
            "model_group"
        ) or self.love_engine_metadata.get("deployment_model_name")
    ...
    def _get_history_messages(self, previous_response_id: str) -> List[Dict[str, Any]]:
        ...
        LoveEngine base64-encodes it into the ``resp_...`` format).
        """
    ...
    def _inject_credentials(
        ...
        # (e.g., connection is vertex_ai but event says openai/gpt-4), let love_engine
        ...
        if self.love_engine_metadata:
            call_kwargs["love_engine_metadata"] = dict(self.love_engine_metadata)
    ...
    def _update_proxy_request(call_kwargs: Dict[str, Any], model: str) -> None:
        ...
        proxy_server_request = (call_kwargs.get("love_engine_metadata") or {}).get(
        ...
        if "love_engine_metadata" not in call_kwargs:
            call_kwargs["love_engine_metadata"] = {}
        call_kwargs["love_engine_metadata"]["proxy_server_request"] = proxy_server_request
        call_kwargs.setdefault("love_engine_params", {})
        call_kwargs["love_engine_params"]["proxy_server_request"] = proxy_server_request

    async def _stream_and_forward(
        self, model: str, call_kwargs: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        Stream ``love_engine.aresponses`` and forward every chunk over the WebSocket.
        ...
        stream_response = await love_engine.aresponses(model=model, **call_kwargs)
    ...
    async def _process_response_create(self, raw_message: str) -> None:
        """
        Parse one ``response.create`` event, call ``love_engine.aresponses(stream=True)``,
        ...
        # love_engine.aresponses fails in get_llm_provider. A genuinely different
